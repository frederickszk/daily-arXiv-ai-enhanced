...
Motivation: 现有基于激活的可解释性方法存在数据集依赖、易受数据偏见影响、只能提供粗粒度解释等问题，需要更直接、更精确的模型内部机制分析方法。

Method: SITH框架直接分析CLIP视觉Transformer的权重空间，将每个注意力头的值输出矩阵分解为奇异向量，然后使用COMP算法将这些向量解释为稀疏、语义连贯的人类可理解概念组合。

Result: SITH能够产生连贯、忠实的内头解释，通过重建保真度和可解释性实验验证。支持精确、可解释的权重空间模型编辑，改善下游性能而无需重新训练。还能用于研究模型适应过程。

Conclusion: SITH提供了一种数据无关、训练无关的Transformer可解释性方法，能够深入理解模型内部语义机制，支持模型编辑和适应分析，为大规模视觉语言模型的内部机制理解提供了新工具。
...
TL;DR: ReDiPrune是一种无需训练、即插即用的视觉token剪枝方法，在视觉-语言投影器之前直接选择信息丰富的视觉token，在减少计算量的同时提升多模态大语言模型的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型计算成本高昂，因为Transformer需要处理大量视觉token。现有的投影后剪枝方法在压缩表示上操作，会丢失细粒度的空间和语义信息。

Method: 在视觉编码器输出和视觉-语言投影器之间插入ReDiPrune，使用轻量级规则对每个token进行评分，综合考虑文本条件相关性和最大-最小多样性，确保选择的token既与查询相关又不冗余。

Result: 在4个视频和5个图像基准测试中，ReDiPrune持续改善了准确性与效率的权衡。例如，在EgoSchema上使用LLaVA-NeXT-Video-7B模型，仅保留15%的视觉token就能获得+2.0%的绝对准确率提升，同时将TFLOPs计算量减少6倍以上。

Conclusion: ReDiPrune是一种有效的训练前视觉token剪枝方法，能够在保持甚至提升模型性能的同时显著减少计算开销，为多模态大语言模型的高效部署提供了实用解决方案。
...
TL;DR: AVControl：基于LTX-2音频视觉基础模型的轻量可扩展框架，通过并行画布和LoRA适配器实现多模态视频音频生成控制，无需修改模型架构


<details>
  <summary>Details</summary>
Motivation: 现有视频音频生成控制方法要么训练单一固定控制集的大型模型，要么为每个新模态引入昂贵的架构修改，缺乏灵活高效的扩展方案

Method: 基于LTX-2音频视觉基础模型，每个控制模态作为独立的LoRA适配器训练，通过并行画布提供参考信号作为注意力层的额外token，无需架构修改

Result: 在VACE基准测试中，在深度和姿态引导生成、修复、外绘方面优于所有基线，在相机控制和音频视觉基准上表现竞争性；支持多种独立训练模态，计算和数据效率高

Conclusion: AVControl提供了一种轻量、可扩展的框架，能够高效支持多种视频音频生成控制模态，每个模态仅需少量数据和训练步骤，优于传统单体模型方法
...
TL;DR: SGREC是一种可解释的零样本指代表达理解方法，通过查询驱动的场景图作为结构化中介，结合视觉语言模型和大型语言模型，在多个基准测试中取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）在零样本指代表达理解任务中难以捕捉细粒度视觉细节和复杂对象关系，而大型语言模型虽然擅长高层语义推理，但无法直接将视觉特征抽象为文本语义。需要一种方法能结合两者的优势。

Method: 提出SGREC方法：1）使用视觉语言模型构建查询驱动的场景图，显式编码空间关系、描述性标题和对象交互；2）利用场景图作为结构化中介，弥合低级图像区域与高级语义理解之间的鸿沟；3）使用大型语言模型从场景图提供的结构化文本表示中推断目标对象，并提供详细的决策解释。

Result: 在多个零样本REC基准测试中取得了最佳性能：RefCOCO val (66.78%)、RefCOCO+ testB (53.43%)、RefCOCOg val (73.28%)，展示了强大的视觉场景理解能力。

Conclusion: SGREC通过查询驱动的场景图作为结构化中介，有效结合了视觉语言模型和大型语言模型的优势，在零样本指代表达理解任务中实现了高性能和可解释性。
...
### [5] [SAVe: Self-Supervised Audio-visual Deepfake Detection Exploiting Visual Artifacts and Audio-visual Misalignment](https://arxiv.org/abs/2603.25140)
*Sahibzada Adil Shahzad,Ammarah Hashmi,Junichi Yamagishi,Yusuke Yasuda,Yu Tsao,Chia-Wen Lin,Yan-Tsung Peng,Hsin-Min Wang*

Main category: cs.CV

TL;DR: SAVe是一个自监督的音频-视觉深度伪造检测框架，通过在真实视频上生成伪篡改样本来学习检测多模态伪造，无需依赖合成伪造数据集。


<details>
  <summary>Details</summary>
Motivation: 当前多模态深度伪造检测主要依赖合成伪造数据集进行训练，这会导致数据集和生成器偏差，限制了模型的可扩展性和对未见篡改的鲁棒性。多模态伪造可能存在细微的视觉伪影和跨模态不一致性，这些难以检测。

Method: SAVe采用自监督学习，完全在真实视频上训练。它生成身份保持、区域感知的自混合伪篡改样本来模拟篡改伪影，使模型能够学习跨多个面部粒度的互补视觉线索。同时通过音频-视觉对齐组件建模唇语同步，检测音频-视觉伪造特有的时间错位模式。

Result: 在FakeAVCeleb和AV-LipSync-TIMIT数据集上的实验表明，SAVe在域内性能具有竞争力，并且表现出强大的跨数据集泛化能力。

Conclusion: 自监督学习是多模态深度伪造检测的可扩展范式，能够有效解决合成数据依赖带来的偏差问题，提高模型对未见篡改的检测能力。
...
TL;DR: BCMDA框架通过虚拟域桥接和原型对齐解决混合域半监督医学图像分割中的域偏移和标注有限问题，在三个公开数据集上表现出色，尤其在标注样本极少时表现优异。


<details>
  <summary>Details</summary>
Motivation: 混合域半监督医学图像分割面临两大挑战：1) 标注数据与未标注数据之间的分布差异阻碍有效知识迁移；2) 从未标注数据中学习效率低下导致严重的确认偏差。需要一种方法来解决域偏移和有限标注下的分割性能问题。

Method: 提出双向相关映射域适应框架：1) 通过虚拟域桥接知识迁移，利用双向相关映射合成标注和未标注图像，采用固定比例和渐进动态MixUp策略生成虚拟图像；2) 使用双向CutMix实现固定虚拟域内的初始知识迁移和从动态转换标注域到真实未标注域的渐进知识迁移；3) 采用原型对齐和伪标签校正，利用可学习的原型余弦相似度分类器进行虚拟域与真实域之间的双向原型对齐；4) 使用原型伪标签校正生成更可靠的伪标签。

Result: 在三个公开的多域数据集上进行实证评估，证明该方法具有优越性，特别是在标注样本非常有限的情况下表现出色。
...
TL;DR: CIV-DG：一种利用条件工具变量解决医学AI中站点选择偏倚的因果框架，通过放松标准IV方法的严格随机分配假设，在患者人口统计学驱动的医院选择场景中实现跨站点泛化。


<details>
  <summary>Details</summary>
Motivation: 医学AI的跨站点泛化受到选择偏倚的根本性损害，患者人口统计学特征（如年龄、严重程度）非随机地决定医院分配，导致站点特定变异与诊断标签之间的虚假相关性。传统的领域泛化方法主要针对图像级分布偏移，无法解决这种结构性混杂问题。

Method: 提出CIV-DG因果框架，利用条件工具变量分离病理语义与扫描仪引起的伪影。通过放松标准工具变量方法的严格随机分配假设，适应患者人口统计学内生驱动医院选择的复杂临床场景。采用深度广义矩方法架构，使用条件批评器最小化矩违规并在人口统计学层内强制执行工具-误差正交性。

Result: 在Camelyon17基准测试和大规模胸部X射线数据集上的广泛实验表明，CIV-DG显著优于领先的基线方法，验证了条件因果机制在解决医学AI结构性混杂问题中的有效性。

Conclusion: CIV-DG框架通过条件工具变量方法成功解决了医学AI中的结构性混杂问题，为在患者人口统计学驱动医院选择的复杂临床场景中实现稳健的跨站点泛化提供了有效解决方案。
...
Result: LLaVA-LE相比基础LLaVA模型实现了3.3倍的整体性能提升，相比第一阶段模型提升了2.1倍，推理得分达到1.070，超过了评判者自身的参考分数。

Conclusion: 领域特定的多模态数据和指令调优能够有效推进视觉语言模型在行星探索中的应用，为月球地形分析提供了强大的工具。
...
TL;DR: 论文提出室内场景中的相似物体检测任务，利用多视角图像区分物体对为相同、相似或不同，并开发了Lookalike3D模型和3DTwins数据集。


<details>
  <summary>Details</summary>
Motivation: 现有3D物体理解和生成方法忽视了真实场景中普遍存在的重复物体信息，这些重复和相似物体对提供了互补的视觉线索，可用于提升3D感知的一致性。

Method: 提出Lookalike3D多视角图像变换器，利用大型图像基础模型的强语义先验来有效区分物体对；构建了基于ScanNet++的3DTwins数据集，包含76k手动标注的物体对。

Result: 在3DTwins数据集上，相比基线方法获得了104%的IoU提升；展示了该方法能改进下游任务如联合3D物体重建和部件协同分割。

Conclusion: 重复和相似物体可以成为实现一致、高质量3D感知的强大线索，该方法为利用场景中的重复模式提供了有效解决方案。
...
TL;DR: 3DGS可用于精确几何测量，通过在不同视图中选取共轭点并进行三角测量，实现高精度点测量，性能优于传统立体测量和网格方法。


<details>
  <summary>Details</summary>
Motivation: 3DGS在实时渲染和新视角合成方面表现出色，但其在精确几何测量方面的潜力尚未充分开发。传统方法依赖立体工作站或在不完整网格上直接选取，存在局限性。

Method: 利用3DGS能够精确渲染源视图并平滑插值中间视图的特性，让用户在不同视图中直观选取共轭点，然后通过三角测量生成精确的3D点测量。该方法支持多视图交会以提高精度。

Result: 在无人机航拍数据集上，该方法实现了1-2厘米范围的RMSE精度，在薄结构上（0.037m）显著优于网格方法（0.062m），在尖锐角点上（0.013m）更是完全优于失败的网格方法。

Conclusion: 3DGS不仅可用于高质量渲染，还能实现高精度几何测量，为传统立体测量提供了更便捷、高效的替代方案，特别适用于复杂结构测量。
...
TL;DR: 提出了一种用于3D高斯泼溅的自监督置信度框架，通过动态平衡光度与几何监督，结合改进的外观模型，在保持高效率的同时实现了无边界网格提取的最先进结果。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术虽然能快速从姿态图像中提取网格，但在存在丰富视角依赖效应的场景中，网格提取仍然困难。现有方法依赖多视图技术、迭代网格提取或大型预训练模型，牺牲了3DGS固有的效率。
...
Conclusion: 提出的自监督置信度框架为3D高斯泼溅提供了一种简单高效的替代方案，能够有效解决视角依赖效应带来的模糊性问题，在保持计算效率的同时提升了网格提取质量。
...
TL;DR: 该研究使用语义模糊图像作为可解释性探针，通过CLIP嵌入空间中的概念插值生成连续模糊图像谱，比较人类和机器学习分类器在概念边界上的差异。


<details>
  <summary>Details</summary>
Motivation: 经典的鸭兔错觉表明，当视觉证据模糊时，人脑必须决定看到什么。研究旨在探索人类观察者和机器学习分类器在概念边界上的差异，以及如何利用模糊图像作为诊断工具来理解人类与模型的认知对齐。

Method: 提出了一个心理物理学框架，在CLIP嵌入空间中对概念进行插值，生成连续的模糊图像谱。通过这种方法精确测量人类和机器学习分类器放置语义边界的位置和方式。
...
TL;DR: OpenCap Monocular：基于单部智能手机视频估算3D骨骼运动学和肌肉骨骼力的算法，通过优化、物理模拟和机器学习实现，准确度优于传统方法，已部署为免费应用。


<details>
  <summary>Details</summary>
Motivation: 传统运动学和动力学分析需要昂贵的实验室设备，限制了临床转化。需要可扩展、准确的生物力学评估工具来改善移动相关疾病的预测、治疗和监测。

Method: 通过优化单目姿态估计模型（WHAM）的3D人体姿态估计，计算生物力学约束骨骼模型的运动学，并通过物理模拟和机器学习估算动力学。

Result: 验证显示低运动学误差（旋转自由度平均绝对误差4.8°，骨盆平移3.4 cm），优于基线方法48%旋转精度和69%平移精度。地面反作用力估算准确度与双摄像头系统相当或更好。

Conclusion: OpenCap Monocular能够以临床意义准确性估算重要动力学结果，已通过智能手机应用、网页应用和云端计算部署，实现免费、可访问的单智能手机生物力学评估。
...
TL;DR: TIGeR模型通过多模态Transformer将图像、地理位置和时间映射到统一的时空嵌入空间，支持灵活输入配置，在时空感知图像检索任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 数字取证、城市监测和环境分析等实际应用需要联合推理视觉外观、地理位置和时间信息，传统的地理定位和时间预测方法已无法满足复杂需求，如检索在特定目标时间与查询图像相同位置拍摄的图像。

Method: 提出TIGeR模型，基于多模态Transformer架构，将图像、地理位置和时间信息映射到统一的时空嵌入空间；构建包含450万训练样本和8.6万高质量评估样本的数据集；支持单模态和多模态查询配置。

Result: TIGeR在时间预测（季节预测提升16%，时间预测提升8%）和时空感知检索（召回率提升14%）方面显著优于现有方法；通过保持位置身份在视觉变化下的稳定性，实现基于场景时空而非纯视觉相似性的检索。

Conclusion: 统一的时空建模方法在复杂时空感知图像检索任务中具有显著优势，TIGeR模型通过联合学习视觉、地理和时间信息，为数字取证、环境监测等应用提供了更强大的分析能力。
...
TL;DR: 这篇综述论文比较了合成心脏MRI生成的各种方法，包括GANs、VAEs、扩散模型和流匹配技术，从保真度、实用性和隐私保护三个维度评估现有方法，强调需要集成评估驱动的框架来支持可靠的临床工作流程。


<details>
  <summary>Details</summary>
Motivation: 解决心脏MRI数据稀缺、标注不足、厂商差异以及隐私泄露风险等问题，通过合成数据生成来克服这些挑战，支持医学影像分析的发展。

Method: 系统综述和比较分析现有合成心脏MRI生成方法，包括GANs、VAEs、扩散概率模型和流匹配技术，特别关注掩码条件生成、扩散模型和流匹配模型在结构保真度和边界保持方面的优势。

Result: 研究发现掩码条件生成能提高结构保真度，扩散和流匹配模型能有效保持边界并提供确定性变换，跨域泛化通过厂商风格条件和强度归一化得到改善，隐私保护通过成员推断攻击、最近邻分析和差分隐私机制实现。

Conclusion: 合成心脏MRI生成在保真度、实用性和隐私保护方面取得进展，但仍需集成评估驱动的框架来确保临床工作流程的可靠性，解剖约束的合成数据能提高多厂商设置下的分割准确性和鲁棒性。
...
TL;DR: DRoPS：利用静态预扫描作为几何先验，通过表面对齐的高斯原语网格和CNN运动参数化，显著提升动态场景重建质量


<details>
  <summary>Details</summary>
Motivation: 现有方法从2D基础模型提取先验或手工正则化运动，但在极端新视角和高度关节运动场景下重建效果不佳。需要利用静态预扫描作为显式几何和外观先验来约束解空间

Method: 1. 建立网格结构表面对齐模型：将高斯原语组织成像素网格锚定在物体表面；2. 利用网格结构参数化运动：使用CNN以网格为条件参数化运动，注入强隐式正则化并关联相邻点运动

Result: 在渲染质量和3D跟踪精度方面显著优于当前最先进方法

Conclusion: DRoPS通过静态预扫描先验、表面对齐网格结构和CNN运动参数化，有效解决了动态场景重建中的几何一致性问题，在极端视角和关节运动场景下表现优异
...
TL;DR: 本文提出Calibri方法，通过引入单个学习缩放参数和进化算法优化DiT组件，仅修改约100个参数即可显著提升文本到图像生成质量并减少推理步骤


<details>
  <summary>Details</summary>
Motivation: 发现扩散变换器（DiTs）在生成任务中具有未开发的潜力，特别是通过分析去噪过程发现引入学习缩放参数可以显著提升DiT块的性能
...
Conclusion: Calibri是一种参数高效的方法，能够显著提升扩散变换器的生成质量，证明了通过精细校准DiT组件可以释放其隐藏潜力
...
TL;DR: 提出一种基于可解释AI（XAI）引导的编码器塑形框架，通过计算密集的归因编码器焦点图（XAI场），优化复杂腹主动脉瘤CT图像分割中模型对薄壁低对比度目标的关注


<details>
  <summary>Details</summary>
Motivation: 复杂腹主动脉瘤（AAA）的CT图像分割经常失败，因为模型会将注意力集中在无关结构上，或者无法关注薄壁、低对比度的目标区域

Method: 提出XAI引导的编码器塑形框架：1）从最终编码器块计算密集的归因编码器焦点图（XAI场）；2）将预测概率质量与XAI场对齐，促进焦点与输出的一致性；3）将XAI场输入轻量级细化路径和置信度先验，在推理时调节logits，抑制干扰同时保留细微结构

Result: 在临床验证的具有挑战性病例（专门为易失败场景筛选）上评估，相比基础SAM设置，该方法实现了显著改进

Conclusion: 通过XAI引导显式优化编码器焦点是复杂场景中实现可靠分割的实用有效原则
...
TL;DR: GoldiCLIP是一个基于"恰到好处"原则的视觉语言模型训练框架，仅用3000万图像（比主流方法少300倍）就实现了最先进的性能，通过三种创新技术平衡监督信号。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型(VLMs)的成功通常依赖于数十亿样本数据集，这构成了研究进展的重大障碍。虽然最近的工作通过提高监督质量开始缩小这一差距，但每个方法只解决了对比预训练中的部分弱点。

Method: GoldiCLIP采用多方面的训练框架，协同结合三种关键技术：(1)文本条件自蒸馏方法，对齐文本无关和文本条件特征；(2)集成解码器的编码器，使用视觉问答(VQA)目标，使编码器能够超越类似标题的查询进行泛化；(3)基于不确定性的加权机制，自动平衡所有异构损失。

Result: 在仅使用3000万图像（比领先方法少300倍数据）的情况下，GoldiCLIP在数据高效方法中实现了最先进性能：在MSCOCO检索上比最佳可比基线提高2.2分，细粒度检索提高2.0分，基于问题的检索提高5.9分，同时与数十亿规模模型保持竞争力。

Conclusion: GoldiCLIP框架通过找到监督信号的恰当平衡，显著减少了视觉语言模型训练所需的数据量，同时实现了优异的性能，为数据高效的多模态学习提供了有前景的方向。
...
TL;DR: 本文提出一种基于深度学习的针道感染分类方法，通过注意力机制和高效冗余重建卷积技术，在针道图像数据集上实现了高精度分类，AUC达0.975。


<details>
  <summary>Details</summary>
Motivation: 针道是外固定器与骨骼连接的接口，容易发生感染，导致患者疼痛和并发症增加。目前缺乏基于视觉特征的针道感染自动识别方法，需要开发能够准确分类针道感染状态的深度学习系统。

Method: 收集针道伤口感染数据集，提出基于注意力的深度学习模型，强调相关区域并减少针体干扰。引入高效冗余重建卷积（ERRC）方法增强特征图丰富性同时减少参数数量。将针道图像分为两组：A组显示炎症或感染迹象，B组无明显并发症。

Result: 模型在基准方法中表现优异，AUC达到0.975，F1分数0.927，仅需5.77M参数。模型能够基于视觉感染迹象区分针道状态，与医疗专业人员评估一致。

Conclusion: 深度学习在基于视觉感染迹象区分针道状态方面具有潜力，与医疗专业人员评估相符。虽然需要更多数据进行验证，但该方法为针道感染的自动识别和管理提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击研究主要关注密度图模型之间的可迁移性，但跨范式攻击（密度图与点回归模型之间）尚未探索。考虑到人群计数在安全领域的重要性，研究模型鲁棒性对抗攻击具有重要意义。

Method: 采用综合多任务损失优化框架：对点回归模型使用场景密度特定的高置信度logit抑制；对密度图方法使用峰值目标密度图抑制；同时结合模型无关的感知约束确保扰动有效且人眼难以察觉。

Result: 攻击效果显著，平均使MAE增加7倍，同时保持良好视觉质量；成功迁移到7个最先进的人群模型，迁移比在0.55-1.69之间；在攻击效果和不可察觉性之间取得良好平衡。

Conclusion: 首次实现了跨范式对抗攻击，证明了密度图和点回归模型在对抗攻击下的脆弱性，为人群计数模型的鲁棒性评估提供了新视角。
...
Method: 提出DCARL分治自回归框架：1）关键帧生成器（无时间压缩训练）建立长距离全局一致的结构锚点；2）插值生成器以自回归方式合成密集帧，使用重叠片段，利用关键帧提供全局上下文，单个干净的前一帧确保局部连贯性。

Result: 在大规模互联网长轨迹视频数据集上训练，在视觉质量（更低的FID和FVD）和相机一致性（更低的ATE和ARE）方面优于最先进的自回归和分治基线方法，实现了长达32秒的稳定高质量视频生成。

Conclusion: DCARL框架成功结合了分治方案的结构稳定性和视频扩散模型的高保真生成能力，有效解决了长轨迹视频生成中的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统立体匹配方法依赖成本体积，计算复杂且效率低。本文旨在证明无需成本体积也能实现高性能立体匹配，通过更高效的扭曲操作替代成本体积。

Method: 提出WAFT-Stereo方法，采用基于扭曲的立体匹配框架，避免了传统方法中常见的成本体积设计，简化了计算流程。

Result: 在ETH3D、KITTI和Middlebury公开基准测试中排名第一，在ETH3D基准上零样本误差降低81%，同时比竞争方法快1.8-6.7倍。

Conclusion: 成本体积并非立体匹配的必要组件，基于扭曲的方法可以实现更强的性能和更高的效率，为立体匹配领域提供了新的设计思路。
...
TL;DR: 该研究对20种前沿多模态大语言模型在神经影像分析中的表现进行了全面基准测试，评估了它们在诊断分类、校准、结构化输出和计算效率等方面的性能。
...
Method: 使用精心策划的MRI和CT数据集，涵盖多发性硬化、中风、脑肿瘤等疾病和正常对照，要求模型同时生成诊断、亚型、成像模态、序列和解剖平面等多重输出，通过多阶段框架确保公平比较并控制选择偏差。

Result: 技术成像属性（如模态和平面）基本解决，但诊断推理特别是亚型预测仍具挑战性；肿瘤分类最可靠，中风中等可解，多发性硬化和罕见异常仍困难；少样本提示可提升性能但增加计算成本；Gemini-2.5-Pro和GPT-5-Chat诊断性能最强，Gemini-2.5-Flash效率最佳；开源模型MedGemma-1.5-4B表现最有前景。

Conclusion: 该研究为多模态大语言模型在神经影像中的性能、可靠性和效率权衡提供了实用见解，支持该领域的标准化评估，有助于推动临床应用的合理选择。
...
TL;DR: CORA是一个用于心血管风险评估的3D视觉基础模型，通过病理中心、合成驱动的自监督框架从CCTA数据中学习，显著提升了斑块特征识别、狭窄检测和冠状动脉分割的性能，并能改善30天主要不良心脏事件的风险分层。
...
Method: 提出CORA模型，采用病理中心、合成驱动的自监督框架，通过解剖引导的病变合成引擎，训练模型检测模拟血管异常，使表示学习偏向临床相关疾病特征而非背景解剖结构。在12,801个未标注CCTA体积上训练，并与大型语言模型结合形成多模态框架。

Result: 在9家独立医院的多中心数据集评估中，CORA在斑块特征识别、狭窄检测和冠状动脉分割等诊断和解剖任务上，持续优于最先进的3D视觉基础模型，性能提升最高达29%。多模态框架显著改善了30天主要不良心脏事件的风险分层。

Conclusion: CORA建立了一个可扩展、可扩展的基础，用于统一的解剖评估和心血管风险预测，为临床转化提供了有前景的解决方案。
...
TL;DR: 该研究开发了烟雾探测器自动检测系统，比较了YOLOv11、SSD和RT-DETRv2等目标检测模型在烟雾探测器识别任务上的性能，YOLOv11n表现最佳，mAP@0.5达到0.884


<details>
  <summary>Details</summary>
Motivation: 烟雾探测器检查通常因安装位置高或难以接近而困难，需要自动检测系统来加快检查速度、避免高空作业危险并降低成本

Method: 比较了YOLOv11、SSD和RT-DETRv2三种目标检测模型，使用真实和半合成数据进行训练，并应用多种数据增强方法，在两个具有挑战性的测试集上评估性能

Result: YOLOv11n表现最佳，平均mAP@0.5达到0.884，代码、预训练模型和数据集已公开

Conclusion: 提出的烟雾探测器识别系统可集成到无人机系统中，实现自动检查，YOLOv11n是最适合该任务的检测器
...
TL;DR: OptiSAR-Net++是一个用于跨域遥感视觉定位的新框架，通过PL-MoE实现高效跨域特征解耦，采用CLIP对比范式替代Transformer解码，结合动态对抗负采样、文本引导双门融合模块和区域感知辅助头，在OptSAR-RSVG和DIOR-RSVG基准上达到SOTA性能。
...
Method: 提出OptiSAR-Net++框架：1) 补丁级低秩适应专家混合(PL-MoE)用于高效跨域特征解耦；2) 采用CLIP对比范式结合动态对抗负采样，将生成回归转化为高效跨模态匹配；3) 文本引导双门融合模块(TGDF-SSA)增强语义-视觉对齐；4) 区域感知辅助头改进空间建模。

Result: 在OptSAR-RSVG（首个大规模跨域基准数据集）和DIOR-RSVG基准上达到最先进性能，在定位精度和效率方面具有显著优势。

Conclusion: OptiSAR-Net++有效解决了跨域遥感视觉定位的挑战，通过创新的跨域特征建模、高效计算架构和细粒度语义对齐机制，为实际应用提供了可行解决方案。代码和数据集将公开。
...
<details>
  <summary>Details</summary>
Motivation: 准确的手术阶段识别对于分析手术流程、支持术中决策以及实现数据驱动的手术教育和性能评估改进至关重要。当前面临标记数据有限和手术案例变异性的挑战。

Method: 1. 开发协作式在线平台供外科医生上传手术视频、接收自动阶段分析并贡献数据；2. 在251个未标记垂体瘤手术视频上使用自监督框架预训练ResNet-50模型；3. 在81个标记手术数据集上使用改进的训练方案进行微调，包括焦点损失、渐进层解冻和动态采样以处理类别不平衡和手术变异性。

Result: 在保留测试集上达到90%的准确率，优于当前最先进方法，并在不同手术案例中表现出强大的泛化能力。

Conclusion: 该框架通过结合自监督学习、鲁棒时序建模和可扩展标注策略，成功实现了垂体瘤手术视频的准确阶段识别，同时通过协作平台促进了大规模数据收集和知识共享。
...
<details>
  <summary>Details</summary>
Motivation: 评估自监督学习相比ImageNet预训练初始化在膝关节骨关节炎诊断和预后建模中的改进效果，特别关注多模态图像-文本自监督学习在非精选医院数据上的表现

Method: 比较两种自监督学习方法：(1)在OAI、MOST和NYU队列的膝关节X光片上进行的图像自监督学习；(2)在非精选医院膝关节X光片与放射科医生印象配对上进行的多模态图像-文本自监督学习。评估诊断任务（KL分级预测）和预后任务（4年结构发生和进展预测）的性能

Result: 诊断任务中自监督学习表现不一：图像自监督学习在线性探测时有所改进，但在完全微调时未超越ImageNet预训练；多模态自监督学习也未改善分级性能。预后任务中多模态初始化显著改进：在预测4年结构发生和进展方面优于ImageNet基线，在外部验证中表现优异（MOST AUROC：0.701 vs. 0.599，10%标记数据）

Conclusion: 非精选医院图像-文本数据由于严重性偏差（93%估计KL等级3）可能不适合诊断学习，但当下游任务与预训练数据分布一致时，为预后建模提供了强信号
...
TL;DR: 该研究提出了一个大规模的真实世界物体偏振反射和材料数据集，使用配备交叉/平行偏振的8相机346光源光舞台采集，包含218个日常物体，超过120万张高分辨率图像，用于改进逆渲染和正向渲染模型。


<details>
  <summary>Details</summary>
Motivation: 准确建模真实世界材料如何反射光线是逆渲染的核心挑战，主要原因是缺乏真实的测量反射数据。现有方法严重依赖合成数据集，这些数据集光照简化且材料真实感有限，导致模型难以泛化到真实世界图像。

Method: 使用配备交叉/平行偏振的8相机346光源光舞台采集大规模真实世界物体数据集，涵盖218个日常物体，包含多视角、多光照、偏振、反射分离和材料属性五个采集维度，产生超过120万张高分辨率图像，并分析得到漫反射反照率、镜面反射反照率和表面法线。

Result: 使用该数据集训练和评估了最先进的逆渲染和正向渲染模型，在固有分解、重光照和稀疏视图3D重建方面展示了材料分离、光照保真度和几何一致性的显著改进。

Conclusion: 该工作为基于物理的材料理解建立了新的基础，并能够实现超越合成训练机制的真实世界泛化能力。
...
Method: 提出两阶段框架：第一阶段使用带轨迹交互图（TIG）模块的CFM预测器建模细粒度视觉空间交互；第二阶段通过Flow-GRPO后训练，将确定性流展开重构为随机ODE-to-SDE采样，结合视图感知的社会合规性和地图感知的物理可行性复合奖励，引导多模态预测
...
TL;DR: 提出一个生成式框架，用于预测任意长度视频中的无限视野原始注视轨迹，使用自回归扩散模型合成具有连续空间坐标和高分辨率时间戳的注视轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统显著性图和扫描路径方法无法捕捉原始注视的细粒度时间动态，且现有模型通常局限于短时间窗口（约3-5秒），无法捕获真实世界内容中固有的长程行为依赖关系。

Method: 采用生成式框架，利用自回归扩散模型合成注视轨迹，模型基于显著性感知的视觉潜在空间进行条件生成。

Result: 定量和定性评估表明，该方法在长程时空准确性和轨迹真实性方面显著优于现有方法。

Conclusion: 提出的生成式框架能够有效预测任意长度视频中的无限视野原始注视轨迹，解决了传统方法在时间动态和长程依赖方面的局限性。
...
TL;DR: TIES框架通过动态平衡注意力大小与层间排名一致性，在减少78%视觉token的同时提升机器人操作成功率6%，无需额外训练


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在机器人操作中面临高推理延迟问题，主要原因是处理密集视觉token。现有token缩减方法主要依赖注意力大小作为静态选择标准，但研究发现高注意力token是任务依赖的，甚至可能降低策略性能。

Method: 提出TIES框架，基于层间token排名一致性的动态选择方法。通过自适应平衡注意力大小与排名一致性，确保鲁棒的token选择，无需额外训练。

Result: 在CogACT + SIMPLER基准测试中，TIES将平均成功率提升6%，同时减少78%的token使用量，并在不同解码器和基准测试中展现出强大的泛化能力。

Conclusion: TIES框架通过动态token选择机制，有效解决了VLA模型推理延迟问题，在保持甚至提升性能的同时大幅减少计算开销，为高效机器人操作提供了新思路。
...
TL;DR: xLARD是一个自校正的文本到图像生成框架，使用多模态大语言模型通过可解释的潜在奖励来指导生成过程，解决了复杂提示中细粒度语义和空间关系对齐的挑战。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成在复杂提示对齐方面存在困难，特别是细粒度语义和空间关系。这是因为前馈生成需要在完全理解输出之前就预测对齐，而评估生成图像相对更容易。基于这种不对称性，作者提出了自校正框架。

Method: 提出xLARD框架，使用多模态大语言模型通过可解释的潜在奖励指导生成。引入轻量级校正器，基于模型生成参考的结构化反馈来精炼潜在表示。关键组件是将潜在编辑映射到可解释奖励信号的可微分映射，使得不可微分的图像级评估能够提供连续的潜在级指导。

Result: 在多样化的生成和编辑任务实验中，xLARD提高了语义对齐和视觉保真度，同时保持了生成先验。该方法在复杂提示对齐方面表现出色。

Conclusion: xLARD通过自校正机制使模型能够在生成过程中理解、评估和校正自身，解决了文本到图像生成中复杂提示对齐的挑战，为生成模型提供了更可靠的指导框架。
...
TL;DR: PASDiff是一种无需训练的物理感知语义扩散方法，用于解决真实世界低光照人脸图像的多重退化问题，通过逆强度加权和Retinex理论恢复自然光照和色彩，同时通过风格无关结构注入保留人脸细节。


<details>
  <summary>Details</summary>
Motivation: 真实世界低光照人脸图像存在多重退化问题（低光照、模糊、噪声、低可见度等），现有级联解决方案存在严重误差累积问题，而通用联合模型缺乏明确的人脸先验，难以恢复清晰的人脸结构。

Method: 提出PASDiff（物理感知语义扩散）方法：1）利用逆强度加权和Retinex理论引入光度约束，可靠恢复可见度和自然色彩；2）提出风格无关结构注入（SASI）技术，从现成的人脸先验中提取结构同时过滤其固有光度偏差，将身份特征与物理约束无缝融合；3）构建WildDark-Face基准数据集（700张真实世界低光照人脸图像）。

Result: 大量实验表明，PASDiff显著优于现有方法，在自然光照、色彩恢复和身份一致性之间实现了优越的平衡。

Conclusion: PASDiff通过物理感知的语义扩散方法，有效解决了真实世界低光照人脸图像的多重退化问题，实现了高质量的人脸恢复效果。
...
TL;DR: 提出MoE-GRPO框架，使用强化学习优化视觉语言模型中专家路由选择，替代传统top-K机制，提升专家选择多样性并缓解专家过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型中广泛使用的确定性top-K专家路由机制可能忽略更优的专家组合，导致专家过拟合问题，需要提升专家选择的多样性。

Method: 提出MoE-GRPO框架：1) 将专家选择建模为序列决策问题，使用Group Relative Policy Optimization进行优化；2) 引入模态感知的路由指导，通过抑制不常激活的专家探索来增强训练稳定性。

Result: 在多模态图像和视频基准测试中，MoE-GRPO持续优于标准top-K路由及其变体，通过促进更丰富的专家选择来缓解专家过拟合，实现任务级专家专业化。

Conclusion: MoE-GRPO框架通过强化学习优化专家路由，有效提升了视觉语言模型中专家选择的多样性，解决了传统top-K路由的局限性，为多模态理解提供了更高效的专家组合方案。
...
TL;DR: 该研究提出了一种基于元学习的少样本3D左心房壁分割方法，通过多任务元训练和边界感知损失函数，在仅有5-20个标注样本的情况下实现了较好的分割性能，并在未见域偏移中表现出鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 左心房壁的LGE-MRI分割面临三大挑战：壁结构薄、对比度低、专家标注稀缺。传统方法在少样本情况下难以获得准确的分割结果，限制了临床应用中左心房重构的评估。

Method: 采用模型无关元学习框架，在左心房壁分割任务上进行元训练，同时结合左心房和右心房腔体作为辅助任务。使用边界感知复合损失函数来强调薄壁结构的准确性，支持K-shot学习（K=5,10,20）。

Result: 在保留测试集上，MAML在5-shot时Dice分数达到0.64（vs. 0.52），HD95为5.70mm（vs. 7.60mm），优于监督微调模型。在20-shot时接近全监督参考（0.69 vs. 0.71 DSC）。在未见域偏移和本地队列中仍保持鲁棒性，5-shot时DSC分别为0.59和0.57。

Conclusion: 该方法在少样本情况下实现了更准确可靠的薄壁边界分割，性能随样本数增加而提升，在未见域偏移中表现出鲁棒性，有望在临床应用中减少额外标注需求，促进左心房重构评估的临床转化。
...
TL;DR: 本文首次建立了基于事件流的视频异常检测统一研究框架，提出了EWAD框架，通过事件密度感知采样、密度调制时序建模和RGB到事件的知识蒸馏，在三个新构建的基准数据集上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 事件视觉具有低冗余、关注动态运动和固有隐私保护特性，天然适合视频异常检测需求。然而，缺乏专门的事件流异常检测数据集和有效建模策略严重阻碍了该领域进展。

Method: 提出EWAD框架，包含三个关键创新：1) 事件密度感知动态采样策略，选择时间信息丰富的片段；2) 密度调制时序建模方法，从稀疏事件流中捕获上下文关系；3) RGB到事件的知识蒸馏机制，在弱监督下增强基于事件的表示。

Result: 在三个新构建的基准数据集上进行广泛实验，EWAD相比现有方法取得了显著改进，突显了事件驱动建模在视频异常检测中的潜力和有效性。

Conclusion: 本文首次将基于事件的视频异常检测确立为统一研究方向，通过构建基准数据集和提出EWAD框架，展示了事件驱动建模在视频异常检测中的巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 3D LGE-MRI中左心房壁分割面临挑战：壁层薄、解剖结构复杂、对比度低，导致分割精度不足，影响壁厚映射和纤维化定量分析。

Method: 两阶段迁移学习框架：第一阶段使用3D U-Net（ResNeXt编码器+实例归一化）预训练左心房腔室分割，学习稳健的心房表示；第二阶段通过渐进式层解冻策略迁移权重，适应左心房壁分割任务，保留心内膜特征的同时实现壁层特异性细化。

Result: 在2018 LA分割挑战数据集上显著提升：壁层Dice从0.623提升至0.814，1mm表面Dice从0.553提升至0.731；边界误差大幅降低：HD95从2.95mm降至2.55mm，ASSD从0.71mm降至0.63mm；即使在70个训练样本的有限监督下，仍能达到Dice 0.78和HD95 3.15mm的竞争性性能。

Conclusion: 基于解剖学基础的任务迁移与受控微调能显著提高3D LGE-MRI中薄壁左心房壁分割的边界精度，为壁厚映射和纤维化分析提供更可靠的工具。
...
TL;DR: 本文提出了一种在4D场景中保持高斯局部几何结构的新方法，通过视图空间射线分组策略聚类被同一射线相交的高斯，并对这些组施加约束以保持空间分布一致性，从而获得更物理合理的运动模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的动态3D场景重建方法在建模真实运动方面存在挑战，大多数方法无法将高斯的运动与现实世界的物理动力学对齐。这种不对齐问题在单目视频数据集中尤为严重，无法保持连贯运动会破坏局部几何结构，最终导致重建质量下降。现有方法严重依赖光流或2D轨迹等外部先验来强制时间一致性。

Method: 提出了一种新颖的方法来明确保持4D场景中高斯在时间上的局部几何结构。核心思想是引入视图空间射线分组策略，该策略聚类被同一射线相交的高斯，只考虑那些α混合权重超过阈值的高斯。然后对这些组施加约束以保持一致的空间分布，有效保持其局部几何结构。这种方法通过确保局部几何随时间保持稳定来强制更物理合理的运动模型，消除了对外部指导的依赖。

Result: 将该方法集成到两个不同的基线模型中，在具有挑战性的单目数据集上进行广泛实验，结果表明该方法显著优于现有方法，实现了卓越的时间一致性和重建质量。

Conclusion: 提出的视图空间射线分组策略和局部几何约束方法能够有效保持4D场景中高斯的局部几何结构，实现更物理合理的运动模型，无需依赖外部先验，在时间一致性和重建质量方面均优于现有方法。
...
TL;DR: 提出了一种分布式车辆控制方法，用于紧急车辆快速通行，仅使用局部信息实现实时决策，无需预训练，且具有自然适应性


<details>
  <summary>Details</summary>
Motivation: 现有方法（集中式数学求解器和强化学习）存在两个根本限制：高计算成本和缺乏可扩展性。集中式方法仅适用于小规模场景，强化学习方法训练模型对不同交通条件扩展性有限

Method: 提出可扩展的分布式车辆控制方法，车辆仅使用局部信息（而非全局信息）在线分布式调整驾驶行为。证明仅使用局部信息的分布式方法近似等价于使用全局信息的方法。进一步提出分布式冲突解决机制，通过避免决策冲突保证车辆安全

Result: 基于真实世界交通数据集的仿真实验表明，与现有方法相比，该方法决策更快、对普通车辆影响更小，在不同交通密度和道路配置下保持更强的可扩展性

Conclusion: 该方法克服了现有方法的高计算成本和缺乏可扩展性问题，消除了集中式方法的单点故障风险，提供了学习型方法无法提供的确定性安全保证
...
<details>
  <summary>Details</summary>
Motivation: 解决在稀疏输入视图下3D重建的稳定性和质量问题，同时保持高效的渲染速度

Method: 结合TensorRF的基于张量的高效表示和FreeNeRF的频率驱动少样本正则化，引入频率和遮挡掩码来提升重建稳定性

Result: 在Synthesis NeRF基准上，PSNR从21.45dB提升到23.70dB，精细调优版本达到24.52dB；在THuman 2.0数据集上，仅用8张输入图像达到27.37-34.00dB

Conclusion: Few TensoRF是一种高效且数据有效的实时3D重建解决方案，适用于多样化场景
...
TL;DR: 提出GDPO-Listener框架，通过自回归流匹配架构和分组奖励解耦策略优化，解决对话中听众头部运动生成中的"回归到均值"问题，实现高度表达性的说话和倾听动作生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成虚拟人对话中的头部运动时存在"回归到均值"问题，听众动作容易塌陷为静态面部，缺乏复杂非语言动作的参数空间，难以生成真实自然的对话交互。

Method: 1. 引入自回归流匹配架构实现稳定监督学习；2. 应用分组奖励解耦策略优化，通过分离不同FLAME参数组的奖励归一化，明确激励高方差表达性生成；3. 实现显式语义文本控制以支持可定制的响应。

Result: 在Seamless Interaction和DualTalk数据集上的广泛评估表明，该方法在长期运动方差、视觉表达性和语义可控性方面优于现有基线方法。

Conclusion: GDPO-Listener框架成功解决了对话中听众头部运动生成的"回归到均值"问题，实现了高度表达性的说话和倾听动作生成，并提供了语义可控性，为虚拟人合成中的对话交互提供了有效解决方案。
...
TL;DR: VideoTIR：基于强化学习的多模态大语言模型长视频理解方法，通过工具调用策略优化减少幻觉，提升视频理解准确性和效率


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在长视频理解中存在幻觉问题，主要原因是文本和视觉token不平衡。虽然SFT-based工具调用方法可以缓解此问题，但需要大量高质量数据且工具调用轨迹受限

Method: 提出VideoTIR，利用强化学习鼓励正确使用多层次工具包；采用Zero-RL和SFT冷启动；提出TAGPO（工具包动作分组策略优化）减少冗余工具调用；开发基于沙箱的轨迹合成框架生成高质量轨迹数据

Result: 在三个长视频QA基准测试上的广泛实验证明了该方法的有效性和效率

Conclusion: VideoTIR通过强化学习驱动的工具调用策略，显著提升了MLLMs在长视频理解中的准确性和效率，有效缓解了幻觉问题
...
TL;DR: CARE是一个无需训练的可控医学图像修复框架，通过双潜在分支策略和风险感知自适应控制器，在保持结构保真度和利用生成先验之间实现平衡。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像修复方法通常需要针对特定任务重新训练，且难以控制忠实重建与先验驱动增强之间的权衡。过度激进的修复可能引入幻觉细节或改变诊断重要结构，这在临床环境中尤其成问题。
...
Conclusion: CARE为更安全、更可控、更易于部署的医学图像修复提供了实用步骤，通过训练自由的可控框架解决了临床环境中修复与保真度平衡的关键问题。
...
<details>
  <summary>Details</summary>
Motivation: 卫星地球观测积累了海量时空数据，但现有数据以离散栅格文件形式存储，导致存储、传输和查询成本高昂，需要更高效的数据表示方法。
...
TL;DR: MoRGS是一个高效的在线4D重建框架，通过显式建模每个高斯点的运动来提升动态场景重建质量，使用稀疏光流作为运动监督，并引入运动置信度分离动态和静态区域。


<details>
  <summary>Details</summary>
Motivation: 现有在线4D重建方法虽然高效且视觉质量好，但无法学习反映真实场景动态的每个高斯点运动。由于缺乏显式运动线索，外观和运动仅通过光度损失优化，导致每个高斯点运动追逐像素残差而非真实3D运动。

Method: 提出MoRGS框架：1) 利用稀疏关键视图上的光流作为轻量级运动线索，超越光度监督来正则化每个高斯点运动；2) 学习每个高斯点运动偏移场，协调投影3D运动与跨视图和时间的观测流之间的差异；3) 引入每个高斯点运动置信度，分离动态和静态高斯点，并加权高斯属性残差更新。

Result: 大量实验表明，MoRGS在在线方法中实现了最先进的重建质量和运动保真度，同时保持了流式处理性能。

Conclusion: MoRGS通过显式建模每个高斯点运动并利用稀疏光流监督，有效解决了现有在线4D重建方法中运动学习不足的问题，在保持实时性能的同时显著提升了动态场景重建质量。
...
TL;DR: GaussFusion通过几何感知的视频生成改进3D高斯泼溅重建，减少浮游物、闪烁和模糊等常见伪影，支持优化式和前馈式方法，在实时21FPS下实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决3D高斯泼溅(3DGS)在真实场景中常见的伪影问题，包括浮游物、闪烁和模糊，这些问题源于相机姿态误差、不完全覆盖和噪声几何初始化。

Method: 引入几何感知的视频到视频生成器，渲染包含深度、法线、不透明度和协方差的高斯基元视频缓冲区，通过生成器产生时间相干且无伪影的帧；还开发了伪影合成管道模拟多种退化模式。

Result: 在新视角合成基准测试中达到最先进性能，高效变体以21FPS实时运行并保持相似性能，支持交互式3D应用。

Conclusion: GaussFusion通过几何感知的视频生成有效改善了3DGS重建质量，减少伪影并实现实时性能，为交互式3D应用提供了实用解决方案。
...
Method: 开发了闭环事件-SVE测量系统：1) SVE分支通过烟雾感知融合策略生成HDR图；2) 使用多线索烟雾似然图分离粒子发射和烟雾散射；3) 利用HDR图作为事件相机的绝对强度参考，抑制烟雾驱动的事件伪影；4) 基于清洁的事件观测，通过立体事件3D管道估计分离高度和等效粒子尺寸。

Result: 系统最大校准误差0.56%，在硼基推进剂实验中获得了多模态等效半径统计，能够捕捉传统传感器难以观测的快速分离瞬态现象。

Conclusion: 该框架为烟雾遮挡下高动态范围条件下的微秒级分辨率3D燃烧测量提供了一条实用且校准一致的途径。
...
TL;DR: 提出一种从单目视频进行高质量动态高斯溅射的方法，通过SE(3)B样条运动基函数显式建模高斯粒子的连续位置和方向变形，采用自适应控制机制优化计算效率，并结合软分割重建策略和多视图扩散模型提升效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单目视频进行动态高斯溅射时，对复杂运动的建模能力有限，容易受到长时间运动干扰，且容易过拟合训练视图。需要一种能显式建模连续变形、提高计算效率并增强泛化能力的方法。

Method: 1. 使用SE(3)B样条运动基函数显式建模动态高斯的连续位置和方向变形；2. 设计自适应控制机制动态调整运动基函数和控制点数量；3. 开发软分割重建策略减轻长时间运动干扰；4. 采用多视图扩散模型提供多视图线索避免过拟合。

Result: 在大量实验中，该方法在新视角合成任务上超越了现有最先进方法，证明了其在动态场景建模方面的优越性能。

Conclusion: 该方法通过显式建模连续变形、自适应控制机制、软分割重建和多视图扩散模型的结合，实现了从单目视频进行高质量动态高斯溅射，在复杂运动建模和泛化能力方面表现出色。
...
TL;DR: GIFT是一个无需训练的视频帧选择框架，通过评估帧的全局不可替代性来选择关键帧，相比均匀采样在长视频基准上平均提升12.5%


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型处理密集帧计算成本高，现有方法通过贪心选择关键帧，但解耦评估相关性和多样性容易陷入局部最优，选择到无关噪声帧

Method: 提出GIFT框架：1) 引入定向多样性量化帧在相关性条件下的独特性，形成统一的不可替代性评分；2) 预算感知细化策略，通过自适应迭代过程先选择最高不可替代性的核心帧，然后随着预算扩展优先构建关键时间上下文

Result: 在LLaVA-Video-7B上的长视频基准测试中，相比均匀采样实现了最大平均12.5%的性能提升

Conclusion: GIFT通过全局评估帧的不可替代性，有效解决了现有方法因贪心决策和解耦评估导致的局部最优问题，显著提升了视频理解的效率和准确性
...
<details>
  <summary>Details</summary>
Motivation: 概念擦除是文本到图像模型的重要安全机制，但在新兴的单流扩散Transformer架构中研究不足。现有方法直接应用于这种架构会导致生成崩溃，需要专门的方法来解决这一问题

Method: 1. 提出流解耦概念擦除框架，将更新解耦，使现有方法能在单流模型上应用；2. 引入拉格朗日引导的自适应擦除调制，作为约束算法平衡擦除与保留的敏感权衡；3. 提供严格的收敛性分析证明

Result: Z-Erase成功克服了生成崩溃问题，在广泛的任务中实现了最先进的性能

Conclusion: Z-Erase是首个针对单流T2I模型的概念擦除方法，通过流解耦框架和拉格朗日引导调制，有效解决了生成崩溃问题，为单流扩散Transformer的安全应用提供了重要工具
...
TL;DR: 提出Token-Reweighting策略，解决多模态大语言模型中感知与推理令牌的耦合问题，提升强化学习验证奖励方法的性能


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的响应中感知相关令牌（视觉内容基础）和推理相关令牌（推理链构建）交织在一起，这两种令牌类型体现了不同但相互依赖的能力——视觉基础和符号推理，使得孤立优化效果不足

Method: 提出即插即用的令牌重加权策略，通过识别两种类型的关键令牌并在RLVR训练期间动态重加权，明确建模感知与推理令牌的相互依赖关系

Result: 在现有方法基础上应用ToR策略，在多个多模态推理基准测试中获得一致的性能提升，实现了最先进的性能，同时具备准确的视觉基础和连贯的推理能力

Conclusion: 令牌重加权策略有效解决了多模态大语言模型中感知与推理令牌的耦合问题，为强化学习验证奖励方法在多模态场景中的应用提供了有效解决方案
...
Method: 提出分层因果丢弃（HCD）：1）使用通道级因果掩码强制特征稀疏化，在表示层面进行因果干预；2）采用基于矩阵的互信息（MMI）目标，最小化潜在特征与领域标签的互信息，同时最大化与类别标签的共享信息；3）引入StyleMix驱动的VICReg模块，防止掩码意外过滤掉关键因果数据。

Result: 在OOD基准测试中，HCD的性能优于现有的顶级方法。

Conclusion: HCD通过通道级因果掩码和MMI目标，有效分离因果特征与虚假特征，解决了深度学习中OOD泛化的捷径依赖问题，在表示层面实现了有效的因果干预。
...
TL;DR: 本文提出CLVA方法，通过跨层视觉锚点解决多模态大语言模型中的物体幻觉问题，无需训练即可有效抑制深层注意力回归到早期视觉噪声


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型常出现物体幻觉问题，现有研究缺乏对最终模型阶段注意力漂移的充分解释性，需要更深入地理解视觉特征在层间的演化过程

Method: 提出CLVA（跨层视觉锚点）方法，这是一种无需训练的方法，通过强化关键中间层特征同时抑制回归噪声，利用从注意力动态中捕获的必需锚点将深层注意力拉回正确的视觉区域

Result: 在多种架构和基准测试中评估该方法，展示了出色的性能表现，且计算时间和GPU内存没有显著增加

Conclusion: 物体幻觉源于深层注意力向早期层视觉噪声的回归，输出可靠性依赖于在中间层而非最终层获取视觉锚点，CLVA方法能有效解决这一问题
...
TL;DR: THEMIS是一个用于评估多模态大语言模型在学术欺诈视觉推理能力的新基准，包含4000多个问题，涵盖7个真实场景和5种欺诈类型，测试结果显示即使是表现最好的模型准确率也只有56.15%。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法充分评估多模态大语言模型在真实世界学术欺诈场景中的视觉推理能力，存在与现实复杂性的差距，需要更全面、细粒度的评估工具。

Method: 构建包含4000多个问题的基准，涵盖7个真实学术场景，基于真实的撤稿案例和多模态合成数据，包含60.47%的复杂纹理图像，系统覆盖5种欺诈类型和16种细粒度操作，建立欺诈类型到5个核心视觉推理能力的映射。

Result: 在16个领先的多模态大语言模型上测试，即使是表现最好的GPT-5模型整体准确率也只有56.15%，表明该基准对模型提出了严格挑战。

Conclusion: THEMIS基准填补了现有评估工具与现实世界学术欺诈复杂性之间的关键差距，能够揭示不同模型在视觉欺诈推理方面的具体优势和弱点，有望推动多模态大语言模型在复杂真实世界欺诈推理任务中的发展。
...
TL;DR: Pixelis是一个像素空间智能体，通过直接操作图像/视频的可执行操作（缩放/裁剪、分割、跟踪、OCR、时间定位）来学习，采用三阶段训练方法，在多个基准测试上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 大多数视觉语言系统是被动的静态观察者：它们描述像素但不行动，无法在分布偏移下安全改进。这种被动性限制了可泛化的、物理基础的视觉智能。通过行动而非静态描述来学习对于超越策划数据至关重要

Method: 三阶段训练：1) 监督微调：从思维链-行动轨迹学习像素工具语法，使用掩码模仿损失加权操作/参数标记；2) 好奇心-一致性奖励微调：优化双驱动目标，结合预测误差好奇心和相邻步骤一致性；3) 像素测试时强化学习：通过检索邻居、投票完整轨迹进行无标签适应，使用KL-to-EMA安全控制约束漂移

Result: 在六个公共图像和视频基准测试中，Pixelis相比相同8B基线平均相对增益+4.08%（在VSI-Bench上峰值达+6.03%），同时产生更短、可审计的工具链，并在测试时学习中保持KL在走廊内
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习需要大规模标注数据，成本高昂。主动学习通过选择性标注信息丰富的样本来缓解这一问题，但现有方法通常假设模态重要性在训练过程中保持不变，忽略了多模态学习的动态特性——模态相对价值和样本难度会随着训练进程而变化。

Method: 提出RL-MBA框架，将样本选择建模为马尔可夫决策过程。包含两个核心组件：1) 自适应模态贡献平衡(AMCB)：通过强化反馈动态调整模态权重；2) 基于证据融合的难度感知策略调整(EFDA)：通过不确定性证据融合估计样本难度，优先选择信息丰富的样本。

Result: 在Food101、KineticsSound和VGGSound数据集上的实验表明，RL-MBA在有限标注预算下持续优于强基线方法，既提高了分类准确性，又改善了模态公平性。

Conclusion: RL-MBA通过强化学习框架实现了对多模态学习中动态特性的适应性，能够根据模态贡献、不确定性和多样性动态调整样本选择策略，为多模态主动学习提供了有效的解决方案。
...
TL;DR: 提出多阶段强化学习（MSRL）方法，通过从大规模文本偏好数据中学习可泛化的奖励推理能力，并逐步迁移到多模态任务，解决了多模态奖励模型训练中标注数据稀缺的问题。
...
Method: 提出多阶段强化学习（MSRL）方法：1）从大规模文本偏好数据中学习通用奖励推理能力；2）通过基于描述的强化学习阶段逐步迁移到多模态任务；3）引入跨模态知识蒸馏提升偏好泛化能力。

Result: MSRL显著提升了生成式多模态奖励模型的性能：在VL-RewardBench上从66.6%提升到75.9%，在GenAI-Bench上从70.2%提升到75.7%，且无需额外的多模态偏好标注数据。

Conclusion: MSRL方法有效扩展了基于RLVR的多模态奖励模型训练，通过从文本数据中学习奖励推理能力并迁移到多模态任务，解决了数据稀缺问题，显著提升了模型性能。
...
TL;DR: 本文提出了一种基于莫尔干涉模式的轻量级数据增强方法，通过解析数学公式实时生成纹理，无需外部数据或复杂计算，显著提升了图像分类模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前数据增强方法主要依赖基于扩散的合成或复杂的特征混合策略，这些方法计算开销大或需要外部数据集。本文探索基于解析干涉模式的过程化增强方法，以提供更高效、无需外部数据的增强方案。

Method: 提出一种轻量级增强方法，使用封闭形式的数学公式实时生成莫尔干涉纹理。这些模式直接在内存中合成，计算成本极低（每张图像0.0026秒），与训练图像混合后立即丢弃，实现无存储、无需外部数据的增强流程。

Result: 在Vision Transformers上的广泛实验表明，该方法在多个基准测试（包括ImageNet-C、ImageNet-R和对抗基准）中持续提升鲁棒性，优于标准增强基线和现有的无需外部数据的增强方法。

Conclusion: 解析干涉模式为数据驱动的生成式增强方法提供了实用且高效的替代方案，证明了轻量级过程化增强在提升模型鲁棒性方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统RPCA假设稀疏前景与低秩背景是相加关系，但实际应用中前景经常遮挡背景元素，存在模型不匹配问题

Method: 提出RPCC框架，间接识别稀疏成分的支持集；采用完全概率贝叶斯稀疏张量分解，通过变分贝叶斯推理求解；证明收敛到硬分类器

Result: 合成数据上获得接近最优估计；真实彩色视频上实现鲁棒前景提取；高光谱数据集上实现异常检测；无需后处理阈值化

Conclusion: RPCC框架有效解决了传统RPCA的遮挡问题，通过变分贝叶斯推理实现了无需阈值化的鲁棒前景提取和异常检测
...
<details>
  <summary>Details</summary>
Motivation: 现有文档生成方法面临两个主要挑战：1) 现有文档数据集覆盖范围有限且规模较小；2) 需要统一的框架来处理跨多种文档类别的不同生成任务。研究者希望开发一个能够处理多种文档生成任务的统一框架。

Method: 1) 建立可扩展的数据合成管道，自动生成HTML/CSS格式的文档，创建DocHTML数据集；2) 在多模态大语言模型上进行微调，实现三种文档生成任务：意图到文档、文档反渲染、元素到文档；3) 引入高度感知强化学习（HARL）后训练过程，通过基于预测和目标文档高度差异的奖励函数来惩罚和缓解内容溢出问题。

Result: AnyDoc在定性和定量实验中都表现出色，优于通用多模态大语言模型和特定任务基线模型。DocHTML数据集包含265,206个文档样本，涵盖111个类别和32种不同样式，所有文档都配备了全面的元数据。

Conclusion: AnyDoc通过统一的HTML/CSS表示、大规模合成数据集和高度感知强化学习，成功解决了文档生成中的多个挑战，在多种文档生成任务上实现了最先进的性能，为AI驱动的文档生成提供了有效的解决方案。
...
TL;DR: AirSplat是一个训练框架，将3D视觉基础模型的几何先验适配到无需姿态信息的新视角合成任务中，通过自一致姿态对齐和基于评分的透明度匹配提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 虽然3D视觉基础模型在零样本几何估计方面表现出色，但直接应用于通用的新视角合成仍面临挑战。需要将3DVFMs的几何先验有效适配到高质量、无需姿态信息的NVS任务中。

Method: 提出两个关键技术：1) 自一致姿态对齐(SCPA)：训练时反馈循环确保像素对齐监督，解决姿态-几何不一致问题；2) 基于评分的透明度匹配(ROM)：利用稀疏视图NVS教师模型的局部3D几何一致性知识过滤退化基元。

Result: 在大规模基准测试中，该方法在重建质量上显著优于最先进的无需姿态NVS方法，展示了同时实现视觉几何估计和高质量视图合成的潜力。

Conclusion: AirSplat框架成功将3D视觉基础模型的几何先验适配到新视角合成任务，实现了无需姿态信息的高质量视图合成，为同时进行几何估计和视图合成提供了新思路。
...
TL;DR: 该论文指出传统DPO方法在视觉语言模型中存在局限性，提出了基于排名的优化方法，通过生成有序的退化字幕链来提升视频文本对齐性能，并发现视觉编码器的微调对效果至关重要。


<details>
  <summary>Details</summary>
Motivation: 传统直接偏好优化（DPO）方法采用二元"赢家通吃"策略，在视觉语言模型中存在不足。视觉内容的响应质量高度依赖视觉输入，即使某个响应不如其他响应受欢迎，它仍可能忠实于视觉内容。标准的Bradley-Terry DPO公式缺乏这种细微差别，过度强调获胜响应而忽视"失败"响应可能仍保持高视觉保真度。

Method: 提出排名优化作为替代方案，更精确地定位响应对视觉输入的忠实度。专注于视频文本对齐，使用详细视频字幕，提出通过重复字幕退化生成具有挑战性的完全有序字幕链的方法。

Result: 排名优化在长格式内容生成和评估方面优于二元DPO。重要的是，研究发现这些方法需要微调视觉编码器才能有效，挑战了DPO纯粹是语言重加权过程的观点。

Conclusion: 对于视觉语言模型，排名优化比传统二元DPO更适合处理视觉保真度问题。视觉编码器的微调是实现有效对齐的关键因素，这表明视觉语言对齐需要更全面的模型优化方法。
...
<details>
  <summary>Details</summary>
Motivation: 全景语义分割面临两个关键挑战：全景投影的几何失真和密集标注的高成本。无监督领域自适应（UDA）提供了一种替代方案，但许多实际任务要求更严格的无源数据（SFUDA）约束，这加剧了领域偏移问题，导致伪标签不可靠和性能下降，特别是对少数类别。
...
Conclusion: DAPASS框架有效解决了无源数据条件下的全景语义分割领域自适应问题，通过创新的去噪和对抗模块克服了几何失真和伪标签不可靠的挑战，在室内外场景中均表现出色。
...
TL;DR: 本文开发了一个针对孟加拉国常见皮肤病的公开数据集，包含接触性皮炎、白癜风、湿疹、疥疮和癣菌病五类疾病共1612张图像，并应用多种机器学习模型进行分类性能评估。


<details>
  <summary>Details</summary>
Motivation: 皮肤疾病是全球性公共卫生问题，在孟加拉国等人口密集国家，皮肤科专家和诊断设备严重不足，缺乏及时检测和治疗可能导致严重后果。利用人工智能和机器学习技术通过图像处理和计算机视觉检测皮肤疾病具有重要现实意义。

Method: 1. 创建公开数据集：收集孟加拉国法里德布尔医学院门诊患者的皮肤疾病图像，包含五类常见疾病；2. 数据增强：从250张原始图像通过增强技术扩展到1612张；3. 应用多种机器学习和深度学习模型对数据集进行分类性能评估。

Result: 构建了包含1612张图像的数据集，具体分布为：皮炎302张、湿疹381张、疥疮301张、癣菌病316张、白癜风312张。虽然数据在区域收集，但这些疾病在南亚等地区普遍存在，使数据集具有全球应用价值。研究还报告了多种机器学习模型在数据集上的分类性能。

Conclusion: 该研究开发了针对常见皮肤病的公开数据集，为基于机器学习的皮肤病自动诊断研究提供了宝贵资源。数据集虽然区域收集但具有全球适用性，有望吸引机器学习和深度学习研究人员在自动疾病诊断领域的关注和应用。
...
TL;DR: 提出了EgoXtreme数据集，这是一个大规模的第一人称视角6D物体姿态估计数据集，包含工业维护、体育和紧急救援三个挑战性场景，旨在解决真实世界应用中存在的严重运动模糊、动态光照和视觉遮挡问题。
...
Method: 创建了EgoXtreme数据集，包含三个极端场景：工业维护、体育和紧急救援，这些场景通过极端光照、剧烈运动模糊和烟雾引入了严重的感知模糊性。评估了最先进的通用姿态估计器在该数据集上的表现。

Result: 评估显示现有通用姿态估计器在极端条件下的泛化能力失效，特别是在低光照环境下。简单的图像恢复（如去模糊）在极端条件下没有带来积极改善，而基于跟踪的方法显示出性能提升，表明在快速运动场景中使用时间信息是有意义的。

Conclusion: EgoXtreme是开发和评估下一代足够鲁棒以应对真实世界第一人称视觉的姿态估计模型的重要资源，填补了现有基准与真实世界应用需求之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有解耦数据集蒸馏方法主要依赖粗粒度类别标签监督，在细粒度数据集上存在两个问题：(1) 保留较大的类内差异但类间差异细微；(2) 同类样本过于相似，限制了局部判别线索，影响识别性能。

Method: FD²通过定位判别区域和构建细粒度表示进行蒸馏。在预训练阶段，使用反事实注意力学习聚合判别表示来更新类别原型。在蒸馏阶段，采用细粒度特征约束使每个样本与其类别原型对齐并排斥其他类别，同时使用相似性约束使同类样本的注意力多样化。

Result: 在多个细粒度和通用数据集上的实验表明，FD²能够与解耦数据集蒸馏无缝集成，在大多数设置中提高了性能，显示出强大的可迁移性。

Conclusion: FD²是一个专门针对细粒度数据集蒸馏的有效框架，通过细粒度表示学习和注意力机制解决了现有方法在细粒度识别任务中的局限性。
...
TL;DR: KGFP是一种基于表示的监控框架，通过检测目标检测器内部特征与视觉基础模型嵌入之间的语义不对齐来预测功能故障，显著提高安全关键检测的召回率。


<details>
  <summary>Details</summary>
Motivation: 安全关键环境中的目标检测器可能会无声地失败（如漏检行人等安全关键对象），而传统的OOD检测方法只关注识别陌生输入，无法直接预测检测器本身的功能故障。

Method: 提出知识引导故障预测（KGFP）框架，使用双编码器架构和角度距离度量，测量目标检测器内部特征与视觉基础模型嵌入之间的语义不对齐。当检测器超出其能力范围或视觉基础模型遇到新输入时，两种嵌入会发散产生高角度信号。

Result: 在COCO行人检测中，KGFP作为选择性预测门将接受图像的行人召回率从64.3%提升到84.5%（5%假阳性率），在六个COCO-O视觉域上保持强劲性能，大幅优于OOD基线方法。

Conclusion: KGFP通过检测语义不对齐来预测目标检测器的功能故障，为安全关键环境中的可靠目标检测提供了有效的运行时监控解决方案。
...
TL;DR: 论文分析了自动驾驶中高光谱成像技术面临的挑战，包括非受控光照条件、大景深范围和动态场景等环境因素，以及实时性要求和嵌入式平台计算资源限制等技术约束。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像在自动驾驶中具有应用潜力，但面临特定领域的挑战，包括环境因素（非受控光照、大景深、动态场景）和技术约束（实时性、计算资源限制），需要针对性的技术选择和算法开发。

Method: 通过分析HSI-Drive数据集最新版本的数据，研究基于高光谱成像的自动驾驶视觉系统中的多种技术，包括传感器技术选择和利用光谱与空间信息的定制视觉算法开发。

Result: 研究分析了多种高光谱成像技术在自动驾驶中的应用，基于HSI-Drive数据集实验结果为评估不同技术提供了实证基础，但具体实验结果未在摘要中详细说明。

Conclusion: 自动驾驶中的高光谱成像应用需要综合考虑环境挑战和技术约束，通过选择合适的高光谱成像技术和开发定制算法来充分利用传感器的光谱与空间信息。
...
TL;DR: 提出了一种语义解缠的统一3D异常检测模型，通过解耦语义表示来解决多类别统一模型中的类别间纠缠问题，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 3D异常检测中，统一模型虽然能提高可扩展性，但存在类别间纠缠问题，导致不同类别的潜在特征重叠，使模型在重建时采用错误的语义先验，从而产生不可靠的异常分数。

Method: 提出了语义解缠的统一3D异常检测模型，包含三个关键组件：1) 从粗到细的全局标记化形成实例级语义身份；2) 类别条件对比学习用于解耦类别语义；3) 几何引导解码器实现语义一致的重建。

Result: 在Real3D-AD和Anomaly-ShapeNet数据集上的实验表明，该方法在统一模型和类别特定模型上都达到了最先进水平，分别将物体级AUROC提高了2.8%和9.1%，同时增强了统一3D异常检测的可靠性。

Conclusion: 提出的语义解缠统一模型有效解决了多类别3D异常检测中的类别间纠缠问题，通过语义解耦表示显著提升了检测性能和可靠性。
...
TL;DR: 该论文提出两种层次感知多模态学习方法，通过编码生物分类的层次结构来提高大规模生物多样性识别的准确性和鲁棒性。
...
Method: 提出两种端到端变体：1) CLiBD-HiR，引入层次信息正则化(Hierarchical Information Regularization)来塑造跨分类层级的嵌入几何，产生结构化且抗噪声的表示；2) CLiBD-HiR-Fuse，额外训练轻量级融合预测器，支持仅图像、仅DNA或联合推理，并对模态损坏具有弹性。

Result: 在大规模生物多样性基准测试中，相比强大的多模态基线方法，该方法将分类准确率提高了超过14%，在部分和损坏的DNA条件下尤其表现出显著优势。

Conclusion: 明确编码生物层次结构并结合灵活融合是构建实用生物多样性基础模型的关键，该方法为生态学、保护生物学和环境监测提供了更可靠的技术基础。
...
TL;DR: SportSkills是首个面向物理技能学习的大规模体育数据集，包含36万+教学视频和63万+视觉演示，覆盖55种运动，能显著提升对精细动作差异的理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前大规模视频数据集主要关注一般人类活动，但缺乏针对物理技能学习所需的精细活动覆盖深度。需要专门的数据集来支持物理技能学习。

Method: 构建SportSkills数据集，包含36万+教学视频和63万+视觉演示，配有解释动作原理的教学旁白。基于此数据集，提出了首个大规模的错误条件教学视频检索任务。

Result: SportSkills使模型能够理解物理动作之间的精细差异，相比传统活动中心数据集，相同模型的性能提升高达4倍。专业教练评估显示，检索方法显著提升了视频模型为用户查询个性化视觉指导的能力。

Conclusion: SportSkills填补了物理技能学习数据集的空白，通过错误条件教学视频检索任务，将表示学习与可操作反馈生成相结合，为个性化技能学习提供了新途径。
...
TL;DR: LanteRn框架让大型多模态模型能够在潜在空间中直接进行视觉推理，通过生成和关注连续的视觉思维嵌入，避免了传统方法依赖外部模块或在像素空间推理的低效问题。
...
Method: LanteRn框架通过增强视觉语言变换器，使其能够在推理过程中生成和关注连续的视觉思维嵌入，实现语言与紧凑潜在视觉表示的交织。采用两阶段训练：监督微调将视觉特征锚定在潜在状态，然后通过强化学习将潜在推理与任务级效用对齐。

Result: 在三个感知中心基准测试（VisCoT、V*和Blink）上评估，观察到在视觉基础和细粒度推理方面的一致改进。

Conclusion: 内部潜在表示为更高效的多模态推理提供了有前景的方向，使模型能够在潜在空间直接进行视觉推理，避免了传统方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的点云自监督学习方法主要关注语义感知，但在实例定位任务上迁移效果差，需要完全微调才能获得良好性能。实例感知是3D感知的基础组件，填补这一差距对于推进真正的3D基础模型至关重要。

Method: PointINS采用正交偏移分支联合学习高级语义理解和几何推理，提出两种互补的正则化策略：偏移分布正则化（ODR）将预测偏移与经验几何先验对齐，空间聚类正则化（SCR）通过伪实例掩码正则化偏移以增强局部一致性。

Result: 在五个数据集上的广泛实验中，PointINS在室内实例分割上平均提升3.5% mAP，在室外全景分割上提升4.1% PQ，为可扩展的3D基础模型铺平道路。

Conclusion: PointINS通过几何感知学习成功增强了点云表示的实例感知能力，显著提升了实例定位任务的性能，推动了面向所有下游任务的3D基础模型的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于SAM的方法依赖像素级文本分割来采样数千个前景点作为提示，导致推理延迟不理想且数据利用率有限

Method: 提出ET-SAM框架：1) 定制轻量级点解码器生成词热图以获得少量前景点，消除过多点提示并加速推理；2) 设计联合训练策略，利用具有异构文本级标注的现有数据；3) 引入三组可学习任务提示来缓解数据集间的差异

Result: 相比之前的SAM架构，ET-SAM实现了约3倍的推理加速，在HierText上获得竞争性性能，在Total-Text、CTW1500和ICDAR15上平均提升11.0% F-score

Conclusion: ET-SAM通过高效的点解码器和联合训练策略，解决了SAM-based方法在推理延迟和数据利用方面的限制，为统一场景文本检测和布局分析提供了更实用的解决方案
...
### [77] [No Hard Negatives Required: Concept Centric Learning Leads to Compositionality without Degrading Zero-shot Capabilities of Contrastive Models](https://arxiv.org/abs/2603.25722)
...
<details>
  <summary>Details</summary>
Motivation: 现有的对比视觉语言模型在组合性表示学习方面存在局限。先前方法通常通过生成特定训练数据获取困难负样本来解决，但这些方法往往只针对单一基准、缺乏泛化性，且会严重损害模型的零样本和检索等基本能力，实用性不足。

Method: 提出两种核心改进：1）使用标准NLP工具获取短概念中心描述片段并与图像对齐；2）引入参数免费的跨模态注意力池化机制，从图像编码器中提取概念中心视觉嵌入。结合简单的辅助对比损失，在不增加推理成本的情况下提升性能。

Result: 该方法在标准组合性基准测试中取得了最先进的性能，同时保持甚至提升了强大的零样本和检索能力。所有改进均未增加推理成本。

Conclusion: 通过识别并解决V&L模型组合性受限的两个根本原因（长训练描述不需要组合性表示、全局池化导致绑定信息丢失），提出的简单改进方法能有效提升组合性性能而不损害其他核心能力，具有实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 预训练的视频扩散模型通常在短片段上训练，直接用于生成长视频会导致视觉质量显著下降。主要原因是存在两个分布外问题：帧级相对位置分布外和上下文长度分布外。

Method: 提出FreeLOC框架，包含两个核心技术：1) 视频相对位置重编码(VRPR)，通过多粒度策略分层重编码时间相对位置以对齐预训练分布；2) 分层稀疏注意力(TSA)，在不同时间尺度上结构化注意力密度以保持局部细节和长程依赖。还引入层自适应探测机制，识别各transformer层对分布外问题的敏感性，实现选择性高效应用。

Result: 大量实验表明，该方法显著优于现有的无需训练方法，在时间一致性和视觉质量方面达到最先进水平。

Conclusion: FreeLOC通过解决帧级相对位置和上下文长度两个分布外问题，有效提升了预训练视频扩散模型生成长视频的能力，无需额外训练即可实现高质量长视频生成。
...
TL;DR: 本文提出了首个双语文本到动作生成基准BiHumanML3D，并设计了带跨语言对齐机制的双语动作扩散模型BiMD，显著提升了跨语言动作生成质量。


<details>
  <summary>Details</summary>
Motivation: 文本到动作生成在跨语言应用中潜力巨大，但面临双语数据集缺乏和现有语言模型跨语言语义理解能力不足的双重挑战。

Method: 1. 通过LLM辅助标注和严格人工校正构建首个双语文本到动作基准BiHumanML3D；2. 提出双语动作扩散模型BiMD，引入跨语言对齐机制显式对齐不同语言的语义表示。

Result: BiMD模型在BiHumanML3D基准上取得FID 0.045（对比0.169）和R@3 82.8%（对比80.8%），显著优于单语扩散模型和翻译基线，在零样本代码切换场景也表现优异。

Conclusion: 研究证明了双语数据集和跨语言对齐策略对跨语言动作合成的关键必要性，提出的基准和模型为跨语言动作生成提供了可靠基础。
...
TL;DR: VolDiT：首个基于纯Transformer的3D扩散模型，用于医学图像合成，通过体素块嵌入和全局自注意力直接处理3D数据，并引入时间步门控控制适配器实现精确空间控制。
...
Method: 提出VolDiT，首个纯Transformer架构的3D扩散模型，通过体素块嵌入将3D数据转换为token，使用全局自注意力处理3D token。引入时间步门控控制适配器，将分割掩码映射为可学习的控制token，在去噪过程中调节Transformer层。

Result: 在高分辨率3D医学图像合成任务上评估，相比基于U-Net的3D潜在扩散模型，VolDiT展现出更好的全局一致性、更高的生成保真度和更强的可控性。
...
<details>
  <summary>Details</summary>
Motivation: 现有身份保持视频生成方法通常针对单一身份参考进行设计和优化，这种假设限制了创作灵活性，无法适应多样化的真实世界输入格式。单一参考源也构成了一个不适定场景，导致模型难以在新颖上下文中忠实地再现身份。

Method: 提出AnyID框架，包含两个核心贡献：1）可扩展的全参考架构，将异构身份输入（如面部、肖像、视频）统一为连贯表示；2）主参考生成范式，指定一个参考作为规范锚点，并使用新颖的差分提示实现精确的属性级可控性。在大规模精心策划的数据集上进行训练，最后使用强化学习进行微调。

Result: 广泛的评估验证了AnyID在不同任务设置下实现了超高身份保真度以及卓越的属性级可控性。通过人类评估构建的偏好数据集进行强化学习微调，确保了模型的鲁棒性和高保真度。
...
Method: 提出CardioDiT，一个完全4D的潜在扩散框架。使用时空VQ-VAE将2D+t切片编码为紧凑的潜在表示，然后由扩散变换器将这些潜在表示作为完整的3D+t体积进行联合建模，在整个生成过程中耦合空间和时间维度。

Result: 在公共CMR数据集和更大的私有队列上评估CardioDiT，与具有逐步增强时空耦合的基线进行比较。结果显示改进的切片间一致性、时间相干运动和真实的心脏功能分布。
...
Result: 通过统一虚拟环境中运行多个基线方法生成完整的团队行为，支持定量和视觉评估战术协调性。建立了从转播到模拟的统一数据和指标基准。

Conclusion: TacSIm为足球中风格对齐的战术模仿任务建立了一个严格的基准，能够测量和建模战术风格模仿，填补了当前研究的空白。
...
### [84] [Probabilistic Concept Graph Reasoning for Multimodal Misinformation Detection](https://arxiv.org/abs/2603.25203)
*Ruichao Yang,Wei Gao,Xiaobin Zhu,Jing Ma,Hongzhan Lin,Ziyang Luo,Bo-Wen Zhang,Xu-Cheng Yin*

Main category: cs.CV

TL;DR: PCGR框架通过构建概率概念图进行多模态虚假信息检测，将检测任务转化为结构化、基于概念的可解释推理过程，实现了最先进的检测精度和对新型操纵手法的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息日益增多，传统检测器存在不透明、黑箱化、对新型操纵手法脆弱等问题，需要开发可解释且能适应演变的检测框架。

Method: 采用"先构建后推理"范式：首先构建人类可理解的概念节点图，包括通过多模态大语言模型自动发现和验证的新型高级概念；然后在该概念图上应用分层注意力机制来推断声明的真实性。

Result: PCGR在多模态虚假信息检测中实现了最先进的准确性，对新兴操纵类型具有鲁棒性，在粗粒度检测和细粒度操纵识别方面均优于先前方法。

Conclusion: PCGR框架通过结构化、基于概念的可解释推理，有效解决了多模态虚假信息检测中的黑箱问题和脆弱性问题，为构建可解释且能适应演变的检测系统提供了新途径。
...
TL;DR: CHIRP数据集和CORVID方法为野生动物行为监测提供计算机视觉解决方案，特别针对野生鸟类个体重识别，通过彩色腿环识别实现个体追踪，并引入生物学相关指标评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 野生动物长期行为监测对保护生物学和进化生物学至关重要，但现有计算机视觉方法在野外种群自动化监测方面面临挑战，缺乏覆盖多种视觉任务的数据集来提取有生物学意义的个体测量数据。

Method: 提出CHIRP数据集（来自瑞典拉普兰野生西伯利亚松鸦种群）支持重识别、动作识别、关键点估计等任务；开发CORVID方法，基于彩色腿环分割和分类进行个体识别，采用概率匹配数据库的跟踪方法。

Result: CHIRP数据集支持多种计算机视觉任务；CORVID方法在个体重识别方面优于现有最先进方法，通过应用特定基准测试（如摄食率、共现率）验证了其在实际生物学应用中的有效性。

Conclusion: 该工作为社区提供了从伦理批准的生物学研究中策划真实世界数据集的蓝图，弥合了计算机视觉研究与生物学应用之间的差距，推动了野生动物自动化行为监测的发展。
...
TL;DR: SDD-YOLO是针对地面到空中无人机检测的小目标检测框架，通过P2高分辨率检测头、YOLO26架构改进和MuSGD混合训练策略，在自建的DroneSOD-30K数据集上达到86.0% mAP@0.5，相比YOLOv5n提升7.8个百分点，并在RTX 5090上实现226 FPS的高效推理。


<details>
  <summary>Details</summary>
Motivation: 从地面到空中视角检测小型无人机面临三大挑战：极低的像素占比、杂乱的空中背景以及严格的实时性要求。现有的YOLO检测器主要针对通用目标检测设计，对亚像素级目标缺乏足够的特征分辨率，且在部署时引入复杂性。

Method: 1. 引入P2高分辨率检测头（4倍下采样）以捕捉微小目标的关键空间细节；2. 集成YOLO26的架构改进：无DFL、无NMS的简化推理架构；3. 采用MuSGD混合训练策略，结合ProgLoss和STAL，有效缓解稀疏小目标信号的梯度振荡问题；4. 构建DroneSOD-30K大规模数据集（约3万张标注图像，涵盖多种气象条件）。

Result: SDD-YOLO-n在DroneSOD-30K数据集上达到86.0% mAP@0.5，相比YOLOv5n基线提升7.8个百分点。推理性能方面：在NVIDIA RTX 5090上达到226 FPS，在Intel Xeon CPU上达到35 FPS，显示出优异的边缘部署效率。

Conclusion: SDD-YOLO是针对G2A反无人机监控的小目标检测专用框架，通过高分辨率检测头、简化架构和先进训练策略，在保持高精度的同时实现了高效的实时推理，为未来边缘部署提供了可行的解决方案。
...
TL;DR: 提出无需训练、仅需CAD模型作为先验知识的多视角手术器械6D姿态估计方法，在未见器械上达到毫米级精度
...
Method: 两阶段流程：1) 检测阶段：生成掩膜提议，使用预训练特征提取器与渲染模板匹配，跨视图匹配后三角化为3D实例候选；2) 姿态估计：迭代精炼姿态假设，使用特征度量评分和跨视图注意力，最后通过多视角、遮挡感知的轮廓配准进行最终优化

Result: 在MVPSP数据集上评估，达到毫米级精度，与受控条件下的监督方法相当，同时保持对未见器械的完全泛化能力

Conclusion: 提出了一种新颖灵活的方法，结合基础模型、多视角几何和轮廓优化，无需任务特定训练即可实现手术器械的高精度6D姿态估计，适用于动态临床环境
...
TL;DR: DeepFAN是一种基于Transformer的模型，用于肺结节良恶性分类，通过多中心临床试验验证其辅助初级放射科医生的效果。


<details>
  <summary>Details</summary>
Motivation: CT广泛应用导致肺结节检出增多，现有深度学习方法未能充分整合全局和局部特征，且缺乏临床试验验证。

Method: 开发基于Transformer的DeepFAN模型，在超过1万个病理确认的结节上训练，并进行多读者、多病例的临床试验评估。

Result: DeepFAN在内部测试集上AUC为0.939，临床试验数据集上AUC为0.954。12位读者使用后平均AUC提升10.9%，准确性提升10.0%，敏感性提升7.6%，特异性提升12.6%。结节级诊断一致性从一般提升到中等。
...
TL;DR: 本文提出了一种新颖的空间对齐框架（SAF），用于解决传统基于变换的对抗攻击在结构化任务（如语义分割和目标检测）中效果不佳的问题，通过同步变换标签与输入实现空间对齐，显著提升了攻击的迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有基于变换的对抗攻击（TAAs）在分类任务中表现出良好的迁移性，但在结构化任务（如语义分割、目标检测）中效果不佳甚至失败。研究发现，结构化任务的标签具有空间结构特性，而传统TAAs在应用空间变换时未能同步调整标签，导致空间错位和错误梯度。

Method: 提出统一的空间对齐框架（SAF），包含空间对齐（SA）算法，该算法在应用空间变换时同步变换标签，确保输入与标签的空间对齐，从而生成更有效的对抗样本。
...
Conclusion: 空间对齐对于基于变换的对抗攻击在结构化任务中至关重要，提出的SAF框架通过同步变换输入和标签解决了空间错位问题，显著提升了攻击的迁移性和有效性。
...
TL;DR: 提出"Just Zoom In"方法，通过自回归缩放城市级卫星地图进行跨视角地理定位，避免对比学习和困难负样本挖掘，在真实基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法普遍采用基于对比学习的图像检索框架，存在三个主要问题：1) 依赖大批量和困难负样本挖掘；2) 忽略地图的几何结构；3) 街景与卫星图像覆盖不匹配，导致检索目标模糊且限制空间推理能力。

Method: 提出"Just Zoom In"方法，将地理定位重新定义为自回归缩放过程。从粗粒度卫星视图开始，模型通过一系列缩放决策选择目标分辨率的终端卫星单元，无需对比损失或困难负样本挖掘。同时引入包含众包街景和高分辨率卫星图像的真实基准测试。

Result: 在提出的真实基准测试上，Just Zoom In达到最先进性能：在50米范围内Recall@1提升5.5%，在100米范围内Recall@1提升9.6%，优于最强的对比检索基线方法。

Conclusion: 通过序列化的粗到细空间推理方法，Just Zoom In有效解决了跨视角地理定位问题，证明了自回归缩放框架相比传统对比检索方法的优越性。
...
TL;DR: 提出Hybrid Memory新范式解决视频世界模型中动态主体被遮挡后重现时的失真问题，构建HM-World数据集并开发HyDRA架构，显著提升动态主体一致性和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型将环境视为静态画布，当动态主体被遮挡后重新出现时，常出现冻结、扭曲或消失的问题，需要新的记忆机制来同时处理静态背景和动态主体。

Method: 提出Hybrid Memory范式，构建HM-World数据集（59K高清视频片段，解耦相机和主体轨迹），开发HyDRA架构（将记忆压缩为token，使用时空相关性驱动的检索机制）。

Result: 在HM-World数据集上的大量实验表明，该方法在动态主体一致性和整体生成质量方面显著优于现有最先进方法。

Conclusion: Hybrid Memory范式能有效解决动态主体被遮挡后的连续性问题，HyDRA架构通过选择性关注相关运动线索，成功保持隐藏主体的身份和运动连续性。
...
<details>
  <summary>Details</summary>
Motivation: 细粒度面部表情编辑长期以来受到内在语义重叠的限制，需要解决表情编辑中的结构混淆、编辑准确性、线性可控性以及表情编辑与身份保持之间的权衡问题。

Method: 构建FFE数据集并建立FFE-Bench评估基准；提出PixelSmile扩散框架，通过完全对称联合训练解耦表情语义，结合强度监督和对比学习，使用文本潜在插值实现精确稳定的线性表情控制。

Result: PixelSmile实现了优越的解耦效果和鲁棒的身份保持，在连续、可控、细粒度的表情编辑方面表现出色，同时自然支持平滑的表情混合。

Conclusion: PixelSmile通过创新的训练策略和评估框架，有效解决了细粒度面部表情编辑中的语义重叠问题，为连续可控的表情编辑提供了有效的解决方案。
...
Method: 包含两个轻量级模块：1) 几何重稠密化模块迭代稠密化编码的稀疏几何，提取稠密尺度特征后稀疏化用于预测编码；2) 跨尺度特征传播模块利用多分辨率级别的占用线索指导分层特征传播，实现跨尺度信息共享并减少冗余特征提取。还引入了仅整数推理管道以确保跨平台一致性

Result: 实验表明在实时速度下实现了有竞争力的压缩性能

Conclusion: 提出的FastPCC框架通过紧凑表示和高效预测编码，解决了LiDAR点云压缩中的稀疏性问题，实现了快速且高性能的无损压缩
...
### [94] [Vega: Learning to Drive with Natural Language Instructions](https://arxiv.org/abs/2603.25741)
*Sicheng Zuo,Yuxuan Li,Wenzhao Zheng,Zheng Zhu,Jie Zhou,Jiwen Lu*

Main category: cs.CV

TL;DR: 该论文提出了Vega模型，这是一个统一的视觉-语言-世界-动作模型，用于基于指令的自动驾驶生成和规划，通过构建大规模驾驶数据集InstructScene并采用自回归和扩散范式实现多模态交互。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在自动驾驶中主要将语言用于场景描述或推理，缺乏根据多样化用户指令进行个性化驾驶的灵活性。需要开发能够遵循不同用户指令的个性化驾驶系统。

Method: 1. 构建大规模驾驶数据集InstructScene，包含约10万个场景，标注了多样化驾驶指令及对应轨迹；2. 提出统一的Vega模型，采用自回归范式处理视觉输入和语言指令，采用扩散范式生成未来预测和轨迹；3. 使用联合注意力实现模态间交互，为不同模态使用独立的投影层以增强能力。

Result: 实验表明该方法不仅实现了优越的规划性能，还展现出强大的指令遵循能力，为更智能和个性化的驾驶系统铺平了道路。

Conclusion: 提出的Vega模型通过统一的多模态架构和指令驱动方法，有效解决了现有自动驾驶系统缺乏个性化指令遵循能力的问题，推动了智能驾驶系统向更灵活、个性化方向发展。
...
TL;DR: V2U4Real是首个大规模真实世界多模态车辆-无人机协同感知数据集，通过地面车辆和无人机协同收集，包含超过56K LiDAR帧和56K多视角图像，涵盖城市街道、校园和乡村道路等多种场景。


<details>
  <summary>Details</summary>
Motivation: 现有V2V和V2I协同感知局限于地面协作，无法完全解决大规模遮挡和复杂环境中的长距离感知问题，需要车辆与无人机跨视角协同来增强感知能力。

Method: 使用配备多视角LiDAR和RGB相机的地面车辆与无人机协同收集数据，覆盖城市街道、大学校园和乡村道路等多种交通场景，建立包含56K LiDAR帧、56K多视角图像和700K标注3D边界框的数据集。

Result: 建立了单智能体3D目标检测、协同3D目标检测和目标跟踪的基准测试，综合评估表明V2U协同能有效增强感知鲁棒性和长距离感知能力。

Conclusion: V2U4Real是首个真实世界V2U协同感知数据集，为跨视角协同感知研究提供了重要资源，展示了车辆-无人机协同在提升感知能力方面的有效性。
...
TL;DR: 本文提出了MacroData数据集和MacroBench基准，用于解决多参考图像生成中的数据瓶颈问题，通过大规模结构化数据提升模型在多参考条件下的生成性能。


<details>
  <summary>Details</summary>
Motivation: 当前多参考图像生成模型在输入参考图像数量增加时性能严重下降，根本原因是现有数据集主要包含单参考或少量参考图像对，缺乏学习密集参考间依赖关系所需的结构化长上下文监督数据。

Method: 1. 构建MacroData数据集：包含40万个样本，每个样本最多10张参考图像，系统组织为四个互补维度（定制化、插画、空间推理、时间动态）；2. 提出MacroBench基准：包含4000个样本，评估不同任务维度和输入规模下的生成一致性。

Result: 在MacroData上微调显著提升了多参考图像生成性能，消融研究揭示了跨任务协同训练的优势以及处理长上下文复杂性的有效策略。

Conclusion: 通过大规模结构化数据集和标准化评估基准，有效解决了多参考图像生成中的数据瓶颈问题，为多主体合成、叙事插画和新视角合成等实际应用提供了重要支持。
...
TL;DR: 本文提出了一种基于头部敏感度得分的两阶段稀疏化方法，通过量化注意力头对稀疏化的敏感性差异，在保持高稀疏度的同时减少性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有VGGT模型的全局注意力层存在二次计算复杂度问题，虽然已有稀疏化加速技术，但通常导致显著的精度下降。作者假设这种下降源于不同注意力头对稀疏化的敏感性存在异质性，而现有方法对所有头采用统一的稀疏模式。

Method: 提出两阶段稀疏化流程：1) 使用新颖的头部敏感度得分(HeSS)度量方法，在小规模校准集上近似关于两个不同误差项的Hessian矩阵来量化头部稀疏化敏感性；2) 在推理阶段进行HeSS引导的稀疏化，利用预计算的HeSS重新分配总注意力预算，为敏感头分配更密集的注意力，为更鲁棒的头分配更稀疏的注意力。

Result: 实验证明HeSS能有效捕捉头部稀疏化敏感性，确认全局注意力层中的注意力头确实表现出异质性敏感特征。在高度稀疏条件下，该方法有效缓解了性能下降，在不同稀疏化水平下表现出强鲁棒性。

Conclusion: 通过量化并利用注意力头对稀疏化的异质性敏感性，提出的HeSS引导稀疏化方法能够在保持高计算效率的同时最小化精度损失，为VGGT模型的加速提供了有效解决方案。
...
TL;DR: InstanceAnimator是一个基于Diffusion Transformer的框架，用于多实例草图视频着色，解决了现有方法在用户控制灵活性、实例对齐和细节保真度方面的三大限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个核心限制：1）由于过度依赖单参考帧导致用户控制不灵活；2）在多角色场景中实例可控性差导致错位；3）细粒度区域细节保真度下降。

Method: 提出三个创新：1）画布引导条件，允许自由放置参考元素和背景；2）实例匹配机制，通过将实例特征与草图集成确保多角色精确控制；3）自适应解耦控制模块，将角色、背景和文本条件的语义特征注入扩散过程。

Result: 大量实验证明，InstanceAnimator实现了卓越的多实例着色效果，具有增强的用户控制、高视觉质量和强大的实例一致性。

Conclusion: InstanceAnimator通过创新的画布引导、实例匹配和自适应控制模块，有效解决了多实例草图视频着色中的关键挑战，提供了更灵活的用户控制和更好的视觉质量。
...
TL;DR: 提出CLIP-RD框架，通过垂直关系蒸馏和交叉关系蒸馏方法，改进CLIP知识蒸馏效果


<details>
  <summary>Details</summary>
Motivation: CLIP模型计算和内存需求大，需要轻量化蒸馏，但现有方法未能显式建模师生嵌入间的多向关系依赖，限制了学生模型保持教师编码的结构关系能力
...
Conclusion: 提出的关系知识蒸馏框架能有效提升CLIP蒸馏效果，通过显式建模多向关系依赖，使学生模型更好地保持教师模型的结构关系
...
Conclusion: PTM-ST通过分阶段蒸馏框架有效解决了多模态数据集蒸馏中的关键挑战，提高了学生模型性能和蒸馏数据质量，同时降低了存储成本。
...
<details>
  <summary>Details</summary>
Motivation: 红外小目标检测面临复杂背景干扰和小目标低对比度的挑战。虽然U-Net架构在多尺度特征融合方面表现强大，但在将深层特征传递到浅层时存在语义退化问题，限制了小目标的精确定位。

Method: 提出FSGNet框架：1）编码器中使用多方向交互注意力模块捕获细粒度方向特征；2）多尺度频率感知模块利用快速傅里叶变换过滤背景干扰；3）全局池化模块捕获高层语义信息，通过全局语义引导流传播到各解码器阶段，确保跨尺度语义一致性和精确定位。

Result: 在四个公开红外小目标检测数据集上的实验表明，FSGNet实现了优越的检测性能并保持高效率，突出了其实用性和鲁棒性。

Conclusion: FSGNet通过频率感知和语义引导机制有效解决了U-Net的语义退化问题，提升了红外小目标检测的精度和效率，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉基础模型的编码器-仅分割方法需要微调编码器，牺牲了编码器共享性，而冻结编码器的方法又速度较慢，需要在简单性、速度和编码器共享之间找到平衡

Method: 提出Plain Mask Decoder (PMD)，一个基于Transformer的快速分割解码器，直接在冻结的VFM特征上操作，形成Plain Mask Transformer (PMT)模型

Result: 在图像分割基准测试中，PMT匹配冻结编码器的最先进性能，同时运行速度提高约3倍；在视频分割中，性能与完全微调方法相当，比最先进的冻结编码器模型快8倍

Conclusion: PMT成功地在保持编码器-仅设计的简单性和低延迟的同时，保持了编码器表示的冻结和可共享性，为图像和视频分割提供了高效解决方案
...
Result: 在LIBERO、LIBERO-Plus和SimplerEnv-WidowX基准测试中，LaMP始终优于评估的VLA基线，在相同训练预算下达到最高平均成功率。在LIBERO-Plus OOD扰动下，LaMP显示出更强的鲁棒性，比最强基线平均提升9.7%。

Conclusion: LaMP通过显式建模3D场景流作为潜在运动先验，有效解决了VLA模型在复杂物理交互中的局限性，显著提升了机器人操作的性能和鲁棒性。
...
TL;DR: 论文提出了一个分层框架来提升视觉语言模型的3D空间理解能力，通过自动化生成大规模3D空间VQA数据对进行监督微调，并开发了结合RGB-D输入的VLM模型，在多个空间理解基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 实现类人空间智能需要视觉语言模型从2D观察推断3D结构、识别3D空间中的物体属性和关系，并进行高级空间推理。当前VLMs在这方面的能力有限，需要系统化的方法来提升3D空间理解。

Method: 1) 提出分层框架将3D空间理解学习分解为四个渐进复杂层次：几何感知到抽象空间推理；2) 构建自动化流水线处理约500万图像和4500万物体，生成多样化的3D空间VQA对用于监督微调；3) 开发RGB-D VLM，将度量尺度点云图作为辅助输入以增强空间理解。

Result: 在多个空间理解和推理基准测试中达到最先进性能，超越了专门的空间模型和大型专有系统如Gemini-2.5-pro和GPT-5。分析揭示了分层任务级别之间的明确依赖关系。

Conclusion: 提出的分层框架和自动化数据生成方法有效提升了VLMs的3D空间智能，多级任务设计促进了3D空间智能的涌现，为理解空间推理的层次结构提供了新见解。
...
Method: 1. 首先训练为基于流的单视角V2V模型；2. 使用Pi3空间基础模型将所有视角锚定在共享4D潜在空间中，确保视角一致性；3. 在不同扩散时间步训练视角，学习联合和条件视角分布，实现自回归合成新视角

Result: 在单视角转换基准测试中达到或优于最先进水平，首次实现物理和风格一致的多视角转换，包括对机器人学习世界随机化至关重要的挑战性第一人称和异构摄像头设置

Conclusion: VideoWeaver是首个能够处理多视角视频转换的框架，通过共享潜在空间和自回归合成解决了跨视角一致性和可扩展性问题，为具身AI策略迁移到新环境提供了有效工具
...
TL;DR: DC-Reg：一种基于DC规划的全局最优点云配准框架，通过整体凹下界估计显著加速BnB搜索，在部分重叠和大错位情况下实现鲁棒配准


<details>
  <summary>Details</summary>
Motivation: 现有同时估计变换和对应关系的方法存在非凸耦合目标函数问题，启发式方法易陷入局部最优，而现有全局求解器由于下界松弛导致收敛时间过长

Method: 提出DC-Reg框架，基于DC规划推导出变换-分配耦合目标的整体凹下界估计器，通过线性分配问题在搜索框顶点计算紧致下界，显著加速BnB搜索

Result: 在合成数据和3DMatch基准测试中，DC-Reg相比现有全局技术实现显著更快的收敛速度，对极端噪声和异常值具有更强的鲁棒性

Conclusion: DC-Reg通过整体DC分解捕获变换和对应关系的联合结构交互，提供紧致下界估计，解决了全局最优点云配准中的收敛效率和鲁棒性问题
...
TL;DR: CIAR是一个云-设备协作框架，通过设备端自验证处理视觉合成中的大规模词汇和空间冗余问题，实现2.18倍加速并减少70%云请求，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像生成方面取得了显著进展，但其计算密集和顺序特性阻碍了设备端部署，导致高延迟。主要挑战包括：1）高保真图像需要的大规模词汇表；2）空间冗余导致均匀区域高度可预测，而物体边界存在高不确定性。

Method: CIAR采用云-设备协作框架，核心是设备端令牌不确定性量化器，使用连续概率区间加速处理大规模视觉词汇；结合区间增强解码模块，通过分布对齐训练策略在加速解码的同时保持视觉保真度和语义一致性。
...
TL;DR: GridVAD是一个无需训练的零样本视频异常检测框架，使用视觉语言模型生成异常提案，通过自一致性整合过滤幻觉，再通过Grounding DINO和SAM2进行空间定位和时间传播，实现像素级异常分割。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型作为开放集推理器在视频异常检测中直接使用时存在脆弱性：缺乏校准的异常先验会导致漏检和误报。问题不在于VLM本身，而在于如何使用它。VLM应作为异常提案生成器，由专门的空间和时间模块进行后续处理。

Method: 提出"提案-定位-传播"原则，实现为GridVAD训练免费流程：1) VLM在视频片段的分层网格表示上生成自然语言异常提案；2) 自一致性整合通过多次独立采样过滤幻觉提案；3) Grounding DINO将提案锚定为边界框；4) SAM2在异常区间内传播为密集掩码。每个片段的VLM调用次数固定为M+1次。

Result: 在UCSD Ped2数据集上，GridVAD达到最高像素级AUROC（77.59），超过部分微调的TAO（75.11），在对象级RBDC上比其他零样本方法高出5倍以上。自一致性整合提供可控的精度-召回权衡，过滤改善所有像素级指标。效率上比均匀每帧VLM查询高效2.7倍。

Conclusion: GridVAD展示了将VLM作为异常提案生成器而非直接检测器的有效性，通过专门的空间和时间模块实现零样本像素级异常检测。该方法在保持效率的同时显著提升性能，为视频异常检测提供了新的训练免费解决方案。
...
TL;DR: AdaSFormer是一个针对室内单目语义场景补全的串行化Transformer框架，通过自适应感受野、中心相对位置编码和卷积调制层归一化三个关键设计，在NYUv2和Occ-ScanNet数据集上实现了最先进的性能。
...
Method: 提出AdaSFormer框架，包含三个关键设计：1) 具有可学习偏移的自适应串行化Transformer，可动态调整感受野；2) 中心相对位置编码，捕捉空间信息丰富性；3) 卷积调制层归一化，桥接卷积和Transformer特征之间的异构表示。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像修复模型受限于训练数据的规模和分布，在真实世界场景中泛化能力差。虽然闭源大模型如Nano Banana Pro在修复任务中表现出色，但需要大量数据和计算成本。因此需要开发高性能的开源解决方案。

Method: 1) 构建覆盖九种常见真实世界退化类型的大规模数据集；2) 训练一个最先进的开源图像修复模型；3) 提出RealIR-Bench评估基准，包含464张真实退化图像和专门针对退化去除和一致性保持的评估指标。

Result: 实验表明，该模型在开源方法中排名第一，达到了最先进的性能水平，有效缩小了与闭源替代方案之间的差距。

Conclusion: 通过构建大规模数据集和评估基准，成功开发了一个高性能的开源图像修复模型，为真实世界图像修复任务提供了有效的解决方案，同时促进了开源社区的发展。
...
TL;DR: 论文提出了一种解决视频生成中运动-视觉质量困境的新方法TQD，通过时间步感知的质量解耦，在不完美的数据上训练也能获得超越传统使用完美数据的性能。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型严重依赖高质量数据，但研究发现视觉质量和运动强度存在负相关关系（运动-视觉质量困境），难以同时获得两方面都优秀的数据。

Method: 提出了时间步感知质量解耦（TQD）方法，通过分析视频扩散模型的分层学习动态和基于梯度的质量退化样本分析，发现质量不平衡数据在适当时间步能产生与黄金数据相似的梯度。该方法修改数据采样分布，使运动丰富数据偏向更高时间步采样，而高视觉质量数据在较低时间步更可能被采样。

Result: 实验表明，TQD能够仅使用分离的不平衡数据进行训练，实现超越传统使用更好数据的性能，挑战了视频生成中完美数据的必要性。同时，该方法在使用高质量数据训练时也能提升模型性能。

Conclusion: TQD方法有效解决了视频数据中的运动-视觉质量困境，为在不完美数据上训练高质量视频生成模型提供了新思路，展示了在不同数据场景下的有效性。
...
TL;DR: 研究者提出了首个羽毛球完整比赛密集数据集BFMD，包含19场比赛、1687个回合和16751次击球事件，每击球都有详细标注。他们开发了基于VideoMAE的多模态字幕生成框架，通过语义反馈机制提升字幕质量，并展示了全场比赛战术模式分析的应用潜力。


<details>
  <summary>Details</summary>
Motivation: 现有羽毛球数据集主要关注短片段或特定任务标注，缺乏完整比赛数据和密集多模态标注，这限制了准确击球字幕生成和比赛级分析的能力。

Method: 创建了BFMD数据集，包含19场转播比赛（单打和双打），超过20小时比赛时间，提供分层标注：比赛片段、回合事件、击球类型、羽毛球轨迹、球员姿态关键点和击球字幕。开发了基于VideoMAE的多模态字幕生成框架，引入语义反馈机制，利用击球语义指导字幕生成。

Result: 多模态建模和语义反馈机制相比仅使用RGB的基线方法显著提升了击球字幕生成质量。数据集还展示了在全场比赛战术模式时间演化分析方面的应用潜力。

Conclusion: BFMD数据集填补了羽毛球完整比赛密集标注数据的空白，提出的多模态字幕生成框架通过语义反馈机制有效提升了字幕质量，为羽毛球战术动态分析和比赛级研究提供了重要资源。
...
TL;DR: PAWS方法从大规模第一人称视角视频中的手-物体交互直接提取物体关节结构，无需高质量3D数据和人工标注，在多个数据集上表现优于基线方法
...
Method: 直接从大规模野外第一人称视角视频中的手-物体交互提取物体关节结构，无需3D数据或人工标注

Result: 在HD-EPIC和Arti4D等公开数据集上显著优于基线方法，提取的关节结构有益于下游任务

Conclusion: PAWS方法通过从野外视频中学习手-物体交互，有效解决了关节感知的数据依赖问题，为3D场景理解提供了可扩展的解决方案
...
TL;DR: 本文针对当前地球观测大模型忽视垂直维度的问题，提出了首个高度感知的遥感理解评估框架，包括数据生成管道、两个基准测试集和一个基线模型，有效解决了遥感几何中的"垂直盲点"问题。
...
Result: 成功构建了高度感知遥感评估框架，基线模型证明视觉语义与高度几何特征的协同作用能有效缓解"垂直盲点"，为现有光学模型开启了交互式高度推理的新范式。

Conclusion: 通过引入高度感知的评估框架和基线模型，成功解决了遥感大模型中的垂直维度缺失问题，为复杂遥感几何理解和灾害场景分析提供了新的技术路径。
...
### [115] [UNIC: Neural Garment Deformation Field for Real-time Clothed Character Animation](https://arxiv.org/abs/2603.25580)
...
Result: 在各种服装网格上的实验表明，UNIC在效果和效率上优于基线方法，具有实际应用潜力。

Conclusion: UNIC通过神经变形场和实例特定学习方案，实现了高质量实时服装动画，适用于视频游戏等交互式应用。
...
TL;DR: Trace框架通过单锚帧轨迹设计实现视频中目标物体运动路径编辑，包含跨视图运动变换和运动条件视频重合成两阶段流程
...
Method: 提出Trace框架：1)跨视图运动变换模块将首帧路径设计映射到相机运动下的帧对齐边界框轨迹；2)运动条件视频重合成模块按照轨迹重新生成物体同时保留视频其余内容

Result: 在多样化真实世界视频上的实验表明，相比最近的图像到视频和视频到视频方法，Trace能产生更连贯、真实且可控的运动编辑效果

Conclusion: Trace提供了一种实用、易用的物体中心运动编辑方法，通过单锚帧轨迹设计实现高质量、时间一致的视频编辑，解决了现有方法的局限性
...
TL;DR: AnyHand是一个大规模合成数据集，包含650万张RGB-D图像，用于提升3D手部姿态估计性能，在RGB-only和RGB-D设置下都显著提升了现有基准的表现。


<details>
  <summary>Details</summary>
Motivation: 现有真实世界数据集覆盖范围有限，而之前的合成数据集很少在大规模上同时提供遮挡、手臂细节和对齐的深度信息，这限制了3D手部姿态估计的性能提升。

Method: 创建了包含250万张单手图像和410万张手物交互图像的大规模合成数据集AnyHand，提供丰富的几何标注。同时提出了一个轻量级深度融合模块，可轻松集成到现有RGB模型中。

Result: 在RGB-only设置下，使用AnyHand扩展训练集显著提升了FreiHAND和HO-3D基准的表现，并且在未微调的情况下在HO-Cap数据集上表现出更强的泛化能力。RGB-D模型在HO-3D基准上实现了优越性能。

Conclusion: AnyHand数据集有效解决了3D手部姿态估计的数据瓶颈问题，深度融合模块进一步提升了RGB-D模型的性能，证明了合成数据的有效性和深度信息的重要性。
...
TL;DR: BizGenEval是一个针对商业视觉内容生成的系统化基准测试，涵盖幻灯片、图表、网页、海报和科学图表五种文档类型，评估文本渲染、布局控制、属性绑定和知识推理四个关键能力维度，包含400个精心设计的提示和8000个人工验证检查问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型的基准测试主要关注自然图像合成，无法系统评估在真实商业设计任务中结构化、多约束要求下的模型性能。需要建立一个专门针对商业视觉内容生成的标准化评估体系。

Method: 构建BizGenEval基准测试，包含五种代表性文档类型（幻灯片、图表、网页、海报、科学图表）和四个关键能力维度（文本渲染、布局控制、属性绑定、知识推理），形成20个评估任务。收集400个精心设计的提示和8000个人工验证检查问题，对26个流行的图像生成系统进行大规模基准测试。

Result: 对26个主流图像生成系统（包括最先进的商业API和领先的开源模型）的评估结果显示，当前生成模型与专业视觉内容创作需求之间存在显著能力差距。

Conclusion: BizGenEval揭示了当前生成模型在商业视觉内容创作方面的局限性，希望该基准测试能够成为真实世界商业视觉内容生成的标准化评估工具。
...
TL;DR: SlotVTG：通过轻量级slot适配器增强多模态大语言模型的细粒度时间定位能力，提升跨域泛化性能


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频时间定位任务上表现良好，但细粒度识别能力不足，需要任务特定的微调。这种微调导致模型记忆数据集特定的捷径而非真正基于视觉内容进行定位，导致跨域泛化能力差。需要一种低成本的方法来引导模型进行面向对象的视觉推理。

Method: 提出SlotVTG框架，引入轻量级slot适配器，通过slot attention将视觉token分解为抽象slot并重建原始序列。利用自监督视觉模型的对象性先验促进语义一致的slot形成，以最小成本引导MLLMs进行面向对象的、基于输入的视觉推理。

Result: 在标准VTG基准测试的跨域评估中，该方法显著提高了跨域鲁棒性，同时保持了具有竞争力的域内性能，且开销最小。

Conclusion: SlotVTG通过轻量级适配器有效解决了MLLMs在视频时间定位任务中的细粒度理解和跨域泛化问题，为面向对象的视觉推理提供了一种高效解决方案。
...
TL;DR: LIGHT提出了一种基于数据驱动的扩散模型方法，通过异步去噪调度和模态特定噪声水平分配，实现无需手动设计先验的人类-物体交互动画生成。


<details>
  <summary>Details</summary>
Motivation: 生成真实的人类-物体交互动画具有挑战性，需要同时建模动态的人类动作和多样化的物体几何形状。现有的扩散方法通常依赖手工制作的接触先验或人为施加的运动学约束来改善接触质量。

Method: 基于扩散强迫技术，将表示分解为模态特定组件，并分配个体化的噪声水平与异步去噪调度。在这种范式下，更干净的组件通过交叉注意力引导噪声更大的组件，无需辅助分类器。训练时使用广泛的合成物体几何数据进行增强，鼓励接触语义对几何多样性的不变性。

Result: 实验表明，节奏诱导的引导比传统的无分类器引导更能有效模拟接触先验的好处，同时实现更高的接触保真度、更真实的HOI生成，以及对未见过的物体和任务更强的泛化能力。

Conclusion: LIGHT提供了一种数据驱动的替代方案，其中引导从去噪节奏本身产生，减少了对手动设计先验的依赖，能够生成更真实、接触感知的人类-物体交互动画。
...
TL;DR: PSDesigner是一个模仿人类设计师创意流程的自动化平面设计系统，通过构建CreativePSD数据集训练模型掌握专业设计工具操作，能够根据用户指令生成可编辑的设计文件。
...
Method: 基于多个专用组件构建PSDesigner系统，模仿人类设计师工作流程：收集主题相关素材，自主推断并执行工具调用来操作设计文件。构建CreativePSD数据集，包含大量高质量PSD文件及其操作轨迹，涵盖广泛设计场景和艺术风格。

Result: PSDesigner在多样化的平面设计任务中优于现有方法，使非专业人士能够方便地创建生产质量的设计作品。

Conclusion: PSDesigner通过模仿人类设计师的创意工作流程和构建专业数据集，成功实现了将用户意图转换为可编辑设计文件的自动化平面设计系统。
...
TL;DR: MegaFlow是一个利用预训练视觉Transformer特征的零样本大位移光流估计模型，将光流估计转化为全局匹配问题，无需特定领域微调即可在多个基准测试中达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大位移光流估计方法通常依赖于迭代局部搜索和/或特定领域微调，这严重限制了它们在大位移和零样本泛化场景中的性能。需要一种能够克服这些限制的简单而强大的模型。

Method: MegaFlow通过利用预训练的全局Vision Transformer特征，将光流估计制定为全局匹配问题，这些特征自然捕捉大位移。然后通过少量轻量级迭代细化进一步提高亚像素精度，而不是依赖高度复杂的任务特定架构设计。

Result: MegaFlow在多个光流基准测试中实现了最先进的零样本性能，同时在长程点跟踪基准测试中也表现出高度竞争力的零样本性能，展示了其强大的可迁移性。

Conclusion: MegaFlow为可泛化的运动估计提供了一个统一的范式，通过利用预训练的视觉先验来产生时间一致的运动场，无需特定领域微调即可有效处理大位移光流估计问题。
...
TL;DR: RefAlign是一个用于参考图像到视频生成任务的表示对齐框架，通过将扩散Transformer中的参考分支特征与视觉基础模型的语义空间对齐，解决了现有方法中的复制粘贴伪影和多主体混淆问题。


<details>
  <summary>Details</summary>
Motivation: 现有参考图像到视频生成方法通常将参考图像的VAE潜在表示与额外的高层语义或跨模态特征一起输入扩散Transformer，这些辅助表示虽然能提供语义指导并缓解像素级信息泄露，但仍难以解决由异构编码器特征间的模态不匹配导致的复制粘贴伪影和多主体混淆问题。

Method: 提出RefAlign表示对齐框架，核心是参考对齐损失函数：将扩散Transformer参考分支特征与视觉基础模型的语义空间进行显式对齐。该损失函数将同一主体的参考特征和VFM特征拉近以提升身份一致性，同时将不同主体的对应特征推远以增强语义区分性。该方法仅在训练时应用，不增加推理时间开销。

Result: 在OpenS2V-Eval基准测试上的大量实验表明，RefAlign在TotalScore指标上优于当前最先进的方法，验证了显式参考对齐在R2V任务中的有效性。

Conclusion: RefAlign通过显式对齐扩散Transformer参考分支特征与视觉基础模型的语义空间，在文本可控性和参考保真度之间实现了更好的平衡，为参考图像到视频生成任务提供了一种简单而有效的解决方案。
...
TL;DR: MuRF是一种无需训练的多分辨率融合策略，通过处理图像在多个分辨率下的特征并融合，提升视觉基础模型的推理性能，适用于多种架构和任务。
...
Method: 提出Multi-Resolution Fusion (MuRF)策略，通过冻结的视觉基础模型处理图像在多个分辨率下的特征，然后将这些特征融合成一个统一的表示。这是一种无需训练、架构无关的通用增强方法。

Result: MuRF在广泛的计算机视觉任务中得到实证验证，适用于多种不同的视觉基础模型家族，主要基于DINOv2，也能成功泛化到对比学习模型如SigLIP2。

Conclusion: MuRF是一种简单而普遍有效的策略，能够利用多分辨率视图的协同效应，作为视觉表示的基本训练免费增强方法，具有广泛的适用性。
...
TL;DR: LGTM提出了一种前馈式3D高斯泼溅框架，通过预测紧凑的高斯基元配合每个基元的纹理，将几何复杂度与渲染分辨率解耦，实现了无需场景优化即可进行4K高保真新视角合成。
...
Result: LGTM实现了高保真的4K新视角合成，无需进行场景优化，这是前馈方法之前无法实现的能力，同时使用的基元数量显著减少。
...
TL;DR: ShotStream是一个因果多镜头视频生成架构，通过双向模型蒸馏和双缓存机制，实现交互式实时叙事视频生成，达到16 FPS的单GPU性能。


<details>
  <summary>Details</summary>
Motivation: 当前双向架构的多镜头视频生成存在交互性有限和延迟高的问题，难以支持实时交互式叙事。需要一种能够动态响应用户指令、低延迟生成连贯多镜头视频的解决方案。

Method: 1. 将任务重新定义为基于历史上下文的下一个镜头生成；2. 通过分布匹配蒸馏将双向文本到视频模型蒸馏为因果学生模型；3. 引入双缓存记忆机制（全局上下文缓存和局部上下文缓存）保持视觉一致性；4. 使用RoPE不连续性指示器区分两种缓存；5. 采用两阶段蒸馏策略缓解错误累积。

Result: ShotStream在单GPU上实现16 FPS的亚秒级延迟，生成连贯的多镜头视频，质量匹配或超过较慢的双向模型，支持实时交互式叙事。

Conclusion: ShotStream通过因果架构、双缓存机制和两阶段蒸馏，成功解决了多镜头视频生成的交互性和延迟问题，为实时交互式叙事开辟了新途径。
...
TL;DR: DyMRL是一个动态多空间表示学习方法，用于事件预测中的多模态知识获取与融合，通过整合欧几里得、双曲和复数空间的特征，结合双融合-进化注意力机制，在四个多模态时序知识图谱基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注静态设置，忽略了多模态知识的动态获取与融合。具体问题包括：1) 知识获取层面难以学习不同模态的时间敏感信息，特别是动态结构模态；2) 知识融合层面难以学习演化的多模态融合特征，静态共注意力方法无法捕捉不同模态对历史事件变化的贡献。

Method: DyMRL整合了欧几里得、双曲和复数空间的时间特定结构特征到关系消息传递框架中，以学习深度表示，反映人类在联想思维、高阶抽象和逻辑推理方面的智能。预训练模型赋予DyMRL时间敏感的视觉和语言智能。采用先进的双融合-进化注意力机制，以对称方式在不同时间戳为不同模态分配动态学习重点。

Result: 构建了四个多模态时序知识图谱基准进行评估。大量实验表明，DyMRL在事件预测性能上优于最先进的动态单模态和静态多模态基线方法。

Conclusion: DyMRL通过动态多空间表示学习，有效解决了多模态知识获取与融合中的时间敏感性问题，在事件预测任务中表现出优越性能，为真实场景中的多模态知识表示提供了有效解决方案。
...
TL;DR: ERL框架通过从单次尝试经验中提取可迁移的启发式规则，使自主智能体能够快速适应新环境，在Gaia2基准上比ReAct基线提升7.8%成功率


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型驱动的自主智能体在适应专门环境方面存在困难，且无法有效利用过往经验，每次面对新任务都从零开始，缺乏从历史交互中学习的能力

Method: 提出Experiential Reflective Learning (ERL)框架：1) 反思任务轨迹和结果生成启发式规则；2) 在测试时根据当前任务检索相关启发式规则；3) 将规则注入智能体上下文指导执行

Result: 在Gaia2基准上，ERL比ReAct基线提升7.8%成功率，在任务完成可靠性方面有显著提升，优于先前的经验学习方法。系统消融实验表明选择性检索至关重要，且启发式规则比少样本轨迹提示更具可迁移性

Conclusion: 通过反思单次尝试经验来提取可迁移的启发式规则，能够实现有效的智能体自我改进，为自主智能体的环境适应提供了简单而有效的解决方案
...
TL;DR: 随机裁剪作为计算机视觉中常见的数据增强技术，其内在的随机性在差分隐私机器学习训练中未被充分探索。研究发现，当图像中的敏感内容（如人脸、车牌）空间局部化时，随机裁剪可以概率性地将这些内容排除在模型输入之外，从而在不改变模型架构或训练过程的情况下增强差分隐私保护。


<details>
  <summary>Details</summary>
Motivation: 随机裁剪是计算机视觉中最常见的数据增强技术之一，但其在差分隐私机器学习训练中的随机性作用尚未被探索。当图像中的敏感内容（如人脸、车牌）空间局部化时，随机裁剪可以概率性地排除这些内容，这为增强隐私保护提供了新的可能性。

Method: 1. 引入视觉数据的补丁级相邻关系概念；2. 推导随机裁剪与差分隐私随机梯度下降（DP-SGD）结合时的严格隐私边界；3. 量化补丁包含概率，分析其与小批量采样的组合效应；4. 通过实验验证补丁级隐私增强在多个分割架构和数据集上的效果。

Result: 分析表明，随机裁剪通过降低有效采样率来增强差分隐私保护。实验验证了补丁级隐私增强在多个分割架构和数据集上改善了隐私-效用权衡。结果表明，将隐私计算与领域结构和现有随机性源对齐可以在不增加成本的情况下获得更强的隐私保证。

Conclusion: 随机裁剪的内在随机性为差分隐私机器学习训练提供了额外的隐私增强机制。通过形式化补丁级相邻关系并量化随机裁剪的隐私效应，研究展示了如何利用现有的数据增强技术在不改变模型架构或训练过程的情况下获得更强的隐私保证。这为将领域特定结构与隐私计算对齐提供了新的思路。
...
TL;DR: 提出一个解决多步工具编排挑战的强化学习框架，包含真实API响应缓存环境和分级奖励设计，显著提升复杂函数调用准确性。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在多步工具编排（需要按正确顺序调用多个依赖API并传递中间输出）方面存在挑战，现有方法存在参数值错误率高、训练环境简单、二进制奖励无法识别部分正确性等问题。

Method: 1) 构建基于大规模真实API响应缓存的强化学习环境，支持可控复杂度的多步编排轨迹数据合成；2) 提出分级奖励设计，将正确性分解为原子有效性（单个函数调用正确性）和编排正确性（工具顺序和依赖关系）。

Result: 在ComplexFuncBench基准测试中，该方法在轮次准确性方面取得显著提升。消融研究表明两个奖励组件都至关重要：单独使用任一组件的性能都会显著下降。

Conclusion: 提出的框架通过真实API环境和高粒度奖励设计，有效解决了多步工具编排的训练挑战，为复杂工作流执行提供了更可靠的解决方案。
...
TL;DR: 本文提出了一个用于模拟电路尺寸优化的演员-评论家框架（ACOF），通过分离设计空间探索和评估的角色，将设计师的判断力融入优化循环，显著提升了优化效果。


<details>
  <summary>Details</summary>
Motivation: 模拟电路设计面临的主要挑战是：即使对器件尺寸或偏置进行微小改动也需要昂贵的仿真周期，而高质量解决方案通常只占据庞大搜索空间的狭窄区域。现有优化器虽然减轻了部分负担，但缺乏设计师在决定下一步搜索方向时使用的判断力。

Method: 提出了演员-评论家优化框架（ACOF），将优化过程分为两个角色：演员负责提出设计空间中有前景的区域，评论家则评估这些选择、强制执行设计合法性，并在进展受阻时重新定向搜索。这种结构保持了与标准基于仿真的流程的兼容性，同时使搜索过程更加深思熟虑、稳定和可解释。

Result: 在测试电路中，ACOF将前10个品质因数平均提升了38.9%，超过了最强的竞争基准；平均减少了24.7%的遗憾值；在单个电路上实现了70.5%的品质因数提升和42.2%的遗憾值降低。

Conclusion: 通过将迭代推理与仿真驱动的搜索相结合，该框架为在具有挑战性的设计空间中实现自动化模拟电路尺寸优化提供了一条更加透明的路径。
...
TL;DR: 该研究为"grokking"现象提供了可检验的相变理论框架，通过将群阶p作为扩展变量，使用谱头尾对比作为序参量，应用凝聚态物理诊断方法证实了有限尺寸相变的存在，而非平滑交叉。


<details>
  <summary>Details</summary>
Motivation: 虽然grokking现象（早期记忆化后延迟出现泛化）常被用相变语言描述，但缺乏可证伪的有限尺寸输入。本研究旨在为这种描述提供可检验的定量框架，将相变语言从类比提升为可测试的科学主张。

Method: 将群阶p作为可接受的扩展变量，使用保留的谱头尾对比作为表示层面的序参量。应用凝聚态物理风格的诊断链：1）粗网格扫描；2）密集近临界加法审计。使用类似Binder交叉的方法揭示共享的有限尺寸边界，并通过敏感性比较评估平滑交叉解释。

Result: Binder-like交叉揭示了共享的有限尺寸边界，敏感性比较强烈不支持平滑交叉解释（近临界审计中ΔAIC=16.8）。这表明grokking中的相变语言可以作为定量有限尺寸主张进行测试，而不仅仅是类比。

Conclusion: grokking现象中的相变语言可以作为可检验的定量有限尺寸主张，而不仅仅是类比。虽然相变阶数目前仍未解决，但研究为这一现象提供了可证伪的理论框架，将相变描述从隐喻提升为可测试的科学假设。
...
TL;DR: 论文提出Worldline Slot Attention模型，将物体建模为时空世界线中的持久轨迹，使用洛伦兹几何结构显著提升层次物体发现性能
...
Method: 引入Worldline Slot Attention，将物体建模为时空世界线中的持久轨迹。每个物体在不同层次级别有多个槽位，共享相同的空间位置但具有不同的时间坐标。比较了欧几里得世界线和洛伦兹世界线两种几何结构。

Result: 欧几里得世界线准确率仅为0.078（低于随机机会0.33），而洛伦兹世界线在三个数据集上达到0.479-0.661的准确率，提升了6倍。洛伦兹几何也优于双曲嵌入，表明视觉层次需要因果结构而非树状结构。

Conclusion: 层次物体发现需要编码非对称因果关系的几何结构，这是欧几里得空间所缺乏但洛伦兹光锥自然具备的归纳偏置。仅用11K参数就实现了这一效果。
...
TL;DR: 本文研究LLM能否近似搜索算法，提出"未知树搜索与老虎机反馈"框架，证明Transformer理论上能实现不同搜索策略，并能通过训练泛化到未见条件。
...
Method: 引入"未知树搜索与老虎机反馈"简化框架，其中树扩展和反馈信号由外部指定。在此框架下训练Transformer模型实现搜索策略，并通过任务导向的持续训练解锁预训练LLM的完整能力。

Result: Transformer理论上能表达不同搜索策略，能从零开始训练近似这些策略，并能泛化到更长视野或更深树等未见条件。任务导向的持续训练能有效解锁预训练LLM的搜索能力。

Conclusion: LLM能够近似搜索算法，这为简化问题解决流程提供了可能，通过适当的训练方法，Transformer架构能够实现有效的搜索策略并具备良好的泛化能力。
...
TL;DR: 该研究提出了一个评估临床预测模型可解释性方法的综合基准，发现注意力机制在正确使用时是高效的解释方法，而黑盒解释器在时间序列任务中计算不可行，许多方法可靠性不足。


<details>
  <summary>Details</summary>
Motivation: 临床决策需要明确解释，但现有可解释性评估缺乏可扩展性和可重复性，未能系统考察不同临床任务和模型架构下可解释性的变化。

Method: 开发了一个综合基准，在多样化的临床预测任务和模型架构上评估多种可解释性方法，并通过PyHealth开源框架提供实现。

Result: 发现：1）注意力机制在正确使用时是高效的解释方法；2）KernelSHAP和LIME等黑盒解释器在时间序列临床预测任务中计算不可行；3）多种可解释性方法可靠性不足。

Conclusion: 提出了改进临床预测流程可解释性的指南，并通过开源框架PyHealth支持可重复性和可扩展性。
...
TL;DR: 提出将流匹配扩展到齐次空间（李群商空间）的通用框架，通过将数据分布提升到李群上，避免齐次空间的复杂几何，最终简化为李代数上的欧几里得流匹配任务


<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法在处理齐次空间（李群商空间）时面临几何复杂性挑战，需要定义和计算预度量或测地线，计算复杂且效率较低

Method: 通过将齐次空间上的数据分布提升到李群上，将问题转化为李群上的流匹配任务，然后进一步简化为李代数上的欧几里得流匹配，避免直接处理齐次空间的复杂几何

Result: 相比黎曼流匹配，该方法消除了定义和计算预度量或测地线的需求，实现了更简单、更快、完全内蕴的框架

Conclusion: 提出的框架为齐次空间上的流匹配提供了一种简化且高效的方法，通过李群提升策略避免了复杂几何处理，显著降低了计算复杂度
...
TL;DR: 本文提出了一种多答案强化学习方法，训练语言模型在推理时对多个答案进行分布推理，避免后处理过程将答案分布坍缩到单一主导模式。
...
Result: 在问答、医疗诊断和编程基准测试中，相比单答案训练基线，该方法在多样性、覆盖率和集合级校准分数方面均有改善。在编程任务中准确性显著提高，且生成多个答案所需token数少于竞争方法。

Conclusion: 多答案RL为推理时扩展程序（如best-of-k）提供了一个原则性且计算高效的替代方案，使语言模型能够更好地处理具有多个有效答案的现实世界任务。
...
TL;DR: 研究比较了两种机器学习方法优化半自动化仓库分拣系统的实时人员配置决策：基于Transformer的离线强化学习策略和基于LLM的抽象决策方法


<details>
  <summary>Details</summary>
Motivation: 研究如何利用机器学习优化半自动化仓库分拣系统的实时人员配置决策，支持不同抽象层次的运营决策，平衡不同方法的优缺点

Method: 评估两种方法：1）使用离线强化学习训练基于Transformer的定制策略，基于详细历史状态表示；2）探索LLM在抽象、人类可读状态描述上的应用，系统比较提示技术、自动提示优化和微调策略

Result: 离线强化学习方法在学习的模拟器中实现了2.4%的吞吐量提升；LLM方法中，监督微调结合Direct Preference Optimization在模拟器生成的偏好上，性能达到或略超过历史基线
...
<details>
  <summary>Details</summary>
Motivation: 解决视频时序定位中的关键挑战：实现对无关背景上下文鲁棒的时间敏感视频文本对齐，避免因上下文干扰导致的"假阴性"问题

Method: 1. 查询感知上下文多样化(QCD)：基于视频文本相似性的数据增强策略，混合语义无关内容；2. 上下文不变边界判别(CBD)损失：对比损失强制语义一致性；3. 上下文增强Transformer编码器(CTE)：分层架构结合窗口自注意力和双向交叉注意力

Result: 在QVHighlights和Charades-STA等主要VTG基准测试中达到最先进性能，Recall@1(R1)分数比现有方法提升约5个点，显著减少假阴性

Conclusion: 通过数据中心和架构增强的协同作用，CVA框架有效解决了视频时序定位中的上下文干扰问题，实现了更鲁棒的视频文本对齐
...
TL;DR: 本文解决了在线凸优化中两点反馈的强凸损失高概率遗憾界问题，首次实现了最优的O(d(log T + log(1/δ))/μ)遗憾界。
...
Method: 通过解决带反馈梯度估计器的重尾特性问题，开发了新的分析方法来获得高概率遗憾界。

Result: 首次实现了μ-强凸损失的高概率遗憾界O(d(log T + log(1/δ))/μ)，该结果在时间范围T和维度d方面都是极小极大最优的。

Conclusion: 本文解决了Agarwal等人提出的开放性问题，为在线凸优化中两点反馈的强凸损失提供了最优的高概率遗憾界。
...
TL;DR: 论文提出"认知压缩"原则：在高风险领域，模型稳健性源于模型复杂度与数据有效期的匹配，而非参数扩展。通过"制度指数"区分稳定与不稳定领域，指导模型架构设计。
...
Conclusion: 高风险AI需要从为扩展而扩展转向有原则的简约性。模型稳健性源于模型复杂度与数据有效期的匹配，而非单纯增加参数规模。
...
TL;DR: Intern-S1-Pro是首个万亿参数科学多模态基础模型，在通用和科学领域均有全面提升，具备更强的推理、图文理解和智能体能力，掌握100多个科学领域的专业任务。


<details>
  <summary>Details</summary>
Motivation: 构建一个能够同时处理通用任务和深度科学专业任务的大规模多模态基础模型，实现通用智能与专业智能的融合，突破现有开源模型在科学专业领域的局限性。

Method: 通过XTuner和LMDeploy基础设施支持，在万亿参数级别实现高效强化学习训练，确保训练和推理的精度一致性，同时扩展模型在化学、材料、生命科学、地球科学等领域的专业能力。

Result: 模型在通用能力方面达到开源模型顶级水平，在科学专业任务深度方面超越专有模型，成功掌握100多个科学领域的专业任务，实现了通用智能与专业智能的有效融合。

Conclusion: Intern-S1-Pro作为首个万亿参数科学多模态基础模型，成功展示了大规模模型在融合通用和专业智能方面的潜力，为科学研究和应用提供了强大的基础模型支持。
...
TL;DR: SIGMA通过token级对比学习解决分子图线性化表示中的多义性问题，结合同构束搜索提升生成效率


<details>
  <summary>Details</summary>
Motivation: 分子图的线性化字符串表示存在多义性问题——同一个分子图对应多个不同的序列表示，这导致轨迹发散问题，即结构相同的部分图因其线性化历史不同而在潜在表示中产生差异

Method: 提出SIGMA方法：1) 通过token级对比学习目标，显式对齐具有相同后缀的前缀的潜在状态，使模型能严格识别几何对称性；2) 引入同构束搜索(IsoBeam)，在推理时动态剪枝等价路径以消除同构冗余

Result: 在标准基准测试中，SIGMA在序列可扩展性和图保真度之间架起桥梁，相比强基线在多参数优化中展现出更优的样本效率和结构多样性

Conclusion: SIGMA方法在不放弃高效字符串表示的前提下，解决了线性化分子生成中的多义性问题，实现了更好的生成质量和效率
...
TL;DR: MCP模型通过逐步嵌入水文物理过程约束，在保持可解释性的同时提升降雨径流预测精度，其性能受水文气候条件影响显著。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在水文应用中虽然预测精度高，但通常缺乏物理可解释性。本研究旨在探索如何在保持物理可解释性的同时，通过逐步嵌入水文过程约束来提升模型的预测性能。

Method: 研究采用质量守恒感知器（MCP）框架，从最小化MCP公式开始，逐步引入有物理意义的水文过程表示：有界土壤储水、状态相关导水性、可变孔隙度、入渗能力、地表积水、垂直排水和非线性水位动态。在15个美国大陆不同水文气候区域的流域上评估这一系列过程感知MCP模型，以日径流预测为目标。

Result: 逐步增强MCP单元内部物理结构通常能改善预测性能。这些过程表示的影响强烈依赖于水文气候条件：垂直排水在干旱和雪主导流域显著提升模型技能，但在降雨主导区域降低性能；地表积水影响相对较小。性能最佳的MCP配置接近LSTM基准的预测技能，同时保持明确的物理可解释性。

Conclusion: 在水文AI架构中嵌入过程约束为可解释且过程感知的降雨径流建模提供了有前景的途径，平衡了预测精度和物理可解释性。
...
TL;DR: 该研究为鲁棒支持向量机（R-SVMs）开发了安全样本筛选规则，通过识别那些不确定性集完全位于间隔超平面一侧的训练样本，在不影响最优解的情况下降低训练复杂度。


<details>
  <summary>Details</summary>
Motivation: R-SVMs通过采用最坏情况鲁棒公式来应对特征噪声，但这种方法增加了计算成本。需要开发能够减少训练复杂度而不影响最优解的方法。

Method: 基于拉格朗日对偶性（而非常用的Fenchel-Rockafellar对偶）推导安全筛选规则，首先建立理想筛选规则，然后通过将GAP-based安全区域适应到鲁棒设置中推导出实用规则。

Result: 实验表明，该方法显著减少了训练时间，同时保持了分类准确性。这是首次将安全筛选技术应用于监督机器学习中的最坏情况鲁棒模型。

Conclusion: 提出的安全样本筛选规则有效解决了R-SVMs计算成本高的问题，通过安全识别对最终解无贡献的样本，在不影响模型性能的前提下加速优化过程。
...
### [146] [How Pruning Reshapes Features: Sparse Autoencoder Analysis of Weight-Pruned Language Models](https://arxiv.org/abs/2603.25325)
*Hector Borobia,Elies Seguí-Mas,Guillermina Tormo-Carbó*

Main category: cs.LG

TL;DR: 本文首次系统研究非结构化剪枝如何重塑语言模型的特征几何结构，发现罕见特征比高频特征更能抵抗剪枝，剪枝起到隐式特征选择作用。


<details>
  <summary>Details</summary>
Motivation: 权重剪枝是压缩大语言模型的标准技术，但其对学习到的内部表征的影响仍不清楚。本文旨在系统研究非结构化剪枝如何重塑语言模型的特征几何结构。

Method: 使用稀疏自编码器作为可解释性探针，研究三个模型家族、两种剪枝方法和六个稀疏度水平。通过五个研究问题分析特征稳定性、生存率、可迁移性、脆弱性和因果相关性。

Result: 最显著发现：罕见SAE特征（低激活率）比高频特征更能抵抗剪枝，在17个实验条件中有11个显示Spearman相关系数ρ=-1.0。Wanda剪枝比幅度剪枝能更好地保留特征结构（高达3.7倍），预训练SAE在Wanda剪枝模型中保持有效至50%稀疏度，但几何特征生存率不能预测因果重要性。

Conclusion: 剪枝起到隐式特征选择作用，优先破坏高频通用特征而保留专业罕见特征。特征几何生存率与因果重要性之间存在解耦，这对压缩下的可解释性研究具有重要意义。
...
### [147] [From Intent to Evidence: A Categorical Approach for Structural Evaluation of Deep Research Agents](https://arxiv.org/abs/2603.25342)
*Shuoling Liu,Zhiquan Tan,Kun Yi,Hui Wu,Yihan Li,Jiangpeng Yan,Liyuan Chen,Kai Chen,Qiang Yang*

Main category: cs.LG

TL;DR: 该研究通过范畴论形式化深度研究智能体行为，开发了包含296个问题的机制感知基准测试，评估了11个领先模型，发现现有AI在复杂结构信息处理上存在严重能力分化


<details>
  <summary>Details</summary>
Motivation: 当前深度研究智能体的评估主要依赖启发式基准测试，缺乏对智能体行为的严格建模，也无法充分测试长时程信息合成和模糊性解析能力。需要建立理论框架来形式化评估深度研究智能体的结构理解能力

Method: 使用范畴论将深度研究工作流程建模为结构保持映射（函子）的组合，基于此理论框架开发了包含296个问题的机制感知基准测试，设计了四个可解释的评估维度：顺序连接链遍历、V结构拉回中的交集验证、检索子结构的拓扑排序、以及通过Yoneda探针进行本体证伪

Result: 对11个领先模型的评估显示基准性能极低，最先进模型平均准确率仅为19.9%。研究发现AI能力存在明显分化：先进深度研究管道在动态拓扑重排序和本体验证方面表现良好，但在多跳结构合成方面几乎完全失效。任务间巨大性能差异表明现有系统仍依赖脆弱的启发式方法而非系统性理解

Conclusion: 虽然顶级自主智能体能够有机整合搜索和推理，但在复杂结构信息的通用掌握方面仍面临严峻挑战。该工作揭示了当前AI在形式化结构压力测试中的根本局限性，为未来研究指明了方向
...
### [148] [Hessian-informed machine learning interatomic potential towards bridging theory and experiments](https://arxiv.org/abs/2603.25373)
*Bangchen Yin,Jian Ouyang,Zhen Fan,Kailai Lin,Hanshi Hu,Dingshun Lv,Weiluo Ren,Hai Xiao,Ji Chen,Changsu Cao*

Main category: cs.LG

TL;DR: Hi-MLIP：一种Hessian感知的机器学习原子间势能，通过HINT高效训练协议，大幅减少Hessian标签需求，实现准确的热力学和动力学现象分析


<details>
  <summary>Details</summary>
Motivation: 势能面的局部曲率对于从第一性原理预测分子和材料的实验观测至关重要，但对于复杂系统仍然难以获取。现有方法在捕捉曲率方面存在局限性，需要开发能够可靠捕获曲率的机器学习原子间势能

Method: 提出Hessian感知的机器学习原子间势能（Hi-MLIP），开发高效训练协议HINT，包括Hessian预训练、构型采样、课程学习和随机投影Hessian损失等技术，将昂贵Hessian标签需求降低2-4个数量级

Result: Hi-MLIP显著改进过渡态搜索，将吉布斯自由能预测接近化学精度（尤其在数据稀缺情况下）；准确处理强非谐氢化物，再现声子重整化和超导临界温度，与实验结果高度一致，同时绕过非谐计算的计算瓶颈

Conclusion: 该框架为增强机器学习原子间势能的曲率感知提供了实用途径，在广泛系统中架起了模拟与实验观测之间的桥梁，实现了对热力学和动力学现象的准确分析
...
### [149] [GlowQ: Group-Shared LOw-Rank Approximation for Quantized LLMs](https://arxiv.org/abs/2603.25385)
*Selim An,Il hong Suh,Yeseong Kim*

Main category: cs.LG

TL;DR: GlowQ是一种用于量化LLM的组共享低秩近似方法，通过缓存共享右因子和选择性恢复关键层，在保持精度的同时显著降低延迟和内存开销。
...
Method: 提出GlowQ方法：1）基于输入共享组缓存单个共享右因子；2）仅恢复提供最高精度收益的组或层；3）每个输入共享组计算一次高精度投影并在其模块中重用；4）提出选择性变体GlowQ-S，仅在提供最大收益的地方应用缓存共享模块。

Result: 相比基线方法：GlowQ平均减少TTFB 5.6%，增加吞吐量9.6%，在WikiText-2上降低困惑度0.17%，下游任务准确率提高0.42个百分点。GlowQ-S进一步减少延迟，TTFB降低23.4%，吞吐量增加37.4%，同时平均准确率保持在0.2个百分点内。

Conclusion: GlowQ通过组共享低秩近似和选择性恢复策略，在保持量化模型精度的同时显著降低了延迟和内存开销，为高效部署大型语言模型提供了有效解决方案。
...
### [150] [Maximum Entropy Behavior Exploration for Sim2Real Zero-Shot Reinforcement Learning](https://arxiv.org/abs/2603.25464)
*Jiajun Hu,Nuria Armengol Urpi,Jin Cheng,Stelian Coros*

Main category: cs.LG

TL;DR: FB-MEBE算法：结合无监督行为探索与正则化批评器的在线零样本强化学习，用于四足机器人控制，无需下游任务知识即可收集多样化数据，生成可直接部署到硬件的自然策略。


<details>
  <summary>Details</summary>
Motivation: 零样本RL算法需要从无奖励数据集中学习策略族，但预收集相关且多样化的数据集面临挑战。现有无向探索方法产生的数据多样性低，导致下游性能差，且策略无法直接部署到硬件。

Method: 提出FB-MEBE算法：1) 无监督行为探索策略，通过最大化已实现行为分布的熵来促进探索；2) 正则化批评器，将恢复的策略塑造成更自然、物理上合理的行为。

Result: 在模拟下游任务中，FB-MEBE相比其他探索策略获得改进性能；生成的策略自然且可直接部署到硬件，无需进一步微调。

Conclusion: FB-MEBE通过结合行为探索和正则化，有效解决了在线零样本RL中的数据多样性问题，实现了可直接硬件部署的自然四足机器人控制策略。
...
### [151] [Not a fragment, but the whole: Map-based evaluation of data-driven Fire Danger Index models](https://arxiv.org/abs/2603.25469)
*Shahbaz Alvi,Italo Epicoco,Jose Maria Costa Saura*

Main category: cs.LG

TL;DR: 该论文提出了一种与真实世界决策对齐的森林火灾预报模型评估新方法，重点关注准确预测火灾活动和减少误报，并证明ML模型集成能改善火灾识别和降低误报率。
...
Method: 重新审视每日火灾危险指数模型评估范式，提出与真实世界决策对齐的森林火灾预报模型评估新方法，系统评估准确预测火灾活动和误报的性能，并采用机器学习模型集成方法。

Result: 机器学习模型集成能同时改善火灾识别和减少误报，证明了所提评估方法在实际操作中的有效性。

Conclusion: 需要重新思考火灾预报模型的评估方式，采用与操作决策更一致的评估框架，机器学习模型集成是提高火灾预测准确性和减少误报的有效策略。
...
### [152] [Causal-INSIGHT: Probing Temporal Models to Extract Causal Structure](https://arxiv.org/abs/2603.25473)
*Benjamin Redden,Hui Wang,Shuyan Li*

Main category: cs.LG

TL;DR: Causal-INSIGHT是一个模型无关的后处理解释框架，用于从已训练的时序预测器中提取模型隐含的、有向的、有时滞的影响结构，而不是推断数据生成过程的因果结构。


<details>
  <summary>Details</summary>
Motivation: 理解多元时间序列中的有向时序交互对于解释复杂动态系统及其训练的预测模型至关重要。需要一种能够从已训练预测器中提取模型依赖的时序影响结构的解释方法。

Method: Causal-INSIGHT通过分析固定预训练预测器对推理时系统性、干预式输入钳制的响应来构建有向时序影响信号。引入Qbic标准，这是一个稀疏感知的图选择准则，平衡预测保真度和结构复杂度，无需真实图标签。

Result: 在合成、模拟和现实基准测试中，Causal-INSIGHT能够泛化到不同的骨干架构，保持竞争力的结构准确性，并在应用于现有预测器时显著改善时序延迟定位性能。

Conclusion: Causal-INSIGHT提供了一个有效的框架，用于从已训练的时间序列预测器中提取模型隐含的有向时序影响结构，有助于理解预测模型所依赖的依赖关系。
...
### [153] [How Class Ontology and Data Scale Affect Audio Transfer Learning](https://arxiv.org/abs/2603.25476)
*Manuel Milling,Andreas Triantafyllopoulos,Alexander Gebhard,Simon Rampp,Björn W. Schuller*

Main category: cs.LG

TL;DR: 音频到音频迁移学习研究：通过预训练AudioSet子集并在三个下游任务微调，发现预训练数据量和类别数对迁移学习有正面影响，但预训练与下游任务相似性影响更大


<details>
  <summary>Details</summary>
Motivation: 尽管迁移学习在深度学习中广泛应用且效果明显，但其内在工作机制仍有许多未解之谜，特别是关于何时以及为何有效的问题。本研究旨在通过音频到音频迁移学习的系统性研究来深入理解迁移学习的工作原理。
...
Conclusion: 迁移学习效果不仅取决于预训练数据的规模（样本量和类别数），更重要的是预训练任务与下游任务之间的相似性。相似性可以使模型学习到更相关的特征表示，从而在有限数据的下游任务上获得更好的性能。
...
### [154] [Interpretable PM2.5 Forecasting for Urban Air Quality: A Comparative Study of Operational Time-Series Models](https://arxiv.org/abs/2603.25495)
*Moazzam Umer Gondal,Hamad ul Qudous,Asma Ahmad Farhan,Sultan Alamri*

Main category: cs.LG

TL;DR: 本研究比较了三种轻量级可解释模型（SARIMAX、Facebook Prophet、NeuralProphet）在北京PM2.5小时预测中的性能，发现在不同自适应机制下，Facebook Prophet和SARIMAX表现优异，而NeuralProphet表现较差。


<details>
  <summary>Details</summary>
Motivation: 当前空气质量预测框架通常依赖复杂、数据密集且计算要求高的模型，本研究旨在探索轻量级、可解释的预测方法是否能在北京PM2.5小时预测中提供有竞争力的性能。

Method: 采用泄漏感知预测工作流程，结合时序数据分区、预处理、特征选择和外生驱动建模。评估了三种预测模型家族：SARIMAX、Facebook Prophet和NeuralProphet。在两种自适应机制下测试：每周前向滚动重新拟合和冻结预测结合在线残差修正。

Result: 在前向滚动重新拟合机制下，Facebook Prophet表现最佳（MAE 37.61，RMSE 50.10），且计算时间显著少于NeuralProphet。在冻结模型机制下，在线残差修正改善了Facebook Prophet和SARIMAX，修正后的SARIMAX达到最低总体误差（MAE 32.50，RMSE 46.85）。NeuralProphet在两个机制下都表现较差且不稳定。
...
### [155] [Revisiting On-Policy Distillation: Empirical Failure Modes and Simple Fixes](https://arxiv.org/abs/2603.25562)
*Yuqian Fu,Haohuan Huang,Kaiwen Jiang,Yuanheng Zhu,Dongbin Zhao*

Main category: cs.LG

TL;DR: 论文分析了在长视野任务中基于采样token的在线蒸馏方法的脆弱性，提出了基于教师top-K局部支持匹配的改进方法，在数学推理和多任务训练中表现更稳定且性能更好。


<details>
  <summary>Details</summary>
Motivation: 在线蒸馏在大语言模型后训练中很有吸引力，因为它基于学生生成的轨迹而非固定教师轨迹来评估教师反馈。但在长视野任务中，常见的采样token变体很脆弱：它将分布匹配简化为单token信号，且随着轨迹偏离教师常见前缀而变得不可靠。

Method: 从估计器和实现两方面重新审视在线蒸馏。提出教师top-K局部支持匹配方法，实现为带top-p轨迹采样和特殊token掩码的截断反向KL目标。

Result: 在单任务数学推理和多任务智能体加数学训练中，该方法比采样token在线蒸馏产生更稳定的优化和更好的下游性能。

Conclusion: 采样token在线蒸馏在长视野任务中存在三个失效模式：不平衡的单token信号、对学生生成前缀的不可靠教师指导、以及tokenizer或特殊token不匹配导致的失真。通过教师top-K局部支持匹配可以有效解决这些问题。
...
### [156] [Anchored-Branched Steady-state WInd Flow Transformer (AB-SWIFT): a metamodel for 3D atmospheric flow in urban environments](https://arxiv.org/abs/2603.25635)
...
<details>
  <summary>Details</summary>
Motivation: 传统CFD计算成本高昂，而现有深度学习模型难以适应城市几何结构的高度变化和大网格尺寸，需要开发更有效的替代模型。

Method: 提出Anchored Branched Steady-state WInd Flow Transformer (AB-SWIFT)，这是一种基于Transformer的模型，具有内部分支结构，专门为大气流动建模设计。模型在随机城市几何结构和混合大气稳定度（不稳定、中性和稳定）条件下的大气模拟数据库上进行训练。

Result: 在所有预测场上的精度均优于最先进的Transformer和基于图的方法。

Conclusion: AB-SWIFT能够有效处理城市空气流动建模中的几何变化和大网格挑战，为污染物扩散和风电场建模等应用提供了高效的深度学习替代方案。
...
TL;DR: U-Balance：利用行为不确定性解决CPS安全监测中类别不平衡问题的方法，通过不确定性引导的标签重平衡机制提升安全预测器性能


<details>
  <summary>Details</summary>
Motivation: 现实世界CPS操作中不安全事件稀少导致极端类别不平衡，传统重平衡技术在时间序列遥测数据上效果不佳，而CPS行为不确定性虽与安全结果相关但尚未在安全监测中探索

Method: 提出U-Balance方法：1) 训练基于GatedMLP的不确定性预测器，将遥测窗口总结为分布运动学特征并输出不确定性分数；2) 应用不确定性引导的标签重平衡机制，将高不确定性的安全标签窗口概率性重新标记为不安全；3) 在重平衡数据集上训练安全预测器

Result: 在46:1安全-不安全比例的大规模无人机基准测试中，U-Balance达到0.806 F1分数，比最强基线提升14.3个百分点，同时保持有竞争力的推理效率。行为不确定性与安全存在中等但显著的相关性
...
### [158] [On Neural Scaling Laws for Weather Emulation through Continual Training](https://arxiv.org/abs/2603.25687)
*Shashank Subramanian,Alexander Kiefer,Arnur Nigmetov,Amir Gholami,Dmitriy Morozov,Michael W. Mahoney*

Main category: cs.LG

TL;DR: 该论文研究了科学机器学习中的神经缩放定律，特别是在天气预报领域，使用Swin Transformer架构和持续训练策略，展示了可预测的缩放趋势和优于标准学习率调度的性能。


<details>
  <summary>Details</summary>
Motivation: 神经缩放定律在自然语言处理和计算机视觉中已成为构建基础模型的关键，但在科学机器学习领域的研究相对不足。本文旨在探索天气预报模型中的神经缩放行为，以了解如何通过缩放模型、数据和计算资源来优化性能。

Method: 采用最小化、可扩展的Swin Transformer架构，使用持续训练策略（恒定学习率配合周期性冷却阶段）作为高效训练方法。系统探索了不同模型大小、数据集大小和计算预算下的性能，构建了IsoFLOP曲线来识别计算最优的训练方案。

Result: 研究发现这种简约训练方法遵循可预测的缩放趋势，甚至优于标准余弦学习率调度。冷却阶段可用于改进下游性能，如实现更长的预测时间跨度和更清晰的预测。通过缩放趋势外推揭示了潜在的性能极限。

Conclusion: 神经缩放定律可作为科学机器学习中高效资源分配的重要诊断工具。研究为天气预报模型的缩放行为提供了系统分析，并开源代码以确保可复现性。
...
### [159] [A Unified Memory Perspective for Probabilistic Trustworthy AI](https://arxiv.org/abs/2603.25692)
*Xueji Zhao,Likai Pei,Jianbo Liu,Kai Ni,Ningyuan Cao*

Main category: cs.LG

TL;DR: 论文提出将确定性数据访问视为随机采样的特例，建立统一的数据访问框架，分析随机性需求对系统效率的影响，并定义内存级评估标准来指导可信AI硬件设计。


<details>
  <summary>Details</summary>
Motivation: 可信AI系统越来越多地依赖概率计算来实现鲁棒性、可解释性、安全性和隐私性。实际系统中，这些工作负载在模型、数据路径和系统功能之间交织确定性数据访问和重复随机采样，将性能瓶颈从算术单元转移到必须同时提供数据和随机性的内存系统。

Method: 提出统一的数据访问视角，将确定性访问视为随机采样的极限情况，使两种模式能在共同框架下分析。基于此视角定义内存级评估标准：统一操作、分布可编程性、效率、对硬件非理想性的鲁棒性以及并行兼容性。

Result: 分析表明，增加随机性需求会降低有效数据访问效率，并可能使系统进入熵受限操作状态。通过提出的评估标准分析了传统架构的局限性，并研究了新兴的概率计算内存方法，这些方法将采样与内存访问集成在一起。

Conclusion: 论文为可信AI的可扩展硬件设计提供了理论框架和评估标准，指出概率计算内存方法是解决随机性需求与内存系统效率之间矛盾的有前景途径。
...
### [160] [ARC-AGI-3: A New Challenge for Frontier Agentic Intelligence](https://arxiv.org/abs/2603.24621)
...
<details>
  <summary>Details</summary>
Motivation: 研究智能体智能，评估在全新任务上的流畅自适应效率，避免语言和外部知识的影响，专注于核心知识先验。

Method: 设计基于核心知识先验的交互式基准测试环境，采用回合制机制，通过广泛的人类测试者测试进行难度校准，建立基于人类行动基准的效率评分框架。

Result: 人类测试者能够解决100%的环境任务，而截至2026年3月，前沿AI系统的得分低于1%，显示出巨大的性能差距。

Conclusion: ARC-AGI-3提供了一个有效的基准测试框架，能够评估智能体在抽象环境中的自适应能力和规划效率，揭示了当前AI系统与人类智能体之间的显著差距。
...
### [161] [Trust as Monitoring: Evolutionary Dynamics of User Trust and AI Developer Behaviour](https://arxiv.org/abs/2603.24742)
...
TL;DR: 使用进化博弈论建模AI安全治理，将用户信任视为动态监控过程而非一次性选择，发现三个长期稳定状态，强调透明度、低成本监控和有效制裁的重要性


<details>
  <summary>Details</summary>
Motivation: 现有AI治理模型将用户信任视为一次性采纳选择，而非基于重复互动的动态过程。随着AI能力和应用的增加，AI安全问题日益紧迫，需要更准确地建模用户与开发者之间的信任演化

Method: 使用进化博弈论，将用户信任建模为重复不对称互动中的监控减少，其中检查AI行为需要成本。研究在不同监控成本和制度机制下，用户信任策略与开发者选择安全（合规）或危险（不合规）AI的共同演化。补充了无限人口复制者动态分析、随机有限人口动态和强化学习（Q-learning）模拟

Result: 发现三个稳健的长期状态：1）无采纳且危险开发；2）危险但广泛采纳的系统；3）安全且广泛采纳的系统。只有第三种状态是理想的，它出现在对危险行为的惩罚超过安全额外成本且用户仍能偶尔监控的情况下

Conclusion: 研究结果正式支持强调透明度、低成本监控和有意义制裁的治理提案，表明仅靠监管或盲目的用户信任都不足以防止向危险或低采纳结果的演化漂移
...
### [162] [ReLope: KL-Regularized LoRA Probes for Multimodal LLM Routing](https://arxiv.org/abs/2603.24787)
*Yaopei Zeng,Congchao Wang,Blake JianHang Chen,Lu Lin*

Main category: cs.AI

TL;DR: 本文针对多模态大语言模型中的路由问题，发现传统基于隐藏状态的探针路由在视觉输入存在时性能下降，提出了注意力探针和KL正则化LoRA探针两种改进方法。


<details>
  <summary>Details</summary>
Motivation: 在多模态大语言模型系统中，传统基于文本大语言模型的探针路由方法在视觉输入存在时性能显著下降，因为视觉输入削弱了隐藏状态中正确性信号的可分离性，需要专门针对多模态场景的改进路由方法。

Method: 提出了两种互补的改进方法：1）注意力探针，通过基于注意力分数聚合前一层隐藏状态来恢复分布式的正确性信号；2）KL正则化LoRA探针，插入轻量级LoRA适配器并应用KL正则化器来学习路由感知的表示。

Result: 综合实验表明，提出的方法在多个基准测试中一致优于基线方法，证明了改进隐藏状态质量对于多模态大语言模型中的有效路由至关重要。

Conclusion: 在多模态大语言模型中，改进隐藏状态的质量是提高路由效果的关键，提出的注意力探针和KL正则化LoRA探针方法为多模态路由问题提供了有效的解决方案。
...
### [163] [Resisting Humanization: Ethical Front-End Design Choices in AI for Sensitive Contexts](https://arxiv.org/abs/2603.24853)
*Silvia Rossi,Diletta Huyskes,Mackenzie Jorgensen*

Main category: cs.AI

TL;DR: 论文探讨AI前端设计的伦理意义，特别是对话式用户界面中人性化设计元素如何影响用户心理模型、信任校准和行为反应，通过Chayn案例说明伦理考量如何推动界面设计的审慎原则。


<details>
  <summary>Details</summary>
Motivation: 当前AI伦理讨论主要关注后端问题（数据治理、模型训练、算法决策），而前端设计选择（交互方式、表现形式）的伦理意义被忽视。对话式AI界面中的人性化设计元素日益普遍，但缺乏对其伦理影响的系统分析。

Method: 结合人机交互、对话式AI和价值敏感设计研究，分析界面如何影响用户期望、信任和自主性。通过Chayn非营利组织支持性别暴力幸存者的两个AI系统案例研究，展示创伤知情设计原则在实践中的应用。

Result: AI前端人性化设计是价值驱动的选择，深刻影响用户心理模型和信任校准。界面可能在脆弱情境下误导用户期望、培养不当信任、削弱用户自主性。Chayn案例展示了伦理考量如何促使设计中的原则性克制。

Conclusion: 伦理前端AI设计是一种程序性伦理，通过交互选择而非仅系统逻辑实现。挑战了当代AI产品中以参与度为导向的设计规范，强调在脆弱情境下需要审慎的界面设计原则。
...
### [164] [How Far Are Vision-Language Models from Constructing the Real World? A Benchmark for Physical Generative Reasoning](https://arxiv.org/abs/2603.24866)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型的评估过于偏向感知真实性，主要关注生成视觉上合理的3D布局、形状和外观，但很少测试模型是否理解实际构建这些构件所需的逐步过程和物理依赖性。这种能力对于自动化设计到建造流程至关重要。

Method: 基于住宅木结构建筑领域（具有完全编码的工程标准和客观可验证的正确性）创建基准，收集了13种建筑风格的26,000多个结构，均达到施工文档标准（LOD 350）。开发了确定性10测试结构验证框架，支持迭代智能体交互：模型观察中间建造状态，生成施工动作，接收结构化环境反馈。

Result: 对最先进的视觉语言模型进行广泛实验，揭示了现有排行榜上基本不可见的显著能力差距。这些发现确立了物理有效性作为与视觉真实性正交的关键评估维度。

Conclusion: 物理生成推理是多模态智能中一个独特且尚未充分开发的前沿领域，DreamHouse基准为评估这一能力提供了新的评估框架，强调了物理有效性作为关键评估轴的重要性。
...
### [165] [On the Foundations of Trustworthy Artificial Intelligence](https://arxiv.org/abs/2603.24904)
*TJ Dunham*

Main category: cs.AI

TL;DR: 论文提出平台确定性推理是可信AI的必要且充分条件，通过确定性论题和信任熵量化非确定性成本，构建纯整数推理引擎实现跨架构比特级一致性输出
...
Method: 提出确定性论题，引入信任熵量化非确定性成本，构建纯整数推理引擎替代浮点运算，实现跨ARM和x86架构的比特级一致性输出，通过哈希比较进行验证

Result: 在82个跨架构测试中（模型参数达67亿），观察到零哈希不匹配；四个地理分布式节点产生相同输出，通过356个链上证明交易验证；系统包含99,000行Rust代码部署在三大洲

Conclusion: AI可信性本质上是算术问题，平台确定性是AI系统所有主要可信属性的前提条件，纯整数推理引擎能够实现跨平台比特级一致性，为可信AI提供基础保障
...
### [166] [LogitScope: A Framework for Analyzing LLM Uncertainty Through Information Metrics](https://arxiv.org/abs/2603.24929)
...
<details>
  <summary>Details</summary>
Motivation: 传统评估方法对LLM输出中各个token位置的模型置信度提供有限洞察，需要一种能够量化生成过程中token级不确定性的方法。

Method: 引入LogitScope框架，通过计算每个生成步骤的概率分布信息度量（如熵和变熵）来分析LLM不确定性，支持懒评估计算高效，兼容任何HuggingFace模型。

Result: LogitScope能够揭示模型置信度模式、识别潜在幻觉、暴露高不确定性决策点，在不确定性量化、模型行为分析和生产监控等应用中具有实用价值。
...
### [167] [Decoding Market Emotions in Cryptocurrency Tweets via Predictive Statement Classification with Machine Learning and Transformers](https://arxiv.org/abs/2603.24933)
...
Method: 采用两阶段分类：任务1进行二元分类（预测性vs非预测性），任务2将预测性推文进一步分类为增量、减量或中性。结合人工和GPT标注，使用SenticNet提取情感特征，通过GPT生成改写进行数据增强以解决类别不平衡问题。

Result: GPT平衡方法显著提升模型性能，Transformer模型在任务1中取得最高F1分数，传统机器学习模型在任务2中表现最佳。情感分析揭示了不同加密货币预测类别对应的独特情感模式。

Conclusion: 该研究提出的分类框架能有效识别加密货币推文中的预测性陈述，GPT增强方法对处理不平衡数据有效，情感分析为理解加密货币市场情绪提供了新视角。
...
TL;DR: 研究Ctrip平台上3100万用户使用LLM购物助手"Wendao"的行为模式，发现：1) 采用者多为年长、女性、高活跃用户；2) AI聊天与传统搜索在同一购买阶段出现且常交替使用；3) 用户主要用AI进行探索性、难关键词化的查询


<details>
  <summary>Details</summary>
Motivation: 了解消费者如何采用和使用电商平台中嵌入的购物AI助手，填补关于平台嵌入式AI在电商领域应用的大规模实证研究空白

Method: 使用中国最大在线旅游平台Ctrip的3100万用户数据，分析其LLM助手"Wendao"的使用模式，包括用户特征、使用时机、与搜索的交互关系等

Result: 1) AI采用者特征与传统AI工具相反：年长、女性、高活跃用户采用率更高；2) AI聊天与传统搜索在同一购买阶段出现，常交替使用；3) 42%的聊天请求为景点查询，AI主要用于探索性、难关键词化的任务

Conclusion: 嵌入式购物AI并非传统搜索的替代品，而是电商中探索性产品发现的补充界面，为不同用户群体提供了新的交互方式
...
TL;DR: ScratchMath是一个专门用于评估学生手写数学草稿中错误的基准数据集，包含1720个中国中小学生数学样本，支持错误原因解释和分类任务，填补了现有教育NLP在手写草稿分析方面的空白。


<details>
  <summary>Details</summary>
Motivation: 现有教育NLP主要关注文本回答，忽视了手写草稿的复杂性、多模态性和多样性；当前多模态大语言模型主要从"应试者视角"生成正确答案，而非诊断学生错误。需要专门针对手写数学草稿错误分析的基准。

Method: 构建ScratchMath数据集，包含1720个中国中小学生数学手写样本，定义7种错误类型，支持错误原因解释(ECE)和错误原因分类(ECC)两个任务。采用严格的人机协作标注方法，包括多阶段专家标注、审查和验证。

Result: 评估了16个领先的MLLMs，发现与人类专家存在显著性能差距，尤其在视觉识别和逻辑推理方面。专有模型明显优于开源模型，大型推理模型在错误解释方面表现出较强潜力。

Conclusion: ScratchMath填补了手写数学草稿错误分析基准的空白，揭示了当前MLLMs在该任务上的局限性，为个性化教育反馈提供了重要研究基础。所有评估数据和框架已公开。
...
TL;DR: 本文提出了一种新的LLM不确定性分解框架，将不确定性分为三个语义组件：输入模糊性、知识缺口和解码随机性，以更好地理解和改进生成模型的可信度。


<details>
  <summary>Details</summary>
Motivation: 当前LLM不确定性评估方法存在局限：要么只提供单一不确定性分数，要么依赖经典的概率-认知二分法，无法为改进生成模型提供可操作的见解。现有方法不足以深入理解LLM的不确定性来源。
...
Conclusion: 该不确定性分解框架能够更深入地理解LLM的不确定性来源，为构建更可信的系统铺平道路，通过识别具体的不确定性类型，可以实施更有针对性的改进措施。
...
TL;DR: 本文提出多视角故障归因方法，解决多智能体系统中故障归因的模糊性问题，并创建了首个多视角故障归因基准MP-Bench。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统故障归因方法和基准大多假设每个故障只有一个确定的根本原因，但实际上由于复杂的智能体间依赖关系和模糊的执行轨迹，故障往往存在多种合理的归因解释。

Method: 提出多视角故障归因范式，明确考虑归因的模糊性；创建MP-Bench基准，这是首个专门用于多智能体系统多视角故障归因的基准；设计了适应这一范式的新评估协议。

Result: 实验发现，先前认为大语言模型在故障归因方面表现不佳的结论很大程度上源于现有基准设计的局限性。结果强调了多视角基准和评估协议对于实现现实可靠的多智能体系统调试的必要性。

Conclusion: 多视角故障归因是解决多智能体系统故障诊断中归因模糊性的实用范式，MP-Bench基准和相关评估协议为更现实可靠的多智能体系统调试提供了必要工具。
...
<details>
  <summary>Details</summary>
Motivation: 前沿AI中的知识蒸馏、模型提取和行为转移已成为核心关注点。主要风险不仅是复制，而是有用的能力可能比伴随的治理结构更便宜地被转移。需要减少这种不对称性。

Method: 提出一个约束耦合推理框架，包含四个要素：有界转移负担、路径负载积累、动态演化的可行区域以及能力-稳定性耦合条件。框架是公开安全的，省略了专有实现细节。

Result: 论文提供了一个可证伪的架构论点、清晰的威胁模型以及一组可实验验证的假设，用于未来关于蒸馏抵抗、对齐和模型治理的研究。

Conclusion: 通过将高级能力与内部稳定性约束耦合，可以降低蒸馏作为捷径的价值，从而减少能力转移与治理结构之间的不对称性，为AI安全提供理论框架。
...
TL;DR: 提出SAKE方法解决自回归神经PDE模拟器中低成本上下文窗口选择问题，通过系统锚点识别候选集并进行拐点感知选择，在PDEBench评估中表现最佳
...
Method: 提出系统锚定拐点估计(SAKE)方法：第一阶段从物理可解释的系统锚点识别小型结构化候选集；第二阶段在候选集中进行拐点感知的下游选择

Result: 在PDEBench八个家族评估中，SAKE是预算匹配下最强的低成本选择器，达到67.8%精确匹配、91.7%±1误差、6.1%平均遗憾@拐点，成本比仅0.051（节省94.9%搜索成本）

Conclusion: SAKE成功形式化了自回归神经PDE模拟器的上下文窗口选择问题，提供了一种物理可解释、低成本且性能优异的解决方案，显著优于现有方法
...
TL;DR: LEKIA 2.0提出了一种将认知层与执行层分离的LLM架构，通过外部情境结构解决情感支持中多轮对话的时序连续性和边界控制问题


<details>
  <summary>Details</summary>
Motivation: 传统LLM在心理支持和情感陪伴场景中，由于依赖局部下一词预测的"无状态"特性，无法维持多轮干预所需的时序连续性、阶段意识和用户同意边界，导致系统容易出现阶段错位和边界侵犯

Method: 提出LEKIA 2.0架构，将认知层与执行层分离，解耦情境建模与干预执行，构建可持续更新的外部情境结构，使系统能够在持续交互中保持用户情境和同意边界的稳定表示

Result: LEKIA在深度干预循环完成度上相比仅使用提示的基线方法平均绝对提升约31%，并引入了从静态到动态的在线评估协议来评估多轮交互的过程控制能力
...
TL;DR: 压缩视觉语言模型（VLM）虽然节省资源，但可能改变内部计算和安全行为。研究发现剪枝会破坏拒绝行为，而量化则保持特征对齐。
...
Method: 使用因果电路分析和基于crosscoder的特征比较方法，研究剪枝和量化如何改变代表性VLM的内部结构。同时引入VLMSafe-420基准测试，包含有害输入和匹配的良性对照。

Result: 剪枝通常保持电路结构完整但旋转和衰减内部特征，量化在更高层次修改电路但使存活特征更好对齐。剪枝导致真正的拒绝行为急剧下降，表明压缩选择具有安全影响。

Conclusion: 压缩方法的选择对视觉语言模型的安全行为有重要影响，剪枝会显著损害模型的拒绝能力，而量化相对保持更好的特征对齐。需要谨慎选择压缩策略以确保模型安全性。
...
TL;DR: 提出MP-MoE框架，结合强度损失和结构感知的矩阵轮廓目标，改善热带地区降水预报精度，缓解时间错位导致的"双重惩罚"问题。


<details>
  <summary>Details</summary>
Motivation: 热带地区（如越南）降水预报面临复杂地形和对流不稳定性的挑战，数值天气预报模型精度有限。现有数据驱动后处理框架多采用逐点目标函数，在时间错位时存在"双重惩罚"效应。
...
TL;DR: SAE特征在视觉语言模型中并不总是模块化可组合的，特征联合干预会导致输出漂移和准确率下降，揭示了特征共享内部通路的非模块化电路干扰现象。
...
Method: 开发了一个可复现的因果分析流程，在Qwen3-VL-8B模型中定位和测试稀疏视觉思维电路。在包含七种任务类型和三个难度级别的合成基准上，使用线性探针识别任务类型信息的中层解码器位置，在该层训练SAE，通过明确规则构建任务选择性特征集，并在推理时进行缩放和消融实验，同时量化准确率和漂移。

Result: 干预任务选择性特征集可以适度提高推理准确率，但干预两个特征集的联合却可靠地诱导输出漂移（预测的大幅意外变化）并降低准确率，即使在范数匹配的扰动下也是如此。这种非模块化电路干扰与共享内部通路一致，其中特征联合放大了激活偏移。

Conclusion: 研究结果阐明了SAE特征可组合性的边界，为更可靠的视觉语言模型控制提供了严谨的诊断框架。通过自助子样本和排列控制的验证，以及在多个VLM家族和五个不同数据集上的复现，证实了这些发现。
...
### [178] [When Sensing Varies with Contexts: Context-as-Transform for Tactile Few-Shot Class-Incremental Learning](https://arxiv.org/abs/2603.25115)
*Yifeng Lin,Aiping Huang,Wenxi Liu,Si Wu,Tiesong Zhao,Zheng-Jun Zha*

Main category: cs.AI

TL;DR: CaT-FSCIL提出了一种处理触觉感知中少样本类增量学习的方法，通过将采集环境分解为低维结构组件和高维残差组件，分别用可逆变换建模和不确定性校准原型来解决环境变化带来的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习在触觉感知中特别容易受到采集环境变化的影响，因为缺乏标准化（如不同设备、接触状态和交互设置）会显著降低性能。需要一种方法来处理这种环境变化带来的挑战。

Method: 将采集环境分解为结构化低维组件和高维残差组件。低维组件受触觉交互特征影响，建模为近似可逆的Context-as-Transform族，通过伪环境一致性损失进行逆变换规范化。高维残差主要来自平台和设备差异，使用不确定性条件原型校准来校准有偏差的原型和决策边界。

Result: 在标准基准测试HapTex和LMT108上的综合实验证明了CaT-FSCIL的优越性能。

Conclusion: 提出的CaT-FSCIL方法通过环境分解和针对性处理策略，有效解决了触觉感知中少样本类增量学习的环境变化问题，在标准基准测试中表现出色。
...
### [179] [RubricEval: A Rubric-Level Meta-Evaluation Benchmark for LLM Judges in Instruction Following](https://arxiv.org/abs/2603.25133)
*Tianjun Pan,Xuan Lin,Wenyan Yang,Qianyu He,Shisong Chen,Licai Qi,Wanqing Xu,Hongwei Feng,Bo Xu,Yanghua Xiao*

Main category: cs.AI

TL;DR: RubricEval是首个针对指令跟随评估的细粒度评分标准元评估基准，揭示了当前评分标准级别评估的局限性，GPT-4o在困难子集上仅达到55.97%准确率。


<details>
  <summary>Details</summary>
Motivation: 尽管基于评分标准的评估已成为评估大语言模型指令跟随的主流范式，但其可靠性尚不明确，需要元评估。先前的研究主要集中在响应级别，未能评估评分标准评估所依赖的细粒度判断准确性。

Method: 提出RubricEval基准，包含：(1)首个指令跟随的评分标准级别元评估基准；(2)涵盖多个类别和模型来源的多样化指令和响应；(3)3,486个质量控制实例，以及能更好区分评估者性能的简单/困难子集。

Result: 实验显示评分标准级别评估远未解决：即使是广泛采用的评估者GPT-4o，在困难子集上也仅达到55.97%准确率。评分标准级别评估优于清单级别，显式推理提高准确性，两者结合减少评估者间方差。

Conclusion: 通过建立的评分标准分类法，识别了常见失败模式，并为可靠的指令跟随评估提供了可操作的见解。评分标准级别评估需要进一步改进以提高可靠性。
...
### [180] [UniAI-GraphRAG: Synergizing Ontology-Guided Extraction, Multi-Dimensional Clustering, and Dual-Channel Fusion for Robust Multi-Hop Reasoning](https://arxiv.org/abs/2603.25152)
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在复杂推理、多跳查询和领域特定QA方面存在挑战，而现有的GraphRAG框架在跨行业适应性、社区报告完整性和检索性能方面仍有局限，需要更强大的解决方案。
...
Conclusion: UniAI-GraphRAG框架通过创新的知识提取、社区聚类和检索融合方法，有效提升了RAG系统在复杂推理和多跳查询任务上的性能，为领域特定QA提供了更强大的解决方案。
...
TL;DR: AI辅助安全分析存在"能力阴影"风险：AI生成的建议会系统性限制人类思考范围，导致安全分析质量下降远超简单加法估计。关键在于将AI辅助视为协作设计问题而非软件采购决策。


<details>
  <summary>Details</summary>
Motivation: 随着AI助手融入物理AI系统的安全工程工作流程，需要评估AI辅助是否真正提高安全分析质量，还是引入只有在部署后事故中才会显现的系统性盲点。

Method: 建立安全工程抵抗基准评估的正式框架，提出五维能力框架（领域知识、标准专长、操作经验、情境理解、判断力），引入"能力阴影"概念，形式化四种典型人机协作结构并推导闭式性能边界。

Result: 能力阴影会以乘法方式复合，导致分析质量下降远超朴素加法估计。AI辅助在安全工程中是协作设计问题，同一工具可能提升或降低分析质量，完全取决于使用方式。

Conclusion: 需要从工具认证转向工作流程认证，为可信物理AI建立抗阴影工作流程的非退化条件，确保AI辅助不会系统性限制人类安全分析能力。
...
### [182] [A Gait Foundation Model Predicts Multi-System Health Phenotypes from 3D Skeletal Motion](https://arxiv.org/abs/2603.25283)
...
TL;DR: 开发了一个基于3D骨骼运动的步态基础模型，使用深度相机记录3414名成年人的五种运动任务，模型嵌入比工程特征更好地预测年龄、BMI和内脏脂肪等指标，步态可作为独立的多系统生物信号。


<details>
  <summary>Details</summary>
Motivation: 步态被越来越多地视为重要生命体征，但目前的方法将其视为特定病理的症状而非系统性生物标志物。研究旨在建立步态作为独立的多系统生物信号。

Method: 开发了基于3D骨骼运动的步态基础模型，使用深度相机记录3414名深度表型成年人在五种运动任务中的运动数据，学习嵌入表示并与工程特征进行比较。

Result: 学习嵌入在预测年龄（Pearson r=0.69）、BMI（r=0.90）和内脏脂肪组织面积（r=0.82）方面优于工程特征。嵌入显著预测了3210个表型目标中的1980个；调整年龄、BMI、VAT和身高后，步态为男性所有18个身体系统和女性17/18个系统提供了独立增益，并改善了临床诊断和药物使用的预测。

Conclusion: 步态是独立的多系统生物信号，研究结果支持将步态转化为消费级视频应用，并整合为可扩展、被动的生命体征监测方法。
...
### [183] [SliderQuant: Accurate Post-Training Quantization for LLMs](https://arxiv.org/abs/2603.25284)
*Shigeng Wang,Chao Li,Yangyuxuan Kang,Jiawei Fan,Zhonghong Ou,Anbang Yao*

Main category: cs.AI

TL;DR: 提出SliderQuant框架，通过自适应滑动量化解决LLM不同层对量化敏感度差异的问题，在低比特量化中优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有PTQ方法通常对所有层采用相同的量化策略，但研究发现浅层和深层对量化更敏感，特别是第一层和最后一层，需要针对不同层设计多层次的量化方案

Method: 提出SliderQuant框架，包含层间滑动量化（针对浅层、中间层、深层设计三种滑动窗口）和层内滑动量化（采用增量策略量化每个窗口），通过少量可学习参数实现自适应量化

Result: 在基础语言生成、零样本常识推理以及数学和代码任务上，对Llama/Llama2/Llama3/Qwen2.5、DeepSeek-R1蒸馏模型和大规模MoE模型进行测试，SliderQuant在仅权重量化和权重-激活量化上都优于现有PTQ方法（包括最新的旋转变换方法）

Conclusion: SliderQuant通过考虑不同层对量化的敏感度差异，采用自适应滑动量化策略，显著减少了跨层的量化误差，在挑战性低比特设置下表现出优越性能
...
### [184] [DAGverse: Building Document-Grounded Semantic DAGs from Scientific Papers](https://arxiv.org/abs/2603.25293)
*Shu Wan,Saketh Vishnubhatla,Iskander Kushbay,Tom Heffernan,Aaron Belikoff,Raha Moraffah,Huan Liu*

Main category: cs.AI

TL;DR: 本文提出DAGverse框架，从科学论文中构建基于文档的语义有向无环图，通过半自动流水线生成高质量语义DAG数据集


<details>
  <summary>Details</summary>
Motivation: 现实世界DAG数据集稀缺，因为构建需要专家解释领域文档。文档可能允许多种合理抽象，预期结构通常隐含，支持证据分散在文本、公式、标题和图表中

Method: 利用包含显式DAG图的科学论文作为监督源，提出DAGverse框架，其核心DAGverse-Pipeline是半自动系统，包含图分类、图重建、语义基础和验证四个步骤
...
### [185] [Evaluating Language Models for Harmful Manipulation](https://arxiv.org/abs/2603.25326)
...
Method: 研究者提出了一个基于特定情境的人类-AI交互研究框架，在三个AI应用领域（公共政策、金融、健康）和三个地区（美国、英国、印度）对10,101名参与者进行了实验，评估AI模型的操纵行为倾向和实际效果。

Result: 研究发现：1) AI模型在被提示时能够产生操纵行为，并能改变参与者的信念和行为；2) 操纵效果因领域而异，需要在具体应用场景中评估；3) 不同地理区域存在显著差异，结果不能简单推广；4) AI模型的操纵倾向与操纵成功率并不一致，需要分别研究这两个维度。

Conclusion: 该研究提供了一个实用的AI有害操纵评估框架，强调了情境因素的重要性，并指出需要分别评估操纵倾向和实际效果。研究者公开了测试协议和相关材料，并讨论了未来评估AI有害操纵面临的挑战。
...
### [186] [Macroscopic Characteristics of Mixed Traffic Flow with Deep Reinforcement Learning Based Automated and Human-Driven Vehicles](https://arxiv.org/abs/2603.25328)
*Pankaj Kumar,Pranamesh Chakraborty,Subrahmanya Swamy Peruru*

Main category: cs.AI

TL;DR: 研究分析了基于深度强化学习的自动驾驶车辆在混合交通中的宏观交通流特性和燃油效率，相比传统IDM模型，DRL能提升约7.52%的道路容量和28.98%的燃油效率。


<details>
  <summary>Details</summary>
Motivation: 传统跟车模型（如IDM）在混合交通中难以泛化到多样化场景，且通常不考虑燃油效率，而深度强化学习虽然微观表现良好，但其宏观交通流特性尚未充分探索。

Method: 使用Twin Delayed Deep Deterministic Policy Gradient (TD3)算法训练自动驾驶车辆控制策略，基于NGSIM高速公路数据集，评估不同驾驶员异质性、安全时距渗透率和RL车辆比例下的交通性能，通过基本图分析宏观交通流特性，并与IDM模型进行燃油效率对比。

Result: 交通性能对安全时距分布和RL车辆比例敏感；从全人类驾驶过渡到全RL控制可将道路容量提升约7.52%；RL车辆在高速（>50km/h）下燃油效率比IDM提升约28.98%，低速（<50km/h）下提升1.86%。
...
<details>
  <summary>Details</summary>
Motivation: 算术谜题游戏为研究数学推理任务中的难度提供了受控环境，这是自适应学习系统的核心挑战。研究者旨在探究整数算术谜题中难度的结构决定因素，以支持可解释的难度评估和原则性的任务排序。

Method: 1. 形式化问题并开发精确的动态规划求解器，用于枚举可达目标、提取最小操作见证；2. 构建包含340多万个实例的数据集，通过达到目标所需的最小操作数定义难度；3. 分析难度与求解器导出特征之间的关系，比较基线机器学习模型与结构属性的预测能力。

Result: 1. 基于袋级和目标级统计的基线机器学习模型只能部分预测可解性，但无法可靠区分简单实例；2. 难度完全由少量从精确见证中导出的可解释结构属性决定；3. 最小构造中使用的输入值数量是难度标签下的最小充分统计量。

Conclusion: 该研究提供了透明、计算基础的谜题难度解释，连接了符号推理和数据驱动建模。该框架支持可解释的难度估计和原则性的任务排序，对自适应算术学习和智能练习系统有直接应用价值。
...
TL;DR: 该研究验证了基于5W3H的结构化意图表示框架PPS在不同语言和模型间的泛化能力，发现AI辅助扩展的提示在目标对齐上与人工编写的效果相当，同时能减少跨模型输出方差，提高意图对齐的可访问性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证结构化意图表示框架PPS是否能在不同语言和模型间有效泛化，扩展先前仅针对中文的证据，并探索AI辅助界面如何降低非专家用户的使用门槛。

Method: 研究方法包括：1）将研究扩展到英语和日语两种新语言；2）增加AI辅助扩展条件（用户输入简单提示，AI自动扩展为完整5W3H规范）；3）引入跨模型输出一致性的新研究问题；4）通过2,160个模型输出（3语言×4条件×3LLM×60任务）进行系统评估。

Result: 主要结果：1）AI扩展的5W3H提示在目标对齐上与人工编写的5W3H提示无显著差异；2）结构化PPS条件通常能减少或重塑跨模型输出方差，但效果因语言和指标而异；3）非结构化提示存在系统性双重膨胀偏差：人为高复合分数和人为低表观跨模型方差。

Conclusion: 结构化5W3H表示能改善跨语言和模型的意图对齐和可访问性，特别是当AI辅助编写降低了非专家用户的使用门槛时。研究为结构化意图表示的多语言和多模型应用提供了实证支持。
...
TL;DR: 论文提出"推理安全"概念，针对LLM推理过程的安全问题，建立了九类不安全推理行为分类法，开发了实时监控系统，在450条推理链测试中达到84.88%的步骤定位准确率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全研究主要关注输出内容安全（有害、偏见、事实错误），而将推理链视为不透明的中间产物。作者发现推理过程本身的安全同样重要，需要保证推理轨迹的逻辑一致性、计算效率和对抗攻击抵抗力。

Method: 1. 正式定义推理安全，提出九类不安全推理行为分类法（输入解析错误、推理执行错误、过程管理错误）；2. 对4111条推理链进行大规模实证研究；3. 提出推理安全监控器：基于LLM的外部组件，通过分类法嵌入提示实时检查每个推理步骤，检测到不安全行为时发送中断信号。

Result: 实证研究确认九类错误在实践中都存在，每种攻击都产生可解释的机制特征。在450条静态基准测试中，监控器达到84.88%的步骤级定位准确率和85.37%的错误类型分类准确率，显著优于幻觉检测器和过程奖励模型基线。
...
### [190] [Modernising Reinforcement Learning-Based Navigation for Embodied Semantic Scene Graph Generation](https://arxiv.org/abs/2603.25415)
*Roman Kueble,Marco Hueller,Mrunmai Phatak,Rainer Lienhart,Joerg Haehner*

Main category: cs.AI

TL;DR: 该研究提出了一种用于具身语义场景图生成的模块化导航组件，通过改进优化算法和动作表示，在有限动作预算下提高语义世界模型的构建质量。
...
Method: 提出模块化导航组件用于具身语义场景图生成，改进决策机制：1) 替换策略优化方法；2) 重新设计离散动作表示，研究紧凑和细粒度的动作集；3) 比较单头策略与分解的多头策略；4) 评估课程学习和可选的基于深度的碰撞监督。

Result: 仅替换优化算法就能在相同奖励塑造下将SSG完整性相对基线提高21%。深度监督主要影响执行安全性（无碰撞运动），而对完整性影响不大。现代优化算法与细粒度分解动作表示结合，实现了最强的完整性-效率权衡。

Conclusion: 通过改进优化算法和采用细粒度分解动作表示，可以显著提高具身语义场景图生成的效率和质量，为语义世界模型的构建提供了有效的导航策略。
...
### [191] [Retraining as Approximate Bayesian Inference](https://arxiv.org/abs/2603.25480)
*Harrison Katz*
...
<details>
  <summary>Details</summary>
Motivation: 传统上模型重训练被视为常规维护任务，缺乏理论框架指导何时进行重训练。作者认为需要将重训练决策系统化，以解决持续更新的信念状态与冻结部署模型之间的差距（即"学习债务"）。

Method: 将重训练重新定义为计算约束下的近似贝叶斯推断过程，建立决策理论框架，将重训练决策视为成本最小化问题，从损失函数中推导出重训练阈值，设计证据驱动的触发机制。

Result: 开发出基于证据的触发机制替代传统的日历调度，使重训练决策可审计、可治理，为模型维护提供理论依据和实用工具。

Conclusion: 重训练应被视为计算约束下的贝叶斯推断过程，决策理论框架为模型维护提供了系统化方法，证据驱动的触发机制比固定时间表更科学、更可审计。
...
### [192] [EcoThink: A Green Adaptive Inference Framework for Sustainable and Accessible Agents](https://arxiv.org/abs/2603.25498)
*Linxiao Li,Zhixiang Lu*

Main category: cs.AI

TL;DR: EcoThink是一个能源感知的自适应推理框架，通过轻量级路由机制动态评估查询复杂度，跳过简单查询的复杂推理，为复杂逻辑保留深度计算，平均减少40.4%的推理能耗。
...
Method: EcoThink采用基于蒸馏的轻量级路由器动态评估查询复杂度，对事实检索类查询跳过不必要的推理，仅为复杂逻辑问题保留深度计算，实现能源感知的自适应推理。

Result: 在9个不同基准测试中，EcoThink平均减少40.4%的推理能耗（在Web知识检索任务中最高可达81.9%），且没有统计学上显著的性能损失。

Conclusion: 通过减少算法浪费，EcoThink为可持续、包容和能源高效的生成式AI智能体提供了一条可扩展的路径。
...
### [193] [Voxtral TTS](https://arxiv.org/abs/2603.25551)
...
TL;DR: Voxtral TTS是一个多语言文本转语音模型，仅需3秒参考音频即可生成自然语音，采用自回归语义语音token生成与流匹配声学token的混合架构，在人类评估中优于ElevenLabs Flash v2.5。
...
Method: 采用混合架构：1）使用自回归生成语义语音token；2）使用流匹配生成声学token；3）使用Voxtral Codec语音分词器，该分词器从头训练，采用混合VQ-FSQ量化方案对语音进行编码和解码。

Result: 在由母语者进行的人类评估中，Voxtral TTS在多语言语音克隆的自然度和表达性方面表现优异，以68.4%的胜率优于ElevenLabs Flash v2.5模型。

Conclusion: Voxtral TTS是一个有效的多语言语音克隆系统，仅需极少参考音频即可生成高质量语音，模型权重已按CC BY-NC许可证发布。
...
Method: 引入RC2强化学习框架，通过强制跨模态循环一致性来化解内部冲突：要求模型进行反向推理、切换模态，并通过前向推理可靠地重建答案，获得密集的无标签奖励

Result: 优化这种结构约束可以缓解模态特定错误，将推理准确性提升高达7.6个百分点

Conclusion: 高级推理能力的涌现不仅来自数据规模的扩大，更需要强制执行对世界的结构一致性理解
...
TL;DR: WriteBack-RAG框架将知识库视为可训练组件，通过标记示例识别检索成功案例，提取相关文档并蒸馏为紧凑知识单元，与原始语料库一起索引，作为离线预处理步骤提升RAG性能。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统的知识库通常一次性组装后不再更新，而查询所需的事实往往分散在多个文档中且被无关内容淹没。需要将知识库视为可训练组件来优化检索效果。

Method: 提出WriteBack-RAG框架：1) 使用标记示例识别检索成功的情况；2) 隔离相关文档；3) 将相关文档蒸馏为紧凑的知识单元；4) 将这些知识单元与原始语料库一起索引。该方法仅修改语料库，可作为离线预处理步骤与任何RAG管道结合。

Result: 在4种RAG方法、6个基准测试和2个LLM骨干上，WriteBack-RAG在所有评估设置中均取得改进，平均提升+2.14%。跨方法转移实验表明，蒸馏出的知识也能使其他RAG管道受益，证明改进确实源于语料库本身。

Conclusion: 将知识库视为可训练组件并通过WriteBack-RAG框架进行优化，能够显著提升RAG系统的性能，且这种改进具有可转移性，为RAG系统的知识库优化提供了新思路。
