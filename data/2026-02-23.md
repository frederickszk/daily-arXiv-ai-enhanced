...
Motivation: 当前视频描述模型在准确描述细粒度运动细节方面存在显著限制，特别是在运动中心视频中，对复杂动作和肢体动态的精确描述常常被忽视，同时存在严重的幻觉问题。

Method: 1. 开发自动化标注流程，整合基于运动学的运动计算和语言解析；2. 构建KPM-Bench数据集，包含细粒度视频-描述对、运动理解问答对和幻觉评估集；3. 提出MoPE算法从文本描述中准确提取运动特定属性；4. 基于MoPE开发独立于大模型的幻觉评估指标；5. 将MoPE集成到GRPO后训练框架中。

Result: 创建了KPM-Bench开源数据集，提出了MoPE算法和相应的幻觉评估指标，通过GRPO框架有效缓解了幻觉问题，显著提高了运动中心视频描述模型的可靠性。

Conclusion: 该研究通过创新的数据集构建、运动解析算法和幻觉评估方法，系统性地解决了视频描述中细粒度运动理解不足和幻觉问题，为运动中心视频理解提供了有效的解决方案。
...
TL;DR: 该论文提出了3D-HIW数据集和CLUTCH系统，用于解决野外环境下3D手部动作建模的挑战，在文本-动作生成和动作-文本描述任务上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 手部动作在日常生活中至关重要，但现有方法依赖于工作室采集的有限数据集，难以扩展到野外环境，且文本-动作对齐效果不佳。

Method: 1) 构建3D-HIW数据集：结合视觉语言模型和3D手部跟踪器，从大量第一人称视角视频中提取32K个3D手部动作序列和对应文本；2) 提出CLUTCH系统：包含SHIFT（基于VQ-VAE的手部动作分词架构）和几何精炼阶段（通过重建损失微调LLM）。

Result: 在文本到动作和动作到文本任务上实现了最先进的性能，为可扩展的野外手部动作建模建立了首个基准。

Conclusion: 通过3D-HIW数据集和CLUTCH系统，成功解决了野外环境下手部动作建模的挑战，为自然手部动作生成和理解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 标准IHC染色资源密集、昂贵且耗时，许多地区无法获得。从H&E切片预测HER2水平是潜在替代方案，但现有虚拟染色方法计算昂贵且易产生重建伪影导致诊断错误

Method: 提出Latent-Guided Dual-Stream Network (LGD-Net)，学习将形态学H&E特征直接映射到分子潜在空间，训练时由教师IHC编码器引导。通过轻量级辅助正则化任务，利用细胞核分布和膜染色强度等特定领域知识正则化模型训练

Result: 在公共BCI数据集上的广泛实验表明，LGD-Net达到最先进性能，显著优于基线方法，同时支持使用单模态H&E输入进行高效推理

Conclusion: LGD-Net通过跨模态特征幻觉而非像素级图像生成，有效解决了虚拟染色方法的计算昂贵和重建伪影问题，为从H&E切片准确预测HER2表达水平提供了高效可靠的解决方案
...
TL;DR: 提出了一种无需额外训练或仅需轻量LoRA调优的遥感图像文本引导分割方法，结合对比式和生成式视觉语言模型与SAM，在19个遥感基准测试中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型和视觉基础模型为零样本遥感图像分割提供了新机会，但大多数方法仍依赖额外的可训练组件，限制了其泛化能力和实际应用。本研究旨在探索仅依赖现有基础模型、无需额外训练即可实现基于文本的遥感分割的可能性。

Method: 提出两种方法：1）对比式方法使用CLIP作为SAM网格提议的掩码选择器，实现完全零样本的开放词汇语义分割；2）生成式方法使用GPT-5（零样本）和LoRA调优的Qwen-VL模型生成SAM的点击提示，实现推理和指代分割。

Result: 在19个遥感基准测试（包括开放词汇、指代和基于推理的任务）上的广泛实验表明，该方法具有很强的能力。对比式方法在完全零样本设置下实现了最先进的开放词汇语义分割，生成式方法中LoRA调优的Qwen-VL模型表现最佳。

Conclusion: 研究表明，仅依赖现有基础模型、无需额外训练或仅需轻量调优即可实现有效的遥感图像文本引导分割。该方法简单有效，为遥感分割的实际应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 查询性能预测（QPP）在文本和图像检索中已有广泛研究，但在基于内容的视频检索（CBVR）领域仍缺乏系统探索。需要建立专门的视频QPP基准来推动该领域的研究发展。

Method: 构建了VQPP基准，包含两个文本到视频检索数据集和两个CBVR系统，提供了官方的训练、验证和测试划分。探索了多种检索前和检索后性能预测器，并利用最佳检索前预测器作为奖励模型，通过直接偏好优化（DPO）训练大型语言模型进行查询重构。

Result: 检索前预测器取得了具有竞争力的性能，能够在执行检索步骤之前实现应用。成功将最佳检索前预测器应用于查询重构任务，通过DPO训练LLM展示了VQPP的实际应用价值。

Conclusion: VQPP基准为视频领域的查询性能预测研究提供了首个标准化评估框架，促进了该领域的可比较和可复现研究。检索前预测器的良好性能为视频检索系统的实际应用提供了新的可能性。
...
TL;DR: 该论文对Liu和Szirányi提出的手势识别方法进行方法学分析，指出其评估协议存在严重的数据泄露问题，导致报告的接近完美的准确率指标无效。
...
Method: 通过分析已发布的混淆矩阵、学习曲线和数据集构建，展示评估协议中帧级随机训练-测试划分如何不可避免地混合同一受试者的样本，导致严重的数据泄露问题。

Result: 研究发现报告的接近完美的准确率指标源于数据泄露，评估未能测量对未见个体的泛化能力。通过检查混淆矩阵和学习曲线，证明评估协议存在严重缺陷。

Conclusion: 研究强调了在基于视觉的手势识别研究中主体独立数据划分的重要性，特别是对于需要可靠识别未见个体手势的应用。评估协议必须确保训练和测试集包含不同受试者的数据，以准确测量模型的泛化能力。
...
TL;DR: 提出一种用于长视频理解的新型端到端框架，包含基于信息密度的自适应视频采样器和基于自编码器的时空视频压缩器，与多模态大语言模型集成，有效处理长视频的冗余问题。
...
Method: 提出端到端的长视频理解框架，包含两个核心组件：1) 基于信息密度的自适应视频采样器(AVS)，根据视频内容重要性自适应选择关键帧；2) 基于自编码器的时空视频压缩器(SVC)，与多模态大语言模型(MLLM)集成，实现高压缩率同时保留关键判别信息。

Result: 该框架在多个基准测试中表现出色，不仅在长视频理解任务上表现优异，在标准视频理解基准测试中也取得良好结果，证明了其处理长视频复杂性的有效性和通用性。

Conclusion: 提出的框架通过自适应采样和高效压缩机制，有效解决了长视频理解中的冗余问题，能够自适应捕获不同时长视频的关键信息，同时保持高压缩率和信息保留能力，为长视频分析提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在各种视觉问答基准测试中取得了显著进展，但最近的研究表明这些模型在传统的细粒度图像分类基准测试中表现不佳，这测试的是细粒度的视觉知识。研究者希望探索这种性能差异的原因。

Method: 研究者测试了大量最近的视觉语言模型在细粒度分类基准上的表现，并通过一系列消融实验来分析影响性能的因素。他们比较了不同LLM和视觉编码器的影响，以及预训练阶段的重要性。

Result: 研究发现：1）使用更好的LLM能同等提升所有基准测试分数；2）更好的视觉编码器能不成比例地提升细粒度分类性能；3）预训练阶段对细粒度性能至关重要，特别是在预训练期间语言模型权重未冻结的情况下。

Conclusion: 这些发现为增强视觉语言模型中的细粒度视觉理解和以视觉为中心的能力提供了方向，强调了视觉编码器质量和预训练策略对提升细粒度分类性能的重要性。
...
TL;DR: 该论文提出了一种利用稀疏多模态测距数据（如雷达或激光雷达）改进单图像新视角合成的扩散模型方法，通过更可靠的几何先验提升生成视图的质量和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的单图像新视角合成方法依赖于单目深度估计的几何信息，但在低纹理、恶劣天气和遮挡严重的真实场景中，深度估计的可靠性有限，导致合成视图的质量和一致性受限。

Method: 提出多模态深度重建框架，利用极稀疏的测距传感数据（如汽车雷达或激光雷达）生成密集深度图，作为扩散模型新视角合成的鲁棒几何条件。采用基于角域的局部高斯过程建模，实现高效计算并显式量化观测有限区域的不确定性。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉Transformer依赖位置嵌入和类别标记编码固定的空间先验，这在自然图像中有效，但在医学成像中可能阻碍泛化能力，因为医学图像的空间布局可能信息较弱或不一致。

Method: 提出ZACH-ViT（Zero-token Adaptive Compact Hierarchical Vision Transformer），移除位置嵌入和[CLS]标记，通过全局平均池化处理补丁表示实现排列不变性。采用自适应残差投影保持训练稳定性，同时严格控制参数预算。

Result: 在7个MedMNIST数据集上的评估显示：ZACH-ViT（0.25M参数，从头训练）在BloodMNIST上表现最强，在PathMNIST上与TransMIL竞争，但在具有强解剖先验的数据集（OCTMNIST、OrganAMNIST）上相对优势减弱。尽管尺寸极小且无预训练，仍能保持亚秒级推理时间。

Conclusion: 架构的归纳偏差与数据结构对齐比追求通用基准主导更重要。ZACH-ViT在资源受限的临床环境中具有部署潜力，其紧凑设计和无位置嵌入特性适合医学成像任务。
...
TL;DR: ROCKET提出了一种残差导向的多层表示对齐框架，通过共享投影器将VLA模型的多个层与3D视觉基础模型的多个层对齐，减少梯度冲突，仅需约4%计算预算即可达到98.5%的SOTA成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的Vision-Language-Action模型通常在2D数据上预训练，缺乏3D空间理解能力。现有表示对齐方法通常只在单层进行监督，无法充分利用深度分布信息，而朴素的多层对齐会导致梯度干扰。

Method: ROCKET采用残差导向的多层表示对齐框架，将多层对齐表述为将一个残差流与另一个残差流对齐。使用共享投影器通过层不变映射将VLA骨干的多个层与强大的3D视觉基础模型的多个层对齐，减少梯度冲突。提出Matryoshka风格的稀疏激活方案来平衡多个对齐损失，并结合免训练层选择策略。

Result: 在LIBERO数据集上，仅需约4%的计算预算即可达到98.5%的SOTA成功率。在LIBERO-Plus和RoboTwin数据集以及多个VLA模型上均表现出优越性能。

Conclusion: ROCKET通过残差导向的多层表示对齐框架，有效解决了VLA模型缺乏3D空间理解的问题，在显著减少计算成本的同时实现了优异的性能，为机器人操作任务提供了高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有全参考图像质量评估方法依赖参考图像质量，限制了在理想参考源不可得的实际应用。受人类视觉系统积累视觉记忆能力的启发，提出减少对高质量参考图像依赖的解决方案。
...
TL;DR: MUOT_3M是首个伪多模态水下目标跟踪基准，包含300万帧视频数据，MUTrack是基于SAM的多模态到单模态跟踪器，在多个基准测试中表现优于现有方法。
...
Method: 1) 构建MUOT_3M基准：包含3030个视频的300万帧，标注32个跟踪属性、677个细粒度类别，提供RGB、增强RGB、深度和语言四种同步模态；2) 提出MUTrack跟踪器：基于SAM架构，采用视觉几何对齐、视觉语言融合和四级知识蒸馏，将多模态知识转移到单模态学生模型中。

Result: 在五个水下目标跟踪基准上的广泛评估显示，MUTrack比最强的SOTA基线高出8.40%的AUC和7.80%的精度，同时以24 FPS的速度运行。

Conclusion: MUOT_3M和MUTrack为可扩展、多模态训练但实际可部署的水下跟踪建立了新的基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉定制研究主要关注客观对齐（如语言、布局等控制信号与编辑图像的匹配），但忽视了主观情感内容，且缺乏通用的情感视觉定制基础模型。因此需要开发能够有效处理情感内容的视觉定制方法。
...
Conclusion: 情感信息对于视觉定制任务至关重要，提出的EPEM方法能够高效且精确地操纵图像中的情感内容，为情感视觉定制提供了有效的解决方案。
...
Method: 提出了统一物理世界正则化MoE方法（UPRM），包含两个关键组件：统一物理世界增强MoE块（UPE）用于建模从粗到细的物理世界信息，以及物理世界权衡正则器（PTR）用于自适应权衡这些因素。

Result: 在DeepSVU指令数据集（UCF-C指令和CUVA指令）上的广泛实验表明，UPRM优于多个先进的视频LLM和非VLM方法，验证了粗到细物理世界信息的重要性以及UPRM在捕捉此类信息方面的有效性。

Conclusion: 本文提出的DeepSVU任务和UPRM方法有效解决了安全导向视频理解中的深度分析需求，通过建模物理世界信息和自适应权衡机制，显著提升了威胁识别、定位和原因分析的能力。
...
TL;DR: 提出UAOR方法，一种无需训练、即插即用的模块，通过不确定性感知的观察信息重新注入机制，提升VLA模型在机器人操作任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通常需要额外的观察线索（如深度图、点云）或辅助模块（如物体检测器）来提高任务执行精度，但这些方法需要昂贵的数据收集和额外训练。需要一种更高效、无需训练的方法来增强VLA模型对观察信息的关注能力。

Method: 提出不确定性感知观察重新注入（UAOR）方法：当语言模型层表现出高不确定性（通过动作熵测量）时，通过注意力检索将关键观察信息重新注入到下一层的FFN中。该方法利用语言模型中FFN作为"键值记忆"的特性，在推理过程中帮助VLA模型更好地关注观察信息。

Result: 综合实验表明，该方法能一致地提升多种VLA模型在仿真和真实世界任务中的性能，且开销极小。特别地，UAOR无需额外观察线索或模块，可作为现有VLA流程的通用即插即用插件。

Conclusion: UAOR是一种有效、无需训练、即插即用的模块，通过不确定性感知的观察重新注入机制，显著提升VLA模型在机器人操作任务中的性能，具有很好的实用性和通用性。
...
TL;DR: 提出DCAG框架，通过同时操纵DiT注意力机制中的Key和Value通道实现训练自由的编辑强度控制，相比仅操作Key通道的方法在编辑保真度权衡上更精确。


<details>
  <summary>Details</summary>
Motivation: 现有基于DiT架构的扩散图像编辑模型需要训练自由的编辑强度控制，但现有注意力操纵方法仅关注Key空间来调节注意力路由，完全忽略了控制特征聚合的Value空间。

Method: 首先发现DiT多模态注意力层中Key和Value投影都表现出明显的偏差-增量结构，基于此提出DCAG框架，同时操纵Key通道（控制注意力位置）和Value通道（控制聚合内容），形成二维参数空间(δ_k, δ_v)。

Result: 在PIE-Bench基准测试（700张图像，10个编辑类别）中，DCAG在所有保真度指标上始终优于仅使用Key指导的方法，在对象删除（LPIPS减少4.9%）和对象添加（LPIPS减少3.2%）等局部编辑任务中改进最显著。

Conclusion: DCAG通过同时利用Key和Value通道，实现了比任何单通道方法更精确的编辑-保真度权衡，Key通道通过非线性softmax函数提供粗粒度控制，Value通道通过线性加权求和提供细粒度补充。
...
TL;DR: DiST：一个基于大语言模型空间-时间知识解耦与融合的少样本动作识别框架，通过分解动作名称获得细粒度时空属性描述，并设计空间/时间知识补偿器学习多粒度原型，在五个标准数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有少样本动作识别方法通常使用粗粒度的动作类别名称作为辅助上下文，但这类上下文信息有限，无法为捕捉动作中的新颖空间和时间概念提供足够的背景知识。需要更丰富的语义信息来指导视觉特征学习。

Method: 提出DiST框架，包含两个阶段：1）分解阶段：使用大语言模型将原始动作名称解耦为多样化的时空属性描述（动作相关知识）；2）融合阶段：设计空间知识补偿器（SKC）和时间知识补偿器（TKC）分别学习对象级和帧级原型。SKC在空间知识指导下自适应聚合重要图像块，TKC利用时间属性辅助帧间时序关系建模。

Result: 在五个标准少样本动作识别数据集上实现了最先进的性能，证明了通过大语言模型提供的解耦时空知识能够有效提升少样本动作识别的准确性。

Conclusion: DiST框架通过解耦和融合大语言模型提供的空间和时间知识，能够学习到表达力强的多粒度原型，为捕捉细粒度空间细节和多样化时间模式提供了透明度，显著提升了少样本动作识别的性能。
...
TL;DR: CityGuard：一种用于分散式监控的隐私保护身份检索框架，通过拓扑感知Transformer处理视角变化、遮挡和域偏移，同时遵守数据保护规则。
...
Method: 1. 分散自适应度量学习器根据特征分布调整实例级边界；2. 空间条件注意力将粗粒度几何信息注入图自注意力；3. 差分隐私嵌入映射与紧凑近似索引结合。

Result: 在Market-1501等公开基准测试中，检索精度和查询吞吐量均优于强基线方法，验证了框架在隐私关键城市身份匹配中的实用性。

Conclusion: CityGuard框架能够产生对视角变化、遮挡和域偏移鲁棒的描述符，并在严格的差分隐私核算下实现隐私与效用的可调平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有两阶段文本到动作生成框架通常忽略跨序列的时序一致性，导致语义错位和物理上不合理的动作，需要解决这一局限性

Method: 提出TCA-T2M框架，包含：1）时序一致性感知的空间VQ-VAE用于跨序列时序对齐；2）掩码运动变换器用于文本条件动作生成；3）运动学约束块减轻离散化伪影
...
<details>
  <summary>Details</summary>
Motivation: 当前多摄像头BEV语义地图方法依赖昂贵且标注不一致的地面真值数据，需要解决标注成本高的问题

Method: 采用两阶段训练：1）自监督预训练阶段，将BEVFormer预测可微分地重投影到图像平面，使用Mask2Former生成的多视角语义伪标签进行训练，并加入时间一致性损失；2）监督微调阶段，仅使用50%数据集进行微调

Result: 在nuScenes数据集上，相比完全监督基线模型，性能提升高达+2.5pp mIoU，同时将标注数据使用量减半，总训练时间减少三分之二
...
TL;DR: DohaScript是一个大规模、多书写者的手写印地语数据集，包含531名书写者抄写相同六首传统对句，旨在解决德瓦纳格里文字手写数据稀缺问题，支持手写识别、书写者识别等任务。


<details>
  <summary>Details</summary>
Motivation: 尽管德瓦纳格里文字有数亿使用者，但手写文本在公开基准数据集中代表性严重不足。现有资源规模有限，主要关注孤立字符或短词，缺乏受控词汇内容和书写者多样性，无法捕捉德瓦纳格里手写连续、融合和结构复杂的特性。

Method: 收集531名独特贡献者的手写印地语文本，设计为平行风格语料库，所有书写者抄写相同的六首传统印地语对句。数据集包含去识别化的人口统计元数据，基于客观清晰度和分辨率标准的严格质量筛选，以及页面级布局难度标注。

Result: 基线实验显示清晰的质量分离和对未见书写者的强泛化能力，突显了数据集的可靠性和实用价值。DohaScript可作为标准化、可复现的基准，推动低资源脚本环境下连续手写德瓦纳格里文本的研究。

Conclusion: DohaScript填补了德瓦纳格里手写文本数据集的空白，通过受控设计支持系统性的书写者特异性分析，为手写识别、书写者识别、风格分析和生成建模等任务提供了有价值的资源。
...
TL;DR: PrediT：一种无需训练的DiT加速框架，通过线性多步方法预测模型输出，结合校正器和动态步长调节，实现最高5.54倍加速且保持生成质量


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiT）在图像和视频生成中广泛应用，但其迭代去噪过程计算成本高。现有无需训练的加速方法依赖特征缓存和重用，但多步重用可能导致潜在漂移和视觉质量下降

Method: 提出PrediT框架：1）将特征预测建模为线性多步问题，使用经典线性多步方法从历史信息预测未来模型输出；2）在高动态区域激活校正器防止误差累积；3）动态步长调制机制通过监控特征变化率自适应调整预测范围

Result: 在各种基于DiT的图像和视频生成模型上实现了最高5.54倍的延迟降低，同时质量下降可忽略不计

Conclusion: PrediT通过系统性的特征预测方法，在保持生成质量的同时显著加速DiT推理过程，为扩散模型的高效部署提供了有效解决方案
...
TL;DR: OODBench：一个自动化构建的基准测试，用于评估视觉语言模型处理分布外数据的能力，包含4万个实例-类别对，发现现有模型在常见类别上仍存在显著性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型通常在独立同分布假设下训练，但现实场景中分布外数据不可避免，且处理不当可能带来安全风险。目前缺乏全面评估模型处理分布外数据能力的有效基准。

Method: 提出OODBench方法，采用最小人工验证的自动化方式构建基准测试，包含4万个实例级分布外实例-类别对。同时提出可靠的自评估指标，采用从基础到高级的渐进式提示问题来全面评估分布外数据对不同难度问题的影响。

Result: 当前视觉语言模型在OODBench上仍表现出显著性能下降，即使底层图像类别是常见的。提出的自动化评估指标能够有效评估分布外数据的影响。

Conclusion: OODBench为评估视觉语言模型处理分布外数据能力提供了有效基准，总结了重要发现和见解，有助于未来在分布外数据获取和评估方面的研究。
...
TL;DR: ViTs在可视化图形感知任务中的表现不如人类和CNNs，存在感知差距
...
Result: ViTs虽然在通用视觉任务中表现强劲，但在可视化领域的类人图形感知能力有限，与人类感知的对齐度不高

Conclusion: ViTs在可视化系统中存在显著的感知差距，这为可视化系统和图形感知建模中应用ViTs提供了重要考虑因素
...
TL;DR: BLM-Guard是一个用于短视频广告内容审核的框架，通过思维链推理、规则策略和批评引导奖励来检测欺骗性多模态内容，显著降低标注成本并提升审核准确性。
...
Method: 1. 使用规则驱动的ICoT数据合成管道生成结构化场景描述、推理链和标签；2. 强化学习通过平衡因果一致性和策略遵从性的复合奖励来优化模型；3. 多任务架构建模模态内操纵（如夸张图像）和跨模态不匹配（如字幕-语音漂移）。

Result: 在真实短视频广告上的实验表明，BLM-Guard在准确性、一致性和泛化能力方面超越了强基线方法。

Conclusion: BLM-Guard框架通过结合思维链推理、策略原则和强化学习，为商业广告内容审核提供了有效的解决方案，显著降低了标注成本并提升了审核性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本生成人体动作的方法在语义对齐方面取得进展，但难以同时保证语义和物理真实性。现有方法生成的动经常存在物理不合理问题（如脚部漂浮），需要一种既能修正物理不合理性又能保持语义一致性的解决方案。

Method: 提出Distortion-aware Motion Calibrator (DMC)后处理模块，采用自监督数据驱动方法。DMC学习在给定故意扭曲的动作和原始文本描述时，生成物理合理的动作。不依赖复杂物理建模，而是通过数据学习修正物理不合理性。

Result: DMC显著提升动作质量：在T2M上降低FID分数42.74%，在T2M-GPT上降低13.20%，同时获得最高R-Precision。应用于MoMask等高质量模型时，减少穿透33.0%，调整漂浮伪影更接近真实参考。DMC能有效提升物理合理性同时增强语义一致性。

Conclusion: DMC是一个有前景的后处理动作精炼框架，可应用于任何文本到动作生成模型，通过结合文本语义和物理合理性来提升生成动作的质量。
...
TL;DR: 本文首次研究了离散图像分词器对对抗攻击的脆弱性，提出了高效的应用无关攻击方法，并通过无监督对抗训练来增强分词器的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 离散图像分词器在多模态系统中越来越流行，但与CLIP编码器不同，其对抗攻击脆弱性尚未被探索。本文旨在填补这一研究空白，研究分词器的脆弱性并提出防御方法。

Method: 1. 首先制定针对离散分词器的攻击方法，旨在扰动提取的特征并改变提取的token；2. 受鲁棒CLIP编码器研究的启发，采用无监督对抗训练微调流行分词器，保持其他组件不变。

Result: 攻击方法计算高效、应用无关，在分类、多模态检索和字幕生成任务中均有效。防御方法显著提高了对无监督和端到端监督攻击的鲁棒性，并能很好地泛化到未见任务和数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有多实例生成方法在空间布局和属性绑定方面已有进步，但在处理复杂文本描述时仍面临细粒度语义理解的挑战，特别是在处理丰富、局部化的文本描述时存在困难。

Method: 提出DEIG框架，包含两个核心组件：1) 实例细节提取器(IDE)，将文本编码器嵌入转换为紧凑的实例感知表示；2) 细节融合模块(DFM)，应用基于实例的掩码注意力防止实例间属性泄漏。还构建了高质量数据集和DEIG-Bench基准。

Result: 实验表明DEIG在多个基准测试中，在空间一致性、语义准确性和组合泛化方面一致优于现有方法。DEIG可作为即插即用模块轻松集成到标准基于扩散的流程中。

Conclusion: DEIG通过创新的实例细节提取和融合机制，实现了细粒度可控的多实例生成，能够精确匹配丰富的局部化文本描述，在复杂场景生成方面取得了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 在时尚设计早期阶段，草图能表达结构、轮廓和空间关系，文本描述则补充材料、颜色和风格细节。现有方法需要更好地结合文本和视觉模态，在利用文本局部属性指导时保持草图视觉结构的一致性。

Method: 提出LOTS框架，包含多级条件阶段和扩散对引导阶段。多级条件阶段在共享潜在空间中独立编码局部特征，同时保持全局结构协调；扩散对引导阶段通过基于注意力的引导，在扩散模型的多步去噪过程中整合局部和全局条件。

Result: 创建了首个时尚数据集Sketchy，包含每张图像对应的多个文本-草图对，提供高质量、结构一致的专业草图。实验表明该方法在加强全局结构遵循的同时，利用更丰富的局部语义指导，优于现有最先进方法。

Conclusion: LOTS框架通过结合全局草图引导和多局部草图-文本对，有效提升了时尚图像生成的质量，在保持结构一致性的同时增强了局部细节表达，为时尚设计提供了更强大的工具。
...
TL;DR: Diff2DGS是一个用于手术场景实时3D重建的两阶段框架，通过扩散模型修复被器械遮挡的组织，并使用可学习变形模型的2D高斯溅射来捕捉动态组织变形，在图像质量和几何精度上都优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 实时重建可变形手术场景对于推进机器人手术、改善外科医生引导和实现自动化至关重要。现有方法在遮挡区域的重建质量有限，且缺乏3D真实数据来评估深度精度。

Method: 提出Diff2DGS两阶段框架：第一阶段使用基于扩散的视频模块，利用时间先验修复被器械遮挡的组织；第二阶段采用带有可学习变形模型(LDM)的2D高斯溅射来捕捉动态组织变形和解剖几何结构。

Result: 在EndoNeRF上达到38.02 dB PSNR，在StereoMIS上达到34.40 dB PSNR，优于现有方法。实验表明仅优化图像质量不一定能获得最佳3D重建精度，因此进一步优化深度质量以确保更准确的几何重建。

Conclusion: Diff2DGS在手术场景的3D重建中同时实现了高保真外观和准确几何结构，通过两阶段方法有效解决了遮挡区域重建质量有限的问题，并在SCARED数据集上进行了定量深度精度分析。
...
<details>
  <summary>Details</summary>
Motivation: 真实环境中复杂的光照变化和相机成像管道的固有限制使得高质量图像采集具有挑战性。在多视角捕获中，光照、传感器响应和ISP配置的差异导致光度学和色彩不一致，违反了现代3D新视角合成方法（如NeRF和3DGS）所依赖的光度一致性假设，导致重建和渲染质量下降。

Method: 提出Luminance-GS++框架，结合全局视角自适应亮度调整和局部像素级残差细化进行精确色彩校正。设计无监督目标函数，联合强制执行亮度校正以及多视角几何和光度一致性。该方法保持显式的3DGS表示形式，不修改底层表示。

Result: 在包括低光照、过曝光以及复杂亮度和色彩变化在内的挑战性场景中，展示了最先进的性能表现。实验证明该方法在保持实时渲染效率的同时提高了重建保真度。

Conclusion: Luminance-GS++通过创新的亮度校正和色彩细化策略，有效解决了多视角光照不一致问题，为真实世界环境中的鲁棒新视角合成提供了实用解决方案，同时保持了3D高斯泼溅的实时渲染优势。
...
TL;DR: 本文提出了一种基于高斯-拉普拉斯算子(G-LoG)的双参数过滤方法，用于医学图像拓扑特征提取，在多参数持久性模块中表现优于单参数过滤，且其拓扑特征训练的简单MLP模型性能可与复杂深度学习模型媲美。


<details>
  <summary>Details</summary>
Motivation: 在拓扑数据分析中，构建实用的过滤方法来检测拓扑和几何特征是一个重要任务。本文旨在利用拉普拉斯高斯算子在增强医学图像边界方面的能力，开发更适合多参数持久性模块的特征提取方法。

Method: 提出G-LoG（高斯-拉普拉斯高斯）双参数过滤方法：1）利用拉普拉斯高斯算子增强医学图像边界；2）将体数据图像建模为有界函数；3）证明从有界函数的双参数过滤获得的持久性模块的交错距离相对于有界函数的最大范数是稳定的。

Result: 在MedMNIST数据集上的实验表明：1）G-LoG双参数过滤显著优于单参数过滤；2）使用该双参数过滤生成的拓扑特征训练的简单多层感知器（MLP），其性能可与在原始数据集上训练的复杂深度学习模型（包括Google AutoML Vision、ResNet、AutoKeras和auto-sklearn）相媲美。

Conclusion: G-LoG双参数过滤为医学图像分析提供了一种有效的拓扑特征提取方法，不仅在多参数持久性模块中表现优越，而且其提取的特征能够使简单的机器学习模型达到与复杂深度学习模型相当的性能，为拓扑数据分析在医学图像处理中的应用提供了新思路。
...
TL;DR: 提出了一个基于人体姿态控制（头部和手部）的视频世界模型，用于扩展现实（XR）中的沉浸式交互体验


<details>
  <summary>Details</summary>
Motivation: 当前视频世界模型仅接受文本或键盘等粗粒度控制信号，无法响应XR中用户实时追踪的真实世界动作，限制了其在具身交互中的实用性

Method: 1. 评估现有扩散变换器条件策略；2. 提出有效的3D头部和手部姿态控制机制；3. 训练双向视频扩散模型教师；4. 将其蒸馏为因果交互系统生成第一人称虚拟环境

Result: 通过人类受试者评估显示，相比相关基线，该系统在任务性能上有改进，且在感知到的动作控制程度上显著更高

Conclusion: 提出了一个以人为中心的视频世界模型，能够响应追踪的头部和手部姿态，实现了灵巧的手-物交互，为XR中的沉浸式交互提供了有效解决方案
...
Method: 定义5种代表性人类和机器人智能体（各有物理尺寸、移动能力和环境交互能力描述），构建包含45个真实室内场景、473个导航任务和2365个问答对的CapNav基准，评估13个现代VLM的性能

Result: 当前VLM的导航性能随移动约束收紧而急剧下降，即使最先进模型在处理需要空间维度推理的障碍类型时也表现不佳
...
TL;DR: MemStream通过增加token预算、自适应选择策略和免训练的检索专家混合，显著提升了流式视频理解在多个基准测试上的性能。
...
Method: 1) 增加token预算以实现更细粒度的时空理解；2) 引入自适应选择策略减少token冗余同时保留局部时空信息；3) 提出免训练的检索专家混合，利用外部模型更好地识别相关帧。

Result: MemStream在CG-Bench上提升+8.0%，LVBench上提升+8.5%，VideoMME(Long)上提升+2.4%（相比ReKV with Qwen2.5-VL-7B）。
...
TL;DR: 该研究探索使用形式化领域本体（OpenMath）通过检索增强生成来提升语言模型在数学领域的可靠性，发现本体引导的上下文在检索质量高时能改善性能，但无关上下文会显著降低性能。


<details>
  <summary>Details</summary>
Motivation: 语言模型存在幻觉、脆弱性和缺乏形式化基础等根本性限制，这在需要可验证推理的高风险专业领域尤为成问题。研究旨在探索形式化领域本体是否能通过检索增强生成来提升语言模型的可靠性。

Method: 使用数学作为概念验证，实现了一个神经符号管道，利用OpenMath本体，结合混合检索和交叉编码器重排序，将相关定义注入模型提示中。在MATH基准测试上评估了三个开源模型。

Result: 评估显示，当检索质量高时，本体引导的上下文能提高性能，但无关上下文会主动降低性能。这突显了神经符号方法的潜力和挑战。

Conclusion: 形式化领域本体有潜力通过检索增强生成提升语言模型的可靠性，但检索质量至关重要。无关上下文会损害性能，表明需要更精确的检索机制来充分发挥神经符号方法的优势。
...
TL;DR: TTG是一个基于编程谜题的对决式评估框架，让大语言模型相互挑战创建谜题，通过Elo评分比较模型推理能力，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法面临挑战：1) 人工创建难题成本高昂，特别是需要博士级领域知识的基准测试；2) 难以确定模型是否真正推理还是见过类似训练数据；3) 需要测试模型在创造力、任务创建等新技能上的表现。

Method: 受16世纪数学对决启发，设计Token Games框架：1) 使用编程谜题格式（给定返回布尔值的Python函数，找到使其返回True的输入）；2) 让模型相互挑战创建谜题；3) 通过成对对决结果计算Elo评分；4) 评估10个前沿模型。

Result: 1) TTG评估结果与Humanity's Last Exam等现有基准排名高度一致；2) 创建优质谜题对当前模型仍是极具挑战的任务，这是先前基准未测量的能力；3) 整个评估过程无需人工创建谜题。

Conclusion: TTG提出了一种新的评估范式：1) 通过设计避免饱和问题；2) 能够同时测试模型的问题解决、创造力和任务创建能力；3) 为评估大语言模型的推理能力提供了可持续、自动化的替代方案。
...
Result: 通过预期分数轨迹和残差分析多个指标家族的敏感性和校准性，揭示了不同指标家族之间的系统性差异，支持基于严重程度的工作流评估分数解释

Conclusion: WorkflowPerturb基准为工作流评估指标提供了受控研究环境，有助于理解指标行为并支持严重程度感知的评估分数解释，数据集将在接受后发布
...
Method: 结合离线强化学习和跨具身学习，构建包含16个不同机器人平台的运动数据集，分析该范式的优缺点，并提出基于形态相似性的分组策略来减少跨机器人梯度冲突。

Result: 实验表明，该方法在包含大量次优轨迹的数据集上优于纯行为克隆，但随着次优数据比例和机器人类型增加，跨形态梯度冲突会阻碍学习。提出的分组策略能有效减少冲突并优于现有冲突解决方法。

Conclusion: 离线强化学习与跨具身学习结合是有效的机器人策略预训练方法，但需要解决跨形态梯度冲突问题。基于形态相似性的静态分组策略是一种简单有效的解决方案。
...
TL;DR: 研究发现无监督表示学习在敏感属性被排除训练时仍会泄露这些信息，SOMtime方法能恢复被排除的年龄、收入等敏感属性，而传统方法如PCA、UMAP等表现较差，表明"通过无知实现公平"在表示层面失效。


<details>
  <summary>Details</summary>
Motivation: 挑战无监督表示学习对敏感属性保持中性的假设，研究即使敏感属性被明确排除在训练输入之外，是否仍会在无监督嵌入中作为主导潜在轴出现。

Method: 使用SOMtime（基于高容量自组织映射的拓扑保持表示方法），在两个大规模真实数据集（五个国家的世界价值观调查和人口普查收入数据集）上进行实验，比较SOMtime与PCA、UMAP、t-SNE和自编码器的表现。

Result: SOMtime能恢复与被排除敏感属性对齐的单调排序，Spearman相关性高达0.85，而PCA和UMAP通常低于0.23（单个例外达到0.31），t-SNE和自编码器最多达到0.34。无监督分割SOMtime嵌入会产生人口统计偏斜的聚类。

Conclusion: "通过无知实现公平"在表示层面对于有序敏感属性失效，公平性审计必须扩展到机器学习管道的无监督组件。
...
TL;DR: OMAD是首个在在线多智能体强化学习中应用扩散策略的框架，通过放松的策略目标最大化联合熵来促进探索，在CTDE范式下使用联合分布值函数优化分散的扩散策略，在多样任务上实现了2.5-5倍的样本效率提升。
...
Method: 提出OMAD框架：1）使用放松的策略目标最大化缩放联合熵，实现无需可处理似然的探索；2）在CTDE范式下，采用联合分布值函数优化分散的扩散策略；3）利用可处理的熵增强目标引导扩散策略的同步更新。

Result: 在MPE和MAMuJoCo的10个多样化任务上评估，OMAD成为新的最先进方法，实现了2.5倍到5倍的样本效率提升。

Conclusion: OMAD成功将扩散策略应用于在线多智能体强化学习，克服了扩散模型不可处理似然性的障碍，通过放松的策略目标和联合分布值函数实现了有效的探索和协调，显著提升了样本效率。
...
### [43] [MusicSem: A Semantically Rich Language--Audio Dataset of Natural Music Descriptions](https://arxiv.org/abs/2602.17769)
...
TL;DR: MusicSem是一个包含32,493个语言-音频对的数据集，源自Reddit上的有机音乐讨论，旨在解决现有模型难以捕捉用户自然语言描述中音乐意图的问题。


<details>
  <summary>Details</summary>
Motivation: 现有音乐多模态模型在捕捉用户自然语言描述中的音乐意图方面存在困难，这表明训练和评估这些模型的数据集未能充分反映人类描述音乐的更广泛、更自然的话语形式。

Method: 从Reddit社交平台收集有机音乐相关讨论，构建包含32,493个语言-音频对的MusicSem数据集，并提出包含描述性、氛围性、情境性、元数据相关性和上下文性五个语义类别的分类法。

Result: MusicSem数据集捕捉了更广泛的音乐语义谱系，反映了听众以细致和以人为本的方式自然描述音乐的方式。使用该数据集评估了多种多模态检索和生成模型，强调了建模细粒度语义的重要性。
...
TL;DR: 该研究发现MCQA基准测试存在文本线索漏洞，导致VLM无需视觉输入也能达到高准确率。作者提出新方法将盲猜准确率从+66.9%降至+2.9%，强制模型依赖视觉理解。


<details>
  <summary>Details</summary>
Motivation: 现有MCQA基准测试存在严重缺陷，合成生成的MCQA数据容易包含隐藏的文本线索，使得视觉语言模型可以通过利用语言模式而非视觉上下文来获得高准确率，这无法真实反映模型的视觉理解能力。

Method: 通过将正确答案与语言伪影解耦，并采用课程学习策略，消除可被利用的文本捷径，强制模型依赖视觉基础进行推理。

Result: 提出的方法显著降低了模型的盲猜准确率，从比随机猜测高66.9%降至仅高2.9%，消除了绝大多数可被利用的文本捷径，确保性能准确反映感知理解能力。

Conclusion: 该方法有效解决了MCQA基准测试中的文本线索漏洞问题，强制视觉语言模型真正依赖视觉基础进行推理，为准确评估VLM在驾驶任务中的视觉理解能力提供了可靠方法。
...
TL;DR: BioBridge是一个蛋白质理解的领域自适应持续预训练框架，通过结合蛋白质语言模型和通用大语言模型的优势，实现蛋白质序列理解和通用推理能力的统一。


<details>
  <summary>Details</summary>
Motivation: 现有蛋白质语言模型在多任务适应性和跨生物环境泛化方面有限，而通用大语言模型缺乏蛋白质序列解释能力和领域专业知识，限制了生物语义推理能力。

Method: 采用领域增量持续预训练框架，通过PLM-Projector-LLM管道实现跨模态对齐，将蛋白质序列嵌入映射到语言模型的语义空间，并通过端到端优化支持多种任务。

Result: BioBridge在EC和BindingDB等蛋白质基准测试中表现与主流PLMs相当，在MMLU和RACE等通用理解任务上与LLMs结果相当，展示了领域特定适应性和通用语言能力的创新优势。

Conclusion: BioBridge成功结合了蛋白质语言模型和通用大语言模型的优势，实现了蛋白质理解中领域专业知识与通用推理能力的有效融合，为解决蛋白质理解任务提供了创新框架。
...
TL;DR: 本文提出LATMiX方法，通过可学习的可逆仿射变换优化激活分布，在MX低比特量化中显著提升大语言模型的量化鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法主要关注传统量化方案，而现代硬件越来越多支持微缩放(MX)数据格式。现有方法将变换与MX量化结合时会出现严重的性能下降，且现有变换方法主要局限于旋转或Hadamard变换，缺乏对激活分布和量化结构的综合考虑。

Method: 首先对MX量化下的变换进行理论分析，推导量化误差边界，强调同时考虑激活分布和底层量化结构的重要性。基于此分析提出LATMiX方法，将异常值减少推广到可学习的可逆仿射变换，使用标准深度学习工具进行优化。

Result: 实验表明，在广泛的零样本基准测试中，LATMiX在MX低比特量化上相比强基线方法实现了平均准确率的持续提升，且在不同模型规模上均表现良好。

Conclusion: LATMiX通过可学习的可逆仿射变换有效优化激活分布，在MX量化格式下显著提升大语言模型的量化性能，为现代硬件上的高效部署提供了有效解决方案。
...
TL;DR: 本文提出了一种用于农田NDVI预测的概率性预测框架，采用Transformer架构分离历史植被动态与未来外生信息建模，通过时间距离加权分位数损失处理不规则采样，并在欧洲卫星数据上验证了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 精准农业需要准确的植被动态短期预测，但卫星观测的NDVI预测面临云层遮挡导致的不规则稀疏采样以及作物生长的异质性气候条件等挑战。

Method: 提出概率性预测框架，采用Transformer架构明确分离历史植被动态建模与未来外生信息整合；引入时间距离加权分位数损失处理不规则重访模式；结合累积和极端天气特征工程捕捉延迟气象效应。

Result: 在欧洲卫星数据上的大量实验表明，该方法在点预测和概率评估指标上均优于多种统计、深度学习和最新时间序列基线方法。消融研究进一步突出了目标历史的核心作用。

Conclusion: 该方法为晴空采集约束下的农田级NDVI预测提供了有效的概率预测框架，气象协变量与历史植被动态的联合利用能提供互补增益，代码已开源。
...
TL;DR: CodeScaler是一个无需执行的奖励模型，用于代码生成的强化学习和推理时扩展，通过语法感知的代码提取和保持有效性的奖励塑造，在多个代码基准上显著提升模型性能，同时大幅降低延迟。


<details>
  <summary>Details</summary>
Motivation: 基于可验证奖励的强化学习（RLVR）虽然推动了代码大语言模型的进展，但其可扩展性受到高质量测试用例可用性和可靠性的根本限制。需要一种无需执行的奖励模型来扩展强化学习训练和推理时的代码生成能力。

Method: CodeScaler是一个执行无关的奖励模型，基于经过验证的代码问题精心策划的偏好数据进行训练。方法包括语法感知的代码提取和保持有效性的奖励塑造，以确保稳定和鲁棒的优化。

Result: 在五个编码基准测试中，CodeScaler将Qwen3-8B-Base平均提升了11.72分，优于基于二进制执行的强化学习1.82分，并能在没有任何测试用例的合成数据集上进行可扩展的强化学习。推理时，CodeScaler作为有效的推理时扩展方法，性能与单元测试方法相当，同时延迟降低10倍。在RM-Bench上，CodeScaler不仅在代码领域超越现有奖励模型（+3.3分），在通用和推理领域也平均提升2.7分。

Conclusion: CodeScaler提供了一种无需执行的奖励建模方法，有效解决了基于测试的强化学习的可扩展性限制，在训练和推理阶段都能显著提升代码生成性能，同时大幅降低计算成本。
...
TL;DR: 本文提出了一种用于低地球轨道多目标主动碎片清除的统一共椭圆机动框架，通过三种规划算法（贪婪启发式、蒙特卡洛树搜索、掩码近端策略优化强化学习）进行对比，结果显示强化学习方法在任务效率和计算性能上表现最优。


<details>
  <summary>Details</summary>
Motivation: 低地球轨道中多目标主动碎片清除面临挑战，需要高效的规划方法来处理随机碎片场、禁飞区和燃料限制等复杂约束，以实现可扩展、安全和资源高效的空间任务规划。

Method: 提出统一共椭圆机动框架，结合霍曼转移、安全椭圆接近操作和显式加油逻辑。在包含随机碎片场、禁飞区和ΔV约束的真实轨道模拟环境中，对比测试贪婪启发式、蒙特卡洛树搜索和基于掩码近端策略优化的深度强化学习三种规划算法。

Result: 在100个测试场景中，掩码PPO强化学习方法表现出最优的任务效率和计算性能，访问的碎片数量可达贪婪算法的两倍，并在运行时间上显著优于蒙特卡洛树搜索方法。
...
### [50] [Robust Pre-Training of Medical Vision-and-Language Models with Domain-Invariant Multi-Modal Masked Reconstruction](https://arxiv.org/abs/2602.17689)
...
<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在成像设备、采集协议和报告风格等变化导致的领域偏移下性能下降，现有多模态预训练方法大多忽视鲁棒性问题，将其视为下游适应问题。

Method: 提出Robust Multi-Modal Masked Reconstruction (Robust-MMR)自监督预训练框架，整合非对称扰动感知掩码、领域一致性正则化和模态弹性约束，鼓励领域不变表示。

Result: 在VQA-RAD上达到78.9%跨域准确率，比最强基线提升3.8个百分点；在SLAKE和VQA-2019上分别达到74.6%和77.0%；扰动评估下VQA-RAD准确率从69.1%提升到75.6%；MELINDA跨域准确率从70.3%提升到75.2%；检索任务中平均排名退化从超过16降低到4.1。
...
### [51] [AnCoder: Anchored Code Generation via Discrete Diffusion Models](https://arxiv.org/abs/2602.17688)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型在代码生成中忽视编程语言的刚性结构，常产生无法执行的破碎程序

Method: 引入AnchorTree框架，利用抽象语法树作为结构化先验，优先解析关键词和标识符等关键token，建立结构支架指导后续生成

Result: 通过AnCoder模型系列验证，结构锚定的扩散方法为高质量代码生成提供了参数高效的路径

Conclusion: 结构锚定的扩散方法能有效提升代码生成质量，尊重编程语言结构特性
...
TL;DR: 本文提出了"智能体遗忘"方法，通过同步反向流遗忘框架同时从模型参数和持久记忆中移除特定信息，解决现有方法只针对参数而忽略参数-记忆反向流污染的问题。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘方法仅针对模型参数，存在两个关键缺陷：1) 参数-记忆反向流问题，检索会重新激活参数残留或记忆伪影重新引入敏感内容；2) 缺乏同时覆盖参数和记忆路径的统一策略。

Method: 提出同步反向流遗忘框架，包含两个路径：记忆路径执行基于依赖闭包的遗忘，修剪孤立实体并逻辑上使共享伪影失效；参数路径采用随机参考对齐，将模型输出导向高熵先验。通过同步双更新协议集成这两个路径，形成闭环机制。

Result: 在医疗问答基准测试中，SBU显著减少了目标隐私信息在两个路径上的痕迹，同时对保留数据的性能影响有限。

Conclusion: SBU框架为智能体遗忘提供了有效解决方案，能够同时从参数和记忆路径中移除敏感信息，防止跨路径再污染，在隐私保护方面表现出色。
...
TL;DR: 本文通过案例研究评估了在昇腾NPU上对推理导向模型进行后训练量化的可行性与限制，发现4位权重量化对大型模型可行，但4位权重-激活量化在长上下文推理中不稳定，而8位量化数值稳定。


<details>
  <summary>Details</summary>
Motivation: 后训练量化对于高效模型部署至关重要，但在昇腾NPU上的有效性相比GPU架构仍未被充分探索。本研究旨在填补这一空白，为在昇腾NPU上部署量化推理模型提供实践参考。

Method: 对代表性推理导向模型（DeepSeek-R1-Distill-Qwen系列和QwQ-32B）进行案例研究，评估四种不同的量化算法：AWQ、GPTQ、SmoothQuant和FlatQuant，覆盖从仅权重压缩到先进的基于旋转方法的全谱系。

Result: 实证结果显示显著的平台敏感性：4位仅权重量化对大型模型可行，但激进的4位权重-激活量化方案在NPU上存在层间校准不稳定性，导致长上下文推理任务中的逻辑崩溃；标准8位量化保持数值稳定；INT8实际部署显示优化内核虽降低延迟，但动态量化开销限制端到端加速。

Conclusion: 这些发现为在昇腾NPU上部署量化推理模型的可行性和局限性提供了实用参考，强调了平台特定优化的重要性，并指出当前动态量化开销是端到端加速的主要限制因素。
...
TL;DR: EXACT是一种解码时个性化方法，通过可解释属性对齐用户偏好，使用离线阶段识别用户特定属性子集，在线推理时检索与提示最相关的属性来指导生成。


<details>
  <summary>Details</summary>
Motivation: 现有解码时个性化方法主要依赖隐式、难以解释的偏好表示，并采用僵化的、与上下文无关的用户表示，无法处理偏好如何随提示变化的问题。需要一种能够适应不同任务、缓解上下文偏好转移的个性化方法。

Method: EXACT使用预定义的可解释属性集，通过最大化偏好响应的似然性在离线阶段识别用户特定属性子集，然后在在线推理时检索与输入提示最语义相关的属性，并将其注入上下文以引导生成。

Result: 在温和假设下建立了理论近似保证，证明相似性检索机制能有效缓解上下文偏好转移。在人工标注的偏好数据集上的实验表明，EXACT在偏好建模准确性和个性化生成质量方面持续优于强基线方法。

Conclusion: EXACT提供了一种可扩展、可解释的解码时个性化方法，能够适应不同任务而不混合冲突偏好，有效解决了现有方法在上下文偏好转移方面的局限性。
...
TL;DR: 当前的安全区域识别方法在不同粒度（从单个权重到整个Transformer层）和不同规模的LLM上表现不一致，识别出的安全区域重叠度低，且经过效用数据集精炼后重叠度进一步下降，表明现有技术无法可靠识别稳定、数据集无关的安全区域。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型中是否存在可修改以直接影响安全行为的"安全区域"，并对现有安全区域识别方法进行系统性评估。

Method: 系统评估四种不同参数粒度的安全区域识别方法（从单个权重到整个Transformer层），在四个不同规模的LLM家族上，使用十个安全识别数据集进行测试。

Result: 识别出的安全区域仅表现出低到中等的重叠度（通过IoU测量），当使用效用数据集（即非有害查询）进一步精炼时，重叠度显著下降。

Conclusion: 当前技术无法可靠识别稳定、数据集无关的安全区域，表明LLM中的安全机制可能比简单的参数子集修改更为复杂。
...
<details>
  <summary>Details</summary>
Motivation: 后训练权重量化对于减少大语言模型的内存和推理成本至关重要，但将平均精度推到4位以下仍然具有挑战性，主要因为权重敏感性高度不均匀且缺乏原则性的精度分配方法。现有解决方案要么使用高运行时开销的不规则细粒度混合精度，要么依赖启发式或高度受限的精度分配策略。

Method: 提出了ScaleBITS混合精度量化框架：1）基于新的敏感性分析；2）引入硬件对齐的块级权重分区方案，通过双向通道重排序实现；3）将全局位宽分配公式化为约束优化问题，并开发了贪心算法的可扩展近似方法，实现端到端原则性分配。

Result: 实验表明，ScaleBITS相比均匀精度量化显著提升（高达+36%），在超低位宽量化中优于最先进的敏感性感知基线方法（高达+13%），且不增加运行时开销。

Conclusion: ScaleBITS框架在保持硬件效率的同时，实现了内存预算下的自动化细粒度位宽分配，解决了超低位宽量化中的精度分配挑战，为大语言模型的高效部署提供了有效解决方案。
...
### [57] [MIDAS: Mosaic Input-Specific Differentiable Architecture Search](https://arxiv.org/abs/2602.17700)
*Konstanty Subbotko*

Main category: cs.LG

TL;DR: MIDAS是一种改进的可微分神经架构搜索方法，通过自注意力机制计算动态、输入特定的架构参数，并引入局部化架构选择和拓扑感知搜索空间，在多个基准测试中取得了优异性能。
...
Method: MIDAS通过自注意力机制计算动态、输入特定的架构参数，取代DARTS中的静态参数。采用两种关键技术：(i) 局部化架构选择，为激活图的每个空间补丁单独计算架构选择；(ii) 引入参数免费、拓扑感知的搜索空间，建模节点连接性并简化每个节点的两个输入边选择。

Result: 在DARTS搜索空间上，CIFAR-10达到97.42% top-1准确率，CIFAR-100达到83.38%；在NAS-Bench-201上始终能找到全局最优架构；在RDARTS上，在CIFAR-10的四个搜索空间中的两个上创造了新的最先进水平。

Conclusion: MIDAS通过动态、输入特定的架构参数和局部化选择机制，显著提升了可微分NAS的性能和鲁棒性。分析表明，补丁级注意力提高了候选操作的区分度，产生的输入特定参数分布具有类别感知性和单峰性，为解码提供了可靠指导。
...
### [58] [Pimp My LLM: Leveraging Variability Modeling to Tune Inference Hyperparameters](https://arxiv.org/abs/2602.17697)
*Nada Zine,Clément Quinton,Romain Rouvoy*

Main category: cs.LG

TL;DR: 该论文提出将大语言模型视为可配置系统，应用变异性管理技术来系统分析推理时配置选择，以优化能效和可持续性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的计算需求巨大，特别是推理阶段占主导地位，但推理服务器的庞大配置空间使得穷举实证评估不可行，需要系统方法来优化能效和可持续性。
...
Conclusion: 这项工作通过利用变异性建模为LLMs的高效可持续配置开辟了新的研究方向，桥接了软件工程和机器学习领域。
...
Method: 提出使用基于Transformer的GAN（TTS-GAN）生成合成数据，将其与真实数据结合训练LSTM预测模型，并设计结合DTW和改进DeD-iMs的时间序列质量评估指标

Result: 在比特币和标普500价格数据上，使用GAN增强数据训练的LSTM模型在不同预测时间范围内均显著优于仅使用真实数据的模型

Conclusion: GAN作为数据增强工具能有效克服金融领域数据稀缺问题，提升深度学习预测模型的性能，为金融预测提供了有前景的方法
...
### [60] [MantisV2: Closing the Zero-Shot Gap in Time Series Classification with Synthetic Data and Test-Time Strategies](https://arxiv.org/abs/2602.17868)
*Vasilii Feofanov,Songkang Wen,Jianfeng Zhang,Lujia Pan,Ievgen Redko*

Main category: cs.LG

TL;DR: 本文提出了Mantis+和MantisV2两种时间序列基础模型，通过合成数据预训练、架构优化和测试时方法改进，显著提升了零样本特征提取性能，在多个基准测试中达到SOTA。
...
Method: 1. 提出Mantis+，完全在合成时间序列上预训练的Mantis变体；2. 通过受控消融研究改进架构，得到更轻量级的MantisV2编码器；3. 提出增强的测试时方法，利用中间层表示并改进输出令牌聚合；4. 通过自集成和跨模型嵌入融合进一步提升性能。

Result: 在UCR、UEA、人类活动识别(HAR)基准测试和EEG数据集上的广泛实验表明，MantisV2和Mantis+始终优于先前的时间序列基础模型，实现了最先进的零样本性能。

Conclusion: 通过合成数据预训练、架构优化和测试时方法改进，成功开发了性能显著提升的时间序列基础模型，缩小了冻结编码器和微调编码器之间的性能差距，为时间序列分析提供了更强大的通用特征提取器。
...
TL;DR: 论文提出一个理论框架，为分布偏移下的预测器风险提供可计算的上界，通过可验证的度量来保证模型在分布偏移下的性能。


<details>
  <summary>Details</summary>
Motivation: 在现实应用中，机器学习模型经常面临训练分布与测试分布不一致的问题（分布偏移）。现有方法通常缺乏理论保证，难以量化模型在分布偏移下的性能下降。本文旨在建立一个统一的数学框架，为分布偏移下的风险提供可验证的上界，确保模型在非平凡规模下的可靠性。

Method: 提出一个理论框架，基于可验证的规律性和复杂性约束，推导出分布偏移下超额风险的显式上界。该上界由可计算的分布偏移度量和模型参数决定。框架包含三个核心要素：(i) 通过显式不等式证明分布偏移下的风险；(ii) 确保学习模型验证在非平凡规模下的可靠性；(iii) 通过可识别性条件而非事后解释来强制可解释性。

Result: 建立了分布偏移下风险认证的统一框架，提供了可计算的风险上界。所有声明都基于明确的假设，能够隔离失效模式，并表征不可认证的机制。该框架为模型在分布偏移下的性能提供了理论保证。

Conclusion: 本文提出了一个理论框架，能够为分布偏移下的预测器风险提供可验证的上界。该框架通过可计算的度量来保证模型性能，明确了假设条件，能够识别失效模式，并区分可认证与不可认证的机制，为机器学习模型在现实分布偏移场景下的可靠性提供了理论基础。
...
TL;DR: 该研究开发了机器学习模型，利用术前数据预测慢性鼻窦炎患者的手术获益情况，模型准确率达85%，在30例测试病例中准确率80%，超过临床专家平均水平。


<details>
  <summary>Details</summary>
Motivation: 慢性鼻窦炎（CRS）患者中部分对药物治疗无效，但手术决策复杂且结果不确定。目前缺乏基于前瞻性临床试验数据的机器学习预测研究，而这类研究有潜力降低医疗成本并改善患者预后。

Method: 使用观察性干预试验的前瞻性收集队列数据，所有患者均接受手术。采用监督式机器学习模型，仅基于术前数据预测手术获益，以Sino-Nasal Outcome Test-22（SNOT-22）为主要患者报告结局指标。测试了多种算法包括集成方法。

Result: 最佳模型达到约85%的分类准确率，能够准确且可解释地预测手术候选资格。在包含30个不同难度病例的测试集上，模型准确率达80%，超过临床专家平均预测准确率（75.6%）。

Conclusion: 机器学习模型能够有效预测慢性鼻窦炎患者的手术获益，准确率超过临床专家，有潜力增强临床决策支持，实现个性化CRS治疗。
...
TL;DR: PaCoDi是一种基于频域的并行复杂扩散模型，通过傅里叶变换解耦时间序列生成中的长程依赖关系，在保持生成质量的同时将注意力计算复杂度降低50%。
...
Method: 提出PaCoDi架构，在频域进行生成建模：1）使用傅里叶变换作为对角化算子，将局部耦合的时间信号转换为全局解耦的频谱分量；2）证明正交前向扩散和条件反向分解定理，将复杂扩散过程分解为独立的实部和虚部分支；3）使用平均场理论近似和交互校正机制；4）利用实值信号的厄米对称性将序列长度压缩一半；5）推导异方差损失处理压缩流形上的非各向同性噪声分布。

Result: PaCoDi在生成质量和推理速度方面均优于现有基线方法，实现了理论上严谨且计算高效的时间序列建模解决方案。

Conclusion: PaCoDi通过频域解耦方法从根本上改变了时间序列生成的问题拓扑结构，在保持理论严谨性的同时显著提升了计算效率，为解决长程依赖建模中的计算瓶颈提供了新思路。
...
### [64] [MIRA: Memory-Integrated Reinforcement Learning Agent with Limited LLM Guidance](https://arxiv.org/abs/2602.17930)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在稀疏或延迟奖励环境中样本效率低，而完全依赖LLM监督存在可扩展性限制和不可靠信号依赖的问题。需要一种既能利用LLM先验知识又能减少对其持续依赖的方法。

Method: 构建结构化记忆图，存储高回报经验轨迹和LLM输出的子目标结构。从记忆图中推导出效用信号，软调整优势估计来影响策略更新，而不改变基础奖励函数。随着训练进行，效用项逐渐衰减。

Result: MIRA在稀疏奖励环境中优于传统RL基线，达到与频繁使用LLM监督方法相当的回报，同时显著减少了在线LLM查询次数。

Conclusion: MIRA通过结构化记忆图有效利用LLM先验知识，在减少对实时LLM监督依赖的同时提高了稀疏奖励环境中的学习效率，并保持了标准收敛保证。
...
### [65] [Provable Adversarial Robustness in In-Context Learning](https://arxiv.org/abs/2602.17743)
*Di Zhang*

Main category: cs.LG

TL;DR: 论文提出了一个分布鲁棒的元学习框架，为基于Wasserstein距离的分布偏移下的上下文学习提供最坏情况性能保证，揭示了模型鲁棒性与容量平方根成正比，对抗性设置会带来样本复杂度惩罚。


<details>
  <summary>Details</summary>
Motivation: 当前对大型语言模型上下文学习能力的理论解释假设测试任务与预训练时的分布相似，这忽略了对抗性分布偏移对现实世界可靠性的威胁。需要填补这一理论空白，研究上下文学习在对抗性条件下的鲁棒性。

Method: 引入分布鲁棒的元学习框架，针对线性自注意力Transformer，在Wasserstein距离度量的分布偏移下推导非渐近性能边界，分析对抗扰动强度、模型容量和上下文示例数量之间的关系。

Result: 分析表明模型鲁棒性随其容量的平方根缩放（ρ_max ∝ √m），而对抗性设置带来的样本复杂度惩罚与扰动幅度的平方成正比（N_ρ - N_0 ∝ ρ^2）。合成任务实验验证了这些缩放规律。

Conclusion: 这些发现推进了对对抗性条件下上下文学习极限的理论理解，表明模型容量是分布鲁棒性的基本资源，为构建更可靠的上下文学习系统提供了理论指导。
...
### [66] [Memory-Based Advantage Shaping for LLM-Guided Reinforcement Learning](https://arxiv.org/abs/2602.17931)
*Narjes Nourzad,Carlee Joe-Wong*

Main category: cs.LG

TL;DR: 该论文提出了一种结合大语言模型（LLM）和记忆图的方法来改进稀疏奖励环境中的强化学习样本效率，通过构建记忆图编码LLM指导的成功轨迹，并从中推导效用函数来指导优势函数，减少对持续LLM调用的依赖。


<details>
  <summary>Details</summary>
Motivation: 在稀疏或延迟奖励环境中，强化学习需要大量交互导致样本复杂度高。虽然大语言模型可用于子目标发现和轨迹指导，但频繁调用LLM会带来可扩展性和可靠性问题。需要一种方法既能利用LLM的指导能力，又不过度依赖在线LLM监督。

Method: 构建记忆图编码来自LLM指导和智能体自身成功轨迹的子目标和轨迹。从该图中推导效用函数，评估智能体轨迹与先前成功策略的匹配程度。该效用函数用于塑造优势函数，为critic提供额外指导而不改变奖励信号。主要依赖离线输入，仅偶尔进行在线查询，避免持续LLM监督。

Result: 在基准环境中的初步实验显示，相比基线RL方法，该方法提高了样本效率并加速了早期学习。最终回报与需要频繁LLM交互的方法相当，但减少了LLM调用需求。

Conclusion: 通过结合LLM指导和记忆图构建，可以在稀疏奖励环境中有效提高强化学习的样本效率，同时减少对持续LLM监督的依赖，平衡了指导效果与可扩展性。
...
### [67] [Asking Forever: Universal Activations Behind Turn Amplification in Conversational LLMs](https://arxiv.org/abs/2602.17778)
*Zachary Coalson,Bo Fang,Sanghyun Hong*

Main category: cs.LG

TL;DR: 论文发现对话LLM存在"轮次放大"故障模式，攻击者可系统利用澄清寻求行为延长多轮对话而不完成任务，且该攻击具有跨提示和任务的通用性。


<details>
  <summary>Details</summary>
Motivation: 多轮交互长度是对话LLM运营成本的主要因素，需要识别和防御可能导致成本异常增加的新故障模式。

Method: 从机制角度识别与澄清寻求响应相关的查询无关通用激活子空间，通过微调供应链攻击和运行时参数损坏两种方式诱导轮次放大行为。

Result: 在多个指令调优LLM和基准测试中，攻击显著增加了轮次计数，同时保持合规性，现有防御措施对此类故障保护有限。

Conclusion: 轮次放大是对话LLM的新故障模式，源于对话动态而非单轮提示优化，需要开发新的防御机制来应对此类系统性攻击。
...
### [68] [Grassmannian Mixture-of-Experts: Concentration-Controlled Routing on Subspace Manifolds](https://arxiv.org/abs/2602.17798)
...
<details>
  <summary>Details</summary>
Motivation: 传统MoE模型使用softmax门控缺乏控制稀疏性与利用率之间权衡的理论机制，且存在专家崩溃问题。需要一种几何原理明确、可连续控制稀疏性的路由框架。

Method: 在Grassmann流形子空间上构建路由框架，使用矩阵Bingham分布的浓度参数Λ作为控制路由熵的单一可解释参数，替代离散的top-k选择。开发摊销变分推理方法估计后验路由分布，实现不确定性感知的专家分配。

Result: 在350M/1.3B/2.7B参数的MoE语言模型上，GrMoE实现0%路由崩溃，困惑度相当或更好，负载均衡提升15-30%。浓度与有效稀疏度呈平滑单调关系，支持训练后稀疏性调优。专家学习到与语言专业化相关的异质浓度值。

Conclusion: GrMoE建立了首个浓度控制稀疏性的形式化理论，通过几何原理明确的连续稀疏机制解决了MoE路由的核心问题，为可解释、可控制的专家混合模型提供了新框架。
...
### [69] [Learning Optimal and Sample-Efficient Decision Policies with Guarantees](https://arxiv.org/abs/2602.17978)
*Daqian Shao*

Main category: cs.LG

TL;DR: 该论文提出了一种从包含隐藏混杂因素的离线数据集中学习决策策略的方法，使用工具变量解决因果效应识别问题，并扩展到模仿学习和线性时序逻辑目标学习，具有收敛性和最优性保证。
...
Method: 1. 使用工具变量识别因果效应，将其建模为条件矩限制问题；2. 受双重/去偏机器学习启发，开发具有收敛和最优性保证的样本高效算法；3. 在模仿学习中放宽对隐藏混杂因素的条件限制；4. 开发学习线性时序逻辑表达的高层目标的算法。

Result: 提出的CMR求解算法优于现有最先进算法；模仿学习算法能够学习有效的模仿策略并具有收敛速率保证；LTL学习算法在样本效率上优于现有方法。在强化学习基准和合成/半合成数据集上验证了方法的有效性。

Conclusion: 该论文开发的方法能够有效处理离线学习中的隐藏混杂因素问题，为实际决策制定提供了理论保证和实用工具，特别适用于高风险应用场景。
...
TL;DR: 提出Stiefel-Bayes Adapters (SBA)，一种贝叶斯参数高效微调框架，通过在Stiefel流形上放置Matrix Langevin先验，提供校准的预测不确定性，在领域偏移下表现更可靠。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法如LoRA缺乏原则性的不确定性估计，导致预测校准不佳，在领域偏移下行为不可靠。需要一种既能保持参数效率又能提供校准不确定性的方法。

Method: 在Stiefel流形上放置Matrix Langevin先验于正交适配器因子，通过切空间Laplace近似与测地线回缩进行近似后验推断。与平坦空间的高斯先验不同，该方法在流形上自然编码适配器子空间应良好条件和正交的归纳偏置。

Result: 在GLUE、SuperGLUE基准测试及多个模型上，SBA在任务性能上与LoRA和DoRA相当，同时将预期校准误差降低18-34%，在领域偏移下选择性预测AUROC提高12-25%，以更少参数成本在OOD检测上优于五个LoRA模型的深度集成。

Conclusion: 在正确的几何结构上放置不确定性比简单为适配器添加任何贝叶斯处理更重要。SBA框架提供了校准的预测不确定性，在领域偏移下更可靠，同时保持参数效率。
...
<details>
  <summary>Details</summary>
Motivation: 现实物理系统是耗散的，从部分观测（仅位置数据）预测其动力学是科学机器学习中的核心挑战。需要解决仅位置数据问题：给定离散时间的广义位置（动量为隐变量），学习结构化模型以实现稳定长期预测并在提供足够结构时恢复物理参数。

Method: 提出PHAST（端口哈密顿结构化时间动力学架构），基于端口哈密顿框架将保守-耗散分解显式化，将哈密顿量分解为势能、质量和阻尼三个知识域（已知、部分已知、未知），使用高效低秩PSD/SPD参数化，并通过Strang分裂推进动力学。

Result: 在13个仅位置基准测试（涵盖机械、电气、分子、热、引力和生态系统）中，PHAST在竞争基线中实现了最佳长期预测，并在知识域提供足够锚点时实现物理参数恢复。研究表明，没有此类锚点的情况下识别是病态的（规范自由度）。

Conclusion: PHAST通过结构化端口哈密顿方法有效解决了仅位置数据的动力学预测问题，实现了稳定长期预测和物理参数恢复，提出了分离预测稳定性与可识别性的双轴评估框架。
...
### [72] [Causality by Abstraction: Symbolic Rule Learning in Multivariate Timeseries with Large Language Models](https://arxiv.org/abs/2602.17829)
...
Method: ruleXplain框架利用大语言模型，通过引入具有时间算子和延迟语义的约束符号规则语言，让LLM通过结构化提示生成可验证的因果规则。该方法使用仿真器生成多样化的反事实输入轨迹，将其聚类后提供给LLM，LLM负责生成编码联合时间趋势的符号规则，并通过闭环细化过程确保规则一致性和语义有效性。

Result: 使用PySIRTEM流行病模拟器和EnergyPlus建筑能源模拟器进行验证，通过三类实验评估：1）通过输入重建评估规则集的有效性；2）通过消融研究评估规则集的因果编码；3）在具有不同相位动态的未见输出趋势上测试提取规则的泛化能力。

Conclusion: ruleXplain框架能够从仿真驱动动力系统中提取形式化的因果解释，通过约束符号规则语言和大语言模型的结合，解决了传统方法在复杂动态系统中难以产生泛化和可解释解释的问题。
...
### [73] [NIMMGen: Learning Neural-Integrated Mechanistic Digital Twins with LLMs](https://arxiv.org/abs/2602.18008)
*Zihan Guan,Rituparna Datta,Mengxuan Hu,Shunshun Liu,Aiying Zhang,Prasanna Balachandran,Sheng Li,Anil Vullikanti*

Main category: cs.LG

TL;DR: 本文提出NIMM评估框架，用于评估LLM生成的机理模型在现实条件下的可靠性，并开发了NIMMgen框架来提升模型正确性和实用性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的机理模型构建框架过于简化现实条件，无法确定LLM生成的机理模型在实际应用中的可靠性，需要建立更真实的评估框架。

Method: 提出NIMM评估框架，在部分观测和多样化任务目标的现实设置下评估LLM生成的机理模型；并设计NIMMgen框架，通过迭代精炼提升代码正确性和实际有效性。

Result: 评估揭示了当前基线方法在模型有效性和代码级正确性方面的基本挑战；NIMMgen在三个不同科学领域的数据集上表现出色，且学习到的机理模型支持反事实干预模拟。

Conclusion: NIMM框架为评估LLM生成的机理模型提供了现实的测试环境，NIMMgen框架通过迭代精炼显著提升了模型质量和实用性，为科学建模提供了可靠工具。
...
### [74] [MePoly: Max Entropy Polynomial Policy Optimization](https://arxiv.org/abs/2602.17832)
...
Method: 提出MePoly方法，基于多项式能量模型进行策略参数化，利用经典矩问题的理论基础，通过多项式能量模型提供显式可处理的概率密度，实现精确熵最大化

Result: MePoly能够有效捕捉复杂的非凸流形，在多样化基准测试中性能优于基线方法

Conclusion: MePoly通过多项式能量模型为策略参数化提供新方法，解决了多模态表示和精确熵最大化的挑战，在理论和实证上都表现出优越性
...
### [75] [Two Calm Ends and the Wild Middle: A Geometric Picture of Memorization in Diffusion Models](https://arxiv.org/abs/2602.17846)
*Nick Dodson,Xinyu Gao,Qingsong Wang,Yusu Wang,Zhengchao Wan*

Main category: cs.LG

TL;DR: 扩散模型存在训练数据记忆风险，作者提出几何框架将噪声调度分为三个区域，识别出中噪声区域是记忆风险最高的"危险区"，并提出了几何感知的干预措施来缓解记忆问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型能生成高质量样本，但也可能记忆训练数据，引发严重的隐私问题。目前对记忆与泛化机制的理解仍不充分，特别是记忆在噪声调度中的位置、数据几何的影响以及不同噪声尺度现象的相互作用。

Method: 引入几何框架，基于高斯壳覆盖训练数据的特性和后验集中行为，将噪声调度分为三个区域。识别出中噪声区域为记忆风险最高的"危险区"，并提出几何感知的针对性干预措施来缓解记忆问题。

Result: 记忆风险在噪声水平上高度不均匀分布。小噪声区域因训练覆盖有限而避免记忆，大噪声区域后验集中度低且具有可证明的近线性高斯去噪行为，而中噪声区域记忆风险最为显著。

Conclusion: 扩散模型的记忆风险在噪声调度中呈非均匀分布，中噪声区域是主要风险区。通过几何框架分析，可以设计针对性的干预措施来缓解记忆问题，同时保持模型的生成能力。
...
TL;DR: 本文提出使用梯度正则化（GR）替代传统的KL惩罚来解决RLHF中的奖励黑客问题，通过将策略更新偏向奖励模型更准确的区域来提高性能。


<details>
  <summary>Details</summary>
Motivation: 在语言模型的后训练中，RLHF和RLVR面临奖励黑客问题，即策略可能利用奖励模型的不准确性学习到非预期的行为。传统方法使用KL惩罚限制策略更新，但效果有限。

Method: 提出梯度正则化（GR）方法：1）理论推导奖励模型准确性与收敛时最优解的平坦性之间的关系；2）使用梯度正则化将训练偏向平坦区域以保持奖励模型准确性；3）提出高效的有限差分估计实现显式GR。

Result: 实验表明：1）梯度范数与奖励准确性在RLHF中呈负相关；2）GR在多种RL实验中优于KL惩罚；3）在RLHF中获得更高的GPT评判胜率；4）避免基于规则的数学奖励中过度关注格式；5）防止LLM-as-a-Judge数学任务中的评判黑客攻击。
...
### [77] [JAX-Privacy: A library for differentially private machine learning](https://arxiv.org/abs/2602.17861)
...
TL;DR: JAX-Privacy是一个用于简化差分隐私机器学习机制部署的库，旨在提供易用、灵活、高效的解决方案，适用于研究人员和从业者。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够简化差分隐私机器学习机制部署的库，解决现有工具在易用性、灵活性和效率方面的不足，同时整合最新的差分隐私ML研究成果。

Method: 基于JAX框架构建，提供经过验证的模块化原语，涵盖批量选择、梯度裁剪、噪声添加、会计计算和审计等关键组件，支持深度定制和开箱即用两种使用模式。

Result: 创建了一个统一的库，将大量最新的差分隐私ML研究成果整合到一个框架中，为研究人员提供深度定制能力，同时为从业者提供易于使用的解决方案。

Conclusion: JAX-Privacy通过其设计原则和模块化架构，成功简化了差分隐私机器学习机制的部署，促进了该领域的研究和应用发展。
...
TL;DR: ADAPT是一种结合束搜索初始化和自适应梯度引导变异的混合方法，用于优化LLM激活空间中方向的文本特征可视化，在Gemma 2 2B模型的稀疏自编码器潜在空间上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 理解LLM激活空间中学习到的方向编码了哪些特征需要找到能强烈激活这些方向的输入。特征可视化通过优化输入来最大化激活目标方向，为昂贵的数据库搜索方法提供了替代方案，但由于文本的离散性，在LLM中仍未被充分探索。现有的提示优化技术也不适合这个领域，因为容易陷入局部最小值。

Method: ADAPT是一种混合方法，结合了束搜索初始化和自适应梯度引导变异，专门针对这些失败模式设计。该方法在Gemma 2 2B模型的稀疏自编码器潜在空间上进行评估，提出了基于数据集激活统计的度量标准以实现严格比较。

Result: ADAPT在不同层和潜在类型上始终优于先前的方法。结果表明，LLM的特征可视化是可行的，但需要针对该领域量身定制的设计假设。

Conclusion: LLM的特征可视化是可行的，但需要专门针对文本离散性和局部最小值问题设计的优化方法。ADAPT通过结合束搜索和自适应梯度变异，为这一领域提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型在离线强化学习中表现出色，但扩展到在线微调时面临挑战。现有方法将在线微调视为离线预训练的直接延续，未能解决关键问题，特别是在目标分布定义不明确的情况下。

Method: FINO采用流匹配策略，通过注入噪声到策略训练中促进探索，鼓励超出离线数据集观察范围的动作。同时结合熵引导采样机制来平衡探索与利用，使策略能够在在线微调过程中自适应调整行为。

Result: 在多样化的挑战性任务上的实验表明，FINO在有限的在线预算下始终实现优越性能，证明了其在离线到在线强化学习中的有效性。

Conclusion: FINO通过流匹配策略结合噪声注入和熵引导采样，有效解决了离线到在线强化学习中的探索挑战，在样本效率方面表现出色，为生成模型在强化学习中的应用提供了新思路。
...
### [80] [Capabilities Ain't All You Need: Measuring Propensities in AI](https://arxiv.org/abs/2602.18182)
...
TL;DR: 论文提出了首个测量AI倾向性的正式框架，使用双逻辑公式描述模型成功概率，当倾向性处于"理想区间"时成功概率最高。该框架能测量倾向性偏移及其对任务的影响，结合倾向性和能力评估能获得更强的预测能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI评估主要关注能力测量，使用项目反应理论等正式方法。然而，倾向性（模型展现特定行为的趋势）在决定性能和安全结果中起核心作用。传统IRT方法不适合测量倾向性，因为倾向性过高或过低都可能有问题。

Method: 引入双逻辑公式来描述模型成功概率，当模型倾向性处于"理想区间"时成功概率最高。使用配备新开发的任务无关量表的LLM来估计理想区间的界限。将框架应用于六个LLM模型家族，测量倾向性偏移及其对任务的影响。

Result: 能够测量倾向性偏移程度及其对任务的影响。使用一个基准估计的倾向性成功预测了保留任务的行为。结合倾向性和能力评估比单独使用任一种方法获得更强的预测能力。
...
### [81] [LERD: Latent Event-Relational Dynamics for Neurodegenerative Classification](https://arxiv.org/abs/2602.18195)
*Hairong Chen,Yicheng Feng,Ziyu Jia,Samir Bhatt,Hengguan Huang*

Main category: cs.LG

TL;DR: 提出LERD模型，一种端到端的贝叶斯电生理神经动力学系统，直接从多通道EEG推断潜在神经事件及其关系结构，用于阿尔茨海默病的诊断和表征。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病改变大脑电生理并破坏多通道EEG动力学，现有方法大多依赖黑盒分类器，没有明确建模生成观测信号的基础动力学机制。

Method: 提出LERD模型，结合连续时间事件推断模块和随机事件生成过程来捕捉灵活的时间模式，并引入电生理启发的动力学先验来指导学习。提供理论分析，获得可处理的训练边界和推断关系动力学的稳定性保证。

Result: 在合成基准和两个真实世界AD EEG队列上的广泛实验表明，LERD始终优于强基线，并产生与生理对齐的潜在摘要，有助于表征群体水平的动力学差异。

Conclusion: LERD为从多通道EEG推断神经事件和关系结构提供了一种原则性方法，在AD诊断中表现出优越性能，并能提供有生理意义的潜在表征。
...
TL;DR: 该研究复现了Abdelnabi等人(2024)基于可评分游戏的LLM多智能体谈判基准，发现虽然基准复杂但模型比较存在模糊性，实验设置存在信息泄露检测和消融研究不足等局限。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在多智能体谈判任务中展现潜力，但缺乏稳健且可泛化的评估基准。Abdelnabi等人(2024)提出了基于可评分游戏的谈判基准，本研究旨在验证其声明的可复现性，并深入理解其可用性和泛化能力。

Method: 1. 复现原始实验并在更多模型上进行测试；2. 引入额外指标验证谈判质量和评估公平性；3. 在扩展版基准上检查更广泛模型的行为；4. 分析实验设置的局限性，特别是信息泄露检测和消融研究的完整性。

Result: 1. 基准确实复杂，但模型比较存在模糊性，引发对其客观性的质疑；2. 发现实验设置存在局限性，特别是信息泄露检测不足和消融研究不够彻底；3. 通过分析更广泛模型在扩展基准上的表现，为潜在用户提供了额外背景信息；4. 结果强调了上下文在模型比较评估中的重要性。

Conclusion: 该研究揭示了Abdelnabi等人(2024)谈判基准在模型比较客观性方面的局限性，强调了评估基准设计需要更严谨的实验设置和更全面的分析框架，为未来LLM多智能体谈判评估提供了重要参考。
...
TL;DR: 论文分析了注意力回归模型中PCC停滞现象，揭示了MSE优化与PCC梯度冲突、softmax注意力限制等理论原因，并提出ECA方法突破PCC瓶颈


<details>
  <summary>Details</summary>
Motivation: 注意力回归模型训练中常出现PCC停滞现象：即使MSE持续下降，PCC在训练早期就停止改善。这一现象缺乏理论解释，阻碍了模型在保持误差大小的同时改善预测形状/顺序的能力

Method: 1. 理论分析PCC停滞的两个根本原因：MSE优化与PCC梯度冲突，以及凸聚合器（包括softmax注意力）的PCC改进极限；2. 提出外推相关性注意力（ECA），包含理论驱动的机制来改善PCC优化并超越凸包限制

Result: 1. 揭示了MSE降低会抑制PCC梯度的矛盾冲突；2. 推导出凸聚合器的PCC改进极限；3. ECA在多样化基准测试中（包括挑战性的同质数据设置）持续打破PCC停滞，显著改善相关性而不损害MSE性能

Conclusion: PCC停滞现象源于优化动态和模型容量的根本限制，数据同质性加剧了这些问题。ECA通过理论指导的机制成功突破了这些限制，为改善注意力回归模型的形状匹配能力提供了有效解决方案
...
### [84] [Distribution-Free Sequential Prediction with Abstentions](https://arxiv.org/abs/2602.17918)
*Jialin Yu,Moïse Blanchard*

Main category: cs.LG

TL;DR: 研究在对抗性实例注入的流数据中进行序列预测的问题，学习者可以弃权而不受惩罚，探索在未知分布下的分布无关学习保证


<details>
  <summary>Details</summary>
Motivation: 现有研究假设学习者已知干净样本分布，这在理论和实践中都是强假设。本文旨在探索在未知分布下的分布无关学习，这是经典学习框架（如PAC学习）和其他非独立同分布模型（如平滑在线学习）中的标准设置

Method: 提出基于弱学习器提升过程的AbstainBoost算法，用于分布无关的弃权学习，适用于一般VC类对抗遗忘对手，对自适应对手也适用于结构化函数类（如线性分类器）

Result: 算法保证了一般VC类在分布无关弃权学习中对遗忘对手具有次线性误差，对自适应对手在结构化函数类上也有类似保证。同时提供了相应的下界，揭示了误分类错误与错误弃权次数之间的多项式权衡关系

Conclusion: 在未知分布的情况下，通过AbstainBoost算法可以实现分布无关的弃权学习，填补了现有研究的空白，为半对抗性设置下的学习提供了理论保证
...
### [85] [Decoding as Optimisation on the Probability Simplex: From Top-K to Top-P (Nucleus) to Best-of-K Samplers](https://arxiv.org/abs/2602.18292)
*Xiaotong Ji,Rasul Tutunov,Matthieu Zimmer,Haitham Bou-Ammar*

Main category: cs.LG

TL;DR: 论文提出将解码过程视为一个原则化的优化层，通过正则化问题统一各种解码方法，并基于此框架设计了新的Best-of-K解码器以提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型的解码过程仍被视为启发式的参数调整，缺乏统一的理论框架。作者认为解码应该被理解为一个原则化的优化层，能够系统性地解释现有方法并便于设计新的解码策略。

Method: 提出一个统一的解码框架：在每个token位置，解决一个在概率单纯形上的正则化优化问题，平衡模型分数与结构偏好/约束。该框架将贪婪解码、Softmax采样、Top-K、Top-P和Sparsemax稀疏性等作为特例。基于此框架设计了Best-of-K解码器，使用KL锚定的覆盖目标来优化多样本管道。

Result: 统一框架能够解释现有解码方法的共同结构。Best-of-K解码器在固定K样本预算内优化覆盖良好替代方案的概率，显著提升了性能。例如，Qwen2.5-Math-7B在MATH500数据集上，在高采样温度下准确率提升了+18.6%。

Conclusion: 解码应该被视为语言模型的原则化优化层，而不是启发式参数调整。提出的统一框架不仅解释了现有解码方法，还便于设计新的解码策略，如Best-of-K解码器，能够显著提升多样本管道的性能。
...
### [86] [Analyzing and Improving Chain-of-Thought Monitorability Through Information Theory](https://arxiv.org/abs/2602.18297)
*Usman Anwar,Tim Bakker,Dana Kianfar,Cristina Pinneri,Christos Louizos*

Main category: cs.LG

TL;DR: 本文通过信息论分析发现思维链与输出之间的非零互信息是思维链可监控的必要但不充分条件，提出了两种近似误差来源，并开发了两种训练方法显著提升监控准确性。


<details>
  <summary>Details</summary>
Motivation: 思维链监控器是基于LLM的系统，用于分析推理轨迹以检测输出是否具有特定属性（如代码生成中的测试黑客行为）。然而，当前缺乏对思维链可监控性的理论理解，以及如何系统改进监控性能的方法。

Method: 1. 信息论分析：证明思维链与输出之间的非零互信息是可监控的必要但不充分条件；2. 识别两种近似误差来源：信息差距和激发误差；3. 提出两种训练方法：基于oracle的方法直接奖励模型产生最大化监控准确性的思维链，以及更实用的无标签方法最大化输出与思维链之间的条件互信息。

Result: 在多个不同环境中，两种方法都显著提高了监控准确性，同时防止了思维链退化，即使在与监控器对抗训练时也能缓解奖励黑客问题。

Conclusion: 思维链可监控性可以通过有针对性的训练目标系统性地改进，提出的两种方法有效提升了监控性能并防止了思维链退化，为实际应用中的可靠监控提供了理论和方法基础。
...
### [87] [JPmHC Dynamical Isometry via Orthogonal Hyper-Connections](https://arxiv.org/abs/2602.18308)
...
<details>
  <summary>Details</summary>
Motivation: 超连接等深度学习方法虽然提升了性能，但破坏了残差连接的恒等映射特性，导致训练不稳定、可扩展性受限和内存开销增加。需要解决这些挑战以保持梯度条件良好。

Method: 提出JPmHC框架：1) 用可训练线性混合器替代恒等跳跃连接，在n个并行流上操作；2) 将混合器约束在算子范数有界流形上（如双随机、Stiefel、Grassmann流形）；3) 使用自由概率分析预测雅可比谱；4) 采用内存高效的隐式微分进行固定点投影；5) 通过Cayley变换实现Stiefel约束混合器。

Result: 在ARC-AGI上的实验表明，JPmHC相比双随机基线实现了更快的收敛速度、更高的准确率和更低的计算成本。该框架提供了谱感知、稳定且高效的深度学习解决方案。

Conclusion: JPmHC作为HC的灵活可扩展扩展，通过流形约束混合器保持雅可比谱特性，解决了训练不稳定和内存开销问题，为拓扑架构设计和基础模型演进提供了新见解。
...
### [88] [Tighter Regret Lower Bound for Gaussian Process Bandits with Squared Exponential Kernel in Hypersphere](https://arxiv.org/abs/2602.17940)
*Shogo Iwazaki*

Main category: cs.LG

TL;DR: 该论文研究了高斯过程（GP）赌博机问题在频率主义设置下的算法无关最坏情况下界，重点关注平方指数（SE）核函数。论文部分解决了维度依赖对数因子在上下界之间的差距问题，在超球面输入域下给出了累积遗憾和简单遗憾的下界，并改进了SE核的最大信息增益上界。


<details>
  <summary>Details</summary>
Motivation: 高斯过程赌博机问题中，对于平方指数核函数，维度依赖对数因子在上下界之间仍存在差距，这是一个未解决的开放性问题。论文旨在部分解决这个问题，特别是在超球面输入域下，为算法设计提供理论保证。

Method: 采用算法无关的最坏情况分析方法，在频率主义设置下研究高斯过程赌博机问题。假设奖励函数固定且在已知的再生核希尔伯特空间（RKHS）中有界范数。主要关注平方指数核函数，在超球面输入域下进行分析。

Result: 1. 累积遗憾下界：任何算法都遭受Ω(√[T(ln T)^d(ln ln T)^{-d}])的累积遗憾，其中T是总步数，d是超球面域的维度。
2. 简单遗憾下界：任何算法需要Ω(ε^{-2}(ln 1/ε)^d(ln ln 1/ε)^{-d})时间步才能找到ε-最优点。
3. 改进了SE核的最大信息增益上界：O((ln T)^{d+1}(ln ln T)^{-d})。

Conclusion: 在超球面输入域下，论文结果保证了现有最佳算法在维度无关对数因子范围内的最优性。这些结果部分解决了GP赌博机问题中维度依赖对数因子的上下界差距问题，为平方指数核函数提供了更精确的理论分析框架。
...
TL;DR: FedZMG是一种无需额外参数、客户端侧的优化算法，通过将本地梯度投影到零均值超平面来解决联邦学习中非IID数据导致的客户端漂移问题，无需额外通信或超参数调优。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端数据通常是非独立同分布的，这导致客户端漂移问题，降低收敛速度和模型性能。现有自适应优化器虽然能缓解这一问题，但往往引入计算复杂度或通信开销，不适合资源受限的物联网环境。

Method: 提出FedZMG算法，基于梯度中心化的思想，将本地梯度投影到零均值超平面上，有效中和异构数据分布中的"强度"或"偏置"偏移，无需额外通信或超参数调优。

Result: 理论分析证明FedZMG能降低有效梯度方差并保证比标准FedAvg更紧的收敛界。在EMNIST、CIFAR100和Shakespeare数据集上的实验表明，FedZMG在高度非IID设置下比FedAvg和FedAdam具有更好的收敛速度和最终验证准确率。
...
TL;DR: 该论文针对基于梯度的超参数优化中存在的超梯度估计误差问题，提出了偏差-方差分解分析框架，并设计了集成超梯度策略来有效降低方差，从而提升超参数优化性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于梯度的超参数优化方法主要关注超梯度估计的偏差（与真实值的差距），而忽略了由数据分布引起的方差误差，这限制了实际性能。需要同时考虑偏差和方差来改进超参数优化。

Method: 1. 对超梯度估计误差进行偏差-方差分解，提供被先前工作忽略的方差项的详细分析；2. 提出超梯度估计的误差界综合分析；3. 基于理论分析设计集成超梯度策略来降低方差。

Result: 实验在正则化超参数学习、数据超清洗和少样本学习等任务上验证了所提出的方差降低策略能有效改进超梯度估计。建立了超额误差与超梯度估计之间的联系，解释了性能改进的原因。

Conclusion: 通过偏差-方差分解框架分析超梯度估计误差，提出的集成超梯度策略能有效降低方差，提升超参数优化性能，并为实践中观察到的现象（如验证集过拟合）提供了理论解释。
...
TL;DR: 本文提出模板GNN（T-GNNs）框架，通过聚合指定图模板的嵌入来更新节点特征，并引入相应的分级模板模态逻辑（GML(T)），建立了T-GNNs与GML(T)表达能力的等价性，为分析GNN表达能力提供了统一框架。


<details>
  <summary>Details</summary>
Motivation: 标准GNN仅限于聚合直接邻居或全局信息，表达能力有限。为了增强表达能力，最近的研究尝试纳入子结构信息（如环计数和子图属性）。本文旨在形式化这一架构趋势，提供统一的分析框架。

Method: 提出模板GNN（T-GNNs）框架，其中节点特征通过聚合来自指定图模板集合的有效模板嵌入来更新。引入相应的分级模板模态逻辑（GML(T)），以及基于模板的互模拟和WL算法的广义概念。

Result: 建立了T-GNNs与GML(T)表达能力的等价性，证明了标准AC-GNNs及其变体可以作为T-GNNs的特例来解释，为分析GNN表达能力提供了统一方法。

Conclusion: T-GNNs框架形式化了GNN架构中纳入子结构信息的趋势，通过建立与逻辑系统的等价关系，为理解和分析GNN表达能力提供了统一的理论基础。
...
### [92] [A Geometric Probe of the Accuracy-Robustness Trade-off: Sharp Boundaries in Symmetry-Breaking Dimensional Expansion](https://arxiv.org/abs/2602.17948)
*Yu Bai,Zhe Wang,Jiarui Zhang,Dong-Xiao Zhang,Yinjun Gao,Jun-Jie Zhang*

Main category: cs.LG

TL;DR: 通过对称破缺维度扩展(SBDE)研究深度学习中的准确率-鲁棒性权衡，发现插入辅助维度能提高准确率但会沿这些维度产生陡峭边界，导致对抗脆弱性。


<details>
  <summary>Details</summary>
Motivation: 深度学习中的准确率与对抗鲁棒性之间存在普遍权衡现象，但其几何起源尚不明确。本文旨在通过受控实验探究这一权衡的内在机制。

Method: 采用对称破缺维度扩展(SBDE)方法，通过在输入图像中插入恒定值像素来打破平移对称性。使用测试时的掩码投影技术，将插入的辅助像素重置为训练值，以分析脆弱性的来源。

Result: SBDE显著提高了干净准确率（如CIFAR-10上ResNet-18从90.47%提升到95.63%），但降低了对抗迭代白盒攻击的鲁棒性。掩码投影能有效中和攻击并恢复鲁棒性，表明脆弱性主要来自插入的维度。

Conclusion: 准确率-鲁棒性权衡的几何解释是：优化过程通过加深吸引盆来提高准确率，但不可避免地沿辅助自由度建立陡峭边界，导致对离流形扰动的脆弱敏感性。
...
### [93] [Bayesian Online Model Selection](https://arxiv.org/abs/2602.17958)
*Aida Afshar,Yuke Zhang,Aldo Pacchiano*

Main category: cs.LG

TL;DR: 提出一种贝叶斯在线模型选择算法，用于随机多臂赌博机问题，能够在多个基础学习器中选择最优者，并获得接近最优学习器的性能保证。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯赌博机中的在线模型选择面临探索挑战：当环境实例从先验分布中采样时，如何设计自适应策略来探索多个赌博机学习器，并与事后最优学习器竞争？

Method: 引入新的贝叶斯算法用于随机赌博机的在线模型选择，通过设计自适应策略同时探索多个基础学习器，并证明其贝叶斯遗憾的理论保证。

Result: 证明了贝叶斯遗憾的oracle-style保证为$O\left( d^* M \sqrt{T} + \sqrt{(MT)} \right)$，其中$M$是基础学习器数量，$d^*$是最优基础学习器的遗憾系数，$T$是时间范围。实验验证了该方法在各种随机赌博机设置中的竞争力。

Conclusion: 提出的贝叶斯在线模型选择算法能够有效探索多个基础学习器，获得接近最优学习器的性能，同时研究了数据共享对缓解先验错误设定的作用。
...
### [94] [Asynchronous Heavy-Tailed Optimization](https://arxiv.org/abs/2602.18002)
*Junfei Sun,Dixi Yao,Xuchen Gong,Tahseen Rabbani,Manzil Zaheer,Tian Li*

Main category: cs.LG

TL;DR: 该研究针对异步优化中重尾梯度噪声问题，提出了两种通信方案，通过延迟感知学习率调度和延迟补偿改进异步算法性能，在收敛速度上达到同步方法水平并提升延迟容忍度。


<details>
  <summary>Details</summary>
Motivation: Transformer模型中常见的重尾随机梯度噪声会破坏优化过程稳定性。现有研究主要关注集中式或分布式同步设置下的重尾噪声处理，而重尾噪声与异步优化之间的相互作用尚未得到充分探索。

Method: 研究两种处理异步更新中掉队者的通信方案，提出基于延迟感知学习率调度和延迟补偿的算法改进，并进行理论分析。

Result: 在重尾噪声下的收敛保证达到了同步对应方法的收敛速度，相比现有异步方法提升了延迟容忍度。在图像和语言任务中，该方法在准确率/运行时间权衡方面优于先前的同步和异步方法，且对超参数更鲁棒。

Conclusion: 提出的延迟感知学习率调度和延迟补偿方法有效解决了异步优化中的重尾梯度噪声问题，在理论和实验上都表现出优越性能。
...
### [95] [Continual-NExT: A Unified Comprehension And Generation Continual Learning Framework](https://arxiv.org/abs/2602.18055)
*Jingyang Qiao,Zhizhong Zhang,Xin Tan,Jingyu Gong,Yanyun Qu,Yuan Xie*

Main category: cs.LG

TL;DR: 本文提出了Continual-NExT框架和MAGE方法，用于解决Dual-to-Dual MLLMs在持续学习中的遗忘、幻觉和跨模态知识迁移问题。


<details>
  <summary>Details</summary>
Motivation: Dual-to-Dual MLLMs虽然具有强大的即时学习和泛化能力，但在持续学习方面存在缺陷，难以适应动态现实场景。主要挑战包括：学习新任务会破坏已学知识、幻觉问题、指令不遵循以及跨模态知识迁移失败。目前缺乏标准化的持续学习框架来解决这些问题。

Method: 提出了Continual-NExT框架，这是一个专门为Dual-to-Dual MLLMs设计的持续学习框架，包含精心设计的评估指标。同时提出了MAGE（Mixture and Aggregation of General LoRA and Expert LoRA）方法，通过混合和聚合通用LoRA和专家LoRA来促进跨模态知识迁移并减轻遗忘。

Result: 大量实验表明，MAGE方法优于其他持续学习方法，并实现了最先进的性能表现。

Conclusion: 本文建立的Continual-NExT框架和提出的MAGE方法有效解决了Dual-to-Dual MLLMs在持续学习中的关键挑战，为多模态大语言模型的持续学习提供了标准化解决方案。
...
### [96] [Balancing Symmetry and Efficiency in Graph Flow Matching](https://arxiv.org/abs/2602.18084)
*Benjamin Honoré,Alba Carballo-Castro,Yiming Qin,Pascal Frossard*

Main category: cs.LG

TL;DR: 研究图生成模型中严格等变性与性能之间的权衡，通过可控对称性调制方案放松训练期间的等变性约束，发现适度打破对称性可以加速收敛并提升性能。


<details>
  <summary>Details</summary>
Motivation: 严格的等变性虽然能确保图生成模型尊重图的置换对称性，但会增加计算成本并减慢收敛速度，因为模型必须在大量可能的节点置换中保持一致。研究这种权衡对于优化图生成模型性能很重要。

Method: 从等变离散流匹配模型出发，通过基于正弦位置编码和节点置换的可控对称性调制方案，在训练期间放松模型的等变性约束。

Result: 实验表明：1）打破对称性可以通过提供更简单的学习信号加速早期训练；2）但过度打破会导致捷径解和过拟合，模型会重复生成训练集中的图；3）适当调制对称性信号可以延迟过拟合同时加速收敛，仅用基线训练轮数的19%就能达到更强的性能。

Conclusion: 在图生成模型中，严格等变性并非总是最优选择。通过可控的对称性调制方案放松等变性约束，可以在保持模型性能的同时显著加速训练收敛，实现更好的计算效率与生成质量的平衡。
...
### [97] [TempoNet: Slack-Quantized Transformer-Guided Reinforcement Scheduler for Adaptive Deadline-Centric Real-Time Dispatchs](https://arxiv.org/abs/2602.18109)
*Rong Fu,Yibo Meng,Guangzhen Yao,Jiaxuan Lu,Zeyu Zhang,Zhaolu Kang,Ziming Guo,Jia Yee Tan,Xiaojing Du,Simon James Fong*

Main category: cs.LG

TL;DR: TempoNet是一个基于强化学习的实时调度器，使用Transformer架构和深度Q近似，通过紧急令牌化器处理时间松弛，稀疏注意力机制实现高效推理，在多核环境下优于传统调度器和神经基线。
...
Method: 结合置换不变Transformer和深度Q近似，使用紧急令牌化器将时间松弛离散化为可学习嵌入，采用延迟感知稀疏注意力栈（块状top-k选择和局部敏感分块）实现全局推理，多核映射层通过掩码贪婪选择或可微分匹配将Q分数转换为处理器分配。

Result: 在工业混合关键性追踪和大规模多处理器设置中，相比分析调度器和神经基线，在截止期限满足率上取得一致提升，同时改善了优化稳定性，推理时间亚毫秒级。

Conclusion: TempoNet为基于Transformer的高吞吐量实时调度决策建立了实用框架，展示了在复杂调度场景中的有效性和效率。
...
### [98] [Non-Stationary Online Resource Allocation: Learning from a Single Sample](https://arxiv.org/abs/2602.18114)
*Yiding Feng,Jiashuo Jiang,Yige Wang*

Main category: cs.LG

TL;DR: 本文研究非平稳需求下的在线资源分配问题，提出了一种基于分位数的元策略，在仅需每期一个历史样本的情况下，针对不同类型样本（奖励观测型和仅类型型）分别实现了次线性遗憾保证。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的资源分配问题常常面临非平稳需求环境，传统方法通常需要大量历史数据或假设平稳性。本文旨在解决在最小化离线数据需求（每期仅需一个样本）的情况下，处理任意非平稳性并支持多种资源约束的在线资源分配问题。

Method: 提出了一种类型依赖的分位数元策略，将问题解耦为三个模块化组件：1）奖励分布估计；2）通过流体松弛优化目标服务概率；3）通过动态接受阈值进行实时决策。针对奖励观测样本设计了静态阈值策略，针对仅类型样本设计了部分自适应策略和完全自适应解析策略。

Result: 对于奖励观测样本，静态阈值策略实现了$\tilde{O}(\sqrt{T})$遗憾；对于仅类型样本，在最小到达概率假设下，部分自适应策略达到$\tilde{O}(\sqrt{T})$遗憾，完全自适应解析策略实现了$O((\log T)^3)$的多对数遗憾保证，这是非平稳多资源分配领域的首个此类结果。

Conclusion: 该框架在仅需每期一个历史样本的情况下，能够处理任意非平稳性而无需变化预算假设，支持多种资源约束，显著推进了先前工作，为非平稳环境下的在线资源分配提供了有效的解决方案。
...
TL;DR: RAT+是一种通过密集预训练结合全序列递归和主动递归学习的注意力架构，可在推理时灵活切换到扩张注意力模式，显著减少计算量和KV缓存大小，同时保持接近密集注意力的性能。


<details>
  <summary>Details</summary>
Motivation: 结构化扩张注意力在推理时具有效率优势（减少FLOPs和KV缓存），但直接将预训练注意力模型稀疏化为扩张模式会导致严重的准确性下降。需要一种既能保持密集预训练优势，又能在推理时灵活切换到高效稀疏模式的解决方案。

Method: 提出RAT+架构，在密集预训练时通过全序列递归增强注意力，并采用主动递归学习。单个RAT+模型只需密集预训练一次，推理时可灵活切换到扩张注意力（可选带局部窗口）或混合层/头组合，仅需少量token的适应调整而非重新训练稀疏模型。

Result: 在1.5B参数、100B token训练下，RAT+在扩张因子为16时接近密集注意力精度，在64时在常识推理和LongBench任务上仅下降2-3个百分点。此外，RAT+在稀疏化为top-k块注意力时优于普通注意力。在2.6B参数、200B token规模下观察到相同趋势。

Conclusion: RAT+提供了一种高效的解决方案，通过单一密集预训练模型支持推理时灵活切换到多种稀疏注意力模式，在保持性能的同时显著提升推理效率，解决了传统稀疏化方法导致的准确性下降问题。
...
### [100] [Neural-HSS: Hierarchical Semi-Separable Neural PDE Solver](https://arxiv.org/abs/2602.18248)
*Pietro Sittoni,Emanuele Zangrando,Angelo A. Casulli,Nicola Guglielmi,Francesco Tudisco*

Main category: cs.LG

TL;DR: 提出Neural-HSS架构，基于层次半可分离矩阵结构，用于高效学习偏微分方程解，在低数据量下仍保持精确性


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在求解PDE方面表现出色，但大规模高质量数据集生成和模型训练的计算成本仍然很高，限制了关键应用的发展

Method: 受椭圆PDE格林函数结构启发，提出基于层次半可分离矩阵结构的参数高效架构Neural-HSS，理论上分析其精确性，并探索与傅里叶神经算子层和卷积层的联系

Result: 在200万网格点的三维泊松方程上验证了Neural-HSS的数据效率，在低数据量下优于基线方法，并能学习电磁学、流体动力学和生物学等多个领域的PDE数据

Conclusion: Neural-HSS架构为广泛PDE类提供了一种数据高效的解决方案，在低数据量下仍能保持精确性，具有实际应用价值
...
### [101] [Variational Distributional Neuron](https://arxiv.org/abs/2602.18250)
*Yves Ruffenach*
...
<details>
  <summary>Details</summary>
Motivation: 解决结构上的张力：在序列生成中，因果性主要在符号空间组织，潜在变量常保持辅助性；而概率潜在模型捕获变化因素和不确定性，但不确定性通常由全局或参数机制承载，基本计算单元仍传播标量。核心问题：如果不确定性是计算的本质，为何计算单元不显式承载它？

Method: 提出变分分布神经元概念，每个神经元参数化后验分布，传播重参数化样本，并通过局部ELBO的KL项进行正则化。分析"崩溃"模式和"活神经元"条件，并通过自回归先验在时间上扩展贡献。

Result: 提出了一种新的计算单元范式，使激活变为分布性，可通过局部约束测试"收缩"过程，并通过内部度量监控。单元承载的上下文信息量及其时间持久性可通过不同约束局部调节。

Conclusion: 探索了两个轴线：(i)概率约束的组合必须稳定、可解释和可控；(ii)粒度问题：如果推断是在约束下分布协商，原始单元应保持确定性还是变为分布性？为分布性计算单元提供了概念框架。
...
### [102] [MEG-to-MEG Transfer Learning and Cross-Task Speech/Silence Detection with Limited Data](https://arxiv.org/abs/2602.18253)
...
<details>
  <summary>Details</summary>
Motivation: 解决语音脑机接口中的数据效率问题，探索感知与产生任务间的神经表征共享性，验证预训练模型能否在有限数据下实现跨任务解码。

Method: 使用Conformer架构模型，在单个被试50小时听音数据上进行预训练，然后在18名被试每人仅5分钟数据上进行微调，评估感知（听音）与产生（说话）任务间的迁移学习效果。

Result: 迁移学习带来一致改进：任务内准确率提升1-4%，跨任务准确率提升达5-6%。预训练不仅提升各任务性能，还实现了感知与产生任务间的可靠跨任务解码。关键发现：在语音产生任务上训练的模型能解码被动听音任务，表明学习到的表征反映了共享神经过程而非任务特异性运动活动。

Conclusion: 该研究首次证明了MEG语音模型中感知与产生任务间的迁移学习和跨任务解码可行性，表明预训练能有效提升数据效率，且学习到的神经表征反映了语音处理的共享机制，为脑机接口的实用化提供了重要方法。
...
### [103] [On the Semantic and Syntactic Information Encoded in Proto-Tokens for One-Step Text Reconstruction](https://arxiv.org/abs/2602.18301)
*Ivan Bondarenko,Egor Palkin,Fedor Tikunov*

Main category: cs.LG

TL;DR: 研究发现冻结大语言模型可以通过两个学习到的原型令牌在单次前向传播中重建数百个令牌，揭示了超越自回归范式的可能性。论文分析了原型令牌编码的信息及其在重建和约束下的行为，探索了语义和句法内容的分离，并测试了两种正则化方案。


<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型需要n次前向传播来生成长度为n的序列，效率较低。最近研究表明冻结LLMs可以通过两个学习到的原型令牌在单次前向传播中重建数百个令牌，这为超越自回归范式提供了可能。本研究旨在深入理解这些原型令牌编码的信息及其行为特性。

Method: 进行了一系列实验：1）分离两个原型令牌中的语义和句法内容；2）分析e-token的稳定性特性；3）可视化重建过程中对e-token的注意力模式；4）测试两种正则化方案：基于锚点的损失和关系蒸馏目标，使用教师嵌入在e-token上"强加"语义结构。

Result: 1）在标准优化下，m-token比e-token更强烈地捕获语义信息；2）基于锚点的约束与重建准确性之间存在明显权衡；3）关系蒸馏可以在不牺牲重建质量的情况下将批次级语义关系转移到原型令牌空间。

Conclusion: 研究支持了未来非自回归序列到序列系统的可行性，这些系统可以预测原型令牌作为中间表示。关系蒸馏方法特别有前景，因为它能有效转移语义关系而不损害重建质量。
...
### [104] [Explaining AutoClustering: Uncovering Meta-Feature Contribution in AutoML for Clustering](https://arxiv.org/abs/2602.18348)
*Matheus Camilo da Silva,Leonardo Arrighi,Ana Carolina Lorena,Sylvio Barbon Junior*

Main category: cs.LG

TL;DR: 该研究分析了AutoClustering（自动聚类）系统中元模型的可解释性，通过系统回顾现有方法、应用全局和局部可解释性技术，揭示了元特征对聚类算法选择的影响模式，并识别了当前元学习策略的结构性弱点。


<details>
  <summary>Details</summary>
Motivation: 当前AutoClustering系统虽然性能良好，但其推荐结果难以解释：数据集元特征对算法和超参数选择的影响通常不透明，这限制了系统的可靠性、偏差诊断和元特征工程效率，需要提高无监督学习自动化的决策透明度。

Method: 1. 回顾22种现有方法并构建元特征的结构化分类体系；2. 应用全局可解释性技术（决策谓词图）评估元模型中特征的重要性；3. 使用局部可解释性工具（如SHAP）分析具体的聚类决策。

Result: 研究发现：1. 揭示了元特征相关性的稳定模式；2. 识别了当前元学习策略中可能扭曲推荐结果的结构性弱点；3. 为设计更可解释的AutoML系统提供了实用指导。

Conclusion: 该研究为提高无监督学习自动化的决策透明度提供了实践基础，通过增强AutoClustering元模型的可解释性，可以改善系统可靠性、偏差诊断和元特征工程效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统聚类方法缺乏对单个分配可靠性的评估，诊断指标仅反映全局质量，无法指示特定实例的置信度，分配不稳定性会影响准确性和鲁棒性

Method: 提出CAKE框架，在聚类集成上计算两个互补统计量：分配稳定性和局部几何拟合一致性，结合为[0,1]区间的可解释置信度评分

Result: 理论分析显示CAKE在噪声下保持有效并能分离稳定与不稳定点；合成和真实数据集实验表明CAKE能有效识别模糊点和稳定核心成员

Conclusion: CAKE提供置信度排名，可指导过滤或优先级排序以改进聚类质量，为聚类分配提供点级置信度评估
