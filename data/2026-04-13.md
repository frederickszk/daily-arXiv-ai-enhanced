...
TL;DR: 提出一种面向数字取证的仇恨与威胁检测多模态框架，根据证据类型（嵌入文本、关联文本、纯图像）选择相应分析方法，提高可追溯性并避免不合理的模态假设


<details>
  <summary>Details</summary>
Motivation: 数字取证调查中常涉及图像、扫描文档等异构证据，这些证据可能包含仇恨、威胁等内容，但现有自动化方法要么假设干净文本输入，要么使用缺乏取证依据的视觉模型

Method: 提出案例驱动的多模态框架，明确识别文本证据的存在和来源（嵌入文本、关联上下文文本、纯图像证据），根据证据配置选择性地应用文本分析、多模态融合或基于ViT的纯图像语义推理

Result: 在取证风格图像证据上的实验评估表明，该方法在异构证据场景中表现出一致且可解释的行为

Conclusion: 通过基于证据可用性的条件推理，该方法模拟了取证决策过程，提高了证据可追溯性，避免了不合理的模态假设，为数字取证中的仇恨和威胁检测提供了更可靠的框架
...
### [2] [Mitigating Hallucination in Large Vision-Language Models via Adaptive Attention Calibration](https://arxiv.org/abs/2505.21472)
*Mehrdad Fazli,Bowen Wei,Ahmet Sari,Ziwei Zhu*

Main category: cs.CV

TL;DR: CAAC框架通过置信度感知的注意力校准，针对空间感知偏差和模态偏差，减少大视觉语言模型的幻觉问题，在长文本生成中表现优异。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在多模态任务中表现出色，但经常出现幻觉问题，自信地描述图像中不存在的对象或属性。现有的免训练干预方法在开放性和长文本生成场景中难以保持准确性。

Method: 提出置信度感知注意力校准（CAAC）框架，针对两种关键偏差：空间感知偏差（注意力在图像token间分布不均）和模态偏差（注意力随时间从视觉转向文本）。采用两步法：视觉token校准（VTC）平衡视觉token间的注意力，自适应注意力重缩放（AAR）根据模型置信度指导强化视觉基础。

Result: 在CHAIR、AMBER和POPE基准测试中，CAAC优于基线方法，特别是在长文本生成中，有效减少了幻觉问题。

Conclusion: CAAC框架通过针对空间感知偏差和模态偏差的置信度感知注意力校准，能够有效减少大视觉语言模型的幻觉问题，在长文本生成场景中表现突出。
...
TL;DR: 该论文提出了一个半自动化框架，将中世纪手稿中的二维微型画转换为三维数字模型，适用于扩展现实、触觉3D打印和Web可视化。


<details>
  <summary>Details</summary>
Motivation: 研究中世纪手稿的二维微型画通常难以进行三维交互和触觉体验，需要一种方法将其转换为适合XR、3D打印和Web可视化的三维模型，以支持文化遗产的可访问性和沉浸式体验。

Method: 评估了7种图像到3D方法（TripoSR、SF3D、SPAR3D、TRELLIS、Wonder3D、SAM 3D、Hi3DGen），使用基于渲染的指标和体积测量。最终采用SAM分割、Hi3DGen网格生成、ZBrush专家细化和AI辅助纹理的流程。

Result: 在69个手稿图像上评估发现，Hi3DGen在拓扑质量和表面细节之间取得平衡。两个案例研究（Decretum Gratiani和Giulio Clovio作品）展示了跨艺术传统的适用性，生成的模型支持WebXR可视化、AR叠加和触觉3D打印。

Conclusion: 提出的半自动化框架成功将中世纪手稿微型画转换为三维数字模型，Hi3DGen是专家细化的良好起点，最终模型支持多种应用场景，包括为视障用户提供触觉体验。
...
### [4] [On Semiotic-Grounded Interpretive Evaluation of Generative Art](https://arxiv.org/abs/2604.08641)
*Ruixiang Jiang,Changwen Chen*

Main category: cs.CV

TL;DR: 提出SemJudge评估器，基于皮尔士符号学理论评估生成艺术中更深层的符号和索引意义，超越现有评估器仅关注图像质量或提示符合度的局限。


<details>
  <summary>Details</summary>
Motivation: 当前生成艺术评估器过于关注表面图像质量或提示符合度，无法评估创作者意图的深层符号或抽象意义，存在对符号和索引意义的"结构性盲视"。

Method: 提出基于皮尔士计算符号学理论的SemJudge评估器，通过分层符号图重建从提示到生成作品的意义构建过程，明确评估符号和索引意义。

Result: 定量实验显示SemJudge在解释密集型艺术基准上比先前评估器更接近人类判断；用户研究表明其能产生更深层、更有洞察力的艺术解释。

Conclusion: SemJudge使生成艺术能够超越生成"漂亮"图像，成为表达复杂人类体验的媒介，为生成艺术评估提供了新的符号学框架。
...
TL;DR: ViSAGE框架在NTIRE 2026视频显著性预测挑战赛中夺冠，通过多专家集成和自适应门控调制来捕捉视频中的时空显著性线索


<details>
  <summary>Details</summary>
Motivation: 为了利用互补的归纳偏置来更好地预测视频显著性，需要设计一个能够捕捉复杂时空线索的框架

Method: 提出ViSAGE多专家集成框架，每个专门的解码器执行自适应门控和调制来精炼时空特征，不同专家的互补预测在推理时融合
...
### [6] [SenBen: Sensitive Scene Graphs for Explainable Content Moderation](https://arxiv.org/abs/2604.08819)
*Fatih Cagatay Akyon,Alptekin Temizel*

Main category: cs.CV

TL;DR: 该研究提出了首个大规模敏感内容场景图基准SenBen，并开发了一个紧凑的学生模型，在敏感内容检测、场景图生成和推理效率方面均表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有内容审核系统只能将图像分类为安全或不安全，但缺乏空间定位和可解释性：无法解释检测到了什么敏感行为、涉及哪些对象、发生在哪里。

Method: 1) 创建SenBen基准：包含13,999帧电影图像，标注了Visual Genome风格的场景图（25个对象类、28个属性、14个谓词）和16个敏感标签；2) 使用多任务蒸馏方法将前沿VLM蒸馏为241M参数的学生模型，采用后缀对象标识、词汇感知召回损失和分离的Query2Label标签头等技术。

Result: 学生模型在SenBen召回率上比标准交叉熵训练提高了6.4个百分点；在场景图指标上，除Gemini模型外优于所有评估的VLM和商业安全API；在所有模型中实现了最高的对象检测和图像描述分数，推理速度快7.6倍，GPU内存使用减少16倍。

Conclusion: 该研究通过SenBen基准和高效的学生模型，为敏感内容检测提供了空间定位和可解释性，在保持高性能的同时大幅提升了推理效率。
...
### [7] [MARINER: A 3E-Driven Benchmark for Fine-Grained Perception and Complex Reasoning in Open-Water Environments](https://arxiv.org/abs/2604.08615)
...
<details>
  <summary>Details</summary>
Motivation: 真实开放水域环境中的细粒度视觉理解和高级推理研究不足，缺乏专门的基准测试。现有模型在复杂海洋场景中的细粒度辨别和因果推理能力有限。

Method: 提出基于实体-环境-事件（3E）范式构建的MARINER基准，包含多源海事图像、细粒度船舶分类、多样化环境和典型海事事件。涵盖细粒度分类、目标检测和视觉问答三大任务。

Result: 对主流多模态大语言模型（MLLMs）进行广泛评估并建立基线，发现即使是先进模型在复杂海洋场景中的细粒度辨别和因果推理方面也存在困难。
...
TL;DR: Tora3是一个轨迹引导的音频-视频生成框架，通过使用物体轨迹作为共享的运动学先验，提高了物理一致性，改善了运动真实性和运动-声音同步。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视频生成方法在运动-声音关系方面存在挑战，经常产生视觉不稳定的物体运动，声音与显著运动或接触事件松散对齐，主要原因是缺乏视频和音频生成共享的显式运动感知结构。

Method: 提出Tora3框架：1) 使用物体轨迹作为共享运动学先验；2) 设计轨迹对齐的运动表示；3) 开发基于轨迹衍生二阶运动状态的运动-音频对齐模块；4) 采用混合流匹配方案；5) 构建PAV大规模音频-视频数据集。

Result: 实验表明Tora3在运动真实性、运动-声音同步和整体音频-视频生成质量方面优于现有开源基线方法。

Conclusion: Tora3通过轨迹引导的生成框架，显著改善了音频-视频生成中的物理一致性和运动-声音对齐，为多模态生成提供了有效的运动感知结构。
...
### [9] [WildDet3D: Scaling Promptable 3D Detection in the Wild](https://arxiv.org/abs/2604.08626)
...
TL;DR: WildDet3D提出了一种统一的几何感知架构，支持文本、点和框提示，并能融入深度信号，同时构建了最大的开放3D检测数据集，在多个基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 单目3D物体检测需要泛化到闭集类别之外，支持多样提示模态，并在可用时利用几何线索。现有方法存在两个瓶颈：仅支持单一提示类型且缺乏融入额外几何线索的机制，以及当前3D数据集覆盖类别有限。

Method: 提出WildDet3D统一几何感知架构，原生支持文本、点和框提示，可在推理时融入辅助深度信号；同时构建WildDet3D-Data数据集，通过从现有2D标注生成候选3D框并保留人工验证结果，包含超过100万图像和13.5K类别。

Result: 在开放世界设置中，WildDet3D在新引入的WildDet3D-Bench上达到22.6/24.8 AP3D（文本/框提示）；在Omni3D上达到34.2/36.4 AP3D；在零样本评估中，在Argoverse 2和ScanNet上达到40.3/48.9 ODS。融入深度线索平均带来+20.7 AP的显著提升。

Conclusion: WildDet3D通过统一的几何感知架构和大型开放数据集，解决了单目3D物体检测在开放世界应用中的关键瓶颈，在多个基准测试中建立了新的最先进水平，并展示了深度线索融入的重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型作为3D环境中具身智能体的推理核心，但容易产生幻觉，导致不安全和不接地的决策。现有推理时幻觉缓解方法主要针对2D视觉语言设置，无法迁移到具身3D推理，因为3D环境中的失败源于物体存在、空间布局和几何接地问题，而非像素级不一致性。

Method: 提出3D-VCD框架，通过对以物体为中心的表示应用语义和几何扰动（如类别替换、坐标或范围破坏）来构建扭曲的3D场景图。通过对比原始和扭曲3D上下文下的预测，抑制对场景证据不敏感、可能由语言先验驱动的标记。

Result: 在3D-POPE和HEAL基准测试中评估，3D-VCD无需重新训练即可持续改进接地推理，证明基于结构化3D表示的推理时对比解码是实现更可靠具身智能的有效实用途径。
...
TL;DR: InsEdit是一个基于指令的视频编辑模型，仅需少量视频编辑数据就能将视频生成模型转化为强大的视频编辑器，同时支持图像编辑
...
Method: 基于HunyuanVideo-1.5构建，结合视觉编辑架构和基于互上下文注意力(MCA)的视频数据管道，创建对齐的视频对，支持从剪辑中间开始编辑

Result: 仅需约10万视频编辑数据，就在视频指令编辑基准测试中达到开源方法的最先进结果，同时支持图像编辑而无需修改

Conclusion: 视频生成主干模型无需大规模视频编辑数据即可成为强大的视频编辑器，InsEdit展示了在有限数据下实现高质量指令视频编辑的可行性
...
<details>
  <summary>Details</summary>
Motivation: 开发能够在手机上运行的手语识别系统，解决传统方法需要大量参数或手动特征工程的问题，为实际部署提供高效解决方案。

Method: 使用EfficientNet-B0作为基础架构，加入两个注意力模块：Squeeze-and-Excitation用于通道注意力，空间注意力层专注于手势特征。在12,637张印度手语字母图像（26个类别）上使用5折交叉验证进行测试。

Result: EfficientSign达到99.94%准确率（±0.05%），与ResNet18的99.97%相当，但参数减少62%（4.2M vs 11.2M）。使用EfficientNet-B0深度特征的传统分类器也表现优异：SVM 99.63%、逻辑回归99.03%、KNN 96.33%，均远超2015年SURF方法的92%。
...
TL;DR: UMUI是一个跨文本、音频、视频的多模态不确定推理任务，要求模型基于任何模态或组合的前提产生校准的概率估计。研究者创建了人工标注评估集，并提出了CLUE方法，其3B参数模型在性能上媲美甚至超越32B参数的基线模型。


<details>
  <summary>Details</summary>
Motivation: 虽然不确定推理在文本领域已有探索，但在其他模态中仅限于单模态二元蕴含判断，缺乏跨模态的细粒度概率推理框架。需要建立一个统一的多模态不确定推理任务来填补这一空白。

Method: 1) 构建人工标注评估集，包含音频、视觉和视听场景的标量概率判断；2) 提出CLUE方法，结合自洽教师校准和基于分布的置信度探测来产生校准预测；3) 在现有文本和音频基准上评估。

Result: 提出的3B参数模型在所有模态上都达到了与32B参数基线模型相当或更强的性能，证明了方法的有效性。

Conclusion: UMUI任务和CLUE方法为跨模态的不确定推理提供了统一框架，小参数模型也能在多模态推理中实现高性能的校准概率估计。
...
TL;DR: 首个面向遥感图像的开源词汇计数模型RS-OVC，能够基于文本和视觉条件准确计数训练中未见的新物体类别


<details>
  <summary>Details</summary>
Motivation: 现有遥感物体计数方法局限于预定义的封闭物体类别，需要昂贵的重新标注和模型重新训练才能适应新物体计数，这限制了其在动态现实监测场景中的应用

Method: 提出RS-OVC模型，这是首个面向遥感图像的开源词汇计数模型，能够基于文本和/或视觉条件进行计数

Result: 模型能够准确计数训练中未见的新物体类别，仅基于文本和视觉条件即可实现

Conclusion: RS-OVC填补了遥感物体计数领域的关键空白，为动态、实时的监测应用提供了更灵活、适应性更强的解决方案
...
TL;DR: 该论文研究了虚拟试穿（VTON）的逆问题——虚拟脱衣（VTOFF），通过系统分析扩散模型架构的三个设计维度，在VITON-HD和DressCode数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 虚拟试穿（VTON）领域已有快速发展，但其逆问题——从穿着图像重建标准服装的虚拟脱衣（VTOFF）——仍是一个研究不足的领域，需要建立稳健的架构基础。

Method: 采用双UNet扩散模型架构，从三个设计维度进行研究：(1) 生成主干：比较不同的Stable Diffusion变体；(2) 条件化：分析不同掩码设计、掩码/非掩码输入条件化、高层语义特征的效用；(3) 损失函数和训练策略：评估辅助注意力损失、感知目标函数和多阶段课程学习计划的影响。

Result: 在VITON-HD和DressCode数据集上实现了最先进的性能，主要指标DISTS下降了9.5%，在LPIPS、FID、KID和SSIM等指标上也表现出竞争力，为未来VTOFF研究提供了更强的基准和指导。

Conclusion: 该研究为虚拟脱衣（VTOFF）建立了稳健的架构基础，通过系统分析扩散模型设计选择揭示了各种配置选项之间的权衡，为未来研究提供了有价值的见解和基准。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶在长尾和开放世界场景的泛化能力仍是主要瓶颈。人类智能统一了理解和想象能力，但现有方法要么使用LLM/VLM进行视觉语言理解和推理，要么研究生成世界模型捕捉驾驶场景的时空演化。本文探索统一模型，结合两者的优势。
...
Conclusion: 统一多模态理解和生成是构建更通用、更鲁棒的具身决策系统的有前景方向。LMGenDrive展示了这种统一方法的有效性，为自动驾驶系统提供了互补优势。
...
TL;DR: 该研究提出一个结合目标检测、分割跟踪和关键点检测的计算机视觉系统，用于从足球比赛视频中提取球员真实世界坐标，计算战术指标为教练提供数据支持。


<details>
  <summary>Details</summary>
Motivation: 体育分析对团队表现至关重要，但传统视频分析难以提取复杂特征。需要计算机视觉模型从比赛录像中识别和跟踪关键实体，为教练决策、球员表现和团队策略提供可操作数据。

Method: 1) 使用YOLO和Faster R-CNN等目标检测模型识别球员，结合SAM2进行分割跟踪；2) 使用CNN模型检测球场关键点；3) 通过单应性变换将相机视角下的位置转换为真实世界坐标；4) 基于转换后的坐标计算速度、距离、热图等战术指标。

Result: 评估了不同目标检测模型在自定义视频素材上的准确性，旨在找到与SAM2结合时能提供最准确结果的最佳模型。通过单应性变换，无论相机角度或运动如何，都能将分割的球员掩码转换为真实世界坐标。

Conclusion: 该系统能够从标准视频分析中提取以前无法获得的可操作性能数据，包括球员速度、覆盖距离、定位热图和更复杂的团队统计数据，为教练和球员提供有价值的战术洞察。
...
TL;DR: 该研究扩展了车牌识别基准数据集，增加了数据规模、修正了标注错误，并提出了新的训练方法，显著提升了车牌识别的性能。
...
Method: 1. 将原始基准数据集扩展到三倍规模，增加两个额外的采集日；2. 修订标注并引入新标签（车牌级：边界框、文本、可读性级别；车辆级：品牌、型号、类型、颜色；图像级：摄像头ID、采集条件、时间、日ID）；3. 提出基于指数移动平均的损失函数和改进的学习率调度器的新训练流程；4. 引入新的协议处理训练和评估分割中的摄像头污染问题。

Result: 基线模型在测试集上达到了89.5%的F1分数，显著超越了之前的最先进水平。针对摄像头污染问题的结果显示影响较小。

Conclusion: 通过扩展数据集、改进标注和训练方法，显著提升了车牌识别系统在真实世界场景中的性能，为车牌可读性评估提供了更可靠的基准。
...
TL;DR: SIC3D是一个可控的图像条件文本到3D生成系统，使用3D高斯泼溅技术，通过两阶段流程实现文本生成3D内容并参考图像进行风格化


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D生成方法存在可控性有限和纹理模糊的问题，主要受限于文本模态的表达能力，需要更精确的纹理控制和风格迁移能力

Method: 采用两阶段流程：第一阶段使用文本到3DGS生成模型从文本生成3D对象内容；第二阶段通过变分风格化分数蒸馏损失将参考图像风格迁移到3DGS，并应用缩放正则化防止伪影

Result: 实验表明SIC3D在几何保真度和风格一致性方面优于现有方法，在定性和定量评估中都表现出色

Conclusion: SIC3D通过结合文本到3D生成和图像条件风格迁移，解决了现有方法的可控性和纹理模糊问题，实现了高质量的3D对象生成
...
TL;DR: R2G是一个多视图电路图基准套件，标准化了5个阶段感知视图，提供从DEF到图的端到端流程，用于评估GNN在物理设计任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 当前GNN在物理设计任务中的应用受到电路表示不一致和缺乏受控评估协议的阻碍，需要标准化基准来解耦表示选择与模型选择。

Method: 提出R2G基准套件，包含30个开源IP核的5个阶段感知视图，提供从DEF到图的端到端管道、加载器、统一分割、领域指标和可复现基线。

Result: 研究发现：(1)视图选择比模型选择更重要，固定GNN下测试R²变化超过0.3；(2)节点中心视图在布局和布线阶段泛化最好；(3)解码器头深度(3-4层)是主要精度驱动因素。

Conclusion: R2G通过标准化电路表示和评估协议，解耦了表示选择与模型选择的混淆因素，为EDA和图形ML研究提供了受控基准，发现视图选择对GNN性能有决定性影响。
...
TL;DR: 提出了一种上下文对齐推理框架，通过强制异质临床证据间的一致性来改善医学视觉语言模型的诊断结论可靠性，减少幻觉并保持校准的不确定性。
...
Method: 引入上下文对齐推理框架，在冻结的视觉语言模型基础上，整合放射组学统计、可解释性激活和词汇基础语义线索等结构化上下文信号，通过上下文验证生成包含支持证据、不确定性估计、局限性和安全说明的结构化输出。

Result: 在胸部X光数据集上，上下文对齐将AUC从0.918提升至0.925，显著减少幻觉关键词（1.14到0.25），生成更简洁的推理解释（19.4到15.3词）而不增加模型置信度（0.70到0.68）。跨数据集评估显示模态信息性显著影响推理行为。

Conclusion: 强制多证据一致性可同时提高医学多模态推理的可靠性和可信度，同时保持底层模型架构不变。辅助信号单独作用有限，只有通过上下文验证整合时才能产生性能提升。
...
### [22] [CatalogStitch: Dimension-Aware and Occlusion-Preserving Object Compositing for Catalog Image Generation](https://arxiv.org/abs/2604.08836)
*Sanyam Jain,Pragya Kandari,Manit Singhal,He Zhang,Soo Ye Kim*

Main category: cs.CV

TL;DR: CatalogStitch是一套模型无关的技术，通过自动维度感知掩码计算和遮挡感知混合修复，简化了产品目录图像生成中的对象合成流程，减少了人工干预。


<details>
  <summary>Details</summary>
Motivation: 现有生成式对象合成方法在应用到真实世界产品目录图像生成时，需要大量人工干预：用户必须根据产品尺寸差异手动调整掩码，并在生成后费力修复被遮挡的元素。
...
Result: 在三种最先进的合成模型（ObjectStitch、OmniPaint和InsertAnything）上评估，CatalogStitch技术在多样化的目录场景中均表现出一致的改进效果。

Conclusion: 通过减少人工干预和自动化繁琐的修正，CatalogStitch将生成式合成转变为适用于生产目录工作流程的实用、人性化工具。
...
TL;DR: DefakeQ是首个专门为深度伪造检测器设计的量化框架，通过自适应双向压缩策略在边缘设备上实现实时部署，在保持检测性能的同时显著减少计算负担。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测方法计算密集且参数量大，难以在资源受限的边缘设备上实时部署。现有量化技术因会破坏检测所需的细微伪造痕迹而性能下降，需要专门针对深度伪造检测特点的量化策略。
...
Conclusion: DefakeQ是首个专门针对深度伪造检测器的量化框架，成功解决了边缘设备实时部署的挑战，为资源受限环境中的深度伪造检测提供了实用解决方案。
...
TL;DR: BIAS是一个快速、受生物学启发的动态视觉显著性检测模型，基于Itti-Koch框架，结合视网膜运动检测器，在连续视频流中实现毫秒级延迟的显著性检测，在交通事故事故分析中表现出色。


<details>
  <summary>Details</summary>
Motivation: 开发一个既具有生物学合理性又计算高效的动态视觉显著性检测模型，能够实时处理连续视频流，弥补传统启发式方法和深度学习模型在自下而上注意力主导视频中的不足。

Method: 基于Itti-Koch框架，引入视网膜启发的运动检测器提取时间特征，结合静态和运动信息生成显著性图；使用贪婪多高斯峰值拟合算法识别注意力焦点，平衡赢家通吃竞争和信息最大化。

Result: 在DHF1K数据集上超越启发式方法和多个深度学习模型，特别是在自下而上注意力主导的视频中表现优异；在交通事故事故分析应用中，实现最先进的因果识别性能，能够提前0.72秒预测事故，具有可靠的准确性。

Conclusion: BIAS成功地将生物学合理性与计算效率相结合，实现了可解释、高速的动态显著性检测，在现实世界应用中表现出强大的实用价值。
...
TL;DR: 本文提出了一种基于不确定性的文本行人搜索方法，通过显式估计图像-文本对的不确定性并融入优化过程，充分利用弱正样本对提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本行人搜索方法通常关注视觉和文本模态间严格的一对一对应匹配（如对比学习），但无意中忽略了弱正样本对——即同一行人但文本描述来自不同视角（摄像头）的图像-文本对。为了充分利用这些弱正样本，需要一种方法来处理这种不确定性。

Method: 提出了一种不确定性感知方法，包含两个模块：1) 不确定性估计：获取给定正样本对的相对置信度；2) 不确定性正则化：基于预测的不确定性自适应调整损失权重。此外，还引入了分组图像-文本匹配损失来进一步促进弱正样本对间的表示空间。

Result: 在三个广泛使用的数据集（CUHK-PEDES、RSTPReid和ICFG-PEDES）上进行实验，相比现有竞争方法，mAP分别提升了3.06%、3.55%和6.94%。

Conclusion: 该方法通过显式估计图像-文本对的不确定性并融入优化过程，有效利用了弱正样本对，避免了模型将潜在弱正样本推离，显著提升了文本行人搜索的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的视觉-语言大模型面临多语言多模态复合攻击的挑战：有害图像与低资源语言文本的组合容易绕过现有防御机制，暴露了当前跨语言跨模态安全方法的结构性盲点

Method: 提出两阶段框架：1) 通过对比有害和良性输入的激活模式识别安全神经元；2) 通过梯度掩码严格限制参数更新在此子空间内，影响参数少于0.03%

Result: 显著提升安全性同时保持多语言多模态泛化能力；分析显示安全神经元在语言和模态间存在适度重叠，支持零样本跨语言跨模态安全能力迁移

Conclusion: Precise Shield通过神经元级、基于迁移的安全增强，为解决VLLM的多语言多模态复合攻击问题提供了新方向
...
TL;DR: 该论文提出了首个针对高光谱图像理解的多模态大语言模型基准HM-Bench，包含19,337个问答对，涵盖13个任务类别，通过双模态评估框架评估了18个代表性MLLM在高光谱图像理解上的表现。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在自然图像理解方面取得了显著进展，但它们在感知和推理高光谱图像方面的能力尚未得到充分探索，而高光谱图像是遥感领域的重要模态。高光谱图像的高维度和复杂的光谱-空间特性给主要基于RGB数据训练的模型带来了独特挑战。

Method: 提出了HM-Bench基准，包含大规模数据集（19,337个问答对，13个任务类别）。由于现有MLLM无法原生处理原始高光谱立方体，提出了双模态评估框架，将HSI数据转换为两种互补表示：基于PCA的合成图像和结构化文本报告，以系统比较不同表示对模型性能的影响。

Result: 对18个代表性MLLM的广泛评估显示，模型在处理复杂空间-光谱推理任务时存在显著困难。结果还表明，视觉输入通常优于文本输入，强调了基于光谱-空间证据的基础对于有效HSI理解的重要性。

Conclusion: HM-Bench是首个专门评估MLLM在高光谱图像理解能力的基准，揭示了当前模型在该领域的局限性，并强调了视觉表示在光谱-空间推理中的重要性，为未来研究提供了重要参考。
...
Method: 1) 引入GeoMMBench基准，涵盖多样化的遥感学科、传感器和任务；2) 评估36个开源和专有大语言模型；3) 提出GeoMMAgent多智能体框架，通过特定领域的遥感模型和工具集成检索、感知和推理能力。

Result: 评估发现现有模型在领域知识、感知基础和推理能力方面存在系统性缺陷。GeoMMAgent显著优于独立的大语言模型，证明了工具增强智能体在应对复杂地球科学和遥感挑战中的重要性。

Conclusion: 该研究强调了工具增强智能体框架在解决地球科学和遥感领域复杂问题中的关键作用，为未来领域导向AI的发展提供了重要基准和方法论。
...
TL;DR: UniDG是一个通用的缺陷生成基础模型，通过大规模数据集和创新的缺陷上下文编辑方法，解决了现有缺陷生成方法泛化能力差、真实感不足的问题，支持基于参考和文本指令的缺陷编辑。
...
Method: 1) 引入UDG数据集，包含30万个正常-异常-掩码-描述四元组；2) 提出UniDG通用缺陷生成基础模型，支持基于参考和文本指令的缺陷编辑；3) 通过自适应缺陷裁剪和结构化双联输入格式进行缺陷上下文编辑；4) 使用MM-DiT多模态注意力融合参考和目标条件；5) 采用两阶段训练策略：多样性监督微调后接一致性强化微调。

Result: 在MVTec-AD和VisA数据集上的广泛实验表明，UniDG在合成质量和下游单类/多类异常检测/定位任务上优于先前的少样本异常生成和图像插入/编辑基线方法。

Conclusion: UniDG通过大规模数据集和创新的缺陷生成方法，显著提升了缺陷生成的泛化能力、真实感和一致性，为缺陷生成领域提供了有效的通用解决方案。
...
TL;DR: MV3DIS提出了一种从粗到精的零样本3D实例分割框架，通过引入3D先验和3D引导的掩码匹配策略，解决了现有方法因忽略多视角相关性和3D先验导致的2D掩码不一致和3D分割碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 传统3D实例分割方法依赖劳动密集型的3D标注进行监督训练，限制了其可扩展性和对新物体的泛化能力。现有零样本方法虽然利用SAM的2D掩码指导3D几何基元合并，但通常独立处理每帧图像，仅依赖2D指标（如SAM预测分数），忽略了多视角相关性和固有3D先验，导致跨视角2D掩码不一致和最终的3D分割碎片化。

Method: MV3DIS采用从粗到精的框架：1）引入3D引导的掩码匹配策略，使用粗糙3D片段作为公共参考来匹配跨视角的2D掩码，并通过3D覆盖分布整合多视角掩码一致性；2）在视图一致的2D掩码指导下，将粗糙3D片段进一步细化为精确的3D实例；3）提出深度一致性加权方案，量化投影可靠性以抑制物体间遮挡带来的模糊性，提高3D到2D对应的鲁棒性。

Result: 在ScanNetV2、ScanNet200、ScanNet++、Replica和Matterport3D数据集上的广泛实验表明，MV3DIS的有效性，其性能优于先前的方法。

Conclusion: MV3DIS通过显式结合3D先验，解决了零样本3D实例分割中多视角不一致和分割碎片化的问题，实现了更准确和鲁棒的3D实例分割。
...
TL;DR: TAIHRI是一个专为近距离人机交互感知设计的视觉语言模型，能够理解用户运动指令并精确定位任务相关关键点的3D空间坐标。
...
Conclusion: TAIHRI为人机交互领域开辟了新的研究方向，能够实现更自然安全的物理交互。
...
TL;DR: M-IDoL是一种自监督医学基础模型，通过信息分解学习多模态表示，使用MoE子空间实现模态特异性，并在每个子空间内进行细粒度语义区分以增强模态内多样性，在21个下游临床任务上表现优异。
...
Method: 提出M-IDoL自监督医学基础模型，采用信息分解学习框架：1）最大化模态间熵，通过可分离的MoE子空间分散多模态表示以实现模态特异性；2）最小化模态内不确定性，在每个MoE子空间内进行细粒度语义区分以丰富每个模态的表示多样性。

Result: 在115万张医学图像上预训练后，M-IDoL在21个下游临床任务上表现出卓越的泛化能力，在5种成像模态（X光、眼底、OCT、皮肤镜和病理）上优于20个基础模型，学习到的表示具有清晰的模态特征分离和细粒度的模态内特征区分。

Conclusion: M-IDoL通过信息分解学习框架有效解决了医学基础模型中的信息模糊问题，实现了更好的模态特异性和多样性，为多模态医学图像分析提供了更强大的基础表示。
...
TL;DR: MASS方法通过可变形2D高斯Surfel表示，从单目第一人称视频重建高保真3D手部模型，解决了现有方法在分辨率、手物交互和计算成本方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从第一人称单目视频重建3D手部时面临三个主要挑战：1）难以捕捉高分辨率几何细节；2）难以处理手与物体的复杂交互；3）计算成本高，不适合实时应用。

Method: 提出Mesh-inellipse Aligned deformable Surfel Splatting (MASS)方法：1）使用网格对齐的Steiner内切椭圆和分形密集化进行网格到Surfel转换；2）提出高斯Surfel变形，通过预测Surfel属性残差更新和使用不透明度掩码来建模手部变形和个性化特征；3）采用两阶段训练策略和新的绑定损失函数。

Result: 在ARCTIC数据集、Hand Appearance数据集和Interhand2.6M数据集上的实验表明，该方法在重建性能上优于现有最先进方法。

Conclusion: MASS方法通过创新的可变形2D高斯Surfel表示，有效解决了从第一人称单目视频重建高保真3D手部的挑战，在保持高质量重建的同时降低了计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 视觉感知在遮挡或光照条件差时不可靠，而触觉传感通过物理接触提供直接几何信息。但仅从稀疏局部触摸重建全局3D几何本质上是不充分约束的，需要先验知识来指导重建。

Method: 利用预训练的大规模2D视觉扩散模型作为语义和几何先验，将视觉扩散模型中编码的几何知识迁移到触觉领域。给定稀疏接触约束和对象的粗略类别描述，将重建表述为优化问题，在强制触觉一致性的同时，引导解向与扩散先验一致的形状发展。

Result: 该方法仅需少量触摸即可重建准确几何形状，优于现有基线方法，并能够实现先前未见对象实例的开放世界3D重建。

Conclusion: TouchAnything框架成功地将预训练视觉扩散模型的几何知识迁移到触觉领域，解决了从稀疏触觉测量重建3D几何的约束不足问题，实现了高效准确的开放世界物体重建。
...
TL;DR: 在遥感影像云分割任务中，提示工程无法弥补CLIP自然图像预训练与卫星光谱影像之间的分布差异，而少量有标签数据（0.1%）的监督微调即可超越零样本性能，表明有标签数据是适应专业影像的有效途径。


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型适应遥感影像的根本挑战：卫星数据的视觉和语言分布都远离自然图像预训练语料库。尽管存在这种不匹配，提示工程仍是主要部署范式，本研究直接测试这一假设在云分割任务中的有效性。

Method: 在CloudSEN12+云分割基准上使用CLIPSeg模型，评估60种提示变体（包括简单标签、领域术语、外观描述符和上下文线索），并与监督微调（使用0.1%-100%有标签数据）和低秩适应方法进行比较。

Result: 所有提示变体均低于零样本基线（0.255 mIoU），工程化提示最低仅0.07 mIoU。相反，仅使用0.1%有标签数据（约8张图像）的监督微调即超越零样本性能，5-10%数据可达到最大mIoU的85%。全微调始终优于低秩适应0.03-0.09 mIoU，在0.5-1%数据时对光谱模糊类别出现性能下降（监督下降现象）。

Conclusion: 对于将视觉语言模型适应专业影像的实践者，有标签数据不是昂贵的提示工程替代方案，而是值得投入的有效路径。语言精炼无法弥合CLIP自然图像表示与卫星光谱影像之间的差距。
...
TL;DR: 提出了一种用于卫星图像语义分割的动态类感知不确定性主动学习方法（DCAU-AL），通过实时跟踪各类别性能差距并动态调整采样权重，有效解决类别不平衡问题，显著提升标注效率。


<details>
  <summary>Details</summary>
Motivation: 卫星图像语义分割标注成本高昂，传统主动学习方法依赖全局不确定性或多样性度量，缺乏对训练过程中表现不佳或稀有类别的适应性，导致系统偏差和类别不平衡问题。

Method: 提出DCAU-AL方法，通过实时跟踪每个类别的分割性能，动态调整采样权重，优先选择性能差距大的类别样本进行标注，从而针对性地改善表现不佳或代表性不足的类别。

Result: 在OpenEarth土地覆盖数据集上的实验表明，DCAU-AL显著优于现有主动学习方法，特别是在严重类别不平衡情况下，实现了更高的每类IoU和更好的标注效率。

Conclusion: DCAU-AL通过动态类感知的样本选择策略，有效解决了卫星图像语义分割中的类别不平衡问题，在减少标注成本的同时提升了模型性能，为大规模卫星应用提供了高效的主动学习解决方案。
...
TL;DR: ActFER是一个主动的面部表情识别框架，通过动态调用工具进行面部检测和对齐，选择性放大局部区域，并使用视觉思维链进行推理，超越了传统的被动识别方法。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型（MLLM）面部表情识别方法仍采用被动范式，依赖外部准备的面部输入并进行单次推理，缺乏主动的面部感知能力。需要一种能够主动获取视觉证据并进行多模态推理的框架。

Method: 提出ActFER框架，动态调用面部检测和对齐工具，选择性放大信息丰富的局部区域，通过视觉思维链对面部动作单元（AUs）和情绪进行推理。开发UC-GRPO强化学习算法，使用AU基础的多级可验证奖励、查询条件对比效用估计和情绪感知EMA校准。

Result: ActFER在UC-GRPO训练下，持续优于被动的MLLM-based FER基线方法，并显著提高了AU预测准确率。

Conclusion: ActFER通过主动的视觉证据获取和多模态推理，为面部表情识别提供了新的范式，UC-GRPO算法有效解决了主动FER中的强化学习挑战。
...
TL;DR: PinpointQA是首个针对室内视频中小物体空间理解的基准数据集，包含1,024个场景和10,094个QA对，旨在评估MLLMs能否精确定位目标物体并表达其位置。


<details>
  <summary>Details</summary>
Motivation: 现有基准在视频空间智能、具身推理和诊断感知方面已有进展，但缺乏直接评估模型能否在视频中定位目标物体并以足够精度表达位置用于下游任务的基准。

Method: 基于ScanNet++和ScanNet200构建，包含四个渐进式挑战任务：目标存在验证、最近参考识别、细粒度空间描述和结构化空间预测。数据集从中间空间表示构建，QA对自动生成并通过质量控制优化。

Result: 实验显示代表性MLLMs在渐进任务链上存在一致的能力差距，特别是结构化空间预测任务特别困难。在PinpointQA上进行监督微调能带来显著提升，尤其是在更难的任务上。

Conclusion: PinpointQA既可作为诊断基准，也可作为有效的训练数据集，为室内视频中小物体空间理解提供了首个系统评估框架。
...
TL;DR: 提出自适应医疗AI框架，通过多指标特征分析和不确定性门控，在数据漂移时选择性集成新数据，防止性能下降


<details>
  <summary>Details</summary>
Motivation: 医疗AI模型在动态临床环境中常因数据漂移导致性能下降，需要持续监控和自适应更新机制

Method: 三阶段方法：1) 多指标特征分析（欧氏距离、余弦相似度、马氏距离）筛选统计相似数据；2) Monte Carlo dropout不确定性门控选择低预测熵图像；3) 增量重训练，性能保障（指标下降不超过5%）

Result: 在肾小球病理图像分类任务中，ResNet18集成模型在多中心数据集上保持稳定性能：AUC约0.92，准确率约89%，新图像添加未导致显著性能下降

Conclusion: 该框架有效应对数据漂移，避免灾难性遗忘，实现医疗影像AI的持续学习能力
...
<details>
  <summary>Details</summary>
Motivation: 随着扩散模型的快速采用，合成数据生成成为满足大规模图像数据集需求的有前景方法。然而，纯扩散模型生成的图像常存在视觉不一致问题，且在此类数据上训练模型会导致自噬反馈循环，引发模型崩溃（MAD）。

Method: 提出BlendFusion框架，使用路径追踪从3D场景生成合成数据。管道包含：1）以对象为中心的相机放置策略；2）鲁棒过滤机制；3）自动标注。使用该管道构建了FineBLEND数据集。

Result: 构建了FineBLEND图像-文本数据集，并实证分析了其质量，与多个广泛使用的图像-文本数据集进行了比较。证明了以对象为中心的相机放置策略相对于对象无关采样方法的有效性。

Conclusion: BlendFusion是一个开源、高度可配置的框架，使社区能够从3D场景创建自己的数据集，为解决扩散模型生成数据的局限性提供了新途径。
...
TL;DR: 本文提出了CAD数据集，这是首个专门用于汽车相关多任务视觉异常检测的大规模基准数据集，包含7个车辆领域和3个任务的100多张图像，支持多任务学习评估。
...
Result: 多任务学习促进了任务间的交互和知识转移，但也暴露了任务间的冲突挑战，CAD数据集为未来研究提供了标准化平台。

Conclusion: CAD数据集填补了汽车相关多任务视觉异常检测基准的空白，推动了该领域的发展，多任务学习在促进知识转移的同时也面临任务冲突的挑战。
...
TL;DR: ImageProtector是一种用户端图像保护方法，通过在图像中嵌入几乎不可察觉的扰动，使多模态大语言模型在处理受保护图像时生成拒绝响应，从而保护个人隐私信息。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在分析互联网图像数据时具有强大能力，但也带来了严重的安全和社会问题。特别是开源权重的MLLMs可能被滥用来大规模提取个人图像中的敏感信息（如身份、位置等隐私细节），因此需要保护用户隐私。

Method: 提出ImageProtector方法，在用户分享图像前主动保护图像，通过嵌入精心设计的、几乎不可察觉的扰动，这种扰动作为视觉提示注入攻击作用于MLLMs。当攻击者使用MLLM分析受保护图像时，模型会被诱导一致地生成拒绝响应。

Result: 在6个MLLMs和4个数据集上实证证明了ImageProtector的有效性。评估了三种潜在对抗措施（高斯噪声、DiffPure和对抗训练），发现它们虽然能部分减轻ImageProtector的影响，但同时会降低模型准确性和/或效率。

Conclusion: 研究聚焦于开源权重MLLMs和大规模自动化图像分析这一实际重要场景，强调了基于扰动的隐私保护方法的潜力和局限性。ImageProtector为保护个人图像隐私提供了一种有效的用户端解决方案。
...
TL;DR: NTIRE 2026 RAIM挑战赛聚焦动态场景下的多曝光图像融合，旨在解决场景运动、光照变化和手持抖动带来的对齐和重影问题，吸引了114个团队参与。


<details>
  <summary>Details</summary>
Motivation: 解决动态场景中多曝光图像融合的实际难题，包括场景运动、光照变化和相机抖动导致的图像错位和重影伪影，提升HDR成像质量。

Method: 建立包含100个训练序列（7曝光级别）和100个测试序列（5曝光级别）的基准数据集，采用PSNR、SSIM、LPIPS综合评分，并考虑感知质量、效率和可复现性进行最终评审。

Result: 吸引了114个团队参与，收到987份提交，获胜方法显著提升了去除多曝光融合伪影和恢复细节的能力。

Conclusion: NTIRE 2026 RAIM挑战赛成功推动了动态场景多曝光图像融合技术的发展，获胜方法在去除伪影和细节恢复方面取得显著进步，相关数据集和代码已开源。
...
TL;DR: SiMing-Bench是首个评估多模态大语言模型在临床技能视频中程序状态跟踪能力的基准，专注于交互驱动的状态更新如何影响整个工作流程的正确性。


<details>
  <summary>Details</summary>
Motivation: 现有视频基准主要关注事件识别、时序排序和长上下文回忆，但忽略了专家程序判断所需的关键能力：跟踪持续交互如何更新程序状态，从而决定后续动作的正确性。

Method: 引入SiMing-Bench基准，通过SiMing-Score数据集实现，包含医生标注的真实临床技能考试视频（心肺复苏、自动体外除颤器操作、球囊面罩通气），每个视频配有标准化分步评分标准和双专家标签。

Result: 在多种开源和闭源MLLMs上观察到与医生判断的一致性较弱；即使在整体程序级相关性可接受的情况下，评分标准定义的中间步骤表现仍然不佳，表明粗粒度全局评估显著高估了当前模型的程序判断能力。

Conclusion: 瓶颈不在于细粒度评分或时间定位，而在于建模连续交互如何随时间更新程序状态的能力。SiMing-Bench揭示了当前MLLMs在程序状态跟踪这一关键能力上的局限性。
...
TL;DR: 提出了一种数据集级别的物体中心监督方案，通过预训练的槽注意力模块学习场景无关的物体码本，为3D高斯泼溅提供一致的物体表示，无需额外的掩码处理或多视角对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用视觉基础模型的2D掩码监督辐射场进行3D分割，但这些监督信号不是根本上的物体中心，需要额外的掩码预处理/后处理或专门的训练设计来解决跨视角的掩码身份冲突，且学习到的3D场景身份是场景依赖的，限制了跨场景的泛化能力。

Method: 基于预训练的槽注意力全局物体中心学习模块，学习场景无关的物体码本，提供跨视角和跨场景的一致、身份锚定的表示。通过将码本与模块的无监督物体掩码结合，可以直接监督3D高斯的身份特征，无需额外的掩码处理或显式的多视角对齐。

Result: 学习到的场景无关码本能够在无需逐场景微调或重新训练的情况下实现物体监督和识别。该方法将无监督物体中心学习引入3D高斯泼溅，产生了更结构化的表示，并在机器人交互、场景理解和跨场景泛化等下游任务中表现出更好的泛化能力。

Conclusion: 提出了一种新颖的物体中心监督方案，通过场景无关的物体码本解决了现有方法中的身份冲突和场景依赖问题，实现了更结构化的3D表示和更好的跨场景泛化能力。
...
TL;DR: 该研究提出了SIA-RAPN基准，用于评估机器人辅助肾部分切除术中肾缝合阶段细粒度动作分割的性能，比较了四种时序模型在临床视频上的表现。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助肾部分切除术中肾缝合阶段的细粒度动作分割面临视觉相似缝合手势、可变持续时间和大类不平衡的挑战，需要专门的评估基准和方法。

Method: 研究定义了SIA-RAPN基准，包含50个临床视频和12个帧级标签，比较了基于I3D特征的四种时序模型：MS-TCN++、AsFormer、TUT和DiffAct，使用多种评估指标进行性能评估。

Result: 在五个分割配置的评估中，DiffAct在F1分数、帧级准确率、编辑分数和帧级mAP上表现最佳，而MS-TCN++在平衡准确率上表现最好。研究还报告了在单端口RAPN数据集上的跨域结果。

Conclusion: SIA-RAPN基准为机器人辅助肾部分切除术中的细粒度动作分割提供了标准化评估框架，DiffAct在多个关键指标上表现出色，为临床手术分析提供了有价值的工具。
...
TL;DR: VLA-World是一个将预测想象与反思推理相结合的视觉-语言-动作世界模型，用于提升自动驾驶的预见性和安全性。该模型通过动作引导生成未来帧图像，然后对生成图像进行推理来优化轨迹预测。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型缺乏对时间动态和全局世界一致性的显式建模，限制了其预见性和安全性。而世界模型虽然能模拟未来场景，但难以对生成的想象未来进行推理或评估。因此需要结合两者的优势。

Method: 1. 使用动作引导的可行轨迹生成下一帧图像，捕捉环境演化的时空线索；2. 对自生成的未来想象帧进行推理，优化预测轨迹；3. 构建nuScenes-GR-20K生成推理数据集；4. 采用三阶段训练策略：预训练、监督微调和强化学习。

Result: 在规划和未来生成基准测试中，VLA-World持续超越了最先进的视觉-语言-动作模型和世界模型基线方法。

Conclusion: VLA-World通过统一预测想象和反思推理，显著提升了自动驾驶的预见性和安全性，同时具有更好的可解释性。该方法为端到端自动驾驶提供了一个有效的世界模型框架。
...
<details>
  <summary>Details</summary>
Motivation: 空间转录组学能提供丰富的组织微环境信息但成本高昂且稀缺，而H&E染色组织学图像虽然信号粒度较粗但数量丰富。需要利用配对数据将转录组学发现的微环境结构迁移到仅需组织学图像的模型

Method: 提出跨模态蒸馏方法，利用配对的空间转录组学和H&E数据，在训练期间将转录组学推导的微环境结构知识迁移到仅基于组织学图像的模型

Result: 蒸馏模型在多种组织类型和疾病背景下，与转录组学推导的微环境结构一致性显著高于基于相同图像特征的形态学无监督基线，并通过细胞类型分析确认恢复了有生物学意义的邻域组成

Conclusion: 该框架在训练时利用配对的空间转录组学和H&E数据，在推理时仅需组织学图像而无需转录组学输入，实现了从丰富但粒度较粗的组织学数据中推断转录组学定义的微环境结构
...
TL;DR: 提出一种多模态物理基础方法，用于在严重手部遮挡下进行度量尺度无模态物体重建和姿态估计，结合视觉、本体感知和触觉信号来减少遮挡区域的模糊性。


<details>
  <summary>Details</summary>
Motivation: 现有遮挡感知的3D生成方法主要依赖视觉信息，在严重手部遮挡情况下效果有限。需要利用物理交互信号（本体感知和触觉接触）来约束被遮挡区域的物体表面位置，提高重建的准确性和物理一致性。

Method: 使用姿态感知、相机对齐的有符号距离场(SDF)表示物体结构，通过Structure-VAE学习紧凑的潜在空间。在潜在空间中训练条件流匹配扩散模型，先在仅视觉图像上预训练，然后在遮挡操作场景上微调，条件包括可见RGB证据、遮挡/可见性掩码、手部潜在表示和触觉信息。在微调和推理过程中加入基于物理的目标和可微分解码器引导，减少手-物体穿透并使重建表面与接触观测对齐。

Result: 仿真实验表明，添加本体感知和触觉信息显著提高了遮挡下的完成度，相比仅视觉基线产生了物理上合理、正确真实世界尺度的重建结果。进一步验证了模型在真实人形机器人上的迁移能力，即使末端执行器与训练时不同。

Conclusion: 多模态物理基础方法通过整合视觉、本体感知和触觉信号，能够在严重手部遮挡下实现度量尺度、物理一致的无模态物体重建和姿态估计，为机器人操作提供了更可靠的环境感知能力。
...
TL;DR: FIRE-CIR模型通过视觉问答推理提升时尚图像检索性能，自动生成属性问题验证图像修改，超越现有方法并提供可解释性


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在组合图像检索中缺乏推理能力，无法明确判断应该保留和修改什么，导致在时尚等细粒度领域表现不佳且缺乏可解释性

Method: 提出FIRE-CIR模型，基于问题驱动的视觉推理：从修改文本自动生成属性聚焦的视觉问题，在参考图像和候选图像中验证对应视觉证据；构建大规模时尚特定视觉问答数据集，包含单图和双图分析问题；在检索时利用显式推理重新排序候选结果

Result: 在Fashion IQ基准测试中，FIRE-CIR在检索准确率上超越了最先进的方法，同时提供属性级别的可解释性洞察

Conclusion: FIRE-CIR通过组合推理和可解释性提升了时尚组合图像检索性能，证明了问题驱动视觉推理方法的有效性
...
TL;DR: OpenPAE是首个开源的N-shot个性化年龄估计基准，通过利用个人历史照片提升年龄估计准确性，相比传统独立样本方法有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有年龄估计方法将每个面部视为独立样本，忽略了个人因遗传、生活方式和健康等因素导致的差异化衰老速率。当有个人历史参考图像时，可以利用这些上下文信息实现个性化估计，但现有基准（NIST FRVT）是闭源且仅限于单参考图像。

Method: 提出了OpenPAE开源基准，建立了一系列复杂度递增的基线方法：从算术偏移法，到闭式贝叶斯线性回归，再到条件注意力神经过程。通过严格的评估协议进行系统比较。

Result: 实验表明个性化方法能持续提升性能，这种改进不仅仅是领域适应，非线性方法显著优于简单替代方案。所有模型、代码、协议和评估分割都已开源。

Conclusion: 个性化年龄估计是有效的，OpenPAE为这一任务提供了首个开源基准，展示了利用个人历史信息可以显著提高年龄估计的准确性，为未来研究奠定了基础。
...
Method: 提出了FaceLiVTv2架构，核心是Lite MHLA（轻量级全局令牌交互模块），用多头线性令牌投影和仿射重缩放变换替代原始的多层注意力设计，减少冗余同时保持头间的表示多样性。将Lite MHLA集成到统一的RepMix块中，协调局部和全局特征交互，并在嵌入阶段采用全局深度卷积进行自适应空间聚合。

Result: 在LFW、CA-LFW、CP-LFW、CFP-FP、AgeDB-30和IJB等数据集上，FaceLiVTv2相比现有轻量级方法持续改善了准确率-效率权衡。相比FaceLiVTv1减少了22%的移动推理延迟，相比GhostFaceNets在移动设备上加速达30.8%，相比EdgeFace和KANFace在不同平台上实现20-41%的延迟改进，同时保持更高的识别准确率。
...
TL;DR: SATO提出了一种基于三角形条带的新型token排序策略，解决了现有方法在生成艺术家质量网格时token排序效率低、破坏边缘流和结构规律性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有自回归变换器在生成艺术家质量网格时，token排序策略存在不足：基于坐标的排序产生过长序列，基于补丁的启发式方法破坏了高质量建模所需的连续边缘流和结构规律性。

Method: 提出SATO框架，采用受三角形条带启发的token排序策略，将序列构建为明确编码UV边界的面连接链，保持艺术家创建网格的组织边缘流和语义布局。统一表示允许同一token序列解码为三角形或四边形网格。

Result: SATO在几何质量、结构连贯性和UV分割方面持续优于先前方法。通过联合训练三角形和四边形数据，既获得结构先验又增强几何规律性。

Conclusion: SATO通过创新的三角形条带token排序策略，有效解决了艺术家质量网格生成中的token排序问题，提供了灵活的统一表示，显著提升了网格生成质量。
...
TL;DR: GREATEN是一个通过融合表面法线作为几何线索来提升合成到真实零样本泛化能力的立体匹配框架，在多个数据集上显著降低误差并提高运行效率。


<details>
  <summary>Details</summary>
Motivation: 合成到真实零样本泛化面临跨域偏移和图像纹理固有歧义的挑战，特别是在遮挡、无纹理、重复和非朗伯（镜面/透明）区域。需要引入域不变、物体固有且具有区分性的几何线索来补偿图像纹理的局限性。

Method: 提出GREATEN框架，包含三个关键组件：1) 门控上下文-几何融合模块，自适应抑制图像特征中不可靠的上下文线索，并与法线驱动的几何特征融合；2) 镜面-透明增强策略，提升对非朗伯区域误导性视觉线索的鲁棒性；3) 稀疏注意力设计（稀疏空间、稀疏双匹配和简单体积注意力），在保持细粒度全局特征提取能力的同时大幅降低计算开销。

Result: 在仅使用合成数据（如SceneFlow）训练的情况下，GREATEN-IGEV在多个真实数据集上取得优异性能：在ETH3D上误差降低30%，在非朗伯Booster数据集上降低8.5%，在KITTI-2015上降低14.1%。运行速度比GREAT-IGEV快19.2%，支持Middlebury的高分辨率（3K）推理，视差范围达768。

Conclusion: 通过引入表面法线作为域不变的几何线索，结合门控融合机制和稀疏注意力设计，GREATEN有效解决了合成到真实零样本泛化中的跨域偏移和纹理歧义问题，在保持计算效率的同时显著提升了立体匹配的泛化性能。
...
TL;DR: 该论文提出了一种基于物理的夜间光污染去除方法，通过改进退化模型和训练策略，有效消除城市夜间摄影中的人工光源污染，恢复自然的夜间景象。


<details>
  <summary>Details</summary>
Motivation: 城市环境中普遍存在的人工照明导致严重的光污染，包括天空辉光、光晕和辉光伪影，这些污染掩盖了自然夜空景象，而现有的夜间去雾方法主要关注细节清晰度，无法有效去除光污染。

Method: 提出了基于物理的退化模型，在先前夜间去雾模型基础上增加了两个关键方面：(1) 定向光源的各向异性扩散；(2) 天际线后不可见表面光源引起的天空辉光。同时构建了利用大型生成模型和合成-真实耦合的训练策略，以弥补配对真实数据的稀缺并增强泛化能力。

Result: 大量实验表明，所提出的公式和学习框架显著减少了光污染伪影，比先前的夜间恢复方法更好地恢复了真实的夜间图像。

Conclusion: 该研究提出的物理退化模型和训练策略有效解决了夜间光污染去除问题，能够更好地恢复原始夜间景象，为夜间摄影恢复提供了新的解决方案。
...
TL;DR: 该研究在SAR-RARP50数据集上对5种深度学习架构进行基准测试，比较它们在机器人辅助前列腺切除术视频中手术器械多类别语义分割的性能，发现DeepLabV3与SegFormer表现相当，并分析了卷积与Transformer方法的优缺点。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助手术中手术器械的精确分割对于实现上下文感知的计算机辅助干预至关重要，如器械跟踪、工作流程分析和自主决策。然而，真实手术场景中器械外观变化和复杂环境对分割模型提出了挑战。

Method: 在SAR-RARP50数据集上对UNet、UNet++、DeepLabV3、Attention UNet和SegFormer五种深度学习架构进行基准测试，使用结合交叉熵和Dice损失的复合损失函数来处理类别不平衡和捕捉精细边界。

Result: 卷积模型如UNet和Attention UNet提供了强大的基线性能，DeepLabV3与SegFormer表现相当，展示了空洞卷积和多尺度上下文聚合在捕捉复杂手术场景中的有效性。基于Transformer的SegFormer进一步增强了全局上下文理解，提高了对不同器械外观和手术条件的泛化能力。

Conclusion: 该研究为手术AI应用中的分割模型选择提供了全面的比较和实践见解，突出了卷积和基于Transformer方法之间的权衡，为实际应用中的模型选择提供了指导。
...
TL;DR: MAG-3D是一个无需训练的多智能体框架，利用现成的视觉语言模型进行3D场景的接地推理，通过规划、接地和编码三个智能体的动态协作，在零样本情况下实现灵活的3D推理。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在3D场景推理方面存在不足，现有方法依赖领域内调优或手工设计的推理流程，限制了灵活性和零样本泛化能力。需要一种无需训练、能够灵活处理复杂3D场景推理的方法。

Method: 提出MAG-3D多智能体框架：1) 规划智能体分解任务并协调整体推理过程；2) 接地智能体执行自由形式的3D接地和从大量3D场景观察中检索相关帧；3) 编码智能体通过可执行程序进行灵活的几何推理和显式验证。

Result: 在具有挑战性的基准测试中实现了最先进的性能，能够在无需训练的情况下，跨不同场景进行灵活的3D接地推理。

Conclusion: MAG-3D通过多智能体协作设计，实现了无需训练的3D接地推理，克服了现有方法依赖特定任务训练或固定推理流程的限制，为3D场景理解提供了灵活且强大的解决方案。
...
TL;DR: ELT是一种基于循环Transformer架构的高效视觉生成模型，通过权重共享的循环块大幅减少参数，同时保持高质量生成能力，支持动态计算成本与生成质量的权衡。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型依赖深层的独特Transformer层堆叠，参数量大且计算成本高。作者希望开发一种参数效率高、能动态调整计算成本的视觉生成模型。

Method: 提出弹性循环Transformer（ELT），采用权重共享的循环Transformer块减少参数；提出Intra-Loop Self Distillation（ILSD）训练方法，通过最大训练循环的教师配置蒸馏中间循环的学生配置，确保深度一致性。

Result: 在相同推理计算设置下，ELT将参数数量减少4倍，在ImageNet 256×256上获得2.0的FID，在UCF-101上获得72.8的FVD，显著提升了视觉合成的效率边界。

Conclusion: ELT通过循环架构和ILSD训练方法，实现了参数高效且支持动态计算-质量权衡的视觉生成模型，为视觉合成领域提供了新的效率解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 计算病理学中的半监督语义分割面临像素级标注稀缺和伪标签监督不可靠的挑战。现有方法缺乏明确的类别级结构指导，导致类别模糊和伪标签质量不稳定。

Method: 基于病理学预训练的Transformer编码器，引入互补的原型级和文本级对齐分支，在共享嵌入空间中将视觉特征与类别原型和文本描述对齐，通过融合对齐表示生成更可靠的伪标签监督。

Result: 在GlaS和CRAG数据集上，仅使用10%标注数据时，Dice系数分别提升2.6%和8.6%；在20%监督下也获得显著改进，优于现有半监督基线方法。

Conclusion: UniSemAlign通过双模态语义对齐将明确的类别级结构注入像素级学习，有效减少类别模糊性，稳定伪标签细化，为计算病理学中的半监督分割提供了有效解决方案。
...
### [61] [MixFlow: Mixed Source Distributions Improve Rectified Flows](https://arxiv.org/abs/2604.09181)
*Nazir Nayal,Christopher Wewer,Jan Eric Lenssen*

Main category: cs.CV

TL;DR: MixFlow通过改进扩散模型的源分布对齐和训练策略，显著降低了生成路径曲率，提高了采样效率和生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型及其变体（如整流流）虽然能生成多样且高质量的图像，但受到缓慢迭代采样的阻碍。主要原因是标准高斯源分布与数据分布之间的独立性导致生成路径曲率过高。

Method: 提出两种互补方法：1) κ-FC框架，允许源分布基于任意信号κ进行条件化，从而更好地与数据分布对齐；2) MixFlow训练策略，在固定无条件分布和基于κ-FC的分布的线性混合上训练流模型，降低生成路径曲率。

Result: 在固定采样预算下，MixFlow相比标准整流流将生成质量（FID）平均提升12%，相比先前基线提升7%，同时显著加速训练收敛。

Conclusion: 通过改进源分布对齐和采用混合训练策略，MixFlow有效解决了扩散模型生成路径曲率高的问题，显著提升了采样效率和生成质量。
...
TL;DR: 使用多模态深度学习框架，结合CT影像和临床数据，术前预测高级别浆液性卵巢癌患者对化疗的病理反应评分


<details>
  <summary>Details</summary>
Motivation: 高级别浆液性卵巢癌具有高度异质性，常晚期诊断。新辅助化疗后延迟手术是常用方案，但化疗反应评分只能在术后获得。研究旨在开发术前预测工具，为多学科团队讨论提供早期治疗反应评估

Method: 提出2.5D多模态深度学习框架：使用预训练的Vision Transformer编码器处理病灶密集的大网膜切片，通过中间融合模块将视觉表征与临床变量整合，预测化疗反应评分

Result: 内部测试集（IEO，41例患者）ROC-AUC达0.95，准确率95%，精确率80%；外部测试集（OV04，70例患者）ROC-AUC为0.68，准确率67%，精确率75%

Conclusion: 初步结果表明基于Transformer的深度学习可用于术前预测化疗反应评分，作为研究性决策支持工具，可协助多学科团队讨论，提供早期无创治疗反应评估
...
TL;DR: 提出了一种从轮廓估计3D形状姿态的全局最优方法，利用轮廓面积在旋转空间中的连续性，通过预计算的轮廓签名响应曲面和椭圆纵横比辅助签名来加速姿态搜索。


<details>
  <summary>Details</summary>
Motivation: 解决从已知3D形状的未遮挡轮廓确定姿态的问题，现有方法通常需要对应点或局部优化，缺乏从轮廓直接获得全局最优姿态的高效方法。

Method: 利用轮廓面积在旋转空间中的连续性，预计算轮廓签名作为响应曲面；使用2D椭圆拟合轮廓的纵横比作为辅助形状签名；通过分辨率引导的候选搜索在旋转空间中进行分支搜索。

Result: 在合成和真实数据上验证，相比可比方法显著提高了精度，成为首个仅从轮廓高效估计全局最优姿态的方法，适用于任意形状（无论凸性和亏格）。

Conclusion: 该方法首次实现了仅从轮廓就能高效估计全局最优姿态，无需对应点引导，适用于任意形状，为轮廓基姿态估计提供了新的解决方案。
...
Method: 提出CT-1模型，基于视觉语言模块和扩散Transformer架构，采用小波基正则化损失在频域学习复杂相机轨迹分布，并将轨迹集成到视频扩散模型中实现空间感知的相机控制。

Result: 实验结果表明，该框架成功弥合了空间推理和视频合成之间的差距，生成了忠实且高质量的相机可控视频，相机控制精度比先前方法提高了25.7%。

Conclusion: CT-1通过将空间推理知识转移到视频生成中，实现了精确的相机轨迹估计和可控的视频生成，为自动化场景提供了有效的解决方案。
...
### [65] [Long-SCOPE: Fully Sparse Long-Range Cooperative 3D Perception](https://arxiv.org/abs/2604.09206)
...
TL;DR: Long-SCOPE：一个完全稀疏的框架，用于解决长距离协同3D感知中的计算瓶颈和特征关联脆弱性问题，在100-150米长距离场景中实现SOTA性能
...
Method: 提出Long-SCOPE完全稀疏框架，包含两个核心组件：1）几何引导查询生成模块，用于准确检测小型远距离物体；2）可学习的上下文感知关联模块，能在严重位置噪声下稳健匹配协同查询

Result: 在V2X-Seq和Griffin数据集上的实验验证表明，Long-SCOPE在100-150米长距离设置中实现了最先进的性能，同时保持了极具竞争力的计算和通信成本

Conclusion: Long-SCOPE通过完全稀疏的设计解决了长距离协同3D感知的关键瓶颈，为实际部署提供了高效且稳健的解决方案
...
### [66] [Adding Another Dimension to Image-based Animal Detection](https://arxiv.org/abs/2604.09210)
*Vandita Shukla,Fabio Remondino,Benjamin Risse*

Main category: cs.CV

TL;DR: 该研究提出了一种利用Skinned Multi Animal Linear模型从RGB图像估计3D边界框的管道，通过相机姿态优化算法将3D框投影到2D图像空间，并计算立方体面可见性指标，为单目3D动物检测算法开发提供标注数据。


<details>
  <summary>Details</summary>
Motivation: 单目动物成像将3D结构降维到2D投影，现有检测算法产生的2D边界框缺乏动物相对于相机的方向信息。开发RGB图像的3D动物检测方法面临标注数据集缺乏的问题，因为标注过程需要3D输入流和RGB数据。

Method: 提出一个管道，利用Skinned Multi Animal Linear模型估计3D边界框，通过专用的相机姿态优化算法将其投影为鲁棒的2D图像空间标签。计算立方体面可见性指标以评估动物哪些侧面被捕捉。

Result: 在Animal3D数据集上评估该方法，展示了跨物种和设置下的准确性能。生成的3D边界框和可见性指标为未来单目3D动物检测算法的开发和基准测试提供了关键步骤。

Conclusion: 该方法为解决单目3D动物检测缺乏标注数据的问题提供了有效解决方案，通过3D边界框估计和可见性分析，为开发更先进的动物检测算法奠定了基础。
...
TL;DR: SHIFT是一个轻量级框架，通过推理时操纵DiT扩散模型的中间激活来移除特定概念，无需重新训练即可实现概念抑制、风格转换和目标对象控制。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为高质量图像生成的主要方法，特别是DiT模型在保持提示遵从性的同时生成高质量样本。然而，现有方法缺乏在不重新训练的情况下灵活控制生成内容的能力，特别是抑制不需要的视觉概念或调整生成风格。

Method: SHIFT框架受到大语言模型中激活引导的启发，通过有针对性操纵推理时的中间激活来实现概念移除。它学习引导向量，动态应用于选定的层和时间步，以抑制不需要的视觉概念，同时保留提示的其余内容和整体图像质量。

Result: SHIFT在DiT生成中提供了有效且灵活的控制，适用于多样化的提示和目标，无需耗时的重新训练。该机制不仅能抑制概念，还能将生成转移到期望的风格域，或偏向添加/更改目标对象。

Conclusion: SHIFT是一个简单但有效的轻量级框架，通过推理时激活引导为DiT扩散模型提供灵活的概念控制能力，扩展了扩散模型的可控性应用范围。
...
TL;DR: 该论文系统研究了用于高效部署的极小NeRV架构，提出了NeRV-T和NeRV-T+两种轻量配置，分析了极端压缩下的重建质量、计算复杂度和解码吞吐量，并探索了知识蒸馏和低精度推理等优化策略。


<details>
  <summary>Details</summary>
Motivation: 现有神经视频表示研究主要关注中等或高容量模型，而在资源受限环境中所需的极端紧凑配置行为尚未得到充分探索。本文旨在研究极小NeRV架构在高效部署中的表现，为资源受限和实时环境提供指导。

Method: 提出了两种轻量级配置NeRV-T和NeRV-T+，通过架构缩放分析容量缩减对性能的影响。探索了基于频率感知焦点监督的知识蒸馏来提升紧凑模型的重建保真度，同时研究了后训练量化和量化感知训练对低精度推理的影响。

Result: 精心设计的极小NeRV变体能够在显著减少参数数量、计算成本和内存需求的同时，实现良好的质量-效率权衡。实验结果表明这些紧凑模型在资源受限环境中具有实用价值。

Conclusion: 该研究为紧凑神经视频表示的实际极限提供了见解，并为在资源受限和实时环境中部署NeRV风格模型提供了指导。官方实现已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D纹理生成方法存在纹理覆盖不完整、跨视角不一致、几何与纹理不对齐等问题，需要开发更高质量的3D纹理生成方案。

Method: 框架包含两个关键组件：1）基于预训练图像编辑骨干的多视角合成框架，通过即插即用模块显式促进几何对齐、跨视角一致性和光照均匀性；2）原生3D纹理生成模型，基于生成的多视角图像和3D几何，将多视角纹理投影到3D表面并在不可见区域合理补全纹理。
...
TL;DR: VGPO通过视觉注意力补偿机制和双重粒度优势重加权策略，增强视觉语言模型在推理过程中的视觉关注度，解决视觉遗忘问题，提升多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于可验证奖励的强化学习虽然提升了视觉语言模型的推理能力，但由于文本主导的特性导致视觉忠实度不足，表现为对视觉token的注意力激活稀疏。更重要的是，经验分析发现时间上的视觉遗忘会加剧这一缺陷。

Method: 提出视觉引导策略优化框架，包含：1）视觉注意力补偿机制，利用视觉相似性定位和放大视觉线索，同时在推理后期逐步提高视觉期望以对抗视觉遗忘；2）双重粒度优势重加权策略：轨迹内层面突出视觉激活相对较高的token，轨迹间层面优先选择视觉累积表现更好的轨迹。

Result: 大量实验表明，VGPO在数学多模态推理和视觉依赖任务中实现了更好的视觉激活和更优的性能表现。

Conclusion: VGPO通过增强视觉语言模型在策略优化过程中的视觉关注，有效解决了视觉遗忘问题，提升了多模态推理任务的性能。
...
### [71] [FashionStylist: An Expert Knowledge-enhanced Multimodal Dataset for Fashion Understanding](https://arxiv.org/abs/2604.09249)
...
<details>
  <summary>Details</summary>
Motivation: 现有时尚数据集存在碎片化和任务特定性问题，主要关注单品属性、搭配共现或弱文本监督，缺乏对整体服装理解的全面支持。需要建立专家级的时尚理解基准来弥补这一空白。

Method: 通过专门的时尚专家标注流程构建FashionStylist数据集，提供专业基础的单品和服装级别标注。数据集支持三个代表性任务：服装到单品定位、服装搭配完成和服装评估。

Result: 实验结果表明，FashionStylist不仅可作为多个时尚任务的统一基准，还能作为有效的训练资源，提升基于MLLM的时尚系统中的定位、搭配完成和服装级别语义评估能力。

Conclusion: FashionStylist填补了现有时尚数据集的空白，提供了专家级的时尚理解基准，支持从单品定位到整体服装评估的全面任务，对推动时尚AI系统发展具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有物理基础训练数据集规模有限（仅数百或数千个示例），无法满足AI系统对物理世界理解的需求，需要大规模、多样化的物理基础数据集来推动物理感知世界模型的发展。

Method: 创建包含2百万视频、153,810个动态3D场景的大规模合成数据集，涵盖力学、光学、流体动力学和磁学等71种基本物理现象。场景具有多物体交互和复杂背景，提供全面的真实标注，包括3D几何、语义、动态运动、物理属性和文本描述。

Result: 实验表明，在PhysInOne上微调基础模型显著提高了物理合理性。数据集在四个新兴应用中表现出色：物理感知视频生成、长/短期未来帧预测、物理属性估计和运动迁移。同时暴露了建模复杂物理动态和估计内在属性的关键差距。
...
### [73] [Rays as Pixels: Learning A Joint Distribution of Videos and Camera Trajectories](https://arxiv.org/abs/2604.09429)
...
TL;DR: Rays as Pixels是一种视频扩散模型，联合学习视频和相机轨迹的分布，通过解耦自交叉注意力机制同时去噪视频帧和相机光线像素，实现相机轨迹预测、视频生成和条件视频生成三种任务。


<details>
  <summary>Details</summary>
Motivation: 传统计算机视觉和图形学中将相机参数恢复和视角渲染视为独立任务，但在图像覆盖稀疏或姿态模糊的情况下，这两个任务相互依赖，需要彼此的输出来完成各自的工作。
...
Result: 模型通过闭环自一致性测试评估，证明其前向和逆向预测一致。轨迹预测所需的去噪步骤远少于视频生成，甚至只需几步去噪即可实现自一致性。在姿态估计和相机控制视频生成方面报告了结果。

Conclusion: Rays as Pixels模型成功地将相机参数恢复和视角渲染任务统一在一个框架中，通过联合学习视频和相机轨迹分布，实现了三个相关任务的统一处理，并通过自一致性验证了模型的有效性。
...
Method: 在YOLOv8训练目标中加入自定义轻量级对齐损失，通过正则化鼓励边界框的网格一致性排列，注入几何先验知识

Result: 在CMP数据集上的实验表明，该方法成功提高了结构规律性，纠正了由透视和遮挡引起的对齐错误，同时保持了与标准检测精度的可控权衡

Conclusion: 通过简单的训练目标增强，可以在不改变标准推理流程的情况下，有效提升立面解析的结构一致性，为程序化重建提供更好的基础
...
<details>
  <summary>Details</summary>
Motivation: 当前渲染技术面临一个关键困境：基于物理的渲染虽然能保证物理真实性，但需要难以获得的真实世界数字模型；而隐式生成模型则牺牲了可控性和几何一致性。因此需要弥合PBR与PRR之间的差距。

Method: 提出GeRM模型，将PBR到PRR的过渡建模为分布转移，学习分布转移向量场（DTV Field）。首先使用多智能体VLM框架构建专家指导的P2P-50K配对数据集，然后提出多条件ControlNet学习DTV Field，通过渐进注入G-buffers、文本提示和增强区域线索来生成可控照片级真实感图像。

Result: 构建了P2P-50K数据集，开发了GeRM模型，能够统一PBR和PRR，允许用户在严格的物理保真度和感知照片真实感之间流畅导航，生成可控的照片级真实感图像。

Conclusion: 该工作首次提出了弥合PBR与PRR差距的问题，并开发了多模态生成渲染模型GeRM，通过分布转移向量场学习实现了可控的照片级真实感渲染，为渲染领域提供了新的解决方案。
...
TL;DR: SFGS是一种从单目视频重建逼真、拓扑感知的3D人体化身的新方法，通过空间三平面和时间六平面捕捉动态特征，结合结构感知高斯模块和手部残差细化模块，实现高质量细节重建。
...
Method: 提出结构感知细粒度高斯溅射（SFGS）方法：1）使用空间三平面和时间六平面捕捉连续帧间的动态特征；2）设计结构感知高斯模块以空间一致方式捕捉姿态依赖细节；3）提出基于细粒度手部重建的残差细化模块；4）仅需单阶段训练。

Result: 在定量和定性评估中均优于现有最先进基线方法，能够生成具有自然运动和精细细节的高保真化身。

Conclusion: SFGS方法能够从单目视频序列中重建具有表现力和一致性的全身3D人体化身，在捕捉精细细节方面表现出色，代码已开源。
...
### [77] [Envisioning the Future, One Step at a Time](https://arxiv.org/abs/2604.09527)
*Stefan Andreas Baumann,Jannik Wiese,Tommaso Martorella,Mahdi M. Kalayeh,Björn Ommer*

Main category: cs.CV

TL;DR: 该论文提出了一种基于稀疏点轨迹的自回归扩散模型，用于开放集未来场景动态预测，相比密集视频预测方法具有更高的采样速度和多样性生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖密集视频或潜在空间预测，将大量计算能力用于处理密集外观信息，而非场景中稀疏的点轨迹。这使得大规模探索未来假设成本高昂，且在需要长时程、多模态运动时性能受限。

Method: 将开放集未来场景动态预测公式化为稀疏点轨迹的逐步推断。使用自回归扩散模型通过短距离、局部可预测的转移来推进这些轨迹，显式建模随时间增长的不确定性。

Result: 该方法在预测准确性上与密集模拟器相当或更优，同时实现了数量级更高的采样速度。能够从单张图像快速生成数千种不同的未来场景，同时保持物理合理性和长程一致性。

Conclusion: 通过基于稀疏点轨迹的动态中心表示，实现了开放集未来预测的可扩展性和实用性，并引入了OWM基准来评估真实世界不确定性下的轨迹分布预测性能。
...
TL;DR: 该论文提出从帧级评估转向事件中心视角的视频异常检测方法，揭示了现有方法在事件级检测上的严重性能差距


<details>
  <summary>Details</summary>
Motivation: 传统基于帧的评估方法将视频视为孤立帧的集合，与真实世界中异常事件表现为连续时间片段的方式不匹配。在实际监控系统中，重要的是可靠地检测、定位和报告连贯的异常事件，而不是标记单个帧。

Method: 1) 审计广泛使用的VAD基准测试的事件结构；2) 提出两种时间事件定位策略：基于分层高斯平滑和自适应二值化的分数细化管道，以及直接生成事件级检测的端到端双分支模型；3) 建立首个基于事件的VAD评估标准，采用时间动作定位指标。

Result: 量化了显著的性能差距：虽然所有最先进模型在NWPUC数据集上的帧级AUC-ROC超过52%，但它们在事件级定位精度在tIoU=0.2时低于10%，所有阈值下的平均事件级F1分数仅为0.11。

Conclusion: 需要从帧级评估转向事件中心视角的视频异常检测，现有方法在实际部署中的性能被严重高估，事件级检测能力亟待提升。
...
### [79] [VL-Calibration: Decoupled Confidence Calibration for Large Vision-Language Models Reasoning](https://arxiv.org/abs/2604.09529)
*Wenyi Xiao,Xinchi Xu,Leilei Gan*

Main category: cs.CV

TL;DR: VL-Calibration：通过强化学习框架将LVLM的置信度分解为视觉置信度和推理置信度，使用视觉不确定性估计和监督，在13个基准测试中有效改善校准并提升视觉推理准确性。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLMs）虽然具备强大的多模态推理能力，但经常出现幻觉和错误回答，且置信度很高，这阻碍了其在高风险领域的应用。现有的基于文本LLM的置信度校准方法通常使用二元答案正确性优化单一整体置信度，这与LVLMs不匹配：错误预测可能源于感知失败或正确感知下的推理错误，单一置信度混淆了这些来源，且视觉不确定性常被语言先验主导。
...
Conclusion: VL-Calibration通过解耦视觉和推理置信度，提供更细粒度的不确定性估计，解决了LVLMs中幻觉和高置信度错误的问题，为高风险应用提供了更可靠的置信度校准方法。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在空间理解和视角识别等视觉感知任务上表现不佳，自然图像数据集对这些低层次视觉技能提供的监督有限。研究针对性的合成监督是否能解决这些弱点

Method: 提出VisionFoundry任务感知合成数据生成流程：仅输入任务名称，使用大语言模型生成问题、答案和文本到图像提示，用T2I模型合成图像，并用专有VLM验证一致性，无需参考图像或人工标注。构建了包含10个任务、1万个图像-问题-答案三元组的VisionFoundry-10K数据集

Result: 在VisionFoundry-10K上训练的模型在视觉感知基准测试中取得显著提升：MMVP上+7%，CV-Bench-3D上+10%，同时保持更广泛的能力，并随着数据量增加显示出良好的扩展性

Conclusion: 有限的任务针对性监督是视觉语言模型瓶颈的重要原因，合成监督是朝着更系统化训练视觉语言模型的有前景路径
...
TL;DR: VisPrompt是一个轻量级的视觉引导提示学习框架，通过跨模态注意力机制将视觉语义反向注入提示表示，提高在标签噪声环境下的鲁棒性。
...
Result: 在合成和真实世界标签噪声下的广泛实验表明，VisPrompt在七个基准数据集上普遍优于现有基线，实现了更强的鲁棒性，同时保持预训练VLM主干冻结，仅引入少量额外可训练参数。

Conclusion: VisPrompt通过视觉引导的提示学习有效抑制噪声引起的干扰，减少提示更新的不稳定性，缓解对错误标记样本的记忆，显著提高了在标签噪声环境下的鲁棒性。
...
TL;DR: VLM视觉语言模型存在编码-接地分离现象：即使模型给出错误答案，其视觉编码能力与正确答案模型同样强。问题在于仲裁机制而非感知能力，通过激活干预可以改善视觉接地效果。


<details>
  <summary>Details</summary>
Motivation: 研究VLM模型在回答问题时出现错误的原因——是感知问题还是仲裁问题。当VLM看到蓝色香蕉却回答"黄色"时，需要探究其根本原因。

Method: 使用多模态仲裁交叉分析（MAC）和逐层Logit Lens探测，追踪视觉信号与先验知识在模型各层的竞争。通过全序列激活修补建立因果关系，并采用训练免费的激活引导（线性和稀疏自编码器引导）进行干预。

Result: 视觉属性在早期层就可线性解码（AUC>0.86），正确与错误样本的编码准确率几乎相同。最终层logit差距（而非编码强度）能更好预测接地结果。全序列激活修补可改变60-84%的输出，图像token承担几乎所有因果影响。早期层激活引导可提升视觉接地达+3.8%。

Conclusion: VLM已经具备良好的视觉感知能力，主要挑战在于如何基于所见内容进行决策。有针对性的干预可以帮助弥合这一差距，改善视觉接地效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D基础模型在静态场景中表现良好，但在动态序列中由于运动导致的几何模糊性而表现不佳，需要解决动态场景重建中的不确定性挑战。

Method: 提出三个协同机制：1) 熵引导子空间投影，利用信息论加权自适应聚合多头注意力分布；2) 局部一致性驱动的几何净化，通过半径邻域约束强制空间连续性；3) 不确定性感知的跨视图一致性，将多视图投影优化建模为异方差最大似然估计问题。

Result: 在动态基准测试中优于当前最先进方法，平均精度误差降低13.43%，分割F-measure提高10.49%，保持前馈推理效率且无需任务特定微调或每场景优化。

Conclusion: 该框架通过建模重建过程中的不确定性，有效解决了动态4D场景重建中的几何模糊问题，在保持效率的同时显著提升了重建质量和分割性能。
...
TL;DR: EpiAgent是一个基于智能体协作的铭文修复系统，采用分层规划方法，通过LLM中央规划器协调多模态分析、历史经验、专业修复工具和迭代自优化，实现超越传统单次方法的灵活自适应修复。


<details>
  <summary>Details</summary>
Motivation: 古代铭文作为文化遗产载体，经历了数百年环境与人为破坏，修复其视觉与文本完整性是数字遗产保护中最具挑战性的任务之一。现有AI方法依赖固定流程，难以应对复杂多变的真实世界退化情况。

Method: 受人类碑铭学家技能协调工作流程启发，提出EpiAgent智能体中心系统，将铭文修复构建为分层规划问题。采用观察-构思-执行-重评估范式，基于LLM的中央规划器协调多模态分析、历史经验、专业修复工具和迭代自优化。

Result: 在真实世界退化铭文上，EpiAgent相比现有方法实现了更优的修复质量和更强的泛化能力。
...
TL;DR: 提出RC-GRPO-Editing框架，通过区域约束的GRPO后训练解决流基图像编辑中噪声信用分配问题，提升编辑区域指令遵循和非目标区域保护


<details>
  <summary>Details</summary>
Motivation: 现有基于流的指令引导图像编辑方法在GRPO奖励驱动后训练中存在噪声信用分配问题：全局探索会扰动非目标区域，导致组内奖励方差增大和GRPO优势噪声，影响编辑效果

Method: 提出区域约束的GRPO后训练框架RC-GRPO-Editing：1) 通过区域解耦的初始噪声扰动实现局部化探索，减少背景诱导的奖励方差；2) 引入注意力集中奖励，使交叉注意力在整个rollout过程中与目标编辑区域对齐

Result: 在CompBench基准测试中，该方法在编辑区域指令遵循和非目标内容保护方面均取得一致改进

Conclusion: RC-GRPO-Editing通过抑制背景诱导的干扰方差，实现了更清晰的局部化信用分配，有效提升了流基图像编辑在确定性ODE采样下的性能
...
TL;DR: EGLOCE是一种无需训练的文本到图像扩散模型概念擦除方法，通过推理时的双重能量引导优化潜在空间，有效移除特定概念（如不当内容、版权角色等）同时保持图像质量和提示对齐。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像扩散模型的普及，移除特定概念（如不当内容、版权角色或风格）对于安全和合规变得至关重要。现有方法需要昂贵的重新训练、以无关概念保真度下降为代价修改参数，或依赖间接推理时调整而影响擦除效果。

Method: EGLOCE是一种无需训练的方法，通过推理时在潜在空间中重新引导噪声潜在变量来移除不需要的概念。采用双目标框架：排斥能量通过潜在空间梯度下降引导生成远离目标概念；保留能量保持与原始提示的语义对齐。与先前需要修改模型权重或提供弱推理时指导的方法不同，EGLOCE完全在推理时操作。

Result: 大量实验表明，EGLOCE在基准测试中改善了概念移除效果，同时保持了图像质量和提示对齐，即使在对抗攻击下也表现良好。该方法实现了即插即用集成，增强了擦除性能。

Conclusion: EGLOCE通过采样时的双重能量引导，为安全和可控的图像生成建立了新范式。这是首个在采样过程中通过基于能量的双重引导实现概念擦除的方法。
...
### [87] [Do Vision Language Models Need to Process Image Tokens?](https://arxiv.org/abs/2604.09425)
*Sambit Ghosh,R. Venkatesh Babu,Chirag Agarwal*

Main category: cs.CV

TL;DR: 研究发现视觉语言模型中的图像表示在早期层就快速收敛到有界复杂度状态，深层处理并非总是必要，这挑战了当前多模态LLM架构的范式


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型通过视觉编码器与大语言模型结合取得了成功，但当前架构中密集图像标记在深层Transformer堆栈中的持续处理带来了巨大计算开销。研究者质疑这种持续处理是否必要，以及视觉表示是否真的在深层中发生有意义的演化

Method: 系统研究视觉语言模型中图像标记的功能角色，分析视觉表示的收敛特性（熵稳定、内在维度压缩、轨迹曲率趋近常数），进行深度方向的视觉截断实验，比较单标记预测与多标记生成任务，分析确定性解码下的推理轨迹变化

Result: 视觉表示快速收敛到有界复杂度状态，稳定后不同层的视觉表示基本可互换；视觉处理的必要性是任务依赖的：单标记预测对视觉深度截断相对鲁棒，但多标记生成需要持续的视觉表示访问；在确定性解码下，减少视觉深度对中间推理轨迹的扰动比对最终输出的影响更大

Conclusion: 研究质疑了"更深层视觉处理对视觉语言模型性能至关重要"的假设，挑战了当前多模态LLM架构的范式，表明视觉表示在早期层就达到稳定状态，深层处理并非总是必要
...
### [88] [SCoRe: Clean Image Generation from Diffusion Models Trained on Noisy Images](https://arxiv.org/abs/2604.09436)
*Yuta Matsuzaki,Seiichi Uchida,Shumpei Takezaki*

Main category: cs.CV

TL;DR: SCoRe是一种无需训练、在生成时使用的频谱再生方法，用于从在噪声图像上训练的扩散模型中生成干净图像。它通过频率截止抑制生成图像中的高频伪影，然后通过SDEdit再生这些频率成分。


<details>
  <summary>Details</summary>
Motivation: 在噪声数据集上训练的扩散模型往往会复制高频训练伪影，显著降低生成质量。现有方法需要重新训练或微调模型，而SCoRe旨在提供无需训练即可从噪声训练模型中生成干净图像的解决方案。

Method: SCoRe利用扩散模型的频谱偏置特性，通过频率截止抑制生成图像中受损的高频成分，然后使用SDEdit再生这些频率成分。关键创新是建立了截止频率与SDEdit初始化时间步之间的理论映射关系，基于径向平均功率谱密度(RAPSD)，防止再生过程中注入过多噪声。

Result: 在合成数据集(CIFAR-10)和真实世界噪声数据集(SIDD)上的实验表明，SCoRe显著优于后处理和噪声鲁棒基线方法，能够恢复更接近干净图像分布的样本，且无需任何重新训练或微调。

Conclusion: SCoRe是一种有效的训练时频谱再生方法，能够从噪声训练的扩散模型中生成干净图像，通过理论推导的频率映射关系优化了再生过程，在多个数据集上表现出优越性能。
...
### [89] [AsymLoc: Towards Asymmetric Feature Matching for Efficient Visual Localization](https://arxiv.org/abs/2604.09445)
*Mohammad Omama,Gabriele Berton,Eric Foxlin,Yelin Kim*

Main category: cs.CV

TL;DR: AsymLoc：一种用于视觉定位的非对称蒸馏框架，通过离线大教师模型处理数据库图像、在线轻量学生模型处理查询图像，结合几何驱动匹配和联合检测器-描述符蒸馏，实现高效准确的视觉定位。


<details>
  <summary>Details</summary>
Motivation: 针对AR/VR和机器人等应用在资源受限边缘设备（如智能眼镜）上对精确实时视觉定位的需求，需要在保持精度的同时进一步降低计算量，以解决电池寿命和散热问题。

Method: 提出非对称视觉定位框架AsymLoc：离线使用大型教师模型处理预建图的数据库图像，在线使用轻量学生模型处理查询图像。通过几何驱动匹配目标和联合检测器-描述符蒸馏目标对齐学生模型与教师模型，实现快速、无参数的最近邻匹配。

Result: 在HPatches、ScanNet、IMC2022和Aachen数据集上的广泛实验表明，AsymLoc使用小一个数量级的模型达到了教师模型95%的定位精度，显著优于现有基线，建立了新的效率-精度权衡最优状态。

Conclusion: AsymLoc通过非对称蒸馏框架成功解决了不同模型特征匹配的挑战，在保持高精度的同时大幅降低了计算需求，为资源受限边缘设备上的视觉定位提供了实用解决方案。
...
### [90] [Realizing Immersive Volumetric Video: A Multimodal Framework for 6-DoF VR Engagement](https://arxiv.org/abs/2604.09473)
...
TL;DR: 提出沉浸式体视频新格式，包含大范围6自由度交互空间、视听反馈和高分辨率动态内容，并开发了基于高斯时空表示的动态光场重建框架和首个多视角视听数据声场重建方法。


<details>
  <summary>Details</summary>
Motivation: 当前虚拟现实和增强现实需要完全沉浸式体验，但直接从真实世界捕获视频构建这种体验仍未被充分探索，需要新的媒体格式和构建方法。

Method: 1) 提出ImViD多视角多模态数据集，采用空间导向捕获理念；2) 开发基于高斯时空表示的动态光场重建框架，包含流引导稀疏初始化、联合相机时间校准和多术语时空监督；3) 提出首个从多视角视听数据重建声场的方法。

Result: 构建了包含5K分辨率60FPS视频的ImViD数据集，开发了统一的沉浸式体视频生产流程，能够生成高质量、时间稳定的视听体内容，支持大范围6自由度交互空间。

Conclusion: 该工作为沉浸式体视频提供了基础定义和实用构建方法，通过统一流程实现了从真实世界捕获视频到沉浸式体验的完整生产链。
...
### [91] [Incremental Semantics-Aided Meshing from LiDAR-Inertial Odometry and RGB Direct Label Transfer](https://arxiv.org/abs/2604.09478)
*Muhammad Affan,Ville Lehtola,George Vosselman*

Main category: cs.CV

TL;DR: 提出了一种模块化、增量式的RGB+LiDAR流水线，通过扫描帧级直接标签转移，从室内扫描生成增量语义辅助的高质量网格，解决大型复杂室内环境中的几何重建问题。


<details>
  <summary>Details</summary>
Motivation: 在大型复杂室内环境（如文化建筑）中，从LiDAR惯性扫描进行几何高保真网格重建仍然具有挑战性。点云稀疏性、几何漂移和固定融合参数会导致孔洞、过度平滑和结构边界处的虚假表面。

Method: 提出模块化增量RGB+LiDAR流水线：1) 视觉基础模型标记每个传入的RGB帧；2) 标签增量投影并融合到LiDAR惯性里程计地图；3) 增量语义感知的截断符号距离函数融合步骤通过行进立方体生成最终网格。这种帧级融合策略保留了LiDAR的几何保真度，同时利用丰富的视觉语义来解决重建边界处的几何模糊性。

Result: 该方法在Oxford Spires数据集上使用几何指标进行定量评估，在NTU VIRAL数据集上进行定性分析。结果表明，语义引导提高了几何重建质量，优于最先进的几何基线ImMesh和Voxblox。

Conclusion: 语义辅助融合对几何网格质量有益，生成的语义标记网格在重建通用场景描述资产方面具有价值，为从室内LiDAR扫描到XR和数字建模提供了路径。
...
### [92] [Online3R: Online Learning for Consistent Sequential Reconstruction Based on Geometry Foundation Model](https://arxiv.org/abs/2604.09480)
*Shunkai Zhou,Zike Yan,Fei Xue,Dong Wu,Yuchen Deng,Hongbin Zha*

Main category: cs.CV

TL;DR: Online3R是一个新的在线学习三维重建框架，通过引入可学习的视觉提示来适应新场景，解决不一致性问题，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 为了解决在线三维重建中的不一致性问题，同时保持预训练几何基础模型的核心能力，需要一种能够适应新场景但又不破坏原有模型性能的方法。

Method: 在冻结的预训练几何基础模型中引入轻量级可学习视觉提示，采用局部-全局自监督学习策略：局部一致性约束基于中间和先前融合结果，全局一致性约束基于稀疏关键帧而非逐帧处理。

Result: 实验表明Online3R在多个基准测试中优于先前的最先进方法，能够有效适应新场景并保持重建一致性。

Conclusion: Online3R通过视觉提示和局部-全局自监督学习策略，成功实现了在线三维重建的适应性和一致性，为在线学习框架提供了有效解决方案。
...
### [93] [RIRF: Reasoning Image Restoration Framework](https://arxiv.org/abs/2604.09511)
*Wending Yan,Rongkai Zhang,Kaihua Tang,Yu Cheng,Qiankun Liu*

Main category: cs.CV

TL;DR: R&R框架通过结构化思维链推理，在图像修复前先诊断退化类型、严重程度和场景语义，为修复器提供可解释的先验知识，并利用退化严重程度作为强化学习信号提升修复质量。
...
Method: 提出R&R框架，包含一个基于Qwen3-VL微调实现的推理器，用于诊断退化类型、量化退化严重程度、推断关键退化相关因素和描述场景语义。将量化退化严重程度作为强化学习信号指导修复器。

Result: 在多个通用图像修复基准测试中，R&R实现了最先进的性能，同时为修复过程提供了独特的可解释性。

Conclusion: R&R框架成功地将语义诊断推理与像素级修复紧密结合，在提升修复性能的同时提供了可解释的诊断过程，为通用图像修复开辟了新方向。
...
### [94] [EgoTL: Egocentric Think-Aloud Chains for Long-Horizon Tasks](https://arxiv.org/abs/2604.09535)
...
TL;DR: EgoTL是一个用于解决VLM在家庭任务中长时程空间指令跟随问题的框架，通过构建"边想边说"的数据采集流程，结合空间估计和记忆库，为100多个日常家庭任务提供基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型在具身智能方面虽有进展，但VLM自动标注存在噪声问题，主要源于缺乏准确的人类动作标签、思维链和空间标注，这些问题在长时程空间指令跟随中被放大，导致模型产生幻觉、跳过步骤或忽视物理属性。

Method: EgoTL构建了"边想边说"的自我中心数据采集流程，采用"先说话后行动"协议记录逐步目标和带时间戳的口头推理，通过度量尺度空间估计器校准物理属性，使用记忆库进行场景上下文理解，并为导航指令和详细操作动作添加片段级标签。

Result: 在100多个日常家庭任务中，对VLMs和世界模型在六个任务维度上进行了基准测试，发现基础模型仍无法胜任自我中心助手或开放世界模拟器的角色。使用EgoTL训练数据微调基础模型后，显著提升了长时程规划推理、逐步推理、指令跟随和空间定位能力。

Conclusion: EgoTL框架通过提供高质量的思维链对齐和度量标注数据，有效解决了VLM在长时程家庭任务中的噪声问题，显著提升了模型的规划和推理能力，为具身智能的发展提供了重要基准和训练数据。
...
### [95] [Tango: Taming Visual Signals for Efficient Video Large Language Models](https://arxiv.org/abs/2604.09547)
*Shukang Yin,Sirui Zhao,Hanchao Wang,Baozhi Jia,Xianquan Wang,Chaoyou Fu,Enhong Chen*

Main category: cs.CV

TL;DR: Tango是一个优化视频大语言模型token剪枝的新框架，通过多样性驱动的注意力选择策略和时空旋转位置编码，在保留10%视频token时能维持98.9%的原始性能，实现1.88倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型的token剪枝方法存在两个关键问题：1) 传统的top-k选择策略未能充分考虑注意力分布的空间多模态和长尾特性；2) 直接的相似性聚类经常产生碎片化簇，导致池化后表示失真。需要优化视觉信号的利用效率。

Method: 提出Tango框架，包含两个核心组件：1) 多样性驱动的注意力token选择策略，更好地处理注意力分布的多模态特性；2) 时空旋转位置编码(ST-RoPE)，通过局部先验保持几何结构，避免相似性聚类中的表示失真问题。

Result: 在多种视频大语言模型和视频理解基准测试中验证了方法的有效性和泛化能力。当仅保留10%的视频token时，Tango在LLaVA-OV上能保持98.9%的原始性能，同时实现1.88倍的推理加速。

Conclusion: Tango框架成功解决了现有token剪枝方法的局限性，通过优化注意力选择和保持几何结构，在显著减少计算量的同时保持了模型性能，为高效视频大语言模型的发展提供了有效解决方案。
...
### [96] [Memory-Guided Trust-Region Bayesian Optimization (MG-TuRBO) for High Dimensions](https://arxiv.org/abs/2604.08569)
*Abhilasha Saroj,Shaked Regev,Guanhao Xu,Jinghui Yuan,Roy Luo,Ross Wang*

Main category: cs.LG

TL;DR: 该研究比较了遗传算法与多种贝叶斯优化方法在交通仿真标定问题上的性能，提出了一种新的Memory-Guided TuRBO方法，在84维高维问题上表现优异。
...
Method: 比较了常用的遗传算法（GA）与多种贝叶斯优化方法：经典贝叶斯优化（BO）、Trust-Region BO（TuRBO）、Multi-TuRBO以及提出的Memory-Guided TuRBO（MG-TuRBO）方法。在2个真实世界交通仿真标定问题（14维和84维）上进行测试，研究了两种采集策略：Thompson采样和新的自适应策略。

Result: 贝叶斯优化方法在低维问题中比遗传算法更快达到良好的标定目标。MG-TuRBO在14维问题中表现相当，但在84维高维问题中表现出明显优势，特别是与自适应策略结合使用时。

Conclusion: MG-TuRBO特别适用于高维交通仿真标定问题，可能对一般高维问题也有用。贝叶斯优化方法在有限仿真预算下比遗传算法更有效，尤其是在高维设置中。
...
### [97] [QuanBench+: A Unified Multi-Framework Benchmark for LLM-Based Quantum Code Generation](https://arxiv.org/abs/2604.08570)
...
<details>
  <summary>Details</summary>
Motivation: 当前量子代码生成评估大多局限于单一框架，难以区分量子推理能力和框架熟悉度，需要跨框架的统一基准来更准确地评估LLM的量子代码生成能力。

Method: 创建QuanBench+基准，包含Qiskit、PennyLane和Cirq三个框架的42个对齐任务；使用可执行功能测试评估模型，报告Pass@1和Pass@5；采用基于KL散度的概率输出接受标准；研究基于反馈的修复机制，允许模型在运行时错误或错误答案后修订代码。
...
Conclusion: 量子代码生成在多框架环境中取得进展，但可靠的多框架量子代码生成问题尚未解决，模型性能仍严重依赖框架特定知识，需要进一步研究来提升跨框架的量子推理能力。
...
### [98] [Robust Reasoning Benchmark](https://arxiv.org/abs/2604.08571)
*Pavel Golikov,Evgenii Opryshko,Gennady Pekhimenko,Mark C. Jeffrey*

Main category: cs.LG

TL;DR: 该论文提出一个包含14种技术的扰动管道来评估LLM的推理鲁棒性，发现前沿模型表现稳定但开源推理模型存在灾难性崩溃，同时揭示了标准注意力机制中推理步骤会永久污染工作记忆的问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在标准数学基准测试中表现优异，但其底层推理过程高度过拟合于标准文本格式。需要评估LLM推理的鲁棒性，并探究其工作记忆机制中的根本问题。

Method: 1) 提出包含14种技术的扰动管道来评估LLM推理鲁棒性；2) 在AIME 2024数据集上应用该管道评估8个最先进模型；3) 通过强制模型在单个上下文窗口中顺序解决多个未扰动数学问题，严格隔离模型的工作记忆容量。

Result: 1) 前沿模型表现出韧性，但开源推理模型遭受灾难性崩溃（平均准确率下降达55%，某些情况下达100%）；2) 从7B到120B参数的开源模型以及Claude Opus 4.6在后续问题上表现出准确率衰减；3) 中间推理步骤会永久污染标准密集注意力机制。

Conclusion: 为实现可靠推理，未来的推理架构必须在模型自身的思维链中集成显式的上下文重置机制，这引出了关于原子推理任务最优粒度的基本开放性问题。
...
### [99] [Ranked Activation Shift for Post-Hoc Out-of-Distribution Detection](https://arxiv.org/abs/2604.08572)
*Gianluca Guglielmo,Marc Masana*

Main category: cs.LG

TL;DR: 本文提出了一种无需超参数调整的后处理OOD检测方法，通过固定参考配置文件替代排序激活幅度，在不同数据集和架构上表现稳定。
...
Method: 提出了一种超参数自由的后处理方法，用固定的分布内参考配置文件替代排序的激活幅度。这是一种即插即用的简单方法，无需对倒数第二层激活函数做假设，也无需超参数调整，同时通过构造保持分布内分类精度。

Result: 该方法在不同数据集和架构上表现出强大且一致的性能，同时保持分布内分类精度。分析表明，抑制和激发激活偏移都独立地有助于更好的OOD判别。

Conclusion: 通过识别现有方法的失效模式并提出基于固定参考配置文件的解决方案，实现了稳定且无需超参数调整的OOD检测，为激活分布差异问题提供了有效解决方法。
...
### [100] [Distilling Genomic Models for Efficient mRNA Representation Learning via Embedding Matching](https://arxiv.org/abs/2604.08574)
*Rasched Haidari,Sam Martin,Maxime Allard*

Main category: cs.LG

TL;DR: 研究人员开发了一种蒸馏框架，将大型基因组基础模型的mRNA表示转移到更小的专门模型中，模型尺寸减小200倍，在mRNA相关任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 大型基因组基础模型虽然性能优异，但参数量巨大（数十亿），在计算资源有限时运行成本高昂，需要开发更高效的模型。

Method: 提出蒸馏框架，将最先进基因组基础模型的mRNA表示转移到专门针对mRNA序列的小型模型中。采用嵌入级蒸馏方法，发现比基于logit的方法更稳定有效。

Result: 在mRNA-bench基准测试中，蒸馏模型在同等规模模型中达到最先进性能，并能与更大架构竞争mRNA相关任务。模型尺寸减小200倍。

Conclusion: 嵌入级蒸馏是生物基础模型的有效训练策略，使基因组学中的序列建模更加高效和可扩展，特别是在大型模型计算困难或不切实际的情况下。
...
### [101] [MolPaQ: Modular Quantum-Classical Patch Learning for Interpretable Molecular Generation](https://arxiv.org/abs/2604.08575)
*Syed Rameez Naqvi,Lu Peng*

Main category: cs.LG

TL;DR: MOLPAQ：一种量子-经典混合分子生成模型，通过量子生成的潜在片段组装分子，实现100%有效性、99.75%新颖性和0.905多样性


<details>
  <summary>Details</summary>
Motivation: 现有分子生成模型通常在有效性、多样性和属性控制之间需要权衡取舍，无法同时满足这些目标

Method: 使用β-VAE在QM9数据集上预训练获得化学对齐的潜在流形；通过降维条件器将分子描述符映射到该空间；参数高效的量子片段生成器产生纠缠节点嵌入；价态感知聚合器重建为有效分子图；通过潜在批评器和化学形状奖励进行对抗性微调

Result: 达到100% RDKit有效性、99.75%新颖性和0.905多样性；相比参数匹配的经典生成器，量子生成器将平均QED提高约2.3%，芳香基团出现率增加约10-12%
...
### [102] [Structured Exploration and Exploitation of Label Functions for Automated Data Annotation](https://arxiv.org/abs/2604.08578)
*Phong Lam,Ha-Linh Nguyen,Thu-Trang Nguyen,Son Nguyen,Hieu Dinh Vo*

Main category: cs.LG

TL;DR: EXPONA是一个自动化程序化标注框架，通过多级标签函数探索和可靠性感知过滤，在保持多样性和可靠性的同时生成高质量弱标签，显著提升标注覆盖率和下游模型性能。
...
Method: EXPONA将标签函数生成构建为一个平衡多样性和可靠性的原则性过程：1）系统探索多级标签函数，涵盖表面、结构和语义三个层面；2）应用可靠性感知机制抑制噪声或冗余启发式规则，同时保留互补信号。

Result: 在11个不同领域的分类数据集上的实验表明：1）标注覆盖率高达98.9%；2）弱标签质量提升达87%；3）下游加权F1性能提升达46%；4）一致优于现有最先进的自动化标签函数生成方法。
...
### [103] [On the Spectral Geometry of Cross-Modal Representations: A Functional Map Diagnostic for Multimodal Alignment](https://arxiv.org/abs/2604.08579)
*Krisanu Sarkar*

Main category: cs.LG

TL;DR: 使用计算几何中的函数映射框架研究独立预训练视觉和语言编码器的跨模态对齐，发现模型在捕获的结构量上收敛但在组织方式上不匹配，提出了"谱复杂度-方向间隙"概念。
...
Method: 使用计算几何中的函数映射框架，将表示流形之间的对应关系表示为图拉普拉斯特征基之间的紧凑线性算子。比较了函数映射、Procrustes对齐和相对表示在跨模态检索中的表现，并分析了拉普拉斯特征谱的相似性和特征向量的对齐情况。

Result: 函数映射在跨模态检索中表现不如Procrustes对齐和相对表示，但揭示了多模态表示的结构特性：两个编码器的拉普拉斯特征谱相似（归一化谱距离0.043），表明独立训练的模型具有可比的内在复杂性；但函数映射显示近零对角优势（均值低于0.05）和大正交误差（70.15），表明特征向量基基本未对齐。

Conclusion: 提出了"谱复杂度-方向间隙"概念：模型在捕获多少结构上收敛，但在如何组织这些结构上不匹配。这为谱对齐方法定义了边界条件，并提出了三个诊断量：对角优势、正交偏差和拉普拉斯可交换性误差，用于表征跨模态表示的兼容性。
...
### [104] [Fully Autonomous Z-Score-Based TinyML Anomaly Detection on Resource-Constrained MCUs Using Power Side-Channel Data](https://arxiv.org/abs/2604.08581)
...
Result: 使用家用迷你冰箱14天数据集评估，在正常操作和受控异常条件下，系统表现出完美的检测性能（精确度和召回率均为1.00），推理延迟在数十微秒级别，总内存占用约3.3KB SRAM和63KB Flash。
...
### [105] [Multivariate Time Series Anomaly Detection via Dual-Branch Reconstruction and Autoregressive Flow-based Residual Density Estimation](https://arxiv.org/abs/2604.08582)
*Jun Liu,Ying Chen,Ziqian Lu,Qinyue Tong,Jun Tang*

Main category: cs.LG

TL;DR: DBR-AF是一个用于多元时间序列异常检测的新框架，通过双分支重建编码器和自回归流模块解决现有方法中的虚假相关性和误导性异常评分问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于重建的异常检测方法存在两个关键限制：1) 过度强调跨变量建模导致虚假相关性过拟合；2) 简单累加多变量重建误差会产生误导性异常评分，难以区分难以重建的样本和真实异常。

Method: 提出DBR-AF框架，包含双分支重建(DBR)编码器和自回归流(AF)模块。DBR编码器解耦跨变量相关性学习和变量内统计特性建模以减少虚假相关性；AF模块使用多层可逆变换建模复杂多元残差分布，并利用密度估计准确识别具有大重建误差的正常样本。

Result: 在七个基准数据集上的广泛实验表明，DBR-AF实现了最先进的性能，消融研究验证了其核心组件的不可或缺性。

Conclusion: DBR-AF通过解耦跨变量相关性和变量内统计特性建模，结合自回归流进行密度估计，有效解决了多元时间序列异常检测中的虚假相关性和异常评分问题，取得了优异性能。
...
### [106] [CSAttention: Centroid-Scoring Attention for Accelerating LLM Inference](https://arxiv.org/abs/2604.08584)
*Chuxu Song,Zhencan Peng,Jiuqi Wei,Chuanhui Yang*

Main category: cs.LG

TL;DR: CSAttention是一种无需训练的稀疏注意力方法，针对可重用上下文的高吞吐量服务优化，通过离线预填充阶段构建查询中心查找表，在解码时实现高效表查找和GPU友好的分数累积，在95%稀疏度和32K-128K长上下文设置下达到接近全注意力的准确度，推理速度比最准确基线提升4.6倍。


<details>
  <summary>Details</summary>
Motivation: 长上下文LLM越来越依赖扩展的可重用预填充提示，使得注意力和KV缓存成为解码时的主要瓶颈。现有稀疏注意力方法在高稀疏度下难以保持准确度，因为查询和键之间存在固有的分布偏移。

Method: CSAttention采用存储换计算的策略，针对离线预填充/在线解码设置进行优化：在离线预填充阶段构建查询中心查找表，这些表的大小在解码期间保持不变，使在线解码能够用高效的表查找和GPU友好的分数累积替代全上下文扫描。

Result: 在95%稀疏度和32K-128K长上下文设置下，CSAttention达到接近全注意力的准确度，在模型准确度和推理速度方面均优于最先进的稀疏注意力方法，在128K上下文长度下比最准确基线的推理速度提升4.6倍。

Conclusion: CSAttention是一种针对可重用上下文高吞吐量服务的有效稀疏注意力方法，通过离线预填充阶段的前期计算和在线解码阶段的高效表查找，在保持高准确度的同时显著提升推理速度。
...
### [107] [Act or Escalate? Evaluating Escalation Behavior in Automation with Language Models](https://arxiv.org/abs/2604.08588)
*Matthew DosSantos DiSorbo,Harang Ju*

Main category: cs.LG

TL;DR: 研究探讨LLM在不确定条件下的决策行为，通过建模分析模型何时应该行动、何时应该升级决策，发现不同模型在隐式阈值上存在显著差异，且这些差异无法通过架构或规模预测。


<details>
  <summary>Details</summary>
Motivation: 有效自动化的关键在于决定何时行动、何时升级决策。当前LLM在不确定性下的决策行为缺乏系统研究，需要理解模型如何权衡行动和升级的成本，以及如何优化这种决策过程。

Method: 建立不确定性下的决策框架：LLM形成预测→估计正确概率→比较行动和升级的预期成本。在五个领域（需求预测、内容推荐、内容审核、贷款审批、自动驾驶）测试多个模型家族，并测试干预措施：改变成本比率、提供准确性信号、训练模型遵循期望的升级规则。

Result: 发现模型在隐式阈值上存在显著差异，这些差异无法通过架构或规模预测；模型的自估计存在模型特定的校准偏差。提示主要对推理模型有帮助；在思维链上进行监督微调能产生最稳健的策略，可泛化到不同数据集、成本比率、提示框架和保留领域。

Conclusion: 升级行为是模型特定的属性，应在部署前进行特征化；通过训练模型明确推理不确定性和决策成本，可以实现更稳健的对齐。
...
### [108] [EngageTriBoost: Predictive Modeling of User Engagement in Digital Mental Health Intervention Using Explainable Machine Learning](https://arxiv.org/abs/2604.08589)
*Ha Na Cho,Daniel Eisenberg,Cheryl King,Kai Zheng*

Main category: cs.LG

TL;DR: 使用机器学习预测数字心理健康干预的用户参与度，通过可解释AI分析影响参与的关键因素


<details>
  <summary>Details</summary>
Motivation: 年轻人心理健康问题日益严重，数字心理健康干预(DMHI)虽有潜力但面临采纳障碍，包括初始使用率低和高退出率，需要理解用户参与模式以改善干预效果

Method: 开发了Ensemble模型EngageTriBoost，分析eBridge平台用户行为数据，使用SHAP可解释性分析识别影响参与的关键因素

Result: EngageTriBoost模型在预测用户参与度方面达到84%准确率，SHAP分析揭示了情绪失调和感知污名化是影响DMHI采纳的关键因素

Conclusion: 可解释机器学习能有效理解DMHI用户参与模式，为改善数字心理健康干预的采纳和效果提供重要见解
...
### [109] [Reservoir observer enhanced with residual calibration and attention mechanism](https://arxiv.org/abs/2604.08592)
*Yichen Liu,Wei Xiao,Tianguang Chu*

Main category: cs.LG

TL;DR: 本文提出了一种改进的储层观测器方法，通过集成残差校准和注意力机制来提升非线性动力系统中未测量变量的推断性能。


<details>
  <summary>Details</summary>
Motivation: 传统储层观测器在推断非线性动力系统的未测量变量时，性能会因输入变量的不同而有显著差异，在最坏情况下可能影响可靠性。因此需要一种方法来提升推断性能，特别是改善最坏情况下的表现。

Method: 将残差校准模块和注意力机制集成到储层观测器设计中。残差校准模块利用估计残差信息来优化观测器输出，注意力机制则利用数据的时间依赖性来丰富储层内部动态的表征。

Result: 在典型混沌系统上的实验表明，该方法显著提高了推断准确性，特别是改善了传统储层观测器在最坏情况下的表现。通过传递熵的概念解释了输入依赖性观测差异的原因以及所提方法的有效性。
...
### [110] [From Selection to Scheduling: Federated Geometry-Aware Correction Makes Exemplar Replay Work Better under Continual Dynamic Heterogeneity](https://arxiv.org/abs/2604.08617)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦持续学习方法主要关注样本重要性估计，但忽略了如何有效利用选定的样本，这限制了在客户端和任务间持续动态异质性下的性能表现

Method: 提出FEAT方法，包含两个关键模块：1) 几何结构对齐模块，通过将特征表示与其对应的等角紧框架原型之间的成对角度相似性对齐，实现结构知识蒸馏；2) 基于能量的几何校正模块，从特征嵌入中移除任务无关的方向分量，减少对多数类的预测偏差

Result: 该方法缓解了不平衡导致的表示崩溃问题，提高了对少数类的敏感性，增强了模型在类别不平衡分布下的鲁棒性

Conclusion: FEAT方法通过几何感知的校正机制，有效解决了联邦持续学习中类别不平衡导致的表示漂移问题，提升了模型在动态异质性环境下的性能
...
### [111] [StructRL: Recovering Dynamic Programming Structure from Learning Dynamics in Distributional Reinforcement Learning](https://arxiv.org/abs/2604.08620)
*Ivo Nowak*

Main category: cs.LG

TL;DR: 通过分析分布强化学习的学习动态，发现可以从学习过程中恢复动态规划式的结构信息，并提出利用这些信号指导采样的StructRL框架


<details>
  <summary>Details</summary>
Motivation: 传统强化学习被视为均匀的数据驱动优化过程，而动态规划方法依赖结构化信息传播实现高效稳定学习。本文旨在探索是否可以从分布强化学习的学习动态中恢复这种结构

Method: 分析回报分布的时间演化，识别状态空间中学习发生的时间和位置信号。引入时间学习指标t*(s)反映状态在训练期间经历最强学习更新的时刻，并基于此提出StructRL框架，利用这些信号指导采样以对齐新兴的传播结构

Result: 经验表明，时间学习指标t*(s)诱导的状态排序与动态规划式的信息传播一致。初步结果显示，分布学习动态提供了一种无需显式模型即可恢复和利用动态规划式结构的机制

Conclusion: 分布强化学习的学习动态可以恢复和利用动态规划式的结构，为强化学习提供了新视角：学习可被解释为结构化传播过程而非纯粹均匀优化过程
...
### [112] [VOLTA: The Surprising Ineffectiveness of Auxiliary Losses for Calibrated Deep Learning](https://arxiv.org/abs/2604.08639)
*Rahul D Ray,Utkarsh Srivastava*

Main category: cs.LG

TL;DR: 本文提出了VOLTA方法，这是一个轻量级、确定性且校准良好的不确定性量化方法，在多个基准测试中优于10种现有UQ方法。


<details>
  <summary>Details</summary>
Motivation: 在安全关键应用中部署深度学习模型时，不确定性量化至关重要，但目前缺乏对不同数据模态和分布偏移下UQ方法性能的共识评估。

Method: 提出了VOLTA方法，这是一个简化的变体，仅包含深度编码器、可学习原型、交叉熵损失和后处理温度缩放。在CIFAR-10、CIFAR-100、SVHN、均匀噪声、CIFAR-10-C和Tiny ImageNet特征等数据集上，与10种UQ基线方法进行了全面比较。

Result: VOLTA在CIFAR-10上达到0.864的竞争性准确率，预期校准误差显著降低（0.010 vs. 基线0.044-0.102），OOD检测AUROC为0.802。统计测试显示VOLTA匹配或优于大多数基线方法。

Conclusion: VOLTA作为一个轻量级、确定性且校准良好的UQ方法，可以作为更复杂UQ方法的有效替代方案，消融研究证实了自适应温度和深度编码器的重要性。
...
### [113] [Creator Incentives in Recommender Systems: A Cooperative Game-Theoretic Approach for Stable and Fair Collaboration in Multi-Agent Bandits](https://arxiv.org/abs/2604.08643)
*Ramakrishnan Krishnamurthy,Arpit Agarwal,Lakshminarayanan Subramanian,Maximilian Nickel*

Main category: cs.LG

TL;DR: 论文研究在线推荐平台中内容创作者间的协作激励问题，通过多智能体随机线性老虎机模型和可转移效用合作博弈分析，证明同质化创作者博弈具有凸性且核心非空，异质化创作者博弈核心非空但凸性不保证，提出基于遗憾的支付规则满足部分Shapley公理并位于核心中。


<details>
  <summary>Details</summary>
Motivation: 在线推荐平台中用户交互在内容创作者间产生相互依赖：对一个创作者内容的反馈会影响系统学习，进而影响其他创作者内容的曝光。需要分析这种环境下的激励机制，确保创作者间的协作稳定且公平。

Method: 采用多智能体随机线性老虎机问题与可转移效用合作博弈框架，将联盟价值定义为成员累积遗憾的负和。分析同质化创作者（固定动作集）和异质化创作者两种情况，提出基于遗憾的支付规则。

Result: 对于同质化创作者，在温和算法条件下诱导的TU博弈是凸的，核心非空且包含Shapley值，确保稳定性和公平性。对于异质化创作者，博弈核心仍然非空，但凸性和Shapley值核心成员资格不再保证。提出的遗憾支付规则满足四个Shapley公理中的三个，且位于核心中。MovieLens-100k数据集实验显示经验支付在不同设置和算法下与Shapley公平性的一致性和差异。

Conclusion: 在线推荐平台中创作者协作可以通过合作博弈框架有效建模，同质化创作者能实现稳定公平的激励机制，异质化创作者需要特殊设计支付规则来平衡稳定性和公平性要求。
...
### [114] [On Divergence Measures for Training GFlowNets](https://arxiv.org/abs/2410.09355)
*Tiago da Silva,Eliezer de Souza da Silva,Diego Mesquita*

Main category: cs.LG

TL;DR: 该论文提出了一种基于不同散度度量的GFlowNets训练方法，通过设计统计高效估计器和控制变量来减少梯度方差，相比传统方法能显著加速收敛。


<details>
  <summary>Details</summary>
Motivation: 传统GFlowNets训练使用期望对数平方差最小化，虽然与变分推断相关，但直接使用标准KL散度最小化会导致有偏且高方差的估计器。需要探索更有效的散度度量来改进训练。

Method: 首先回顾四种散度度量（Renyi-α、Tsallis-α、反向和正向KL散度），设计其随机梯度的统计高效估计器。然后设计基于REINFORCE留一法和分数匹配估计器的控制变量来减少学习目标梯度的方差。

Result: 适当最小化这些散度能产生理论上正确且经验上有效的训练方案，通常比先前提出的优化方法收敛速度显著更快。控制变量设计有效减少了梯度方差。

Conclusion: 该工作通过缩小GFlowNets训练与广义变分近似之间的差距，为基于散度最小化视角的算法思想铺平了道路，提供了更高效稳定的训练框架。
...
### [115] [PRAGMA: Revolut Foundation Model](https://arxiv.org/abs/2604.08649)
...
Method: 采用Transformer架构，在大规模异构银行事件语料库上使用自监督目标进行掩码建模预训练，该目标专门针对金融记录的离散、变长特性设计。

Result: PRAGMA在下游任务（如信用评分、欺诈检测、生命周期价值预测）中表现出优越性能，仅需在提取的嵌入上训练简单线性模型即可获得良好效果，轻量级微调可进一步提升性能。

Conclusion: PRAGMA能够直接从原始事件序列中在多个领域实现卓越性能，为金融应用提供了通用的表示层基础模型。
...
### [116] [Skip-Connected Policy Optimization for Implicit Advantage](https://arxiv.org/abs/2604.08690)
*Fengwei Teng,Jinyi Bai,Xinhao Yao,Demi Ruohan Wang,Jiahao Zhao,Zhijiang Guo*

Main category: cs.LG

TL;DR: SKPO提出了一种新的强化学习方法，通过将推理过程分解为上游和下游阶段，使用跳跃连接机制，解决了密集奖励在蒙特卡洛估计下的高方差问题，在数学推理和代码生成等任务上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 虽然理论上细粒度的密集奖励可以提升RLVR性能，但实践中蒙特卡洛估计会导致早期推理token的优势估计方差高且符号不一致，反而使结果不如仅使用最终结果的GRPO方法。
...
Result: 在Qwen2.5-Math-7B和Llama-3.2-3B模型上，相比最强基线分别获得3.91%和6.17%的相对提升，在数学基准测试、通用推理和代码生成等跨领域任务上均表现优异。进一步分析发现，即使最终正确率相同，SKPO生成的轨迹具有更高的中间步骤质量。

Conclusion: SKPO通过创新的跳跃连接架构有效解决了密集奖励估计的方差问题，不仅提升了最终性能，还改善了推理过程的中间步骤质量，为强化学习在复杂推理任务中的应用提供了新思路。
...
### [117] [EvoLen: Evolution-Guided Tokenization for DNA Language Model](https://arxiv.org/abs/2604.08698)
*Nan Huang,Xiaoxiao Zhou,Junxia Cui,Mario Tapia-Pacheco,Tiffany Amariuta,Yang Li,Jingbo Shang*

Main category: cs.LG

TL;DR: EvoLen：一种结合进化分层和长度感知解码的DNA分词器，通过跨物种进化信号更好地保留功能序列模式（如调控基序），在多种DNA语言模型基准测试中表现优于标准BPE方法。


<details>
  <summary>Details</summary>
Motivation: DNA语言模型中的分词设计研究不足。与自然语言不同，DNA缺乏固有的分词边界或预定义的组合规则，使得分词成为基础建模决策而非自然指定的过程。现有方法（如BPE）擅长捕捉反映人类生成语言规律的分词结构，但DNA是由生物功能和进化约束而非语言惯例组织的。作者认为DNA分词应优先考虑功能序列模式（如调控基序）——这些是受进化约束且通常在物种间保留的短重复片段。

Method: 提出EvoLen分词器，将进化信息直接整合到分词过程中：1）使用跨物种进化信号对DNA序列进行分组；2）在每个组上训练单独的BPE分词器；3）通过优先保留模式的规则合并所得词汇表；4）应用带有动态规划的长度感知解码。

Result: 通过控制实验，EvoLen在功能序列模式保留、跨基因组上下文区分以及与进化约束对齐方面表现更好，同时在多种DNA语言模型基准测试中匹配或优于标准BPE方法。

Conclusion: 分词引入了关键归纳偏置，结合进化信息能产生更具生物学意义和可解释性的序列表示。EvoLen证明了进化信息整合对DNA分词的重要性。
...
### [118] [Efficient RL Training for LLMs with Experience Replay](https://arxiv.org/abs/2604.08706)
*Charles Arnal,Vivien Cabannes,Taco Cohen,Julia Kempe,Remi Munos*

Main category: cs.LG

TL;DR: 本文挑战了LLM后训练中必须使用新鲜在线数据的传统观念，系统研究了经验回放缓冲区设计，证明精心设计的回放缓冲区能显著减少推理计算而不损害性能


<details>
  <summary>Details</summary>
Motivation: 挑战LLM后训练中必须使用新鲜在线数据的普遍假设，探索经验回放技术在LLM后训练中的应用潜力，解决生成数据的高计算成本问题

Method: 系统研究LLM后训练的回放缓冲区设计，形式化最优设计为陈旧性方差、样本多样性和生成计算成本之间的权衡，提出替代严格在线采样的方案

Result: 精心设计的回放缓冲区能大幅减少推理计算而不降低最终模型性能，在某些情况下甚至能提升性能，同时保持策略熵

Conclusion: 严格在线采样在生成成本高昂时是次优的，经验回放技术对LLM后训练具有重要价值，能有效平衡性能与计算效率
...
### [119] [Every Response Counts: Quantifying Uncertainty of LLM-based Multi-Agent Systems through Tensor Decomposition](https://arxiv.org/abs/2604.08708)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型多智能体系统在复杂任务上优于单智能体系统，但其复杂的交互引入了通信动态和角色依赖带来的可靠性挑战。现有不确定性量化方法通常针对单轮输出设计，无法处理MAS特有的复杂性，特别是多步推理中的级联不确定性、智能体间通信路径的变异性以及通信拓扑结构的多样性。

Method: 提出MATU框架，通过张量分解量化不确定性。该方法不仅分析最终文本输出，而是将整个推理轨迹表示为嵌入矩阵，并将多次执行运行组织成高阶张量。通过张量分解技术，分离并量化不同的不确定性来源，提供可泛化到不同智能体结构的全面可靠性度量。

Result: 实验表明MATU能够有效估计跨不同任务和通信拓扑结构的整体且稳健的不确定性，为多智能体系统提供了可靠的不确定性量化方法。

Conclusion: MATU框架成功解决了多智能体系统特有的不确定性量化挑战，通过张量分解方法提供了全面且可泛化的可靠性度量，为复杂多智能体系统的可靠性评估提供了新思路。
...
### [120] [IKKA: Inversion Classification via Critical Anomalies for Robust Visual Servoing](https://arxiv.org/abs/2604.08754)
*Darya Pavlenko*

Main category: cs.LG

TL;DR: IKKA是一种基于拓扑的加权框架，通过将异常点视为结构信息观测来提升视觉伺服在分布偏移下的鲁棒性，在嵌入式平台上显著降低误差并提高处理速度。


<details>
  <summary>Details</summary>
Motivation: 传统异常处理方法通常将异常点视为噪声或干扰，但IKKA认为这些异常点实际上包含了重要的结构信息，特别是在决策边界附近的小扰动可能导致定性不同的控制响应或分类结果。这为解决视觉伺服在分布偏移下的鲁棒性问题提供了新思路。

Method: IKKA将局部极值性、边界横截性和多尺度持续性结合成一个单一的异常权重W(x)=E(x)×T(x)×M(x)，用于调节在模糊决策区域附近的控制更新。该方法在Raspberry Pi 4的CPU-only嵌入式视觉伺服管道中实现。

Result: 在涉及昏暗照明和瞬时遮挡的压力场景下，IKKA将95%分位数的横向误差相对于混合基线降低了24%（从0.124降至0.094），同时将吞吐量从20.0Hz提高到24.8Hz。非参数分析确认了较大的效应量（Cliff's delta = 0.79）。

Conclusion: IKKA通过将异常点重新定义为结构信息观测，提供了一种有效的拓扑加权框架，显著提高了视觉伺服系统在分布偏移下的鲁棒性和性能，特别适用于资源受限的嵌入式平台。
...
### [121] [Adaptive Simulation Experiment for LLM Policy Optimization](https://arxiv.org/abs/2604.08779)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在运营管理中有提升效率的潜力，但部署时需要制定策略来保证响应质量、塑造用户体验并影响运营价值。现有方法缺乏系统化的策略选择框架，需要一种能有效识别最优策略的方法。

Method: 将LLM视为随机模拟器，提出基于成对比较的自适应模拟实验框架LLM-PO。考虑两种策略空间：无结构空间（无参数假设）和结构化空间（基于偏好模型）。在无结构情况下推导最优采样比例的闭式解；在结构化情况下制定正则化凸规划计算最优比例。开发自适应实验程序，保证统计可靠性。

Result: 理论分析表明LLM-PO能以高概率识别最优策略，并渐近达到基本数据要求。数值实验证明LLM-PO始终优于基准方法，能有效提升LLM性能。

Conclusion: LLM-PO框架为LLM策略选择提供了系统化方法，在两种策略空间中都能保证统计可靠性，实际应用中能显著提升LLM性能，为运营管理中的LLM部署提供了有效工具。
...
### [122] [$p1$: Better Prompt Optimization with Fewer Prompts](https://arxiv.org/abs/2604.08801)
*Zhaolin Gao,Yu,Wang,Bo Liu,Thorsten Joachims,Kianté Brantley,Wen Sun*

Main category: cs.LG

TL;DR: 该研究分析了提示优化的有效性条件，发现系统提示间的方差是关键因素，并提出p1方法通过筛选高方差用户提示来改进优化效果。


<details>
  <summary>Details</summary>
Motivation: 研究提示优化在不同任务中效果差异的原因，探索什么因素决定任务是否适合进行提示优化，以提高语言模型在不更新权重情况下的性能。

Method: 将奖励方差分解为响应间方差和系统提示间方差，分析两者对优化效果的影响。提出p1方法，通过筛选在候选系统提示间方差高的用户提示子集来改进优化。

Result: 实验显示p1方法在推理基准测试中显著优于在全数据集上训练的方法，也优于GEPA等基线方法。仅使用AIME 24中的两个提示就能训练出泛化良好的系统提示。

Conclusion: 提示优化的成功取决于系统提示间方差是否足够大，用户提示筛选能有效提高优化效果，为提示优化提供了理论指导和实用方法。
...
### [123] [Smartwatch-Based Sitting Time Estimation in Real-World Office Settings](https://arxiv.org/abs/2604.08808)
*Olivia Zhang,Zhilin Zhang*

Main category: cs.LG

TL;DR: 本文提出一种利用智能手表IMU信号估计久坐时间的方法，通过引入基于欧拉角导出的旋转向量序列作为运动动态的新表示，在真实办公环境中提高了算法性能。
...
Method: 提出一种从智能手表IMU信号估计久坐时间的方法，引入基于欧拉角导出的旋转向量序列作为运动动态的新表示，用于捕捉办公室工作人员日常活动中的运动模式。

Result: 在34小时数据集上的实验表明，利用旋转向量序列提高了算法性能，证明了该方法在自然环境中进行稳健久坐时间估计的潜力。

Conclusion: 旋转向量序列作为运动动态表示的有效性得到验证，该方法在真实办公环境中为久坐时间估计提供了有前景的解决方案，有助于健康监测和干预。
...
### [124] [Structural Evaluation Metrics for SVG Generation via Leave-One-Out Analysis](https://arxiv.org/abs/2604.08809)
*Haonan Zhu,Adrienne Deganutti,Elad Hirsch,Purvanshi Mehta*

Main category: cs.LG

TL;DR: 论文提出了一种基于留一法（LOO）分析的SVG结构质量评估方法，能够量化SVG生成的结构化编辑能力，而不仅仅是视觉相似度。


<details>
  <summary>Details</summary>
Motivation: 当前SVG生成评估主要关注视觉相似度，忽略了SVG的核心价值——结构化可编辑性。现有指标无法评估哪些生成元素对视觉质量有贡献、视觉概念如何映射到代码部分，以及输出是否支持有意义的后续编辑。

Method: 引入元素级留一法分析：分别渲染包含和不包含每个元素的SVG，测量视觉变化，从而推导出一套结构质量指标。该方法能够分解整体统计量到每个元素的贡献。

Result: 从单一机制获得：1）通过LOO评分实现零样本伪影检测；2）概念-元素归因映射；3）四个结构指标（纯度、覆盖率、紧凑性、局部性）从互补角度量化SVG模块化。在5个生成系统、3个复杂度层级、超过19,000次编辑上验证了这些指标。

Conclusion: 提出的留一法分析为SVG生成提供了全面的结构质量评估框架，超越了传统的视觉相似度评估，能够量化SVG的结构化编辑能力这一核心价值。
...
### [125] [HiFloat4 Format for Language Model Pre-training on Ascend NPUs](https://arxiv.org/abs/2604.08826)
...
<details>
  <summary>Details</summary>
Motivation: 大型基础模型的训练和部署需要巨大的计算和内存成本，因此需要开发低精度训练技术。4位浮点格式(FP4)如MXFP4和NVFP4在线性GEMM操作中已显示出潜力，但华为昇腾NPU上的HiFloat4格式在大规模训练中的表现尚未得到系统研究。

Method: 在昇腾NPU集群上进行实验，所有线性GEMM和专家GEMM操作完全使用FP4精度。评估密集架构(如Pangu和LLaMA风格模型)和混合专家模型(MoE)，并探索针对FP4训练的稳定化技术以减少数值退化。

Result: FP4训练能保持相对误差在完整精度基线的1%以内，同时保留4位计算效率优势。研究提供了NPU上FP4训练的全面实证研究，并突出了FP4格式在大规模密集和MoE模型中的实际权衡。

Conclusion: FP4格式在大规模训练中具有实用价值，HiFloat4与MXFP4的比较为昇腾NPU上的低精度训练提供了重要参考，稳定化技术能有效减少数值退化，使FP4训练在保持效率的同时维持模型质量。
...
### [126] [Post-Hoc Guidance for Consistency Models by Joint Flow Distribution Learning](https://arxiv.org/abs/2604.08828)
*Chia-Hong Hsu,Randall Balestriero*

Main category: cs.LG

TL;DR: 提出JFDL方法，使预训练的Consistency Models能够进行事后指导，无需依赖扩散模型教师，填补了现有方法的关键空白


<details>
  <summary>Details</summary>
Motivation: Classifier-free Guidance (CFG) 在扩散模型中实现了保真度与多样性的权衡，但采样成本高。一致性模型虽然采样快，但现有指导方法需要从扩散模型教师进行知识蒸馏，限制了CFG的应用范围

Method: 提出联合流分布学习（JFDL），这是一种轻量级的对齐方法，能够在预训练的一致性模型中实现指导。通过将预训练的一致性模型作为ODE求解器，验证无条件分布和条件分布的速度场隐含的噪声是高斯分布

Result: JFDL为一致性模型提供了可调节的指导旋钮，生成的指导图像具有与CFG相似的特征。应用于原始的一致性训练模型后，解锁了指导生成功能，并在CIFAR-10和ImageNet 64x64数据集上降低了FID

Conclusion: 这是首次实现一致性模型无需扩散模型教师就能获得有效的事后指导，填补了当前一致性模型方法的关键空白
...
<details>
  <summary>Details</summary>
Motivation: 解决标准注意力机制在处理长序列时的计算效率问题，同时提供多尺度表示能力，通过层次化结构捕捉不同粒度的序列特征。

Method: 使用可训练因果下采样在L个分辨率级别处理序列，每个级别生成特定得分矩阵，通过学习凸权重组合这些矩阵，形成层次化核变换器。

Result: 在三个数据集上均优于重新训练的标准注意力基线：ListOps提升4.77pp，顺序CIFAR-10提升1.44pp，IMDB字符级情感分析提升7.47pp，计算开销仅为1.31倍。

Conclusion: HKT在计算效率和多尺度表示方面优于标准注意力，严格包含单头标准注意力和因果卷积，提供理论保证和可解释的误差分解。
...
### [128] [Discrete Meanflow Training Curriculum](https://arxiv.org/abs/2604.08837)
...
<details>
  <summary>Details</summary>
Motivation: MeanFlow模型虽然具有优秀的少步采样性能和诱人的单步采样性能，但现有模型需要极大的训练预算。本文旨在显著降低训练MeanFlow模型所需的计算和数据成本。

Method: 提出Discrete Meanflow (DMF)训练课程，利用Meanflow目标的特定离散化，得到一个一致性属性，从预训练的Flow模型初始化，实现高效训练。

Result: DMF课程在CIFAR-10上仅用2000轮训练就达到单步FID 3.36，显著降低了训练计算和数据需求。

Conclusion: 更快的MeanFlow训练课程，特别是从现有Flow模型微调的方法，将推动未来单步采样模型的高效训练方法发展。
...
### [129] [Spectral Geometry of LoRA Adapters Encodes Training Objective and Predicts Harmful Compliance](https://arxiv.org/abs/2604.08844)
*Roi Paul*

Main category: cs.LG

TL;DR: 研究LoRA权重增量的低秩谱特征能否识别微调目标类型，以及这些几何特征是否能预测下游有害行为。在Llama-3.2-3B-Instruct上制造38个LoRA适配器，发现DPO训练的分类器能完美区分不同目标，几何特征与有害行为有强相关性，但跨方法泛化失败。


<details>
  <summary>Details</summary>
Motivation: 研究LoRA权重增量的几何特征是否包含关于微调目标类型的信息，以及这些几何特征是否能预测模型在下游任务中的有害行为表现。这对于监控模型微调过程、检测潜在有害修改具有重要意义。

Method: 在Llama-3.2-3B-Instruct上制造38个LoRA适配器，分为四类：健康SFT基线、反转无害偏好的DPO、反转有益偏好的DPO、激活导向适配器。提取每层的谱特征（范数、稳定秩、奇异值熵、有效秩、奇异向量余弦对齐）。使用逻辑回归分类器和主成分分析进行几何特征分析，并在HEx-PHI提示上进行行为评估。

Result: DPO训练的分类器在二元漂移检测、所有六对目标比较和严重性排序上达到AUC~1.00。主成分分析显示训练目标是PC1（AUC~1.00），与PC2上的训练时长正交。查询投影权重检测漂移发生，值投影权重识别具体目标。跨方法泛化完全失败。行为评估显示反转无害偏好的DPO适配器在有害合规性上显著升高，几何特征与行为排名相关性ρ=0.72。

Conclusion: 在受控制造机制下，LoRA权重空间几何携带目标身份、强度排序信息，并与有害合规性有粗略关联。跨方法监控需要针对每种方法进行校准，几何特征可用于检测微调目标类型和潜在有害修改。
...
### [130] [Dictionary-Aligned Concept Control for Safeguarding Multimodal LLMs](https://arxiv.org/abs/2604.08846)
...
<details>
  <summary>Details</summary>
Motivation: 现有的MLLM安全防护方法存在局限性：提示工程、响应分类或微调方法对不断演变的恶意模式效果有限，可能需要重新运行查询，或计算资源需求大。现有的激活引导方法通常只能处理有限的安全相关概念，或难以在不影响其他概念的情况下调整特定概念。

Method: 提出Dictionary-Aligned Concept Control (DACO)框架：1) 构建包含15,000个多模态概念的概念字典，通过检索40万+图文刺激并总结其激活方向，创建DACO-400K数据集；2) 使用稀疏编码通过概念字典干预激活；3) 提出新的引导方法，使用字典初始化稀疏自编码器训练，并自动标注SAE原子的语义用于MLLM安全防护。

Result: 在多个MLLM（QwenVL、LLaVA、InternVL）和安全基准测试（MM-SafetyBench、JailBreakV）上的实验表明，DACO显著提高了MLLM的安全性，同时保持了通用能力。

Conclusion: DACO通过概念字典和稀疏自编码器提供了一种灵活有效的MLLM安全控制方法，能够细粒度地干预模型激活，在提升安全性的同时不影响模型的通用能力，解决了现有方法的局限性。
...
### [131] [Temporal Dropout Risk in Learning Analytics: A Harmonized Survival Benchmark Across Dynamic and Early-Window Representations](https://arxiv.org/abs/2604.08870)
*Rafael da Silva,Jeff Eicher,Gregory Longo*

Main category: cs.LG

TL;DR: 本研究提出了一个面向生存分析的辍学风险建模基准，使用OULAD数据集比较动态周度模型和连续时间模型，发现随机生存森林在连续时间臂中表现最佳，而泊松分段指数在动态臂中略优，主要预测信号来自时间行为特征而非静态背景属性。


<details>
  <summary>Details</summary>
Motivation: 学习分析中辍学预测研究存在异质性协议问题，现有比较研究过于关注区分度而忽视时间可解释性和校准。需要建立统一的生存分析基准来评估时间辍学风险模型。

Method: 使用OULAD数据集建立两个协调的评估臂：动态周度臂（人-周期表示）和连续时间臂（扩展模型家族）。评估协议包含四个分析层：预测性能、消融分析、可解释性和校准分析。

Result: 在连续时间臂中，随机生存森林在区分度和特定时间Brier分数上领先；在动态臂中，泊松分段指数在综合Brier分数上略优于其他五个紧密簇模型。所有模型家族都显示主要预测信号来自时间行为特征而非人口统计或结构特征。

Conclusion: 协调的多维基准在学习分析中具有价值，辍学风险应被视为时间行为过程而非静态背景属性的函数，这为辍学预测研究提供了更全面的评估框架。
...
### [132] [How does Chain of Thought decompose complex tasks?](https://arxiv.org/abs/2604.08872)
*Amrut Nadgir,Vijay Balasubramanian,Pratik Chaudhari*

Main category: cs.LG

TL;DR: 研究发现语言模型分类错误随类别数呈幂律增长，通过将任务分解为多步小分类问题（树状分解）可显著降低错误率，类似思维链方法


<details>
  <summary>Details</summary>
Motivation: 许多语言任务可建模为分类问题，但传统方法中分类错误随类别数增加而显著增长，需要探索更有效的任务分解策略

Method: 提出树状分解方法，将复杂分类任务分解为一系列较小分类问题（每个具有相同类别数），建立类似思维链的层次结构，分析不同深度和度数的优化关系

Result: 发现分类错误随类别数呈幂律增长；存在临界度数阈值，低于该阈值时增加思考深度反而有害，高于阈值时存在最优深度可最小化错误；存在无法通过增加深度超越的最小错误极限

Conclusion: 树状分解方法能有效降低语言模型分类错误，但存在最优深度限制，超过该限制无法进一步改善性能，为思维链方法的优化提供了理论指导
...
### [133] [A Mathematical Framework for Temporal Modeling and Counterfactual Policy Simulation of Student Dropout](https://arxiv.org/abs/2604.08874)
*Rafael da Silva,Jeff Eicher,Gregory Longo*

Main category: cs.LG

TL;DR: 本研究提出一个结合反事实政策模拟层的时间建模框架，用于预测高等教育学生辍学。使用LMS参与数据和行政退学记录，将辍学建模为离散时间事件，模型在时间保留测试中达到0.8405的AUC。政策模拟显示不同干预策略对生存率的影响差异。


<details>
  <summary>Details</summary>
Motivation: 高等教育学生辍学是一个重要问题，需要基于数据的预测和干预策略评估。现有方法缺乏结合时间建模和政策模拟的框架，无法评估不同干预策略在现实约束下的效果。

Method: 提出时间建模框架，将辍学操作化为时间到事件结果，使用惩罚性类别平衡逻辑回归在离散时间建模每周风险。添加反事实政策模拟层，通过场景索引政策产生生存对比ΔS(T)，在明确触发/调度合同下评估干预效果。

Result: 模型在时间保留测试中达到0.8405的AUC，校准可接受但在高风险区间支持稀疏。政策模拟显示：冲击分支产生正生存对比（T=18时0.0102-0.0819），机制感知分支为负（T=18时-0.0078）。性别子组分析显示方向稳定但效应小。

Conclusion: 该框架能够在观察数据约束下进行内部结构场景比较，展示了结合时间建模和政策模拟的潜力。结果非因果识别，但为评估不同干预策略提供了实用工具，强调了时间参与信号的重要性。
...
### [134] [Revisiting the Capacity Gap in Chain-of-Thought Distillation from a Practical Perspective](https://arxiv.org/abs/2604.08880)
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明CoT蒸馏在教师-学生能力差距较大时可能失败，但现有评估方法存在局限性，需要更现实的评估来理解能力差距的实际影响

Method: 重新审视常用的实验设置，发现CoT蒸馏常导致性能下降的问题被掩盖，提出更现实的评估协议，分析不同任务和设置下能力差距的影响

Result: 能力差距的影响在不同任务和设置中并不总是主导因素，尤其是在候选教师性能差异显著时，结果为CoT蒸馏中教师-学生配对选择提供了实用指导

Conclusion: 需要更现实的评估方法来准确评估CoT蒸馏效果，能力差距的影响因任务和设置而异，研究结果为实际应用中的教师选择提供了指导
...
### [135] [Uncertainty-Aware Transformers: Conformal Prediction for Language Models](https://arxiv.org/abs/2604.08885)
*Abhiram Vellore,Niraj K. Jha*

Main category: cs.LG

TL;DR: CONFIDE是一个基于保形预测的Transformer语言模型不确定性量化框架，通过内部嵌入构建统计有效的预测集，提高准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: Transformer的黑盒性质限制了其在高风险场景中的可信度和部署。为了使模型在关键应用中真正有用和可信，需要提供决策背后的清晰推理过程，而不仅仅是预测结果。

Method: CONFIDE将保形预测应用于BERT和RoBERTa等编码器架构的内部嵌入，使用[CLS]标记嵌入或展平的隐藏状态构建类条件非一致性分数，支持超参数调优，生成统计有效的预测集并提供实例级解释。

Result: 在BERT-tiny上测试准确率提升高达4.09%，相比NM2和VanillaNN等方法获得更高的正确效率。早期和中间Transformer层通常能产生更好校准和更具语义意义的表示。在资源受限模型和标签模糊的高风险任务中，CONFIDE提供了鲁棒性和可解释性。

Conclusion: CONFIDE作为一个实用诊断框架，在保形预测基线方法上实现了效率和鲁棒性的改进，为高风险应用中的Transformer模型提供了可信的不确定性量化解决方案。
...
### [136] [A Closer Look at the Application of Causal Inference in Graph Representation Learning](https://arxiv.org/abs/2604.08890)
*Hang Gao,Kunyu Li,Huang Hong,Baoquan Cui,Fengge Wu*

Main category: cs.LG

TL;DR: 该论文证明了图表示学习中聚合不同图元素为单一因果变量的做法会损害因果有效性，提出了基于最小不可分图数据单元的理论模型，并开发了可集成到现有图学习流程的因果建模增强模块。


<details>
  <summary>Details</summary>
Motivation: 现有图表示学习方法在建模因果关系时，经常将不同的图元素聚合为单一因果变量，这种做法可能违反因果推断的核心假设。由于图结构数据的固有复杂性，这种聚合操作会损害因果有效性，需要建立更严谨的理论基础。

Method: 1. 证明聚合操作会损害因果有效性；2. 提出基于最小不可分图数据单元的理论模型来保证因果有效性；3. 分析实现精确因果建模的成本并确定问题简化的条件；4. 构建反映真实世界因果结构的可控合成数据集进行实验验证；5. 开发可集成到现有图学习流程的因果建模增强模块。

Result: 1. 理论证明了聚合操作会损害因果有效性；2. 提出的理论模型能够保证因果有效性；3. 通过可控合成数据集验证了理论；4. 开发的因果建模增强模块在综合对比实验中表现出有效性。

Conclusion: 图表示学习中的因果建模需要基于最小不可分的数据单元来保证有效性，聚合操作会损害因果推断的准确性。提出的理论模型和增强模块为解决这一问题提供了有效途径，同时分析了精确因果建模的成本和简化条件。
...
### [137] [Using Synthetic Data for Machine Learning-based Childhood Vaccination Prediction in Narok, Kenya](https://arxiv.org/abs/2604.08902)
*Jimmy Bach,Yang Li,Yaqi Liu,John Sankok,Rose Kimani,Carrie B. Dolan,Julius N. Odhiambo,Haipeng Chen*

Main category: cs.LG

TL;DR: 该研究在肯尼亚马赛族低资源环境中，通过机器学习预测儿童漏种疫苗风险，并利用合成数据保护隐私，结果显示预测准确率高且合成数据不影响性能。


<details>
  <summary>Details</summary>
Motivation: 在肯尼亚纳罗克县马赛族等游牧人群中，数据利用不足阻碍疫苗接种系统，导致覆盖率估计不准确、资源分配低效，且敏感健康数据隐私保护需求高。

Method: 数字化8年儿童疫苗接种记录（n=6,913），应用逻辑回归和XGBoost机器学习模型识别漏种风险儿童，并采用表格扩散合成数据生成（TabSyn）技术保护患者隐私。

Result: 分类技术能可靠预测儿童漏种疫苗风险，部分疫苗模型的召回率、精确率和F1分数超过90%；使用合成数据训练模型不会降低预测性能，同时保护原始数据隐私。

Conclusion: 研究支持在数字基础设施有限的诊所中采用合成数据实施健康信息学策略，实现隐私保护、可扩展的儿童免疫覆盖率预测。
...
### [138] [Bridging SFT and RL: Dynamic Policy Optimization for Robust Reasoning](https://arxiv.org/abs/2604.08926)
*Taojie Zhu,Dongyang Xu,Ding Zou,Sen Zhao,Qiaobo Hao,Zhiguo Yang,Yonghong He*

Main category: cs.LG

TL;DR: DYPO是一个统一的大语言模型后训练框架，通过动态平衡监督微调（SFT）的稳定性和强化学习（RL）的探索性，解决传统方法中的偏差-方差权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型后训练方法面临基本困境：监督微调（SFT）虽然稳定（低方差）但存在高拟合偏差，而强化学习（RL）能够探索（低偏差）但面临高梯度方差。现有统一优化策略往往采用简单的损失加权，忽略了这些不同梯度信号之间的统计冲突。

Method: DYPO包含三个核心组件：1）组对齐损失（GAL），利用内在组动态显著降低RL梯度方差；2）多教师蒸馏机制，通过多样化推理路径纠正SFT拟合偏差；3）动态利用-探索门控机制，基于奖励反馈自适应地在稳定SFT和探索性RL之间进行仲裁。

Result: 理论分析证实DYPO线性减少拟合偏差并最小化总体方差。大量实验表明，DYPO显著优于传统顺序流程，在复杂推理基准上平均提升4.8%，在分布外任务上提升13.3%。

Conclusion: DYPO提供了一个统一的后训练框架，通过结构性地缓解SFT和RL之间的统计冲突，有效解决了大语言模型后训练中的偏差-方差权衡问题，在多个任务上取得了显著性能提升。
...
### [139] [Delve into the Applicability of Advanced Optimizers for Multi-Task Learning](https://arxiv.org/abs/2604.08939)
*Zhipeng Zhou,Linxiao Cao,Pengcheng Wu,Peilin Zhao,Chunyan Miao*

Main category: cs.LG

TL;DR: APT框架通过自适应动量机制和轻量方向保持方法，解决了多任务学习中高级优化器与MTL方法之间的不匹配问题，显著提升了现有MTL方法的性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现现有基于优化的多任务学习方法在采用高级优化器时存在一个被忽视的问题：即时梯度在实际参数更新中作用有限，导致MTL框架无法充分发挥学习动态的潜力。同时发现Muon优化器本身具有多任务学习能力，但其正交化过程对梯度质量很敏感。

Method: 提出APT框架，包含两个核心组件：1）简单的自适应动量机制，平衡高级优化器与MTL方法的优势；2）轻量方向保持方法，促进Muon优化器的正交化过程。

Result: 在四个主流MTL数据集上的广泛实验表明，APT能够持续增强现有的MTL方法，带来显著的性能提升。

Conclusion: APT框架有效解决了高级优化器与多任务学习方法之间的协同问题，通过自适应动量机制和方向保持技术，显著提升了MTL方法的实际效果。
...
### [140] [Predictive Entropy Links Calibration and Paraphrase Sensitivity in Medical Vision-Language Models](https://arxiv.org/abs/2604.08941)
*Binesh Sadanandan,Vahid Behzadan*

Main category: cs.LG

TL;DR: 医学视觉语言模型存在置信度校准不佳和对问题重述敏感的问题，研究发现这两个问题都与决策边界接近有关。通过评估五种不确定性量化方法，发现简单的单模型预测熵方法在错误检测和重述敏感预测识别方面优于复杂的集成方法。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在安全部署中存在两个关键问题：1）置信度校准不佳（mis-calibrated confidence），2）对问题重述的敏感性（sensitivity to question rephrasing）。这些问题威胁到模型在医疗领域的可靠应用。

Method: 在MedGemma 4BIT和LLaVA RAD7B两个模型上，评估了五种不确定性量化方法：1）单模型预测熵，2）五成员LoRA集成，3）MC Dropout，4）其他方法。使用MIMIC-CXR（分布内）和PadChest（分布外）胸部X光数据集进行基准测试，通过AUROC等指标评估错误检测和重述敏感预测识别能力。

Result: 1）单模型预测熵能有效预测哪些样本会在问题重述时改变预测（MedGemma AUROC 0.711，LLaVA-RAD AUROC 0.878）；2）LoRA集成在分布转移下表现不佳（ECE 42.9，准确率34.1）；3）MC Dropout获得最佳校准（ECE 4.3）和选择性预测覆盖；4）单次前向传播的总熵在错误检测（AUROC 0.743 vs 0.657）和重述筛选方面优于集成方法。

Conclusion: 医学视觉语言模型的置信度校准不佳和对问题重述敏感问题有共同原因——接近决策边界。简单的单模型不确定性量化方法（如预测熵）在识别不可靠预测方面优于复杂的集成方法，为医学VLM的安全部署提供了实用解决方案。
...
### [141] [WOMBET: World Model-based Experience Transfer for Robust and Sample-efficient Reinforcement Learning](https://arxiv.org/abs/2604.08958)
*Mintae Kim,Koushil Sreenath*

Main category: cs.LG

TL;DR: WOMBET是一个基于世界模型的强化学习经验转移框架，通过不确定性惩罚规划生成高质量离线数据，结合自适应采样实现从源任务到目标任务的高效知识迁移。
...
Method: 1) 在源任务中学习世界模型；2) 通过不确定性惩罚规划生成离线数据；3) 筛选高回报、低认知不确定性的轨迹；4) 在目标任务中采用离线与在线数据的自适应采样进行微调。

Result: 理论证明不确定性惩罚目标提供了真实回报的下界，并推导了有限样本误差分解。实验表明WOMBET在连续控制基准测试中优于强基线，提高了样本效率和最终性能。

Conclusion: 联合优化数据生成和转移的策略能有效提升强化学习经验转移效果，WOMBET框架为机器人RL中的样本效率问题提供了有前景的解决方案。
...
### [142] [Efficient Hierarchical Implicit Flow Q-learning for Offline Goal-conditioned Reinforcement Learning](https://arxiv.org/abs/2604.08960)
*Zhiqiang Dong,Teng Pang,Rongjian Xu,Guoqiang Wu*

Main category: cs.LG

TL;DR: 本文提出了一种用于离线目标条件强化学习（GCRL）的均值流策略，通过引入平均速度场来改进分层策略建模，并采用LeJEPA损失增强目标表示，在OGBench基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 离线目标条件强化学习中，由于高斯策略表达能力有限以及高层策略无法生成有效子目标，长时程控制仍然具有挑战性。现有分层架构如HIQL在处理复杂任务时存在局限性。

Method: 提出目标条件均值流策略，在分层策略建模中引入平均速度场，通过学习的平均速度场捕获高层和低层策略的复杂目标分布，实现一步采样的高效动作生成。同时引入LeJEPA损失来排斥目标表示嵌入，鼓励更具区分性的表示并改善泛化能力。

Result: 实验结果表明，该方法在OGBench基准测试中的基于状态和基于像素的任务上都取得了强大的性能表现。

Conclusion: 均值流策略通过引入平均速度场和改进目标表示，有效解决了离线GCRL中长时程控制的挑战，为复杂任务的学习提供了更有效的解决方案。
...
### [143] [The nextAI Solution to the NeurIPS 2023 LLM Efficiency Challenge](https://arxiv.org/abs/2604.09034)
*Gyuwon Park,DongIl Shin,SolGil Oh,SangGi Ryu,Byung-Hak Kim*

Main category: cs.LG

TL;DR: 该研究在NeurIPS LLM效率挑战赛中，使用QLoRA微调技术，在单张A100 40GB GPU的严格约束下，24小时内成功优化了LLaMa2 70B模型，实现了资源利用显著降低的同时保持了高精度性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型复杂度增加，资源使用和透明度问题日益突出。研究旨在解决这些挑战，在严格资源约束下优化基础模型，证明大规模模型在资源受限环境中的可行性。

Method: 采用量化低秩适配（QLoRA）微调技术，结合Flash Attention 2等先进注意力机制。使用自定义数据集，该数据集从多样开源资源和基准测试中精心构建，并实验了多种LoRA配置以平衡计算效率和模型精度。

Result: 成功在单GPU约束下微调了LLaMa2 70B模型，不仅显著降低了资源利用率，还在多个QA基准测试中表现出高准确性，验证了在资源受限环境中优化大规模模型的可行性。

Conclusion: 研究证明了在严格资源约束下优化大规模语言模型的可行性，强调了LLM在现实世界应用中的潜力，为资源受限环境中的高效模型优化提供了有效方法。
...
### [144] [PDE-regularized Dynamics-informed Diffusion with Uncertainty-aware Filtering for Long-Horizon Dynamics](https://arxiv.org/abs/2604.09058)
*Min Young Baeg,Yoon-Yeong Kim*

Main category: cs.LG

TL;DR: PDYffusion：一种基于物理动力学信息的扩散框架，通过PDE正则化和UKF预测器实现稳定长期时空预测，解决了累积误差和物理不一致性问题。


<details>
  <summary>Details</summary>
Motivation: 长期时空预测面临累积误差、噪声放大和物理一致性缺乏的挑战。传统扩散模型依赖均方误差目标，无法捕捉物理定律支配的底层动力学。

Method: 提出PDYffusion框架，包含两个核心组件：PDE正则化插值器和UKF预测器。插值器通过微分算子强制物理一致的中间状态，预测器利用无迹卡尔曼滤波器显式建模不确定性并减轻迭代预测中的误差累积。

Result: 在多个动力学数据集上的实验表明，PDYffusion在CRPS和MSE指标上表现优异，同时通过SSR测量保持稳定的不确定性行为。理论分析显示插值器满足PDE约束平滑性，预测器在提出的损失公式下收敛。

Conclusion: PDYffusion为长期预测提供了平衡且稳健的解决方案，分析了预测精度与不确定性之间的内在权衡，实现了物理一致的稳定长期时空预测。
...
### [145] [Feature-Label Modal Alignment for Robust Partial Multi-Label Learning](https://arxiv.org/abs/2604.09064)
*Yu Chen,Weijun Lv,Yue Huang,Xiaozhao Fang,Jie Wen,Yong Xu,Guanbin Li*

Main category: cs.LG

TL;DR: 提出基于特征-标签模态对齐的PML方法，通过低秩正交分解生成伪标签，全局投影与局部结构对齐，多峰值类原型学习，显著提升分类精度和噪声鲁棒性。
...
Method: 1. 使用低秩正交分解生成伪标签，过滤噪声标签近似真实标签分布；2. 通过全局投影到共同子空间和局部邻域结构保持来对齐特征和伪标签；3. 多峰值类原型学习机制，利用伪标签作为软成员权重增强判别性。
...
### [146] [Beyond Isolated Clients: Integrating Graph-Based Embeddings into Event Sequence Models](https://arxiv.org/abs/2604.09085)
*Harry Proshian,Nikita Severin,Sergey Nikolenko,Kireev Ivan,Andrey Savchenko,Ivan Sergeev,Maria Postnova,Ilya Makarov*

Main category: cs.LG

TL;DR: 该论文提出三种模型无关策略，将用户-物品交互图的结构信息融入对比自监督学习，以提升时序事件预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 大规模数字平台产生大量带时间戳的用户-物品交互事件，这些事件对预测用户属性（如欺诈预防和推荐）至关重要。现有的自监督学习方法虽然能有效建模事件的时间顺序，但通常忽略了用户-物品交互图的全局结构信息。

Method: 提出了三种模型无关的策略来将结构信息融入对比自监督学习：1）丰富事件嵌入；2）将客户端表示与图嵌入对齐；3）添加结构预训练任务。这些策略旨在弥补时序建模与图结构信息之间的差距。

Result: 在四个金融和电商数据集上的实验表明，该方法能持续提升预测准确性（AUC最高提升2.3%），并发现图密度是选择最优集成策略的关键因素。

Conclusion: 将用户-物品交互图的全局结构信息融入对比自监督学习能显著提升时序事件预测性能，且图密度是决定哪种集成策略最有效的关键指标。
...
### [147] [GeoPAS: Geometric Probing for Algorithm Selection in Continuous Black-Box Optimisation](https://arxiv.org/abs/2604.09095)
...
<details>
  <summary>Details</summary>
Motivation: 连续黑盒优化中的自动算法选择通常依赖于有限的探测预算下计算的固定景观描述符，但这些描述符在问题分割或跨基准评估中性能会下降。需要一种更鲁棒、可迁移的表示方法。

Method: 提出GeoPAS方法：1) 通过在不同位置、方向和对数尺度上采样多个粗略的二维切片来表示问题实例；2) 使用共享的有效性感知卷积编码器将每个切片映射到嵌入向量；3) 基于切片尺度和幅度统计进行条件化；4) 通过置换不变的方式聚合特征；5) 通过对数尺度性能预测实现风险感知的求解器选择，并对尾部失败施加显式惩罚。

Result: 在COCO/BBOB基准测试中，使用包含12个求解器的组合，在2-10维问题上，GeoPAS在留实例出、分组随机和留问题出评估中均优于单个最佳求解器。多尺度几何切片为算法选择提供了有用的可迁移静态信号。

Conclusion: 几何探测方法能够有效改进连续黑盒优化中的算法选择性能，尽管少数重尾区域仍然存在并主导平均性能。多尺度几何切片为算法选择提供了可迁移的表示方法。
...
### [148] [Score-Driven Rating System for Sports](https://arxiv.org/abs/2604.09143)
*Vladimír Holý,Michal Černý*

Main category: cs.LG

TL;DR: 本文提出了一种基于分数的评分系统，作为经典Elo评分系统的推广，使用对数似然的梯度作为更新机制，能够处理多种比赛结果类型。


<details>
  <summary>Details</summary>
Motivation: 传统Elo评分系统主要针对简单的胜/负结果，无法有效处理点差、胜/平/负、完整排名等多种比赛结果类型，需要一种更通用的评分框架。

Method: 提出分数驱动的评分系统，使用对数似然函数的梯度作为评分更新机制，该框架能够适应多种比赛结果类型，并推导了分数的理论性质。

Result: 证明了分数具有零期望值、所有玩家分数总和为零、随玩家评分增加而减少等性质，确保内部一致性和公平性，同时系统具有回归特性，评分会随时间趋向于潜在的真实技能水平。

Conclusion: 该框架为现有的体育表现动态模型提供了理论依据，并为构建新模型提供了系统化方法，是传统Elo评分系统的有效推广。
...
### [149] [Truncated Rectified Flow Policy for Reinforcement Learning with One-Step Sampling](https://arxiv.org/abs/2604.09159)
*Xubin Zhou,Yipeng Yang,Zhan Li*

Main category: cs.LG

TL;DR: TRFP是一种基于截断整流流的混合确定性-随机架构，解决了生成式策略在最大熵强化学习中面临的似然和熵难计算、多步采样不稳定等问题，实现了有效的多模态行为建模和高效的一步采样。


<details>
  <summary>Details</summary>
Motivation: 标准高斯策略参数化本质上是单模态的，无法建模复杂的多模态动作分布。虽然扩散和流匹配等生成式策略提供了更具表达力的替代方案，但将其整合到最大熵强化学习中面临两大挑战：连续时间生成式策略的似然和熵通常难以计算，多步采样会导致长时域反向传播不稳定和推理延迟。

Method: 提出截断整流流策略（TRFP），采用混合确定性-随机架构。通过梯度截断和流拉直技术，使熵正则化优化变得可处理，同时支持稳定训练和有效的一步采样。

Result: 在多目标玩具环境和10个MuJoCo基准测试中，TRFP能有效捕捉多模态行为，在标准采样下优于大多数基线方法，在一步采样下仍保持高度竞争力。

Conclusion: TRFP框架成功解决了生成式策略在最大熵强化学习中的关键挑战，实现了多模态行为建模和高效推理的平衡，为序列决策提供了更强大的策略表示方法。
...
### [150] [Automated Batch Distillation Process Simulation for a Large Hybrid Dataset for Deep Anomaly Detection](https://arxiv.org/abs/2604.09166)
*Jennifer Werner,Justus Arweiler,Indra Jungjohann,Jochen Schmid,Fabian Jirasek,Hans Hasse,Michael Bortz*

Main category: cs.LG

TL;DR: 本文提出了一种用于化工过程异常检测的混合数据集，结合了实验数据和模拟数据，通过自动化工作流程生成大规模、标注良好的训练数据。
...
Method: 1. 扩展已有的实验数据集，增加相应的模拟数据集，创建混合数据集；2. 开发基于Python的过程模拟器，采用定制化的指标缩减策略处理微分代数方程；3. 利用实验数据库的丰富元数据和结构化异常标注，自动将实验记录转换为模拟场景；4. 通过单一参考实验校准后，准确预测其他实验的动态特性；5. 自动化生成大量实验运行的时间序列数据，涵盖正常操作和多种执行器与控制相关异常。

Result: 1. 成功创建并公开发布了混合数据集；2. 模拟器经过校准后能够准确预测实验动态；3. 实现了大规模实验活动的自动化、一致性模拟；4. 为模拟到实验的风格迁移、伪实验数据生成以及深度异常检测方法研究提供了独特基础。

Conclusion: 从过程模拟角度看，本研究展示了大规模实验活动的自动化、一致性模拟方法；从数据驱动的异常检测角度看，混合数据集为模拟到实验的风格迁移、伪实验数据生成以及化工过程监测中深度异常检测方法的未来研究提供了宝贵资源。
...
### [151] [The causal relation between off-street parking and electric vehicle adoption in Scotland](https://arxiv.org/abs/2604.09271)
*Bernardino D'Amico,Achille Fonzone,Emma Hart*

Main category: cs.LG

TL;DR: 研究揭示电动汽车普及存在结构性障碍：私人停车位是催化因素而非根本约束，家庭收入才是真正的准入门槛。传统观测模型高估了停车位的作用，实际是收入差异导致的偏差。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探究电动汽车普及中的"充电鸿沟"——拥有与没有私人停车位家庭间的差异，是真实的基础设施约束还是社会经济不平等的副产品，以指导更公平有效的政策制定。

Method: 采用概率因果框架分析苏格兰全国代表性家庭数据集，超越传统预测模型，通过明确中和其他因果因素的混淆效应，估计政策干预效果。

Result: 发现电动汽车普及存在结构性层级：私人停车位是转化催化剂（使拥有概率从3.3%增至5.6%），但主要加速已有经济能力购买的家庭；家庭收入才是根本的负担能力天花板，低收入与高收入群体间市场非参与率差异达23.1个百分点。

Conclusion: 传统观测模型高估了私人停车位的独立效应，实际是选择偏差的结果。建议双轨政策策略：通过金融工具降低非参与者的负担能力门槛，同时为高密度城市环境中"潜在意向"群体解决家庭充电接入问题。
...
### [152] [Distributed Online Convex Optimization with Compressed Communication: Optimal Regret and Applications](https://arxiv.org/abs/2604.09276)
*Sifan Yang,Dan-Yue Li,Lijun Zhang*

Main category: cs.LG

TL;DR: 本文研究了分布式在线凸优化中的压缩通信问题，建立了压缩比δ对收敛性能的理论下界，并提出了一种结合误差反馈机制和在线压缩策略的最优算法。
...
Result: 建立了凸函数和强凸函数的理论下界分别为Ω(δ^{-1/2}√T)和Ω(δ^{-1}log T)。提出的算法达到了最优的遗憾界：凸函数为O(δ^{-1/2}√T)，强凸函数为O(δ^{-1}log T)。

Conclusion: 该研究首次为具有压缩通信和域约束的分布式非光滑优化提供了理论保证，提出的方法具有很好的泛化性，可以通过在线到批处理的转换扩展到离线随机设置。
...
### [153] [Are Independently Estimated View Uncertainties Comparable? Unified Routing for Trusted Multi-View Classification](https://arxiv.org/abs/2604.09288)
*Yilin Zhang,Cai Xu,Haishun Chen,Ziyu Guan,Wei Zhao*

Main category: cs.LG

TL;DR: 论文提出TMUR方法解决多视图分类中证据尺度不一致问题，通过统一路由机制分离视图特定证据提取与融合仲裁，使用全局上下文生成专家权重，提高可靠性评估。


<details>
  <summary>Details</summary>
Motivation: 传统可信多视图分类假设不同视图的证据在数值上可比，但实际上不同视图在特征空间、噪声水平和语义粒度上存在差异，独立训练的分支仅优化预测准确性，缺乏跨视图证据强度一致性约束，导致融合时的不确定性可能被分支特定的尺度偏差主导而非真实的样本级可靠性。

Method: 提出TMUR方法，将视图特定证据提取与融合仲裁解耦。使用视图私有专家和协作专家，通过统一路由器观察全局多视图上下文生成样本级专家权重。采用软负载平衡和多样性正则化促进专家平衡利用和更具区分性的专家专业化。

Result: 论文提供了理论分析，说明为什么独立的证据监督无法识别跨视图的共同证据尺度，以及当可靠性依赖于样本时，为什么统一的全局路由优于分支本地仲裁。

Conclusion: TMUR通过解耦视图特定证据提取与融合仲裁，使用全局上下文路由机制，解决了多视图分类中证据尺度不一致问题，提高了可靠性评估的准确性。
...
### [154] [Stability Enhanced Gaussian Process Variational Autoencoders](https://arxiv.org/abs/2604.09331)
*Carl R. Richardson,Jichen Zhang,Ethan King,Ján Drgoňa*

Main category: cs.LG

TL;DR: 提出SEGP-VAE模型，使用高维视频数据间接训练低维线性时不变系统，通过稳定性增强的高斯过程变分自编码器实现


<details>
  <summary>Details</summary>
Motivation: 解决使用高维视频数据间接训练低维线性时不变系统的问题，同时确保系统稳定性并避免数值问题

Method: 提出稳定性增强的高斯过程变分自编码器，其先验的均值和协方差函数从LTI系统定义推导，将LTI参数搜索空间限制在半收缩系统集合，使用完全无约束参数化

Result: 模型可以使用无约束优化算法训练，避免了非Hurwitz状态矩阵引起的数值问题，在螺旋粒子视频数据集上实现了准确的潜状态预测

Conclusion: SEGP-VAE成功结合了概率模型和可解释的物理模型，能够从高维观测中有效学习稳定的低维线性系统
...
### [155] [Hierarchical Flow Decomposition for Turning Movement Prediction at Signalized Intersections](https://arxiv.org/abs/2604.09336)
*Md Atiqur Rahman Mallick,Kamrul Hasan,Pulock Das,Liang Hong,S M Shazzad Rassel*

Main category: cs.LG

TL;DR: HFD-TM是一个用于交叉口转向运动预测的分层深度学习框架，通过先预测走廊直行流量再扩展到转向流，显著提高了预测精度并降低了计算时间。


<details>
  <summary>Details</summary>
Motivation: 交叉口转向运动预测对自适应信号控制至关重要，但由于转向流的高波动性而难以准确预测。研究发现走廊直行流量占总流量的65.1%，波动性较低，并能解释35.5%的转向运动方差，这为分层预测提供了理论基础。

Method: 提出HFD-TM分层深度学习框架：首先预测走廊直行流量，然后将这些预测扩展到各个转向流。采用物理信息损失函数来强制执行流量守恒，保持结构一致性。

Result: 在纳什维尔六交叉口走廊的六个月LiDAR数据上评估，HFD-TM达到每15分钟间隔2.49辆车的平均绝对误差，比Transformer降低5.7%，比GRU降低27.0%。分层分解带来最大性能提升，训练时间比DCRNN低12.8倍。
...
### [156] [ECHO: Efficient Chest X-ray Report Generation with One-step Block Diffusion](https://arxiv.org/abs/2604.09450)
*Lifeng Chen,Tianqi You,Hao Liu,Zhimin Bao,Jile Jiao,Xiao Han,Zhicai Ou,Tao Sun,Xiaofeng Mou,Xiaojie Jin,Yi Xu*

Main category: cs.LG

TL;DR: ECHO是一种用于胸部X光报告生成的高效扩散模型，通过直接条件蒸馏框架实现单步推理，比自回归模型快8倍，同时提升临床准确性指标。


<details>
  <summary>Details</summary>
Motivation: 传统自回归视觉语言模型推理延迟高，扩散模型需要多次去噪迭代。压缩多步去噪为单步可进一步降低延迟，但现有方法因token分解去噪器引入均值场偏差而降低文本连贯性。

Method: 提出ECHO扩散视觉语言模型，采用直接条件蒸馏(DCD)框架构建非分解监督来编码联合token依赖，缓解均值场限制；引入响应非对称扩散(RAD)训练策略提高训练效率。

Result: ECHO超越最先进的自回归方法，RaTE和SemScore分别提升64.33%和60.58%，同时实现8倍推理加速且不损害临床准确性。

Conclusion: ECHO通过创新的DCD框架和RAD训练策略，成功解决了扩散模型单步推理时的文本连贯性问题，为医学影像报告生成提供了高效准确的解决方案。
...
### [157] [Bringing Clustering to MLL: Weakly-Supervised Clustering for Partial Multi-Label Learning](https://arxiv.org/abs/2604.09359)
*Yu Chen,Weijun Lv,Yue Huang,Xuhuan Zhu,Fang Li*

Main category: cs.LG

TL;DR: 本文提出了一种用于部分多标签学习的新型弱监督聚类方法WSC-PML，通过成员矩阵分解将聚类与多标签学习相结合，有效处理标签噪声问题。


<details>
  <summary>Details</summary>
Motivation: 多标签学习中的标签噪声问题，特别是在部分多标签学习中，候选标签同时包含相关和不相关标签，给模型训练带来重大挑战。传统聚类方法由于聚类成员值总和为1而多标签分配需要二进制值且总和可为任意数，存在根本性不兼容，无法直接应用于多标签场景。

Method: 提出WSC-PML方法，通过成员矩阵分解将聚类成员矩阵A分解为两个组件：A = Π ⊙ F，其中Π保持聚类约束，F保留多标签特性。采用三阶段流程：从噪声标签中学习初始原型、构建自适应置信度弱监督、通过迭代聚类细化进行联合优化。

Result: 在24个数据集上的广泛实验表明，该方法在所有评估指标上都优于六种最先进的方法。

Conclusion: 提出的弱监督聚类方法WSC-PML通过创新的成员矩阵分解，成功弥合了聚类与多标签学习之间的差距，为部分多标签学习中的标签噪声处理提供了有效解决方案。
...
### [158] [SafeAdapt: Provably Safe Policy Updates in Deep Reinforcement Learning](https://arxiv.org/abs/2604.09452)
...
<details>
  <summary>Details</summary>
Motivation: 在安全关键任务中部署强化学习智能体需要安全保证，但实际部署环境往往具有非平稳动态或性能目标变化，需要更新策略。现有方法要么不提供形式化保证，要么只在后验验证策略安全性，缺乏先验安全保证。

Method: 引入Rashomon集合：在策略参数空间中经过认证满足演示数据分布内安全约束的区域。通过将任意RL算法的更新投影到Rashomon集合上，为策略更新提供形式化、可证明的保证。

Result: 在网格世界导航环境（Frozen Lake和Poisoned Apple）中验证了该方法，能够在下游适应过程中保证源任务上的先验可证明确定性安全。相比正则化基线方法会灾难性遗忘安全约束，该方法实现了强适应性的同时保持安全保证。

Conclusion: 提出的Rashomon集合方法为持续强化学习中的安全策略更新提供了先验形式化保证，解决了现有方法缺乏先验安全保证的问题，在保持适应性的同时确保安全性。
...
### [159] [Is More Data Worth the Cost? Dataset Scaling Laws in a Tiny Attention-Only Decoder](https://arxiv.org/abs/2604.09389)
*Götz-Henrik Wiegand,Lorena Raichle,Rico Städeli,Tomas Hrycej,Bernhard Bermeitinger,Siegfried Handschuh*

Main category: cs.LG

TL;DR: 在小规模可控设置下研究Transformer语言模型的数据集规模效应，发现仅使用约30%的训练数据即可达到约90%的完整数据验证准确率，呈现明显的收益递减规律。


<details>
  <summary>Details</summary>
Motivation: Transformer语言模型训练成本高昂，虽然大规模下的缩放规律已被描述，但在可控的小规模设置中其影响仍较少被探索。本研究旨在隔离数据集规模效应，为计算和数据受限环境提供实用指导。

Method: 使用简化版的仅注意力解码器架构，通过在逐渐增大的2的幂次方子集上进行训练，观察性能变化，以隔离数据集规模的影响。

Result: 观察到平滑的性能改进伴随明显的收益递减，与缩放规律行为一致。仅使用约30%的训练数据即可达到约90%的完整数据验证token级准确率。

Conclusion: 研究结果为可控、组件隔离设置下的数据集缩放提供了可操作的见解，为计算和数据受限环境（如小型研究实验室和探索性模型开发）中平衡数据集规模和计算成本提供了实用指导。
...
### [160] [Efficient Unlearning through Maximizing Relearning Convergence Delay](https://arxiv.org/abs/2604.09391)
*Khoa Tran,Simon S. Woo*

Main category: cs.LG

TL;DR: 本文提出了一种新的机器遗忘评估指标——重学习收敛延迟，以及基于该指标的影响消除遗忘框架，用于更全面地评估和实现模型对特定数据的遗忘效果。


<details>
  <summary>Details</summary>
Motivation: 当前机器遗忘方法和评估指标仅关注模型预测层面，无法深入了解模型对遗忘数据的真实理解程度，这限制了评估模型遗忘效果的能力。

Method: 提出重学习收敛延迟指标，同时捕捉权重空间和预测空间的变化；基于此提出影响消除遗忘框架，通过降低遗忘集性能、结合权重衰减和权重噪声注入来实现遗忘。

Result: 实验表明该方法优于现有指标和评估方法，接近理想遗忘性能；提供了理论保证（指数收敛和上界）以及在分类和生成遗忘任务中的强保留和抗重学习能力。

Conclusion: 重学习收敛延迟指标提供了更全面的遗忘评估，影响消除遗忘框架能有效移除遗忘集的影响，同时保持保留集的准确性，为机器遗忘提供了更可靠的解决方案。
...
### [161] [OASIS: Online Activation Subspace Learning for Memory-Efficient Training](https://arxiv.org/abs/2604.09406)
...
<details>
  <summary>Details</summary>
Motivation: 训练大型语言模型受内存限制，其中激活值占内存的很大一部分。现有方法主要关注权重参数化或梯度子空间，而激活内存通常通过架构修改或基于周期性更新的压缩方案处理。

Method: 提出OASIS算法，在训练过程中动态跟踪和更新低维激活子空间，将中间激活投影到这个演化子空间上，减少内存而不修改前向计算。演化激活子空间诱导低秩梯度表示，使梯度和优化器状态能直接在该子空间中维护，同时使用投影感知优化器在子空间更新间稳定传输优化器状态。

Result: 在各种微调和预训练任务中，OASIS实现了比完整微调低2倍的峰值内存，同时匹配其性能并优于先前的低秩方法。
...
### [162] [Offline Local Search for Online Stochastic Bandits](https://arxiv.org/abs/2604.09423)
*Gerdus Benadè,Rathish Das,Thomas Lavastida*

Main category: cs.LG

TL;DR: 该论文提出了一种将离线局部搜索算法转换为在线随机组合多臂老虎机算法的新框架，实现了O(log³ T)的近似遗憾，优于现有多项式依赖T的方法。


<details>
  <summary>Details</summary>
Motivation: 组合多臂老虎机是重要的在线决策环境，现有研究主要关注利用离线算法设计知识来改进在线性能。虽然离线贪心和线性优化算法已被成功应用于在线设置，但局部搜索这类在理论和实践中广泛使用的算法尚未得到充分探索。研究者希望填补这一空白，为离线局部搜索算法提供有效的在线转换方法。

Method: 提出了一种通用框架，将离线局部搜索算法转换为在线随机组合老虎机算法。该框架适用于那些离线局部搜索能够收敛到近似最优解的问题。通过精心设计的在线学习机制，实现了对数级别的遗憾界。

Result: 新框架实现了O(log³ T)的近似遗憾，相比现有离线到在线转换框架产生的多项式依赖T的遗憾（包括近似遗憾），取得了显著改进。该框架具有很好的灵活性，成功应用于三个在线随机组合优化问题：最小化总完成时间的调度、寻找拟阵的最小成本基以及不确定聚类问题。

Conclusion: 局部搜索算法可以有效地从离线设置转换到在线随机组合老虎机环境，实现对数级别的近似遗憾。该框架为利用丰富的离线局部搜索算法解决在线组合优化问题提供了新的途径，在理论和实际应用中都很有价值。
...
### [163] [AdaCubic: An Adaptive Cubic Regularization Optimizer for Deep Learning](https://arxiv.org/abs/2604.09437)
*Ioannis Tsingalis,Constantine Kotropoulos,Corentin Briat*

Main category: cs.LG

TL;DR: AdaCubic是一种新颖的正则化技术，通过辅助优化问题动态调整三次项权重，结合Hutchinson方法近似Hessian矩阵降低计算成本，在深度学习应用中表现出色且无需超参数调优。


<details>
  <summary>Details</summary>
Motivation: 现有自适应优化算法通常需要大量超参数调优，而三次正则化方法在深度学习中的可扩展应用尚未充分探索。研究者希望开发一种既保持三次正则化牛顿方法收敛保证，又能在实际深度学习任务中高效运行且无需精细调优的优化器。

Method: 提出AdaCubic正则化技术，核心是通过带有三次约束的辅助优化问题动态调整三次项权重。采用Hutchinson方法近似Hessian矩阵以降低计算成本，同时继承了三次正则化牛顿方法的局部收敛保证。

Result: 在计算机视觉、自然语言处理和信号处理任务上的实验表明，AdaCubic优于或与多种广泛使用的优化器竞争。更重要的是，AdaCubic使用固定超参数集进行评估，无需精细调优，在无法进行超参数调优的场景中具有显著优势。

Conclusion: AdaCubic是首个在可扩展深度学习应用中利用三次正则化的优化器，结合了理论保证和实际性能优势，为研究者和从业者提供了无需超参数调优的高效优化选择。
...
### [164] [Integrated electro-optic attention nonlinearities for transformers](https://arxiv.org/abs/2604.09512)
*Luis Mickeler,Kai Lion,Alfonso Nardi,Jost Kellner,Pierre Didier,Bhavin J. Shastri,Niao He,Rachel Grange*

Main category: cs.LG

TL;DR: 使用薄膜铌酸锂马赫-曾德尔调制器作为模拟非线性计算单元，显著降低Transformer中Softmax等非线性操作的延迟，在保持高精度的同时实现高速低功耗计算。


<details>
  <summary>Details</summary>
Motivation: Transformer中的Softmax等非线性操作虽然只占总计算量的不到1%，但可能成为推理延迟的瓶颈。传统数字计算方式存在延迟和功耗问题，需要更高效的硬件实现方案。

Method: 采用薄膜铌酸锂马赫-曾德尔调制器作为模拟非线性计算单元，实现电光替代方案来执行Softmax和Sigmoid函数。在Vision Transformer和大型语言模型中进行评估，并在高达10GBaud的编码速度下表征系统噪声。

Result: 系统在保持高度竞争力的准确率的同时，即使在模拟单元采用激进的4位输入输出量化下也能保持性能。系统在高达10GBaud的编码速度下表现出良好的噪声特性，模型在各种噪声条件下保持鲁棒性。

Conclusion: 薄膜铌酸锂调制器可以作为混合封装硬件中的非线性函数单元，实现高速且能量高效的非线性计算，为Transformer模型的硬件加速提供了有前景的解决方案。
...
### [165] [Toward World Models for Epidemiology](https://arxiv.org/abs/2604.09519)
*Zeeshan Memon,Yiqi Su,Christo Kurisummoottil Thomas,Walid Saad,Liang Zhao,Naren Ramakrishnan*

Main category: cs.LG

TL;DR: 该论文提出流行病学是世界模型的天然应用场景，建立了流行病学世界模型的概念框架，并通过三个案例研究展示了其在政策制定中的必要性。


<details>
  <summary>Details</summary>
Motivation: 世界模型已成为学习潜在动态、模拟反事实未来和支持不确定性下规划的统一范式。作者认为计算流行病学是世界模型的天然且未充分开发的应用场景，因为流行病决策需要考虑潜在疾病负担、不完美且依赖政策的监测信号，以及干预效果受适应性人类行为调节。

Method: 引入流行病学世界模型的概念框架，将流行病建模为受控的部分可观测动态系统，其中：(i)真实流行病状态是潜在的，(ii)观测是噪声的且内生于政策，(iii)干预作为序列行动，其效果通过行为和社会反馈传播。通过三个案例研究说明世界模型在政策相关推理中的必要性。

Result: 提出了一个将流行病学问题形式化为世界模型的具体框架，展示了该框架如何捕捉流行病决策中的关键挑战：行为监测中的策略性误报、住院和死亡等时间滞后信号中的系统性延迟，以及相同历史在不同干预序列下产生分歧的反事实干预分析。

Conclusion: 流行病学是世界模型的理想应用领域，因为其固有的潜在状态、政策依赖的观测和通过行为反馈传播的干预效果。明确的世界建模对于政策相关的推理至关重要，特别是在处理监测偏差、时间滞后信号和反事实干预分析时。
...
### [166] [ANTIC: Adaptive Neural Temporal In-situ Compressor](https://arxiv.org/abs/2604.09543)
*Sandeep S. Cranganore,Andrei Bodnar,Gianluca Galleti,Fabian Paischer,Johannes Brandstetter*

Main category: cs.LG

TL;DR: ANTIC是一种端到端原位压缩流水线，通过自适应时间选择器和空间神经压缩模块，对高维物理场进行时空压缩，显著减少存储需求


<details>
  <summary>Details</summary>
Motivation: 高分辨率、时空演化场的大规模高维偏微分方程模拟产生PB到EB级数据，对现代高性能计算基础设施造成存储瓶颈

Method: ANTIC包含：1）自适应时间选择器，在模拟时识别和过滤信息丰富的快照；2）基于持续微调的空间神经压缩模块，使用神经场学习相邻快照间的残差更新

Result: 实验结果表明，该方法实现了数个数量级的存储减少，同时保持物理精度

Conclusion: ANTIC通过单次流式处理实现时空组件联合压缩，有效缓解了显式存储整个时间演化轨迹的需求
...
### [167] [RAMP: Hybrid DRL for Online Learning of Numeric Action Models](https://arxiv.org/abs/2604.08685)
...
Method: 提出RAMP策略：1）同时训练深度强化学习策略；2）从过去的交互中学习数值动作模型；3）使用该模型在可能时规划未来动作。开发了Numeric PDDLGym框架，将数值规划问题转换为Gym环境。

Result: 在标准IPC数值领域的实验结果显示，RAMP在可解性和规划质量方面显著优于PPO（著名的深度强化学习算法）。

Conclusion: RAMP策略通过在线学习数值动作模型，成功整合了强化学习和数值规划，形成了有效的正向反馈循环，为自动化规划提供了新的解决方案。
...
### [168] [Parameterized Complexity Of Representing Models Of MSO Formulas](https://arxiv.org/abs/2604.08707)
...
<details>
  <summary>Details</summary>
Motivation: Courcelle定理在参数化复杂度中很重要，但主要关注MSO2公式的可满足性检查。本研究旨在扩展该定理，探索MSO2公式模型（包含自由变量）的紧凑表示问题，将Courcelle定理与知识表示领域连接起来。

Method: 研究展示了MSO2公式模型可以用决策图表示，其大小相对于树宽和公式大小是参数化线性的。具体证明了：对于树宽，模型可以用参数化线性大小的SDD表示；对于路径宽，可以用参数化线性大小的OBDD表示。同时基于Razgon(2014)的下界结果，证明了存在某些有界树宽图类不允许树宽参数化的OBDD表示。

Result: 1. 证明了MSO2公式模型可以用参数化线性大小的决策图表示；2. 对于树宽，SDD大小是参数化线性的；3. 对于路径宽，OBDD大小是参数化线性的；4. 证明了存在有界树宽图类不允许树宽参数化的OBDD表示。

Conclusion: 该研究成功扩展了Courcelle定理，不仅证明了MSO2公式模型可以用紧凑的决策图表示，还建立了参数化复杂度与知识表示领域之间的新联系，为MSO2公式的模型表示提供了新的理论框架。
...
### [169] [Artifacts as Memory Beyond the Agent Boundary](https://arxiv.org/abs/2604.08756)
*John D. Martin,Fraser Mince,Esra'a Saleh,Amy Pajak*

Main category: cs.AI

TL;DR: 该研究将认知的具身观点形式化到强化学习中，证明环境可以作为智能体的外部记忆，某些观察（称为"人工制品"）可以减少表示历史所需的信息量。


<details>
  <summary>Details</summary>
Motivation: 认知的具身观点认为智能行为不仅依赖于内部记忆，还依赖于智能体对环境资源的主动利用。本研究旨在将这一直觉形式化到强化学习框架中，探索环境如何功能性地作为智能体的记忆。

Method: 引入数学框架来形式化环境如何作为智能体的记忆，定义"人工制品"概念，并证明这些观察可以减少表示历史所需的信息。通过实验验证理论，展示当智能体观察空间路径时，学习高性能策略所需的内存会减少。

Result: 实验证实，当智能体观察空间路径时，学习高性能策略所需的内存确实减少了。有趣的是，这种效果是无意中产生的，通过智能体的感知流隐式实现。研究还表明这些发现满足先前用于解释外部记忆的定性特性。

Conclusion: 环境可以作为智能体的外部记忆，某些观察（人工制品）可以减少表示历史所需的信息。这一发现为利用环境作为显式内部记忆的替代品提供了原则性方法，未来研究可进一步探索如何系统性地利用环境资源。
...
### [170] [SPPO: Sequence-Level PPO for Long-Horizon Reasoning Tasks](https://arxiv.org/abs/2604.08865)
*Tianyi Wang,Yixia Li,Long Li,Yibiao Chen,Shaohan Huang,Yun Chen,Peng Li,Yang Liu,Guanhua Chen*

Main category: cs.AI

TL;DR: SPPO是一种序列级PPO算法，通过将推理过程重新定义为序列级上下文赌博机问题，使用解耦的标量值函数获得低方差优势信号，解决了标准PPO在长链思维推理任务中的不稳定性和内存成本问题。


<details>
  <summary>Details</summary>
Motivation: 标准token级PPO在具有可验证奖励的LLM推理任务中存在两个主要问题：1）长链思维时间跨度上时间信用分配的不稳定性；2）价值模型的内存成本过高。虽然GRPO等无评论家方法缓解了这些问题，但需要多个样本进行基线估计，导致计算开销大，严重限制了训练吞吐量。

Method: SPPO将推理过程重新定义为序列级上下文赌博机问题，采用解耦的标量值函数来推导低方差优势信号，无需多重采样。这种方法结合了PPO的样本效率和基于结果更新的稳定性。

Result: 在数学基准测试上的广泛实验表明，SPPO显著超越了标准PPO，并且与计算密集的基于组的方法性能相当，为对齐推理LLM提供了一个资源高效的框架。

Conclusion: SPPO提供了一个可扩展的算法，在保持PPO样本效率的同时，通过序列级更新解决了长链思维推理中的稳定性和内存问题，为对齐推理LLM提供了资源高效的解决方案。
...
### [171] [StaRPO: Stability-Augmented Reinforcement Policy Optimization](https://arxiv.org/abs/2604.08905)
*Jinghan Zhang,Fengran Mo,Tharindu Cyril Weerasooriya,Ruimin Dai,Xiaoyan Han,Yanjie Fu,Dakuo Wang,Kunpeng Liu*

Main category: cs.AI

TL;DR: StaRPO是一个增强推理稳定性的强化学习框架，通过自相关函数和路径效率两个轻量级指标评估推理过程的局部连贯性和全局目标导向性，提升大语言模型在复杂推理任务中的逻辑一致性和准确性。
...
Method: 提出StaRPO框架，将推理稳定性分解为两个可计算的轻量级指标：评估局部步骤间连贯性的自相关函数（ACF）和评估全局目标导向性的路径效率（PE），将这些稳定性奖励与任务奖励结合提供过程感知反馈。

Result: 在两个骨干模型上验证了ACF和PE奖励与逻辑错误的相关性；在四个推理基准测试中，StaRPO持续优于基线方法，能够同时提升最终答案准确性和逻辑稳定性。

Conclusion: StaRPO通过将推理稳定性显式纳入优化目标，有效解决了现有RL框架忽视推理过程逻辑结构的问题，为大语言模型的复杂推理任务提供了更全面的优化框架。
...
### [172] [Enhancing LLM Problem Solving via Tutor-Student Multi-Agent Interaction](https://arxiv.org/abs/2604.08931)
*Nurullah Eymen Özdemir,Erhan Oztop*

Main category: cs.AI

TL;DR: PETITE框架通过导师-学生多智能体系统，让同一LLM扮演不同角色进行结构化交互，在代码生成任务上以更少的token消耗达到或超越现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 受人类认知发展中结构化社会互动（如导师-学习者角色交换）的启发，探索是否可以通过导师-学生多智能体系统创造协同效应，让LLM超越现有框架的能力，而不依赖更强的监督模型或异构集成。

Method: 提出PETITE框架，在自主编码问题领域实例化两个来自同一LLM的智能体：学生智能体生成并迭代优化解决方案，导师智能体提供结构化评估反馈（无真实答案访问权限），通过互补角色结构实现协同。

Result: 在APPS编码基准测试中，PETITE相比Self-Consistency、Self-Refine、Multi-Agent Debate和Multi-Agent Review等最先进方法，达到相似或更高的准确率，同时显著减少token消耗。

Conclusion: 基于发展心理学的角色差异化交互结构为增强LLM问题解决能力提供了原则性和资源高效的范式，通过结构化同伴式交互实现性能提升。
...
### [173] [SEA-Eval: A Benchmark for Evaluating Self-Evolving Agents Beyond Episodic Assessment](https://arxiv.org/abs/2604.08988)
...
TL;DR: 该论文提出了SEA-Eval基准测试，用于评估自进化智能体在跨任务连续演化中的性能，发现当前最先进框架存在显著的进化瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体在单次任务执行中表现良好，但受限于静态工具集和任务间记忆缺失，无法积累经验或跨任务优化策略。需要建立科学评估框架来推动智能体从单纯任务执行器向真正自进化数字实体的发展。

Method: 1) 提出了基于数字具身和跨任务连续演化的自进化智能体(SEA)形式化定义；2) 引入了SEA-Eval基准测试，从两个维度评估SEA特性：任务内执行可靠性和长期进化性能；3) 将任务组织成顺序流，分析随时间变化的成功率和令牌消耗，量化进化增益和结构稳定性。

Result: 实证评估揭示了当前最先进框架存在显著的进化瓶颈：相同的成功率下，令牌消耗差异可达31.2倍，顺序分析显示不同的进化轨迹。现有单次基准测试无法捕捉这些差异。

Conclusion: SEA-Eval为推进智能体从单纯任务执行器向真正自进化数字实体提供了严格的科学基础，能够量化现有单次基准测试无法评估的进化增益和结构稳定性。
...
### [174] [Advantage-Guided Diffusion for Model-Based Reinforcement Learning](https://arxiv.org/abs/2604.09035)
*Daniele Foffano,Arvid Eriksson,David Broman,Karl H. Johansson,Alexandre Proutiere*

Main category: cs.AI

TL;DR: 本文提出AGD-MBRL方法，通过优势估计引导扩散世界模型，解决传统扩散引导方法在短视界下的局限性，提高模型强化学习的样本效率和最终回报。


<details>
  <summary>Details</summary>
Motivation: 基于模型的强化学习中，自回归世界模型存在误差累积问题，而扩散世界模型虽然能缓解这个问题，但现有的扩散引导方法要么只使用策略信息（丢弃价值信息），要么基于奖励（在短扩散视界下变得短视）。需要一种能够考虑长期回报的引导方法。

Method: 提出优势引导扩散方法（AGD-MBRL），使用智能体的优势估计来引导反向扩散过程，使采样集中在预期能获得更高长期回报的轨迹上。开发了两种引导方法：Sigmoid优势引导（SAG）和指数优势引导（EAG）。该方法与PolyGRAD架构无缝集成，引导状态组件同时保持动作生成受策略条件约束。

Result: 在MuJoCo控制任务（HalfCheetah、Hopper、Walker2D和Reacher）上，AGD-MBRL相比PolyGRAD、在线Diffuser风格的奖励引导以及无模型基线（PPO/TRPO），提高了样本效率和最终回报，在某些情况下提升幅度达到2倍。

Conclusion: 优势感知引导是解决扩散模型强化学习中短视界问题的简单有效方法，能够显著提升性能，且无需改变扩散训练目标。
...
### [175] [Overhang Tower: Resource-Rational Adaptation in Sequential Physical Planning](https://arxiv.org/abs/2604.09072)
*Ruihong Shen,Shiqian Li,Yixin Zhu*

Main category: cs.AI

TL;DR: 人类在认知资源受限时，物理预测机制和规划策略会同时发生双重转变：从基于直觉物理引擎的模拟转向基于CNN的视觉启发式，同时从深思熟虑的前瞻规划转向浅层视野规划。


<details>
  <summary>Details</summary>
Motivation: 研究人类如何在认知资源受限的情况下进行序列物理规划，解决物理预测机制（直觉物理引擎vs启发式）和规划策略（深思熟虑vs短视）这两个长期分离的争议，揭示认知架构如何适应资源约束。

Method: 使用"悬挑塔"建造任务，要求参与者在保持稳定性的同时最大化水平悬挑。通过操纵任务复杂性和时间压力，观察物理预测机制和规划策略的变化。

Result: 发现双重转变：早期阶段直觉物理引擎模拟占主导，复杂性增加时CNN视觉启发式占优势；时间压力缩短深思熟虑的前瞻规划，转向更浅的规划视野。这种双重转变是先前单一机制理论无法预测的。

Conclusion: 人类具有层次化的资源理性认知架构，能够灵活地在计算成本和预测保真度之间进行权衡。研究结果将两个长期争议（模拟vs启发式、短视vs深思熟虑规划）统一为受认知预算动态重构的机制库。
...
### [176] [Camera Artist: A Multi-Agent Framework for Cinematic Language Storytelling Video Generation](https://arxiv.org/abs/2604.09195)
*Haobo Hu,Qi Mao,Yuanhang Li,Libiao Jin*

Main category: cs.AI

TL;DR: Camera Artist是一个多智能体框架，通过建模真实电影制作流程来生成具有明确电影语言的叙事视频，解决了现有系统叙事不连贯和电影质量有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统虽然能从剧本自动生成视频，但缺乏明确的机制来构建相邻镜头间的叙事连续性，且对电影语言的运用不够深入，导致故事碎片化和电影质量有限。

Method: 基于现有智能体流程，引入专门的Cinematography Shot Agent，通过递归故事板生成增强镜头间的叙事连续性，并通过电影语言注入产生更具表现力的电影导向镜头设计。
...
### [177] [Mind the Gap Between Spatial Reasoning and Acting! Step-by-Step Evaluation of Agents With Spatial-Gym](https://arxiv.org/abs/2604.09338)
*Lars Benedikt Kaesberg,Tianyu Yang,Niklas Bauer,Terry Ruas,Jan Philip Wahle,Bela Gipp*

Main category: cs.AI

TL;DR: Spatial-Gym：一个用于评估AI模型空间推理能力的交互式基准测试环境，通过2D网格路径寻找任务测试模型在三种设置下的表现，发现当前模型与人类表现存在巨大差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在一次性设置中评估模型，这与人类在交互环境中逐步解决问题的过程不同。需要开发一个能隔离空间约束推理、模拟人类逐步决策过程的评估环境。

Method: 引入Spatial-Gym，一个Gymnasium环境，将2D网格谜题中的路径寻找作为顺序决策任务进行测试，支持可选回溯。在500个episode上评估8个模型在三种设置（一次性、逐步、带回溯逐步）下的表现，并与人类、随机和A*基准进行比较。

Result: 最佳模型GPT-OSS 120B解决率为16.0%，比人类基准（98.0%）低82个百分点。逐步格式通过消除格式错误帮助较弱模型（提升达5.4%），但通过限制全局规划损害较强模型（下降达5.6%）。回溯提高episode完成率，但仅对较弱模型提高解决率；较强模型很少回溯且未从中受益。

Conclusion: 模型未能随难度增加推理努力；接收空间环境图像的视觉模型解决率降低73%；扩展思维链推理在逐步设置中仍保持3-5倍准确率优势。Spatial-Gym能够诊断模型局限性，并为通过强化学习改进空间推理提供框架。
...
### [178] [Do We Really Need to Approach the Entire Pareto Front in Many-Objective Bayesian Optimisation?](https://arxiv.org/abs/2604.09417)
*Chao Jiang,Jingyu Huang,Miqing Li*

Main category: cs.AI

TL;DR: SPMO框架专注于在有限评估预算下寻找单个高质量解，而非近似整个帕累托前沿，通过ESPI采集函数在噪声和无噪声场景下工作，经理论和实验验证优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在多目标优化中，当目标数量增加时，需要大量解来近似整个帕累托前沿，这在贝叶斯优化中尤其困难，因为评估预算有限。决策者最终只需选择一个解部署，因此在有限评估下，专注于寻找单个高质量解比近似整个前沿更有用。

Method: 提出SPMO（单点多目标搜索框架），专注于沿目标间良好权衡方向提高解的质量。提出ESPI（期望单点改进）采集函数，适用于噪声和无噪声场景，可通过样本平均近似和梯度方法有效优化。

Result: 理论证明了ESPI在SAA下的收敛保证，实验表明SPMO在计算上可行，并在广泛的基准测试和实际问题中优于现有最先进方法。
...
### [179] [E3-TIR: Enhanced Experience Exploitation for Tool-Integrated Reasoning](https://arxiv.org/abs/2604.09455)
*Weiyang Guo,Zesheng Shi,Liye Zhao,Jiayuan Ma,Zeen Zhu,Junxian He,Min Zhang,Jing Li*

Main category: cs.AI

TL;DR: E3-TIR是一种针对工具集成推理的预热训练范式，通过动态整合专家前缀、专家引导和自主探索三种经验类型，有效解决了传统训练方法中的探索效率低、数据成本高和性能瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM工具集成推理训练范式存在显著局限：Zero-RL因缺乏先验指导导致探索效率低下和模式退化，SFT-then-RL则受限于高数据成本和低熵崩溃导致的能力瓶颈。需要一种能平衡探索多样性与训练效率的新方法。

Method: 提出E3-TIR预热训练范式，将训练过程动态整合为三种经验类型：专家前缀、专家引导和自主探索。通过围绕专家"锚点"执行多样化分支探索，并采用混合策略优化机制，有效缓解分布偏移和共享前缀引起的优化冲突。

Result: 在工具使用任务上，E3-TIR比传统范式性能提升6%，同时所需合成数据量不到10%。在综合性能、数据成本和训练效率的ROI指标上，相比基线获得1.46倍的增益。

Conclusion: E3-TIR通过动态整合多种经验类型和混合策略优化，有效解决了工具集成推理训练中的关键挑战，在性能、数据效率和训练效率方面均显著优于现有方法。
...
### [180] [Strategic Algorithmic Monoculture:Experimental Evidence from Coordination Games](https://arxiv.org/abs/2604.09502)
*Gonzalo Ballestero,Hadi Hosseini,Samarth Khanna,Ran I. Shorrer*

Main category: cs.AI

TL;DR: 该研究区分了算法单一文化的两种形式：基础行动相似性（初级单一文化）和基于激励调整的相似性（战略单一文化），并通过实验发现LLM在基础相似性上表现高，能像人类一样根据协调激励调整相似性，但在奖励差异时维持异质性方面落后于人类。


<details>
  <summary>Details</summary>
Motivation: 随着AI智能体在多智能体环境中运作，协调结果变得重要。需要区分算法单一文化的不同形式，理解智能体如何根据激励调整相似性，并比较人类与LLM在这些方面的表现差异。

Method: 设计了一个简单的实验框架，清晰分离初级算法单一文化（基础行动相似性）和战略算法单一文化（基于激励调整的相似性）。该实验部署在人类被试和大型语言模型（LLM）上，比较两者在不同协调激励下的行为模式。

Result: LLM表现出高水平的基础行动相似性（初级单一文化），并且像人类一样能够根据协调激励调整相似性（战略单一文化）。LLM在相似行动上协调能力极强，但在奖励差异时维持异质性方面落后于人类。

Conclusion: 算法单一文化存在不同形式，LLM在协调激励响应方面表现出与人类相似的模式，但在维持异质性方面存在局限。这为理解多智能体环境中AI协调行为提供了重要见解。
...
### [181] [QoS-QoE Translation with Large Language Model](https://arxiv.org/abs/2604.08703)
*Yingjie Yu,Mingyuan Wu,Ahmadreza Eslaminia,Lingzhi Zhao,Kaizhuo Yan,Klara Nahrstedt*

Main category: cs.MM

TL;DR: 该研究构建了首个结构化QoS-QoE翻译数据集，并评估了大语言模型在该任务上的性能，为多媒体质量预测和优化提供了基准和基础。


<details>
  <summary>Details</summary>
Motivation: 现有QoS-QoE关系研究分散在不同论文、实验设置和报告格式中，缺乏系统性整理和重用，限制了跨场景泛化和大规模分析。

Method: 通过自动化流程构建结构化QoS-QoE关系数据集，包括论文筛选、关系提取和迭代数据评估；并评估大语言模型在监督微调前后的QoS-QoE翻译能力。

Result: 构建了包含参数定义、支持证据和上下文元数据的结构化数据集；大语言模型在连续值和离散标签预测的双向翻译任务中表现出色。

Conclusion: 该数据集为QoS-QoE翻译的基准测试提供了基础，支持未来基于大语言模型的多媒体质量预测和优化推理研究。
...
### [182] [Generalizing Video DeepFake Detection by Self-generated Audio-Visual Pseudo-Fakes](https://arxiv.org/abs/2604.09110)
*Zihe Wei,Yuezun Li*

Main category: cs.MM

TL;DR: AVPF方法通过自生成的音频-视觉伪伪造样本来增强视频深度伪造检测模型的泛化能力，无需真实伪造数据，在多个数据集上平均性能提升达7.4%


<details>
  <summary>Details</summary>
Motivation: 现有视频深度伪造检测方法在真实场景中泛化能力有限，主要原因是训练数据集多样性不足，限制了模型对未见案例的适应能力

Method: 提出AVPF方法，仅使用真实样本生成音频-视觉伪伪造训练样本，这些样本包含真实世界深度伪造中常见的多样化音频-视觉对应模式，训练过程仅依赖真实数据和AVPF，无需真实伪造数据

Result: 在多个标准数据集上的广泛实验表明，该方法具有强大的泛化能力，平均性能提升高达7.4%

Conclusion: AVPF是一种简单而有效的方法，通过自生成的伪伪造样本显著增强视频深度伪造检测模型的泛化能力，为解决真实场景中的检测挑战提供了有效方案
...
### [183] [2D or 3D: Who Governs Salience in VLA Models? -- Tri-Stage Token Pruning Framework with Modality Salience Awareness](https://arxiv.org/abs/2604.09244)
...
<details>
  <summary>Details</summary>
Motivation: 多视觉模态VLA模型虽然提升了空间感知能力，但由于模态扩展导致输入令牌数量增加，面临更大的加速需求。现有的令牌剪枝方案专为2D-only VLA模型设计，忽略了2D/3D模态显著性差异。

Method: 提出三阶段分析框架捕捉2D/3D模态显著性差异和动态变化，基于此设计相应的三阶段令牌剪枝框架，实现最优的2D/3D令牌选择和高效剪枝。

Result: 实验表明，该框架在最小精度损失下实现了最高2.55倍的推理加速，仅产生5.8%的开销。

Conclusion: 提出的三阶段令牌剪枝框架有效解决了MVLA模型的加速需求，通过考虑模态显著性差异实现了高效优化。
