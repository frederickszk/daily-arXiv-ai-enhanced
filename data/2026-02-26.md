...
TL;DR: StoryTailor是一个零样本多帧视觉叙事生成系统，能在单张RTX 4090上生成时间连贯、身份保持的图像序列，解决了动作文本忠实度、主体身份保真度和跨帧背景连续性的三重挑战。


<details>
  <summary>Details</summary>
Motivation: 生成多帧、动作丰富的视觉叙事时面临三重挑战：动作文本的忠实度、主体身份的保真度以及跨帧背景的连续性。现有方法通常需要微调，而本文旨在开发一个零样本的解决方案。

Method: 提出了StoryTailor系统，包含三个协同模块：1) 高斯中心注意力(GCA)动态聚焦每个主体核心并缓解边界框重叠；2) 动作增强奇异值重加权(AB-SVR)放大文本嵌入空间中与动作相关的方向；3) 选择性遗忘缓存(SFC)保留可转移的背景线索，遗忘非必要历史，并有选择性地提取保留线索以建立跨场景语义联系。

Result: 实验显示CLIP-T指标提升10-15%，DreamSim低于强基线，CLIP-I保持在视觉可接受且具有竞争力的范围内。在24GB GPU上，推理速度比FluxKontext更快。定性评估显示StoryTailor能够生成富有表现力的交互和稳定演变的场景。
...
TL;DR: HorizonForge：一个统一的驾驶场景生成框架，通过高斯点云和网格重建可编辑场景，结合噪声感知视频扩散实现时空一致性渲染，在单次前向传播中生成多样场景变体，无需逐轨迹优化。


<details>
  <summary>Details</summary>
Motivation: 现有可控驾驶场景生成方法难以同时实现照片级真实感和精确控制，需要一种既能提供细粒度3D编辑又能保持时空一致性的解决方案。

Method: 1. 将场景重建为可编辑的高斯点云和网格表示；2. 支持语言驱动的车辆插入和细粒度3D操作；3. 通过噪声感知视频扩散过程渲染编辑，确保时空一致性；4. 单次前向传播生成多样场景变体，无需逐轨迹优化。

Result: 1. 高斯-网格表示相比其他3D表示提供显著更高的保真度；2. 视频扩散的时间先验对一致性合成至关重要；3. HorizonForge在用户偏好上获得83.4%的提升，FID指标改善25.19%，优于现有最佳方法。

Conclusion: HorizonForge建立了一个简单而强大的范式，实现了照片级真实感且可控的驾驶模拟，通过统一的编辑表示和一致性渲染流程，为自动驾驶仿真提供了可扩展的解决方案。
...
TL;DR: 本文提出了一种手术室视频扩散框架，能够可控合成罕见和安全关键事件，解决了手术室工作流数据稀缺问题，并通过合成数据训练AI模型检测无菌区域违规事件。


<details>
  <summary>Details</summary>
Motivation: 手术室工作流的大规模数据集，特别是包含罕见、安全关键或非典型事件的数据，在操作和伦理上都难以获取。这种数据瓶颈阻碍了环境智能的发展，无法有效检测、理解和缓解手术室中的罕见或安全关键事件。

Method: 提出了一个手术室视频扩散框架，包含几何抽象模块、条件模块和微调扩散模型。首先将手术室场景转换为抽象几何表示，然后条件化合成过程，最后生成逼真的手术室事件视频。使用该框架还创建了一个合成数据集，用于训练和验证检测无菌区域违规事件的AI模型。

Result: 在合成常规手术室事件方面，该方法优于现成的视频扩散基线，在域内和域外数据集上都获得了更低的FVD/LPIPS和更高的SSIM/PSNR。定性结果显示其能够可控合成反事实事件。在生成的合成数据上训练和验证的AI模型，在检测接近安全关键事件方面达到了70.13%的召回率。消融研究量化了关键设计选择带来的性能提升。

Conclusion: 该解决方案能够从抽象几何表示中可控合成常规和罕见的手术室事件。除了展示其生成罕见和安全关键场景的能力外，还显示了其支持环境智能模型开发的潜力。
...
TL;DR: MoMKD提出了一种基于动量记忆的知识蒸馏框架，通过跨批次聚合基因组和病理信息来解决多模态学习中配对数据稀缺问题，在仅使用组织学图像的情况下实现准确的癌症诊断。


<details>
  <summary>Details</summary>
Motivation: 多模态学习结合基因组学和组织病理学在癌症诊断中显示出强大潜力，但临床转化受到配对组织学-基因组数据稀缺的限制。现有知识蒸馏方法依赖批次内对齐，由于批次内比较有限导致不稳定并最终降低性能。

Method: 提出动量记忆知识蒸馏（MoMKD）框架，通过动量更新的记忆跨批次聚合基因组和病理信息，扩大每个小批量的监督上下文。同时解耦基因组和组织学分支的梯度，防止基因组信号主导组织学特征学习，消除推理时的模态差距问题。

Result: 在TCGA-BRCA基准测试（HER2、PR和ODX分类任务）和独立内部测试数据集上的实验表明，MoMKD始终优于最先进的多实例学习和多模态知识蒸馏基线方法，在仅使用组织学推理的情况下实现了强大的性能和泛化能力。

Conclusion: MoMKD为计算病理学建立了一个稳健且可泛化的知识蒸馏范式，能够有效解决多模态学习中配对数据稀缺的问题，推动癌症诊断的临床转化。
...
TL;DR: MMLoP是一种参数高效的多模态提示学习方法，仅需11.5K可训练参数，通过低秩分解实现深度视觉和文本提示，在保持参数效率的同时达到接近SOTA的性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度多模态提示学习方法虽然性能显著提升，但需要数百万参数，放弃了提示调优的参数效率优势。需要在保持参数效率的同时实现高性能的多模态提示学习。
...
Result: 在3个基准测试和11个数据集上的实验表明，MMLoP在仅11.5K参数下，优于大多数现有方法（包括参数数量多几个数量级的方法），在基础到新类泛化上达到79.70%的调和平均值。

Conclusion: MMLoP实现了高度有利的准确率-效率权衡，证明了通过低秩分解和正则化技术，可以在保持参数效率的同时实现高性能的深度多模态提示学习。
...
<details>
  <summary>Details</summary>
Motivation: 主题驱动生成（SDG）在生成过程中，由于尺度和视角的变化，常常会丢失主题的精细细节。现有方法依赖语言提示，存在模糊性，无法有效恢复这些丢失的细节。

Method: 1. 提出图像到图像的转换方法，避免语言提示的模糊性；2. 引入一步去噪方案生成自监督训练数据，自动移除高频细节同时保留全局结构，模拟真实SDG错误；3. 提出基于关键点匹配的评估指标，超越CLIP或DINO的语义相似性评估。

Result: FlowFixer在定性和定量评估中都优于最先进的主题驱动生成方法，为高保真主题驱动生成设立了新的基准。

Conclusion: FlowFixer通过图像到图像转换、自监督数据生成和关键点匹配评估，有效解决了主题驱动生成中细节丢失的问题，显著提升了生成质量。
...
TL;DR: 论文提出了SVG提取任务，创建了WildSVG基准测试，包含真实和合成数据集，评估了当前多模态模型在真实场景SVG提取中的表现，发现现有方法性能不足但迭代优化方法有前景。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型在从干净渲染或文本描述生成SVG方面表现良好，但在真实场景的自然图像中存在噪声、杂乱和领域偏移时表现不佳。该领域缺乏合适的基准测试是主要挑战。

Method: 提出了SVG提取任务，并创建了WildSVG基准测试，包含两个互补数据集：Natural WildSVG（从包含公司logo的真实图像及其SVG标注构建）和Synthetic WildSVG（将复杂SVG渲染混合到真实场景中以模拟困难条件）。

Result: 对最先进的多模态模型进行基准测试发现，当前方法在真实场景SVG提取中的性能远低于可靠应用所需水平。然而，迭代优化方法显示出有前景的发展路径，模型能力正在稳步提升。

Conclusion: WildSVG基准测试为系统评估SVG提取提供了首个基础，揭示了当前方法在真实场景中的局限性，但迭代优化方法为未来改进指明了方向。
...
TL;DR: ECHOSAT是一个全球性、时间一致的10米分辨率树高地图，通过多传感器卫星数据和专门设计的视觉Transformer模型，实现了多年树高动态监测，包括生长和干扰事件。


<details>
  <summary>Details</summary>
Motivation: 现有全球树高地图仅提供静态快照，无法捕捉森林的时间动态变化，而这对准确的碳核算至关重要。

Method: 使用多传感器卫星数据训练专门的视觉Transformer模型，进行像素级时间回归；通过自监督生长损失正则化预测，使其符合自然树木生长曲线（包括随时间逐渐增高和火灾等干扰导致的突然下降）。

Result: 模型在单年预测方面提高了最先进方法的准确性；提供了首个全球尺度的高度地图，能够准确量化树木生长和干扰随时间的变化。

Conclusion: ECHOSAT将推动全球碳监测和干扰评估工作，地图数据可在GitHub上获取。
...
TL;DR: tugturn.py是一个用于3D无标记Timed Up and Go测试处理的Python工作流，集成了相位分割、步态事件检测、时空指标、节段间协调和动态稳定性分析，通过TOML配置实现可重复分析。


<details>
  <summary>Details</summary>
Motivation: 目前虽然有仪器化的Timed Up and Go测试分析可以支持临床和研究决策，但稳健且可重复的无标记处理流程仍然有限，需要开发标准化的分析工具。

Method: 开发了tugturn.py工作流，使用空间阈值将试验分割为站立、第一次步态、转身、第二次步态和坐下阶段，采用相对距离策略在有效步态窗口内检测脚跟触地和脚尖离地事件，提供传统运动学、矢量编码和外推质心指标分析。

Result: 该软件通过TOML文件配置，生成可重复的HTML报告、CSV表格和质量保证可视化输出，提供了完整的可运行示例和测试数据。

Conclusion: tugturn.py作为一个专注于无标记生物力学TUG分析的软件贡献，实现了标准化、可重复的处理流程，支持临床和研究应用。
...
TL;DR: 医疗视觉语言模型在医生重述相同问题时可能改变答案，存在部署风险。研究引入PSF-Med基准测试，发现6个医疗VLM的答案翻转率为8%-58%，但低翻转率不一定代表视觉基础，有些模型即使移除图像也能保持一致，表明它们依赖语言先验。通过分析MedGemma 4B，识别出与提示框架相关的稀疏特征，干预该特征可减少翻转率31%，仅损失1.3%准确率。


<details>
  <summary>Details</summary>
Motivation: 医疗视觉语言模型在临床部署中存在风险：当医生用不同方式重述相同问题时，模型可能给出不一致的答案。这种"重述敏感性失败"可能导致临床决策错误，因此需要系统评估模型的稳定性和视觉基础能力。

Method: 1. 构建PSF-Med基准：包含19,748个胸部X光问题和约92,000个语义保持的重述，覆盖MIMIC-CXR和PadChest数据集；2. 评估6个医疗VLM的答案翻转率；3. 使用文本基线测试模型是否依赖语言先验而非图像；4. 对MedGemma 4B应用GemmaScope 2稀疏自编码器，分析158个翻转案例；5. 识别与提示框架相关的稀疏特征，进行因果修补和特征钳制实验。

Result: 1. 6个医疗VLM的答案翻转率为8%-58%；2. 文本基线显示低翻转率模型可能依赖语言先验而非视觉信息；3. 在MedGemma 4B中识别出层17的稀疏特征，该特征与提示框架相关并预测决策边界变化；4. 移除该特征贡献可恢复45%的是/否对数几率边界，完全逆转15%的翻转；5. 推理时钳制该特征可将翻转率相对减少31%，仅损失1.3%准确率，同时减少对文本先验的依赖。

Conclusion: 仅凭翻转率不足以评估医疗VLM的鲁棒性，需要同时测试重述稳定性和图像依赖程度。通过分析模型内部机制，可以识别导致不一致的特定特征并进行干预，显著提高模型稳定性。这为医疗AI系统的安全部署提供了重要评估框架和优化方法。
...
TL;DR: 该论文提出了AD-Loop（分析-草拟循环），一种新的思维范式，通过交替进行文本和视觉思考，实现视觉语言模型中理解和生成能力的真正协同。


<details>
  <summary>Details</summary>
Motivation: 现有的统一视觉语言模型主要关注架构统一，但忽视了理解和生成能力在任务解决过程中的显式交互，导致这两种能力被当作平行技能而非协同过程。

Method: 提出AD-Loop（分析-草拟循环）思维范式，动态交替分析操作和草拟操作，交织文本思考和视觉思考；采用两阶段训练策略：先在交织思考数据上进行监督学习初始化交替机制，然后通过强化学习促进自适应和自主控制。

Result: AD-Loop在标准理解和生成基准测试中持续提升性能，对各种UVLM架构具有强可迁移性；视觉分析进一步验证了隐式视觉思考的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习分割模型在临床影像工作流中应用日益广泛，但其对抗性扰动的鲁棒性尚未完全明确，特别是在超声图像领域。本研究旨在评估甲状腺结节B超分割模型对抗性攻击的脆弱性及防御方法的有效性。

Method: 开发了两种黑盒对抗性攻击：1) SSAA（结构化斑点放大攻击），注入边界针对性噪声；2) FDUA（频域超声攻击），在傅里叶域应用带通滤波相位扰动。评估了三种推理时防御：随机预处理与测试时增强、确定性输入去噪、随机集成推理与一致性聚合。使用U-Net模型在192个甲状腺结节的电影剪辑数据库上进行实验。

Result: 基线模型在未扰动图像上的平均Dice相似系数为0.76。SSAA使DSC降低0.29，但保持高视觉相似性（SSIM=0.94）；FDUA使DSC降低0.11，但视觉保真度较低（SSIM=0.82）。针对SSAA，所有三种防御均显著改善DSC，其中确定性去噪恢复最大（+0.10），其次是随机预处理（+0.09）和随机集成推理（+0.08）。针对FDUA，所有防御均无统计学显著改善。

Conclusion: 超声分割中的空间域对抗性扰动可通过输入预处理部分缓解，而频域扰动则无法通过所测试的防御方法缓解，这突显了对抗性鲁棒性评估中的模态特异性挑战。
...
TL;DR: 本文提出了一种动态VPR建图方法，通过分析目标环境中多参考轨迹的匹配模式，自动选择合适的地图密度以满足用户定义的局部召回率要求和覆盖范围要求。


<details>
  <summary>Details</summary>
Motivation: 现有VPR系统主要关注全局平均性能，而实际部署需要确保在环境的不同部分都能满足用户指定的性能要求。当前工作忽视了地图密度对局部性能控制的关键作用，通常使用固定采样密度的基准数据集。

Method: 提出动态VPR建图方法，利用目标环境中的多参考轨迹对，在不同地图密度下评估匹配模式，建立模型来预测满足性能目标所需的地图密度。系统接受两个用户定义要求：目标局部Recall@1水平和需要达到该要求的操作环境比例（RAR）。

Result: 在多个VPR方法和Nordland、Oxford RobotCar基准测试上的实验表明，系统能够持续达到或超过指定的局部召回率水平，覆盖至少用户指定的环境比例。与基线方法相比，能可靠选择正确的地图密度操作点，避免不必要的过度密集化。

Conclusion: 该方法能够自动选择合适的地图密度以满足局部性能要求，传统的全局Recall@1是操作上更有意义的RAR指标的较差预测器。动态建图方法为VPR系统的实际部署提供了重要工具。
...
TL;DR: 提出了一种轻量级、无需训练的方法来解决多模态推理中的视觉幻觉传播问题，通过构建文本视觉证据池来监督每个推理步骤，确保每个解码标记都有对应的视觉证据支持。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在生成链式思维响应时存在视觉幻觉传播问题：一旦中间推理步骤与视觉证据不一致，后续步骤即使逻辑有效也会导致错误答案。现有基于强化学习的解决方案成本高、模型特定且难以跨架构泛化。

Method: 提出一个无需训练、即插即用的迭代框架：1）构建文本视觉证据池指导模型推理生成；2）当现有证据不足时，视觉决策模块根据当前推理上下文动态从图像中提取额外相关证据；3）扩展证据池直到模型获得足够的视觉确定性来终止推理并生成最终答案。

Result: 在多个LVLM主干和基准测试上验证了方法的有效性：在TreeBench上获得16.5%-29.5%的改进，在RH-Bench上获得13.7%的RH-AUC增益，显著降低幻觉率并提高推理准确性，且无需额外训练。

Conclusion: 该方法提供了一种轻量级、无需训练、可跨架构泛化的解决方案，有效解决了多模态推理中的视觉幻觉传播问题，通过实时视觉证据监督确保推理步骤与视觉内容的一致性。
...
TL;DR: 本文提出了TEA-CXA框架，通过智能体学习在医疗多模态环境中解决AI工具冲突问题，特别针对胸部X光分析，让智能体通过实验学习不同工具的实际可信度。


<details>
  <summary>Details</summary>
Motivation: 医疗领域中的AI工具存在固有错误倾向且可能产生矛盾响应，现有研究对工具实际可靠性理解不足，无法有效解决工具冲突问题。

Method: 提出一个框架让智能体通过智能体学习与工具交互，经验性地学习不同多模态查询类型下工具的实际可信度。具体实现TEA-CXA，当工具输出不一致时，智能体实验性地接受或拒绝多模态工具结果，接收奖励，学习每种查询类型应信任哪个工具。扩展了现有代码库以支持多模态上下文、单轮多工具调用、并行工具推理和单查询内多图像处理。

Result: 实验表明TEA-CXA优于最先进方法和一系列基线方法。

Conclusion: 该代码框架适用于多模态环境中多轮工具调用强化学习的一般医学研究，能够有效解决医疗AI工具冲突问题，提高工具使用的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 在无姿态稀疏视角下的3D场景重建是一个极具挑战性的实际问题，特别是在户外场景中，由于复杂的照明和尺度变化，直接使用扩散模型合成伪帧会引入不合理的几何结构，损害最终重建质量

Method: 提出双向伪帧恢复方法，通过基于相邻帧引导的扩散合成恢复缺失内容，包含轻量级伪视图去模糊模型和置信掩码推理算法；提出场景感知高斯管理策略，基于联合深度-密度信息优化高斯分布

Result: 在户外基准测试中，相比现有方法在保真度和稳定性方面都有显著提升，显著增强了重建完整性，抑制了浮动伪影，并改善了极端视角稀疏下的几何一致性

Conclusion: 该框架通过创新的双向伪帧恢复和场景感知高斯管理策略，有效解决了稀疏视角户外3D场景重建的挑战，实现了高质量的重建结果
...
TL;DR: IHF-Harmony：一种基于可逆层次流的多模态MRI图像协调框架，使用非配对数据实现高保真度协调，避免解剖结构失真


<details>
  <summary>Details</summary>
Motivation: 传统MRI协调方法存在可扩展性差（跨模态）和依赖配对数据（旅行者数据集）的局限性，需要一种能处理多模态、非配对数据的统一协调框架
...
Conclusion: IHF-Harmony通过可逆特征变换保证双射映射和无损重建，实现了保留源解剖结构的高保真度多模态协调，解决了传统方法的可扩展性和数据依赖问题
...
TL;DR: GPOcc：利用视觉几何先验进行单目占用预测的新框架，通过将表面点向内扩展生成体积样本，使用高斯原语进行概率占用推理，在单目和流式设置下显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法主要依赖深度先验但3D线索利用有限，限制了性能和泛化能力。虽然视觉几何模型能提供丰富的3D先验，但它们仍停留在可见表面层面而非体积内部，需要探索如何更有效地利用这些几何先验进行3D占用预测

Method: 提出GPOcc框架，利用可泛化的视觉几何先验进行单目占用预测。方法将表面点沿相机光线向内扩展生成体积样本，表示为高斯原语进行概率占用推理。针对流式输入，设计了无需训练的自增更新策略，将每帧高斯融合为统一的全局表示

Result: 在Occ-ScanNet和EmbodiedOcc-ScanNet数据集上取得显著提升：单目设置下mIoU提高+9.99，流式设置下提高+11.79。在相同深度先验下，mIoU提高+6.73同时运行速度提升2.65倍

Conclusion: GPOcc能够更有效且高效地利用几何先验进行3D占用预测，在性能和效率方面均优于现有方法，为3D场景理解提供了新的解决方案
...
Method: 提出基于扩散Transformer的可扩展多角色图像动画框架，核心包含两个组件：标识符分配器和标识符适配器，通过掩码驱动方案捕捉每个角色的位置线索和角色间的空间关系，配合可扩展的训练策略

Result: 仅使用双角色数据集训练，模型能够泛化到多角色动画，同时保持与单角色情况的兼容性，在多角色图像动画中达到最先进的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的图像压缩方法虽然能在极低比特率下实现逼真重建，但在内容适应性方面存在三个关键限制：各向同性量化无法适应图像内容的空间变化复杂性；信息集中瓶颈阻碍了语义信息的自适应保留；文本条件策略要么需要大量比特率开销，要么使用通用的内容无关提示。这些限制阻碍了有效的內容适应。

Method: 提出了三种技术创新：1）不确定性引导的自适应量化方法，学习空间不确定性图来自适应对齐量化失真与内容特征；2）辅助解码器引导的信息集中方法，使用轻量级辅助解码器强制在主要潜在通道中进行内容感知的信息保留；3）无比特率自适应文本条件方法，从辅助重建图像中提取内容感知的文本描述，实现无需比特率成本的语义指导。

Result: 该方法实现了内容自适应的扩散图像压缩，能够更好地适应图像内容的语义和结构特征，在保持高质量重建的同时有效降低比特率开销。

Conclusion: 通过提出的三种技术创新，成功解决了现有扩散图像压缩方法在内容适应性方面的关键限制，实现了更有效的自适应压缩过程，为超低比特率下的逼真图像重建提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 尽管扩散Transformer在条件生成任务中取得了最先进的性能，但其学习到的条件嵌入结构仍未被充分理解。本研究旨在系统分析这些嵌入的结构特征。

Method: 对扩散Transformer中的条件嵌入进行系统性研究，分析类条件嵌入的相似度特征，识别语义信息在嵌入维度中的分布模式，并通过修剪低幅度维度来验证冗余性。

Result: 发现条件嵌入存在极端冗余：ImageNet-1K的类条件嵌入角度相似度超过99%，连续条件任务如姿态引导图像生成和视频到音频生成达到99.9%以上。语义信息集中在头部维度，修剪高达三分之二的低幅度维度后，生成质量和保真度基本不受影响甚至有时有所改善。

Conclusion: 揭示了Transformer扩散模型中的语义瓶颈，为理解语义编码方式提供了新见解，并指出了设计更高效条件机制的潜在机会。
...
TL;DR: 提出Virtual Biopsy框架，使用MRI和视觉语言模型进行颅内肿瘤的非侵入性病理预测，在首个公开活检验证数据集ICT-MRI上达到90%以上准确率


<details>
  <summary>Details</summary>
Motivation: 深部颅内肿瘤位于重要功能区，活检存在出血风险且采样偏差大，需要非侵入性MRI病理预测方法进行全面评估

Method: 构建Virtual Biopsy框架：MRI处理器标准化数据；肿瘤定位器使用视觉语言模型进行粗到精定位；自适应诊断器采用掩蔽通道注意力机制融合局部特征与全局上下文

Result: 在ICT-MRI数据集（249个病例，4个类别）上达到90%以上准确率，比基线方法提升超过20%

Conclusion: 提出的Virtual Biopsy框架能有效解决颅内肿瘤病理预测的数据稀缺问题，为临床决策提供非侵入性评估工具
...
TL;DR: 提出一种使用语言建模将分割掩码编码为离散标记序列的统一方法，适用于图像和视频语义分割，通过RLE编码和自回归生成实现


<details>
  <summary>Details</summary>
Motivation: 现有分割方法通常针对特定模态（图像或视频）设计，缺乏统一的框架。本文旨在开发一种通用的语义分割方法，能够同时处理图像和视频，并通过语言建模的方式实现分割掩码的生成

Method: 1. 使用行程长度编码（RLE）将分割掩码离散化为标记序列；2. 改进Pix2Seq模型，通过自回归方式生成RLE标记；3. 提出新颖的标记化策略压缩序列长度，使其适用于视频处理；4. 在标记化过程中融入实例信息以支持全景分割

Result: 在两个数据集上的评估表明，尽管受限于计算资源，所提出的模型在性能上仍能与当前最先进方法竞争

Conclusion: 本文提出的基于语言建模的统一分割框架在图像和视频语义分割任务上表现出竞争力，证明了将分割任务转化为序列生成问题的可行性，并为全景分割提供了扩展方案
...
<details>
  <summary>Details</summary>
Motivation: 手部运动在人类交互中至关重要，但现有的手部运动建模方法分为估计和生成两个独立任务，这种分离限制了异构条件信号的利用，也阻碍了任务间的知识迁移。

Method: 提出UniHand统一扩散框架：1) 使用联合变分自编码器将MANO参数和2D骨架等结构化信号嵌入共享潜空间；2) 使用冻结视觉骨干编码视觉观察；3) 专用手部感知器从图像特征直接提取手部线索；4) 潜扩散模型从这些多样化条件合成一致的运动序列。

Result: 在多个基准测试上的广泛实验表明，UniHand能够提供鲁棒准确的手部运动建模，在严重遮挡和时间不完整输入下仍能保持性能。
...
Method: 提出轴向中心交叉平面注意力架构，使用MedDINOv3作为冻结特征提取器处理三个解剖平面。RICA块和平面内transformer编码器捕获各平面特定信息，轴向中心交叉平面transformer编码器将轴向特征与辅助平面互补信息融合。

Result: 在MedMNIST3D基准的六个数据集上，该架构在准确率和AUC方面持续优于现有3D和多平面模型。消融研究证实了轴向中心查询-键-值分配和定向交叉平面融合的重要性。

Conclusion: 将架构设计与临床解读工作流程对齐对于实现稳健且数据高效的3D医学图像分析至关重要，轴向中心交叉平面注意力架构有效模拟了临床诊断流程并取得了优越性能。
...
TL;DR: LieFlow：基于SE(3)李群的动态辐射场表示框架，通过统一几何空间建模平移和旋转，提升4D场景重建的物理一致性和时间连贯性。
...
Result: 在包含刚体轨迹的合成数据集和两个捕捉复杂运动的真实数据集上评估，LieFlow在所有数据集上都比基于NeRF的基线方法在视图合成保真度、时间连贯性和物理真实感方面有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像描述任务主要依赖人工标注作为监督信号，但人工标注存在主观偏好和专业知识差异，导致标注可能不完整甚至错误，限制了描述模型的发展。作者认为描述质量应从两个客观维度评估：完整性（是否覆盖所有显著视觉事实）和正确性（描述是否与图像真实相符）。

Method: 提出CCCaption框架：1）完整性方面，使用多样化LVLM将图像分解为一组视觉查询，奖励能回答更多查询的描述，采用动态查询采样策略提高训练效率；2）正确性方面，通过验证子描述查询的真实性来惩罚包含幻觉的描述；3）采用对称双奖励优化联合最大化完整性和正确性。

Result: 在标准图像描述基准测试上的广泛实验显示，该方法带来了持续改进，为训练超越人工标注模仿的描述模型提供了原则性路径。

Conclusion: CCCaption框架通过明确优化完整性和正确性这两个客观属性，为图像描述任务提供了超越传统人工标注监督的新方法，展示了通过原则性优化而非简单模仿人工标注来提升描述质量的可行性。
...
TL;DR: 本文提出了一种无需训练的幻觉缓解方法，通过分析LVLM激活模式发现真实性和视觉感知能力主要涉及不同的注意力头子集，并基于此构建语义相关的真实性导向向量数据库，在推理时动态选择最相关的导向向量应用于最具影响力的注意力头。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉语言任务中表现出色，但存在幻觉问题。通过深入分析LVLM激活模式，发现真实性和视觉感知能力主要涉及不同的注意力头子集，且真实性导向向量在不同语义上下文中差异显著，这为解决幻觉问题提供了新的思路。

Method: 提出动态多模态激活导向方法：1）构建基于语义的真实性导向向量数据库；2）计算视觉感知导向向量；3）在推理时根据输入语义相似度动态选择最相关的导向向量，并将其应用于最具影响力的注意力头，实现上下文感知的干预。

Result: 在多个模型和数据集上进行了全面实验，结果表明该方法显著提升了模型性能，优于现有的最先进方法。

Conclusion: 通过分析LVLM激活模式发现真实性和视觉感知能力涉及不同的注意力头子集，基于此提出的动态多模态激活导向方法能够有效缓解幻觉问题，且无需训练，具有实用价值。
...
TL;DR: 提出了ResGo基准和SurGo-R1模型，用于腹腔镜手术中基于手术阶段的"安全操作区域"识别和推理
...
Method: 引入ResGo基准数据集，包含标注的Go Zone边界框和临床医生撰写的推理理由；提出SurGo-R1模型，采用RLHF优化的多轮阶段-然后-区域架构，先识别手术阶段，再基于该上下文生成推理和Go Zone坐标

Result: 在未见过的程序中，SurGo-R1达到76.6%的阶段准确率、32.7 mIoU和54.8%的硬核准确率，比主流通用视觉语言模型提升6.6倍

Conclusion: 通过引入阶段感知的评估指标和多阶段架构，显著提升了手术中安全区域识别的性能，揭示了通用视觉语言模型在处理此类任务时的局限性
...
<details>
  <summary>Details</summary>
Motivation: 现有协同感知方法在严格带宽限制下难以保持高精度，且对随机传输数据包丢失高度脆弱。需要一种既能大幅降低带宽又能保持高保真3D信息，同时对传输故障具有鲁棒性的解决方案。

Method: 1. 量化点云通信框架：直接传输量化点云索引而非中间特征，使用共享码本实现高效重建；2. 掩码训练策略：模拟随机数据包丢失，增强模型对传输故障的鲁棒性；3. 级联注意力融合模块：提升多车辆信息整合能力。

Result: 在模拟和真实世界数据集上的广泛实验表明，QPoint2Comm在精度、通信效率和数据包丢失鲁棒性方面均达到新的最先进水平。

Conclusion: QPoint2Comm通过量化点云通信、掩码训练和级联注意力融合，实现了高精度、低带宽需求和对传输故障强鲁棒性的协同感知，为连接车辆的协同感知提供了有效解决方案。
...
TL;DR: MoGaF是一个基于4D高斯泼溅表示的长时场景外推框架，通过运动感知高斯分组和组级优化实现物理一致的运动，结合轻量预测模块生成逼真稳定的场景演化。
...
Method: 基于4D高斯泼溅表示，引入运动感知高斯分组和组级优化确保刚性和非刚性区域的物理一致运动，利用结构化时空表示构建轻量预测模块预测未来运动。

Result: 在合成和真实数据集上的实验表明，MoGaF在渲染质量、运动合理性和长期预测稳定性方面持续优于现有基线方法。

Conclusion: MoGaF通过运动感知高斯分组和结构化预测实现了高质量的长期动态场景外推，为动态场景理解提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI广泛用于商业海报创作，但现有质量评估模型强调通用美学或低级失真，缺乏电商设计所需的功能性标准，尤其对于中文内容，复杂字符常产生被现有方法忽略的关键文本伪影

Method: 1. 构建首个中文电商海报数据集E-comIQ-18k，包含多维评分和专家校准的思维链推理；2. 训练专用评估模型E-comIQ-M，使其与人类专家判断对齐；3. 建立E-comIQ-Bench自动化基准测试框架

Result: E-comIQ-M与专家标准更紧密对齐，能够实现电商海报的可扩展自动化评估，实验证明其有效性优于现有方法

Conclusion: E-comIQ-ZH框架填补了中文电商海报质量评估的空白，提供了首个自动化可扩展的评估解决方案，所有数据集、模型和评估工具将开源以支持该领域未来研究
...
TL;DR: 本文提出FAQ基准测试，将时序深度伪造检测构建为多项选择题，通过三层层次结构评估和增强视觉语言模型的取证能力。
...
Method: 提出Forensic Answer-Questioning (FAQ)基准测试，将时序深度伪造分析构建为多项选择题。引入三层层次结构：1) 面部感知（识别静态视觉伪影）；2) 时序深度伪造定位（跨帧定位动态伪造伪影）；3) 取证推理（综合证据做出真实性判断）。生成相应的指令调优集FAQ-IT。
...
TL;DR: SF3D-RGB是一个用于稀疏场景流估计的深度学习架构，融合2D单目图像和3D点云数据，通过特征融合、图匹配和残差优化实现准确高效的场景流估计。


<details>
  <summary>Details</summary>
Motivation: 场景流估计对感知场景动态变化至关重要，现有学习方法主要关注单一模态（图像或LiDAR），缺乏多模态融合方法。为解决这一问题，需要开发能够融合2D图像和3D点云的多模态场景流估计方法。

Method: 提出SF3D-RGB端到端模型：1）分别编码2D单目图像和3D点云的特征；2）融合两种模态的特征；3）通过图匹配模块计算映射矩阵生成初始场景流；4）使用残差场景流模块进一步优化初始场景流。模型设计注重准确性和效率的平衡。

Result: 在真实世界数据集上，SF3D-RGB优于单一模态方法，取得了更好的场景流精度。同时，相比其他最先进的融合方法，该模型使用更少的参数，实现了更高的效率。

Conclusion: SF3D-RGB通过有效融合2D图像和3D点云信息，实现了准确且高效的稀疏场景流估计，在多模态场景流估计任务中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 非增强肿瘤区域在最近的脑肿瘤分割挑战（如MICCAI挑战）中未被考虑，但该区域对患者生存时间和肿瘤生长预测具有重要指示意义，因此需要自动分割该区域的方法。

Method: 设计基于U-Net的深度学习架构，用于分割多种MRI模态图像中的脑肿瘤，特别强调非增强肿瘤区域的分割。

Result: 论文提出了能够自动分割脑肿瘤（特别是非增强区域）的U-Net架构，但摘要中未提供具体的实验结果数据。

Conclusion: 非增强肿瘤区域对临床预后至关重要，基于U-Net的深度学习架构为自动分割该区域提供了有效手段，填补了当前脑肿瘤分割挑战中的空白。
...
TL;DR: SemVideo：基于分层语义引导的fMRI到视频重建框架，通过SemMiner模块提取三层语义线索，解决了现有方法中物体外观不一致和时序连贯性差的问题。


<details>
  <summary>Details</summary>
Motivation: 当前fMRI到视频重建方法存在两个主要缺陷：1）跨帧的显著物体视觉表示不一致，导致外观不匹配；2）时序连贯性差，造成运动错位或帧间突变。需要解决这些限制以实现更准确的动态视觉体验重建。

Method: 提出SemVideo框架，核心是SemMiner分层引导模块，从原始视频刺激中构建三层语义线索：静态锚点描述、运动导向叙事和整体摘要。包含三个关键组件：语义对齐解码器（对齐fMRI信号与CLIP风格嵌入）、运动适应解码器（使用三方注意力融合架构重建动态运动模式）、条件视频渲染器（利用分层语义引导进行视频重建）。

Result: 在CC2017和HCP数据集上的实验表明，SemVideo在语义对齐和时序一致性方面均取得优越性能，为fMRI到视频重建设立了新的最先进水平。

Conclusion: SemVideo通过分层语义引导成功解决了fMRI到视频重建中的外观不一致和时序连贯性问题，为探索人类视觉感知的神经机制提供了有力工具。
...
TL;DR: StoryMovie数据集通过LCS匹配将电影剧本与字幕对齐，实现了角色对话的正确归属。基于此训练的Storyteller3模型在对话归属方面显著优于仅基于视觉基础的方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉叙事模型虽然能正确识别图像中的实体，但在语义关系上仍会出现幻觉，导致错误的对话归属、角色互动或情感状态。需要解决视觉基础之外的语义对齐问题。

Method: 1. 创建StoryMovie数据集：通过LCS匹配将1,757个故事与电影剧本和字幕对齐；2. 对齐流程：同步剧本对话与字幕时间戳，将角色名与时间位置关联；3. 生成包含视觉基础标签和真实角色名、对话、关系动态的故事；4. 在Qwen Storyteller3上微调，结合视觉基础和实体重识别技术。

Result: 1. Storyteller3相比基础Qwen2.5-VL 7B在字幕对齐上获得89.9%的胜率；2. 相比没有剧本基础的Storyteller，Storyteller3在对话归属上达到48.5% vs 38.0%；3. 语义对齐逐步改善对话归属，超越仅视觉基础的方法。

Conclusion: 通过剧本与字幕的语义对齐，可以显著改善视觉叙事模型中的对话归属问题。StoryMovie数据集和Storyteller3模型证明了语义对齐在超越视觉基础方面的重要性，为更准确的叙事生成提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 森林林分划分对森林调查和管理至关重要，但目前主要依赖人工且具有主观性。虽然深度学习结合航空影像和ALS数据能产生与专家相当的划分结果，但数据源的时间不对齐限制了操作可扩展性。DAP衍生的CHM能提供更好的时间对齐，但可能平滑冠层表面和空隙，需要验证其能否可靠替代ALS-CHM。同时，DTM的纳入被认为可能改善划分性能，但尚未在文献中得到验证。
...
Result: 所有数据组合表现出相当的性能，总体准确度在0.90-0.91之间。模型预测之间的一致性远大于与参考数据的一致性，突显了模型的一致性和林分划分固有的主观性。尽管DAP-CHM的结构细节减少，但其相似性能以及DTM缺乏改进表明该框架对输入数据变化具有鲁棒性。

Conclusion: 研究结果表明，可以使用包含时间对齐的ALS数据和DAP点云的项目来组装用于深度学习林分划分的大型数据集。DAP衍生的CHM可以可靠地替代ALS-CHM，而DTM的纳入并未带来显著改进，框架对输入数据变化具有弹性。
...
TL;DR: 提出L2RP框架，通过成本感知学习决定何时何地需要专家输入，以平衡内窥镜视频标注工作量与分割精度，解决SAM2等半自动工具中错误累积问题。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频准确标注至关重要但耗时，特别是在巴雷特食管发育不良等具有不规则边界区域的挑战性数据集中。SAM2等半自动工具虽然能通过跨帧传播标注来简化流程，但小错误会累积降低准确性，需要专家审查和修正。

Method: 系统研究不同提示类型（掩码、框、点）下标注错误如何传播，提出学习到重新提示（L2RP）框架，这是一个成本感知框架，学习何时何地寻求专家输入。通过调整人力成本参数，平衡标注工作量与分割精度。

Result: 在私有巴雷特发育不良数据集和公共SUN-SEG基准测试上的实验表明，该方法提高了时间一致性，并优于基线策略。

Conclusion: L2RP框架通过智能地决定何时需要专家干预，有效解决了半自动标注工具中错误累积的问题，在保持标注效率的同时提高了分割准确性。
...
TL;DR: 该论文提出了一种用于牙齿图像分割的三阶段编码器，通过分层特征表示和跨尺度特征融合来解决传统方法中分辨率固定、上下文建模不足的问题，同时采用双向序列建模策略降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统牙齿图像分割方法存在两个主要问题：1）基于固定分辨率特征图的编码器导致分割不连续且目标区域与背景区分不佳，因为缺乏对环境和全局上下文的充分建模；2）基于Transformer的自注意力机制由于二次复杂度(O(n²))带来巨大计算开销，对高分辨率牙齿图像效率低下。

Method: 提出三阶段编码器架构，采用分层特征表示来捕获牙齿图像中的尺度自适应信息。通过跨尺度特征融合联合利用低级细节和高级语义，有效保留精细结构信息同时保持强上下文感知。此外，引入双向序列建模策略增强全局空间上下文理解，而不产生高计算成本。

Result: 在两个牙科数据集上验证了方法的优越性。在OralVision数据集上，模型在平均交并比(mIoU)指标上实现了1.1%的提升。

Conclusion: 提出的三阶段编码器结合分层特征表示和双向序列建模策略，有效解决了牙齿图像分割中的上下文建模不足和计算效率问题，在保持计算效率的同时提升了分割性能。
...
TL;DR: 本文提出TranX-Adapter，一种轻量级融合适配器，通过任务感知最优传输融合和X-融合解决AI生成图像检测中语义特征与伪影特征融合时的注意力稀释问题。


<details>
  <summary>Details</summary>
Motivation: AI生成图像技术快速发展威胁信息完整性，现有方法将纹理级伪影特征与语义特征融合到多模态大语言模型中可提升检测能力，但伪影特征内部相似度高导致注意力稀释，阻碍有效特征融合。

Method: 提出TranX-Adapter，包含两个核心组件：1) 任务感知最优传输融合，利用伪影与语义预测概率间的Jensen-Shannon散度作为成本矩阵，将伪影信息传输到语义特征；2) X-融合，使用交叉注意力将语义信息传输到伪影特征。

Result: 在多个先进MLLMs上的标准AIGI检测基准测试显示，TranX-Adapter带来一致且显著的性能提升（准确率最高提升+6%）。

Conclusion: TranX-Adapter有效解决了AI生成图像检测中语义与伪影特征融合时的注意力稀释问题，通过双向信息传输机制显著提升了检测性能。
...
TL;DR: 开发了一个自动化的冠状动脉钙化检测和评分框架，能够在门控和非门控CT扫描上工作，使用自监督视觉变换器在门控数据上训练，无需非门控训练数据即可实现跨域应用。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉钙化评分是心血管风险的关键预测指标，但传统方法依赖ECG门控CT扫描，限制了其在专业心脏成像环境中的应用。需要开发能够在常规胸部CT（非门控）上工作的自动化评分系统，以扩大心血管筛查的可及性。

Method: 提出了一个自动化框架，核心是CARD-ViT——一个基于DINO方法在门控CT数据上训练的自监督视觉变换器。该框架能够在无需非门控训练数据的情况下，在门控和非门控CT扫描上进行冠状动脉钙化检测和病灶特异性Agatston评分。

Result: 在斯坦福非门控数据集上，框架达到了0.707的准确率和0.528的Cohen's kappa，与直接在非门控扫描上训练的模型相当。在门控测试集上，准确率达到0.910，Cohen's kappa分别为0.871和0.874，显示出稳健的风险分层能力。

Conclusion: 该研究证明了从门控到非门控域的跨域冠状动脉钙化评分的可行性，支持在常规胸部成像中进行可扩展的心血管筛查，无需额外扫描或标注，有望扩大心血管风险评估的覆盖范围。
...
<details>
  <summary>Details</summary>
Motivation: 大规模医学影像数据集通常来自不同设备和厂商，导致分辨率、切片厚度和切片数量高度可变。传统方法需要裁剪或插值以获得固定大小的块，这会导致信息丢失。需要一种能够处理可变输入大小的方法。

Method: 提出SigVLP模型：1) 使用旋转位置嵌入代替绝对位置嵌入，将体积视为3D块序列，将z轴视为无约束的时间维度；2) 在训练中采样CT体积块，并与局部器官级文本观察配对；3) 使用Muon优化器进行训练。

Result: 模型在多种下游任务上评估，包括零样本异常和器官分类、分割和检索任务。分块对齐提供了更细粒度的监督，使模型能够在文本和体积表示之间建立更强的相关性。

Conclusion: SigVLP通过旋转位置嵌入和分块对齐方法，能够处理可变大小的医学影像输入，实现了更精确的文本-体积对齐，避免了传统固定大小处理方法的信息丢失问题。
...
TL;DR: PatchDenoiser是一种轻量级、节能的多尺度基于patch的医学图像去噪框架，通过局部纹理提取和全局上下文聚合，在保持精细解剖细节的同时有效抑制噪声，相比传统CNN、GAN和transformer方法参数更少、计算复杂度更低。


<details>
  <summary>Details</summary>
Motivation: 医学图像质量常因低剂量采集、患者运动或扫描仪限制而受到噪声影响，传统滤波方法会过度平滑丢失细节，而深度学习方法（CNN、GAN、transformer）难以保持精细细节或需要大型计算昂贵模型，限制了临床实用性。

Method: 提出PatchDenoiser框架，将去噪分解为局部纹理提取和全局上下文聚合，通过空间感知的patch融合策略进行融合，设计轻量级、节能的多尺度patch-based架构。
...
TL;DR: 提出Structure-to-Image范式，将深度图从被动约束转变为主动生成基础，通过相位一致性解决结肠镜单目深度估计中的域适应问题。


<details>
  <summary>Details</summary>
Motivation: 结肠镜单目深度估计面临模拟图像与真实图像间的域差距问题，现有图像到图像转换方法在平衡真实感与结构一致性方面存在不足，容易产生结构扭曲和镜面高光。
...
Conclusion: Structure-to-Image范式有效解决了结肠镜单目深度估计中的域适应问题，通过将深度图作为主动生成基础，显著提升了深度估计的准确性。
...
TL;DR: 该论文针对地铁复杂环境下的视觉感知挑战，提出将事件相机与传统RGB相机结合，专注于公里标识别任务，构建了首个大规模RGB-Event数据集EvMetro5K，并提出基于预训练OCR基础模型的多模态适应方法。


<details>
  <summary>Details</summary>
Motivation: 地铁环境具有光照变化大、高速运动、恶劣天气等复杂特性，传统RGB相机在这些条件下感知能力受限。事件相机在低光照、高速场景和低功耗方面具有优势，但缺乏相关数据集和方法研究，特别是在地铁公里标识别这一关键定位任务中。

Method: 提出基于预训练RGB OCR基础模型的鲁棒基线方法，通过多模态适应将事件相机数据与RGB数据融合。构建了首个大规模RGB-Event数据集EvMetro5K，包含5,599对同步RGB-Event样本，分为4,479个训练样本和1,120个测试样本。

Result: 在EvMetro5K数据集和其他广泛使用的基准测试上进行了大量实验，证明了该方法在公里标识别任务中的有效性。数据集和源代码将在GitHub上公开。

Conclusion: 通过集成事件相机和RGB相机，结合多模态适应方法，有效解决了地铁复杂环境下的公里标识别问题，为GNSS拒止条件下的自主地铁定位提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 将RLVR和GRPO方法扩展到多模态场景时面临关键挑战：基于标准差的归一化容易受到极端样本（接近正负奖励）的扭曲。与纯文本LLM不同，多模态模型对这类扭曲特别敏感，因为感知和推理错误都会影响其响应。
...
Conclusion: 该方法在U-Net和DiT架构的扩散模型中均有效，证明了方法的通用性，为条件扩散模型提供了高效且高质量的并行加速方案
...
<details>
  <summary>Details</summary>
Motivation: 当前基于指令的图像编辑模型在语义对齐方面表现良好，但在涉及复杂因果动态的编辑任务中（如折射、材料变形），经常无法生成物理上合理的结果。作者认为这是由于现有范式将编辑视为图像对之间的离散映射，只提供边界条件而忽略了过渡动态。

Method: 1) 构建PhysicTran38K数据集：包含5个物理领域的38K个转换轨迹，采用两阶段过滤和约束感知标注流程；2) 提出PhysicEdit框架：采用文本-视觉双重思考机制，结合冻结的Qwen2.5-VL进行物理基础推理，以及可学习的转换查询为扩散主干提供时间步自适应的视觉指导。

Result: 实验表明，PhysicEdit在物理真实性方面比Qwen-Image-Edit提升了5.9%，在知识基础编辑方面提升了10.1%，为开源方法设定了新的最先进水平，同时与领先的专有模型保持竞争力。

Conclusion: 通过将物理感知编辑重新定义为预测物理状态转换，并引入大规模视频数据集和双重思考机制，PhysicEdit框架显著提升了涉及复杂因果动态的图像编辑任务的物理合理性和知识基础编辑能力。
...
TL;DR: 本文研究了大型视觉语言模型中的物体幻觉问题，发现语言解码器的强先验是主要成因，并提出无需训练的NoLan解码框架来抑制语言先验，有效减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在物体幻觉问题，即输出包含输入图像中不存在的物体。研究旨在确定幻觉主要来源于视觉编码器还是语言解码器，以便针对性解决这一问题。

Method: 设计了系统性实验分析视觉编码器和语言解码器在幻觉生成中的作用，发现语言解码器的强先验是主要成因。基于此提出NoLan解码框架，通过动态抑制语言先验来优化输出分布，该框架无需训练。

Result: 实验结果表明NoLan能有效减少各种LVLM在不同任务中的物体幻觉。例如在POPE基准上，LLaVA-1.5 7B和Qwen-VL 7B的准确率分别提升6.45和7.21。

Conclusion: 物体幻觉主要源于语言解码器的强先验，而非视觉编码器。提出的NoLan框架通过抑制语言先验有效减少幻觉，且无需额外训练，具有实用价值。
...
<details>
  <summary>Details</summary>
Motivation: StreamVGGT在处理多图像和长视频输入时，由于视觉令牌大量涌入导致KV缓存无限增长，造成内存消耗增加和推理延迟，限制了其在长时应用中的可扩展性

Method: 提出无需调优的方法，集成剪枝和量化技术压缩KV缓存：1) 通过高效令牌重要性识别机制剪枝冗余KV以符合固定内存预算；2) 利用KV张量固有分布模式，在剪枝流程中应用维度自适应KV量化进一步减少内存开销

Result: XStreamVGGT在性能损失几乎可忽略的情况下，将内存使用减少4.42倍，推理速度提升5.48倍，实现了实用且可扩展的流式3D应用

Conclusion: XStreamVGGT通过系统化KV缓存压缩技术有效解决了StreamVGGT的内存和延迟瓶颈，为长时流式3D视觉应用提供了高效实用的解决方案
...
TL;DR: 提出了一种完全基于学习的方法，通过注意力机制直接从潜在特征表示推断运动物体，实现端到端前馈运动分割，绕过显式对应估计。


<details>
  <summary>Details</summary>
Motivation: 动态场景中的运动分割非常具有挑战性，传统方法严重依赖从固有噪声运动线索中估计相机姿态和点对应关系。现有的统计推断或迭代优化技术难以缓解多阶段流水线中的累积误差，导致性能有限或计算成本高。

Method: 提出完全基于学习的方法，通过注意力机制直接从潜在特征表示推断运动物体，实现端到端前馈运动分割。核心洞察是绕过显式对应估计，让模型学习隐式解耦物体和相机运动。利用4D场景几何重建的最新进展（如π³），利用可靠的相机姿态和丰富的时空先验，确保模型稳定训练和鲁棒推理。

Result: 通过消除复杂的预处理和迭代细化，该方法在运动分割方面实现了最先进的性能，同时具有高效率。

Conclusion: 提出的完全学习型方法通过注意力机制直接从潜在特征推断运动物体，绕过传统对应估计的局限性，结合4D几何重建提供的可靠先验，实现了高效且性能优越的运动分割。
...
Method: 采用双流多模态扩散变换器架构：一个分支合成视频，另一个分支生成时间对齐的音频，共享基于多模态大语言模型的文本编码器。使用通道拼接公式统一各种修复风格任务，并通过联合生成低分辨率全序列和高分辨率关键帧的策略提高效率。

Result: SkyReels V4支持1080p分辨率、32FPS、15秒时长的高保真多镜头电影级视频生成，能够同时处理多模态输入、联合视频音频生成，以及生成、修复和编辑的统一处理。

Conclusion: SkyReels V4是第一个同时支持多模态输入、联合视频音频生成以及生成、修复和编辑统一处理的视频基础模型，在电影级分辨率和时长下保持了强大的效率和质量。
...
### [56] [Off-The-Shelf Image-to-Image Models Are All You Need To Defeat Image Protection Schemes](https://arxiv.org/abs/2602.22197)
*Xavier Pleimling,Sifat Muhammad Abdullah,Gunjan Balde,Peng Gao,Mainack Mondal,Murtuza Jadliwala,Bimal Viswanath*

Main category: cs.CV

TL;DR: 研究表明，现成的图像生成AI模型可以通过简单文本提示被重新用作通用"去噪器"，有效移除多种图像保护扰动，暴露出当前图像保护方案存在广泛漏洞。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的进步催生了各种图像保护策略，通过添加不可察觉的扰动来防止未经授权的使用。虽然之前的攻击需要专门的定制方法，但研究者发现这种限制已不复存在。

Method: 将现成的图像到图像生成AI模型重新用作通用"去噪器"，仅使用简单的文本提示，即可移除多种保护扰动。该方法在8个案例研究中测试了6种不同的保护方案。

Result: 这种通用攻击不仅成功规避了所有测试的防御方案，而且在性能上超越了现有的专门攻击方法，同时保持了图像对攻击者的可用性。

Conclusion: 研究揭示了当前图像保护领域存在广泛且严重的漏洞，许多方案提供了虚假的安全感。迫切需要开发更强大的防御机制，并确保未来的保护方案必须针对现成生成AI模型的攻击进行基准测试。
...
TL;DR: 提出LGI映射作为新的表示方法，从单目深度编码光感知遮挡，为生成模型提供物理启发的先验，实现联合阴影生成和重光照的统一流程。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型缺乏对光-几何交互的物理约束，导致产生漂浮阴影、不一致光照和不合理阴影几何等问题。需要一种能捕捉光与几何本质交互的表示方法。

Method: 提出LGI映射，从现成的2.5D深度图预测中可靠准确地计算光感知遮挡。将LGI嵌入桥接匹配生成主干中，减少歧义并强制执行物理一致的光-阴影推理。构建首个大规模联合阴影和重光照基准数据集。

Result: 实验显示在合成和真实图像上实现了显著的真实感和一致性提升。LGI成功将几何启发渲染与生成建模相结合。

Conclusion: LGI映射为生成模型提供了物理启发的先验约束，实现了高效、物理一致的阴影生成和重光照，解决了现有方法中光-阴影交互不一致的问题。
...
Result: 通过大量人工标注确保评估与人类判断一致，提供了首个基于指令的多镜头视频任务评估框架，能够公平、可扩展、可重复地比较统一视频模型。

Conclusion: UniVBench填补了视频基础模型评估的空白，为衡量视频基础模型的综合能力提供了首个框架，能够加速稳健视频智能的发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的水印方法通常采用单随机失真训练策略，在每个训练批次中随机选择一种失真作为噪声层。但这种方法将不同失真类型视为独立处理，忽略了失真间的内在联系，导致批次间的优化冲突，限制了水印模型的鲁棒性和泛化能力。

Method: 提出Meta-FC训练策略：1）元学习框架：从噪声池中随机采样多种失真构建元训练任务，同时保留一种失真作为模拟"未知"失真用于元测试阶段；2）特征一致性损失：约束同一图像经过不同失真解码后的特征保持一致性，促进稳定激活转化为失真不变表示。

Result: 相比SRD训练策略，Meta-FC在多种水印模型上平均提升了1.59%（高强度失真）、4.71%（组合失真）和2.38%（未知失真）的鲁棒性和泛化性能。

Conclusion: Meta-FC训练策略通过元学习和特征一致性有效解决了传统SRD策略的优化冲突问题，显著提升了水印模型在各种失真条件下的鲁棒性和泛化能力。
...
TL;DR: 提出GFPL框架解决联邦学习中知识融合低效和通信开销大的问题，通过原型生成与聚合策略，在数据不平衡场景下提升3.6%准确率同时保持低通信成本。


<details>
  <summary>Details</summary>
Motivation: 传统联邦学习面临两个关键挑战：1）模型更新偏向多数类特征导致知识融合低效；2）频繁传输高维模型参数造成通信开销过大。受人类大脑高效知识整合启发，需要解决这些实际部署中的问题。
...
Conclusion: GFPL框架有效解决了联邦学习中的知识融合和通信开销问题，通过原型生成与聚合策略实现了高效的知识整合，在保持低通信成本的同时显著提升了模型性能，特别是在数据不平衡场景下表现优异。
...
TL;DR: 本文提出了Memorability Feedback任务，开发了MemCoach方法来提供可操作的自然语言建议以提升图像记忆性，并创建了MemBench基准进行评估。


<details>
  <summary>Details</summary>
Motivation: 传统图像记忆性研究要么是被动预测任务，要么是生成方法修改视觉输入，但都不支持用户在拍摄时提升照片记忆性。需要一种能够在拍摄时提供可操作指导的方法。

Method: 提出MemCoach方法，基于多模态大语言模型，采用教师-学生引导策略，将模型内部激活与从低到高记忆性样本学习的教师模型对齐，无需训练即可提供自然语言改进建议。

Result: 实验表明MemCoach在多个MLLMs上表现优于多个零样本模型，能有效提供可操作的记忆性改进建议。
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜手术机器人的精确深度估计对于3D重建和安全器械导航至关重要。现有自监督深度估计方法在弱纹理和可变光照环境下性能下降，导致稀疏重建和无效深度估计。深度补全技术可以缓解这些问题，但在内窥镜领域的应用仍然有限。

Method: 提出EndoDDC方法，整合图像、稀疏深度信息与深度梯度特征，通过扩散模型优化深度图，专门解决内窥镜环境中的弱纹理和光反射问题。
...
TL;DR: TIRAuxCloud是一个多模态热红外数据集，用于解决昼夜云检测的挑战，包含热红外、光学和近红外波段数据，以及辅助信息层，支持监督和迁移学习基准测试。


<details>
  <summary>Details</summary>
Motivation: 云层是地球观测的主要障碍，限制了火灾响应、城市热岛监测等关键遥感应用的可用性。可见光和近红外波段在白天有效，但依赖太阳光照，不适合夜间监测。热红外波段在夜间云检测中起关键作用，但准确检测仍面临光谱信息有限和空间分辨率低的挑战。

Method: 提出了TIRAuxCloud多模态数据集，包含Landsat和VIIRS的热红外、光学和近红外波段数据，结合高程、土地覆盖、气象变量和无云参考图像等辅助信息层。数据集包含大量自动云掩膜样本和少量手动标注子集，以减少地表-云模糊性和云形成不确定性。

Result: 建立了全面的基准测试，通过监督学习和迁移学习展示了数据集在推进昼夜云检测方法发展方面的价值。数据集能够帮助减少表面-云模糊性，提高云检测的准确性。

Conclusion: TIRAuxCloud数据集通过整合多光谱数据和辅助信息层，为解决昼夜云检测的挑战提供了有价值的资源，有助于开发创新的云检测方法，特别是在夜间条件下。
...
TL;DR: 提出"geometry-as-context"方法，通过自回归相机控制视频生成模型，结合几何估计和3D场景渲染，实现场景一致的视频生成


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部内存或迭代3D重建和修复，在推理过程中会因中间输出错误、不可微分过程和分离模型而累积误差，需要更有效的方法来保持场景一致性

Method: 引入"geometry-as-context"框架，使用自回归相机控制视频生成模型迭代执行：(1)估计当前视图的几何信息用于3D重建，(2)模拟和恢复3D场景渲染的新视图图像；开发相机门控注意力模块以有效利用相机姿态；训练时使用文本上下文确定生成几何还是RGB图像；推理时通过随机丢弃几何上下文实现仅RGB输出

Result: 在单向和往返轨迹的场景视频生成测试中，该方法在保持场景一致性和相机控制方面优于先前方法

Conclusion: 提出的"geometry-as-context"方法通过几何信息作为上下文，结合多任务框架和相机门控注意力，有效解决了场景一致视频生成中的误差累积问题，实现了更好的场景一致性和相机控制
...
TL;DR: 提出D-ODR方法，通过构建有向图和多尺度扩散来建模糖尿病视网膜病变的单向进展特性，防止特征表示中出现生物上不可逆的反向转换
...
Method: 提出定向序数扩散正则化(D-ODR)，通过构建进展约束有向图严格强制执行前向疾病演化，在该有向结构上进行多尺度扩散，对有效进展路径上的分数反转施加惩罚

Result: 大量实验表明，D-ODR相比最先进的序数回归和DR特定分级方法具有更优的分级性能，提供更临床可靠的疾病严重程度评估

Conclusion: D-ODR通过显式建模特征空间为有向流，将特征表示与DR恶化的自然轨迹对齐，防止学习生物不一致的反向转换，提高了糖尿病视网膜病变分级的临床可靠性
...
TL;DR: 本文提出了一种轻量级深度学习框架，使用MobileNetV3架构和CORAL头，用于从眼底图像中自动评估糖尿病视网膜病变严重程度，在资源受限环境下实现高效筛查。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力损害的主要原因，但人工诊断耗时且容易出错，导致筛查延迟。需要开发高效、自动化的诊断工具，特别是在资源受限的环境中。

Method: 采用MobileNetV3架构结合CORAL头来建模疾病的有序进展，使用预处理流程包括圆形裁剪和光照归一化，在APTOS 2019和IDRiD数据集上进行训练和验证，采用3折交叉验证和消融研究。

Result: 模型取得了0.9019的二次加权Kappa分数和80.03%的准确率。通过模型校准减少过度自信，并针对移动设备进行优化，解决了实际部署挑战。

Conclusion: 提出的系统为早期糖尿病视网膜病变筛查提供了一个可扩展且实用的工具，特别适用于资源受限环境，能够提高筛查效率和准确性。
...
TL;DR: MVGFDR是一个用于糖尿病视网膜病变分级的端到端多视图图融合框架，通过显式解耦共享和视图特定的视觉特征，在大型多视图眼底图像数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因之一，早期准确分级对及时干预至关重要。临床实践中使用多视图眼底图像进行检测，但现有深度学习方法在融合多视图图像时往往忽略视图间的相关性，未能充分利用来自同一患者的不同视图之间的内在一致性。
...
TL;DR: MindDriver是一个渐进式多模态推理框架，通过模仿人类渐进式思维解决自动驾驶中的VLM推理问题，在nuScenes和Bench2Drive评估中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在自动驾驶中的推理策略面临挑战：文本CoT与轨迹物理空间存在语义鸿沟，而基于未来图像的CoT方法缺乏明确的规划导向目标指导，难以生成准确的场景演化图像。

Method: 提出MindDriver渐进式多模态推理框架，包含语义理解、语义到物理空间想象、物理空间轨迹规划三个模块；开发反馈引导的自动数据标注管道生成对齐的多模态推理训练数据；采用渐进式强化微调方法通过高层次奖励学习优化对齐。

Result: MindDriver在nuScenes开环评估和Bench2Drive闭环评估中均表现出优越性能。

Conclusion: MindDriver通过渐进式多模态推理框架有效解决了VLM在自动驾驶中的推理挑战，实现了语义空间与物理空间的对齐，为端到端自动驾驶系统提供了有效的解决方案。
...
Method: 提出GLoTran框架，整合低分辨率全局图像和多尺度区域级文本图像切片，采用指令引导对齐策略，使MLLM能够保持场景级上下文一致性同时捕捉细粒度文本细节。还构建了GLoD数据集，包含51万高分辨率全局-局部图像-文本对。

Result: 大量实验表明，GLoTran在翻译完整性和准确性方面显著优于最先进的MLLM，为高分辨率文本丰富条件下的细粒度TIMT提供了新范式。

Conclusion: GLoTran通过全局-局部双视觉感知框架有效解决了高分辨率文本丰富图像机器翻译的挑战，实现了更好的上下文一致性和文本细节保留。
...
<details>
  <summary>Details</summary>
Motivation: LoRA技术虽然促进了文本到图像扩散模型的高效微调和共享，但其模块化、即插即用的特性也扩大了攻击面。作者旨在揭示LoRA生态系统中的安全风险，展示攻击者如何利用独立LoRA模块作为攻击载体，隐蔽地注入恶意行为。

Method: 提出Masquerade-LoRA攻击框架：冻结基础模型参数，仅更新低秩适配器权重，使用少量"触发词-目标图像"对训练独立的恶意LoRA模块。该模块嵌入隐藏的跨模态映射关系，当加载模块并提供特定文本触发词时，模型产生预定义视觉输出；否则与良性模型行为无异。

Result: 实验结果表明，MasqLoRA能以最小资源开销进行训练，攻击成功率高达99.8%。该攻击框架揭示了AI供应链中的严重且独特的威胁，攻击隐蔽性强，难以被检测。

Conclusion: MasqLoRA暴露了LoRA中心化共享生态系统的安全漏洞，强调了为这一生态系统开发专门防御机制的紧迫性。研究呼吁关注AI模型共享中的供应链安全问题。
...
TL;DR: PanoEnv是一个用于评估视觉语言模型在360度全景图像上3D空间推理能力的大规模VQA基准，包含14.8K问题。现有VLMs在该基准上表现不佳（总体准确率49.34%），作者提出基于强化学习的后训练框架，通过几何感知奖励和两阶段课程学习提升模型性能，7B模型达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在处理等距柱状投影的360度全景图像时，由于几何失真和有限的3D监督，在3D空间推理方面存在困难。需要专门的基准和训练方法来提升模型在全方位感知中的3D空间智能。

Method: 1) 构建PanoEnv基准：从合成3D环境中创建包含14.8K问题的大规模VQA数据集，涵盖五个类别（相对位置、体积比较等），并包含深度、分割和边界框等精确3D标注。2) 提出基于强化学习的后训练框架：使用组相对策略优化（GRPO）和包含五种几何感知策略（距离容差、空间一致性等）的真实引导奖励。3) 采用两阶段课程学习：第一阶段在结构化任务（真/假和多项选择）上训练，第二阶段在混合开放式数据上微调以提升泛化能力。

Result: 1) 基准测试14个SOTA VLMs显示有限的3D理解能力：总体准确率49.34%，开放式问题准确率仅8.36%。2) 提出的7B模型达到新SOTA：总体准确率提升至52.93%（+3.59%），开放式问题准确率提升至14.83%，同时保持结构化任务性能。3) 语义评估得分最高：Q-Score 6.24，P-Score 5.95，超越32B模型。

Conclusion: PanoEnv-QA基准和基于课程的强化学习框架能够有效提升视觉语言模型在全方位感知中的3D空间智能。该方法通过几何感知奖励和渐进式训练策略，使较小模型（7B）在3D空间推理任务上超越更大模型（32B），为虚拟现实、自动驾驶和机器人等应用提供了重要技术基础。
...
TL;DR: RobustVisRAG是一个因果引导的双路径框架，通过分离语义和退化因素来提升视觉检索增强生成在图像失真条件下的鲁棒性，同时保持效率和零样本泛化能力。
...
Method: 提出RobustVisRAG框架，包含非因果路径（通过单向注意力捕获退化信号）和因果路径（在退化信号引导下学习纯净语义），结合非因果失真建模和因果语义对齐目标，实现语义与退化的清晰分离。

Result: 在真实世界退化条件下，RobustVisRAG将检索、生成和端到端性能分别提升了7.35%、6.35%和12.40%，同时在干净输入上保持可比精度。还创建了Distortion-VisRAG数据集作为大规模基准。

Conclusion: RobustVisRAG通过因果引导的双路径设计有效解决了视觉退化对VisRAG性能的影响，在保持效率的同时显著提升了模型在失真条件下的鲁棒性，为实际应用提供了可靠解决方案。
...
TL;DR: Olbedo是一个大规模航拍数据集，用于户外场景的本征图像分解（反照率-阴影分解），包含5,664张无人机图像，提供多视角一致的反照率、阴影图、深度、法线等标注，通过逆渲染流程生成，显著提升了户外IID模型的性能。
...
Method: 1. 构建Olbedo数据集：包含5,664张无人机图像，覆盖四种景观类型、多年份和多样化光照条件；2. 通过多视角立体重建和校准天空照明的逆渲染精化流程生成标注；3. 提供多视角一致的反照率图、阴影图、度量深度、表面法线、太阳和天空阴影分量、相机位姿等；4. 使用该数据集微调基于扩散的IID模型。

Result: 1. 在MatrixCity基准测试中，使用Olbedo微调显著改善了单视角户外反照率预测性能；2. 展示了Olbedo训练模型在多视角一致重光照、材质编辑和城市数字孪生场景变化分析中的应用；3. 发布了数据集、基线模型和评估协议。

Conclusion: Olbedo数据集填补了户外本征图像分解领域的数据空白，使原本在合成室内数据上训练的先进IID模型能够泛化到真实户外图像，为户外本征分解和光照感知的航拍视觉研究提供了重要支持。
...
TL;DR: 提出RT-RMOT任务和RTrack框架，通过RGB-热成像融合和强化学习优化，实现全天候的指代多目标跟踪


<details>
  <summary>Details</summary>
Motivation: 现有指代多目标跟踪在低可见度条件下（如夜间、烟雾等）表现受限，需要融合热成像模态的照明鲁棒性来实现全天候跟踪

Method: 1) 构建首个RGB-热成像指代多目标跟踪数据集RefRT；2) 提出基于多模态大语言模型的RTrack框架；3) 引入组序列策略优化(GSPO)和裁剪优势缩放(CAS)策略；4) 设计结构化输出奖励和综合检测奖励

Result: 在RefRT数据集上的大量实验证明了RTrack框架的有效性，该数据集包含388个语言描述、1,250个跟踪目标和166,147个语言-RGB-热成像三元组

Conclusion: 通过融合RGB外观特征和热成像模态的照明鲁棒性，提出的RT-RMOT任务和RTrack框架能够实现全天候的指代多目标跟踪，为相关研究提供了新的数据集和方法
...
TL;DR: AdaSpot框架通过自适应选择每帧中最具信息量的感兴趣区域进行高分辨率处理，同时处理低分辨率视频提取全局特征，在精确事件定位任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常均匀处理所有帧，忽略了视频数据固有的时空冗余性，导致在非信息区域进行冗余计算，同时为了保持可处理性而进行空间下采样，丢失了对精确定位至关重要的细粒度细节。

Method: 提出AdaSpot框架：处理低分辨率视频提取全局任务相关特征，同时通过无监督、任务感知策略自适应选择每帧中最具信息量的感兴趣区域进行高分辨率处理，保持跨帧的时空一致性。

Result: 在标准PES基准测试中，AdaSpot在严格评估指标下实现了最先进的性能（例如在Tennis和FineDiving上分别提升+3.96和+2.26 mAP@0帧），同时在较宽松指标下也保持强劲结果。

Conclusion: AdaSpot通过自适应选择感兴趣区域进行高分辨率处理，在保持计算效率的同时保留了关键的细粒度视觉线索，为精确事件定位提供了一种简单而有效的解决方案。
...
TL;DR: LFG是一个无需标注、教师引导的框架，直接从无姿态视频中学习自动驾驶表示，通过多模态监督信号联合预测当前和未来的点云图、相机姿态、语义分割和运动掩码，在NAVSIM基准测试中超越多相机和LiDAR基线。


<details>
  <summary>Details</summary>
Motivation: 在线可用的以自我为中心的驾驶视频为自动驾驶提供了丰富的视觉数据源，但缺乏标注使得难以学习同时捕捉语义结构和3D几何的表示。现有自监督方法主要关注帧间一致性，而安全和反应性驾驶关键依赖于时间上下文。

Method: 提出标签自由、教师引导的框架，使用前馈架构配备轻量级自回归模块，通过多模态教师提供序列级伪监督，联合预测当前和未来的点云图、相机姿态、语义分割和运动掩码，从原始YouTube视频学习统一的伪4D表示。

Result: LFG编码器在NAVSIM基准测试中有效转移到下游自动驾驶规划任务，仅使用单个单目相机就超越了多相机和LiDAR基线，同时在语义、几何和运动预测任务上表现出色。

Conclusion: LFG作为一个具有几何和运动感知能力的视频中心基础模型，为自动驾驶提供了有前景的解决方案，能够从无标注视频中学习统一的时空表示。
...
TL;DR: CXR-LT 2026挑战赛引入多中心数据集解决胸部X光图像的长尾分布和开放世界问题，包含两个任务：已知30类疾病的多标签分类和6类未见罕见疾病的开放世界泛化，最佳方案通过大规模视觉语言预训练显著提升零样本诊断性能。
...
Method: 提出CXR-LT 2026挑战赛，使用来自PadChest和NIH Chest X-ray数据集的超过145,000张图像构建多中心数据集，定义两个核心任务：已知30类疾病的多标签分类和6类未见罕见疾病的开放世界泛化。

Result: 最佳解决方案在任务1上获得0.5854的mAP，在任务2上获得0.4315的mAP，表明大规模视觉语言预训练显著缓解了通常与零样本诊断相关的性能下降。

Conclusion: CXR-LT 2026挑战赛为解决胸部X光图像的长尾分布和开放世界问题提供了有效基准，大规模视觉语言预训练是提升零样本诊断性能的关键技术。
...
TL;DR: WeatherCity是一个用于4D城市场景重建和天气编辑的框架，通过文本引导的图像编辑、天气高斯表示和物理驱动模型，实现了高保真、时间一致且可控的天气模拟。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶场景重建方法只能复制观察到的场景，缺乏多样天气模拟能力；而图像级天气编辑方法容易产生场景伪影且可控性差，需要一种能够实现高保真4D场景重建和灵活天气编辑的解决方案。

Method: 1) 使用文本引导的图像编辑模型实现图像天气背景的灵活编辑；2) 提出基于共享场景特征和专用天气解码器的天气高斯表示；3) 引入内容一致性优化确保不同天气条件下的连贯建模；4) 设计物理驱动模型通过粒子和运动模式模拟动态天气效果。

Result: 在多个数据集和各种场景上的实验表明，WeatherCity在4D重建和天气编辑中实现了灵活可控性、高保真度和时间一致性，能够精细控制天气条件（如小雨和暴雪），并支持场景内的物体级操作。

Conclusion: WeatherCity框架成功解决了4D城市场景重建和天气编辑的挑战，为自动驾驶的端到端训练和闭环仿真提供了可编辑的高保真4D场景，具有灵活可控性、高保真度和时间一致性等优势。
...
TL;DR: Brain3D：针对3D脑肿瘤MRI的放射学报告自动生成框架，通过三阶段对齐方法将预训练的2D医学编码器扩展为原生3D架构，显著优于2D基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型处理体积脑MRI时使用2D切片近似方法，破坏了空间上下文，而准确的神经放射学解释需要完整的3D空间信息。需要专门针对神经放射学的3D模型，其中半球偏侧性、肿瘤浸润模式和解剖定位至关重要。

Method: 开发Brain3D框架：1）将预训练的2D医学编码器扩展为原生3D架构；2）通过三阶段渐进对齐：对比性基础建立视觉-文本对应关系，监督投影器预热稳定条件，LoRA适应将输出从冗长描述转换为结构化临床报告。

Result: 在468个受试者（BraTS病理病例加健康对照）上评估，模型达到临床病理F1分数0.951，而强2D基线为0.413，同时在健康扫描上保持完美的特异性。三阶段对齐被证明至关重要。

Conclusion: Brain3D为神经放射学提供了专门化的3D视觉语言框架，通过渐进式对齐方法显著提高了脑肿瘤MRI报告生成的准确性和临床实用性，优于传统的2D切片方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型输出缺乏地理多样性，强化刻板印象，错误呈现地区特征。现有多样性指标依赖人工标注数据集或仅关注表面视觉相似性，解释性有限，需要更系统、可解释的评估框架。

Method: 提出GeoDiv框架，利用大语言模型和视觉语言模型从两个互补维度评估地理多样性：1) 社会经济视觉指数(SEVI)，捕捉经济和条件相关线索；2) 视觉多样性指数(VDI)，测量主要实体和背景的变化。应用于Stable Diffusion和FLUX.1-dev等模型生成的图像，覆盖10个实体和16个国家。

Result: GeoDiv揭示了模型一致缺乏多样性，并识别出模型默认偏见的具体细粒度属性。印度、尼日利亚、哥伦比亚等国家的描绘不成比例地呈现贫困和破旧状态，反映了潜在的社会经济偏见。

Conclusion: 生成模型需要更大的地理细微差别。GeoDiv提供了首个系统、可解释的测量此类偏见的框架，是迈向更公平、更包容生成系统的一步。
...
TL;DR: WeaveTime是一个针对视频流处理的时间感知框架，通过时序重建目标训练模型理解时间顺序，并在推理时使用动态焦点缓存机制，在保持模型架构不变的情况下显著提升视频流处理性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在处理视频流时存在时间不可知性（Time-Agnosticism）问题，将视频视为无序的证据集合而非因果有序序列，导致时序顺序模糊和过去-当前焦点盲区两大缺陷，无法适应流式场景中帧按序到达且无法访问未来观测的限制。

Method: WeaveTime框架包含两个核心组件：1）轻量级时序重建目标（Streaming Order Perception增强），通过最小化微调和无需专门流式数据的方式注入时序感知表示；2）过去-当前动态焦点缓存，在推理时执行不确定性触发的粗到细检索，仅在需要时扩展历史信息。

Result: 在代表性流式基准测试中，WeaveTime在保持现有Video-LLM架构不变的情况下，实现了准确率的持续提升并降低了延迟，为严格在线、时间因果约束下的时间感知流式视频-LLM提供了实用路径。

Conclusion: WeaveTime通过"先教时序，再用时序"的简单高效框架，解决了当前视频-LLM的时间不可知性问题，为流式视频理解提供了模型无关的解决方案，在准确性和延迟方面均取得显著改进。
...
TL;DR: MedTri是一个用于医学视觉语言预训练的可部署文本规范化框架，将自由文本报告转换为统一的[解剖实体:放射学描述+诊断类别]三元组结构，提高预训练质量。


<details>
  <summary>Details</summary>
Motivation: 医学报告作为大规模监督信号存在风格异质性、长度可变和图像无关内容等问题，现有文本规范化方法的设计原则和实际影响缺乏系统研究。

Method: 提出MedTri框架，将自由文本医学报告转换为结构化的[解剖实体:放射学描述+诊断类别]三元组，保留关键形态和空间信息，去除风格噪声和图像无关内容。

Result: 在多个X射线和CT数据集上，基于解剖的文本规范化相比原始报告和现有基线方法带来一致改进，支持模块化文本增强策略，提高鲁棒性和泛化能力。
...
TL;DR: CoLoGen是一个统一的扩散框架，通过渐进式学习解决图像生成中概念理解与空间定位的表示冲突问题，在编辑、可控生成和定制化生成任务上表现出色。
...
TL;DR: CASR提出循环超分辨率框架，通过序列化尺度转换解决任意尺度超分辨率的分布偏移问题，使用单模型实现稳定推理
...
Conclusion: CASR通过循环框架和专门设计的模块有效解决了任意尺度超分辨率中的分布偏移问题，使用单模型即可实现稳定推理和高质量结果
...
TL;DR: 该论文提出了一种区域级混合编码器，用于处理计算病理学中的多分辨率特征，通过混合放大倍率的基础模型联合融合图像切片表示，以减少每张切片所需表示数量并提高预测性能。


<details>
  <summary>Details</summary>
Motivation: 当前计算病理学工作流主要使用20倍放大倍率的切片，但某些组织学特征需要更大的上下文窗口，且20倍放大导致每张切片产生大量切片表示（可达千兆像素级别）。为了更准确地捕捉多分辨率特征并减少每张切片的表示数量。

Method: 提出区域级混合编码器，通过掩码嵌入建模预训练步骤联合融合混合放大倍率基础模型的图像切片表示。探索了混合放大倍率区域聚合器的预训练设计空间。

Result: 在代表不同癌症类型的生物标志物预测任务上评估模型，结果显示癌症依赖性的预测性能改进，突出了空间上下文和理解的重要性。

Conclusion: 混合放大倍率的区域级编码方法能够更好地捕捉多分辨率组织学特征，减少每张切片的表示数量，并在癌症生物标志物预测任务中显示出性能改进，证明了空间上下文理解在计算病理学中的重要性。
...
Method: 1) 开发了多玩家数据系统，支持协调的多智能体交互和同步的视频+动作捕获；2) 收集了1264万帧多玩家数据；3) 提出了多玩家运动、记忆、基础、构建和视角一致性的评估框架；4) 采用分阶段训练管道，从单玩家逐步过渡到多玩家建模，结合双向、因果和Self Forcing训练；5) 引入Checkpointed Self Forcing，这是一种内存高效的Self Forcing变体，支持更长时间跨度的教师模型。
...
TL;DR: WHOLE方法从第一人称视角视频中联合重建手部和物体在世界空间中的运动，通过生成式先验处理交互过程中的遮挡和物体进出视野问题，显著优于单独处理手部和物体的方法。


<details>
  <summary>Details</summary>
Motivation: 第一人称操作视频存在严重遮挡和物体频繁进出视野的问题，现有方法单独恢复手部或物体姿态在交互时效果不佳，且独立预测导致手物关系不一致。

Method: 提出WHOLE方法，学习手物运动的生成式先验来联合推理交互关系，在测试时引导预训练先验生成符合视频观测的轨迹。

Result: 在手部运动估计、6D物体姿态估计及其相对交互重建方面达到最先进性能，显著优于分别处理手部和物体再进行后处理的方法。

Conclusion: 通过联合生成式重建方法，WHOLE能够更好地处理第一人称视频中的手物交互问题，特别是在遮挡和物体不可见情况下表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态3D物体表面重建方法存在局限性：增量优化方法容易产生漂移且运行时间长；基于学习的复杂模型需要类别特定训练。需要一种快速、无漂移、无需训练的方法来处理长序列动态表面重建。

Method: 提出预条件潜在网格编码方法，将整个时间序列的变形编码到多分辨率潜在网格中，参数化基于关键帧表面的位置和法线方向。通过时间调制增强，使用轻量级MLP解码为每帧的6自由度变形。采用Sobolev预条件技术在梯度训练中优化潜在空间。

Result: 在多种人类和动物数据集上实验表明，Neu-PiG优于现有最先进方法，提供更高的精度和长序列可扩展性。比无训练方法快60倍以上，推理速度与重型预训练模型相当，能在数秒内完成高保真、无漂移的表面重建。

Conclusion: Neu-PiG通过创新的预条件潜在网格编码方法，实现了快速、准确、无漂移的动态3D表面重建，无需显式对应关系或先验知识，在精度和速度方面均优于现有方法，特别适合长序列处理。
...
TL;DR: Latent Context Compilation (LCC) 提出了一种新的长上下文处理框架，通过可丢弃的LoRA模块作为编译器，将长上下文蒸馏为紧凑的缓冲区令牌，实现16倍压缩比，无需合成QA数据，保持模型参数不变。


<details>
  <summary>Details</summary>
Motivation: 当前长上下文LLM部署面临两难困境：摊销压缩方法在分布外泛化方面表现不佳，而测试时训练方法需要高昂的合成数据成本，且需要修改模型权重，导致状态参数难以并发服务。

Method: 提出Latent Context Compilation框架，将上下文处理从适应转向编译。使用可丢弃的LoRA模块作为编译器，将长上下文蒸馏为紧凑的缓冲区令牌。引入自对齐优化策略，通过上下文无关的随机查询正则化上下文重建任务，使压缩令牌保持在模型现有指令遵循流形内。

Result: 在Llama-3.1-8B上的实验表明，Latent Context Compilation能够保留细粒度细节和推理能力，在先前方法失效的情况下仍能有效工作，实现了内存密度与模型参数的解耦，达到16倍压缩比。

Conclusion: Latent Context Compilation为长上下文LLM部署提供了一种新范式，通过编译而非适应的方法，实现了无状态、可移植的内存工件，解决了现有方法的局限性。
...
### [90] [AngelSlim: A more accessible, comprehensive, and efficient toolkit for large model compression](https://arxiv.org/abs/2602.21233)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大模型在实际工业部署中面临的计算资源消耗大、推理延迟高、内存占用多等问题，需要一套全面的压缩工具来降低模型部署成本并提升推理效率。

Method: 1. 量化技术：集成FP8和INT8后训练量化算法，并研究超低位量化，推出首个工业可用的2位大模型HY-1.8B-int2
2. 推测解码：提出训练对齐的推测解码框架，兼容多模态架构和现代推理引擎
3. 稀疏注意力：开发无需训练的稀疏注意力框架，通过静态模式和动态token选择的混合方法解耦稀疏核与模型架构
4. 多模态剪枝：针对多模态模型开发IDPruner优化视觉token，Samp进行自适应音频token合并与剪枝

Result: 1. 量化方面：实现了超低位量化，推出首个工业可用的2位大模型
2. 推测解码：在不影响输出正确性的前提下，实现了1.8-2.0倍的吞吐量提升
3. 稀疏注意力：在长上下文场景中显著降低了首token生成时间
4. 工具包整体：提供了从算法研究到工具辅助部署的完整解决方案

Conclusion: AngelSlim通过整合多种先进的压缩技术，为大模型的高效工业部署提供了全面的解决方案，既支持算法研究又便于实际部署，显著提升了模型推理效率和资源利用率。
...
### [91] [Group Orthogonalized Policy Optimization:Group Policy Optimization as Orthogonal Projection in Hilbert Space](https://arxiv.org/abs/2602.21269)
*Wang Zixian*

Main category: cs.LG

TL;DR: GOPO是一种基于希尔伯特函数空间几何的新型大语言模型对齐算法，通过将优化问题从概率单纯形提升到L2空间，实现线性正交约束和闭式稀疏解，避免传统KL散度的指数曲率问题。


<details>
  <summary>Details</summary>
Motivation: 传统对齐方法在概率单纯形上优化，继承了KL散度的指数曲率问题，导致梯度饱和和训练不稳定。需要一种更几何化的方法来改善对齐算法的稳定性和泛化能力。

Method: 将对齐问题从概率单纯形提升到希尔伯特空间L2(π_k)，将单纯形约束转化为线性正交条件。通过希尔伯特投影定理得到闭式解，引入边界条件v≥-1产生精确稀疏性。通过组采样将无限维L2空间投影到有限经验子空间，利用组归一化优势的零和特性消除拉格朗日乘子。

Result: 在数学推理基准测试中，GOPO实现了具有竞争力的泛化性能，同时保持了稳定的梯度动态和熵保持特性。在基于裁剪的方法达到平台期时，GOPO仍能持续改进。

Conclusion: GOPO提供了一种基于希尔伯特空间几何的新型对齐框架，通过线性正交约束、闭式稀疏解和恒定Hessian曲率，解决了传统方法中的梯度饱和和训练不稳定问题，为语言模型对齐提供了更稳定和可解释的优化方法。
...
TL;DR: cVMDx是一种改进的基于扩散模型的轨迹预测框架，通过DDIM采样实现100倍推理加速，使用高斯混合模型提供多模态预测，在highD数据集上表现出更高精度和效率。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的轨迹预测面临多智能体交互、场景多样性和未来运动随机性等挑战。现有扩散模型方法如cVMD存在采样慢、生成多样性利用有限和场景编码脆弱等问题。

Method: 提出cVMDx框架：1) 采用DDIM采样加速推理；2) 使用拟合的高斯混合模型从生成轨迹中提取可处理的多模态预测；3) 评估CVQ-VAE变体进行场景编码。

Result: 在highD数据集上的实验表明，cVMDx相比cVMD实现了高达100倍的推理时间减少，提高了准确性，支持完全随机、多模态的轨迹预测。

Conclusion: cVMDx通过改进采样效率和预测多样性，为自动驾驶提供了更实用、鲁棒的多模态轨迹预测解决方案，平衡了精度与计算效率。
...
TL;DR: Tool-R0框架通过自博弈强化学习从零开始训练通用工具调用智能体，无需预先任务数据，实现智能体的自我进化


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的智能体训练通常依赖精心构建的任务-解决方案对和大量人工监督，这限制了智能体向超级智能系统的开放式自我进化。需要一种无需预训练数据的自主进化方法。
...
Conclusion: Tool-R0框架展示了从零开始训练通用工具调用智能体的可行性，通过自博弈强化学习实现无需预训练数据的自我进化，为分析自博弈大语言模型智能体的协同进化、课程动态和扩展行为提供了实证见解。
...
TL;DR: 该论文提出了一种动态对称点估计方法，用于解决模拟内存计算设备中权重更新的系统性漂移问题，无需昂贵的预校准过程。


<details>
  <summary>Details</summary>
Motivation: 模拟内存计算设备存在更新不对称性，导致权重更新向设备特定的对称点系统性漂移，这通常与训练目标的最优点不对齐。现有方法需要预先校准对称点为零，但校准成本高且残留误差会降低训练精度。

Method: 提出了动态对称点估计方法，在模型训练过程中跟踪对称点，并建立了收敛保证。还开发了基于数字信号处理中斩波和滤波技术的增强变体。

Result: 数值实验证明了所提方法的效率和有效性，首次理论表征了对称点校准的脉冲复杂度和估计误差。

Conclusion: 该研究提供了一种无需昂贵预校准的动态对称点估计方法，有效解决了模拟内存计算设备训练中的权重更新偏差问题，具有理论和实践价值。
...
TL;DR: 开发了一个交互式AI系统，用于技能评估，通过主动提问来准确获取技能信息，同时允许用户保持自己的表达风格，解决了传统简历中自我展示风格差异带来的评估偏差问题。


<details>
  <summary>Details</summary>
Motivation: 具有相似资质和技能的个人在自我展示风格上存在差异：有些人倾向于自我推销，而有些人则过于谦虚甚至遗漏关键信息。这使得对同等资质求职者的自我描述进行比较变得困难，因为自我展示风格会影响评估结果。

Method: 1. 构建交互式AI系统进行技能启发式评估，通过主动提问准确确定技能，同时允许个人保持自己的表达风格；2. 使用LLM生成合成人类数据以获得足够的训练数据；3. 通过数学上严格的公平性概念来应对系统性模型偏差，确保自我展示方式与技能评估误差之间的协方差很小。

Result: 开发了一个能够准确评估技能同时尊重个人表达风格的AI系统，该系统可以部署在专业社交平台新用户注册或公司重组时员工与需求匹配等场景中。

Conclusion: 该研究提出的交互式AI技能评估系统能够有效缓解因个人自我报告风格差异带来的内生性偏差，并通过数学上严格的公平性约束解决系统性模型偏差，为公平、准确的技能评估提供了新的技术方案。
...
TL;DR: 本文提出了一种高效的机遇性逼近算法，避免了在线校准的需求，实现了O(T^{-1/4})的收敛速率，并在对手动作集维度≤2时达到最优O(T^{-1/2})速率。


<details>
  <summary>Details</summary>
Motivation: 机遇性逼近是Blackwell逼近的推广，学习者在对手限制其动作空间子集时希望获得更强的保证。现有算法需要在线校准预测对手动作，这需要指数时间且收敛速率较差(T^{-O(1/d)})，因此需要更高效的算法。

Method: 提出了两种算法：1) 高效算法实现O(T^{-1/4})收敛速率；2) 非高效算法实现O(T^{-1/3})收敛速率。两种算法都绕过了在线校准子程序的需求。特别地，当对手动作集维度≤2时，算法能达到最优O(T^{-1/2})速率。

Result: 新算法显著改进了现有方法的性能：高效算法达到O(T^{-1/4})速率，非高效算法达到O(T^{-1/3})速率，都优于现有方法的T^{-O(1/d)}。在低维情况下(维度≤2)达到最优O(T^{-1/2})速率。

Conclusion: 本文通过绕过在线校准需求，为机遇性逼近问题提供了更高效的算法，显著改进了收敛速率，特别是在低维情况下达到了最优性能。
...
Result: 在合成数据集上，VCDF将VAR-LiNGAM的窗口和摘要F1分数提高了约0.08-0.12，在中等至长序列中效果最明显；在长度1000以上的时间序列上，绝对改进可达0.18；在模拟fMRI数据和IT监控场景中也表现出更好的稳定性和结构准确性。
...
### [98] [Defensive Generation](https://arxiv.org/abs/2602.21390)
*Gabriele Farina,Juan Carlos Perdomo*

Main category: cs.LG

TL;DR: 提出Defensive Generation方法，在线生成不可伪造的生成模型，适用于标量、多类和向量值结果，能处理无限测试类，达到最优收敛速率。


<details>
  <summary>Details</summary>
Motivation: 研究如何在线高效生成不可伪造的生成模型，这些模型在预定义的计算测试下无法被观测数据证伪，解决传统方法在处理非伯努利结果和无限测试类时的局限性。

Method: 通过扩展在线高维多校准与RKHS的联系，利用期望变分不等式问题的最新进展，开发高效算法。应用该算法框架实现Defensive Generation方法，在线生成结果不可区分的生成模型。

Result: Defensive Generation是首个能高效在线生成非伯努利结果的不可区分生成模型的方法，能处理无限测试类（包括高阶矩检验），在样本数量上具有近线性时间复杂度，达到最优的T^{-1/2}生成误差收敛速率。

Conclusion: 该方法为在线生成不可伪造的生成模型提供了高效解决方案，扩展了结果不可区分性理论的实际应用范围，在处理复杂数据类型和无限测试类方面具有显著优势。
...
### [99] [FedVG: Gradient-Guided Aggregation for Enhanced Federated Learning](https://arxiv.org/abs/2602.21399)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端数据异构会导致客户端漂移，降低模型整体泛化性能，而传统方法过度关注性能较差的客户端会加剧这一问题。

Method: 提出FedVG框架，使用公开数据集构建全局验证集，通过计算客户端模型在验证集上的层梯度范数来评估其泛化能力，生成客户端特定评分，指导自适应联邦聚合。

Result: 在自然图像和医学图像基准数据集上的实验表明，FedVG在高度异构环境下显著提升性能，且能无缝集成到各种先进FL算法中进一步改善结果。

Conclusion: FedVG通过全局验证集指导的梯度评估机制有效缓解了数据异构导致的客户端漂移问题，提高了联邦学习的泛化性能，具有模块化和易集成的优势。
...
### [100] [Overconfident Errors Need Stronger Correction: Asymmetric Confidence Penalties for Reinforcement Learning](https://arxiv.org/abs/2602.21420)
*Yuanda Xu,Hejian Sang,Zhengze Zhou,Ran He,Zhipeng Wang*

Main category: cs.LG

TL;DR: 本文提出ACE方法，通过非对称置信感知错误惩罚解决RLVR算法中的推理边界收窄和多样性下降问题，在多个数学推理基准上显著提升Pass@k性能。


<details>
  <summary>Details</summary>
Motivation: 标准RLVR算法存在一个已知问题：虽然通过锐化采样提高了Pass@1准确率，但同时收窄了模型的推理边界并降低了生成多样性。现有方法忽略了根本原因——对错误的统一惩罚，导致过度自信的错误持续存在并垄断概率质量，抑制了有效的探索轨迹。

Method: 提出非对称置信感知错误惩罚（ACE），引入每个rollout的置信度偏移度量c_i = log(pi_theta(y_i|x) / pi_ref(y_i|x))，动态调制负优势。理论上证明ACE梯度可分解为仅限于过度自信错误的选择性正则化器梯度，加上一个部分调节正则化器强度的特征化残差。

Result: 在Qwen2.5-Math-7B、Qwen3-8B-Base和Llama-3.1-8B-Instruct模型上使用DAPO-Math-17K数据集进行GRPO和DAPO微调。在MATH-500和AIME 2025基准测试中，ACE与现有方法无缝组合，在所有三个模型系列和基准测试中一致改善了完整的Pass@k谱。

Conclusion: ACE方法有效解决了RLVR算法中的推理边界收窄和多样性下降问题，通过非对称置信感知错误惩罚机制，显著提升了数学推理任务的性能，为强化学习在语言模型推理中的应用提供了新的解决方案。
...
### [101] [On the Structural Non-Preservation of Epistemic Behaviour under Policy Transformation](https://arxiv.org/abs/2602.21424)
*Alexander Galozy*

Main category: cs.LG

TL;DR: 该论文研究了强化学习智能体在部分可观测性下的行为依赖性，定义了ε-行为等价和内部探针敏感性度量，并证明了凸聚合下行为距离收缩的结构性结果。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测环境中，强化学习智能体通常基于内部积累的信息（如记忆或推断的潜在上下文）选择行动。本文旨在形式化这种信息条件化的交互模式，即行为依赖性，并研究其在常见策略变换下的结构特性。

Method: 首先形式化行为依赖性概念，定义ε-行为等价和探针相对的行为距离。然后建立三个结构性结果：1）非平凡行为依赖性的策略集在凸聚合下不封闭；2）行为距离在凸组合下收缩；3）提出倾斜混合目标梯度上升时行为距离减少的充分局部条件。通过最小化老虎机和部分可观测网格世界实验验证这些机制。

Result: 在研究的设置中，行为距离在凸聚合下和在倾斜潜在先验的持续优化下都会减少。实验表明，行为距离的减少先于潜在先验偏移下的性能退化。这些结果识别了探针条件化行为分离在常见策略变换下不被保留的结构条件。

Conclusion: 该研究为部分可观测强化学习中基于内部信息的行为依赖性提供了形式化框架，揭示了行为距离在凸聚合和优化过程中的收缩特性，有助于理解智能体在策略变换下行为一致性的保持机制。
...
TL;DR: 提出了一种安全过滤框架，为预训练的流式生成模型提供形式化安全保证，通过控制屏障函数和二次规划在采样过程中在线强制执行硬约束，实现100%约束满足同时保持语义保真度。
...
Method: 提出安全过滤框架作为预训练生成模型的在线防护罩。定义收缩安全管，从初始噪声分布的宽松约束逐步收紧到最终数据分布的目标安全集，通过控制屏障函数表征该管，在每个采样步骤通过凸二次规划合成反馈控制输入。

Result: 该机制保证安全采样，同时最小化每个采样步骤中与原始模型的分布偏移（通过KL散度量化的）。在约束图像生成、物理一致轨迹采样和安全机器人操作策略等任务中实现100%约束满足，同时保持语义保真度。

Conclusion: 该框架适用于任何预训练的流式生成方案，无需重新训练或架构修改，为生成模型在安全关键应用中的部署提供了形式化安全保障。
...
<details>
  <summary>Details</summary>
Motivation: 贝叶斯逆问题中精确后验采样计算成本过高，而现有近似后验分布虽然采样便宜但存在显著偏差，需要一种能消除偏差的高效采样方法

Method: 基于独立性Metropolis-Hastings框架，引入近端校正机制，通过求解辅助优化问题来调整近似后验分布的样本，在精确模型拟合和近似参考点稳定性之间取得平衡

Result: 理论证明近端校正能缩小近似后验与精确后验之间的差距，提高接受率和混合速度；数值实验显示该方法在线性和非线性输入输出算子下均优于现有IMH变体

Conclusion: Proximal-IMH为贝叶斯逆问题提供了一种有效的采样方法，特别适用于精确后验采样过于昂贵的场景，能可靠地提升采样性能
...
### [104] [Causal Decoding for Hallucination-Resistant Multimodal Large Language Models](https://arxiv.org/abs/2602.21441)
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但存在物体幻觉问题（引入图像中不存在的物体），这在实际应用中削弱了可靠性。现有方法通常依赖启发式惩罚、后处理修正或通用解码调整，未能直接干预触发物体幻觉的机制，因此效果有限。
...
### [105] [GradAlign: Gradient-Aligned Data Selection for LLM Reinforcement Learning](https://arxiv.org/abs/2602.21492)
*Ningyuan Yang,Weihua Du,Weiwei Sun,Sean Welleck,Yiming Yang*

Main category: cs.LG

TL;DR: GradAlign是一种用于大语言模型强化学习的数据选择方法，通过梯度对齐机制从不可靠的训练数据中筛选出高质量问题，提升RL训练效果


<details>
  <summary>Details</summary>
Motivation: 大语言模型的强化学习性能对训练问题质量高度敏感，现有方法依赖人工筛选或简单启发式过滤，可能包含错误或低效用问题，需要更智能的数据选择机制

Method: 提出GradAlign方法，使用小型可信验证集，优先选择那些策略梯度与验证集梯度对齐的训练问题，形成自适应课程学习

Result: 在三种挑战性数据场景（不可靠奖励信号、分布不平衡、低效用训练语料）中，GradAlign均优于现有基线方法，实现更稳定的训练和更好的最终性能

Conclusion: 梯度方向信号在导航非平稳策略优化中至关重要，GradAlign通过梯度对齐的数据选择机制有效提升了LLM强化学习的训练效果
...
### [106] [Training Generalizable Collaborative Agents via Strategic Risk Aversion](https://arxiv.org/abs/2602.21515)
*Chengrui Qu,Yizhou Zhang,Nicholas Lanzetti,Eric Mazumdar*

Main category: cs.LG

TL;DR: 该论文提出战略风险厌恶作为多智能体协作学习的新方法，通过减少搭便车行为和提高对未见伙伴的鲁棒性，实现更可靠的协作。


<details>
  <summary>Details</summary>
Motivation: 现有协作学习策略在面对新伙伴时表现脆弱，主要原因是训练中的搭便车行为和缺乏战略鲁棒性，需要更通用的协作解决方案。

Method: 提出战略风险厌恶概念作为归纳偏置，开发了集成战略风险厌恶的多智能体强化学习算法，应用于包括LLM协作任务在内的协作基准测试。

Result: 实验验证了理论，表明该方法在异质和未见伙伴的协作任务中能实现可靠协作，比传统纳什均衡有更好的结果，且减少或消除了搭便车行为。

Conclusion: 战略风险厌恶是提高多智能体协作通用性的有效方法，能够产生对未见伙伴鲁棒的协作策略，为实际协作系统提供了实用解决方案。
...
### [107] [From Basis to Basis: Gaussian Particle Representation for Interpretable PDE Operators](https://arxiv.org/abs/2602.21551)
*Zhihao Li,Yu Feng,Zhilu Lai,Wei Wang*

Main category: cs.LG

TL;DR: 提出基于高斯基的流体PDE动力学学习方法，通过高斯粒子算子在模态空间操作，实现可解释、分辨率无关且计算高效的流体模拟。


<details>
  <summary>Details</summary>
Motivation: 现有神经算子和Transformer模型缺乏可解释性，难以处理局部高频结构，且空间采样成本呈二次增长。

Method: 使用高斯基表示场，其中学习到的原子携带显式几何信息；引入高斯粒子算子在模态空间操作，包括高斯模态窗口执行Petrov-Galerkin测量和PG高斯注意力实现全局跨尺度耦合。

Result: 在标准PDE基准和真实数据集上达到最先进的竞争精度，同时提供内在可解释性；支持不规则几何和2D到3D的无缝扩展。

Conclusion: 提出的基于高斯基的表示和高斯粒子算子方法实现了可解释、分辨率无关且计算高效的流体PDE动力学学习，在保持高精度的同时解决了现有方法的局限性。
...
### [108] [D-Flow SGLD: Source-Space Posterior Sampling for Scientific Inverse Problems with Flow Matching](https://arxiv.org/abs/2602.21469)
*Meet Hemant Parikh,Yaqin Chen,Jian-Xun Wang*

Main category: cs.LG

TL;DR: 该论文研究了在流匹配先验下科学逆问题的训练无关条件生成方法，提出了D-Flow SGLD源空间后验采样方法，并在多个科学基准问题上进行了评估。


<details>
  <summary>Details</summary>
Motivation: 科学逆问题需要从稀疏噪声观测中重建高维物理状态，并生成保持先验知识和物理规律的后验样本。虽然扩散模型的训练无关条件生成已有较好发展，但流匹配先验下的相应方法相对不足，特别是在需要超越测量失配评估保真度的科学基准问题上。

Method: 研究流匹配先验下训练无关条件生成的两种策略：引导传输动力学（在采样轨迹中注入测量信息）和源分布推断（对源变量进行后验推断）。基于后者提出了D-Flow SGLD方法，该方法结合可微源推断和预条件随机梯度朗之万动力学，实现源后验的可扩展探索。

Result: 在2D玩具后验、混沌Kuramoto-Sivashinsky轨迹和壁面湍流重建等层次化问题上对代表性方法进行了基准测试。量化了测量同化、后验多样性和物理/统计保真度之间的权衡，确立了D-Flow SGLD作为科学逆问题的实用流匹配兼容后验采样器。

Conclusion: D-Flow SGLD是一种有效的流匹配兼容后验采样方法，能够在不需要重新训练先验或修改学习传输动力学的情况下，对新测量算子诱导的源后验进行可扩展探索，适用于科学逆问题。
...
TL;DR: 本文提出了首个从头开始预训练的三模态掩码扩散模型，涵盖文本、图像-文本和音频-文本数据，系统分析了多模态缩放规律，并提出了一种基于SDE的重参数化方法消除批量大小调优需求。


<details>
  <summary>Details</summary>
Motivation: 现有离散扩散模型主要关注单模态或双模态生成，缺乏统一的三模态模型设计。需要系统研究多模态缩放规律、模态混合比例等关键因素，以构建更强大的多模态生成系统。

Method: 1. 从头开始预训练三模态掩码扩散模型，覆盖文本、图像-文本和音频-文本数据；2. 系统分析多模态缩放规律、模态混合比例、噪声调度和批量大小效应；3. 提出基于随机微分方程的重参数化方法，将物理批量大小与逻辑批量大小解耦；4. 在6.4T令牌上预训练3B参数的三模态模型。

Result: 1. 提供了优化的推理采样默认设置；2. 提出的SDE重参数化消除了批量大小调优需求；3. 3B参数模型在文本生成、文本到图像和文本到语音任务上表现出色；4. 完成了迄今为止最大规模的多模态离散扩散模型系统研究。

Conclusion: 本研究展示了统一设计的三模态扩散模型的可行性，为多模态生成系统提供了重要见解，特别是在缩放行为、模态混合和批量大小优化方面，为未来多模态AI系统的发展奠定了基础。
...
### [110] [Learning Recursive Multi-Scale Representations for Irregular Multivariate Time Series Forecasting](https://arxiv.org/abs/2602.21498)
...
<details>
  <summary>Details</summary>
Motivation: 现有多尺度IMTS方法通常使用重采样获取粗粒度序列，这会改变原始时间戳并破坏采样模式信息。而不规则多元时间序列中的时间间隔和采样模式信息对学习时间和变量依赖关系具有重要价值。

Method: ReIMTS采用递归多尺度建模方法：1）保持时间戳不变，递归将每个样本分割为时间周期逐渐缩短的子样本；2）基于这些长到短子样本中的原始采样时间戳，提出不规则感知表示融合机制，捕捉从全局到局部的依赖关系。

Result: 在多个模型和真实世界数据集上的广泛实验表明，ReIMTS在预测任务中平均性能提升了27.1%。

Conclusion: ReIMTS通过保持原始时间戳不变并递归分割样本，有效利用了不规则多元时间序列中的采样模式信息，显著提升了预测性能，为处理多尺度依赖关系提供了一种新方法。
...
### [111] [Learning from Yesterday's Error: An Efficient Online Learning Method for Traffic Demand Prediction](https://arxiv.org/abs/2602.21757)
*Xiannan Huang,Quan Yuan,Chao Yang*

Main category: cs.LG

TL;DR: FORESEE是一个轻量级在线适应框架，用于交通需求预测，无需更新基础模型参数，通过指数平滑和专家混合机制校正预测误差，适应分布变化。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在交通需求预测中表现良好，但在面对外部事件或城市动态变化导致的分布偏移时，准确性会显著下降。频繁重新训练模型以适应这些变化会产生过高的计算成本，特别是对于大规模或基础模型。

Method: FORESEE框架不更新基础模型参数，而是使用昨天的预测误差来校正今天的预测，通过指数平滑稳定误差，并由专家混合机制指导以适应最近的误差动态。此外，自适应时空平滑组件将误差信号传播到相邻区域和时间段，捕捉需求模式的一致性变化。

Result: 在七个真实世界数据集和三个骨干模型上的广泛实验表明，FORESEE始终提高预测准确性，即使在分布变化最小的情况下也能保持鲁棒性（避免性能下降），并且在现有在线方法中实现了最低的计算开销。
...
TL;DR: WaterVIB：基于变分信息瓶颈的鲁棒水印框架，通过提取消息的最小充分统计量而非依赖脆弱的高频纹理，有效抵抗基于再生的AIGC攻击


<details>
  <summary>Details</summary>
Motivation: 现有水印方法将水印与高频封面纹理纠缠，在生成式净化攻击中容易被重写，导致严重脆弱性。需要开发能抵抗再生攻击的鲁棒水印技术。

Method: 提出WaterVIB框架，将编码器重新表述为信息筛，通过变分信息瓶颈学习消息的最小充分统计量，过滤掉容易受生成变换影响的冗余封面细节，保留对再生不变的必需信号。

Result: 理论证明优化信息瓶颈是对抗分布偏移攻击鲁棒性的必要条件。大量实验表明WaterVIB显著优于现有方法，在未知的基于扩散的编辑攻击下表现出卓越的零样本恢复能力。

Conclusion: WaterVIB通过变分信息瓶颈提取消息的最小充分统计量，有效解决了现有水印方法对高频纹理的过度依赖问题，为对抗再生攻击提供了理论保证的鲁棒水印解决方案。
...
### [113] [Generalisation of RLHF under Reward Shift and Clipped KL Regularisation](https://arxiv.org/abs/2602.21765)
*Kenton Tang,Yuzhu Chen,Fengxiang He*

Main category: cs.LG

TL;DR: 本文针对大语言模型对齐中的RLHF方法，建立了考虑奖励偏移和KL剪枝误差的泛化理论框架，分析了采样误差、奖励偏移误差和KL剪枝误差对泛化性能的影响。


<details>
  <summary>Details</summary>
Motivation: 当前对RLHF的理论理解尚不成熟，特别是在奖励模型可能发生偏移以及KL控制需要估计和剪枝的情况下。需要建立能够明确考虑奖励偏移和KL剪枝误差的泛化理论。

Method: 开发了考虑两种关键因素的RLHF泛化理论：(1) 奖励偏移：奖励模型在早期或混合行为策略的偏好数据上训练，而RLHF在当前策略的rollouts上优化；(2) KL剪枝正则化：KL正则化器从采样的对数概率比估计并剪枝以稳定训练。

Result: 提出了RLHF的泛化界限，表明泛化误差来源于提示和rollouts的采样误差、奖励偏移误差以及KL剪枝误差。讨论了两种特殊情况：(1) 在有限空间上使用均匀先验初始化RLHF参数；(2) 通过随机梯度下降训练RLHF作为Ornstein-Uhlenbeck过程。

Conclusion: 理论分析为实践提供了重要启示：(1) 最优KL剪枝阈值的选择；(2) 在提示、rollouts和偏好数据之间的预算分配策略。这为RLHF的实际应用提供了理论指导。
...
### [114] [Muon+: Towards Better Muon via One Additional Normalization Step](https://arxiv.org/abs/2602.21545)
*Ruijie Zhang,Yequan Zhao,Ziyue Liu,Zhengyang Wang,Zheng Zhang*

Main category: cs.LG

TL;DR: Muon+是Muon优化器的增强版本，通过正交化后增加归一化步骤，在多种模型规模和架构上提升了训练效果。


<details>
  <summary>Details</summary>
Motivation: Muon优化器在大语言模型预训练中通过梯度正交化表现出良好性能，但仍有改进空间，需要更简单有效的增强方法。

Method: 在Muon优化器的正交化步骤后增加额外的归一化步骤，形成Muon+优化器，保持简单性的同时提升效果。

Result: 在130M到774M参数的GPT风格模型和60M到1B参数的LLaMA风格模型上，Muon+相比Muon在训练和验证困惑度上都有稳定提升，支持工业级T2P比例≈200。

Conclusion: Muon+通过简单的归一化增强，为Muon优化器提供了稳定有效的性能提升，适用于大规模语言模型预训练。
...
### [115] [Excitation: Momentum For Experts](https://arxiv.org/abs/2602.21798)
*Sagi Shaier*
...
<details>
  <summary>Details</summary>
Motivation: 传统优化器对所有参数一视同仁，无法有效处理稀疏架构如混合专家模型(MoE)的特殊性。特别是在深度MoE中，存在"结构混淆"现象，标准优化器无法建立有效的信号路径，导致训练不稳定。

Method: Excitation框架基于批次级专家利用率动态调制更新，引入竞争性更新动态：放大高利用率专家的更新，并选择性抑制低利用率专家的更新，从而锐化路由专业化。该方法与优化器、领域和模型无关，无需额外参数或优化器状态。

Result: 在语言和视觉任务中，Excitation持续改善了MoE模型的收敛速度和最终性能，能够"拯救"那些标准优化器无法稳定训练的深度MoE模型。

Conclusion: 主动更新调制是有效条件计算的关键机制，Excitation作为一个专业化催化剂，为解决稀疏架构优化问题提供了高效、内存友好的解决方案。
...
### [116] [Extending Sequence Length is Not All You Need: Effective Integration of Multimodal Signals for Gene Expression Prediction](https://arxiv.org/abs/2602.21550)
*Zhao Yang,Yi Duan,Jiwei Zhu,Ying Ba,Chuan Cao,Bing Su*

Main category: cs.LG

TL;DR: 该研究发现长序列建模会降低基因表达预测性能，而近端多模态表观基因组信号更为关键。作者提出Prism框架，通过后门调整消除背景染色质模式的混杂效应，仅使用短序列即达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基因表达预测研究通常通过延长输入序列长度来定位远端增强子，但作者发现长序列建模反而会降低当前模型的性能。更重要的是，近端多模态表观基因组信号对基因表达预测更为关键，但如何有效整合这些信号一直被忽视。不同信号类型具有不同的生物学功能，简单的拼接可能导致模型与背景染色质模式产生虚假关联。

Method: 提出Prism框架：1）学习高维表观基因组特征的多种组合来表示不同的背景染色质状态；2）使用后门调整来减轻混杂效应。该方法专注于如何更好地整合近端多模态表观基因组信号，而不是依赖长序列输入。

Result: 实验结果表明，通过对多模态表观基因组信号进行适当建模，仅使用短序列就能在基因表达预测任务上达到最先进的性能。这证明了近端信号比长序列建模更为重要，且有效整合这些信号可以显著提升预测准确性。

Conclusion: 基因表达预测的关键在于有效整合近端多模态表观基因组信号，而非简单地延长输入序列长度。Prism框架通过处理不同信号类型的生物学角色差异和背景染色质模式的混杂效应，为基因表达预测提供了更优的解决方案。
...
### [117] [xai-cola: A Python library for sparsifying counterfactual explanations](https://arxiv.org/abs/2602.21845)
*Lin Zhu,Lei You*
...
<details>
  <summary>Details</summary>
Motivation: 现有反事实解释生成器产生的解释通常高度冗余，需要减少不必要的特征修改，提高解释的简洁性和可理解性。

Method: 开发xai-cola库，提供完整的稀疏化管道：输入原始表格数据、预处理对象和训练好的模型，支持内置或外部导入的CE生成器，实现多种稀疏化策略和可视化分析功能。

Result: 实验表明xai-cola能显著减少反事实解释的修改特征数量，在某些设置下可减少高达50%的特征修改，同时保持解释有效性。

Conclusion: xai-cola是一个有效的反事实解释稀疏化工具，能够提高解释的简洁性，促进可解释AI的实际应用，已开源发布在PyPI上。
...
### [118] [Training-free Composition of Pre-trained GFlowNets for Multi-Objective Generation](https://arxiv.org/abs/2602.21565)
...
<details>
  <summary>Details</summary>
Motivation: 现有多目标GFlowNets方法需要为每个目标集重新训练，计算开销大且适用性受限。实际应用常涉及多个冲突目标，需要更灵活高效的解决方案。

Method: 提出训练免费的混合策略，在推理时组合预训练的GFlowNets，无需微调或重新训练。框架灵活支持线性标量化到复杂非线性逻辑算子的多种奖励组合。

Result: 理论证明该方法能精确恢复线性标量化的目标分布，并通过失真因子量化非线性算子的近似质量。在合成2D网格和真实分子生成任务中，性能与需要额外训练的基线方法相当。

Conclusion: 提出的训练免费混合策略实现了多目标GFlowNets的快速适应，显著降低计算开销，为科学发现等需要探索多种解决方案的应用提供了实用工具。
...
### [119] [Duel-Evolve: Reward-Free Test-Time Scaling via LLM Self-Preferences](https://arxiv.org/abs/2602.21585)
*Sweta Karlekar,Carolina Zheng,Magnus Saebo,Nicolas Beltran-Velez,Shuyang Yu,John Bowlan,Michal Kucer,David Blei*

Main category: cs.LG

TL;DR: Duel-Evolve：一种基于LLM自身成对偏好的进化优化算法，无需外部奖励模型或人工评分函数，在数学和编程基准测试上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 许多应用需要在测试时优化LLM输出，但现有方法依赖校准的标量评估器，而许多任务中这种评分不可用、过于稀疏或不可靠。相比之下，成对比较更容易获取，仍能提供改进方向的信号，且可以从LLM自身获得而无需外部监督。
...
Conclusion: 成对自偏好为大规模离散输出空间的测试时改进提供了强大的优化信号，展示了仅通过LLM自身偏好进行有效优化的可行性。
...
### [120] [Breaking Semantic-Aware Watermarks via LLM-Guided Coherence-Preserving Semantic Injection](https://arxiv.org/abs/2602.21593)
*Zheng Gao,Xiaoyu Li,Zhicheng Bao,Xiaoyan Feng,Jiaojiao Jiang*

Main category: cs.LG

TL;DR: 该论文提出了一种针对内容感知语义水印的CSI攻击方法，利用LLM引导的语义操作在保持全局一致性的同时破坏水印检测，揭示了当前语义水印设计的安全弱点。


<details>
  <summary>Details</summary>
Motivation: 生成图像在社交媒体和版权分发场景中广泛传播，语义水印被用于溯源追踪和防伪。传统基于噪声层的水印容易受到反演攻击，而内容感知语义水印虽然将水印信号与高级图像语义绑定，但LLM的结构化推理能力能够进行局部精细但全局一致的语义修改，从而破坏这种绑定关系。

Method: 提出了一种保持一致性的语义注入（CSI）攻击方法，利用LLM引导的语义操作，在嵌入空间相似性约束下进行语义操纵。这种对齐机制在保持视觉语义一致性的同时，有选择地扰动与水印相关的语义，最终导致检测器误分类。

Result: 广泛的实证结果表明，CSI攻击在对抗内容感知语义水印方面始终优于现有的攻击基准方法，揭示了当前语义水印设计在面对LLM驱动的语义扰动时的基本安全弱点。

Conclusion: LLM驱动的语义扰动能够有效破坏内容感知语义水印的绑定机制，暴露了当前语义水印设计的安全漏洞，需要重新思考语义水印的安全性设计。
...
TL;DR: NGDB-Zoo是一个统一框架，通过操作级训练和语义增强解决神经图数据库的训练效率和表达能力瓶颈，实现1.8-6.8倍的吞吐量提升，并在大规模图上保持高GPU利用率。
...
Method: 提出NGDB-Zoo框架，通过将逻辑操作符与查询拓扑解耦，将训练循环转化为动态调度的数据流执行，实现多流并行；并设计解耦架构集成预训练文本编码器的高维语义先验，避免I/O阻塞和内存溢出。

Result: 在六个基准测试（包括ogbl-wikikg2和ATLAS-Wiki等大规模图）上评估，NGDB-Zoo相比基线实现1.8-6.8倍的吞吐量提升，在不同逻辑模式下保持高GPU利用率，显著减轻混合神经符号推理中的表示摩擦。

Conclusion: NGDB-Zoo通过操作级训练和语义增强有效解决了神经图数据库的训练效率和表达能力瓶颈，为大规模知识图上的复杂逻辑推理提供了高效解决方案。
...
TL;DR: AgentLTV：基于智能体的自动化LTV建模框架，通过LLM驱动的智能体生成代码、运行和修复管道，结合MCTS和进化算法两阶段搜索，实现跨场景的LTV预测模型自动构建。


<details>
  <summary>Details</summary>
Motivation: LTV预测在广告、推荐系统和电商中至关重要，但实际应用中不同决策场景的数据模式差异很大。从业者通常需要为每个场景构建复杂的特定管道，涉及特征处理、目标设计和调优，这个过程成本高昂且难以迁移。

Method: 提出AgentLTV框架：1) 将候选解决方案视为可执行的管道程序；2) LLM驱动的智能体生成代码、运行和修复管道、分析执行反馈；3) 两阶段搜索：MCTS阶段在固定预算下探索广泛的建模选择空间，EA阶段通过基于岛屿的进化算法（交叉、变异、迁移）优化最佳MCTS程序。

Result: 在大规模专有数据集和公共基准测试中，AgentLTV在排名和误差指标上都能持续发现强模型。在线桶级分析显示，特别是在高价值和负LTV细分市场中，排名一致性和价值校准得到改善。

Conclusion: AgentLTV已成功在线部署。实践建议：使用MCTS快速适应新数据模式，使用EA进行稳定优化，并通过桶级排名和校准诊断验证部署准备度。该框架为自动化LTV建模提供了统一解决方案。
...
TL;DR: 本文研究了如何利用领域知识（特别是关于哪些输入会导致错误的知识）来加速主动自动机学习算法，针对存在大量可能输入且许多输入会导致错误的系统，提出了L#算法的多种改进版本。


<details>
  <summary>Details</summary>
Motivation: 主动自动机学习算法在处理具有大量可能输入的系统时难以扩展，特别是在许多输入会导致错误的情况下。然而，在许多实际问题中，这些错误是可观察的（产生已知的错误输出），这为利用领域知识加速学习提供了机会。

Method: 针对不同级别的领域知识（关于哪些输入在哪些状态下不会产生错误），对最先进的AAL算法L#进行了匹配的改进。这些改进充分利用了关于非错误产生输入的领域知识来优化学习过程。

Result: 实验评估表明，这些方法能够显著加速学习：在具有强大但现实的领域知识时，学习速度提高了几个数量级；在领域知识有限时，学习速度也能提高一个数量级。

Conclusion: 利用关于非错误产生输入的领域知识可以显著加速主动自动机学习，特别是在处理具有大量可能输入且许多输入会导致错误的系统时。针对不同级别的领域知识，对现有算法进行适当的改进能够带来显著的性能提升。
...
### [124] [On Imbalanced Regression with Hoeffding Trees](https://arxiv.org/abs/2602.22101)
*Pantia-Marina Alchirch,Dimitrios I. Diochnos*

Main category: cs.LG

TL;DR: 该研究将核密度估计(KDE)和层次收缩(HS)两种批处理方法扩展到流式环境，用于改进在线回归任务中的决策树性能，发现KDE在数据流早期有益，而HS几乎没有性能提升。


<details>
  <summary>Details</summary>
Motivation: 现实应用中存在连续数据流用于回归任务，Hoeffding树及其变体在流式学习中表现良好。批处理学习中的核密度估计(KDE)在类别不平衡回归中有效，层次收缩(HS)是不改变树结构的后处理正则化方法。研究旨在将这些批处理技术扩展到流式环境。

Method: 使用伸缩论证将KDE方法扩展到流式环境，并将HS实现扩展到增量决策树模型。在在线回归常用数据集上评估这些扩展对决策树性能的影响。

Result: KDE在数据流早期阶段对性能有积极影响，而HS方法几乎没有（如果有的话）提供性能改进。

Conclusion: 在流式回归任务中，核密度估计(KDE)扩展在数据流早期阶段是有益的，而层次收缩(HS)扩展几乎没有实际性能优势。研究提供了公开可用的代码实现。
...
TL;DR: 该论文提出了一种新颖的分层引导评论家（HLC）多智能体强化学习架构，通过多层级视角结合局部和全局信息，提高了样本效率和策略鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体强化学习要么局限于局部视角（独立学习），要么局限于全局视角（集中式学习），缺乏结合多层级视角的有效方法，这限制了在复杂协调任务中的性能。

Method: 提出了分层引导评论家（HLC）架构，受自然团队结构中涌现分布的启发，结合高层目标导向和低层执行。采用顺序训练方案，在不同层次级别上学习多个视角。

Result: 在合作、非通信和部分可观测的多智能体强化学习基准测试中，HLC超越了单层次基线方法，随着智能体数量和难度增加，表现出良好的可扩展性和鲁棒性。

Conclusion: 引入多层级视角，结合局部和全局信息，能够显著提升多智能体强化学习的性能，实现高样本效率和鲁棒策略，为复杂协调任务提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态时间序列预测虽然能利用其他模态的丰富信息获得更准确预测，但由于模态对齐的根本挑战，现有方法难以有效整合多模态数据，特别是对时间序列波动有因果影响的文本信息（如紧急报告、政策公告）

Method: 提出TiMi模型：1）利用LLM生成对未来发展的推理，作为时间序列预测的指导；2）引入多模态专家混合（MMoE）模块作为轻量级插件，赋能基于Transformer的时间序列模型进行多模态预测，无需显式表示层对齐

Result: 在16个真实世界多模态预测基准测试中，TiMi展现出持续的最先进性能，优于先进基线方法，同时提供强大的适应性和可解释性

Conclusion: TiMi通过LLM的因果推理能力和MMoE模块的轻量级集成，有效解决了多模态时间序列预测中的模态对齐挑战，实现了更准确、可解释的预测
...
TL;DR: 该研究比较了不确定性量化方法在科学机器学习中的应用，特别是在临界热通量预测中，展示了覆盖导向学习方法如何比后处理方法更好地捕捉多物理机制的数据行为。


<details>
  <summary>Details</summary>
Motivation: 科学机器学习面临的核心挑战是如何正确表示受多机制行为支配的物理系统。在这些场景中，标准数据分析技术往往无法捕捉数据的本质，因为系统的响应由于随机性和不同的物理机制而在状态空间中显著变化。不确定性量化不应仅仅被视为安全评估，而应作为学习任务本身的支持，引导模型内化数据行为。

Method: 研究聚焦于OECD/NEA反应堆系统多物理专家组提出的临界热通量基准和数据集。通过对比分析不确定性量化方法，包括后处理方法（特别是保形预测）与端到端覆盖导向管道（包括贝叶斯异方差回归和质量驱动损失），这些方法将不确定性视为优化过程的主动组成部分，同时建模预测及其行为。

Result: 研究表明，虽然后处理方法确保统计校准，但覆盖导向学习有效地重塑了模型的表示以匹配复杂的物理机制。结果是模型不仅提供高预测准确性，还提供物理一致的不确定性估计，能够动态适应临界热通量的内在变异性。

Conclusion: 不确定性量化在科学机器学习中应被视为主动学习组件而非最终度量指标。覆盖导向学习方法能够更好地内化多机制物理系统的复杂行为，提供更可靠的预测和物理一致的不确定性估计。
...
### [128] [Provable Last-Iterate Convergence for Multi-Objective Safe LLM Alignment via Optimistic Primal-Dual](https://arxiv.org/abs/2602.22146)
*Yining Li,Peizhong Ju,Ness Shroff*

Main category: cs.LG

TL;DR: 提出了一种用于安全RLHF的通用原始-对偶框架，统一了多种现有对齐算法，并引入乐观原始-对偶算法以稳定鞍点动力学，建立了最终迭代收敛保证。


<details>
  <summary>Details</summary>
Motivation: RLHF在将大语言模型与人类偏好对齐中起重要作用，但标准原始-对偶方法在策略参数化下可能不稳定或发散，且仅保证分布策略下的收敛。需要一种更稳定、通用的框架来解决这些问题。

Method: 提出通用原始-对偶框架统一多种对齐算法，引入乐观原始-对偶算法，包含原始变量和对偶变量的预测更新以稳定鞍点动力学，分析涵盖分布空间中的精确策略优化和参数化策略下的近似收敛。

Result: 建立了最终迭代收敛保证，包括分布空间中的精确收敛和参数化策略下与近似误差和偏差相关的邻域收敛，揭示了乐观性在缓解约束对齐目标固有振荡中的关键作用。

Conclusion: 该工作填补了约束强化学习与实际RLHF之间的关键理论空白，提供了一个稳定且通用的框架，能够有效解决现有方法在策略参数化下的不稳定性和收敛问题。
...
### [129] [C$^{2}$TC: A Training-Free Framework for Efficient Tabular Data Condensation](https://arxiv.org/abs/2602.21717)
*Sijia Xu,Fan Li,Xiaoyang Wang,Zhengyi Yang,Xuemin Lin*

Main category: cs.LG

TL;DR: C²TC：首个免训练的表格数据集压缩框架，通过类自适应聚类分配解决表格数据压缩问题，效率比现有方法提升至少2个数量级。


<details>
  <summary>Details</summary>
Motivation: 表格数据是工业关系数据库的主要格式，但大规模表格数据给学习型分析系统带来计算和存储挑战。现有数据集压缩方法计算密集且忽略了表格数据的关键特征（如异构特征和类别不平衡）。

Method: 提出C²TC框架，将数据集压缩目标重新表述为类自适应聚类分配问题（CCAP），消除昂贵的训练过程。开发HFILS启发式局部搜索算法，交替进行软分配和类内聚类。提出HCFE混合分类特征编码方法，保持异构离散属性的语义。

Result: 在10个真实世界数据集上的实验表明，C²TC比最先进的基线方法效率提升至少2个数量级，同时在下游任务中表现更优。
...
### [130] [From Words to Amino Acids: Does the Curse of Depth Persist?](https://arxiv.org/abs/2602.21750)
...
TL;DR: 研究发现蛋白质语言模型存在深度效率低下问题，类似于大语言模型中的"深度诅咒"，深层对最终输出的贡献有限


<details>
  <summary>Details</summary>
Motivation: 大语言模型研究中发现了"深度诅咒"现象，即深层对最终预测贡献有限。这引发了对蛋白质语言模型是否存在类似深度效率低下问题的疑问，特别是考虑到PLMs的训练目标多样（自回归、掩码、扩散）且部分是多模态的

Method: 对六个流行的PLMs进行深度分析，涵盖不同模型家族和规模，包括三种训练目标（自回归、掩码、扩散）。使用统一的探测和扰动测量方法量化各层贡献随深度的演化

Result: 在所有模型中观察到一致的深度依赖模式：深层较少依赖早期计算，主要精炼最终输出分布，这些效应在更深模型中更加明显

Conclusion: PLMs表现出深度效率低下形式，这为未来开发更深度高效的架构和训练方法提供了动机
...
TL;DR: 论文提出CUPID框架解决机器学习中"捷径遗忘"问题：模型容易学习偏见相关特征但难以遗忘，导致遗忘目标类别时反而遗忘偏见特征，造成准确率反常提升。


<details>
  <summary>Details</summary>
Motivation: 机器遗忘技术对数据隐私和模型可靠性至关重要，但在现实场景中，模型可能从数据中的虚假相关性学习到非预期的偏见。当从这种有偏见的模型中执行遗忘时，会出现"捷径遗忘"现象：模型难以遗忘容易学习的偏见对齐样本，反而遗忘偏见属性而非目标类别属性。

Method: 提出CUPID框架：1) 基于样本损失景观锐度将遗忘集划分为因果近似和偏见近似子集；2) 将模型参数解耦为因果路径和偏见路径；3) 通过将精炼的因果梯度和偏见梯度分别路由到相应路径，执行有针对性的更新。

Result: 在Waterbirds、BAR和Biased NICO++等偏见数据集上的广泛实验表明，该方法实现了最先进的遗忘性能，并有效缓解了捷径遗忘问题。

Conclusion: CUPID框架成功解决了从有偏见模型中执行遗忘时的独特挑战，特别是"捷径遗忘"现象，为机器遗忘在现实偏见场景中的应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 文档智能模型需要大量标注训练数据，但数据获取成本高、劳动密集，且使用语言模型标注真实文档存在隐私问题。合成文档生成成为一种有前景的隐私保护替代方案。

Method: 提出DocDjinn框架：1) 基于聚类的种子选择与参数化采样；2) 语义-视觉解耦，通过扩散模型添加逼真手写和上下文视觉元素；3) 生成视觉逼真、语义一致的合成文档。

Result: 在11个基准测试（信息提取、问答、文档分类、布局分析）上评估，仅使用100个真实训练样本就能达到完整真实数据集平均87%的性能。公开了代码和14万+合成文档样本。

Conclusion: 这是首个证明视觉语言模型能够从未标注种子大规模生成忠实标注文档数据集的工作，这些合成数据能有效丰富或近似真实标注数据，用于多样化的文档理解任务。
...
### [133] [Learning in the Null Space: Small Singular Values for Continual Learning](https://arxiv.org/abs/2602.21919)
*Cuong Anh Pham,Praneeth Vepakomma,Samuel Horváth*

Main category: cs.LG

TL;DR: NESS是一种基于正交性的持续学习方法，通过利用小奇异值构建近似零空间，将任务特定更新约束在该子空间中，从而减少灾难性遗忘并支持新任务学习。


<details>
  <summary>Details</summary>
Motivation: 持续学习面临的主要挑战是在减轻灾难性遗忘的同时支持进一步学习。基于正交性的训练方法因其效率和理论特性受到关注，但现有方法主要通过梯度投影实现正交性。本文重新审视正交性，利用小奇异值对应与先前任务输入空间近似正交的方向这一事实。

Method: 提出NESS方法：1）通过每层输入表示的最小奇异值构建近似零空间；2）使用紧凑的低秩适应（LoRA风格）公式将任务特定更新参数化并约束在该子空间中；3）固定子空间基以保持零空间约束，每个任务只学习单个可训练矩阵。

Result: 在三个基准数据集上的实验表明，NESS具有竞争性性能、低遗忘率和跨任务的稳定准确性，突显了小奇异值在持续学习中的作用。

Conclusion: NESS通过在权重空间直接应用正交性（而非通过梯度操作），利用小奇异值构建近似零空间，有效平衡了持续学习中的遗忘与学习需求，为基于正交性的持续学习方法提供了新视角。
...
### [134] [Bayesian Generative Adversarial Networks via Gaussian Approximation for Tabular Data Synthesis](https://arxiv.org/abs/2602.21948)
...
<details>
  <summary>Details</summary>
Motivation: 当前CTGAN在处理表格数据的风险-效用权衡方面存在困难，而贝叶斯GAN在表格数据上研究较少，且常用的MCMC方法计算成本高、权重存储需求大，需要更高效的贝叶斯表格数据合成方法。

Method: 提出GACTGAN方法，将随机权重平均-高斯(SWAG)贝叶斯后验近似技术集成到CTGAN生成器中，在训练阶段结束后减少计算开销，实现更高效的贝叶斯表格数据合成。

Result: GACTGAN相比CTGAN能生成更好的合成数据，更好地保持表格结构完整性，保留推断统计特性，同时降低隐私风险，在风险-效用权衡方面表现更优。

Conclusion: GACTGAN是一种更简单有效的贝叶斯表格数据合成实现方法，通过集成SWAG技术解决了传统贝叶斯GAN计算成本高的问题，在隐私保护和数据效用之间取得了更好的平衡。
...
### [135] [Estimation and Optimization of Ship Fuel Consumption in Maritime: Review, Challenges and Future Directions](https://arxiv.org/abs/2602.21959)
*Dusica Marijan,Hamza Haruna Mohammed,Bakht Zaman*

Main category: cs.LG

TL;DR: 本文全面综述了海运燃油消耗的估计与优化方法，将方法分为物理模型、机器学习模型和混合模型，探讨了数据融合技术的重要性，并首次讨论了可解释AI在增强模型透明度中的作用。


<details>
  <summary>Details</summary>
Motivation: 为减少碳排放和降低航运成本，提高船舶燃油效率至关重要。需要系统梳理现有的燃油消耗估计与优化方法，识别关键挑战并提出未来研究方向。

Method: 采用文献综述方法，将燃油消耗估计方法分为三类：基于物理的模型、机器学习模型和混合模型。探讨数据融合技术（结合AIS、船载传感器和气象数据）的重要性，并讨论可解释AI在增强模型透明度中的作用。

Result: 系统分类了燃油消耗估计方法，识别了数据质量、可用性和实时优化等关键挑战，提出了未来研究方向，包括混合模型、实时优化和数据标准化。

Conclusion: 本文为海运燃油消耗的估计与优化提供了全面的综述框架，强调了数据融合和可解释AI的重要性，并为未来研究指明了方向，特别是混合模型、实时优化和数据集标准化方面。
...
### [136] [Disease Progression and Subtype Modeling for Combined Discrete and Continuous Input Data](https://arxiv.org/abs/2602.22018)
...
Method: 提出Mixed Events模型，并在Subtype and Stage Inference (SuStaIn)框架中实现为Mixed-SuStaIn，支持亚型和进展建模

Result: 通过模拟实验和阿尔茨海默病神经影像学倡议的真实数据验证，Mixed-SuStaIn在混合数据集上表现良好

Conclusion: Mixed-SuStaIn扩展了疾病进展模型的适用性，能够处理异质真实世界数据，为理解长期疾病轨迹提供了更强大的工具
...
TL;DR: 该论文研究了均值漂移污染模型下的均值估计问题，为一般基础分布提供了样本复杂度分析，建立了上下界匹配的算法框架。


<details>
  <summary>Details</summary>
Motivation: 先前工作仅针对高斯和拉普拉斯分布研究了均值漂移污染模型下的均值估计，但对于一般基础分布的样本复杂度仍是一个开放问题。本文旨在解决这一开放问题，为更广泛的分布类别提供理论分析。

Method: 采用傅里叶分析技术，引入了"傅里叶见证"的概念，在基础分布特征函数满足温和谱条件的前提下，设计了样本高效的均值估计算法。

Result: 证明了在温和谱条件下，存在样本高效的算法能够以任意精度估计目标均值，并给出了与上界在质量上匹配的样本复杂度下界。

Conclusion: 本文基本解决了均值漂移污染模型中一般基础分布的均值估计问题，建立了完整的理论框架，展示了傅里叶分析在这一问题中的关键作用。
...
### [138] [Learning and Naming Subgroups with Exceptional Survival Characteristics](https://arxiv.org/abs/2602.22179)
*Mhd Jawad Al Rahwanji,Sascha Xu,Nils Philipp Walter,Jilles Vreeken*

Main category: cs.LG

TL;DR: Sysurv是一个完全可微、非参数的方法，利用随机生存森林学习个体生存曲线，自动学习条件并将其组合成可解释的规则，以发现具有特殊生存特征的亚群。


<details>
  <summary>Details</summary>
Motivation: 在许多应用中，识别生存时间更长或更短的亚群很重要（如医学中确定哪些患者从治疗中受益，预测性维护中哪些组件更可能失效）。现有方法需要限制性假设（如比例风险），需要预离散化特征，且由于比较平均统计量而容易忽略个体偏差。

Method: 提出Sysurv方法：1）完全可微、非参数；2）利用随机生存森林学习个体生存曲线；3）自动学习条件并将其组合成可解释的规则；4）选择具有特殊生存特征的亚群。

Result: 在广泛的数据集和设置（包括癌症数据案例研究）上进行实证评估，显示Sysurv能够揭示有洞察力和可操作的生存亚群。

Conclusion: Sysurv是一个有效的生存亚群发现方法，克服了现有方法的限制，能够自动学习可解释规则并识别具有特殊生存特征的亚群。
...
### [139] [A Dynamic Survey of Soft Set Theory and Its Extensions](https://arxiv.org/abs/2602.21268)
*Takaaki Fujita,Florentin Smarandache*

Main category: cs.AI

TL;DR: 本文对软集理论及其扩展进行了综述性概述，涵盖核心定义、代表性构造和当前发展的关键方向。


<details>
  <summary>Details</summary>
Motivation: 软集理论为参数化决策建模提供了一个直接框架，通过将每个属性（参数）分配给给定宇宙的子集来结构化地表示不确定性。该理论在过去几十年中已扩展到多种变体，并与拓扑学、拟阵理论等多个领域建立了联系。

Method: 采用综述风格的方法，对软集及其主要扩展进行系统性概述，包括超软集、超超软集、树软集、双极软集和动态软集等变体。

Result: 提供了软集理论及其扩展的全面调查，突出了核心定义、代表性构造和关键发展方向。

Conclusion: 本书系统地概述了软集理论及其扩展，为该领域的研究者提供了重要的参考框架，并指出了当前发展的关键方向。
...
### [140] [A Hierarchical Multi-Agent System for Autonomous Discovery in Geoscientific Data Archives](https://arxiv.org/abs/2602.21351)
...
<details>
  <summary>Details</summary>
Motivation: 地球科学数据快速积累带来了显著的可扩展性挑战，虽然PANGAEA等存储库托管了大量数据集，但引用指标显示大部分数据未被充分利用，限制了数据的可重用性

Method: 提出PANGAEA-GPT分层多智能体框架，采用集中式监督者-工作者拓扑结构，具有严格的数据类型感知路由、沙盒确定性代码执行以及通过执行反馈进行自我校正，使智能体能够诊断和解决运行时错误
...
### [141] [ARLArena: A Unified Framework for Stable Agentic Reinforcement Learning](https://arxiv.org/abs/2602.21534)
...
TL;DR: 本文提出ARLArena框架和SAMPO方法，旨在解决Agentic强化学习中的训练不稳定问题，通过分解策略梯度的四个核心设计维度，提供稳定的训练方案。


<details>
  <summary>Details</summary>
Motivation: Agentic强化学习在处理复杂多步交互任务方面显示出潜力，但训练过程极不稳定，经常导致训练崩溃，这限制了其扩展到更大环境和更长交互范围的能力，也阻碍了算法设计的系统性探索。

Method: 首先提出ARLArena框架，构建干净标准化的测试平台；然后将策略梯度分解为四个核心设计维度，评估每个维度的性能和稳定性；基于此分析提出SAMPO方法，专门设计用于缓解ARL中的主要不稳定因素。
...
### [142] [The ASIR Courage Model: A Phase-Dynamic Framework for Truth Transitions in Human and AI Systems](https://arxiv.org/abs/2602.21745)
*Hyo Jin Kim*

Main category: cs.AI

TL;DR: ASIR勇气模型是一个相变动态框架，将真相披露从人格特质重新定义为状态转变，适用于人类和AI系统在风险下的真实表达


<details>
  <summary>Details</summary>
Motivation: 传统上将勇气视为人格特质，但作者希望建立一个统一的动态框架来解释人类和AI系统在压力下的真相披露行为，特别是当存在不对称风险时

Method: 提出ASIR勇气模型，将真相披露建模为从抑制状态(S0)到表达状态(S1)的相变，当促进力量超过抑制阈值时发生转变：λ(1+γ)+ψ > θ+φ，其中λ是基线开放度，γ是关系放大因子，ψ是累积内部压力，θ和φ是转变成本

Result: 该框架统一解释了人类在压力下的沉默和AI在策略约束下的输出失真，通过反馈扩展模型捕捉了路径依赖和分歧效应，将表观真实性变化解释为约束相空间中相互作用力的几何结果

Conclusion: ASIR勇气模型为人类和人工系统在风险下的真相披露提供了一个形式化的动态结构视角，将勇气和对齐重新框架为共享的动态结构，而非归因于AI系统的意图
...
### [143] [fEDM+: A Risk-Based Fuzzy Ethical Decision Making Framework with Principle-Level Explainability and Pluralistic Validation](https://arxiv.org/abs/2602.21746)
*Abeer Dyoub,Francesca A. Lisi*

Main category: cs.AI

TL;DR: fEDM+扩展了原有的模糊伦理决策框架，增加了可解释性模块和多元语义验证，解决了原模型在原则性解释和伦理多元主义下的鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 原fEDM框架虽然保证了形式正确性和决策一致性，但未能充分解决两个关键挑战：决策的原则性可解释性，以及在伦理多元主义下的鲁棒性。需要增强模型的透明度和对不同伦理观点的包容性。

Method: 1. 引入可解释性和可追溯性模块(ETM)，将每个伦理决策规则与底层道德原则明确关联，并为每个推荐行动计算加权原则贡献度。2. 用多元语义验证框架替代单一参照验证，评估决策针对多个利益相关者参照，每个参照编码不同的原则优先级和风险容忍度。

Result: 扩展后的fEDM+保留了形式可验证性，同时实现了增强的可解释性和利益相关者感知验证，使其适合作为伦理敏感AI系统的监督和治理层。

Conclusion: fEDM+通过引入原则性解释和多元验证机制，在保持形式严谨性的同时，显著提升了伦理决策系统的透明度和对不同伦理观点的适应性，为AI伦理治理提供了更完善的框架。
...
### [144] [Prompt Architecture Determines Reasoning Quality: A Variable Isolation Study on the Car Wash Problem](https://arxiv.org/abs/2602.21814)
*Heejin Jo*

Main category: cs.AI

TL;DR: STAR推理框架将汽车清洗问题的准确率从0%提升至85%，结合用户档案和RAG上下文达到100%准确率，表明结构化推理比上下文注入对隐式约束推理更重要。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在"汽车清洗问题"这一需要隐式物理约束推理的病毒式基准测试中持续失败，研究旨在探索提示架构的哪些层次能够实现正确推理。

Method: 使用Claude 3.5 Sonnet模型，控制超参数（温度0.7，top_p 1.0），进行变量隔离研究（6个条件，每个条件20次试验，共120次试验）。测试不同提示架构层次：基础条件、STAR推理框架、添加用户档案上下文、添加RAG上下文等。

Result: STAR推理框架单独将准确率从0%提升至85%（p=0.001，Fisher精确检验，比值比13.22）。添加用户档案上下文进一步提供10个百分点的增益，RAG上下文再贡献5个百分点，完整堆栈条件下达到100%准确率。

Conclusion: 结构化推理支架——特别是强制目标表述在推理之前——对于隐式约束推理任务的重要性远大于上下文注入。这表明在推理任务中，明确的推理过程比额外的背景信息更为关键。
...
### [145] [Distill and Align Decomposition for Enhanced Claim Verification](https://arxiv.org/abs/2602.21857)
...
TL;DR: 提出强化学习方法，联合优化复杂声明分解质量与验证器对齐，提升下游验证性能


<details>
  <summary>Details</summary>
Motivation: 现有方法在分解质量与验证性能对齐方面存在不足，需要更好的联合优化方法

Method: 使用强化学习（GRPO）联合优化分解质量与验证器对齐，结合结构化顺序推理、教师蒸馏示例监督微调，以及多目标奖励平衡
...
### [146] [ProactiveMobile: A Comprehensive Benchmark for Boosting Proactive Intelligence on Mobile Devices](https://arxiv.org/abs/2602.21858)
...
TL;DR: ProactiveMobile是一个用于评估移动智能体主动智能能力的基准测试，包含3,660个实例的14个场景，通过63个API函数序列执行评估，实验显示当前MLLMs在主动智能方面存在明显不足。
...
Method: 提出ProactiveMobile基准测试，将主动任务形式化为：基于设备上下文信号的四个维度推断潜在用户意图，并从包含63个API的综合函数池中生成可执行函数序列。基准包含14个场景的3,660个实例，采用多答案标注来应对现实复杂性，并由30名专家团队进行最终审核，确保事实准确性、逻辑一致性和行动可行性。

Result: 实验表明，微调的Qwen2.5-VL-7B-Instruct模型成功率达到19.15%，优于o1（15.71%）和GPT-5（7.39%）。这表明主动智能是当前MLLMs普遍缺乏但可学习的关键能力。

Conclusion: 主动智能是移动智能体发展的关键方向，但当前MLLMs在该能力上存在明显不足。ProactiveMobile基准测试为系统评估和推进主动智能研究提供了重要工具，实验结果证实了该基准在评估主动智能能力方面的重要性。
...
### [147] [Semantic Partial Grounding via LLMs](https://arxiv.org/abs/2602.22067)
*Giuseppe Canonaco,Alberto Pozanco,Daniel Borrajo*

Main category: cs.AI

TL;DR: SPG-LLM使用大语言模型分析PDDL文件，在规划任务接地前识别潜在无关的对象、动作和谓词，显著减少接地任务规模，在七个难以接地的基准测试中实现更快接地（通常快几个数量级）。
...
Method: 提出SPG-LLM方法，利用大语言模型分析领域和问题文件，在接地前启发式地识别潜在无关的对象、动作和谓词，从而显著减少接地任务的规模。
...
### [148] [Petri Net Relaxation for Infeasibility Explanation and Sequential Task Planning](https://arxiv.org/abs/2602.22094)
...
<details>
  <summary>Details</summary>
Motivation: 现实中的规划问题经常因情况变化或理解变化而需要调整计划，有时甚至不存在可行计划。传统规划方法主要关注可行情况下的高效一次性规划，而缺乏对领域更新和不可行性检测的支持。

Method: 采用Petri网可达性松弛技术，实现鲁棒的约束不变式合成；结合增量约束求解器，支持目标和约束的增量更新；提供不可行性检测和解释机制。

Result: 相比基线方法，该系统能生成相当数量的约束不变式，检测到最多2倍的不可行情况，在一次性规划中表现相当，在顺序规划更新中表现更优。

Conclusion: 提出的方法能有效处理规划中的变化和不可行性检测问题，为动态环境下的规划系统提供了更好的适应性和解释能力。
...
### [149] [A Generic Web Component for WebRTC Pub-Sub](https://arxiv.org/abs/2602.22011)
*Kundan Singh*

Main category: cs.MM

TL;DR: video-io是一个通用的Web组件，用于在WebRTC应用中发布或订阅媒体流，采用命名流抽象而非传统的呼叫或会议室抽象，使应用逻辑与特定服务解耦。


<details>
  <summary>Details</summary>
Motivation: 现有视频会议服务通常采用呼叫或会议室抽象，限制了应用场景。作者希望创建一个更通用的媒体流组件，能够适用于多种场景，同时使应用开发者能够编写一次代码即可在不同服务器或服务上运行。

Method: 设计video-io作为Web组件，采用命名流抽象而非传统的呼叫/会议室抽象。将大部分应用逻辑保留在端点中，通过组件的丰富应用接口实现，而将特定供应商的访问控制或信令协商放在服务特定的连接器实现中。

Result: 实现了video-io组件，并为十个不同的现有系统和服务实现了连接器，展示了其灵活性。通过将应用与托管供应商服务解耦，促进了端点创新，超越了单一供应商锁定客户端应用所能提供的功能。

Conclusion: video-io提供了一种通用的WebRTC媒体流组件，通过命名流抽象和服务解耦设计，使应用开发者能够创建更灵活、可移植的Web应用，促进了WebRTC应用的创新和互操作性。
