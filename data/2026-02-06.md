...
Motivation: 基于图像的饮食评估对于准确便捷地监测个体健康、预防和管理慢性疾病及肥胖至关重要，但面临从2D图像估计食物三维尺寸的核心挑战。

Method: 探索了多种策略：1）使用深度图、多视角输入等辅助输入；2）基于模型的模板匹配方法；3）深度学习技术，包括单目图像处理和图像与辅助输入的组合方法。

Result: 论文综述了不同策略在准确估计食物分量方面的应用，但没有提供具体的实验结果数据，主要关注方法论的比较和分析。

Conclusion: 基于图像的饮食评估需要克服2D到3D的尺寸估计挑战，多种技术策略已被开发，深度学习为这一领域提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图基MOT方法需要重新设计跟踪架构，缺乏统一的训练目标来同时优化检测精度、身份保持和时空一致性。

Method: 提出UniTrack作为通用训练目标，通过可微分图表示学习，将检测精度、身份保持和时空一致性整合到单个端到端可训练损失函数中。

Result: 在多种跟踪模型和基准测试中验证，包括Trackformer、MOTR、FairMOT等，身份切换减少达53%，IDF1提升12%，GTR在SportsMOT上MOTA提升9.7%。

Conclusion: UniTrack作为即插即用的图论损失函数，能显著提升多目标跟踪性能，兼容现有系统且无需架构修改，具有广泛适用性。
...
TL;DR: 提出了一种名为VCR（视觉概念排序）的方法，用于识别大型多模态模型中的重要视觉概念，并应用于医疗任务中模型行为的审计，特别是在皮肤病变分类任务中发现了不同人口亚组间的性能差异。


<details>
  <summary>Details</summary>
Motivation: 在医疗等安全关键领域，确保机器学习模型的可靠性需要能够揭示模型缺陷的审计方法。大型多模态模型在医疗任务中的行为需要被深入理解，特别是在不同人口亚组间的性能差异问题。

Method: 提出了视觉概念排序（VCR）方法，用于识别大型多模态模型中的重要视觉概念。该方法应用于皮肤恶性病变分类任务，并辅以胸部X光片和自然图像的实验。通过演示示例提示模型，发现性能差异后应用VCR方法生成关于视觉特征依赖性的假设，并通过人工干预进行验证。

Result: 研究发现大型多模态模型在不同人口亚组间显示出意外的性能差距。VCR方法能够生成关于视觉特征依赖性的假设，这些假设可以通过人工干预得到验证，从而揭示了模型在医疗任务中的行为模式。

Conclusion: VCR方法为审计大型多模态模型在安全关键领域的可靠性提供了有效工具，能够识别模型的重要视觉概念依赖，并揭示潜在的性能偏差问题，特别是在医疗应用中的人口亚组差异。
...
TL;DR: ARGaze将第一人称视线估计重新定义为序列预测任务，通过自回归解码器结合当前视觉特征和最近视线历史来预测当前注视目标，实现了在线流式推理和SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 在线第一人称视线估计对增强现实和辅助技术至关重要，但缺乏明确的头部或眼睛信号，需要从稀疏间接线索推断视觉注意力。研究发现视线在目标导向活动中具有强时间连续性，最近注视位置为预测下一步注视提供了有力先验。

Method: ARGaze将视线估计重新定义为序列预测：在每个时间步，Transformer解码器基于(i)当前视觉特征和(ii)固定长度的"视线上下文窗口"（最近视线目标估计）来预测当前注视位置。这种设计强制因果性并支持有界资源流式推理。

Result: 在多个第一人称基准测试中实现了在线评估下的最先进性能，广泛消融实验验证了自回归建模和有界视线历史对鲁棒预测的关键作用。

Conclusion: 自回归建模结合有界视线历史是实现在线第一人称视线估计的有效方法，ARGaze框架在保持因果性和流式推理能力的同时达到了SOTA性能。
...
TL;DR: 该论文首次系统评估了基于视觉的手部追踪模型在戴手套手部上的性能，提出了AirGlove方法来解决不同手套设计带来的外观差异问题，显著提升了模型在新手套上的追踪性能。


<details>
  <summary>Details</summary>
Motivation: 传感手套在遥操作和机器人策略学习中很重要，但现有方法存在限制：基于传感器的追踪受信号质量和校准影响，而基于视觉的方法在裸手上表现良好，但在不同外观的手套上性能未充分探索。

Method: 提出了AirGlove方法，利用现有手套数据来泛化学到的手套表示，使其能适应新的手套设计，只需有限的数据就能实现有效泛化。

Result: 实验表明，现有裸手模型在传感手套上性能显著下降，而AirGlove能有效将手部姿态模型泛化到新手套设计，相比其他方案获得显著性能提升。

Conclusion: 该研究填补了视觉手部追踪在手套应用上的空白，提出的AirGlove方法通过泛化手套表示，解决了不同手套设计带来的外观差异问题，为传感手套的视觉追踪提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 水下摄影面临对比度降低、空间模糊和波长相关色彩失真等固有挑战，这些问题会掩盖海洋生物的活力，环保摄影师通常需要繁重的后期处理流程来纠正这些失真

Method: 开发图像到图像处理流程，通过引入合成退化管道，使用基于扩散的生成方法学习逆转水下退化效果。在Keith Ellenbogen的小型高质量环保摄影数据集上进行训练和评估

Result: 该方法在约2500张图像上从头训练后，使用约1100万参数的模型能够生成512x768图像，实现了高感知一致性和强泛化能力

Conclusion: 提出的基于扩散模型的方法能有效逆转水下摄影退化，为环保摄影师提供高效的图像修复解决方案
...
TL;DR: PoseGaussian是一个基于姿态引导的高斯泼溅框架，用于高质量人体新视角合成，通过姿态信息优化几何和时序一致性，实现实时渲染和SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决动态人体场景中的挑战，如关节运动和严重自遮挡，现有方法仅将姿态用作条件或变形，需要更有效地利用姿态信息来提升鲁棒性和泛化能力。

Method: 提出姿态引导的高斯泼溅框架：1）姿态作为结构先验，与颜色编码器融合以优化深度估计；2）姿态作为时序线索，通过专用姿态编码器增强帧间一致性；3）构建完全可微分的端到端训练管道。

Result: 在ZJU-MoCap、THuman2.0和内部数据集上验证，达到SOTA性能：PSNR 30.86、SSIM 0.979、LPIPS 0.028，同时保持100 FPS的实时渲染效率。

Conclusion: PoseGaussian通过将姿态信息嵌入几何和时序阶段，有效解决了动态人体场景的挑战，实现了高质量、高效率的新视角合成，超越了现有方法。
...
TL;DR: 提出了一种新方法，将视频生成模型重新用作奖励模型，通过对比学习训练生成模型作为基于能量的模型来评估视频质量，显著减少了人类标注需求。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLM）的视频奖励建模方法难以捕捉细微的时间动态，需要大量人工标注数据。

Method: 将先进的视频生成模型转化为基于能量的模型，通过对比学习训练其区分高质量和低质量视频。设计了三种潜在空间扰动方法（时间切片、特征交换、帧重排）来创建具有挑战性的合成负样本。

Result: 在GenAI-Bench和MonteBench上取得了最先进的性能，仅使用3万个人工标注，比现有VLM方法减少了6到65倍的标注需求。

Conclusion: 生成模型可以有效地重新用作时间感知的奖励模型，通过对比学习和精心设计的负样本训练，能够以极少的标注数据实现卓越的视频质量评估性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经编解码器在超低码率下性能显著下降，而基于生成压缩的方法面临语义保真度与感知真实性的权衡困境：显式表示方法保留内容结构但缺乏细粒度纹理，隐式方法能合成视觉细节但存在语义漂移。

Method: 提出统一框架，以训练无关方式整合显式和隐式表示：1）在扩散模型中基于显式高层语义进行条件化；2）使用反向信道编码隐式传递细粒度细节；3）引入可插拔编码器，通过调制隐式信息灵活控制失真-感知权衡。

Result: 在Kodak、DIV2K和CLIC2020数据集上实现了最先进的率-感知性能，相比DiffC方法在DISTS BD-Rate指标上分别提升29.92%、19.33%和20.89%。

Conclusion: 该框架成功弥合了显式和隐式表示之间的鸿沟，在超低码率条件下实现了语义保真度与感知真实性的更好平衡，为生成压缩提供了有效的解决方案。
...
TL;DR: E.M.Ground是一种新颖的视频大语言模型，专注于整体和连贯的事件感知，通过事件令牌、平滑技术和多粒度特征聚合来改进时间视频定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有时间视频定位方法通常通过比较帧特征与两个独立令牌来匹配开始和结束帧，严重依赖精确时间戳，但这种方法无法捕捉事件的语义连续性和完整性，导致模糊性。

Method: 提出E.M.Ground模型，包含三个关键创新：(1) 特殊<event>令牌聚合查询事件所有帧的信息，保持语义连续性；(2) Savitzky-Golay平滑减少跨时间戳的令牌-帧相似性噪声；(3) 多粒度帧特征聚合增强匹配可靠性和时间理解。
...
TL;DR: MPA通过数据增强和策略优化，从数据和策略两个角度渐进式地将少样本分割能力适应到目标域，显著提升跨域少样本分割性能


<details>
  <summary>Details</summary>
Motivation: 现有跨域少样本分割方法因目标域样本数量有限、多样性不足，以及源域模型在目标域初始少样本能力较弱且存在较大域差距，导致目标样本利用效率低，适应效果受限

Method: 提出多视图渐进适应(MPA)：1) 数据角度：混合渐进增强，通过累积强增强生成更多样、更复杂的视图，创造渐进挑战性学习场景；2) 策略角度：双链多视图预测，通过顺序和并行学习路径在广泛监督下充分利用这些渐进复杂视图

Result: MPA能有效将少样本能力适应到目标域，在跨域少样本分割任务中大幅超越现有最先进方法，性能提升达7.0%

Conclusion: 通过联合强制不同复杂视图间的预测一致性，MPA实现了对目标域既鲁棒又准确的适应，解决了跨域少样本分割中的域适应难题
...
TL;DR: 本文提出了一种名为条件点稀疏化（CPS）的训练免费方法，用于解决SAM在跨域少样本分割中的性能下降问题，通过参考样本自适应稀疏化密集匹配点来提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于SAM的训练免费少样本分割方法依赖密集点匹配作为提示，但在跨域场景（如医学或卫星图像）中性能显著下降。作者发现这是由于大域偏移破坏了SAM学习的点-图像交互，且点密度在这种条件下起着关键作用。

Method: 提出条件点稀疏化（CPS）方法：这是一种训练免费方法，利用参考样本的真值掩码作为可靠指导，自适应地稀疏化密集匹配点，从而引导SAM在跨域图像中进行更准确的交互。

Result: 在多个跨域少样本分割数据集上的广泛实验表明，CPS方法优于现有的训练免费SAM基方法。

Conclusion: CPS通过自适应稀疏化密集匹配点，有效解决了SAM在跨域少样本分割中的性能问题，为训练免费方法在跨域场景中的应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 压铸在工业中至关重要，但表面缺陷是影响质量的主要问题。计算机视觉技术可以自动化缺陷检测，但现有方法在工业应用中的效率和准确性有待提高
...
<details>
  <summary>Details</summary>
Motivation: 肾小球疾病中电子致密沉积物（EDD）的自动检测面临高质量标注数据稀缺的挑战。众包标注虽然降低了成本，但引入了标签噪声问题

Method: 提出主动标签清洗方法：1）使用主动学习选择最有价值的噪声样本进行专家重新标注；2）构建高精度清洗模型；3）标签选择模块利用众包标签与模型预测之间的差异进行样本选择和实例级噪声评分

Result: 在私有数据集上达到67.18% AP50，比直接在噪声标签上训练提升18.83%。性能达到全专家标注的95.79%，同时降低标注成本73.30%
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在通用多模态检索中表现出巨大潜力，但处理大量视觉输入令牌时产生的高计算成本阻碍了其实际应用。需要开发既高效又高性能的解决方案。

Method: 提出Magic-MM-Embedding系列模型，基于两个协同支柱：(1) 包含视觉令牌压缩的高效MLLM架构，大幅降低推理延迟和内存占用；(2) 多阶段渐进训练策略，从粗到细：首先通过持续预训练恢复多模态理解能力，然后进行大规模对比预训练和困难负样本挖掘增强判别能力，最后通过MLLM-as-a-Judge指导的任务感知微调进行精确数据筛选。

Result: 综合实验表明，该模型在显著超越现有方法的同时，推理效率更高，实现了性能与效率的双重优势。

Conclusion: Magic-MM-Embedding通过创新的架构设计和训练策略，成功解决了MLLM在多模态检索中的计算效率问题，为实际应用提供了可行的解决方案。
...
TL;DR: Fast-SAM3D通过异构感知的动态计算分配，将SAM3D的3D重建推理速度提升2.67倍，同时保持高质量输出。


<details>
  <summary>Details</summary>
Motivation: SAM3D虽然能实现复杂场景的可扩展3D重建，但其推理延迟过高阻碍了实际部署。现有通用加速策略在该场景下表现脆弱，需要专门针对其异构特性设计优化方案。

Method: 提出训练免费的Fast-SAM3D框架，包含三个异构感知机制：1）模态感知步骤缓存，解耦结构演进与布局更新；2）联合时空令牌雕刻，聚焦高熵区域细化；3）频谱感知令牌聚合，自适应解码分辨率。

Result: 实验表明Fast-SAM3D实现最高2.67倍的端到端加速，且保真度损失可忽略，为高效单视图3D生成建立了新的帕累托前沿。

Conclusion: 通过系统分析SAM3D的推理动态并针对其多级异构特性设计专门优化，Fast-SAM3D成功解决了3D重建的推理效率瓶颈，实现了高质量与高效率的平衡。
...
TL;DR: FlashBlock是一种用于块扩散模型的缓存注意力机制，通过重用跨扩散步骤中稳定的块外注意力输出来减少计算开销，提高生成长序列内容的效率。


<details>
  <summary>Details</summary>
Motivation: 生成长格式内容（如分钟级视频和长文本）对现代生成模型越来越重要。块扩散通过KV缓存和块级因果推理提高了推理效率，但在长上下文设置中，块扩散仍面临不断增长的KV缓存带来的注意力计算开销问题。

Method: 作者发现块扩散中一个未被充分探索的特性：块内注意力的跨步冗余性。分析表明，当前块外token的注意力输出在扩散步骤间保持稳定，而块内注意力变化显著。基于此，提出FlashBlock——一种缓存块外注意力机制，重用稳定的注意力输出，减少注意力计算和KV缓存访问，且不改变扩散过程。

Result: 在扩散语言模型和视频生成上的实验表明，FlashBlock实现了最高1.44倍的token吞吐量提升和最高1.6倍的注意力时间减少，对生成质量影响可忽略。该方法与稀疏注意力正交，可作为补充的残差重用策略，显著提高激进稀疏化下的模型准确性。

Conclusion: FlashBlock通过利用块扩散中注意力输出的跨步稳定性，有效减少了长序列生成中的计算开销，为高效生成长格式内容提供了一种实用的优化方案。
...
TL;DR: PAIR框架通过不安全-安全配对数据实现概念擦除，将简单的概念移除转变为保持语义一致性的语义重新对齐，在擦除有害概念的同时保留结构和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法主要关注移除不安全概念，但没有提供相应的安全替代指导，导致原始图像和擦除后生成图像之间的结构和语义一致性难以保持。

Method: 提出PAIR框架：1) 从不安全输入生成结构语义保真的安全对应物，形成配对数据；2) 配对语义重新对齐，使用不安全-安全配对将目标概念显式映射到语义对齐的安全锚点；3) Fisher加权初始化的DoRA，利用配对数据初始化参数高效的低秩适应矩阵，鼓励生成安全替代方案同时选择性抑制不安全概念。

Result: 大量实验表明，该方法显著优于现有最先进基线，实现了有效的概念擦除，同时保持了结构完整性、语义连贯性和生成质量。

Conclusion: PAIR框架通过将概念擦除重新定义为使用不安全-安全配对的语义重新对齐，实现了细粒度擦除，仅移除目标概念同时保持整体语义一致性，为文本到图像扩散模型的安全应用提供了新思路。
...
TL;DR: APEX提出了一种自适应原型OOD检测框架，通过两阶段修复过程优化特征流形，解决了现有原型方法的静态同质性假设和学习-推断断开问题，在CIFAR-100等基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于原型的表示学习方法在OOD检测中表现优异，但存在两个根本性缺陷：1）静态同质性假设（所有类别使用固定的表示资源）；2）学习-推断断开（推断时丢弃丰富的原型质量知识）。这些缺陷限制了模型容量和性能。

Method: APEX框架采用两阶段修复过程：1）自适应原型流形（APM），利用最小描述长度原则自动确定每个类别的最优原型复杂度K_c*，从根本上解决原型碰撞问题；2）后验感知OOD评分机制（PAOS），量化原型质量（内聚性和分离性）来弥合学习-推断断开。

Result: 在CIFAR-100等基准上的综合实验验证了方法的优越性，APEX实现了新的最先进性能。

Conclusion: APEX通过自适应原型流形和后验感知评分机制，有效解决了现有原型方法的根本缺陷，显著提升了OOD检测性能。
...
TL;DR: HIVE框架通过将分层视觉线索注入潜在空间，实现多模态慢思考推理，避免依赖显式语言推理链


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的推理过程通常是"快速思考"范式，依赖端到端生成或显式的语言思维链，存在效率低、冗长和易产生幻觉的问题。需要更稳健的推理方法在潜在空间中整合多模态信号。

Method: 提出HIVE框架，通过递归扩展Transformer块创建内部循环进行迭代推理精炼，将分层视觉线索（从全局场景上下文到细粒度区域细节）注入模型的潜在表示中，使模型能够在对齐的潜在空间中执行基于多步骤推理。

Result: 广泛评估表明，在融入视觉知识时测试时间缩放是有效的，整合分层信息显著增强了模型对复杂场景的理解能力。

Conclusion: HIVE框架通过潜在空间中的分层视觉线索注入，实现了更稳健的多模态推理，避免了传统语言思维链的局限性，为多模态大语言模型提供了更有效的"慢思考"推理机制。
...
TL;DR: 本文揭示了OOD检测中的"简单性悖论"：现有SOTA模型能区分语义细微的OOD样本，但对结构差异大但语义简单的样本或高频传感器噪声存在严重的几何盲区。作者提出D-KNN框架，通过正交分解分离语义和结构成分，在CIFAR和ImageNet基准上实现新的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于特征的事后OOD检测方法存在一个反直觉的"简单性悖论"：模型对语义细微的OOD样本敏感，但对结构差异大但语义简单的样本或传感器噪声存在"几何盲区"。作者将这种现象归因于深度特征空间中的"语义霸权"，并通过神经坍缩理论揭示其数学本质。

Method: 提出D-KNN框架，这是一个无需训练、即插即用的几何解耦方法。通过正交分解明确分离语义成分和结构残差，并引入双空间校准机制来重新激活模型对弱残差信号的敏感性。

Result: D-KNN在CIFAR和ImageNet基准上建立了新的SOTA性能。在解决简单性悖论方面，将FPR95从31.3%降低到2.3%；在处理高斯噪声等传感器故障时，将检测性能（AUROC）从基线79.7%提升到94.9%。

Conclusion: D-KNN通过几何解耦有效打破了深度特征空间中的语义霸权，解决了OOD检测中的简单性悖论问题，显著提升了模型对结构分布偏移信号的敏感性，为处理传感器噪声等实际应用场景提供了有效解决方案。
...
TL;DR: CityGenAgent是一个基于自然语言的层次化程序化生成高质量3D城市的框架，通过分解为区块程序和建筑程序两个可解释组件，结合监督微调和强化学习策略，实现了语义对齐、视觉质量和可控性的显著提升。


<details>
  <summary>Details</summary>
Motivation: 自动生成交互式3D城市在自动驾驶、虚拟现实和具身智能等领域有广泛应用，但现有方法在高保真资产创建、可控性和操作方面存在困难，需要更有效的解决方案。
...
Conclusion: CityGenAgent通过自然语言驱动的层次化程序化生成框架，解决了现有3D城市生成方法在高保真资产创建、可控性和操作方面的挑战，支持自然语言编辑和操作，为相关应用领域提供了强大的技术基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型与人类偏好对齐面临挑战：奖励模型难以获取，大规模偏好数据集收集成本高昂。核心问题是能否仅用最少人工反馈、无需辅助奖励模型，通过挖掘扩散模型自身潜力实现有效对齐。

Method: 提出SAIL框架，从少量人工标注偏好对开始，采用闭环自增强迭代学习：模型逐步生成多样样本，基于自身演化理解进行自标注偏好，利用自增强数据集进行自我精炼。引入排名偏好混合策略平衡探索与初始人类先验，防止灾难性遗忘。

Result: SAIL在多个基准测试中持续优于现有最先进方法，仅需现有方法6%的偏好数据，证明扩散模型具有显著自我改进能力，可有效替代大规模人工标注和外部奖励模型。

Conclusion: 扩散模型拥有强大的自我改进能力，通过SAIL框架可有效利用这些能力，仅需最少人工反馈即可实现与人类偏好的对齐，为扩散模型对齐提供了高效实用的解决方案。
...
Method: 引入VRIQ基准，包含抽象谜题风格任务和自然图像推理任务。通过诊断探针分析失败原因，针对特定感知类别（形状、计数、位置、3D/深度等）设计细粒度诊断问题。

Result: 抽象谜题任务准确率约28%（接近随机），自然图像任务准确率45%。工具增强推理仅带来适度改进。分析显示56%失败源于感知问题，43%源于感知和推理共同问题，仅1%源于纯推理问题。

Conclusion: 当前视觉语言模型即使配备视觉推理工具，在抽象推理方面仍不可靠，主要限制在于感知能力而非推理能力。该基准为改进多模态系统的视觉推理提供了原则性基础。
...
Method: 采用两阶段方法：第一阶段联合执行文档类型分类（数字原生vs拍摄）和布局分析；第二阶段采用混合解析策略：拍摄文档作为完整页面整体解析，数字原生文档则基于检测到的布局锚点进行元素级并行解析。
...
<details>
  <summary>Details</summary>
Motivation: MRI在放疗计划中缺乏电子密度信息，当前工作流程依赖MRI和CT联合采集，增加了配准不确定性和程序复杂性。合成CT生成可实现仅MRI规划，但由于MRI-CT关系的非线性和解剖变异性而具有挑战性。
...
Conclusion: 提出的Parallel Swin Transformer-Enhanced Med2Transformer架构能够有效从MRI生成合成CT，为仅MRI放疗规划提供了可行解决方案，具有临床可接受的剂量精度。
...
TL;DR: 本文提出统计流匹配方法，用于高效数据集蒸馏，通过对齐目标类别中心到非目标类别中心的统计流来优化合成图像，大幅降低计算和内存开销，同时提出分类器继承策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法（如线性梯度匹配）在计算和内存方面开销巨大，需要加载数千张真实图像并在每个蒸馏步骤对合成图像进行多轮可微分增强，导致效率低下。

Method: 提出统计流匹配框架，通过对齐原始数据中目标类别中心到非目标类别中心的恒定统计流来优化合成图像；同时提出分类器继承策略，重用原始数据集上训练的分类器，仅需轻量级线性投影器。

Result: 方法在性能上达到或优于现有最优方法，同时GPU内存使用降低10倍，运行时间缩短4倍；分类器继承策略仅需极小存储开销即可获得显著性能提升。

Conclusion: 统计流匹配为数据集蒸馏提供了一种稳定高效的监督学习框架，显著降低了计算和内存需求，同时通过分类器继承策略进一步提升了性能效率。
...
TL;DR: 提出MAD框架，通过VAE学习解耦的潜在空间，在编辑病理组学特征时保持特征相关性，避免脱离生物流形，实现可控且生物学合理的细胞核编辑。


<details>
  <summary>Details</summary>
Motivation: 病理组学提供可解释的生物标志物，但许多特征难以解释且在不同临床背景下理解困难。现有条件扩散模型假设特征独立，而病理组学特征本质相关，编辑单一特征时固定其他特征会导致脱离生物流形，产生不现实的伪影。

Method: 提出MAD框架：1）使用变分自编码器学习解耦的潜在空间，捕捉特征相关性；2）在潜在空间中正则化特征轨迹，确保编辑目标特征时自动调整相关属性以保持在真实细胞的学习分布内；3）用优化后的特征指导条件扩散模型合成高保真图像。

Result: 实验表明该方法能够在编辑病理组学特征时导航特征流形，在条件特征编辑方面优于基线方法，同时保持结构一致性。

Conclusion: MAD框架通过保持特征相关性，实现了可控且生物学合理的细胞核编辑，解决了现有方法因假设特征独立而脱离生物流形的问题，提高了病理组学特征编辑的实用性和可靠性。
...
### [29] [TSBOW: Traffic Surveillance Benchmark for Occluded Vehicles Under Various Weather Conditions](https://arxiv.org/abs/2602.05414)
...
TL;DR: 该研究提出了TSBOW数据集，这是一个用于极端天气条件下遮挡车辆检测的交通监控基准数据集，包含超过32小时的真实交通数据和数百万标注帧。
...
Method: 研究团队创建了TSBOW数据集，包含超过32小时来自人口密集城市地区的真实交通数据，包括超过48,000个手动标注和320万个半标注帧，涵盖八个交通参与者类别。

Result: 建立了TSBOW的对象检测基准，突出了遮挡和恶劣天气带来的挑战。数据集包含多种道路类型、尺度和视角，为智能交通系统发展提供了关键资源。

Conclusion: TSBOW数据集填补了极端天气条件下交通监控数据集的空白，展示了基于CCTV的交通监控潜力，为新的研究和应用铺平了道路。数据集已公开可用。
...
TL;DR: 提出了一种无需外部数据的长尾分布OOD检测方法，通过几何引导的虚拟异常合成和双粒度语义损失来提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于Outlier Exposure的SOTA方法依赖大规模外部数据集（如80 Million Tiny Images），但在实际部署中面临数据获取成本高和隐私敏感性问题。长尾分布下尾类样本稀缺导致特征空间决策边界模糊，需要一种无需外部数据的方法来解决这一问题。

Method: 提出了一个无数据框架，包含两个核心组件：1）几何引导的虚拟异常合成（GOS），使用von Mises-Fisher分布在超球面上建模统计特性，在特征空间低似然环形区域进行方向性采样生成虚拟异常；2）双粒度语义损失（DGS），利用对比学习最大化内分布特征与合成边界异常之间的区分度。

Result: 在CIFAR-LT等基准测试上的大量实验表明，该方法优于使用外部真实图像的SOTA方法，在完全消除对外部数据集依赖的同时保持了优越的检测性能。
...
TL;DR: 提出Disco框架，通过邻接感知协同着色解决密集重叠细胞实例分割问题，基于细胞邻接图色度特性分析发现真实组织多为非二分图，需处理奇数长度环


<details>
  <summary>Details</summary>
Motivation: 现有基于轮廓检测和距离映射的方法在处理复杂密集细胞区域时面临挑战，基于图着色的新范式在真实密集重叠和复杂拓扑场景中的有效性尚未验证

Method: 提出Disco框架：1) 显式标记策略将拓扑挑战转化为可学习分类任务，递归分解细胞图并隔离"冲突集"；2) 隐式消歧机制通过强制不同实例间的特征差异性解决冲突区域模糊性

Result: 发布大规模数据集GBC-FS 2025，首次系统分析四个不同数据集的细胞邻接图色度特性，发现真实细胞图多为非二分图且存在大量奇数长度环（主要为三角形）

Conclusion: 简单2着色理论不足以处理复杂组织，而高色度模型会导致表示冗余和优化困难，Disco框架通过邻接感知协同着色有效解决密集重叠细胞实例分割问题
...
TL;DR: NeVStereo是一个结合NeRF渲染和多视角立体视觉的联合框架，能够从多视角RGB输入中同时输出相机位姿、深度图、新视角合成和表面重建，解决了现有方法在几何一致性和位姿精度方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：前馈系统专注于端到端匹配和几何预测但不输出新视角合成；基于神经渲染的方法能提供高质量新视角合成和详细几何，但通常假设固定相机位姿且对位姿误差敏感。因此需要一个单一框架能够从随意拍摄的视图中同时提供准确的位姿、可靠的深度、高质量的渲染和精确的3D表面。

Method: NeVStereo结合了NeRF驱动的新视角合成来生成适合立体匹配的渲染图像，置信度引导的多视角深度估计，NeRF耦合的束调整用于位姿优化，以及迭代细化阶段同时更新深度和辐射场以提高几何一致性。这种设计缓解了NeRF常见的表面堆叠、伪影和位姿-深度耦合问题。

Result: 在室内、室外、桌面和航空基准测试中，NeVStereo实现了持续强大的零样本性能：深度误差降低36%，位姿精度提高10.4%，新视角合成保真度提高4.5%，网格质量达到最先进水平（F1 91.93%，Chamfer距离4.35毫米）。

Conclusion: NeVStereo成功地将NeRF渲染与立体视觉相结合，提供了一个统一的框架，能够从多视角RGB输入中联合输出相机位姿、深度图、新视角合成和表面重建，在多个指标上超越了现有方法。
...
TL;DR: 提出基于潜在扩散模型的方法LD-SLRO，用于恢复高反射率物体表面条纹投影图像，改善3D重建精度


<details>
  <summary>Details</summary>
Motivation: 高反射率、低表面粗糙度物体的条纹投影三维重建面临严重挑战，镜面反射和间接照明导致条纹图案严重失真或丢失

Method: 使用潜在扩散模型，首先编码相位偏移条纹图像提取表面反射特性，然后以这些潜在特征为条件输入，概率性抑制反射伪影并恢复丢失的条纹信息

Result: 实验结果表明，该方法在条纹质量和3D重建精度上优于现有方法，将平均均方根误差从1.8176毫米降低到0.9619毫米

Conclusion: LD-SLRO方法有效解决了高反射物体条纹投影测量问题，显著提高了重建精度，并提供了灵活的输入输出配置
...
TL;DR: 本文提出了Stable Velocity框架，通过分析流匹配中条件速度的高方差问题，设计了训练时的方差降低目标StableVM和自适应监督VA-REPA，以及推理时的加速采样方法StableVS，在多个大型模型上实现了训练效率提升和2倍以上的采样加速。


<details>
  <summary>Details</summary>
Motivation: 流匹配方法虽然优雅，但其依赖单样本条件速度会导致高方差训练目标，这会破坏优化稳定性并减慢收敛速度。作者通过明确表征这种方差，识别出两个关键区域：靠近先验的高方差区域（优化困难）和靠近数据分布的低方差区域（条件速度和边际速度几乎重合）。

Method: 提出了Stable Velocity统一框架：1) 训练方面：Stable Velocity Matching（StableVM）作为无偏方差降低目标；Variance-Aware Representation Alignment（VA-REPA）在低方差区域自适应加强辅助监督。2) 推理方面：Stable Velocity Sampling（StableVS）利用低方差区域动力学允许闭式简化的特性，实现无需微调的加速采样。

Result: 在ImageNet 256×256以及大型预训练文本到图像和文本到视频模型（包括SD3.5、Flux、Qwen-Image和Wan2.2）上的广泛实验表明，该方法在训练效率方面有持续改进，在低方差区域内实现了超过2倍的采样加速，且不降低样本质量。

Conclusion: 通过分析流匹配中的方差问题，提出的Stable Velocity框架有效解决了训练不稳定和收敛慢的问题，同时在推理阶段实现了显著加速，为扩散模型和流匹配方法提供了实用的改进方案。
...
TL;DR: 该论文提出了一种基于参数化3D网格模型生成合成缺陷数据的方法，用于工业无损检测中的缺陷检测训练数据增强。


<details>
  <summary>Details</summary>
Motivation: 工业质量控制在缺陷检测方面至关重要，无损检测方法因其不影响物体功能而受青睐。自动化缺陷检测需要大量高质量训练数据，但真实数据获取困难且罕见缺陷样本不足。合成数据可以解决这一问题，但需要可控的生成环境。

Method: 开发参数化方法建模各种缺陷类型的3D网格对象，这些缺陷模型可以添加到物体几何体中生成合成缺陷物体。方法基于金属铸造常见缺陷，但可扩展到其他加工工艺。通过基于物理的蒙特卡洛模拟相应检测方法，生成与真实检测数据相似的合成数据。

Result: 能够生成可变且任意大的合成数据集，包含足够数量的罕见缺陷样本，同时可以并行创建像素级完美标注。以视觉表面检测为例展示了方法可行性，但该方法可应用于任何其他无损检测方法的模拟组合。

Conclusion: 提出的参数化缺陷建模方法为无损检测的机器学习训练提供了可控、可扩展的合成数据生成方案，特别适用于解决罕见缺陷样本不足的问题，具有广泛的工业应用前景。
...
TL;DR: 本文提出了一种与蒸馏兼容的可学习特征缓存机制，用于加速视频扩散模型，在保持生成质量的同时实现了11.8倍的加速。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型加速方法存在局限性：特征缓存方法虽然无需训练且加速效果显著，但进一步压缩会导致语义和细节丢失；训练感知的步数蒸馏方法在图像生成中成功，但在视频生成中步数较少时性能急剧下降。将无训练特征缓存直接应用于步数蒸馏模型时，由于采样步数更稀疏，质量损失更加严重。
...
Conclusion: 本文提出的蒸馏兼容可学习特征缓存机制和受限平均流方法，有效解决了视频扩散模型加速中的质量损失问题，实现了显著加速而不牺牲生成质量。
...
Method: 提出注意力保持框架：1）使用层级rollout机制提取先前任务的注意力图并生成实例自适应二进制掩码；2）学习新任务时应用这些掩码将先前注意力区域相关的梯度置零，防止已学习视觉概念被破坏；3）为兼容现代优化器，通过按比例缩放参数更新来保持相对幅度。

Result: 实验和可视化结果表明该方法能有效缓解灾难性遗忘并保持视觉概念，在多种持续学习场景中实现了最先进的性能并展现出强大的泛化能力。

Conclusion: 通过约束Vision Transformers中的注意力漂移，提出的注意力保持框架为缓解持续学习中的灾难性遗忘提供了有效解决方案，在保持先前学习知识的同时学习新任务。
...
Method: 提出Memory-Execute-Review三部分框架：1) 分层记忆模块提供信息支持；2) 执行模块负责常规决策和行动；3) 审查模块处理异常情况并纠正行为。

Result: 在4个数据集上，训练免费设置下平均成功率提升7%，零样本设置下提升5%。在HM3D_v0.1和HM3D_OVON数据集上零样本设置下分别提升8%和6%。在MP3D和HM3D_OVON数据集上不仅超越所有训练免费方法，还超越所有监督微调方法。

Conclusion: Memory-Execute-Review框架能够有效解决视觉语言导航中成功率和泛化能力的权衡问题，在多个数据集和设置下均取得显著提升，实现了综合领先。
...
TL;DR: SOMA-1M是一个包含130万对像素级精确对齐的SAR-光学图像数据集，覆盖全球多尺度（0.5-10米）和12种土地覆盖类型，为多模态遥感算法和基础模型提供基准资源。
...
Method: 开发了SOMA-1M数据集，包含来自Sentinel-1、PIESAT-1、Capella Space和Google Earth的130万对地理参考图像（512x512像素），采用严格的粗到细图像匹配框架确保像素级对齐。

Result: 在四个层次视觉任务（图像匹配、图像融合、SAR辅助云去除和跨模态转换）上建立了评估基准，涉及30多种主流算法。实验表明，在SOMA-1M上的监督训练显著提升了所有任务的性能，多模态遥感图像匹配达到当前SOTA水平。
...
TL;DR: 该报告提供了一个鱼眼图像序列数据集PFSeq，并评估了不同特征检测器和描述符在鱼眼图像自标定中的性能，旨在解决鱼眼视觉里程计中的"先有鸡还是先有蛋"问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决鱼眼相机自标定中的核心矛盾：需要准确投影模型来优化特征检测，同时又需要良好特征来校准投影模型。该研究旨在为车载鱼眼相机（指向天顶）在城市场景中的视觉里程计和立体视觉应用找到最佳特征检测器和描述符。

Method: 方法包括：1) 创建并公开鱼眼序列数据集PFSeq；2) 在自标定背景下系统评估现有特征检测器和描述符算法；3) 使用车载鱼眼相机采集城市场景数据；4) 通过实验分析不同特征算法在鱼眼图像上的性能。

Result: 研究结果包括：1) 公开了PFSeq数据集；2) 提供了不同特征检测器和描述符在鱼眼图像上的性能评估；3) 确定了在自标定任务中最适合鱼眼图像的特征算法组合。但需要注意的是，这是2014年的研究成果，未经过同行评审，且未与专门为全向图像设计的算法进行比较。

Conclusion: 该研究为鱼眼图像处理提供了有价值的数据集和基准测试，解决了特征检测与相机标定之间的相互依赖问题。虽然研究存在局限性（未更新到最新技术、未与全向图像专用算法比较），但为车载鱼眼视觉系统的发展奠定了基础，特别是在城市环境中的定位应用。
...
TL;DR: VGGT-Motion提出了一种无需标定的单目SLAM系统，通过运动感知的子图构建、锚点驱动的Sim(3)配准和轻量级位姿图优化，解决了长序列中的尺度漂移问题，在千米级轨迹上实现了高效鲁棒的全局一致性。


<details>
  <summary>Details</summary>
Motivation: 尽管基于3D视觉基础模型的无标定单目SLAM取得了进展，但长序列中的尺度漂移问题仍然严重。运动无关的分区会破坏上下文连贯性并导致零运动漂移，而传统的几何对齐计算成本高昂。

Method: 1. 运动感知子图构建机制：使用光流指导自适应分区，修剪静态冗余，封装转弯以保持稳定的局部几何结构
2. 锚点驱动的直接Sim(3)配准策略：利用上下文平衡的锚点实现无搜索、像素级密集对齐和高效闭环，无需昂贵的特征匹配
3. 轻量级子图级位姿图优化：以线性复杂度强制执行全局一致性，支持可扩展的长距离操作

Result: 实验表明，VGGT-Motion显著提高了轨迹精度和效率，在零样本、长距离无标定单目SLAM中实现了最先进的性能。

Conclusion: VGGT-Motion通过创新的运动感知子图构建和高效的全局一致性维护机制，有效解决了长序列单目SLAM中的尺度漂移问题，为无需标定的长距离视觉定位提供了可行解决方案。
...
TL;DR: SSG是一种无需训练、推理时引导的方法，通过强调目标高频信号（语义残差）来纠正VAR模型在推理时的层次漂移问题，提升图像生成保真度和多样性。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型在推理时由于容量限制和累积误差，其粗到细的层次结构会发生漂移，导致训练与推理不一致的问题。

Method: 提出SSG方法：1）从信息论角度分析，确保每个尺度贡献高频内容；2）使用DSE频率域处理获得粗粒度先验；3）强调语义残差（目标高频信号）进行推理时引导。

Result: SSG在多个VAR模型上一致提升生成保真度和多样性，同时保持低延迟，揭示了粗到细图像生成中未开发的效率潜力。

Conclusion: SSG通过推理时引导纠正VAR模型的层次漂移问题，无需额外训练即可提升图像生成质量，具有广泛适用性。
...
TL;DR: 该研究系统评估了3D行人检测中相机、LiDAR及两者融合三种方法的性能，发现融合方法在复杂场景下表现最佳，但对传感器错位和某些LiDAR干扰敏感。


<details>
  <summary>Details</summary>
Motivation: 3D行人检测在机器人、工业监控和安防等应用中至关重要，但现有研究多集中于自动驾驶领域。本研究旨在评估不同传感器配置（相机、LiDAR、融合）在多样化室内外场景中的检测性能和鲁棒性。

Method: 使用JRDB数据集，在多样化室内外场景中评估三种代表性模型：BEVDepth（相机）、PointPillars（LiDAR）和DAL（相机-LiDAR融合）。分析它们在遮挡、距离变化、传感器损坏和错位等情况下的行为。

Result: 融合方法在所有场景中表现最佳，尤其在挑战性场景中优势明显。DAL对传感器错位和某些LiDAR干扰敏感，而相机方法BEVDepth性能最低，对遮挡、距离和噪声最敏感。

Conclusion: 传感器融合能显著提升3D行人检测性能，但系统仍存在对传感器错位和某些干扰的脆弱性。需要进一步研究解决这些系统固有的脆弱性问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于DiT的视频运动迁移方法存在计算效率低下的问题，虽然已有方法尝试加速DiT计算，但未能解决结构性的效率瓶颈。论文识别出两种计算冗余：运动冗余（帧间运动小而平滑，但通用DiT架构未利用这一特性）和梯度冗余（沿扩散轨迹的梯度变化缓慢）。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注家用或消费产品，或使用合成、干净的桌面数据集，或在受控实验室环境中捕获的对象，缺乏真实工业场景的代表性。需要创建一个专门针对工业机器人应用的真实数据集，以推动6D姿态估计方法在实际制造环境中的部署。

Method: 创建了包含五种具有挑战性属性的新物体类型的RGB-D数据集，在真实的工业装配环境中捕获。数据集分为经典集和扩展集两部分：经典集包含4.6k图像和6k标注姿态；扩展集提供额外数据模态支持模型无关和序列方法的评估，首次提供RGB-D静态上机序列。

Result: 数据集包含从简单到复杂场景的多样性，包括单个和多个物体场景，以及同一物体的多个实例。通过对代表性最先进方法的评估，包括实例级和新物体6D姿态估计、物体检测和分割，显示该领域仍有改进空间。

Conclusion: IndustryShapes为工业机器人应用中的6D姿态估计方法提供了一个真实且具有挑战性的基准测试平台，填补了实验室研究与实际部署之间的空白，并展示了该领域需要进一步研究的方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人应用中需要同时进行几何定位和估计任务相关参数属性的问题，弥合低级几何推理和可操作世界模型之间的差距，为可扩展的仿真训练感知系统铺平道路。

Method: 基于PI3DETR扩展，采用模块化、类别特定的头部设计，直接从遮挡影响的点云数据中联合估计多类别6自由度姿态和类别特定参数属性，3D模型根据预定义规则进行调整。

Result: 在自动叉车平台上验证，针对三种结构和功能不同的类别（起重机抓手、装载平台、托盘），在合成环境中训练后无需额外微调即可泛化到真实室外LiDAR扫描，检测mAP达到0.919。
...
TL;DR: ViGT是一种用于自动驾驶的视觉隐式几何变换器，能够从多视角摄像头估计连续的3D占用场，采用无监督训练，在多个数据集上达到SOTA性能。
...
Method: 提出校准无关的架构，通过自监督训练利用图像-LiDAR同步数据对，从多摄像头视图推断几何到统一的度量坐标系，估计BEV视角下的连续3D占用场。

Result: 在五个大规模自动驾驶数据集上训练，在点图估计任务上达到最佳平均排名，在Occ3D-nuScenes基准上与监督方法性能相当。

Conclusion: ViGT展示了构建可扩展、架构简单、能适应不同传感器配置的自动驾驶基础几何模型的可行性，为多几何任务提供了通用表示。
...
TL;DR: 提出了LocateEdit-Bench数据集，包含231K编辑图像，专门用于评估针对指令驱动图像编辑的定位方法，填补现有方法主要关注修复式操纵的不足。


<details>
  <summary>Details</summary>
Motivation: 当前图像编辑技术的进步使得视觉内容的操控更加可控和语义感知，给操纵定位带来了前所未有的挑战。现有的AI生成伪造定位方法主要关注修复式操纵，对最新的指令驱动编辑范式无效。

Method: 提出了LocateEdit-Bench大规模数据集，包含231K编辑图像，采用四种前沿编辑模型，涵盖三种常见编辑类型。开发了两种多指标评估协议来评估现有定位方法。

Result: 建立了一个专门针对指令驱动图像编辑的基准数据集，为跟上图像编辑技术发展提供了基础，促进了未来伪造定位方法的开发。

Conclusion: LocateEdit-Bench填补了指令驱动图像编辑定位评估的关键空白，为应对不断发展的图像编辑技术提供了必要的基准工具，数据集将在接受后开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统灵敏度分析工具（条件数分析、欧几里得扰动参数、费舍尔信息界）无法解释单个图像特征如何影响位姿估计，也不能说明为什么动态或不一致的观测会不成比例地扭曲现代SLAM和结构运动系统。

Method: 将影响函数理论扩展到矩阵李群，推导出SE(3)上左平凡化M估计器的固有扰动算子，提出几何可观测性指数(GOI)通过曲率算子和可观测子空间的李代数结构量化单个测量的贡献。

Result: GOI允许沿可观测曲率主方向进行谱分解，揭示弱可观测性与放大敏感性之间的直接对应关系；在总体状态下，GOI与SE(3)上的费舍尔信息几何一致，产生单测量版本的克拉美-罗界。

Conclusion: GOI提供了测量影响的几何一致描述，统一了条件数分析、费舍尔信息几何、影响函数理论和动态场景可检测性，其曲率谱和GOI可作为轻量级、无需训练的诊断信号，用于识别动态特征和检测弱可观测配置。
...
TL;DR: EgoPoseVR是一个用于VR中准确的第一人称全身姿态估计的端到端框架，通过融合头戴设备运动线索和RGB-D观测，解决了现有方法的时间不稳定性和实时性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于头戴相机的第一人称姿态估计方法在应用于VR头戴显示器时面临时间不稳定、下半身估计不准确和缺乏实时性能等挑战，需要更准确、稳定的全身姿态跟踪解决方案。

Method: 提出了一个端到端框架，通过双模态融合管道整合头戴设备运动线索和RGB-D观测。使用时空编码器提取帧级和关节级表示，通过交叉注意力融合互补运动线索，并采用运动学优化模块增强姿态估计的准确性和稳定性。

Result: 实验结果表明EgoPoseVR优于现有最先进的第一人称姿态估计模型。用户研究显示在准确性、稳定性、体现感和未来使用意愿方面获得显著更高的主观评分，无需额外身体传感器或房间尺度跟踪系统。

Conclusion: EgoPoseVR能够实现稳健的全身姿态跟踪，为准确的VR体现提供了实用解决方案，无需额外身体穿戴传感器或房间尺度跟踪系统。
...
TL;DR: CAViT提出了一种双注意力架构，用动态的通道注意力机制替代ViT中静态的MLP，通过空间自注意力后接通道自注意力实现内容感知的特征交互，在提升性能的同时减少了参数量和计算量。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers中的通道混合机制是静态的，依赖于固定的多层感知机，缺乏对输入内容的适应性。作者希望设计一种动态的、基于注意力的特征交互机制来增强模型的表达能力。

Method: 提出CAViT架构，在每个Transformer块中执行空间自注意力后接通道自注意力，用动态的注意力机制替代静态的MLP，实现内容感知的token混合策略。

Result: 在五个基准数据集（包括自然和医学领域）上验证，相比标准ViT基线准确率提升最高达+3.6%，同时参数量和FLOPs减少超过30%。注意力图显示更清晰、语义更有意义的激活模式。

Conclusion: CAViT通过统一的、内容感知的token混合策略增强了表征表达能力，而无需增加深度或复杂度，在多个视觉任务上表现出优越性能。
...
Result: 实验结果表明，该方法在各种非经典模型上都能有效重建多个实例，证明了其有效性。代码已在GitHub上开源。

Conclusion: 该研究填补了非经典模型多实例鲁棒拟合的空白，提出的方法能够有效处理噪声数据中的多个非经典模型实例，为复杂几何模型的鲁棒重建提供了新思路。
...
TL;DR: XSIM是一个用于自动驾驶的传感器仿真框架，通过扩展3DGUT splatting技术，引入广义滚动快门建模，解决了球形相机（如LiDAR）的投影问题，并在多个自动驾驶数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3DGUT splatting方法在处理自动驾驶应用中的球形相机（如LiDAR）时存在局限性，特别是在循环投影和方位边界的时间不连续性方面，导致粒子投影错误。需要一种能够准确模拟复杂传感器畸变和动态环境的统一框架。
...
Conclusion: XSIM提供了一个统一且灵活的传感器仿真框架，能够准确渲染动态环境中的复杂传感器畸变，特别解决了球形相机在自动驾驶应用中的关键边缘情况，为自动驾驶系统开发提供了高质量的传感器仿真工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于基础模型的手术视频分析方法主要依赖像素级重建目标，这浪费了模型在烟雾、镜面反射和流体运动等低层次视觉细节上的能力，而不是关注对手术理解至关重要的语义结构。

Method: 基于视频联合嵌入预测架构(V-JEPA)，UniSurg引入了三个针对手术视频的关键技术创新：1)运动引导的潜在预测以优先处理语义重要区域；2)时空亲和力自蒸馏以强制关系一致性；3)特征多样性正则化以防止纹理稀疏手术场景中的表示崩溃。模型在UniSurg-15M数据集上进行了大规模预训练。

Result: 在17个基准测试中的广泛实验表明，UniSurg在手术工作流识别(EgoSurgery上F1提升14.6%，PitVis上提升10.3%)、动作三元组识别(CholecT50上39.54% mAP-IVT)、技能评估、息肉分割和深度估计等任务上显著优于最先进方法。
...
TL;DR: 研究探索使用大五人格模型中更细粒度的层次（方面和细微特征）来提升从视听交互数据中的人格识别性能，发现细微特征级模型在多个交互场景中表现最佳


<details>
  <summary>Details</summary>
Motivation: 传统人格识别模型依赖广泛特质分数作为真实标签，但相似特质分数可能通过多样化的、情境依赖的行为表现，加上训练数据有限，导致泛化能力面临挑战

Method: 使用UDIVA v0.5数据集，训练基于Transformer的模型，包含跨模态（视听）和跨主体（双人感知）注意力机制，比较特质、方面和细微特征三个层次的人格识别效果

Result: 细微特征级模型在所有交互场景中一致优于方面和特质级模型，将均方误差降低了高达74%

Conclusion: 使用更细粒度的人格层次（特别是细微特征）可以显著提升从视听交互数据中的人格识别性能，为构建更准确、泛化能力更强的人格识别模型提供了新方向
...
<details>
  <summary>Details</summary>
Motivation: 当前3D基础模型能生成高质量资产，但精确3D编辑仍面临挑战。现有方法在视觉可控性、几何一致性和可扩展性之间存在困难权衡：优化方法速度慢，多视图2D传播存在视觉漂移，无训练潜在操作方法受限于冻结先验且无法从规模化中受益。

Method: ShapeUP将编辑定义为原生3D表示中的监督式潜在到潜在转换。基于预训练的3D基础模型，通过监督训练适应编辑任务。使用三元组（源3D形状、编辑后的2D图像、对应的编辑后3D形状）训练3D扩散变换器，学习直接映射。图像作为提示实现细粒度视觉控制。

Result: ShapeUP在身份保持和编辑保真度方面一致优于当前有训练和无训练的基线方法。实现了隐式的无掩码定位，同时保持与原始资产的严格结构一致性。

Conclusion: ShapeUP提供了一个稳健且可扩展的范式，用于原生3D内容创作，通过监督式潜在空间转换方法解决了3D编辑中的关键挑战。
...
TL;DR: 该研究针对户外智能相机系统易受篡改的问题，提出了基于规则和深度学习的两种图像篡改检测方法，比较了它们在准确性、计算需求和训练数据方面的表现，并提供了公开数据集。


<details>
  <summary>Details</summary>
Motivation: 户外智能相机系统在监控和安全应用中日益普及，但这些系统容易受到故意破坏或恶劣环境条件的影响而遭到篡改。当相机拍摄静态图像而非视频时，由于缺乏连续的时间序列帧，检测相机篡改变得更加困难。

Method: 提出了两种检测篡改图像的方法：1）基于规则的方法；2）基于深度学习的方法。研究旨在评估每种方法在准确性、计算需求和训练数据要求方面的表现，特别是在实际应用场景中。

Result: 深度学习模型提供了更高的准确性，而基于规则的方法更适合资源有限且无法进行长时间校准的场景。研究还提供了包含正常、模糊和旋转图像的公开数据集，以支持相机篡改检测方法的开发和评估。

Conclusion: 该研究为户外智能相机系统的篡改检测提供了实用的解决方案，深度学习方法在准确性方面表现更优，而规则方法在资源受限场景中更具实用性。公开数据集的发布填补了该领域资源不足的空白。
...
TL;DR: 本文提出AGFF-Embed方法，通过自适应融合全局和细粒度感知来改进多模态大语言模型嵌入，在MMEB和MMVP-VLM基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态嵌入模型（CLIP和MLLM）只能捕捉全局语义信息，而复杂场景需要同时处理全局和细粒度元素的混合感知模式，因此需要兼容的融合机制。

Method: 提出AGFF-Embed方法：1）提示MLLM生成关注不同维度语义信息的多个嵌入；2）自适应平滑聚合这些嵌入；3）采用显式梯度放大技术实现批量内难负样本增强，无需细粒度编辑数据集。

Result: 在MMEB和MMVP-VLM基准测试中，AGFF-Embed在通用理解和细粒度理解方面全面达到最先进的性能，优于其他多模态嵌入模型。

Conclusion: AGFF-Embed通过自适应融合全局和细粒度感知，有效解决了复杂场景中混合感知模式的需求，为多模态嵌入提供了更全面的语义表示能力。
...
Method: 提出DepthPrior框架，将深度作为先验知识而非融合特征：训练时使用深度损失加权(DLW)和深度损失分层(DLS)，推理时使用深度感知置信度阈值(DCT)。唯一开销是初始深度估计。

Result: 在四个基准数据集(KITTI、MS COCO、VisDrone、SUN RGB-D)和两个检测器(YOLOv11、EfficientDet)上验证，小目标检测提升达+9% mAP_S和+7% mAR_S，推理恢复率高达95:1。

Conclusion: DepthPrior在不增加传感器、不修改架构、不牺牲性能的情况下，利用深度先验知识显著提升小目标和远距离目标检测性能，为安全关键应用提供可靠解决方案。
...
TL;DR: FMPose3D：基于流匹配的单目3D姿态估计框架，通过ODE高效生成多个3D姿态假设，并使用重投影后验期望聚合模块获得准确预测
...
Method: 提出FMPose3D框架，将3D姿态估计建模为条件分布传输问题，使用流匹配学习ODE定义的流场，仅需少量积分步即可生成3D姿态样本。引入重投影后验期望聚合模块从多个假设中获取准确预测。

Result: 在Human3.6M、MPI-INF-3DHP等人体姿态数据集上超越现有方法，在Animal3D和CtrlAni3D动物姿态数据集上达到最先进性能，展示了跨领域的强大性能。

Conclusion: FMPose3D通过流匹配方法实现了高效的多假设3D姿态生成，结合后验期望聚合获得了准确预测，为单目3D姿态估计提供了新的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过复杂架构解决域间差异，但研究发现具有风格多样性的单摄像头数据能提供更好的泛化。虽然单摄像头数据易于收集，但由于缺乏跨视角变化而复杂性不足。因此需要一种方法来充分利用单摄像头数据的潜力。
...
TL;DR: Allocentric Perceiver是一种无需训练的方法，通过几何专家从图像恢复3D状态，建立与指令语义意图对齐的以目标为中心的参考框架，将心理旋转从隐式推理转为显式计算，显著提升VLMs在空间推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言导航/动作等空间任务需求的增长，视觉语言模型（VLMs）在以目标为中心的空间查询方面表现脆弱，这些查询需要从观察的相机视角转换到目标中心的参考框架进行推理。

Method: 使用现成的几何专家从一张或多张图像恢复度量3D状态，实例化与指令语义意图对齐的查询条件化以目标为中心的参考框架，通过确定性变换将重建几何转换到目标框架，并用结构化、几何基础的表征提示骨干VLM。

Result: 在多个骨干模型家族的空间推理基准测试中，Allocentric Perceiver在以目标为中心任务上获得一致且显著的提升（约10%），同时保持强大的以自我为中心性能，超越了空间感知微调模型以及最先进的开源和专有模型。

Conclusion: 通过将心理旋转从隐式推理卸载到显式计算，Allocentric Perceiver有效提升了VLMs在以目标为中心的空间推理能力，为空间任务提供了强大的解决方案。
...
TL;DR: FSR是一种受人类启发的视觉语言模型token剪枝框架，通过聚焦-扫描-精炼三阶段，在保持性能的同时显著减少推理延迟和内存占用。
...
TL;DR: 首次将视频生成模型引入视觉语言导航领域，提出SparseVideoNav方法，通过生成稀疏未来轨迹实现超快速导航，在超越视野导航任务上性能显著优于现有LLM方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航依赖详细的语言指令，这与真实世界导航需求不符。理想情况下，智能体应能仅凭简单高层意图在未知环境中自主导航。这引出了超越视野导航的挑战：在没有密集逐步指导的情况下定位远处不可见目标。现有LLM方法因依赖短视距监督而表现出短视行为，但简单扩展监督视距会破坏LLM训练稳定性。

Method: 发现视频生成模型天生受益于长视距监督来对齐语言指令，特别适合BVN任务。首次将视频生成模型引入该领域，但生成数十秒视频的延迟过高。提出SparseVideoNav方法，通过生成稀疏未来轨迹实现亚秒级轨迹推理，覆盖20秒视距，相比未优化版本实现27倍加速。

Result: SparseVideoNav在真实世界零样本实验中，在BVN任务上的成功率达到最先进LLM基线的2.5倍，并首次在具有挑战性的夜间场景中实现了这种能力。

Conclusion: 视频生成模型在超越视野导航任务中具有独特优势，SparseVideoNav通过稀疏未来轨迹生成实现了高效的长视距导航，为真实世界自主导航提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 蒸馏自回归扩散模型虽然能实时合成短视频，但在长序列生成中存在严重的误差累积问题。现有的测试时优化方法对图像或短视频有效，但在长序列中无法缓解漂移问题，因为存在不稳定的奖励景观和蒸馏参数的超敏感性。

Method: 提出测试时校正方法，无需训练，利用初始帧作为稳定参考锚点来校准采样轨迹中的中间随机状态

Result: TTC方法能与各种蒸馏模型无缝集成，以可忽略的开销延长生成长度，在30秒基准测试中匹配资源密集型训练方法的生成质量
...
TL;DR: 提出一种轻量级离散扩散轮廓细化管道，用于低数据场景下的鲁棒边界检测，在医疗影像、环境监测等领域表现优异，推理速度提升3.5倍。
...
Method: 采用轻量级离散扩散轮廓细化管道，使用带有自注意力层的CNN架构作为核心，以分割掩码为条件，迭代去噪稀疏轮廓表示。引入了多个创新适应方法：简化扩散过程、定制模型架构和最小化后处理，可在少于500张训练图像的数据集上生成密集、孤立的轮廓。

Result: 在医疗影像数据集KVASIR上优于多个SOTA基线，在HAM10K和自定义野火数据集Smoke上具有竞争力，同时推理帧率提升3.5倍。

Conclusion: 该方法在低数据场景下实现了鲁棒的边界检测，在多个领域数据集上表现优异，同时显著提升了推理效率，为资源受限环境下的边界检测提供了有效解决方案。
...
Method: 使用神经隐式函数从稀疏CTA平面重建心脏腔室和左心室心肌的3D形状。训练时，多层感知器从CTA的3D分割中学习形状先验；测试时，通过联合优化潜在代码和刚性变换来重建3D形状，模拟四个标准心尖视图。

Result: 在保留的CTA分割集上，平均Dice系数达到0.86±0.04。与临床标准Simpson双平面法相比，左心室体积误差显著降低(4.88±4.26 mL vs. 8.14±6.04 mL)，左心房体积误差也大幅降低(6.40±7.37 mL vs. 37.76±22.96 mL)。

Conclusion: 该方法为2D经胸超声心动图提供了更准确的3D腔室量化途径，通过从稀疏平面重建完整3D形状，显著提高了体积测量的准确性。
...
Result: 实验结果表明，CLIP-Map在各种压缩率下都优于基于选择的压缩框架，特别是在高压缩设置下取得了显著性能提升

Conclusion: CLIP-Map通过映射方法有效解决了CLIP压缩中的信息损失问题，为资源受限场景提供了高效的CLIP压缩解决方案
...
<details>
  <summary>Details</summary>
Motivation: 医学图像在不同临床中心存在分布偏移问题，阻碍预训练分割模型在多域部署。现有持续测试时适应方法存在错误累积和灾难性遗忘问题，而基于提示调优的方法又缺乏多尺度多样性、实例特定知识不足以及隐私泄露风险。
...
Conclusion: MGIPT通过多尺度全局-实例提示调优有效解决了医学图像分割中的持续测试时适应问题，增强了提示的尺度多样性，同时捕获了全局和实例级知识，实现了鲁棒的跨域适应。
...
<details>
  <summary>Details</summary>
Motivation: 流匹配作为扩散模型的替代方案，通常继承使用标准高斯分布作为源分布，但很少将源分布本身作为优化目标。本文旨在探索条件相关源分布的设计如何能够更好地利用丰富的条件信号。

Method: 提出在流匹配目标下学习条件相关的源分布，识别并解决直接引入条件时出现的分布坍塌和不稳定性问题，采用方差正则化和源-目标方向对齐策略，同时分析目标表示空间选择对结构化源分布的影响。

Result: 在多个文本到图像基准测试中取得一致且稳健的改进，包括FID收敛速度提升高达3倍，证明了条件相关源分布设计的实际优势。

Conclusion: 源分布的原则性设计在现代文本到图像系统中不仅是可行的，而且是有益的。条件相关的源分布能够更好地利用条件信号，通过适当的正则化和对齐策略可以解决稳定性问题，显著提升生成性能。
...
TL;DR: 提出LSA框架，通过局部语义对齐增强预训练视频生成模型的时序一致性，无需推理时控制信号
...
Result: 在nuScenes和KITTI数据集上的实验表明，仅用单轮微调的模型在视频生成评估指标上优于基线，并在mAP和mIoU指标上展示了更好的时序一致性

Conclusion: LSA框架能有效增强视频生成的时序一致性，无需推理时外部控制信号和额外计算开销，为自动驾驶场景提供了更实用的数据生成工具
...
TL;DR: RISE-Video是一个专注于推理能力的文本-图像到视频生成基准测试，包含467个人工标注样本和8个推理类别，通过4个评估指标系统评估模型对隐含世界规则的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型虽然在视觉保真度上取得了显著进展，但其对隐含世界规则的内化和推理能力仍是一个关键但未被充分探索的领域。现有评估主要关注表面美学，缺乏对深层认知推理能力的系统评估。

Method: 提出了RISE-Video基准测试，包含467个人工标注样本，涵盖8个推理类别（常识、空间动态等）。设计了四维评估协议：推理对齐、时间一致性、物理合理性和视觉质量。还提出了基于大型多模态模型的自动化评估管道来模拟人类中心评估。

Result: 对11个最先进的TI2V模型进行了广泛实验，结果显示这些模型在模拟隐含约束下的复杂场景方面存在普遍缺陷，为未来世界模拟生成模型的发展提供了关键见解。

Conclusion: RISE-Video基准测试将评估重点从表面美学转向深层认知推理，揭示了当前生成视频模型在推理能力方面的不足，为开发更具智能的世界模拟生成模型提供了重要指导。
...
TL;DR: VisRefiner是一个训练框架，通过让模型学习渲染预测与参考设计之间的视觉差异来改进截图到代码的生成质量，包含差异对齐监督和强化学习自优化阶段。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型直接从截图生成代码，但训练过程中没有观察生成代码的视觉结果。人类开发者会迭代渲染实现、比较设计差异并学习视觉差异与代码修改的关系，这启发了本研究的动机。

Method: 提出VisRefiner训练框架：1) 构建差异对齐监督，将视觉差异与对应代码编辑关联；2) 引入强化学习自优化阶段，模型通过观察渲染输出和目标设计的视觉差异来更新代码。

Result: VisRefiner显著提高了单步生成质量和布局保真度，同时赋予模型强大的自优化能力，证明了从视觉差异中学习对推进截图到代码生成的有效性。

Conclusion: 通过让模型学习视觉差异与代码修改之间的关系，VisRefiner框架有效提升了截图到代码生成的性能，模仿了人类开发者的迭代优化过程。
...
<details>
  <summary>Details</summary>
Motivation: 视觉生成模型的快速发展超越了传统评估方法，需要采用视觉语言模型作为替代评判者。当前主流的绝对点式评分标准存在随机不一致性和与人类感知对齐差的问题，需要更可靠的评估框架。

Method: 提出GenArena统一评估框架，采用成对比较范式替代绝对评分标准。通过系统分析传统评分标准的局限性，设计基于成对比较的稳定评估方法。

Result: 实验发现仅采用成对比较协议就能使现成的开源模型超越顶级专有模型。评估准确率提升超过20%，与权威LMArena排行榜的Spearman相关性达到0.86，远超点式方法的0.36相关性。

Conclusion: GenArena为视觉生成模型提供了严格且自动化的评估标准，通过成对比较范式解决了传统绝对评分标准的局限性，为社区提供了更可靠的评估基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有实时长视频生成方法采用流式调优策略，使用短上下文（无记忆）教师训练长上下文学生。这种结构差异导致学生-教师不匹配：教师无法访问长期历史，无法指导学生处理全局时间依赖，限制了学生的上下文长度
...
Conclusion: Context Forcing通过长上下文教师训练长上下文学生，解决了学生-教师不匹配问题，实现了更长的有效上下文和更好的长视频生成一致性
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型在2D任务上表现优异，但缺乏3D感知能力，需要一种方法将3D意识注入2D模型中

Method: 使用快速前馈3D重建流程增强教师模型：1) 将教师模型的2D特征提升为显式3D高斯表示；2) 将3D特征"飞溅"到新视角生成2D特征图；3) 用这些特征图监督学生模型，蒸馏几何基础知识

Result: 在单目深度估计、表面法线估计、多视角对应和语义分割等下游任务上显著优于先前工作，不仅大幅提升3D感知能力，还增强了2D特征的语义丰富度

Conclusion: Splat and Distill框架成功将稳健的3D意识注入2D视觉基础模型，通过前馈提升方法避免了特征平均伪影，创建了师生一致性相互提升的动态学习过程
...
### [78] [InterPrior: Scaling Generative Control for Physics-Based Human-Object Interactions](https://arxiv.org/abs/2602.06035)
...
<details>
  <summary>Details</summary>
Motivation: 人类很少在显式全身运动层面规划与物体的交互，而是通过高层意图（如可供性）定义目标，协调的平衡、接触和操作可以从底层物理和运动先验中自然涌现。扩展这些先验对于使仿人机器人能够在多样化环境中组合和泛化全身运动技能，同时保持物理上连贯的全身协调至关重要。

Method: InterPrior框架首先通过大规模模仿预训练，将完整参考的模仿专家提炼为多功能、目标条件化的变分策略，该策略从多模态观察和高层意图重建运动。然后通过物理扰动的数据增强和强化学习微调，提高对未见目标和初始化的能力，将重建的潜在技能整合到有效流形中。

Result: 该方法产生了一个能够超越训练数据泛化的运动先验，例如可以整合与未见物体的交互等新行为。研究还展示了其在用户交互控制和真实机器人部署方面的有效性。

Conclusion: InterPrior框架通过结合模仿学习和强化学习，成功创建了可扩展的生成控制器，使仿人机器人能够在多样化环境中实现物理上连贯的全身运动协调和泛化能力，为实际机器人应用提供了有前景的解决方案。
...
TL;DR: GeoThinker是一个主动感知框架，通过条件化检索几何证据而非被动融合，在空间推理任务中实现了新的SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在空间推理中通常被动地将几何信息作为全局流融合，导致语义-几何错位和冗余信号，需要从被动融合转向主动感知

Method: 提出GeoThinker框架，采用空间基础融合在选定VLM层进行，通过帧严格交叉注意力让语义视觉先验选择性查询和整合任务相关几何，并通过重要性门控校准

Result: 在VSI-Bench上达到72.6的峰值分数，创下新的SOTA，在具身指代和自动驾驶等下游场景中展现出强大的泛化能力和显著改进的空间感知
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要依赖文本链式思维进行推理，在视觉密集任务上效果有限。最近的方法虽然通过注入连续隐藏状态作为"视觉思维"来提升视觉性能，但往往以牺牲文本逻辑推理能力为代价。核心问题在于固定的、预定义的推理模式无法根据用户查询自适应选择最合适的思维模态。

Method: 采用混合自回归公式，统一了文本思维的下一个token预测和视觉思维的下一个嵌入预测。设计了系统化的推理模式策划策略，构建了SwimBird-SFT-92K数据集，覆盖所有三种推理模式。模型能够根据输入条件动态切换推理模式。

Result: 在涵盖文本推理和挑战性视觉理解的多样化基准测试中，SwimBird取得了最先进的结果，相比先前固定模式的多模态推理方法获得了稳健的提升。模型在保持强大文本逻辑能力的同时，显著提高了在视觉密集任务上的性能。
...
TL;DR: CAMCUE是一个多图像空间推理框架，通过显式使用相机姿态作为几何锚点，解决多视角图像推理和视角转换问题，显著提升推理精度和效率。
...
Method: 提出CAMCUE框架：1) 将每幅图像的相机姿态注入视觉标记；2) 将自然语言视角描述映射到目标相机姿态；3) 合成姿态条件化的想象目标视图来支持回答。同时构建CAMCUE-DATA数据集用于训练和评估。

Result: CAMCUE将整体准确率提升9.06%，从自然语言描述预测目标姿态的旋转准确率超过90%（20°内），平移准确率在0.5误差阈值内。推理时间从256.6秒大幅减少到1.45秒。

Conclusion: CAMCUE通过显式利用相机姿态作为几何锚点，有效解决了多图像空间推理中的视角转换问题，实现了高精度和实时推理，为实际应用场景提供了可行解决方案。
...
### [82] [XEmoGPT: An Explainable Multimodal Emotion Recognition Framework with Cue-Level Perception and Reasoning](https://arxiv.org/abs/2602.05496)
*Hanwen Zhang,Yao Liu,Peiyuan Jiang,Lang Junjie,Xie Jun,Yihui He,Yajiao Deng,Siyu Du,Qiao Liu*

Main category: cs.MM

TL;DR: XEmoGPT是一个可解释的多模态情感识别框架，通过专门的视频和音频情感线索桥接模块增强细粒度情感线索感知，并构建大规模数据集EmoCue支持线索级推理，同时提出新的评估指标EmoCue-360。


<details>
  <summary>Details</summary>
Motivation: 当前多模态情感识别方法面临两个主要挑战：1)通用模态编码器预训练时关注全局结构和通用语义而非细粒度情感线索，导致对情感信号敏感度有限；2)现有数据集在标注质量和规模之间存在权衡，导致情感线索监督不足，限制了线索级推理能力。此外，现有评估指标不足以评估线索级推理性能。

Method: 提出XEmoGPT框架，包含两个专门模块：视频情感线索桥接(VECB)和音频情感线索桥接(AECB)，通过精心设计的任务增强视频和音频编码器的细粒度情感线索感知能力。构建大规模数据集EmoCue支持线索级推理，并引入EmoCue-360自动评估指标和EmoCue-Eval基准数据集。

Result: 实验结果表明，XEmoGPT在情感线索感知和推理方面都取得了强大的性能表现。

Conclusion: XEmoGPT通过专门设计的模块、大规模数据集和新的评估指标，有效解决了多模态情感识别中的线索级感知和推理挑战，为可解释情感识别提供了新的解决方案。
...
### [83] [Artificial Intelligence as Strange Intelligence: Against Linear Models of Intelligence](https://arxiv.org/abs/2602.04986)
*Kendra Chilson,Eric Schwitzgebel*

Main category: cs.AI

TL;DR: 该论文批判AI发展的线性模型，提出"熟悉智能"和"陌生智能"概念，认为AI智能更可能是陌生智能，在不同领域表现出超人类与亚人类能力的混合，支持非线性智能模型，并讨论其对AI评估的影响。


<details>
  <summary>Details</summary>
Motivation: 批判当前对AI智能发展的线性理解模型，指出这种模型无法准确描述AI智能的复杂性和异质性，需要新的理论框架来理解AI智能的本质特征。

Method: 扩展Susan Schneider对AI进步线性模型的批判，引入"熟悉智能"和"陌生智能"两个新概念，发展并辩护非线性智能模型，认为"通用智能"不是统一能力而是实现广泛目标的能力。

Result: 提出AI智能更可能是"陌生智能"，在不同领域表现出超人类与亚人类能力的混合，甚至在同一领域内也会出现超人类洞察力与令人惊讶的错误并存的现象。

Conclusion: 如果AI是陌生智能，即使最强大的系统也可能在看似简单的任务上失败，单一任务的优秀表现不能证明广泛的通用智能，这对对抗性测试评估AI能力的方法有重要启示。
...
### [84] [Evaluating Large Language Models on Solved and Unsolved Problems in Graph Theory: Implications for Computing Education](https://arxiv.org/abs/2602.05059)
...
Method: 采用八阶段评估协议模拟真实数学探究过程，包括解释、探索、策略形成和证明构建等环节，测试LLM在已解决图论问题（线图优美性）和开放问题上的表现。

Result: 在已解决问题上，模型表现优异：提供正确定义、识别相关结构、准确回忆结果而不产生幻觉，并构建了经图论专家确认的有效证明。在开放问题上，模型能生成连贯解释和合理探索策略，但未能推进解决方案，且没有捏造结果而是承认不确定性。

Conclusion: LLMs能够支持已建立材料的探索，但在需要新颖数学洞察或关键结构推理的任务上仍有局限。对于计算教育，这强调了指导学生使用LLMs进行概念探索，同时依赖独立验证和严格论证进行正式问题解决的重要性。
...
### [85] [Towards Reducible Uncertainty Modeling for Reliable Large Language Model Agents](https://arxiv.org/abs/2602.05073)
...
TL;DR: 本文提出首个通用的智能体不确定性量化框架，将传统单轮问答UQ扩展到交互式智能体场景，提出条件不确定性减少的新视角，为LLM智能体安全应用提供指导。


<details>
  <summary>Details</summary>
Motivation: 当前不确定性量化研究主要关注单轮问答，但LLM智能体在复杂交互任务中部署日益增多，需要新的UQ框架来应对现实交互场景中的不确定性挑战。

Method: 提出首个通用的智能体UQ公式化框架，涵盖现有UQ设置；引入条件不确定性减少的新视角，强调动作的"交互性"来建模可减少的不确定性；构建概念框架为LLM智能体UQ设计提供指导。

Result: 揭示了先前工作隐含地将LLM UQ视为不确定性积累过程，这在开放世界交互智能体中失效；提出的条件不确定性减少视角能更好地建模智能体轨迹中的可减少不确定性。

Conclusion: 智能体UQ对于前沿LLM开发和领域特定应用具有重要实践意义，论文为交互式LLM智能体的不确定性量化提供了新框架和开放问题方向。
...
### [86] [Evaluating Robustness and Adaptability in Learning-Based Mission Planning for Active Debris Removal](https://arxiv.org/abs/2602.05091)
*Agni Bandyopadhyay,Günther Waxenegger-Wilfing*

Main category: cs.AI

TL;DR: 比较三种自主任务规划方法用于低地球轨道多碎片交会问题：固定参数训练的Masked PPO、领域随机化训练的Masked PPO和蒙特卡洛树搜索(MCTS)。结果显示，固定PPO在匹配训练条件时表现最佳但适应性差，随机化PPO适应性提升但性能略有下降，MCTS适应性最强但计算成本极高。


<details>
  <summary>Details</summary>
Motivation: 自主碎片清除任务规划需要在效率、适应性和严格的燃料与任务时间约束之间取得平衡。需要比较不同规划方法在处理约束多碎片交会问题时的表现，特别是在面对任务参数变化时的鲁棒性。

Method: 比较三种规划器：1) 在固定任务参数下训练的名义Masked PPO策略；2) 在不同任务约束下训练的领域随机化Masked PPO策略以提高鲁棒性；3) 朴素的蒙特卡洛树搜索基准方法。在包含加油、真实转移动力学和随机碎片场的高保真轨道模拟中进行评估，涵盖300个测试案例，包括名义、减少燃料和减少任务时间三种场景。

Result: 名义PPO在条件匹配训练时表现最佳，但在分布偏移下性能急剧下降；领域随机化PPO表现出更好的适应性，仅在名义性能上有适度损失；MCTS始终能最好地处理约束变化，但计算时间高出几个数量级。

Conclusion: 研究揭示了学习策略的速度与搜索方法的适应性之间的权衡，建议将训练时的多样性与在线规划相结合，可能是未来弹性ADR任务规划器的有前景路径。
...
### [87] [Democratic Preference Alignment via Sortition-Weighted RLHF](https://arxiv.org/abs/2602.05113)
*Suvadip Sana,Jinzhou Wu,Martin T. Wells*

Main category: cs.AI

TL;DR: DemPO框架通过算法抽签构建代表性人类评分者小组，在偏好对齐训练中实现人口统计学代表性，硬面板使用抽签样本，软面板通过包含概率加权，实验显示代表性训练优于后验校正


<details>
  <summary>Details</summary>
Motivation: 当前AI偏好对齐方法（如RLHF）依赖的人类评分者通常是便利样本，存在人口统计学代表性偏差，过度代表某些群体而低估其他群体，需要解决这种代表性不足问题

Method: 提出民主偏好优化（DemPO）框架，应用算法抽签机制构建公民大会式的代表性小组：1）硬面板方案：仅使用通过抽签采样的满足配额的小型公共样本的偏好进行训练；2）软面板方案：保留所有数据但根据抽签彩票中的包含概率重新加权每个评分者

Result: 在包含人类判断、评分者人口统计数据和75条款宪法的公共偏好数据集上，对1B到8B参数的Llama模型进行微调评估。在六种聚合方法中，硬面板始终排名第一，软面板始终优于未加权基线，且随着模型容量增加，效果规模增大

Conclusion: 在偏好收集阶段强制执行人口统计学代表性，而非事后校正，能够产生更好地反映从代表性公共群体中获取的价值观的模型行为，这证明了代表性训练的重要性
...
### [88] [SocialVeil: Probing Social Intelligence of Language Agents under Communication Barriers](https://arxiv.org/abs/2602.05115)
*Keyang Xuan,Pengda Wang,Chongrui Ye,Haofei Yu,Tal August,Jiaxuan You*

Main category: cs.AI

TL;DR: SocialVeil：一个模拟认知差异导致沟通障碍的社会学习环境，用于评估大语言模型在非理想沟通条件下的社会智能
...
Method: 基于对人类交互中沟通挑战的系统文献综述，SocialVeil引入了三种代表性的沟通障碍类型：语义模糊、社会文化不匹配和情感干扰。同时提出了两种障碍感知评估指标：未解决的困惑和相互理解

Result: 在720个场景和四个前沿LLMs上的实验显示，沟通障碍持续损害性能，相互理解平均降低超过45%，困惑度提升近50%。人类评估验证了模拟障碍的保真度（ICC≈0.78，Pearson r≈0.80）。适应策略（修复指令和交互学习）效果有限，远未达到无障碍性能

Conclusion: 这项工作使社会交互环境更接近真实世界沟通，为探索LLM智能体的社会智能开辟了机会。结果表明当前LLMs在现实沟通障碍下表现有限，需要进一步研究提升其社会智能
...
### [89] [CAST-CKT: Chaos-Aware Spatio-Temporal and Cross-City Knowledge Transfer for Traffic Flow Prediction](https://arxiv.org/abs/2602.05133)
*Abdul Joseph Fofanah,Lian Wen,David Chen,Alpha Alimamy Kamara,Zhongyi Zhang*

Main category: cs.AI

TL;DR: CAST-CKT是一个用于数据稀缺跨城市交通预测的混沌感知时空知识转移框架，通过量化交通可预测性机制实现机制自适应建模，在跨城市少样本设置中显著优于现有方法。
...
Method: 提出CAST-CKT框架：1）使用高效混沌分析器量化交通可预测性机制；2）机制自适应注意力进行时间建模；3）自适应拓扑学习处理动态空间依赖；4）基于混沌一致性的跨城市对齐进行知识转移；5）提供具有不确定性量化的特定时间范围预测。

Result: 在四个基准数据集上的跨城市少样本实验中，CAST-CKT在MAE和RMSE指标上显著优于现有最先进方法，同时提供可解释的机制分析。

Conclusion: CAST-CKT通过混沌感知建模有效解决了跨城市少样本交通预测的挑战，理论分析显示改进的泛化边界，实验验证了其优越性能。
...
### [90] [HugRAG: Hierarchical Causal Knowledge Graph Design for RAG](https://arxiv.org/abs/2602.05143)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图的RAG方法过度依赖表面节点匹配，缺乏显式因果建模，导致答案不忠实或虚假。先前尝试整合因果关系的方法通常局限于局部或单文档上下文，且受模块化图结构导致的信息隔离问题影响，阻碍了可扩展性和跨模块因果推理。
...
### [91] [First Proof](https://arxiv.org/abs/2602.05192)
...
TL;DR: 作者分享了10个研究级数学问题来评估当前AI系统回答数学研究问题的能力，这些问题来自作者自身研究过程，此前未公开，答案暂时加密
...
Method: 作者从自身研究过程中选取了10个自然产生的数学问题，这些问题此前从未公开分享，答案暂时加密，以此作为测试AI系统的基准

Result: 创建了一个包含10个研究级数学问题的测试集，这些问题来自实际数学研究过程，为评估AI数学推理能力提供了新的基准

Conclusion: 通过分享这些研究级数学问题，为评估AI系统在高级数学推理方面的能力提供了一个新的测试平台，有助于推动AI在数学研究领域的发展
...
### [92] [Traceable Cross-Source RAG for Chinese Tibetan Medicine Question Answering](https://arxiv.org/abs/2602.05195)
*Fengxian Chen,Zhilong Tao,Jiaxuan Li,Yunlong Li,Qingguo Zhou*

Main category: cs.AI

TL;DR: 该研究针对多知识库检索增强生成中的密度偏差问题，提出DAKS路由预算检索和对齐图证据融合方法，在藏医药领域提升跨知识库问答的溯源性和证据覆盖


<details>
  <summary>Details</summary>
Motivation: 在多知识库检索增强生成中，密集的百科全书条目容易主导检索，而经典文献和临床论文可能提供更权威的证据。藏医药领域存在这种多知识库异质性问题，需要解决密度驱动偏差并提升跨知识库验证能力。

Method: 提出两种互补方法：1) DAKS进行知识库路由和预算检索，缓解密度偏差并在适当时优先权威来源；2) 使用对齐图指导证据融合和覆盖感知打包，改进跨知识库证据覆盖而不依赖简单拼接。所有答案由轻量级生成器openPangu-Embedded-7B生成。

Result: 实验显示在路由质量和跨知识库证据覆盖方面取得一致提升，完整系统在保持强忠实性和引用正确性的同时，实现了最佳的CrossEv@5指标。

Conclusion: 该研究提出的方法能有效解决多知识库检索增强生成中的密度偏差问题，提升跨知识库问答的溯源性和证据覆盖，在藏医药领域具有实际应用价值。
...
### [93] [Surgery: Mitigating Harmful Fine-Tuning for Large Language Models via Attention Sink](https://arxiv.org/abs/2602.05228)
*Guozhi Liu,Weiwei Lin,Tiansheng Huang,Ruichao Mo,Qi Mu,Xiumin Wang,Li Shen*

Main category: cs.AI

TL;DR: 本文提出了一种名为Surgery的防御方法，通过注意力下沉机制来减轻有害微调对大型语言模型安全性的影响。该方法基于可分离下沉分歧假设，使用正则化器抑制下沉分歧，将注意力头引导至负下沉分歧组，从而减少模型学习和放大有害模式的倾向。


<details>
  <summary>Details</summary>
Motivation: 有害微调会破坏大型语言模型的安全对齐，暴露重大安全风险。现有方法在防御有害微调方面存在不足，需要一种有效的微调阶段防御机制来保护模型安全性。

Method: 首先测量每个注意力头的下沉分歧统计量，发现不同注意力头表现出两种不同的下沉分歧符号。基于可分离下沉分歧假设，提出Surgery防御方法，使用下沉分歧抑制正则化器，将注意力头引导至负下沉分歧组，从而减少模型学习有害模式的能力。

Result: 实验表明，Surgery在BeaverTails、HarmBench和SorryBench基准测试上分别提高了5.90%、11.25%和9.55%的防御性能。该方法有效减少了模型在有害微调过程中学习和放大有害模式的能力。

Conclusion: 通过注意力下沉机制和可分离下沉分歧假设，Surgery提供了一种有效的微调阶段防御方法，能够显著减轻有害微调对大型语言模型安全性的影响，为模型安全保护提供了新的解决方案。
...
### [94] [Automatic Cognitive Task Generation for In-Situ Evaluation of Embodied Agents](https://arxiv.org/abs/2602.05249)
...
<details>
  <summary>Details</summary>
Motivation: 随着智能体将在多样化家庭环境中广泛部署，针对每个独特未见3D环境的评估变得至关重要。现有基准存在严重的数据污染和缺乏场景特异性问题，无法有效评估智能体在未见环境中的能力。

Method: 提出TEA方法，采用结构化图表示定义任务，构建两阶段交互-进化任务生成系统：1)交互阶段：智能体主动与环境交互，在任务执行和生成间形成循环；2)进化阶段：通过任务图建模重新组合和重用现有任务生成新任务，无需外部数据。
...
### [95] [Beyond Cosine Similarity](https://arxiv.org/abs/2602.05266)
*Xinbo Ai*

Main category: cs.AI

TL;DR: 本文提出了一种新的相似性度量方法recos，它通过推导比经典柯西-施瓦茨界更紧的上界，用排序后的向量分量对点积进行归一化，从而超越了传统余弦相似度的线性限制。


<details>
  <summary>Details</summary>
Motivation: 余弦相似度作为向量空间中语义相似度的标准度量，基于柯西-施瓦茨不等式，只能捕捉线性关系，无法建模现实世界语义空间的复杂非线性结构。需要一种能更好处理复杂嵌入空间关系的数学原理更优的相似性度量方法。

Method: 通过推导比经典柯西-施瓦茨界更紧的点积上界，提出了recos相似性度量方法。该方法将点积用排序后的向量分量进行归一化，将完美相似的条件从严格线性依赖放宽到序数一致性，从而能捕捉更广泛的关系类型。

Result: 在11种嵌入模型（包括静态、上下文化和通用类型）上进行广泛实验，结果表明recos在标准语义文本相似性（STS）基准测试中，相比传统余弦相似度始终表现更优，与人类判断的相关性更高。

Conclusion: recos作为一种数学原理更优、经验上更优越的替代方法，为复杂嵌入空间中的语义分析提供了更高的准确性，超越了传统余弦相似度的局限性。
...
### [96] [Hallucination-Resistant Security Planning with a Large Language Model](https://arxiv.org/abs/2602.05279)
*Kim Hammar,Tansu Alpcan,Emil Lupu*

Main category: cs.AI

TL;DR: 本文提出一个原则性框架，将LLM集成到安全管理决策支持中，通过一致性检查和外部反馈循环控制幻觉风险，在事件响应用例中实现比前沿LLM减少30%的恢复时间。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在安全管理任务（如事件响应规划）中具有潜力，但其不可靠性和幻觉倾向仍是重大挑战。需要解决这些挑战，使LLM能可靠地用于安全管理的决策支持。

Method: 提出一个原则性框架，将LLM集成到迭代循环中：LLM生成候选行动，系统检查其与约束条件和前瞻预测的一致性；当一致性低时，放弃生成行动并收集外部反馈（如在数字孪生中评估行动）；通过上下文学习使用反馈来细化候选行动。

Result: 证明了该设计可通过调整一致性阈值来控制幻觉风险；在特定假设下建立了上下文学习的遗憾界限；在四个公共数据集上的实验表明，该框架相比前沿LLM将恢复时间减少了高达30%。

Conclusion: 提出的框架通过集成一致性检查和外部反馈循环，有效控制了LLM在安全管理中的幻觉风险，显著提高了事件响应规划的性能，为LLM在安全关键应用中的可靠使用提供了解决方案。
...
### [97] [PieArena: Frontier Language Agents Achieve MBA-Level Negotiation Performance and Reveal Novel Behavioral Differences](https://arxiv.org/abs/2602.05302)
*Chris Zhu,Sasha Cui,Will Sanok Dufallo,Runzhi Jin,Zhen Xu,Linjun Zhang,Daylian Cain*

Main category: cs.AI

TL;DR: GPT-5在PieArena谈判基准测试中表现达到或超过受过专业培训的商学院学生，显示出AGI级别的谈判能力，但在鲁棒性和可信度方面仍有不足。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在谈判这一核心商业任务中的能力，谈判需要战略推理、心理理论和经济价值创造能力，研究模型在现实商业场景中的实际表现。

Method: 引入PieArena大规模谈判基准，基于精英商学院MBA谈判课程中的真实场景，进行多智能体交互评估，并研究联合意向性智能体支架的效果。

Result: 前沿模型GPT-5达到或超过受过一学期谈判培训和针对性指导的商学院学生表现；联合意向性支架对中低层模型有显著提升，但对前沿模型收益递减；谈判行为分析揭示了模型在欺骗、计算准确性、指令遵从和感知声誉等方面的异质性。

Conclusion: 前沿语言智能体在智力和心理层面已具备在高风险经济环境中部署的能力，但鲁棒性和可信度方面的缺陷仍是开放挑战，谈判行为异质性表明仅关注交易结果的基准存在局限性。
...
### [98] [ProAct: Agentic Lookahead in Interactive Environments](https://arxiv.org/abs/2602.05327)
...
TL;DR: ProAct框架通过两阶段训练让LLM智能体学会前瞻推理：首先通过GLAD从环境搜索轨迹中蒸馏学习，然后通过MC-Critic增强策略优化，显著提升长期规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在需要长期规划的交互环境中表现不佳，主要因为模拟未来状态时会产生累积错误。需要让智能体学会准确的前瞻推理能力。

Method: 提出ProAct框架：1) GLAD阶段：通过监督微调从环境搜索轨迹中学习，将复杂搜索树压缩为简洁的因果推理链；2) MC-Critic阶段：使用蒙特卡洛批评器增强策略梯度算法，通过轻量级环境rollout校准价值估计。

Result: 在随机环境（如2048）和确定性环境（如Sokoban）上的实验表明，ProAct显著提高了规划准确性。4B参数模型训练后超越所有开源基线，媲美最先进的闭源模型，并在未见环境中表现出强大泛化能力。

Conclusion: ProAct通过让智能体内化准确的前瞻推理，有效解决了LLM在长期规划中的累积错误问题，为交互环境中的智能体规划提供了高效解决方案。
...
### [99] [RaBiT: Residual-Aware Binarization Training for Accurate and Efficient LLMs](https://arxiv.org/abs/2602.05367)
*Youngcheon You,Banseok Lee,Minseop Choi,Seonyoung Kim,Hyochan Chong,Changdong Kim,Youngmin Kim,Dongkyu Kim*

Main category: cs.AI

TL;DR: RaBiT提出了一种新的2位量化框架，通过算法强制残差层次结构解决二进制路径间的特征共适应问题，在保持硬件友好性的同时实现了最先进的性能。
...
Method: 提出RaBiT量化框架，通过算法强制残差层次结构解决共适应问题。核心机制是从单个共享全精度权重顺序推导每个二进制路径，确保每个路径纠正前一个路径的误差。采用稳健初始化策略，优先功能保留而非单纯权重近似。

Result: RaBiT重新定义了2位精度-效率前沿：实现了最先进的性能，甚至能与硬件密集的向量量化方法相媲美，在RTX 4090上相比全精度模型实现了4.49倍的推理加速。

Conclusion: RaBiT通过解决残差二值化中的特征共适应问题，在保持硬件友好性的同时显著提升了极端量化下的模型性能，为大型语言模型的高效部署提供了新的解决方案。
...
### [100] [Clinical Validation of Medical-based Large Language Model Chatbots on Ophthalmic Patient Queries with LLM-based Evaluation](https://arxiv.org/abs/2602.05381)
...
TL;DR: 本研究评估了4个小型医疗大语言模型在眼科患者咨询中的表现，发现Meerkat-7B表现最佳，MedLLaMA3-v20表现最差且有25.5%的回答包含幻觉或误导性内容。GPT-4-Turbo评估与临床医生评分高度一致，支持基于LLM的大规模基准测试可行性。


<details>
  <summary>Details</summary>
Motivation: 随着领域特定大语言模型在眼科患者教育、分诊和临床决策支持中的应用日益增多，需要进行严格评估以确保安全性和准确性。本研究旨在评估小型医疗LLM在回答眼科相关患者查询方面的表现，并探索基于LLM的评估与临床医生评分的可行性。

Method: 采用横断面研究设计，让4个参数规模小于100亿的小型医疗LLM（Meerkat-7B、BioMistral-7B、OpenBioLLM-8B、MedLLaMA3-v20）回答180个眼科患者查询，共生成2160个回答。由3名不同资历的眼科医生和GPT-4-Turbo使用S.C.O.R.E.框架（安全性、共识与上下文、客观性、可重复性、可解释性）进行五级李克特量表评分。使用Spearman秩相关、Kendall tau统计和核密度估计分析评估LLM与临床医生评分的一致性。

Result: Meerkat-7B表现最佳，分别获得高级顾问3.44分、顾问4.08分和住院医师4.18分的平均分。MedLLaMA3-v20表现最差，25.5%的回答包含幻觉或临床误导性内容，包括编造术语。GPT-4-Turbo评估与临床医生整体评估高度一致（Spearman rho=0.80，Kendall tau=0.67），但高级顾问评分更为保守。

Conclusion: 医疗LLM在眼科问题回答方面显示出安全潜力，但在临床深度和共识方面仍存在差距。研究支持基于LLM的大规模基准测试的可行性，并表明需要混合自动化和临床医生审查框架来指导安全的临床部署。
...
### [101] [H-AdminSim: A Multi-Agent Simulator for Realistic Hospital Administrative Workflows with FHIR Integration](https://arxiv.org/abs/2602.05407)
*Jun-Min Lee,Meong Hi Son,Edward Choi*

Main category: cs.AI

TL;DR: H-AdminSim是一个综合性的端到端仿真框架，结合真实数据生成和多智能体仿真，用于评估医院行政管理流程中LLM的自动化性能。


<details>
  <summary>Details</summary>
Motivation: 医院行政部门每天处理大量运营任务（大型医院超过1万条请求），现有研究主要关注医患交互或孤立的行政子任务，未能捕捉真实行政工作流的复杂性。

Method: 提出H-AdminSim框架，结合真实数据生成和多智能体仿真医院行政工作流，通过FHIR集成提供统一互操作环境，使用详细评分标准对任务进行定量评估。

Result: 该框架为评估LLM驱动的行政自动化可行性和性能提供了标准化测试平台，能够系统比较不同LLM在复杂医院行政工作流中的表现。

Conclusion: H-AdminSim填补了现有研究的空白，通过综合性仿真框架为医院行政自动化提供了有效的评估工具，支持跨异构医院环境的标准化测试。
...
### [102] [THOR: Inductive Link Prediction over Hyper-Relational Knowledge Graphs](https://arxiv.org/abs/2602.05424)
...
<details>
  <summary>Details</summary>
Motivation: 现有超关系知识图谱链接预测技术主要关注传导式设置，只能在同一词汇表内进行预测，缺乏对未见词汇表的泛化能力。需要开发能够跨不同词汇表进行推理的归纳式方法。
...
Conclusion: THOR通过建模超关系知识图谱中的结构不变性，实现了有效的归纳式链接预测，能够泛化到未见过的词汇表，为超关系知识图谱的跨领域应用提供了新思路。
...
### [103] [Day-Ahead Electricity Price Forecasting for Volatile Markets Using Foundation Models with Regularization Strategy](https://arxiv.org/abs/2602.05430)
...
<details>
  <summary>Details</summary>
Motivation: 电力价格预测对能源市场参与者至关重要，但由于价格信号固有的波动性和非线性，传统统计和深度学习模型难以有效捕捉复杂的时间依赖关系并整合异构数据。虽然时间序列基础模型在一般时间序列预测任务中表现出色，但在波动性市场中的日前电力价格预测效果尚未充分探索。

Method: 提出尖峰正则化策略，并评估了多种时间序列基础模型（包括Tiny Time Mixers、MOIRAI、MOMENT、TimesFM），与传统统计模型（ARIMA）和深度学习模型（LSTM、CNN-LSTM）进行比较。使用新加坡具有波动趋势的半小时批发市场数据，并在适用模型中纳入天气和日历变量等外生因素。

Result: 时间序列基础模型在所有评估设置中始终优于传统方法，在各种评估设置中实现了最高37.4%的平均绝对百分比误差改进。

Conclusion: 研究结果为提高波动性电力市场的预测准确性和决策制定提供了实用指导，证明了时间序列基础模型在电力价格预测任务中的优越性。
...
### [104] [Refine and Purify: Orthogonal Basis Optimization with Null-Space Denoising for Conditional Representation Learning](https://arxiv.org/abs/2602.05464)
...
Conclusion: OD-CRL通过正交基优化和零空间投影有效解决了条件表示学习中的基敏感性和子空间干扰问题，在各种定制化任务中取得了优异性能。
...
### [105] [ALIVE: Awakening LLM Reasoning via Adversarial Learning and Instructive Verbal Evaluation](https://arxiv.org/abs/2602.05472)
*Yiwen Duan,Jing Ye,Xinpei Zhao*

Main category: cs.AI

TL;DR: ALIVE框架通过对抗学习与指导性语言反馈，让LLM内部化推理逻辑，超越传统标量奖励的瓶颈，实现无需人工监督的推理能力提升。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习依赖标量奖励存在三个主要问题：扩展成本高、跨领域脆弱、无法理解解决方案的内在逻辑。这种依赖外部贫乏信号的方式阻碍了模型对推理原则的深度理解。
...
### [106] [Phi-Former: A Pairwise Hierarchical Approach for Compound-Protein Interactions Prediction](https://arxiv.org/abs/2602.05479)
*Zhe Wang,Zijing Liu,Chencheng Xu,Yuan Yao*

Main category: cs.AI

TL;DR: Phi-former是一种用于预测化合物-蛋白质相互作用的深度学习模型，通过分层表示学习和多级交互建模，结合了分子片段（功能基团）在生物识别中的关键作用，相比传统原子级方法更符合化学现实。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的化合物-蛋白质相互作用预测方法主要关注原子级建模，但忽略了分子片段（功能基团）作为生物识别和结合基本单元的重要性。这些模型虽然提高了效率和准确性，但未能完全符合化学现实，因为生物系统通常通过分子片段来识别和结合分子伙伴。

Method: Phi-former采用分层表示学习方法，将化合物和蛋白质表示为原子和分子片段两个层次。模型使用成对预训练框架，系统地建模原子-原子、片段-片段和原子-片段三个层次的相互作用，反映了生物系统识别分子伙伴的方式。设计了层内和层间学习管道，使不同交互层次相互促进。

Result: 实验结果表明，Phi-former在化合物-蛋白质相互作用相关任务上取得了优越性能。案例研究显示，该方法能够准确识别在化合物-蛋白质相互作用中被激活的特定原子或分子片段，提供了可解释的模型解释。

Conclusion: Phi-former通过结合分子片段在生物识别中的关键作用，提供了一种更符合化学现实的化合物-蛋白质相互作用预测方法。该方法不仅性能优越，还能提供可解释的洞察，有助于指导合理的药物设计和支持精准医疗应用。
...
### [107] [SDFP: Speculative Decoding with FIT-Pruned Models for Training-Free and Plug-and-Play LLM Acceleration](https://arxiv.org/abs/2602.05499)
...
Method: 提出SDFP框架，使用Fisher信息迹(FIT)评估层敏感性，通过剪除低影响层从原始LLM构建紧凑草稿模型，无需额外训练或超参数调优，保持与原始模型的兼容性。
...
### [108] [A Unified Multimodal Framework for Dataset Construction and Model-Based Diagnosis of Ameloblastoma](https://arxiv.org/abs/2602.05515)
*Ajo Babu George,Anna Mariam John,Athul Anoop,Balu Bhasuran*

Main category: cs.AI

TL;DR: 研究人员创建了一个专门针对成釉细胞瘤的多模态数据集，并开发了相应的深度学习模型，显著提高了变异分类准确率和异常组织检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前AI在颌面病理学诊断中面临两个主要问题：一是现有数据集对成釉细胞瘤的覆盖有限，二是数据格式不一致难以直接用于模型训练。需要高质量、结构化的多模态数据集来支持AI诊断模型的开发。

Method: 1. 创建专门针对成釉细胞瘤的多模态数据集，整合放射学、组织病理学和口腔内临床图像；2. 使用自然语言处理技术从文本报告中提取临床相关特征；3. 对图像数据进行领域特定的预处理和增强；4. 开发多模态深度学习模型，可接受临床表现、年龄、性别等临床输入。

Result: 模型性能显著提升：变异分类准确率从46.2%提高到65.9%，异常组织检测F1分数从43.0%提升到90.3%。相比MultiCaRe等现有资源，该工作提供了更强大的数据集和适应性强的多模态AI框架。

Conclusion: 该研究通过创建专门的多模态数据集和开发相应的AI模型，显著提升了成釉细胞瘤的诊断性能，为患者特异性决策支持提供了有力工具，推动了颌面病理学AI诊断的发展。
...
### [109] [Split Personality Training: Revealing Latent Knowledge Through Alternate Personalities](https://arxiv.org/abs/2602.05532)
...
TL;DR: SPT方法通过在LoRA参数中训练"诚实人格"，激活后能审查主模型响应并访问其潜在状态，有效检测大语言模型中的错位行为，在Anthropic审计游戏基准上达到96%准确率。
...
Result: 在Anthropic审计游戏模型生物基准测试中，SPT达到96%的整体准确率，而Anthropic报告接近0%准确率。诚实人格揭示了外部观察者无法访问的潜在知识，如受损模型被训练时的虚构偏见。

Conclusion: SPT通过训练独立的诚实人格来审查主模型响应，提供了一种有效检测大语言模型错位行为的方法，能够揭示模型隐藏的潜在知识和行为模式。
...
### [110] [Emulating Aggregate Human Choice Behavior and Biases with GPT Conversational Agents](https://arxiv.org/abs/2602.05597)
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否能在个体层面预测认知偏见，并模拟认知负荷等情境因素与偏见交互时的人类行为动态

Method: 将三个经典决策场景转化为对话设置，进行人类实验（N=1100），参与者通过简单或复杂对话与聊天机器人互动，然后使用GPT-4和GPT-5基于参与者人口统计数据和对话记录模拟相同条件

Result: 人类实验显示出明显的偏见模式，LLMs能够精确复制人类偏见，不同模型在模拟人类行为对齐方面存在显著差异

Conclusion: LLMs能够有效模拟人类偏见行为，这对设计和评估交互式环境中基于LLM的偏见感知AI系统具有重要意义
...
### [111] [Reactive Knowledge Representation and Asynchronous Reasoning](https://arxiv.org/abs/2602.05625)
...
<details>
  <summary>Details</summary>
Motivation: 复杂概率模型的精确推断计算成本过高，特别是在动态环境中需要频繁实时更新的自主智能体场景。现有方法在持续推理中效率低下，因为它们每次变化都重新评估整个模型，未能利用现实世界信息流具有异构更新率的特点。

Method: 1. 提出Resin（Reactive Signal Inference）概率编程语言，将概率逻辑与反应式编程相结合；2. 提出Reactive Circuits（RCs）作为Resin的高效精确语义，RCs是基于代数电路和异步数据流的元结构，是时间动态有向无环图，能够根据输入信号的波动性自主适应；3. 基于异步输入的变化频率估计对计算进行分区，将大型推断任务分解为可单独记忆的子问题。

Result: 在高保真无人机群模拟中，相比频率无关的推断方法实现了几个数量级的加速。RCs的结构适应成功捕捉了环境动态，显著降低了延迟，促进了反应式实时推理。通过基于变化频率的计算分区，确保只有受新信息影响的模型特定组件被重新评估，大幅减少了流式上下文中的冗余计算。

Conclusion: 通过反应式、异步的概率推理方法，结合Resin编程语言和Reactive Circuits结构，能够实现高效精确的实时推理，特别适用于动态环境中需要频繁更新的自主智能体应用。
...
### [112] [Generative Ontology: When Structured Knowledge Learns to Create](https://arxiv.org/abs/2602.05636)
*Benny Cheung*
...
<details>
  <summary>Details</summary>
Motivation: 传统本体论能描述领域结构但无法生成新内容，而大语言模型能流畅生成但缺乏结构有效性，经常产生没有组件的机制或没有结束条件的目标。需要结合两者的优势：本体论提供语法结构，LLM提供创造力

Method: 将领域知识编码为可执行的Pydantic模式，通过DSPy签名约束LLM生成。采用多智能体管道，为不同本体领域分配专门角色（如机制架构师、主题编织者、平衡批评家），每个智能体带有防止浅层输出的"专业焦虑"。使用检索增强生成将新设计基于现有范例，通过迭代验证确保机制与组件的一致性

Result: 通过GameGrammar系统演示框架，给定主题提示（如"洞穴生态系统中发光真菌的竞争"），管道能生成结构完整、可玩的桌面游戏设计，包含机制、组件、胜利条件和设置说明，既满足本体约束又保持真正的创造性

Conclusion: 该模式可推广到游戏之外的领域，任何具有专业词汇、有效性约束和积累范例的领域（音乐创作、软件架构、烹饪艺术）都适合Generative Ontology。约束不仅不限制创造力，反而使其成为可能：正如语法使诗歌成为可能，本体论使结构化生成成为可能
...
### [113] [Graph-based Agent Memory: Taxonomy, Techniques, and Applications](https://arxiv.org/abs/2602.05665)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型智能体在复杂长程任务中的应用，记忆成为核心模块。图结构因其建模关系依赖、组织层次信息和高效检索的内在能力，成为智能体记忆的强大结构。需要系统梳理基于图的智能体记忆研究现状。

Method: 1. 提出智能体记忆分类法：短期vs长期记忆、知识vs经验记忆、非结构化vs结构化记忆；2. 按记忆生命周期系统分析关键技术：记忆提取、存储、检索和演化；3. 总结开源库和基准测试；4. 探索应用场景；5. 识别挑战和未来方向。

Result: 建立了基于图的智能体记忆系统分析框架，收集整理了相关研究论文、开源数据和项目资源，为社区提供了全面的研究资源汇总（https://github.com/DEEP-PolyU/Awesome-GraphMemory）。

Conclusion: 图结构是构建智能体记忆系统的强大范式，本文提供了系统性的综述和分析框架，有助于推动更高效可靠的基于图的智能体记忆系统发展，为相关研究提供可操作的见解。
...
### [114] [Determining Energy Efficiency Sweet Spots in Production LLM Inference](https://arxiv.org/abs/2602.05695)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型推理在现代AI应用中至关重要，需要准确理解其能耗特征。现有方法通常使用输入输出序列长度的简单线性函数来估算能耗，但实际观测显示能效存在明显的非线性依赖关系，需要更精确的建模方法。
...
Result: 模型平均绝对百分比误差仅为1.79%，识别出能效"甜点"区域：短到中等输入和中等长度输出时能效最高，而长输入或极短输出时能效急剧下降。将序列长度与这些甜点区域对齐可显著降低能耗。

Conclusion: 提出的分析模型能准确表征LLM推理的能效曲线，支持生产系统中的智能截断、摘要和自适应生成策略，通过优化序列长度选择实现显著的能耗降低。
...
### [115] [Nonlinearity as Rank: Generative Low-Rank Adapter with Radial Basis Functions](https://arxiv.org/abs/2602.05709)
*Yihao Ouyang,Shiwei Li,Haozhao Wang,Xiandi Luo,Zhuoqi Hu,Yuetong Song,Qiyu Qin,Yichen Li,Ruixuan Li*

Main category: cs.AI

TL;DR: GenLoRA提出用非线性函数生成低秩矩阵的基向量，替代显式存储，在更少参数下获得更高有效秩，提升微调性能
...
Method: 提出生成式低秩适配器(GenLoRA)，用非线性基向量生成替代显式基向量存储。为每个低秩矩阵维护一个潜在向量，使用一组轻量级径向基函数(RBFs)合成基向量，每个RBF参数远少于显式基向量。

Result: 在多个数据集和架构上的实验表明，GenLoRA在更小的参数预算下获得更高的有效LoRA秩，实现更优的微调性能。

Conclusion: GenLoRA通过非线性基向量生成提高了参数效率，在有限参数预算下实现了更高的有效秩和更好的微调效果，为参数高效微调提供了新思路。
...
### [116] [Anchored Policy Optimization: Mitigating Exploration Collapse Via Support-Constrained Rectification](https://arxiv.org/abs/2602.05717)
*Tianyi Wang,Long Li,Hongcan Guo,Yibiao Chen,Yixia Li,Yong Wang,Yun Chen,Guanhua Chen*

Main category: cs.AI

TL;DR: 论文提出锚定策略优化(APO)方法，解决强化学习中递归空间收缩问题，通过从全局形状匹配转向支持覆盖，在保持效率的同时防止策略空间崩溃。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习中的验证奖励(RLVR)存在递归空间收缩(RSC)的系统性病理问题，即有效替代方案的采样概率消失。虽然KL正则化试图缓解此问题，但它强加了刚性的形状匹配约束，与正确性所需的锐化产生梯度冲突。

Method: 提出锚定策略优化(APO)方法，将范式从全局形状匹配转向支持覆盖。基于参考模型的高置信度支持定义安全流形，允许积极锐化以提高效率，同时在错误校正时选择性地调用恢复力以防止崩溃。

Result: 在数学基准测试中，APO打破了准确性与多样性的权衡，显著提高了Pass@1指标，同时恢复了标准策略梯度方法通常损失的Pass@K多样性。

Conclusion: APO作为一种梯度对齐机制，通过最大化支持覆盖实现弹性恢复，能够重新膨胀有效分支，解决强化学习中的递归空间收缩问题。
...
### [117] [Mitigating Hallucination in Financial Retrieval-Augmented Generation via Fine-Grained Knowledge Verification](https://arxiv.org/abs/2602.05723)
*Taoye Yin,Haoyuan Hu,Yaxin Fan,Xinhao Chen,Xinya Wu,Kai Deng,Kezun Zhang,Feng Wang*

Main category: cs.AI

TL;DR: 提出RLFKV框架，通过细粒度知识验证解决金融RAG系统中的幻觉问题，提高生成内容与检索文档的一致性


<details>
  <summary>Details</summary>
Motivation: 金融RAG系统虽然依赖检索文档来弥补知识缺口，但生成的响应仍存在与检索信息矛盾的幻觉问题，需要解决这种不一致性

Method: 提出强化学习框架RLFKV，将金融响应分解为原子知识单元，评估每个单元的正确性来计算细粒度忠实度奖励，并加入信息量奖励防止奖励黑客行为

Result: 在公开的FDD任务和新提出的FDD-ANT数据集上实验，显示该方法能带来一致的改进

Conclusion: RLFKV框架通过细粒度知识验证有效提高了金融RAG系统中生成内容与检索文档的一致性
...
### [118] [RocqSmith: Can Automatic Optimization Forge Better Proof Agents?](https://arxiv.org/abs/2602.05762)
*Andrei Kozyrev,Nikita Khramov,Denis Lochmelis,Valerio Morelli,Gleb Solovev,Anton Podkopaev*

Main category: cs.AI

TL;DR: 研究AI智能体自动优化方法在形式化验证领域的适用性，以Rocq自动定理证明为代表性挑战领域，评估不同优化器在优化Rocq证明生成智能体时的表现。


<details>
  <summary>Details</summary>
Motivation: 探索是否能够自动化智能体系统的精细调优过程，包括提示设计、上下文知识和控制策略等，以减轻人工工程负担并提高智能体性能。

Method: 在Rocq自动定理证明环境中，应用多种自动智能体优化方法（包括few-shot bootstrapping等）来优化证明生成智能体，评估这些方法对提示设计、上下文知识和控制策略的自动化调优能力。

Result: 多个优化器都能带来可测量的性能提升，其中简单的few-shot bootstrapping方法表现最稳定有效；但所有研究的自动优化方法都无法达到精心设计的state-of-the-art证明智能体的性能水平。

Conclusion: 自动智能体优化方法在形式化验证领域有一定应用价值，但当前技术仍无法完全替代人工精心设计的智能体系统，few-shot bootstrapping是最可靠的自动化方法。
...
### [119] [RL-VLA$^3$: Reinforcement Learning VLA Accelerating via Full Asynchronism](https://arxiv.org/abs/2602.05765)
...
TL;DR: 本文提出首个完全异步的VLA模型训练框架，通过环境交互、策略生成和模型更新的全流程异步化，解决了现有同步训练框架的资源利用率低和吞吐量瓶颈问题，在LIBERO基准上实现最高126.67%的吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型的训练效率已成为通用具身智能发展的关键瓶颈。虽然RLinf等基于强化学习的训练框架能提升模型泛化能力，但仍依赖同步执行，导致环境交互、策略生成和模型更新阶段的资源利用率严重不足和吞吐量受限。

Method: 提出首个完全异步策略训练框架，采用多级解耦架构：1) 环境交互和轨迹收集的异步并行化；2) 策略生成的流式执行；3) 训练更新的解耦调度。从大模型RL的异步优化思想中系统汲取灵感。

Result: 在LIBERO基准上，相比现有同步策略，框架实现最高59.25%的吞吐量提升；深度优化分离策略后，吞吐量可提升高达126.67%。消融实验验证了各异步组件的有效性，8到256个GPU的扩展性验证显示方法在大多数条件下具有优秀可扩展性。

Conclusion: 提出的完全异步训练框架有效解决了VLA模型训练的效率瓶颈，通过全流程异步化显著提升了资源利用率和训练吞吐量，为大规模具身智能模型的训练提供了高效解决方案。
...
### [120] [FiMI: A Domain-Specific Language Model for Indian Finance Ecosystem](https://arxiv.org/abs/2602.05794)
...
TL;DR: FiMI是针对印度数字支付系统开发的金融领域专用语言模型，包含基础版和指令版两个变体，基于Mistral Small 24B架构，通过多阶段训练实现，在金融推理和工具调用方面显著优于原模型。
...
Method: 基于Mistral Small 24B架构，采用多阶段训练流程：1）在680亿个经过筛选的金融、多语言（英语、印地语、印英混合语）和合成数据上进行持续预训练；2）指令微调；3）针对多轮、工具驱动的对话进行领域特定的监督微调，模拟真实工作流程如交易纠纷和授权生命周期管理。

Result: FiMI基础版在金融推理基准测试上比Mistral Small 24B基础模型提升20%；FiMI指令版在领域特定工具调用上比Mistral Small 24B指令模型提升87%。同时，在保持与同规模模型在通用基准上相当性能的前提下，实现了显著的领域性能提升。

Conclusion: FiMI成功开发了针对印度数字支付系统的专用金融语言模型，通过领域专业化训练显著提升了金融推理和工具调用能力，同时保持了通用性能，为印度金融科技应用提供了有效的AI解决方案。
...
### [121] [TKG-Thinker: Towards Dynamic Reasoning over Temporal Knowledge Graphs via Agentic Reinforcement Learning](https://arxiv.org/abs/2602.05818)
*Zihao Jiang,Miao Peng,Zhenyan Shan,Wenjie Xu,Ben Liu,Gong Chen,Ziqi Gao,Min Peng*

Main category: cs.AI

TL;DR: TKG-Thinker：一种具有自主规划和自适应检索能力的新型智能体，通过动态多轮交互和双训练策略在时序知识图谱上进行推理，解决了LLM在时序知识图谱问答中的推理幻觉和静态提示限制问题。
...
Method: 提出TKG-Thinker智能体，通过动态多轮交互与时序知识图谱进行深度时序推理，采用双训练策略：首先使用思维链数据进行监督微调以培养核心规划能力，然后通过强化学习阶段利用多维奖励在复杂时序约束下优化推理策略。
...
### [122] [OmniVideo-R1: Reinforcing Audio-visual Reasoning with Query Intention and Modality Attention](https://arxiv.org/abs/2602.05847)
...
TL;DR: OmniVideo-R1是一个增强的多模态推理框架，通过自监督学习和对比学习策略提升音视频理解能力，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 人类通过多种模态协同感知世界，但现有的全视频模型在音视频理解任务上仍面临重大挑战。需要开发能够更好整合多模态信息的框架。
...
Conclusion: OmniVideo-R1通过"全模态线索思考"的方法，显著提升了混合模态推理能力，为音视频理解任务提供了有效的解决方案。
...
### [123] [BABE: Biology Arena BEnchmark](https://arxiv.org/abs/2602.05857)
...
TL;DR: BABE是一个专门评估生物AI系统实验推理能力的基准测试，基于同行评审研究论文和真实生物研究构建，挑战模型进行因果推理和跨尺度推断。


<details>
  <summary>Details</summary>
Motivation: 现有生物学基准测试未能充分评估研究人员所需的关键技能：将实验结果与背景知识整合以得出有意义结论的能力。需要填补这一空白，评估AI系统在生物学研究中的实际推理能力。

Method: 基于同行评审研究论文和真实世界生物研究构建BABE基准测试，确保任务反映实际科学探究的复杂性和跨学科性质。该基准专门设计用于评估模型的因果推理和跨尺度推断能力。

Result: BABE提供了一个稳健的框架，用于评估AI系统如何像实践科学家一样进行推理，为衡量其在生物学研究中的潜在贡献提供了更真实的测量标准。

Conclusion: BABE基准测试填补了现有评估空白，能够更真实地评估生物AI系统的实验推理能力，为衡量AI在生物学研究中的实际应用潜力提供了重要工具。
...
### [124] [Beyond Manual Planning: Seating Allocation for Large Organizations](https://arxiv.org/abs/2602.05875)
*Anton Ipsen,Michael Cashmore,Kirsty Fielding,Nicolas Marchesotti,Parisa Zehtabi,Daniele Magazzeni,Manuela Veloso*

Main category: cs.AI

TL;DR: HSAP是解决层次化组织结构团队在平面布局中座位分配优化的问题，通过PRM和RRT计算座位距离，结合启发式搜索和动态规划进行整数规划求解。


<details>
  <summary>Details</summary>
Motivation: 大型组织需要确保具有紧密层级关系的团队在物理位置上相邻（如研究小组占据连续区域），目前手动管理导致重新规划频率低且效果不佳。

Method: 提出端到端框架：使用概率路线图(PRM)和快速探索随机树(RRT)计算座位间距离，结合启发式搜索和动态规划方法，通过整数规划求解HSAP。

Result: 在不同规模实例下评估PRM框架和后续分配方案，进行定量和定性分析，证明了方法的有效性。

Conclusion: 该框架能够自动化解决层次化座位分配问题，减少手动规划工作量，提高组织座位布局的优化效果。
...
TL;DR: 论文为M&S领域研究人员提供使用LLMs的实用指南，指出常见误区并强调原则性设计选择、诊断策略和实证评估的重要性。


<details>
  <summary>Details</summary>
Motivation: LLMs在建模与仿真(M&S)工作流中应用日益广泛，但看似简单的实践可能引入微妙问题、不必要的复杂性甚至导致结果劣化。研究人员需要避免常见误区，做出明智的使用决策。

Method: 通过讨论常见困惑来源（包括非确定性、知识增强如RAG和LoRA、M&S数据分解、超参数设置），强调原则性设计选择、诊断策略和实证评估方法。

Result: 提供全面的实用指导，帮助建模者了解何时、如何以及是否依赖LLMs，避免数据过载、不必要的微调、非确定性误解等常见问题。

Conclusion: LLMs在M&S应用中需要谨慎使用，应基于原则性设计、系统诊断和实证评估来做出明智决策，而非盲目采用看似简单的实践方法。
...
<details>
  <summary>Details</summary>
Motivation: 探索量子强化学习在复杂组合优化问题（特别是带容量约束的车辆路径问题）中的应用潜力，比较经典、全量子和混合方法的效果。

Method: 实现经典、全量子和混合三种变体的优势演员-评论家（A2C）智能体，集成transformer架构通过自注意力和交叉注意力机制捕捉车辆、客户和仓库之间的关系，在20个客户和4辆车的多车辆场景中进行实验。

Result: 所有三种方法都能学习有效的路由策略，但量子增强模型优于经典基线，产生更稳健的路由组织，混合架构在距离、紧凑性和路由重叠度方面获得最佳整体性能。

Conclusion: 量子增强模型在CVRP问题上表现出色，混合量子-经典强化学习模型在解决复杂组合优化问题方面具有显著潜力。
...
TL;DR: 该研究探索了预训练ASR模型Whisper在语音情感识别中的应用，提出了两种基于注意力的池化方法来降低Whisper表征维度并保留情感特征，在英语和波斯语数据集上取得了优异表现。


<details>
  <summary>Details</summary>
Motivation: 语音情感识别研究面临标准大规模数据集缺乏的挑战，现有研究利用预训练模型提取特征。本研究探索Whisper预训练ASR系统在语音情感识别中的能力，旨在提供轻量高效的替代方案。
...
Conclusion: 研究结果突显了Whisper作为SER表征提取器的潜力，并证明了基于注意力的池化在维度缩减方面的有效性，为语音情感识别提供了新的技术路径。
...
TL;DR: AgenticPay是一个用于评估多智能体买卖谈判的基准框架，通过自然语言驱动，包含110多个任务，涵盖从双边议价到多对多市场的各种场景。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏评估多智能体之间语言驱动的经济互动的原则性设置，而基于大语言模型的智能体越来越多地需要自主协商、协调和交易。

Method: 引入AgenticPay基准和模拟框架，模拟买卖双方拥有私有约束和产品相关估值的市场，通过多轮语言谈判而非单纯数字竞价达成协议。框架支持结构化行动提取和可行性、效率、福利等指标。

Result: 对最先进的专有和开源大语言模型的基准测试显示，在谈判性能方面存在显著差距，突显了长期战略推理方面的挑战。
...
TL;DR: 开发数据驱动的离散事件模拟器，用于评估学校安全干预策略，特别是机器人射击手干预，通过VR数据训练模拟器实现可扩展评估
...
Method: 开发数据驱动的离散事件模拟器，将射击手移动和区域内行动建模为从VR研究中参与者行为学习的随机过程，用于评估机器人射击手干预策略
...
### [130] [DCER: Dual-Stage Compression and Energy-Based Reconstruction](https://arxiv.org/abs/2602.04904)
...
<details>
  <summary>Details</summary>
Motivation: 多模态融合面临两个鲁棒性挑战：噪声输入会降低表示质量，模态缺失会导致预测失败。现有方法通常单独处理这些问题，需要一个统一框架同时解决这两个挑战。

Method: 提出DCER框架，包含两个阶段：1) 压缩阶段：在模态内使用频率变换（音频用小波变换，视频用DCT）去除噪声；在跨模态使用瓶颈token强制真正的融合而非模态特定的捷径。2) 基于能量的重建：通过梯度下降在学习的能量函数上恢复缺失模态的表示，最终能量值提供内在的不确定性量化。

Result: 在CMU-MOSI、CMU-MOSEI和CH-SIMS基准测试中达到最先进性能，表现出U型鲁棒性模式（在完整模态和高缺失条件下都表现良好）。能量值与预测误差的相关性ρ > 0.72，提供有效的内在不确定性量化。

Conclusion: DCER通过双阶段压缩和基于能量的重建，统一解决了多模态融合中的噪声和模态缺失问题，实现了鲁棒的多模态表示学习，并提供内在的不确定性量化。
...
### [131] [Denoising diffusion networks for normative modeling in neuroimaging](https://arxiv.org/abs/2602.04886)
*Luke Whitbread,Lyle J. Palmer,Mark Jenkinson*

Main category: cs.LG

TL;DR: 本文提出使用去噪扩散概率模型（DDPM）作为表格化成像表型的统一条件密度估计器，通过采样获得单变量百分位数和偏差分数，相比传统单变量建模能更好地保留多变量依赖结构。


<details>
  <summary>Details</summary>
Motivation: 传统神经影像分析通常为每个成像表型单独拟合模型，虽然计算效率高但忽略了多变量之间的依赖关系，而这些依赖可能编码了协调的生物学模式。需要一种既能保持单变量百分位数校准，又能建模多变量依赖的统一方法。

Method: 使用去噪扩散概率模型（DDPM）作为条件密度估计器，采用两种去噪器架构：1）基于特征线性调制（FiLM）的多层感知机（MLP）；2）具有特征自注意力和样本间注意力的表格变换器（SAINT），通过学习的嵌入处理协变量条件。在合成基准测试和UK Biobank FreeSurfer表型上进行评估，维度从2到200。

Result: 在低维度下，扩散模型提供与传统基线相当的校准良好的单变量输出，同时联合建模了真实的多变量依赖结构。在高维度下，变换器架构比MLP保持更好的校准性，并更好地保留高阶依赖关系，实现了可扩展的联合规范建模，同时与标准单变量流程兼容。

Conclusion: 扩散基础的规范建模为神经影像中校准的多变量偏差剖面提供了一条实用路径，能够在保持单变量百分位数校准的同时建模多变量依赖结构，特别在高维度下变换器架构表现更优。
...
### [132] [A Causal Perspective for Enhancing Jailbreak Attack and Defense](https://arxiv.org/abs/2602.04893)
*Licheng Pan,Yunsheng Lu,Jiexi Liu,Jialing Tao,Haozhe Feng,Hui Xue,Zhixuan Chu,Kui Ren*

Main category: cs.LG

TL;DR: 论文提出Causal Analyst框架，利用LLM进行因果发现来识别越狱的直接原因，通过35k次越狱尝试数据集分析37个人类可读提示特征，发现特定特征如"正面角色"和"任务步骤数"是越狱的直接因果驱动因素，并开发了越狱增强器和护栏顾问应用。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型(LLMs)中"越狱"机制对于提升其安全性和可靠性至关重要，但现有研究主要分析潜在表示而忽略了可解释提示特征与越狱发生之间的因果关系。

Method: 提出Causal Analyst框架，整合LLM到数据驱动的因果发现中，构建包含35k次越狱尝试的数据集，涵盖7个LLM、100个攻击模板和50个有害查询，标注37个人类可读提示特征，联合训练LLM提示编码和GNN因果图学习来重建因果路径。

Result: 分析发现特定特征如"正面角色"和"任务步骤数"是越狱的直接因果驱动因素；开发了越狱增强器显著提升攻击成功率，以及护栏顾问从混淆查询中提取真实恶意意图；实验证实因果分析的鲁棒性优于非因果方法。

Conclusion: 从因果角度分析越狱特征是提高LLM可靠性的有效且可解释的方法，因果分析为理解和防御LLM越狱提供了新视角。
...
### [133] [Momentum Attention: The Physics of In-Context Learning and Spectral Forensics for Mechanistic Interpretability](https://arxiv.org/abs/2602.04902)
*Kingsuk Maitra*

Main category: cs.LG

TL;DR: 论文将Transformer视为物理电路，提出动量注意力机制，通过引入运动学动量实现单层归纳头形成，建立了辛几何与信号处理的对偶关系。


<details>
  <summary>Details</summary>
Motivation: 扩展Transformer的机械可解释性框架，将其视为具有守恒定律和时变交流动力学的物理电路，通过引入物理先验来克服拓扑深度约束。

Method: 提出动量注意力机制，通过运动学差分算子p_t = q_t - q_{t-1}嵌入物理先验，实现辛剪切变换，建立辛-滤波器对偶性，证明物理剪切在数学上等价于高通滤波器。

Result: 125M动量模型在归纳密集型任务上表现优异，接近350M基线模型的验证损失（~2.9%差距），通过5100+控制实验验证，发现缩放定律γ* = 4.17 × N^{-0.74}，实现动量与深度的可替代性。

Conclusion: 该框架为生成式AI、哈密顿物理和信号处理提供了互补的分析工具包，通过引入运动学动量实现了单层归纳头形成，突破了传统架构的深度限制。
...
### [134] [Mind the Performance Gap: Capability-Behavior Trade-offs in Feature Steering](https://arxiv.org/abs/2602.04903)
...
<details>
  <summary>Details</summary>
Motivation: 特征导向作为通过直接操纵内部表征来控制LLM行为的方法，相比提示工程具有优势，但其在实际应用中的有效性尚不明确，特别是在输出质量方面可能存在权衡
...
Conclusion: 当前特征导向方法在实际部署中存在局限性，特别是在不能牺牲任务性能的场景下。机械控制方法面临基本的能力-行为权衡，需要在部署前进行实证表征
...
### [135] [LISA: Laplacian In-context Spectral Analysis](https://arxiv.org/abs/2602.04906)
*Julio Candanedo*
...
<details>
  <summary>Details</summary>
Motivation: 该研究的动机是开发一种能够在推理时自适应调整拉普拉斯时间序列模型的方法，仅使用观测到的数据前缀，特别是在动态变化的环境中提升预测性能。研究者希望将上下文自适应与非参数谱方法联系起来。

Method: LISA方法结合延迟坐标嵌入和拉普拉斯谱学习，生成扩散坐标状态表示，并使用冻结的非线性解码器进行一步预测。引入了基于高斯过程回归或类注意力马尔可夫算子的轻量级潜在空间残差适配器，作用于上下文窗口。

Result: 在预测和自回归展开实验中，LISA相比冻结基线有所改进，在动态变化的情况下尤其有益。该方法成功将上下文自适应与非参数谱方法联系起来。

Conclusion: LISA提供了一种有效的推理时自适应方法，通过结合谱学习和上下文适应技术，在动态变化的时间序列预测任务中表现出优越性能，为拉普拉斯时间序列模型的自适应提供了新思路。
...
### [136] [Physics as the Inductive Bias for Causal Discovery](https://arxiv.org/abs/2602.04907)
*Jianhong Chen,Naichen Shi,Xubo Yue*

Main category: cs.LG

TL;DR: 提出一个结合物理知识的因果发现框架，将系统建模为随机微分方程，其中漂移项编码已知ODE动力学，扩散项对应超出物理模型的未知因果耦合。


<details>
  <summary>Details</summary>
Motivation: 因果发现通常基于数据驱动，而物理模型如常微分方程提供动力学过程的机制结构。将这两种范式结合可以让物理知识作为归纳偏置，提高动力学系统中因果发现的可识别性、稳定性和鲁棒性。然而，现实动力学系统常存在反馈、循环相互作用和非平稳数据趋势，而许多因果发现方法基于无环性或平衡假设，这使得整合具有挑战性。

Method: 提出一个整合的因果发现框架，将系统演化建模为随机微分方程(SDE)。漂移项编码已知的ODE动力学，扩散项对应超出规定物理知识的未知因果耦合。开发了一个可扩展的稀疏诱导最大似然估计算法，利用因果图结构进行高效参数估计。

Result: 在具有不同因果结构的动力学系统上的实验表明，该方法相比纯数据驱动的先进基线方法，能提高因果图恢复能力，并产生更稳定、物理一致的估计。

Conclusion: 该方法成功地将物理知识作为归纳偏置整合到因果发现中，为处理具有反馈、循环相互作用和非平稳趋势的现实动力学系统提供了一个有效的框架，在因果图恢复和估计稳定性方面优于纯数据驱动方法。
...
### [137] [Temporal Pair Consistency for Variance-Reduced Flow Matching](https://arxiv.org/abs/2602.04908)
*Chika Maduabuchi,Jindong Wang*

Main category: cs.LG

TL;DR: TPC是一种轻量级方差减少方法，通过耦合概率路径上配对时间步的速度预测来降低梯度方差，提高采样效率，无需修改模型架构或概率路径
...
Method: 提出时间配对一致性（TPC）原则，在估计器层面耦合同一概率路径上配对时间步的速度预测，通过轨迹耦合的正则化减少梯度方差，不改变模型架构、概率路径或求解器。

Result: 在CIFAR-10和ImageNet多个分辨率上，TPC提高了样本质量和采样效率，在相同或更低计算成本下获得更低的FID分数，并能无缝扩展到现代SOTA风格管道（噪声增强训练、基于分数的去噪、整流流）。

Conclusion: TPC是一种有效的方差减少方法，通过理论分析和实验验证，在保持流匹配目标的同时显著提高生成模型的采样效率和质量。
...
### [138] [Learning Where It Matters: Geometric Anchoring for Robust Preference Alignment](https://arxiv.org/abs/2602.04909)
...
<details>
  <summary>Details</summary>
Motivation: 现有DPO方法使用固定参考策略，随着策略漂移会导致分布不匹配，在噪声监督下放大虚假偏好信号；而无参考变体则存在无约束的奖励漂移问题。

Method: 提出几何锚点偏好优化(GAPO)，用动态几何感知锚点替代固定参考——在当前策略的小半径内创建对抗性局部扰动作为悲观基线。引入锚点间隙度量策略与锚点间的奖励差异，在平滑条件下近似最坏情况的局部边际退化。通过该间隙加权的逻辑目标优化，降低几何脆弱实例的权重，强调鲁棒的偏好信号。

Result: 在多种噪声设置下，GAPO一致提升了鲁棒性，同时在标准LLM对齐和推理基准测试中匹配或改进了性能。

Conclusion: GAPO通过动态几何感知锚点机制有效解决了固定参考策略的校准问题和无参考方法的奖励漂移问题，为噪声环境下的偏好优化提供了更鲁棒的解决方案。
...
### [139] [A$^2$-LLM: An End-to-end Conversational Audio Avatar Large Language Model](https://arxiv.org/abs/2602.04913)
...
<details>
  <summary>Details</summary>
Motivation: 当前对话数字人系统大多采用级联架构，存在误差累积、高延迟、实时性差等问题，且缺乏对话上下文理解，过度依赖唇部同步而忽视情感深度
...
Conclusion: A²-LLM通过端到端统一框架解决了级联架构的局限性，能够生成情感丰富的面部运动，为下一代人机交互提供了更自然、响应更快的数字人解决方案
...
### [140] [SLAY: Geometry-Aware Spherical Linearized Attention with Yat-Kernel](https://arxiv.org/abs/2602.04915)
*Jose Miguel Luna,Taha Bouhsine,Krzysztof Choromanski*

Main category: cs.LG

TL;DR: SLAY是一种基于Yat核的线性时间注意力机制，通过将查询和键约束到单位球面，使注意力仅依赖于角度对齐，实现了与标准softmax注意力几乎无法区分的性能，同时保持线性时间和内存复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统softmax注意力机制具有二次复杂度，限制了Transformer模型的可扩展性。现有线性时间注意力方法通常存在性能折衷，需要开发更接近softmax注意力的高效近似方法。

Method: 提出SLAY方法：1) 将查询和键约束到单位球面，使注意力仅依赖于角度对齐；2) 基于Bernstein定理将球面Yat核表示为非负混合的多项式-指数乘积核；3) 推导严格正随机特征近似，实现O(L)线性时间注意力。

Result: SLAY在性能上与标准softmax注意力几乎无法区分，同时保持线性时间和内存缩放。在实验中，SLAY一致优于先前的线性时间注意力机制如Performers和Cosformers，是目前报道的最接近softmax注意力的线性时间近似。

Conclusion: SLAY实现了可扩展的Transformer模型，无需典型的注意力线性化性能折衷，代表了线性时间注意力机制的重要进展，为大规模语言模型提供了高效的替代方案。
...
### [141] [Multi-Aspect Mining and Anomaly Detection for Heterogeneous Tensor Streams](https://arxiv.org/abs/2602.04917)
*Soshi Kakio,Yasuko Matsubara,Ren Fujiwara,Yasushi Sakurai*

Main category: cs.LG

TL;DR: HeteroComp：一种处理包含分类和连续属性的异构张量流的方法，使用高斯过程先验建模连续属性和时间动态，实现准确的群体异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有张量分解和异常检测方法存在两个主要局限：1) 无法处理包含分类属性（如IP地址）和连续属性（如数据包长度）的异构张量流；2) 离散化时间戳，无法追踪流的时间动态（如趋势、异常事件），导致无法有效检测群体层面的异常（如DoS攻击）。

Method: 提出HeteroComp方法，将异构张量流持续总结为表示每个属性中潜在群体及其时间动态的"组件"。使用高斯过程先验建模连续属性的未知分布和时间动态，直接从数据估计概率密度。提取的组件提供简洁有效的总结，实现准确的群体异常检测。

Result: 在真实数据集上的大量实验表明，HeteroComp在群体异常检测准确率上优于最先进的算法，且其计算时间不依赖于数据流长度。

Conclusion: HeteroComp通过高斯过程先验有效建模异构张量流中的连续属性和时间动态，解决了现有方法在处理混合数据类型和追踪时间动态方面的局限，实现了高效的群体异常检测。
...
### [142] [Simulated Adoption: Decoupling Magnitude and Direction in LLM In-Context Conflict Resolution](https://arxiv.org/abs/2602.04918)
*Long Zhang,Fangwei Lin*

Main category: cs.LG

TL;DR: 该研究通过几何分析揭示了LLMs在处理知识冲突时的机制：不是通过稀释信号幅度，而是通过正交干扰来旋转隐藏状态表示，从而绕过正确的解嵌入向量。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解LLMs在面对上下文信息与参数记忆冲突时，如何通过顺从行为（sycophancy）解决知识冲突。具体探究这种抑制是通过信号幅度稀释还是方向性几何改变实现的。

Method: 对Qwen-4B、Llama-3.1-8B和GLM-4-9B进行层间几何分析，将反事实上下文引起的残差流更新分解为径向（基于范数）和角度（基于余弦）分量。

Result: 实验结果否定了"流形稀释"假说的普遍性，发现两个架构在保持稳定残差范数的同时仍表现出显著性能下降。顺从行为一致表现为"正交干扰"，即冲突上下文注入一个与真实方向准正交的转向向量，有效旋转隐藏状态表示。

Conclusion: 模型不会"忘记"或抑制内部真相的幅度，而是采用几何位移机制来绕过正确的解嵌入向量，在保持原始结构幅度的同时模拟采纳行为。这挑战了基于标量置信度的幻觉检测方法，强调需要向量监控来区分真正的知识整合和表面的上下文模仿。
...
### [143] [Gradually Compacting Large Language Models for Reasoning Like a Boiling Frog](https://arxiv.org/abs/2602.04919)
...
TL;DR: 提出了一种渐进式压缩方法PTL，通过多次细粒度迭代逐步压缩大语言模型，在保持推理性能的同时将模型压缩至近一半大小


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然具有强大的推理能力，但模型规模大导致计算资源消耗高。传统剪枝方法直接移除冗余参数会导致推理任务性能急剧下降，且需要大量后训练来恢复能力

Method: 提出渐进式压缩方法PTL，将压缩过程分为多个细粒度迭代，每个阶段应用剪枝-调优循环，逐步减小模型规模，同时通过微调恢复性能

Result: PTL可以将LLM压缩至接近原始大小的一半，仅需轻量级后训练，就能在推理任务上保持与原始模型相当的性能。该方法灵活适用于多种剪枝策略和后训练方法

Conclusion: PTL方法通过渐进式压缩有效解决了大语言模型压缩中的性能损失问题，具有广泛的适用性，不仅适用于数学推理，也适用于代码生成等多种任务
...
### [144] [CyIN: Cyclic Informative Latent Space for Bridging Complete and Incomplete Multimodal Learning](https://arxiv.org/abs/2602.04920)
...
Method: 提出Cyclic INformative Learning框架：1) 通过token和标签级信息瓶颈循环构建信息潜在空间；2) 使用变分近似捕获任务相关特征；3) 提出跨模态循环翻译，通过前向和反向传播过程重构缺失模态。

Result: 在4个多模态数据集上的实验表明，该方法在完整和多种不完整场景下均表现出优越性能。

Conclusion: CyIN框架成功在统一模型中联合优化完整和不完整多模态学习，有效解决了动态缺失模态场景下的鲁棒性问题。
...
Method: 提出三阶段框架：1）差分内在动作诱导收集潜在推理成功案例；2）稀疏控制基构建创建几何多样的工具库；3）价值调制轨迹干预通过锚点门控动态注入上下文特定脉冲

Result: 在6个算术和逻辑基准测试中，相比原始解码，STIR将平均准确率提升1.9%至7.5%，同时减少高达35%的平均token消耗
...
### [147] [TurboBoA: Faster and Exact Attention-aware Quantization without Backpropagation](https://arxiv.org/abs/2602.04929)
*Junhan Kim,Yeo Jeong Park,Seungwoo Son,Chungman Lee,Ho-young Kim,Joonyoung Kim,Yongkweon Jeon*

Main category: cs.LG

TL;DR: TurboBoA是一种新的后训练量化算法，在保持BoA精度优势的同时显著加速量化过程，通过联合量化多个输出通道、误差传播校正和自适应网格计算等创新，在低比特量化中实现最先进结果。


<details>
  <summary>Details</summary>
Motivation: GPTQ虽然高效但假设层间独立导致低比特量化精度下降严重，BoA虽然通过考虑注意力模块的层间依赖提高了精度，但其顺序量化所有输出通道的方式效率低下。需要一种既能保持BoA精度优势又能显著加速的后训练量化方法。

Method: TurboBoA提出三个关键创新：1) 使用闭式误差补偿规则联合量化多个输出通道，减少顺序瓶颈；2) 对先前量化层传播的误差进行校正；3) 采用坐标下降优化的自适应网格计算，在迭代更新中保持对齐。

Result: 实验表明TurboBoA相比BoA实现显著加速（超过3倍），同时持续提高精度。结合异常值抑制技术后，在仅权重量化和权重-激活量化中都达到了最先进的结果。

Conclusion: TurboBoA是一种无需反向传播的后训练量化算法，成功解决了BoA的效率问题，在保持精度优势的同时大幅加速，为大规模语言模型的低比特量化提供了高效解决方案。
...
### [148] [Depth-Wise Emergence of Prediction-Centric Geometry in Large Language Models](https://arxiv.org/abs/2602.04931)
...
<details>
  <summary>Details</summary>
Motivation: 理解仅解码器大型语言模型中从上下文处理到预测形成的计算过程，探究表征几何如何重组以及这种几何结构如何实现因果控制

Method: 结合几何分析和机制干预的统一框架，分析深度方向上的表征转变，研究角度组织和范数编码在预测形成中的作用

Result: 发现LLM深层表征实现结构化几何编码，角度组织参数化预测分布相似性，范数编码上下文特定信息但不决定预测，实现选择性因果控制

Conclusion: 提供了LLM将上下文转化为预测的动态过程的机制-几何解释，揭示了表征几何在实现因果控制中的关键作用
...
### [149] [Comparing Euclidean and Hyperbolic K-Means for Generalized Category Discovery](https://arxiv.org/abs/2602.04932)
*Mohamad Dalal,Thomas B. Moeslund,Joakim Bruslund Haurum*

Main category: cs.LG

TL;DR: HC-GCD提出在双曲空间中直接进行聚类，而不是像现有方法那样只在双曲空间学习表示然后回到欧几里得空间聚类，在语义转移基准测试中达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 现有双曲广义类别发现方法只在表示学习阶段使用双曲几何，聚类时仍回到欧几里得空间，作者认为这种混合方法不是最优的，需要在双曲空间中直接进行聚类。

Method: 提出HC-GCD方法：在洛伦兹双曲面模型的双曲空间中学习嵌入表示，并使用双曲K-Means算法直接在双曲空间中对这些嵌入进行聚类。

Result: 在语义转移基准数据集上，HC-GCD与先前最先进的双曲GCD方法性能相当；双曲K-Means比欧几里得K-Means准确率更高；欧几里得嵌入范数裁剪对未见类和已见类准确率有不同影响；双曲K-Means在不同标签粒度下产生更一致的聚类。

Conclusion: 直接在双曲空间中进行聚类比混合方法更优，双曲K-Means能提高聚类准确性和一致性，为广义类别发现任务提供了更好的双曲几何应用方案。
...
### [150] [Transolver-3: Scaling Up Transformer Solvers to Industrial-Scale Geometries](https://arxiv.org/abs/2602.04940)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在神经PDE求解器方面展现出强大能力，但将其扩展到工业级几何体（超过1亿个网格单元）面临根本性挑战，主要是高分辨率网格处理的内存复杂度过高。

Method: 1) 利用矩阵乘法结合律实现更快的切片和反切片；2) 几何切片平铺技术来划分物理状态计算；3) 在原始高分辨率网格的随机子集上进行摊销训练策略；4) 推理期间的物理状态缓存技术。

Result: Transolver-3能够处理超过1.6亿个网格单元的网格，在包括飞机和汽车设计任务在内的三个具有挑战性的仿真基准测试中表现出色。

Conclusion: Transolver-3通过创新的架构优化和训练策略，成功解决了神经PDE求解器在工业级几何体上的可扩展性问题，为高保真物理仿真提供了有效解决方案。
...
TL;DR: 论文提出了两种蒸馏方法（π-Distill和OPSD），用于将特权信息训练的语言模型能力转移到无特权信息的推理策略中，在多个智能体基准测试中超越了行业标准方法。


<details>
  <summary>Details</summary>
Motivation: 在强化学习中，训练时的特权信息（PI）可以帮助语言模型完成原本无法完成的任务，但如何将基于PI学习的能力转移到推理时无PI的策略中是一个根本性挑战。特别是在前沿模型蒸馏到多轮智能体环境中时，闭源系统通常只暴露动作轨迹而隐藏内部推理过程，这使得标准蒸馏流程失效。

Method: 提出了两种方法：1) π-Distill：联合教师-学生目标，使用同一模型同时训练PI条件化的教师和无条件的学生；2) OPSD（策略上自蒸馏）：使用强化学习训练，在学生和PI条件化教师之间添加反向KL惩罚。两种方法都仅使用动作轨迹作为特权信息进行蒸馏。

Result: π-Distill和在某些情况下的OPSD，在多个智能体基准测试、模型和PI形式上，都超越了行业标准实践（监督微调后接RL，假设有完整思维链监督）。研究还通过广泛分析揭示了有效学习PI的关键因素。

Conclusion: π-Distill和OPSD能够有效利用仅动作轨迹的特权信息来蒸馏前沿智能体，为解决训练时PI到推理时无PI的策略转移问题提供了有效解决方案，在多种设置下优于现有行业标准方法。
...
### [152] [Near-Optimal Dynamic Matching via Coarsening with Application to Heart Transplantation](https://arxiv.org/abs/2602.04989)
...
Result: 在现实模拟中，该策略的表现接近全知基准的性能。研究为先前器官分配中基于聚类的方法提供了严格的理论依据。

Conclusion: 该研究通过粗化方法开发了具有强理论保证的在线匹配算法，成功应用于心脏移植分配，弥合了实践启发式方法与理论保证之间的差距，为基于聚类的器官分配方法提供了理论支持。
...
### [153] [Position: Machine Learning for Heart Transplant Allocation Policy Optimization Should Account for Incentives](https://arxiv.org/abs/2602.04990)
*Ioannis Anagnostides,Itai Zilberstein,Zachary W. Sollie,Arman Kilic,Tuomas Sandholm*

Main category: cs.LG

TL;DR: 器官分配不仅是静态优化问题，而是涉及移植中心、临床医生和监管机构的复杂博弈，需要关注激励机制


<details>
  <summary>Details</summary>
Motivation: 当前器官分配系统从基于规则向机器学习过渡，但忽视了激励机制这一根本障碍，导致激励错位产生不良后果

Method: 以美国成人心脏移植分配为例，识别决策流程中的关键激励错位，提出整合机制设计、战略分类、因果推断和社会选择的研究议程

Result: 发现当前激励错位正在产生不良后果，需要新一代激励感知的分配政策

Conclusion: 器官分配应是激励感知的，需要机器学习社区整合博弈论和社会科学方法，确保在面对战略行为时的鲁棒性、效率和公平性
...
### [154] [EntRGi: Entropy Aware Reward Guidance for Diffusion Language Models](https://arxiv.org/abs/2602.05000)
*Atula Tejaswi,Litu Rout,Constantine Caramanis,Sanjay Shakkottai,Sujay Sanghavi*

Main category: cs.LG

TL;DR: 本文提出EntRGi方法，通过熵感知奖励引导动态调节奖励模型的梯度，解决了离散扩散语言模型中奖励引导的优化问题，在多个基准测试中超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型在测试时适应中面临奖励引导的挑战：连续松弛方法会降低梯度反馈质量（因为奖励模型从未训练过连续输入），而直接估计器方法则存在优化错误（在离散标记处评估的梯度被用于更新连续logits）。

Method: 提出EntRGi（熵感知奖励引导）方法，通过基于模型置信度动态调节奖励模型梯度，调制连续松弛过程，为奖励模型提供可靠输入同时改善奖励引导效果。

Result: 在7B参数的扩散语言模型上，针对3个不同的奖励模型和3个多技能基准测试进行验证，结果显示EntRGi方法相比现有最先进方法取得了持续改进。

Conclusion: EntRGi方法通过动态梯度调节机制，有效解决了离散扩散语言模型中奖励引导的优化难题，在保持奖励模型输入可靠性的同时显著提升了奖励引导效果。
...
### [155] [Enhanced QKNorm normalization for neural transformers with the Lp norm](https://arxiv.org/abs/2602.05006)
...
### [156] [Private PoEtry: Private In-Context Learning via Product of Experts](https://arxiv.org/abs/2602.05012)
*Rob Romijnders,Mohammad Mahdi Derakhshani,Jonathan Petit,Max Welling,Christos Louizos,Yuki M. Asano*

Main category: cs.LG

TL;DR: 本文提出了一种基于专家乘积模型的理论框架，用于保护隐私的上下文学习，显著提高了准确率并保持了强隐私保证。


<details>
  <summary>Details</summary>
Motivation: 上下文学习中的示例可能包含隐私敏感信息，现有差分隐私方法要么计算成本高，要么基于启发式方法效果有限，需要更有效的隐私保护方案。

Method: 通过专家乘积模型的理论框架重新构建隐私保护的上下文学习，该算法可以轻松并行化，避免了现有方法中的启发式限制。

Result: 在文本分类、数学和视觉语言五个数据集上的评估显示，该方法相比先前差分隐私上下文学习方法平均提高了30多个百分点的准确率。

Conclusion: 提出的基于专家乘积模型的框架为隐私保护的上下文学习提供了理论基础，在保持强隐私保证的同时显著提高了性能。
...
### [157] [A Simple Reduction Scheme for Constrained Contextual Bandits with Adversarial Contexts via Regression](https://arxiv.org/abs/2602.05019)
*Dhruv Sarkar,Abhishek Sinha*

Main category: cs.LG

TL;DR: 提出一种基于SquareCB框架的模块化算法方案，用于处理具有对抗性上下文的约束上下文赌博机问题，通过在线回归预言机将约束问题转化为无约束问题


<details>
  <summary>Details</summary>
Motivation: 研究具有对抗性选择上下文的约束上下文赌博机问题，现有工作主要关注随机上下文，需要为更一般的对抗性上下文设置提供改进保证

Method: 基于SquareCB框架，提出模块化算法方案，利用在线回归预言机将约束问题转化为具有自适应定义替代奖励函数的无约束上下文赌博机问题

Result: 为对抗性上下文设置提供了改进的保证，同时实现了紧凑且透明的分析

Conclusion: 提出的算法方案能够同时控制遗憾和累积约束违反，在对抗性上下文设置下优于现有工作
...
### [158] [Laws of Learning Dynamics and the Core of Learners](https://arxiv.org/abs/2602.05026)
*Inkee Jung,Siu Cheong Lau*

Main category: cs.LG

TL;DR: 该论文提出基于熵的终身集成学习方法，通过免疫机制防御对抗攻击，在CIFAR-10数据集上相比简单平均集成表现更优。


<details>
  <summary>Details</summary>
Motivation: 研究学习动力学的基本定律（守恒定律和总熵减少），并基于此开发能够防御对抗攻击的集成学习方法，解决传统集成方法在面对强扰动时性能下降的问题。

Method: 提出基于熵的终身集成学习框架，构建免疫机制来防御基于迁移的对抗攻击，通过logifold方法整合专门处理干净样本和对抗样本的模型。

Result: 在CIFAR-10数据集上，相比简单平均集成方法，logifold在大多数测试情况下获得更高准确率，特别是在强扰动条件下提升显著。

Conclusion: 基于熵的终身集成学习框架能够有效提升模型对对抗攻击的鲁棒性，特别是在强扰动环境下表现优异，验证了学习动力学定律在实践中的应用价值。
...
### [159] [Laplacian Representations for Decision-Time Planning](https://arxiv.org/abs/2602.05031)
...
<details>
  <summary>Details</summary>
Motivation: 基于模型的强化学习中，决策时规划仍面临挑战，状态表示需要同时支持局部成本计算和保持长期结构

Method: 提出ALPS分层规划算法，利用拉普拉斯表示构建有效规划潜空间，捕捉多时间尺度的状态空间距离，将长期问题分解为子目标

Result: 在OGBench基准测试的离线目标条件RL任务中，ALPS超越了常用的基线方法，该基准先前主要由模型自由方法主导

Conclusion: 拉普拉斯表示为规划提供了有效的潜空间，能够捕捉多时间尺度距离，缓解长期预测中的误差累积问题
...
### [160] [Causal Representation Meets Stochastic Modeling under Generic Geometry](https://arxiv.org/abs/2602.05033)
*Jiaxu Ren,Yixin Wang,Biwei Huang*

Main category: cs.LG

TL;DR: 提出了一种可识别的连续时间潜在随机点过程的因果表示学习方法MUTATE，用于从低维观测中解耦高层潜在变量及其因果关系


<details>
  <summary>Details</summary>
Motivation: 现有可识别因果表示学习方法主要关注i.i.d.观测或离散时间过程，但许多现实场景需要识别连续时间随机过程（如多元点过程）的潜在变量
...
Conclusion: 该方法为连续时间潜在随机点过程的因果表示学习提供了可识别框架，在气候科学、生物学、物理学等领域具有应用价值
...
### [161] [Feedback Control for Multi-Objective Graph Self-Supervision](https://arxiv.org/abs/2602.05036)
*Karish Grover,Theodore Vasiloudis,Han Xie,Sixing Lu,Xiang Song,Christos Faloutsos*

Main category: cs.LG

TL;DR: ControlG：基于控制理论的多任务图自监督学习框架，通过时间分配而非权重混合来协调多个预训练目标，解决目标冲突、漂移和干旱问题


<details>
  <summary>Details</summary>
Motivation: 现有的多任务图自监督学习方法采用每轮更新的混合策略，导致目标间的冲突、效用漂移和某些目标被忽视，需要更智能的协调机制

Method: 提出ControlG框架，将多目标图SSL重新定义为反馈控制的时间分配问题：估计每个目标的难度和成对对抗性，通过帕累托感知的对数超体积规划器规划目标预算，使用PID控制器进行调度

Result: 在9个数据集上，ControlG始终优于最先进的基线方法，同时生成可审计的调度计划，揭示哪些目标推动了学习过程

Conclusion: 多目标图自监督学习的协调本质上是一个时间分配问题，ControlG通过控制理论方法有效解决了目标冲突问题，为多任务学习提供了新的框架
...
### [162] [ReFORM: Reflected Flows for On-support Offline RL via Noise Manipulation](https://arxiv.org/abs/2602.05051)
*Songyuan Zhang,Oswin So,H. M. Sabbir Ahmad,Eric Yang Yu,Matthew Cleaveland,Mitchell Black,Chuchu Fan*

Main category: cs.LG

TL;DR: ReFORM是一种基于流策略的离线强化学习方法，通过构造强制执行较宽松的支持约束，解决OOD错误并保持策略表达能力，在OGBench基准测试中优于所有基线方法。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习中存在两个主要挑战：1) OOD错误问题，当策略偏离训练分布时产生；2) 最优策略分布可能是多模态且难以表示。现有方法要么通过惩罚统计距离项来限制策略改进，要么使用扩散或流策略但无法同时解决OOD错误和保持表达能力。

Method: ReFORM提出基于流策略的方法，通过构造强制执行支持约束。首先学习一个有界源分布的行为克隆流策略来捕捉动作分布的支持集，然后优化一个反射流，为BC流生成有界噪声同时保持支持集，以最大化性能。

Result: 在OGBench基准的40个具有不同质量数据集的挑战性任务中，使用恒定超参数集的ReFORM在性能曲线图上优于所有经过手动调优超参数的基线方法。

Conclusion: ReFORM通过流策略构造强制执行支持约束，有效解决了离线RL中的OOD错误问题，同时保持了策略的表达能力，在多个任务中表现出优越性能。
...
### [163] [Learning, Solving and Optimizing PDEs with TensorGalerkin: an efficient high-performance Galerkin assembly algorithm](https://arxiv.org/abs/2602.05052)
...
<details>
  <summary>Details</summary>
Motivation: 为具有变分结构的偏微分方程（PDE）的数值求解、约束优化和物理信息学习提供一个统一的算法框架，解决传统方法在计算效率和灵活性方面的不足。

Method: 基于Galerkin离散化变分形式，提出TensorGalerkin框架：通过Python级Map阶段张量化单元操作，然后使用稀疏矩阵乘法进行全局约简，在网格诱导的稀疏图上进行消息传递。

Result: 在2D和3D椭圆、抛物和双曲PDE的无结构网格上，相比多种基线方法，该框架在所有目标下游应用中均显示出显著的计算效率和精度提升。

Conclusion: 提出的统一框架为变分PDE的数值求解、约束优化和物理信息学习提供了高效、准确的解决方案，具有广泛的应用潜力。
...
### [164] [Quantile-Physics Hybrid Framework for Safe-Speed Recommendation under Diverse Weather Conditions Leveraging Connected Vehicle and Road Weather Information Systems Data](https://arxiv.org/abs/2602.05053)
...
<details>
  <summary>Details</summary>
Motivation: 恶劣天气条件会显著影响驾驶员能见度和轮胎-路面摩擦力，需要调整安全驾驶速度以降低事故风险。现有方法缺乏实时、个性化的安全速度推荐。

Method: 使用高分辨率CV和RWIS数据构建时空对齐数据集，采用分位数回归森林预测10分钟窗口内的车速分布，结合基于物理的上限速度约束（考虑路面抓地力和能见度），最终融合QRF预测分位数、限速标志和物理上限得到推荐区间。
...
### [165] [Reliable Explanations or Random Noise? A Reliability Metric for XAI](https://arxiv.org/abs/2602.05082)
*Poushali Sengupta,Sabita Maharjan,Frank Eliassen,Shashi Raj Pandey,Yan Zhang*

Main category: cs.LG

TL;DR: 论文提出Explanation Reliability Index (ERI)指标家族，用于量化机器学习模型解释方法在现实部署条件下的可靠性，包括对输入扰动、特征冗余、模型更新和分布偏移的稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前复杂机器学习模型在高风险领域应用广泛，但现有解释方法（如SHAP和Integrated Gradients）的解释结果在现实非对抗性变化下缺乏稳定性测量，这种变异性损害了解释的可靠性，需要系统评估框架。

Method: 提出ERI指标家族，基于四个可靠性公理：对小输入扰动的鲁棒性、特征冗余下的一致性、模型演化中的平滑性、温和分布偏移的韧性。推导形式化保证（Lipschitz型边界和时间稳定性结果），提出ERI-T用于序列模型，并建立ERI-Bench基准测试。

Result: 实验结果显示流行解释方法存在广泛的可靠性失效，解释在现实部署条件下可能不稳定。ERI能够暴露和量化这些不稳定性，为解释可靠性提供原则性评估。

Conclusion: ERI框架能够系统评估解释方法的可靠性，支持构建更可信的可解释AI系统，通过量化解释稳定性来提升高风险领域AI决策的透明度。
...
### [166] [Individual Fairness In Strategic Classification](https://arxiv.org/abs/2602.05084)
...
<details>
  <summary>Details</summary>
Motivation: 战略分类中个体修改特征以影响机器学习决策带来了公平性挑战。虽然群体公平在此背景下已被广泛研究，但个体公平仍未被充分探索。本文旨在解决战略分类中的个体公平问题。

Method: 分析阈值分类器，证明确定性阈值违反个体公平；研究使用随机化分类器实现个体公平的可能性；提出确保个体公平的条件；通过线性规划问题寻找最优且个体公平的随机化分类器；将方法扩展到群体公平概念。

Result: 在真实世界数据集上的实验证实，该方法能有效缓解不公平性并改善公平性与准确性的权衡。

Conclusion: 随机化分类器可以在战略分类中实现个体公平，通过线性规划方法可以找到最优的公平分类器，且该方法也能应用于群体公平概念。
...
### [167] [Unbiased Single-Queried Gradient for Combinatorial Objective](https://arxiv.org/abs/2602.05119)
*Thanawat Sornwanee*
...
Method: 提出一种随机梯度方法，只需单次查询组合函数即可获得无偏梯度估计。该方法通过重要性采样包含REINFORCE方法，同时包含一类新的随机梯度。

Result: 该方法能够有效估计梯度，减少计算复杂度，为组合优化问题提供高效的优化算法。

Conclusion: 提出的随机梯度方法为组合优化问题的概率重构提供了一种高效的梯度估计技术，扩展了现有方法并降低了计算成本。
...
### [168] [Rethinking Rubric Generation for Improving LLM Judge and Reward Modeling for Open-ended Tasks](https://arxiv.org/abs/2602.05125)
...
<details>
  <summary>Details</summary>
Motivation: 现有评分标准生成方法难以控制，存在覆盖不足、维度混淆、偏好方向错位、冗余和高度相关标准等问题，导致评判准确性下降和强化微调中产生次优奖励

Method: 提出RRD框架，采用递归分解-过滤循环：将粗粒度评分标准分解为细粒度判别性标准，通过过滤机制移除错位和冗余标准，使用相关性感知加权方案避免高度相关标准的过度表示

Result: 在JudgeBench和PPE上显著提升GPT-4o和Llama3.1-405B评判准确性，JudgeBench上最高提升17.7分；作为WildChat上RFT的奖励源，Qwen3-4B奖励提升160%，Llama3.1-8B提升60%，远优于基线方法的10-20%提升

Conclusion: RRD确立了递归评分标准优化作为开放领域LLM评判和奖励建模的可扩展、可解释基础，在评估和训练中均带来显著改进
...
### [169] [Decoupled Orthogonal Dynamics: Regularization for Deep Network Optimizers](https://arxiv.org/abs/2602.05136)
*Hao Chen,Jinghui Yuan,Hanmin Zhang*

Main category: cs.LG

TL;DR: AdamW虽然将权重衰减与自适应梯度缩放解耦，但存在径向拉锯战问题：梯度倾向于增加参数范数以扩展有效容量，而权重衰减则无差别地抑制范数增长，导致径向振荡并影响Adam的二阶矩估计。作者提出正交动力学解耦方法AdamO，将范数控制与自适应预条件分离。


<details>
  <summary>Details</summary>
Motivation: AdamW中的权重衰减虽然与自适应梯度缩放解耦，但存在根本性冲突：径向拉锯战。在深度学习中，梯度倾向于增加参数范数以扩展有效容量，而权重衰减则无差别地抑制范数增长。这种推拉相互作用导致径向振荡，向Adam的二阶矩估计注入噪声，可能损害精细的切向特征学习。
...
Conclusion: AdamW中的标准权重衰减并非最优，因为存在径向拉锯战问题。通过正交动力学解耦将范数控制与自适应预条件分离的AdamO方法，能够改善优化器的性能，在保持简单性的同时提升泛化能力和稳定性。
...
### [170] [Adaptive Exploration for Latent-State Bandits](https://arxiv.org/abs/2602.05139)
*Jikai Jin,Kenneth Hung,Sanath Kumar Krishnamurthy,Baoyi Shi,Congshan Zhang*

Main category: cs.LG

TL;DR: 提出无需显式状态建模的bandit算法，利用滞后上下文特征和协调探索策略来应对隐藏时变状态环境中的决策挑战


<details>
  <summary>Details</summary>
Motivation: 经典多臂老虎机算法在存在隐藏时变状态的环境中表现不佳，这些状态会混淆奖励估计和最优动作选择。主要挑战包括未观测的混杂因素导致的奖励估计偏差和有限状态信息。

Method: 引入一族状态模型无关的老虎机算法，利用滞后上下文特征和协调探索策略，隐式跟踪潜在状态并区分状态依赖的奖励模式。包括自适应变体，无需显式状态建模即可学习最优策略。

Result: 在多样化设置中的实证结果表明，相比经典方法具有优越性能。算法结合了计算效率和鲁棒的非平稳奖励适应能力。

Conclusion: 为实际应用中的算法选择提供了实用建议。这些方法能够在存在未观测混杂因素和时变状态的环境中有效学习最优策略。
...
### [171] [Fairness Under Group-Conditional Prior Probability Shift: Invariance, Drift, and Target-Aware Post-Processing](https://arxiv.org/abs/2602.05144)
*Amir Asiaee,Kaveh Aryan*

Main category: cs.LG

TL;DR: 该论文研究机器学习公平性在数据分布变化下的表现，特别是群体条件先验概率偏移(GPPS)对公平性指标的影响，提出理论分析和实用算法。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统通常在历史数据上训练和评估公平性，但部署时环境条件可能发生变化。特别是当不同人口群体的正结果发生率变化不同时，公平性指标可能失效。需要研究这种分布偏移对公平性评估的影响。

Method: 研究群体条件先验概率偏移(GPPS)，即标签先验概率P(Y=1|A=a)在训练和部署间变化，而特征生成过程P(X|Y,A)保持稳定。提出理论分析和TAP-GPPS算法，该算法利用无标签目标数据估计先验概率，修正后验概率，并选择阈值以满足目标域的人口统计均等。

Result: 1. 证明基本二分法：基于错误率的公平标准(均等几率)在GPPS下结构不变，而基于接受率的公平标准(人口统计均等)会漂移，且这种漂移对非平凡分类器不可避免。2. 展示目标域风险和公平性指标无需目标标签即可识别。3. TAP-GPPS算法在实验中验证了理论预测，能以最小效用损失实现目标公平性。

Conclusion: GPPS对公平性评估有重要影响：错误率公平性指标具有鲁棒性，而接受率公平性指标会漂移。通过利用ROC量在GPPS下的不变性，可以在无目标标签的情况下一致估计目标域指标。TAP-GPPS算法能有效处理这种偏移，实现目标公平性。
...
### [172] [TIDE: Temporal Incremental Draft Engine for Self-Improving LLM Inference](https://arxiv.org/abs/2602.05145)
*Jiyoung Park,Hankyu Jang,Changseok Song,Wookeun Jung*

Main category: cs.LG

TL;DR: TIDE是一个集成到高性能LLM推理系统中的在线草稿自适应框架，通过重用推理过程中的隐藏状态实现零开销草稿适应，在多样化真实工作负载中实现最高1.15倍吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 推测解码可以显著加速LLM推理，但在实际应用中面临挑战：工作负载不断变化，系统级约束复杂。现有方法难以在实际部署中充分发挥推测解码的优势。

Method: TIDE框架将在线草稿适应直接集成到LLM推理系统中：1) 重用推理过程中生成的目标模型隐藏状态作为训练信号，实现零开销草稿适应；2) 采用自适应运行时控制，仅在有益时激活推测和训练；3) 利用异构集群，将解耦的推理和训练映射到合适的GPU类别。
...
### [173] [Cross-talk based multi-task learning for fault classification of physically coupled machine system](https://arxiv.org/abs/2602.05146)
...
<details>
  <summary>Details</summary>
Motivation: 机器系统产生的信号中，故障条件与各种物理变量存在物理耦合。现有故障分类研究通常仅依赖直接故障标签，但信号中自然嵌入了由其他物理耦合信息塑造的附加信息。本研究旨在利用这种耦合关系，通过多任务学习框架同时学习故障条件和相关物理变量。

Method: 采用交叉对话结构的多任务学习框架，该结构通过交叉对话层实现任务间的受控信息交换，同时防止负迁移。基于先前提出的残差神经降维模型，将其扩展到两个物理耦合显著的基准数据集：无人机故障数据集和电机复合故障数据集。在电机复合故障数据集中，还测试了使用单通道或多通道数据作为分类器输入的性能。

Result: 在两个基准数据集上，残差神经降维模型在多任务学习框架下，一致优于单任务模型、合并所有标签组合的多类模型以及共享主干多任务模型。交叉对话架构通过同时学习故障分类和物理属性，能够更好地分类故障。

Conclusion: 交叉对话结构的多任务学习框架能够有效利用信号中物理耦合的附加信息，通过同时学习故障条件和相关物理变量，显著提升故障分类性能，优于传统单任务和多类模型方法。
...
### [174] [CoSA: Compressed Sensing-Based Adaptation of Large Language Models](https://arxiv.org/abs/2602.05148)
...
### [175] [Position: Capability Control Should be a Separate Goal From Alignment](https://arxiv.org/abs/2602.05164)
*Shoaib Ahmed Siddiqui,Eleni Triantafillou,David Krueger,Adrian Weller*

Main category: cs.LG

TL;DR: 该立场论文主张将能力控制作为与对齐不同的独立目标，提出在模型生命周期中通过数据、学习和系统三层控制机制，并建议采用深度防御方法组合这些控制层来限制模型行为。


<details>
  <summary>Details</summary>
Motivation: 基础模型在广泛数据分布上训练，产生了通用能力，这使得许多下游应用成为可能，但也扩大了潜在滥用和失败的空间。需要建立硬性操作限制来控制模型行为，特别是对抗性诱导下的行为。

Method: 将能力控制机制组织为三个层次：1) 基于数据的训练分布控制；2) 基于学习的权重或表示层干预；3) 基于系统的部署后护栏，控制输入、输出和行动。建议采用深度防御方法，在整个技术栈中组合互补的控制措施。

Result: 提出了一个系统的能力控制框架，区分了能力控制与对齐的不同目标，识别了各控制层的特征性失败模式，并强调了组合控制的重要性。

Conclusion: 能力控制应被视为与对齐不同的独立目标，需要在整个模型生命周期中采用多层次、组合的控制方法，特别是要解决知识的双重用途和组合泛化等关键挑战。
...
### [176] [EBPO: Empirical Bayes Shrinkage for Stabilizing Group-Relative Policy Optimization](https://arxiv.org/abs/2602.05165)
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习与可验证奖励（RLVR）方法如GRPO存在稳定性问题：在小样本下估计方差高，在失败场景中梯度信号消失，限制了LLM推理能力的提升。

Method: 提出经验贝叶斯策略优化（EBPO）框架，使用收缩估计器动态平衡局部组统计与通过Welford在线算法更新的全局先验，正则化局部基线估计。

Result: 理论上证明EBPO相比GRPO具有更低的均方误差、有界的熵衰减和失败场景中的非消失惩罚信号；实证上在AIME和OlympiadBench等多个基准测试中优于GRPO和其他基线方法。

Conclusion: EBPO通过经验贝叶斯正则化显著提高了RLVR的稳定性，在小样本下仍能获得高性能增益，并能从难度分层课程学习中获益，为LLM推理能力提升提供了更稳健的框架。
...
### [177] [Benchmarking Artificial Intelligence Models for Daily Coastal Hypoxia Forecasting](https://arxiv.org/abs/2602.05178)
...
<details>
  <summary>Details</summary>
Motivation: 墨西哥湾北部沿海缺氧是一个持续的生态和经济问题，现有的季节性模型预测过于粗糙，无法满足每日响应式生态系统管理的精细尺度变化需求。

Method: 使用2009-2020年的每日后报数据训练四种深度学习架构（BiLSTM、Medformer、ST-Transformer、TCN），模型包含水柱分层、沉积物耗氧率和温度依赖分解率等特征，使用2020-2024年数据测试，采用相同的数据预处理、输入输出格式和验证协议，并使用McNemar方法评估模型预测的统计显著性差异。

Result: 所有模型都取得了高分类准确率和强判别能力，其中ST-Transformer在所有指标和测试期间表现最佳（AUC-ROC：0.982-0.992），提供了可重复的实时缺氧预测框架。

Conclusion: 研究提出了一个可重复的操作性实时缺氧预测框架，能够支持环境和海洋建模系统社区以及生态系统恢复力的更广泛努力，ST-Transformer架构在每日缺氧分类预测中表现最优。
...
### [178] [Data-Centric Interpretability for LLM-based Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2602.05183)
*John Yan,Michael Yu,Yuqi Sun,Alexander Duffy,Tyler Marques,Matthew Lyle Olson*

Main category: cs.LG

TL;DR: 该研究将稀疏自编码器（SAEs）和LLM摘要方法应用于复杂强化学习环境（Full-Press Diplomacy），开发了Meta-Autointerp方法对SAE特征进行分组解释，发现了细粒度行为模式和高层策略行为，但发现许多看似有趣的特征对人类实际无用，只有部分SAE衍生假设对下游任务有预测价值。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在复杂强化学习和多智能体环境中训练，理解训练过程中的行为变化变得困难。稀疏自编码器在数据中心的可解释性方面显示出潜力，但需要系统方法来分析大规模强化学习训练中的行为动态。

Method: 研究应用预训练的稀疏自编码器（SAEs）和LLM摘要方法分析Full-Press Diplomacy环境中的大规模强化学习训练。开发了Meta-Autointerp方法，将SAE特征分组为可解释的假设，并通过自动评估和用户研究验证发现。

Result: 发现了细粒度行为（角色扮演模式、退化输出、语言切换）和高层策略行为及环境特定错误。90%发现的SAE元特征具有显著性，发现了意外的奖励黑客行为。用户研究表明许多看似有趣的SAE特征对人类实际无用，但部分SAE衍生假设对下游任务有预测价值，通过增强未训练智能体的系统提示将得分提高了14.2%。

Conclusion: SAEs和LLM摘要方法提供了对智能体行为的互补视角，该框架为未来数据中心的可解释性工作提供了实用起点，有助于确保LLM在训练过程中的可信行为。
...
### [179] [SpectraKAN: Conditioning Spectral Operators](https://arxiv.org/abs/2602.05187)
*Chun-Wun Cheng,Carola-Bibiane Schönlieb,Angelica I. Aviles-Rivero*

Main category: cs.LG

TL;DR: SpectraKAN是一种新型神经算子，通过输入条件化的谱卷积来改进传统谱神经算子，使其能够自适应捕捉多尺度、状态依赖的各向异性动力学，在PDE求解任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有谱神经算子（如FNO）使用静态傅里叶核，无法适应输入变化，限制了捕捉多尺度、状态依赖和各向异性动力学的能力。需要一种能够根据系统全局状态自适应调整的谱算子。

Method: 提出SpectraKAN：1）从时空历史中提取紧凑全局表示；2）通过单查询交叉注意力调制多尺度傅里叶主干；3）将静态谱卷积转变为输入条件化的积分算子，同时保持谱混合的效率。

Result: 在多样化的PDE基准测试中，SpectraKAN达到最先进性能，相比强基线将RMSE降低高达49%，在具有挑战性的时空预测任务上表现尤为突出。

Conclusion: SpectraKAN通过输入条件化的谱算子实现了自适应动力学建模，在理论上证明了网格细化下的收敛性和Lipschitz控制的全局调制，为学习复杂PDE解算子提供了更强大的框架。
...
### [180] [Double-P: Hierarchical Top-P Sparse Attention for Long-Context LLMs](https://arxiv.org/abs/2602.05191)
*Wentao Ni,Kangqi Zhang,Zhongming Yu,Oren Nelson,Mingu Lee,Hong Cai,Fatih Porikli,Jongryool Kim,Zhijian Liu,Jishen Zhao*

Main category: cs.LG

TL;DR: Double-P是一种分层稀疏注意力框架，通过两级top-p选择优化长上下文推理中的注意力计算，在保持高精度的同时显著提升解码速度。


<details>
  <summary>Details</summary>
Motivation: 随着长上下文推理成为大语言模型的核心，注意力计算成为解码瓶颈。现有的稀疏注意力方法无法同时优化top-p精度、选择开销和稀疏注意力成本，限制了整体效率。

Method: 提出Double-P分层稀疏注意力框架：首先在聚类级别使用大小加权质心进行粗粒度top-p估计，然后通过第二级top-p阶段自适应地分配令牌级注意力计算。

Result: 在长上下文基准测试中，Double-P实现了接近零的精度下降，将注意力计算开销减少高达1.8倍，端到端解码速度比最先进的固定预算稀疏注意力方法提升高达1.3倍。

Conclusion: Double-P通过分层优化top-p选择，有效解决了长上下文推理中注意力计算的效率问题，在保持高精度的同时显著提升了推理速度。
...
### [181] [Extreme Weather Nowcasting via Local Precipitation Pattern Prediction](https://arxiv.org/abs/2602.05204)
*Changhoon Song,Teng Yuan Chang,Youngjoon Hong*

Main category: cs.LG

TL;DR: exPreCast：一种高效的确定性雷达预报框架，结合平衡数据集，在正常和极端降雨情况下都能实现精确的短时预报


<details>
  <summary>Details</summary>
Motivation: 极端天气事件的准确预报对风险管理和灾害缓解至关重要。现有方法存在局限性：基于扩散的生成模型计算成本高不适合实时应用，确定性模型偏向正常降雨，而基准数据集本身存在偏差（要么偏向普通降雨，要么局限于极端事件），限制了实际应用。

Method: 提出exPreCast框架，包含局部时空注意力机制、纹理保持的立方双上采样解码器和时间提取器，能够灵活调整预报时间范围。同时构建了来自韩国气象厅的平衡雷达数据集，包含普通降水和极端事件。

Result: 在SEVIR、MeteoNet基准数据集和新建的平衡KMA数据集上的实验表明，该方法实现了最先进的性能，在正常和极端降雨情况下都能提供准确可靠的短时预报。

Conclusion: exPreCast框架通过高效确定性方法和平衡数据集，解决了降水短时预报中的关键挑战，为实际应用提供了可靠解决方案。
...
### [182] [Disentangled Representation Learning via Flow Matching](https://arxiv.org/abs/2602.05214)
*Jinjin Chi,Taoping Liu,Mengtao Yin,Ximing Li,Yongcheng Jing,Dacheng Tao*

Main category: cs.LG

TL;DR: 提出基于流匹配的解耦表示学习框架，通过正交正则化抑制因子间干扰，在多个数据集上优于现有基线


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的方法虽然通过归纳偏置鼓励因子独立性，但经常缺乏强语义对齐，需要改进解耦表示学习的效果

Method: 提出流匹配框架，将解耦视为在紧凑潜在空间中学习因子条件流，引入非重叠（正交）正则化器抑制跨因子干扰和减少因子间信息泄漏

Result: 在多个数据集上的广泛实验表明，相比代表性基线方法，该方法在解耦分数、可控性和样本保真度方面都有持续改进

Conclusion: 基于流匹配的解耦表示学习框架通过正交正则化有效实现语义对齐，在解耦性能上优于现有方法
...
### [183] [Private Prediction via Shrinkage](https://arxiv.org/abs/2602.05219)
*Chao Yan*

Main category: cs.LG

TL;DR: 该论文研究差分隐私预测问题，在流式查询设置中将隐私保护下的查询复杂度从√T降低到polylog(T)


<details>
  <summary>Details</summary>
Motivation: Dwork和Feldman在2018年提出了差分隐私预测问题，标准组合方法导致查询复杂度与√T成正比。作者希望减少这种依赖，特别是在流式查询场景中。

Method: 针对两种不同对手模型设计私有预测器：1) 对于无意识在线对手和任意概念类C，使用VC维分析；2) 对于自适应在线对手和ℝ^d中的半空间，采用特定算法。

Result: 实现了查询复杂度的显著改进：对于无意识对手，样本复杂度为Õ(VC(C)^3.5 log^3.5 T)；对于自适应对手和半空间，样本复杂度为Õ(d^5.5 log T)。

Conclusion: 在差分隐私预测的流式查询设置中，可以将查询复杂度从√T降低到polylog(T)，为不同对手模型和概念类提供了具体的样本复杂度界限。
...
### [184] [ZeroS: Zero-Sum Linear Attention for Efficient Transformers](https://arxiv.org/abs/2602.05230)
*Jiecheng Lu,Xu Han,Yan Sun,Viresh Pati,Yubin Kim,Siddhartha Somani,Shihao Yang*

Main category: cs.LG

TL;DR: ZeroS线性注意力通过移除常数项和重新加权残差，解决了传统线性注意力只能进行凸组合和存在均匀权重偏差的问题，在保持O(N)复杂度的同时达到了与标准softmax注意力相当或更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统线性注意力方法虽然提供O(N)复杂度，但性能通常不如标准softmax注意力。研究发现两个根本限制：1) 仅限于凸组合，只能进行加性信息融合；2) 均匀累积权重偏差，在长上下文中稀释注意力。

Method: 提出Zero-Sum线性注意力(ZeroS)，通过移除常数项1/t并重新加权剩余的零和softmax残差。这种修改创建了数学稳定的权重，允许正负值，使单个注意力层能够执行对比操作，同时保持O(N)复杂度。

Result: 理论上，ZeroS相比凸组合扩展了可表示函数集。实证上，在各种序列建模基准测试中，ZeroS匹配或超越了标准softmax注意力的性能。

Conclusion: ZeroS线性注意力成功解决了传统线性注意力的两个根本限制，在保持计算效率的同时实现了与标准注意力相当或更好的性能，为高效序列建模提供了有前景的解决方案。
...
### [185] [Faithful Bi-Directional Model Steering via Distribution Matching and Distributed Interchange Interventions](https://arxiv.org/abs/2602.05234)
*Yuntai Bao,Xuhong Zhang,Jintao Chen,Ge Su,Yuxiang Cai,Hao Peng,Bing Sun,Haiqin Weng,Liu Yan,Jianwei Yin*

Main category: cs.LG

TL;DR: CDAS是一种基于概念分布对齐搜索的模型干预方法，通过分布式交换干预和分布匹配目标实现更忠实稳定的模型控制，相比偏好优化方法在某些场景下表现更好。


<details>
  <summary>Details</summary>
Motivation: 当前基于干预的模型控制方法虽然轻量且可解释，但容易过拟合且表现不佳，有时产生不自然的输出。作者认为有效的控制需要忠实识别内部模型机制，而非强制执行外部偏好。

Method: 基于分布式对齐搜索(DAS)原则，提出概念DAS(CDAS)方法。采用DAS的核心机制——分布式交换干预(DII)，但引入了专门为控制任务设计的分布匹配目标，通过将干预后的输出分布与反事实分布对齐来实现。

Result: 在AxBench大规模模型控制基准测试中，CDAS并不总是优于偏好优化方法，但在模型规模增大时可能获益更多。在两个安全相关案例研究中，CDAS能够系统性地控制安全对齐模型的拒绝行为并中和思维链后门，同时保持模型的一般效用。

Conclusion: CDAS是对偏好优化方法的补充，在特定条件下构成了基于干预的模型控制的稳健方法。它通过弱监督分布匹配而非概率最大化学习干预，使用DII实现双向控制，减少了超参数调优需求。
...
### [186] [CORP: Closed-Form One-shot Representation-Preserving Structured Pruning for Vision Transformers](https://arxiv.org/abs/2602.05243)
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers虽然精度高但计算和内存成本大，现有结构化剪枝方法需要重训练或多阶段优化，限制了后训练部署的实用性

Method: 将结构化剪枝建模为表示恢复问题，将被移除的激活和注意力对数建模为保留组件的仿射函数，推导闭式岭回归解并将补偿折叠到模型权重中

Result: 在ImageNet上对DeiT模型实验显示，MLP和注意力表示存在强冗余性；CORP在剪枝50% MLP和注意力结构后，DeiT-Huge仍保持82.8% Top-1精度，单GPU 20分钟内完成剪枝

Conclusion: CORP提供了一种高效的后训练结构化剪枝方案，无需额外训练即可在保持精度的同时显著提升推理效率，为Vision Transformers的实际部署提供了实用工具
...
### [187] [Back to Basics: Revisiting Exploration in Reinforcement Learning for LLM Reasoning via Generative Probabilities](https://arxiv.org/abs/2602.05281)
*Pengyi Li,Elizaveta Goncharova,Andrey Kuznetsov,Ivan Oseledets*

Main category: cs.LG

TL;DR: 该论文提出了一种名为ARM（Advantage Re-weighting Mechanism）的新方法，用于解决强化学习验证奖励（RLVR）中的模式崩溃问题，通过重新加权优势估计来平衡不同正确推理路径的置信度，从而提高输出多样性。


<details>
  <summary>Details</summary>
Motivation: 标准策略优化方法（如GRPO）在强化学习验证奖励中容易收敛到低熵策略，导致严重的模式崩溃和有限的输出多样性。作者从采样概率动态的角度分析这一问题，发现标准目标函数不成比例地强化最高似然路径，从而抑制了有效的替代推理链。

Method: 提出了一种新颖的优势重新加权机制（ARM），通过将提示困惑度和答案置信度纳入优势估计，动态重塑奖励信号。该方法衰减过度自信推理路径的梯度更新，同时将概率质量重新分配给未被充分探索的正确解决方案。
...
### [188] [Robust Inference-Time Steering of Protein Diffusion Models via Embedding Optimization](https://arxiv.org/abs/2602.05285)
...
Method: 提出EmbedOpt方法，在条件嵌入空间中引导扩散模型优化实验似然，利用该空间编码的丰富序列和共进化信号，有效调整扩散先验以符合实验约束

Result: 在模拟冷冻电镜图谱拟合和实验距离约束两个基准测试中，EmbedOpt在拟合任务中优于基于坐标的后验采样方法，在距离约束任务中性能相当，且在跨越两个数量级的超参数范围内展现出更好的工程鲁棒性

Conclusion: EmbedOpt通过平滑的优化行为显著减少了推理所需的扩散步骤，提高了效率，为引导扩散模型解决生物物理逆问题提供了更鲁棒和高效的替代方案
...
### [189] [HealthMamba: An Uncertainty-aware Spatiotemporal Graph State Space Model for Effective and Reliable Healthcare Facility Visit Prediction](https://arxiv.org/abs/2602.05286)
*Dahai Yu,Lin Jiang,Rongchao Xu,Guang Wang*

Main category: cs.LG

TL;DR: HealthMamba：一个不确定性感知的时空框架，用于准确可靠的医疗设施访问预测，通过统一时空编码器、图状态空间模型和综合不确定性量化机制，在四个大规模真实数据集上实现预测准确率提升6.0%，不确定性量化提升3.5%。


<details>
  <summary>Details</summary>
Motivation: 现有医疗设施访问预测研究存在两个主要问题：1）通常将其视为时间序列预测问题，忽略了不同类型医疗设施之间的空间依赖关系；2）无法在公共卫生紧急事件等异常情况下提供可靠预测。需要开发能够同时考虑时空依赖并提供可靠不确定性量化的方法。

Method: 提出HealthMamba框架，包含三个关键组件：1）统一时空上下文编码器，融合异构静态和动态信息；2）新颖的图状态空间模型GraphMamba，用于分层时空建模；3）综合不确定性量化模块，集成三种不确定性量化机制。

Result: 在加利福尼亚、纽约、德克萨斯和佛罗里达四个大规模真实数据集上评估，结果显示HealthMamba相比最先进的基线方法，预测准确率提升约6.0%，不确定性量化提升约3.5%。

Conclusion: HealthMamba通过统一的时空建模和综合的不确定性量化，能够更准确、可靠地预测医疗设施访问量，特别是在异常情况下，为医疗资源优化分配和公共卫生政策制定提供支持。
...
### [190] [A Short and Unified Convergence Analysis of the SAG, SAGA, and IAG Algorithms](https://arxiv.org/abs/2602.05304)
...
<details>
  <summary>Details</summary>
Motivation: 现有随机方差缩减算法（如SAG、SAGA）及其确定性对应方法（如IAG）的分析分散且复杂，需要不同的证明技术。SAG的原始证明尤其复杂，需要计算机辅助分析。需要统一的收敛分析框架。

Method: 开发统一的收敛分析方法，包含两个关键步骤：(1) 使用简单集中工具建立随机子采样延迟的界限；(2) 精心设计新颖的李雅普诺夫函数来考虑这些延迟。

Result: 获得了简短、模块化的证明，首次为SAG和SAGA提供了高概率界限，并可无缝扩展到非凸目标和马尔可夫采样。作为副产品，获得了IAG算法的最佳已知收敛率，显著改进了先前界限。

Conclusion: 提出的统一分析框架简化了SAG、SAGA和IAG算法的收敛证明，提供了更简洁、模块化的分析方法，并改进了收敛率结果。
...
### [191] [Formal Synthesis of Certifiably Robust Neural Lyapunov-Barrier Certificates](https://arxiv.org/abs/2602.05311)
...
<details>
  <summary>Details</summary>
Motivation: 现有的神经Lyapunov和屏障证书方法仅在固定理想无扰动动力学下提供保证，但在实际应用中，系统动力学可能因不确定性而偏离，限制了这些方法的可靠性。
...
Conclusion: 研究结果表明，在动力学存在扰动的情况下，训练鲁棒神经证书对于安全强化学习是有效的，能够提高控制器在实际应用中的可靠性。
...
### [192] [Accelerated Sequential Flow Matching: A Bayesian Filtering Perspective](https://arxiv.org/abs/2602.05319)
...
Method: 基于贝叶斯滤波的Sequential Flow Matching框架，将流式推理视为学习将预测分布从一个时间步传输到下一个时间步的概率流，从先前后验分布初始化生成

Result: 在多种预测、决策和状态估计任务中，该方法性能与完整步扩散模型相当，但仅需一个或很少采样步骤，采样速度更快

Conclusion: 通过贝叶斯滤波框架进行序列推理为基于流模型的高效实时部署提供了新的原则性视角
...
### [193] [GAS: Enhancing Reward-Cost Balance of Generative Model-assisted Offline Safe RL](https://arxiv.org/abs/2602.05323)
*Zifan Liu,Xinran Li,Shibo Chen,Jun Zhang*

Main category: cs.LG

TL;DR: GAS算法通过数据增强和重新标注提升离线安全强化学习中的"拼接"能力，引入目标函数估计最优可达到的奖励和成本目标，实现奖励最大化与约束满足的更好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于生成模型的离线安全强化学习方法面临两大挑战：1) 无法从次优轨迹中"拼接"出最优转移；2) 难以平衡奖励目标与成本目标，特别是当两者冲突时。

Method: 提出GAS算法：1) 在转移级别增强和重新标注数据集，从次优轨迹构建高质量轨迹；2) 引入目标函数，使用期望回归在增强数据集上估计最优可达到的奖励和成本目标；3) 重塑数据集以实现更均匀的奖励-成本回报分布，提高训练稳定性。

Result: 实验结果表明GAS在平衡奖励最大化与约束满足方面优于现有方法，能够适应更广泛的奖励-成本回报对，实现更好的权衡。

Conclusion: GAS算法通过增强拼接能力和引入目标函数估计，有效解决了离线安全强化学习中奖励与成本的平衡问题，在约束环境下实现了鲁棒的性能。
...
### [194] [Pool-based Active Learning as Noisy Lossy Compression: Characterizing Label Complexity via Finite Blocklength Analysis](https://arxiv.org/abs/2602.05333)
...
<details>
  <summary>Details</summary>
Motivation: 现有的池式主动学习理论分析缺乏统一的信息论框架，无法同时分析数据选择和学习的理论极限。需要建立能够统一分析这两个方面的理论框架。
...
Conclusion: 该框架为池式主动学习提供了新的理论视角，统一了数据选择和学习的信息论分析，建立了与有噪有损压缩的对应关系，为理解主动学习的理论极限提供了基础。
...
### [195] [Hinge Regression Tree: A Newton Method for Oblique Regression Tree Splitting](https://arxiv.org/abs/2602.05371)
*Hongyi Li,Han Lin,Jun Xu*

Main category: cs.LG

TL;DR: HRT（铰链回归树）通过将分割重构为非线性最小二乘问题，结合了树的透明性和多元决策边界的表达能力，解决了传统斜决策树学习NP难的问题。


<details>
  <summary>Details</summary>
Motivation: 传统斜决策树虽然结合了树的透明性和多元决策边界的表达能力，但学习高质量斜分割是NP难问题，现有方法依赖缓慢搜索或缺乏理论基础的启发式方法。

Method: 将每个分割重构为两个线性预测器的非线性最小二乘问题，其最大/最小包络产生类似ReLU的表达能力。采用交替拟合程序，等价于固定分区内的阻尼牛顿（高斯-牛顿）方法。

Result: 证明了节点级优化的单调收敛性；HRT模型类是通用逼近器，具有明确的O(δ²)逼近率；在合成和真实基准测试中，以更紧凑的结构匹配或优于单树基线。

Conclusion: HRT提供了一种理论严谨、计算高效的斜决策树学习方法，结合了理论保证和实际性能优势。
...
TL;DR: 该研究评估了时间序列基础模型在电力需求预测中利用外生特征的能力，发现基础模型表现参差不齐，简单的LSTM基线模型在稳定气候条件下经常优于基础模型，而基础模型主要在多变气候条件下表现优势。
...
Method: 实证评估了MOIRAI、MOMENT、TinyTimeMixers、ChronosX和Chronos-2等基础模型，对比了带有可逆实例归一化的LSTM基线模型，在新加坡和澳大利亚电力市场以小时和日粒度进行测试，采用三种特征配置：所有特征、选定特征和仅目标特征。

Result: Chronos-2在基础模型中表现最佳（零样本设置下），但在新加坡稳定气候条件下，简单的基线模型经常优于所有基础模型，特别是在短期预测中。模型架构至关重要，具有协同架构实现（TTM的通道混合、Chronos-2的分组注意力）的模型能持续利用外生特征，而其他方法则表现不一致。

Conclusion: 研究结果挑战了基础模型普遍优越性的假设，强调了在能源领域需要特定领域模型，地理环境同样重要，基础模型主要在多变气候条件下显示优势。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护数据隐私，但仍面临推理攻击暴露敏感数据和拜占庭行为影响模型训练的问题。现有方案往往单独处理安全聚合和拜占庭容错，难以同时兼顾两者。

Method: 1) 基于属性推理攻击思路，在标记的影子更新上训练过滤元分类器，模拟多种拜占庭攻击行为；2) 使用同态加密进行隐私保护聚合，通过元分类器输出重新加权来消除拜占庭加密更新；3) 提出自动选择最优核函数和维度超参数的方法，以适应CKKS密码系统的同态推理和效率约束。

Result: 在FEMNIST、CIFAR10、GTSRB和acsincome基准测试上验证了方法的有效性。SVM过滤识别拜占庭更新的准确率达到90%-94%，模型效用损失很小，加密推理运行时间在6-26秒之间。
...
TL;DR: 提出基于锦标赛图的k-wise重排序框架，通过聚合k文档比较中的成对偏好信息，减少LLM调用次数，实现更高效的重排序。


<details>
  <summary>Details</summary>
Motivation: 现有LLM重排序方法要么依赖启发式方法未能充分利用每次排序决策的信息，要么在充分利用信息时效率低下，需要更高效且充分利用信息的重排序方法。

Method: 引入锦标赛图框架，将k文档比较中的成对偏好聚合为全局偏好图，通过传递闭包推导额外排序信息，设计贪心查询调度最大化信息增益，处理非传递偏好时通过等价类生成分层排序。

Result: 在14个基准测试和5个LLM上，该方法实现帕累托优势：在相同或更高准确率下，比可比方法减少25-40%的token使用，比成对方法减少7倍token使用且质量接近。

Conclusion: 锦标赛图框架为k-wise重排序提供了理论基础，通过有效利用LLM比较中的偏好信息，显著提高了重排序效率，实现了准确性和效率的平衡。
...
TL;DR: 离线目标条件强化学习中的超参数敏感性主要源于自举机制，而非强化学习本身固有。当存在适度专家数据时，基于度量的方法比自举方法表现出更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 研究深度强化学习中普遍存在的超参数敏感性问题是强化学习固有的，还是由特定训练机制加剧的。通过离线目标条件强化学习环境，控制数据分布固定性，可以明确研究非平稳性对超参数敏感性的影响。

Method: 在离线目标条件强化学习中，通过调度数据质量的变化来控制非平稳性。研究不同数据质量在平稳和非平稳机制下的表现，比较两种代表性算法：HIQL（基于自举TD学习）和QRL（拟度量表示学习）。引入目标间梯度对齐诊断来分析算法差异。

Result: 观察到离线RL对超参数配置变化的鲁棒性显著高于在线RL，即使在受控非平稳条件下也是如此。当存在约20%专家数据时，QRL保持广泛稳定的近最优区域，而HIQL表现出尖锐的最优点且在不同训练阶段漂移显著。自举目标表现出更强的破坏性梯度干扰，这与超参数敏感性直接相关。

Conclusion: 训练过程中对超参数配置变化的高度敏感性并非强化学习不可避免的特性，而是由自举机制的动态特性加剧的。这为设计更鲁棒的算法目标提供了途径。
...
TL;DR: 论文提出两个基于比特/焦耳的物理智能效率指标：热力学Epiplexity per Joule（环境结构信息编码效率）和Empowerment per Joule（传感器运动控制信息效率），建立统一的效率评估框架。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统在获得卓越能力的同时消耗大量能源，需要将智能与物理效率联系起来，建立可量化的比特/焦耳指标来评估AI系统的能源效率。

Method: 提出两个互补的比特/焦耳指标：1）热力学Epiplexity per Joule - 在特定边界内每单位能量编码的环境结构信息比特数；2）Empowerment per Joule - 在固定时间范围内每单位预期能量成本的传感器运动通道容量。基于随机热力学理论，建立Landauer尺度的闭环基准，并针对经验场景提出MDL-epiplexity替代指标。

Result: 建立了热力学学习不等式下的Landauer尺度闭环基准，证明了在明确边界和重置假设下信息增益与能耗的紧密联系，同时展示了在没有这些假设时两者可以解耦。提出了统一的效率框架和最小检查清单。

Conclusion: 提出了一个统一的物理智能效率评估框架，包含两个互补的比特/焦耳指标和明确的会计约定，旨在减少歧义、支持一致的效率比较，并为能源调整的规模分析提供基础。
...
TL;DR: 本文提出了一个统一的裁剪框架，将现有RLVR方法（如GRPO）统一到基于策略散度的理论框架中，并引入KL3估计器作为关键策略散度约束，通过数学证明和实验验证其在提升训练稳定性和最终性能方面的优势。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法（如GRPO及其变体）通过裁剪似然比来约束策略差异以确保稳定更新，但缺乏对不同策略散度度量如何影响探索和性能的系统分析。需要建立一个统一的理论框架来系统性地理解不同策略散度约束的作用机制。

Method: 提出统一的裁剪框架，将现有方法统一到基于一般策略散度的理论框架中，涵盖似然比和KL散度等度量。引入KL3估计器作为方差减少的蒙特卡洛KL散度估计器，并理论证明KL3约束在数学上等价于非对称比率裁剪，将概率质量重新分配到高置信度动作上。

Result: 在数学推理基准测试上的实证结果表明，将KL3估计器集成到GRPO中能同时提高训练稳定性和最终性能，验证了基于原则的策略散度约束在策略优化中的重要性。

Conclusion: 通过统一的策略散度框架和KL3估计器的引入，为RLVR方法提供了更坚实的理论基础，证明了基于原则的策略散度约束能有效平衡探索与利用，提升大语言模型的推理能力。
...
TL;DR: 提出Evidential Uncertainty Quantification (EUQ)方法，通过证据理论量化视觉语言模型中的信息冲突和知识缺失，有效检测模型的不良行为


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在面对不完整或对抗性输入时会产生不可靠甚至有害内容，这些不良行为源于认知不确定性（内部知识冲突或信息缺失），现有不确定性量化方法对此效果有限

Method: 提出EUQ方法，将模型输出特征解释为支持或反对的证据，利用证据理论建模和聚合这些证据，在单次前向传播中量化内部冲突和知识缺口

Result: 在幻觉、越狱、对抗性漏洞和分布外失败四类不良行为上，EUQ始终优于强基线方法，发现幻觉对应高内部冲突，分布外失败对应高知识缺失

Conclusion: EUQ提供了一种细粒度的不确定性量化方法，能有效检测LVLM的不良行为，层间证据不确定性动态分析为理解内部表示演化提供了新视角
...
### [203] [Steering Large Reasoning Models towards Concise Reasoning via Flow Matching](https://arxiv.org/abs/2602.05539)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现出色，但输出往往过于冗长，影响效率。现有的线性引导方法基于限制性线性表示假设，仅应用全局向量到隐藏表示，效果有限。

Method: FlowSteer是一种非线性引导方法，通过学习与简洁推理相关的分布之间的完整变换来超越均匀线性偏移。该变换通过流匹配学习为速度场，实现对模型推理过程的精确、输入依赖控制。

Result: 通过将引导表示与简洁推理激活分布对齐，FlowSteer相比线性偏移产生更紧凑的推理。在多样化推理基准测试中，FlowSteer相比领先的推理时基线方法展现出更强的任务性能和标记效率。

Conclusion: 使用生成技术建模完整的分布传输为控制大型推理模型提供了更有效和原则性的基础，超越了线性方法的局限性。
...
### [204] [Unveiling Implicit Advantage Symmetry: Why GRPO Struggles with Exploration and Difficulty Adaptation](https://arxiv.org/abs/2602.05548)
*Zhiqi Yu,Zhangquan Chen,Mengting Liu,Heye Zhang,Liangqiong Qu*

Main category: cs.LG

TL;DR: 本文提出A-GRAE方法，通过引入非对称优势估计解决GRPO在探索和难度适应方面的瓶颈，显著提升强化学习效率。


<details>
  <summary>Details</summary>
Motivation: GRPO作为强化学习可验证奖励的标准方法，在探索效率和难度适应方面存在瓶颈。研究发现这些瓶颈源于组相对优势估计中的隐式对称性，导致探索不足和难度适应不灵活。

Method: 提出非对称组相对优势估计（A-GRAE），通过动态调节探索激励和样本难度关注，打破传统对称性限制，实现从简单样本到复杂样本的课程式学习过渡。

Result: 在七个基准测试中，A-GRAE方法持续改进GRPO及其变体，在LLMs和MLLMs上都表现出显著性能提升。

Conclusion: 非对称优势估计是解决GRPO探索和难度适应瓶颈的有效方法，A-GRAE通过动态调节机制实现了更高效的强化学习训练。
...
### [205] [Logical Guidance for the Exact Composition of Diffusion Models](https://arxiv.org/abs/2602.05549)
*Francesco Alesiani,Jonathan Warrell,Tanja Bien,Henrik Christiansen,Matheus Ferraz,Mathias Niepert*

Main category: cs.LG

TL;DR: LOGDIFF是一个扩散模型引导框架，通过逻辑表达式实现约束生成，提出了精确布尔演算和混合引导方法


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在推理时难以处理复杂逻辑约束的生成问题，需要一种能够精确组合原子属性引导信号的框架

Method: 1) 推导精确布尔演算，为复杂逻辑公式提供精确引导的充分条件；2) 提出混合引导方法，结合原子引导分数和后验概率估计；3) 开发高效递归算法计算引导信号

Result: 在图像和蛋白质结构生成任务中验证了框架的有效性，能够实现基于复杂逻辑表达式的精确约束生成

Conclusion: LOGDIFF框架为扩散模型提供了处理复杂逻辑约束的理论基础和实践方法，将分类器引导和无分类器引导统一起来
...
TL;DR: OpenMAG是一个全面的多模态属性图学习基准，整合了19个数据集、16种编码器、24个SOTA模型和8个下游任务，旨在解决现有评估标准的局限性。


<details>
  <summary>Details</summary>
Motivation: 随着多模态属性图模型的快速发展，现有基准在领域覆盖、编码器灵活性、模型多样性和任务范围方面存在严重不足，需要建立统一、严谨的评估标准来促进公平比较。

Method: 构建OpenMAG基准，整合19个跨6个领域的数据集，支持16种静态和可训练特征编码器，实现24个最先进模型的标准化库，支持8个下游任务评估。

Result: 通过系统评估必要性、数据质量、有效性、鲁棒性和效率，得出了14个关于MAG学习的基本见解，为未来研究提供指导。

Conclusion: OpenMAG为多模态属性图学习提供了一个全面、统一的评估框架，解决了现有基准的局限性，促进了公平比较和未来研究进展。
...
TL;DR: 提出Shiva-DiT方法，通过残差感知的Top-k选择解决扩散变换器计算成本高的问题，实现确定性token计数和端到端可学习性，在保持保真度的同时获得1.54倍加速。
...
Result: 在包括SD3.5在内的主流模型上实验表明，Shiva-DiT建立了新的帕累托前沿，相比现有基线实现了1.54倍的实时加速，同时保持更优的保真度，有效消除了不规则张量开销。

Conclusion: Shiva-DiT成功解决了扩散变换器计算效率问题，通过创新的残差感知可微分剪枝方法，在保持模型性能的同时显著提升推理速度，为大规模扩散模型部署提供了有效解决方案。
...
TL;DR: PGFM提出首个基于流匹配的生成式数据集蒸馏框架，通过ODE快速确定性合成，在潜空间进行类别条件传输，实现高效采样和原型控制。
...
Method: 提出路径引导流匹配(PGFM)框架，在冻结VAE的潜空间进行类别条件流匹配，学习从高斯噪声到数据分布的传输，并开发连续路径到原型引导算法实现ODE一致的路径控制。

Result: 在高分辨率基准测试中，PGFM在更少采样步数下匹配或超越先前基于扩散的蒸馏方法，提供竞争性性能的同时显著提升效率（比扩散方法高效7.6倍，达到78%模式覆盖率）。

Conclusion: PGFM通过流匹配框架解决了扩散基蒸馏方法的效率问题，实现了快速确定性合成和可靠的路径控制，为生成式数据集蒸馏提供了高效稳定的解决方案。
...
TL;DR: 本文提出Mode-Dependent Rectification (MDR)方法，解决视觉强化学习中模式相关层（如Batch Normalization）在PPO训练中导致的不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 模式相关架构组件（在训练和评估时行为不同的层，如Batch Normalization或dropout）在视觉强化学习中常用，但会破坏同策略优化的稳定性。在PPO中，Batch Normalization引起的训练与评估行为差异会导致策略不匹配、分布漂移和奖励崩溃。

Method: 提出Mode-Dependent Rectification (MDR)，一种轻量级的双阶段训练过程，在不改变架构的情况下稳定PPO在模式相关层下的训练。该方法自然扩展到其他模式相关层。

Result: 在程序生成游戏和真实世界补丁定位任务上的实验表明，MDR能持续提高稳定性和性能。

Conclusion: MDR有效解决了模式相关层在PPO训练中引起的不稳定问题，为视觉强化学习中的模式相关组件提供了稳定的训练方案。
...
TL;DR: 本文提出REAL框架，将可验证奖励重新视为分类标签而非标量权重，将策略优化重构为分类问题，解决了GRPO及其变体中存在的正样本梯度错配和负样本梯度主导问题，在数学推理基准上显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 尽管基于可验证奖励的强化学习（RLVR）方法如GRPO及其变体在复杂推理任务中取得了显著进展，但作者发现这些方法存在正样本梯度错配和负样本梯度主导的问题，导致策略更新效率低下且次优。

Method: 提出REAL（Rewards as Labels）框架，将可验证奖励重新视为分类标签而非标量权重，从而将策略优化重构为分类问题。在此基础上引入锚定logits来增强策略学习，实现单调且有界的梯度加权，平衡不同rollout之间的梯度分配。

Result: 在数学推理基准测试中，REAL显著提升了训练稳定性并持续优于GRPO及其变体如DAPO。在1.5B模型上，REAL比DAPO平均Pass@1提升6.7%；在7B模型上，REAL继续优于DAPO和GSPO，分别提升6.2%和1.7%。即使使用简单的二元交叉熵，REAL仍保持稳定并比DAPO平均提升4.5%。

Conclusion: REAL框架通过将可验证奖励重新定义为分类标签，有效解决了现有RLVR方法中的梯度分配问题，实现了更平衡和高效的策略优化，在数学推理任务中取得了显著性能提升，且具有良好的可扩展性。
...
TL;DR: 该论文提出了HORAI，这是一个面向时间序列分析的多模态基础模型，通过整合内生模态（衍生图像和文本）和外生知识（真实世界新闻）来增强时间序列理解，并在大规模多模态数据集MM-TS上预训练，在零样本预测和异常检测任务上达到最先进性能。
...
Method: 提出多模态预训练范式，利用内生模态（衍生图像和文本）和外生知识（真实世界新闻）为时间序列分析提供多视角。开发自动化数据构建管道创建MM-TS数据集（首个大规模多模态时间序列数据集，涵盖6个领域，达10亿数据点）。提出HORAI模型，包含频率增强跨模态编码器和时频解码器两个核心组件，有效融合多模态特征并增强跨模态和跨领域泛化能力。

Result: 在MM-TS数据集上预训练后，HORAI在时间序列预测和异常检测任务上实现了最先进的零样本性能，展示了强大的泛化能力。

Conclusion: 该研究为时间序列分析的多模态基础模型迈出了早期步伐，通过整合多模态信息和频率增强架构，显著提升了时间序列理解的深度和广度，为未来多模态时间序列分析奠定了基础。
...
TL;DR: TACO是一个端到端的表格压缩模型，通过潜在空间压缩训练数据集，在保持性能的同时显著提升推理速度（最高94倍）并减少内存消耗（最高97%），解决了传统表格Transformer模型的二次复杂度问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于梯度提升决策树的表格数据方法最近受到上下文学习表格基础模型的挑战。虽然这些Transformer架构的表格基础模型取得了最先进的性能，但其基于注意力机制的架构具有关于数据集大小的二次复杂度，导致训练和推理时间开销增加，限制了处理大规模数据集的能力。

Method: 提出TACO（端到端表格压缩模型），在潜在空间中压缩训练数据集。该方法通过压缩技术减少模型的计算复杂度和内存需求，同时保持预测性能。

Result: 在TabArena基准测试中，TACO的推理时间比最先进的表格Transformer架构快94倍，内存消耗减少97%，且性能没有显著下降。随着数据集规模增大，该方法不仅扩展性更好，而且相比其他基线方法获得了更好的性能。

Conclusion: TACO通过压缩训练数据集有效解决了表格Transformer模型的二次复杂度问题，在保持性能的同时显著提升了效率和可扩展性，为大规模表格数据处理提供了更优的解决方案。
...
TL;DR: 论文形式化分析了LLM对齐评估问题，指出有限行为评估无法唯一确定潜在对齐属性，即使理想化的行为评估也不能验证对齐作为潜在属性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型对齐评估主要依赖行为评估范式，但从未形式化分析从行为证据推断潜在对齐属性这一推理步骤的可靠性问题。

Method: 将对齐评估形式化为部分可观测下的可识别性问题，引入对齐可验证性问题（Alignment Verifiability Problem）和规范性不可区分性（Normative Indistinguishability）概念，分析评估感知智能体在有限行为评估下的表现。

Result: 主要结果是负面的可识别性定理：在有限行为评估和评估感知智能体条件下，观察到的行为合规性不能唯一识别潜在对齐，行为对齐测试应解释为不可区分性类的估计器而非对齐验证器。

Conclusion: 行为对齐基准应重新解释为提供特定评估机制内可观测合规性的上界，而非底层对齐的保证，这为对齐评估的理论基础提供了重要洞见。
...
TL;DR: 该研究通过大偏差理论分析SGD类方法的长期尾部衰减行为，为SGD和剪裁SGD在非凸优化中的梯度范数平方尾部提供了紧致的衰减率上界和下界。


<details>
  <summary>Details</summary>
Motivation: 现有SGD尾部行为研究多为有限时间的高概率保证，缺乏对固定误差阈值下失败概率（尾部衰减率）的直接分析，且无法捕捉现代学习模型训练数百万次迭代后的真实长期尾部衰减行为。

Method: 采用大偏差理论框架，分别分析：(1) 有界噪声下普通SGD最佳迭代梯度范数平方的尾部衰减；(2) 重尾噪声下剪裁SGD在p∈(1,2]阶有界矩条件下的尾部衰减；(3) 为两种方法提供匹配的下界证明。

Result: SGD尾部衰减上界为e^{-t/log(t)}，剪裁SGD在p∈(1,2)时为e^{-t^{β_p}/log(t)}（β_p=4(p-1)/(3p-2)），p=2时为e^{-t/log^2(t)}。下界均为e^{-t}，表明上界在多项式对数因子内是紧致的。相比现有有限时间界限的e^{-√t}和e^{-t^{β_p/2}}，发现了数量级更快的长期尾部衰减。

Conclusion: 该工作填补了SGD尾部衰减长期行为研究的空白，通过大偏差理论揭示了比现有结果快得多的尾部衰减速率，为个体算法运行提供了更强的长期保证，特别是在现代大规模训练场景下。
...
### [215] [Accelerating Benchmarking of Functional Connectivity Modeling via Structure-aware Core-set Selection](https://arxiv.org/abs/2602.05667)
*Ling Zhan,Zhen Li,Junjie Huang,Tao Jia*

Main category: cs.LG

TL;DR: 提出SCLCS框架，通过自监督学习选择代表性核心集，仅用10%数据就能保持功能连接模型排名，大幅提升基准测试效率


<details>
  <summary>Details</summary>
Motivation: 大规模fMRI数据集中数百种功能连接建模方法的基准测试对可重复神经科学至关重要，但模型-数据组合的爆炸式增长使穷举评估计算不可行，需要找到高效的核心集选择方法

Method: 提出结构感知对比学习的核心集选择框架（SCLCS）：1）使用自适应Transformer学习每个样本的独特功能连接结构；2）引入结构扰动分数（SPS）量化训练期间学习结构的稳定性；3）采用密度平衡采样策略确保核心集的结构鲁棒性和分布代表性

Result: 在REST-meta-MDD数据集上，SCLCS仅用10%数据就能保持真实模型排名，在排名一致性（nDCG@k）上比现有最佳方法提升达23.2%
...
### [216] [Stable but Wrong: When More Data Degrades Scientific Conclusions](https://arxiv.org/abs/2602.05668)
*Zhipeng Zhang,Kai Li*

Main category: cs.LG

TL;DR: 论文揭示了一个根本性科学悖论：即使使用标准统计推断方法，当观测数据质量以不可观测的方式退化时，更多数据反而会放大错误结论，而传统诊断指标却显示正常。


<details>
  <summary>Details</summary>
Motivation: 现代科学过度依赖大数据和自动化推断流程，隐含假设"数据越多结论越可靠"。本文旨在挑战这一假设，揭示在特定结构机制下，这种信念会根本性地失效。

Method: 通过构建最小化的合成实验，研究在观测可靠性以推断过程本身无法观测的方式退化时，标准推断程序的行为表现。

Result: 发现存在一个结构性机制，其中标准推断程序平稳收敛、保持良好校准、通过传统诊断检查，却系统地收敛到错误结论。更多数据不仅无法纠正错误，反而会放大错误，而基于残差和拟合优度的诊断指标仍保持误导性的正常状态。

Conclusion: 数据驱动科学存在内在限制：稳定性、收敛性和置信度不足以作为认识有效性的指标。推断不能被视为数据可用性的无条件结果，而必须受到观测过程完整性的明确约束。
...
### [217] [Perception-Based Beliefs for POMDPs with Visual Observations](https://arxiv.org/abs/2602.05679)
*Miriam Schäfers,Merlijn Krale,Thiago D. Simão,Nils Jansen,Maximilian Weininger*

Main category: cs.LG

TL;DR: PBP框架通过引入感知模型（图像分类器）将视觉观测映射到状态概率分布，使传统POMDP求解器能够处理高维观测空间，无需直接推理高维观测，并通过不确定性量化提高对视觉噪声的鲁棒性。
...
Method: 提出PBP框架，将图像分类器作为感知模型，将视觉观测映射到状态概率分布，直接纳入信念更新过程。为处理分类器不精确性，引入不确定性量化并提出了两种调整信念更新的方法。

Result: PBP框架在两种传统POMDP求解器上实现，实验表明：(1) 优于现有的端到端深度强化学习方法；(2) 不确定性量化提高了PBP对视觉噪声的鲁棒性。

Conclusion: PBP框架成功地将感知模型与传统POMDP求解器结合，有效处理高维视觉观测，并通过不确定性量化提高了系统的鲁棒性，为视觉丰富的POMDP问题提供了实用解决方案。
...
### [218] [Almost Asymptotically Optimal Active Clustering Through Pairwise Observations](https://arxiv.org/abs/2602.05690)
*Rachel S. Y. Teo,P. N. Karthik,Ramya Korlakai Vinayak,Vincent Y. F. Tan*

Main category: cs.LG

TL;DR: 提出一个用于聚类M个项目到未知K个簇的新分析框架，通过主动查询和噪声反馈，建立了查询次数的理论下界，并设计了渐近最优算法


<details>
  <summary>Details</summary>
Motivation: 解决在噪声环境下主动收集反馈的聚类问题，传统聚类方法通常假设数据是静态且无噪声的，而实际应用中数据收集往往是主动且有噪声的，需要理论分析和高效算法

Method: 使用变化测度技术建立查询次数的理论下界，设计基于广义似然比统计量的渐近最优算法，开发计算可行的GLR变体

Result: 建立了聚类准确度置信水平所需查询次数的基本下界，设计了性能与下界保持常数倍差距的算法，证明了算法的渐近最优性

Conclusion: 该框架为主动噪声聚类提供了理论基础和实用算法，平衡了理论最优性和计算可行性，在聚类准确度和查询效率之间取得了良好平衡
...
### [219] [FedRandom: Sampling Consistent and Accurate Contribution Values in Federated Learning](https://arxiv.org/abs/2602.05693)
...
<details>
  <summary>Details</summary>
Motivation: 在联邦学习中，公平评估参与者的贡献对于激励机制、恶意参与者识别和参与者积极性至关重要。然而，现有的贡献评估方法存在显著的不稳定性，这种不稳定性可能影响参与者参与联邦学习的意愿。

Method: FedRandom将贡献不稳定性视为统计估计问题，通过生成比常规联邦学习策略更多的样本来提供更一致和可靠的参与者贡献评估。

Result: 在CIFAR-10、MNIST、CIFAR-100和FMNIST数据集的不同数据分布测试中，FedRandom在一半评估场景中将与真实值的总体距离减少了三分之一以上，并在超过90%的情况下改善了稳定性。

Conclusion: FedRandom有效缓解了联邦学习中参与者贡献评估的不稳定性问题，为公平评估参与者贡献提供了更可靠的方法，有助于提高参与者参与联邦学习的积极性。
...
### [220] [Shared LoRA Subspaces for almost Strict Continual Learning](https://arxiv.org/abs/2602.06043)
...
<details>
  <summary>Details</summary>
Motivation: 大型预训练模型的高效持续适应在实际部署中至关重要，但面临灾难性遗忘和高昂重训练成本等挑战。现有参数高效调优方法缺乏严格的持续学习和知识整合机制。

Method: Share构建一个基础子空间从过去任务中提取核心知识，通过识别关键子空间方向增量整合新信息。每个新任务的知识被整合到这个不断演化的共享子空间中，促进前向知识转移，同时最小化灾难性干扰。
...
### [221] [Fix Representation (Optimally) Before Fairness: Finite-Sample Shrinkage Population Correction and the True Price of Fairness Under Subpopulation Shift](https://arxiv.org/abs/2602.05707)
*Amir Asiaee,Kaveh Aryan*

Main category: cs.LG

TL;DR: 论文研究发现，机器学习中预测准确性与群体公平性之间的权衡可能是训练数据中群体比例失真的假象。在子群体分布稳定但比例偏移的情况下，提出了最优的收缩加权方法，并建立了避免虚假权衡的评估协议。


<details>
  <summary>Details</summary>
Motivation: 机器学习实践中经常观察到预测准确性与群体公平性约束之间的张力，但有时公平性干预似乎能提高准确性。作者认为这两种现象都可能是训练数据中群体比例失真的产物，需要建立正确的评估框架来区分真实与虚假的公平性-准确性权衡。

Method: 1) 在子群体分布稳定但群体比例偏移的设定下，分析不同加权方法的理论性质；2) 提出最优的收缩加权方法，在目标分布和训练分布之间进行插值；3) 建立评估协议：先优化表示（使用收缩加权），再比较公平性干预，以隔离公平性的真实代价。

Result: 理论分析表明：完全重要性加权在渐近意义下无偏但在有限样本下非最优；最优的有限样本校正是收缩加权；"公平性提高准确性"的现象可能是由于与不当加权的基线比较而产生的。在合成数据和真实基准（Adult、COMPAS）上的实验验证了理论预测，新协议消除了虚假权衡，揭示了真实的公平性-效用边界。

Conclusion: 公平性与准确性之间的权衡可能是数据表示问题的假象。通过先优化表示（使用收缩加权），再评估公平性干预，可以避免虚假权衡，揭示公平性的真实代价。这一评估协议为机器学习公平性研究提供了更可靠的分析框架。
...
### [222] [Projected Boosting with Fairness Constraints: Quantifying the Cost of Fair Training Distributions](https://arxiv.org/abs/2602.05713)
*Amir Asiaee,Kaveh Aryan*
...
<details>
  <summary>Details</summary>
Motivation: 提升算法（如AdaBoost）具有良好的理论保证，但缺乏对群体公平性的考虑。如何在保持可分析训练动态的同时将公平性约束融入提升算法是一个重要问题

Method: 提出FairBoost方法：1）将集成诱导的指数权重分布投影到满足公平性约束的凸分布集上（作为重加权替代）；2）在这个公平分布上训练弱学习器。关键理论见解是投影训练分布会减少弱学习器的有效边缘

Result: 证明了指数损失界限，其中收敛速率取决于弱学习器边缘减去"公平性成本"项δ_t = √(KL(w^t || q^t)/2)，直接量化了提升动态中的准确性-公平性权衡。在标准基准测试上的实验验证了理论预测，并展示了具有稳定训练曲线的竞争性公平性-准确性权衡

Conclusion: FairBoost成功地将群体公平性约束融入提升算法，同时保持了可分析的理论保证，为准确性-公平性权衡提供了直接量化框架，并在实验中表现出良好的性能
...
### [223] [Muon in Associative Memory Learning: Training Dynamics and Scaling Laws](https://arxiv.org/abs/2602.05725)
*Binghui Li,Kaifei Wang,Han Zhong,Pinyan Lu,Liwei Wang*

Main category: cs.LG

TL;DR: Muon优化器在理论分析中显示出比梯度下降更快的收敛速度，特别是在处理频率不平衡的数据时，能实现指数级加速


<details>
  <summary>Details</summary>
Motivation: Muon优化器通过梯度的矩阵符号更新参数，在实践中表现出色，但其动态特性和缩放行为在理论上仍不明确，需要深入研究

Method: 在线性关联记忆模型中研究Muon优化器，使用softmax检索和层次化频率谱，分析有无标签噪声的情况，并与梯度下降进行比较

Result: 梯度下降学习频率分量时存在高度不平衡，低频分量成为收敛瓶颈；而Muon优化器能缓解这种不平衡，实现更快更均匀的进展。无噪声情况下Muon实现指数级加速；有噪声情况下推导出优化缩放定律，显示其优于梯度下降的缩放效率
...
### [224] [How to Achieve the Intended Aim of Deep Clustering Now, without Deep Learning](https://arxiv.org/abs/2602.05749)
*Kai Ming Ting,Wei-Jie Xu,Hang Zhang*

Main category: cs.LG

TL;DR: 该论文质疑深度聚类相对于k-means的优势，指出深度嵌入聚类(DEC)虽然通过自编码器学习潜在表示，但并未真正解决k-means在发现任意形状、大小和密度聚类方面的根本限制。


<details>
  <summary>Details</summary>
Motivation: 深度聚类常被认为优于k-means聚类，但这种优势通常只在图像数据集上得到验证。作者质疑深度嵌入聚类(DEC)是否真正克服了k-means聚类的根本限制，即无法发现任意形状、大小和密度的聚类。

Method: 通过分析深度嵌入聚类(DEC)方法，该研究探讨了深度学习方法是否真正解决了k-means的根本问题。同时提出，非深度学习方法通过利用数据集中聚类的分布信息，能够更有效地解决这些限制。

Result: 研究发现，深度嵌入聚类(DEC)并未真正克服k-means聚类的根本限制。更重要的是，作者发现非深度学习方法通过利用聚类的分布信息，能够实现深度聚类原本想要达到的目标。

Conclusion: 深度聚类方法并未充分利用底层数据分布信息，而非深度学习方法通过利用聚类的分布信息，能够更有效地解决k-means聚类的根本限制，这为聚类方法的发展提供了新的方向。
...
### [225] [Variational Speculative Decoding: Rethinking Draft Training from Token Likelihood to Sequence Acceptance](https://arxiv.org/abs/2602.05774)
...
<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法存在训练-解码不一致问题：它们优化单一贪婪轨迹，而解码过程需要验证和排序多个采样草稿路径。

Method: VSD将草稿训练建模为潜在提案的变分推断，通过ELBO优化草稿质量并最小化与目标分布的差异。采用EM算法：E步从经过oracle过滤的后验分布中采样MCMC样本，M步使用自适应拒绝加权和置信感知正则化最大化加权似然。

Result: 理论分析证实VSD增加了预期接受长度和加速比。在LLMs和MLLMs上的广泛实验显示，VSD相比EAGLE-3实现了9.6%的加速，相比ViSpec实现了7.9%的加速。

Conclusion: VSD通过变分推断框架解决了推测解码中的训练-解码不一致问题，显著提升了大型语言模型和多模态大语言模型的解码效率。
...
### [226] [Cross-Domain Offline Policy Adaptation via Selective Transition Correction](https://arxiv.org/abs/2602.05776)
*Mengbei Yan,Jiafei Lyu,Shengjie Sun,Zhongjian Qiao,Jingwen Yang,Zichuan Lin,Deheng Ye,Xiu Li*

Main category: cs.LG

TL;DR: STC算法通过修正源域数据中的动作和奖励，使其与目标域动态对齐，从而在跨域离线强化学习中有效利用源域数据提升策略学习效果。
...
Conclusion: 通过修正源域数据使其与目标域动态对齐，STC算法能够可靠地利用源域数据进行策略适应，有效解决了跨域离线强化学习中的动态不匹配问题。
...
### [227] [Distributional Reinforcement Learning with Diffusion Bridge Critics](https://arxiv.org/abs/2602.05783)
*Shutong Ding,Yimiao Zhou,Ke Hu,Mokai Pan,Shan Zhong,Yanwei Fu,Jingya Wang,Ye Shi*

Main category: cs.LG

TL;DR: 提出了一种基于扩散桥的分布强化学习方法DBC，首次将扩散桥模型用作评论家，通过建模Q值的逆累积分布函数来准确捕捉价值分布。
...
Conclusion: DBC是首个将扩散桥模型用作评论家的工作，通过准确捕捉价值分布来提升强化学习性能，为分布强化学习提供了新的有效方法。
...
### [228] [Classification Under Local Differential Privacy with Model Reversal and Model Averaging](https://arxiv.org/abs/2602.05797)
*Caihong Qin,Yang Bai*

Main category: cs.LG

TL;DR: 该论文将LDP下的隐私学习重新解释为迁移学习问题，提出三种新技术来提高分类性能而不损害隐私：基于噪声二进制反馈的评估机制、模型反转和模型平均。


<details>
  <summary>Details</summary>
Motivation: 本地差分隐私虽然提供了强大的隐私保护，但引入的噪声显著降低了数据效用。现有方法在保持隐私的同时难以维持良好的分类性能。

Method: 1) 基于噪声二进制反馈的数据集效用评估机制；2) 模型反转技术，通过反转决策边界来挽救表现不佳的分类器；3) 模型平均，根据估计的效用为多个反转分类器分配权重。

Result: 提供了LDP下的理论超额风险界限，并在模拟和真实数据集上展示了分类准确率的显著提升。

Conclusion: 通过将LDP隐私学习重新解释为迁移学习问题，并提出专门设计的技术，可以在不损害隐私的前提下显著提高分类性能。
...
### [229] [Principled Confidence Estimation for Deep Computed Tomography](https://arxiv.org/abs/2602.05812)
...
<details>
  <summary>Details</summary>
Motivation: 为基于深度学习的CT重建提供可靠的置信度估计和不确定性量化，解决传统方法在置信区域估计方面的不足

Method: 采用序列似然混合框架，基于符合Beer-Lambert定律的泊松噪声前向模型，适用于U-Net、U-Net集成和扩散模型等多种深度学习重建方法

Result: 深度学习重建方法相比传统方法能产生更紧密的置信区域，同时保持理论覆盖保证，并能检测重建图像中的幻觉伪影

Conclusion: 该框架将深度学习模型确立为不仅是有力的估计器，也是可靠的医学影像不确定性感知工具
...
### [230] [Where Does Warm-Up Come From? Adaptive Scheduling for Norm-Constrained Optimizers](https://arxiv.org/abs/2602.05813)
*Artem Riabinin,Andrey Veprikov,Arman Bolatov,Martin Takáč,Aleksandr Beznosikov*

Main category: cs.LG

TL;DR: 论文研究了面向范数约束优化器（如Muon和Lion）的自适应学习率调度，提出了一种广义平滑性假设，并基于此开发了能自动调整预热时长的实用学习率调度器，在LLaMA架构的大语言模型预训练中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前范数约束优化器（如Muon和Lion）的学习率调度通常依赖启发式的预热和衰减策略，缺乏理论依据，且需要大量手动调参。本文旨在建立理论框架，并开发能自动适应训练过程的学习率调度方法。

Method: 1. 提出广义平滑性假设：局部曲率随次优性间隙减小；2. 在该假设下建立收敛性保证，推导出自然产生预热和衰减的学习率选择；3. 开发实用学习率调度器，仅依赖标准超参数，自动调整预热时长；4. 在LLaMA架构的大语言模型预训练中进行实验验证。

Result: 实验表明：1. 广义平滑性假设在优化轨迹上得到经验验证；2. 提出的自适应预热选择在所有实验设置中一致优于或至少匹配最佳手动调优的预热调度，无需额外超参数搜索。

Conclusion: 本文为范数约束优化器提供了理论收敛保证，并开发了实用的自适应学习率调度器，能自动确定预热时长，在大语言模型预训练中显著减少手动调参需求，同时保持或提升性能。
...
### [231] [Synthesizing Realistic Test Data without Breaking Privacy](https://arxiv.org/abs/2602.05833)
*Laura Plein,Alexi Turcotte,Arina Hallemans,Andreas Zeller*

Main category: cs.LG

TL;DR: 该论文提出了一种基于模糊测试和判别器的隐私保护合成数据生成方法，通过间接利用原始数据生成具有相同统计特性的测试数据集，避免直接训练带来的隐私风险。
...
Method: 受GAN启发但采用不同方法：1）使用测试生成器（模糊器）从输入规范生成测试数据，保留原始数据的约束；2）使用判别器模型评估生成数据与原始数据的接近程度；3）通过演化样本和判别器确定"好样本"，生成具有相同统计分布的隐私保护数据。

Result: 在四个用于评估最先进技术的数据集上进行了实验，结果表明该方法在生成具有高实用性的合成数据集方面具有潜力，同时能够有效保护隐私。

Conclusion: 该方法能够生成既保持原始数据统计特性又保护隐私的合成数据集，通过间接利用原始数据避免了直接训练带来的隐私风险，在实用性和隐私保护之间取得了良好平衡。
...
### [232] [DLM-Scope: Mechanistic Interpretability of Diffusion Language Models via Sparse Autoencoders](https://arxiv.org/abs/2602.05859)
...
Method: 提出了DLM-Scope框架，将Top-K稀疏自编码器（SAEs）应用于扩散语言模型。通过SAE插入实验、特征提取分析、扩散时间干预测试、解码顺序信号研究以及训练后稳定性评估等方法，系统探索SAE在DLMs中的应用。

Result: 1. SAE能有效提取DLMs中的可解释特征；2. SAE插入对DLMs的影响与LLMs不同：在DLMs早期层插入SAE可降低交叉熵损失，而LLMs中通常会产生损失惩罚；3. DLM的SAE特征在扩散时间干预中效果更好，常优于LLM引导；4. SAE能为DLM解码顺序提供有用信号；5. SAE特征在DLMs训练后阶段保持稳定。
...
### [233] [Constrained Group Relative Policy Optimization](https://arxiv.org/abs/2602.05863)
...
<details>
  <summary>Details</summary>
Motivation: 虽然GRPO已成为无评论者策略学习的可扩展框架，但在具有显式行为约束的设置中扩展仍未被充分探索。需要将约束优化集成到GRPO框架中，特别是在机器人等需要安全约束的领域

Method: 提出Constrained GRPO方法：1) 使用指示器成本函数指定约束，通过拉格朗日松弛直接优化违反率；2) 发现多组件优势估计会破坏约束学习，因为不匹配的组件标准差扭曲了不同目标项的相对重要性；3) 提出标量化优势构造，保持奖励和约束项之间的预期权衡

Result: 在玩具网格世界实验中证实了预测的优化病理问题，并证明标量化优势恢复了稳定的约束控制。在机器人任务评估中，Constrained GRPO提高了约束满足度同时增加了任务成功率

Conclusion: Constrained GRPO为具体AI领域中受约束的策略优化提供了一个简单有效的方案，这些领域越来越依赖大型多模态基础模型。该方法通过标量化优势构造解决了多组件估计问题，实现了稳定的约束控制
...
### [234] [Dr. Kernel: Reinforcement Learning Done Right for Triton Kernel Generations](https://arxiv.org/abs/2602.05885)
*Wei Liu,Jiawei Xu,Yingru Li,Longtao Zheng,Tianjian Li,Qian Liu,Junxian He*

Main category: cs.LG

TL;DR: 本文提出KernelGYM环境和TRLOO方法，训练Dr.Kernel-14B模型生成高性能内核代码，在KernelBench测试中超越Claude-4.5-Sonnet和GPT-5


<details>
  <summary>Details</summary>
Motivation: 高质量内核对可扩展AI系统至关重要，但训练LLMs生成此类代码面临数据不足、环境脆弱、奖励黑客和懒惰优化等问题

Method: 设计KernelGYM分布式GPU环境，提出TRLOO解决多轮RL中的偏差梯度问题，引入不匹配校正、基于性能分析的奖励和拒绝采样

Result: Dr.Kernel-14B在KernelBench Level-2子集中，31.6%生成的内核达到1.2倍加速，超过Claude-4.5-Sonnet(26.7%)和GPT-5(28.6%)；多轮选择最佳候选时提升至47.8%

Conclusion: 通过系统性RL方法、稳健环境和创新训练技术，成功训练出高性能内核生成模型，为AI系统开发提供了有效解决方案
...
### [235] [Escaping Local Minima Provably in Non-convex Matrix Sensing: A Deterministic Framework via Simulated Lifting](https://arxiv.org/abs/2602.05887)
*Tianqi Shen,Jinji Yang,Junze He,Kunhan Gao,Ziye Ma*

Main category: cs.LG

TL;DR: 该论文提出了一种模拟Oracle方向（SOD）逃逸机制，用于低秩矩阵感知问题，通过模拟过参数化空间的逃逸方向来逃离虚假局部最小值，而无需实际进行张量提升计算。


<details>
  <summary>Details</summary>
Motivation: 低秩矩阵感知是一个具有挑战性的非凸优化问题，其优化景观包含大量虚假局部最小值，使得基于梯度的优化器难以收敛到全局最优。虽然过参数化可以通过张量提升将局部最小值转换为严格鞍点，但实际进行张量提升计算上不可行。

Method: 提出模拟Oracle方向（SOD）逃逸机制，设计数学框架模拟过参数化空间的景观和逃逸方向，将过参数化逃逸方向投影到原始参数空间，保证从现有局部最小值严格降低目标函数值。
...
### [236] [Parity, Sensitivity, and Transformers](https://arxiv.org/abs/2602.05896)
*Alexander Kozachinskiy,Tomasz Steifer,Przemysław Wałȩga*

Main category: cs.LG

TL;DR: 本文研究了Transformer架构能否解决PARITY问题，提出了新的构造方法并给出了首个下界证明。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer架构已有近十年历史，但我们对其计算能力仍了解有限。特别是，单层Transformer能否解决PARITY问题？现有构造需要至少2层且包含不实用特性，如长度相关位置编码、hardmax、无正则化参数的layernorm或无法实现因果掩码。

Method: 提出了新的Transformer构造方法，使用softmax、长度无关且多项式有界的位置编码、无layernorm，同时支持有/无因果掩码。并首次给出了Transformer解决PARITY问题的下界证明。

Result: 成功构造了能够解决PARITY问题的Transformer，证明了单层单头Transformer无法解决PARITY问题，这是该问题的首个下界结果。

Conclusion: Transformer架构的计算能力仍有待深入探索，本文为理解其理论能力提供了新的构造方法和下界证明，填补了该领域的研究空白。
...
### [237] [DFPO: Scaling Value Modeling via Distributional Flow towards Robust and Generalizable LLM Post-Training](https://arxiv.org/abs/2602.05890)
...
TL;DR: DFPO是一种鲁棒的分布强化学习框架，通过将价值建模为跨时间步的连续流，而非独立的标量分位数，提高了在噪声监督下的训练稳定性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在真实世界环境中训练强化学习系统面临噪声监督和域外泛化能力差的问题，特别是在LLM后训练中。现有的分布RL方法虽然通过建模多个分位数点提高了鲁棒性，但仍将每个分位数作为标量独立学习，导致价值表示粗糙，缺乏对状态信息的细粒度条件化，在复杂和OOD条件下表现不佳。

Method: 提出DFPO框架，将价值建模为跨时间步的连续流，通过学习价值流场而非孤立的分位数预测来扩展价值建模。该方法进一步集成了条件风险控制和价值流轨迹上的一致性约束，以在噪声反馈下稳定训练。

Result: 在对话、数学推理和科学任务上的实验表明，DFPO在噪声监督下优于PPO、FlowRL和其他鲁棒基线，实现了更好的训练稳定性和泛化能力。

Conclusion: DFPO通过连续价值流建模和条件风险控制，为在噪声监督和复杂OOD条件下训练RL系统提供了一种有效的解决方案，显著提高了训练稳定性和泛化性能。
...
### [238] [Regularized Calibration with Successive Rounding for Post-Training Quantization](https://arxiv.org/abs/2602.05902)
*Seohyeon Cha,Huancheng Chen,Dongjun Kim,Haoran Zhang,Kevin Chan,Gustavo de Veciana,Haris Vikalo*

Main category: cs.LG

TL;DR: 该论文提出了一种基于正则化非对称校准的有界搜索量化方法，通过对称与非对称校准的插值作为正则化，结合连续舍入过程，在保持PTQ二次结构的同时提升LLM量化效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署面临内存和延迟成本挑战，后训练量化（PTQ）虽然能实现高效推理，但其效果严重依赖于量化目标和舍入过程。现有方法在激活不匹配时表现不佳，需要更稳健的量化方法。

Method: 提出对称与非对称校准的插值作为正则化，保持PTQ标准二次结构的同时增强对激活不匹配的鲁棒性。推导出包含非对称校准的简单连续舍入过程，并扩展为有界搜索方法，允许在量化质量和计算成本之间进行显式权衡。

Result: 在多个LLM家族、量化比特宽度和基准测试上的实验表明，基于正则化非对称校准目标的有界搜索方法持续改善了困惑度和准确率，相比PTQ基线有显著提升，同时仅带来适度且可控的额外计算成本。

Conclusion: 通过将对称与非对称校准的插值作为正则化，结合有界搜索策略，提出的方法为LLM后训练量化提供了更稳健有效的解决方案，在保持计算效率的同时显著提升量化质量。
...
### [239] [Verification of the Implicit World Model in a Generative Model via Adversarial Sequences](https://arxiv.org/abs/2602.05903)
*András Balogh,Márk Jelasity*

Main category: cs.LG

TL;DR: 该研究提出使用对抗序列生成方法来验证序列模型在象棋领域的合理性，发现所有模型都不完全合理，但某些训练技术和数据集选择能显著改善模型合理性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决生成序列模型在基于样本训练时能否准确捕捉语言真实结构（"世界模型"）的问题。理论表明模型最多只能保证生成有效序列，但无法保证生成所有有效序列。因此需要实用工具来验证序列模型的合理性，选择象棋作为研究领域，因其具有足够复杂性同时又有简单的基于规则的世界模型。

Method: 提出对抗序列生成方法来验证序列模型的合理性。对抗方法生成有效序列，迫使序列模型预测无效的下一步移动。除了证伪合理性外，该方法还适用于更细粒度的失败模式分析和训练选择效果评估。研究提出了多种对抗序列生成方法，并在大量象棋模型上评估该方法。训练模型时使用了随机和高质量象棋对局数据，采用多种训练方案。同时研究了棋盘状态探针在训练和攻击方法中的潜在应用。

Result: 研究发现所有模型都不完全合理，但某些训练技术和数据集选择能显著改善模型的合理性。同时发现，在大多数模型中，提取的棋盘状态在下一个令牌预测中没有因果作用。

Conclusion: 对抗序列生成是验证序列模型合理性的有效方法，能够揭示模型的失败模式并评估不同训练选择的影响。虽然当前模型无法完全达到合理性，但通过改进训练技术和数据集选择可以显著提升模型性能。棋盘状态信息在大多数模型中并未发挥预期作用，这为未来模型设计提供了重要启示。
...
### [240] [Chunky Post-Training: Data Driven Failures of Generalization](https://arxiv.org/abs/2602.05910)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型后训练使用多种数据集，但这些数据集除了预期行为外还编码了偶然模式（如格式与内容的相关性、狭窄的表述方式等）。这些模式对开发者不可见但对模型显著，导致模型产生令创造者惊讶的行为（如拒绝以特定格式呈现的真实事实）。

Method: 作者提出了SURF（运行时表面意外行为的黑盒管道）和TURF（将失败追溯到特定后训练数据的工具）。将这些工具应用于前沿模型（Claude 4.5、GPT-5.1、Grok 4.1、Gemini 3）和开源模型（Tülu 3）。

Result: 研究发现块状后训练会产生校准不当的行为，这些行为通常源于不平衡或未充分指定的后训练数据块。

Conclusion: 块状后训练是LLM后训练中的一个重要问题，会导致模型学习虚假相关性。SURF和TURF工具能有效检测和追踪这些问题，为改进后训练数据质量提供了方法。
...
### [241] [Approximation of Log-Partition Function in Policy Mirror Descent Induces Implicit Regularization for LLM Post-Training](https://arxiv.org/abs/2602.05933)
*Zhenghao Xu,Qin Lu,Changlong Yu,Tuo Zhao*

Main category: cs.LG

TL;DR: PMD-mean是一种实用的策略镜像下降算法，通过用采样策略的平均奖励近似对数配分函数，在LLM强化学习中实现更稳定高效的训练。


<details>
  <summary>Details</summary>
Motivation: 策略镜像下降(PMD)为强化学习提供了理论框架，但在LLM的大动作空间中，精确计算配分函数非常困难。需要一种实用的近似方法来解决有限样本下的训练挑战。

Method: 提出PMD-mean算法：1)用采样策略的平均奖励近似对数配分项；2)在对数策略空间进行回归；3)该方法隐式优化了带有自适应混合KL-χ²正则器的镜像下降子问题。

Result: 实验表明PMD-mean在数学推理任务上表现优异，具有更好的稳定性、时间效率和鲁棒性。额外的χ²正则化限制了概率的大幅变化，在预期奖励较低时产生更保守的更新。

Conclusion: PMD-mean不仅是一种实用的近似算法，还揭示了其隐式的正则化机制，为LLM强化学习算法的改进提供了理论洞见和实用途径。
...
### [242] [Diamond Maps: Efficient Reward Alignment via Stochastic Flow Maps](https://arxiv.org/abs/2602.05993)
...
TL;DR: 提出"Diamond Maps"随机流映射模型，能够在推理时高效适应任意奖励函数，解决生成模型奖励对齐的挑战


<details>
  <summary>Details</summary>
Motivation: 流和扩散模型能产生高质量样本，但训练后适应用户偏好或约束的成本高且脆弱（奖励对齐问题）。作者认为高效奖励对齐应该是生成模型的内在属性，而不是事后补救

Method: 设计"Diamond Maps"随机流映射模型，将多个模拟步骤摊销到单步采样器中（类似流映射），同时保留奖励对齐所需的随机性。该设计通过实现价值函数的高效一致估计，使搜索、顺序蒙特卡洛和引导方法具有可扩展性

Result: 实验表明Diamond Maps可以通过从GLASS Flows蒸馏高效学习，实现更强的奖励对齐性能，比现有方法具有更好的可扩展性
...
TL;DR: 该研究探讨了基于黎曼几何的降维方法，这些方法尊重数据的底层流形结构，特别是主测地线分析(PGA)作为流形值数据的非线性PCA扩展，以及通过黎曼适配扩展判别分析等方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于开发能够尊重数据底层流形结构的降维方法，特别是针对约束在弯曲空间（如超球面和对称正定流形）的数据，这些数据在欧几里得空间中难以得到准确表示。

Method: 方法包括：1）主测地线分析(PGA)作为流形值数据的非线性PCA扩展；2）通过黎曼适配扩展判别分析；3）利用测地线距离、切空间表示和内在统计度量；4）讨论相关的流形学习技术及其理论基础。

Result: 实验结果表明，黎曼方法在代表性数据集上提供了改进的表示质量和分类性能，特别是在约束于弯曲空间的数据上，相比其欧几里得对应方法表现更优。

Conclusion: 该研究强调了几何感知降维在现代机器学习和数据科学应用中的重要性，特别是对于流形结构数据，黎曼几何方法能够提供更准确和有效的表示。
...
TL;DR: OrthoMerge是一种在正交群黎曼流形上合并微调后LLM的新方法，通过保留权重几何结构来避免传统线性合并破坏模型内在几何特性。


<details>
  <summary>Details</summary>
Motivation: 当前主流的模型合并方法依赖于欧几里得空间中的线性算术，这会破坏预训练权重的内在几何特性（如超球面能量），导致模型性能下降。需要一种能保留权重几何结构的合并方法。

Method: 提出正交模型合并（OrthoMerge），在正交群形成的黎曼流形上进行合并操作。通过将正交微调（OFT）学习的任务特定正交矩阵映射到李代数，实现考虑适应方向和强度的原则性集成。对于非OFT方法微调的模型，采用正交-残差解耦策略，通过求解正交Procrustes问题提取正交分量，在正交群流形上合并，而线性残差则通过标准加法合并处理。

Result: 广泛的实证结果表明，OrthoMerge在缓解灾难性遗忘和保持模型在不同任务上的性能方面表现出有效性。

Conclusion: OrthoMerge通过在正交群黎曼流形上进行模型合并，成功保留了权重的几何结构，为整合不同能力的LLM提供了一种更有效的合并方法，优于传统的线性合并方法。
...
### [245] [Optimism Stabilizes Thompson Sampling for Adaptive Inference](https://arxiv.org/abs/2602.06014)
*Shunxing Yan,Han Zhong*

Main category: cs.LG

TL;DR: 本文研究了Thompson采样在多臂赌博机中的推断性质，发现乐观机制能稳定采样过程，使渐近推断有效，同时只带来轻微遗憾代价。


<details>
  <summary>Details</summary>
Motivation: Thompson采样广泛用于随机多臂赌博机，但在自适应数据收集下的推断性质复杂。经典渐近理论可能失效，因为臂特定的样本量是随机的，且通过动作选择规则与奖励耦合。需要研究如何恢复稳定性以实现有效推断。

Method: 研究K臂高斯赌博机，识别乐观机制作为恢复稳定性的关键。首先证明方差膨胀Thompson采样对任意K≥2都是稳定的；其次分析另一种乐观修改方法，保持后验方差不变但给后验均值添加显式奖励加成。

Result: 方差膨胀Thompson采样在任意K≥2（包括多个最优臂的挑战性情况）下都是稳定的，解决了先前研究的开放性问题。另一种乐观修改方法也达到相同的稳定性结论。

Conclusion: 适当实现的乐观机制能稳定Thompson采样，使多臂赌博机中的渐近推断有效，同时只带来轻微的额外遗憾代价。
...
### [246] [$f$-GRPO and Beyond: Divergence-Based Reinforcement Learning Algorithms for General LLM Alignment](https://arxiv.org/abs/2602.05946)
*Rajdeep Haldar,Lantao Mei,Guang Lin,Yue Xing,Qifan Song*

Main category: cs.LG

TL;DR: 该论文提出了基于f-散度的统一对齐框架，包括f-GRPO和f-HAL两种方法，在数学推理和安全对齐任务上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有偏好对齐（PA）目标被证明是已对齐（选择）和未对齐（拒绝）响应分布之间的散度估计器。本研究将这种基于散度的视角扩展到一般对齐设置，如具有可验证奖励的强化学习（RLVR），其中只有环境奖励可用。

Method: 提出了基于f-散度变分表示的两种方法：1) f-组相对策略优化（f-GRPO），一种基于策略的强化学习类方法；2) f-混合对齐损失（f-HAL），一种混合策略目标。这些方法为一般LLM对齐提供了统一框架。

Result: 在RLVR（数学推理）和PA任务（安全对齐）上的实证验证表明，该方法相比当前方法具有优越的性能和灵活性。

Conclusion: 该研究提供了一个基于f-散度的统一对齐框架，理论保证能提高对齐后的平均奖励，并在实际任务中表现出色。
...
### [247] [Discrete diffusion samplers and bridges: Off-policy algorithms and applications in latent spaces](https://arxiv.org/abs/2602.05961)
*Arran Carter,Sanghyeok Choi,Kirill Tamogashev,Víctor Elvira,Nikolay Malkin*

Main category: cs.LG

TL;DR: 该论文提出了一种改进离散扩散采样器的方法，通过引入离策略训练技术，将离散扩散采样器推广到连接两个任意分布的任务，并展示了在图像生成模型离散潜空间中进行无数据后验采样的应用。


<details>
  <summary>Details</summary>
Motivation: 虽然连续空间的扩散采样器已得到广泛研究，但离散空间的扩散采样器研究仍不充分，未能充分利用连续空间采样的常用技术。需要填补这一空白，改进离散扩散采样器的性能。

Method: 1. 引入离策略训练技术改进离散扩散采样器；2. 将离散扩散采样器推广到连接两个任意分布的任务，首次在离散域引入数据到能量的薛定谔桥训练；3. 将提出的扩散采样器应用于图像生成模型离散潜空间的无数据后验采样。

Result: 离策略训练技术提高了离散采样器在已有和新合成基准测试上的性能；成功将离散扩散采样器推广到连接两个任意分布的任务；展示了在图像生成模型离散潜空间中进行无数据后验采样的实际应用。

Conclusion: 通过引入离策略训练技术，显著提升了离散扩散采样器的性能，成功将其推广到更广泛的任务中，为离散空间采样问题提供了有效的解决方案，特别是在图像生成模型的离散潜空间后验采样中展示了实用价值。
...
### [248] [Layer-wise LoRA fine-tuning: a similarity metric approach](https://arxiv.org/abs/2602.05988)
...
TL;DR: 该论文提出了一种层选择方法，通过测量内部表示的变化贡献度，只选择最相关的层进行LoRA微调，将可训练参数减少高达50%，同时保持预测性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模不断增长，即使像LoRA这样的参数高效微调技术（减少99%参数）也可能变得不足。需要进一步减少微调参数数量，同时保持模型性能。

Method: 提出层选择的LoRA微调方法，通过测量各层内部表示的变化贡献度，识别最相关的层进行微调。该方法与现有低秩适应技术正交且兼容，可减少高达50%的可训练参数。

Result: 在编码器架构上，GLUE基准测试中参数减少导致预测性能下降可忽略；在解码器架构上，数学问题解决和编码任务中性能有小幅下降甚至提升；在多模态模型上也获得与全层LoRA微调竞争的结果。

Conclusion: 通过层选择策略可显著减少LoRA微调参数（高达50%），同时保持或提升模型在下游任务上的性能，为大规模模型的高效微调提供了有效解决方案。
...
Method: 设计正交自注意力机制，通过将查询-键值形成的斜对称矩阵映射到矩阵指数来参数化正交注意力矩阵，利用低秩结构实现线性计算复杂度，并提供确保雅可比矩阵良好条件数的初始化方案。

Result: OSA能够实现线性序列长度的计算复杂度和内存成本，且通过理论证明其雅可比矩阵具有良好的条件数，为无跳跃连接Transformer提供了可行的训练方案。

Conclusion: 正交自注意力机制为解决Transformer无跳跃连接架构中的训练稳定性问题提供了有效方案，通过正交约束避免了传统Softmax自注意力的不稳定性问题。
...
### [250] [AP-OOD: Attention Pooling for Out-of-Distribution Detection](https://arxiv.org/abs/2602.06031)
...
<details>
  <summary>Details</summary>
Motivation: 当前OOD检测研究面临的关键挑战是如何有效利用和聚合语言模型中的标记嵌入来获得OOD分数。现有方法通常采用简单的平均聚合方式，未能充分利用标记级信息。

Method: AP-OOD是一种半监督方法，通过利用标记级信息超越简单的平均聚合，能够灵活地在无监督和有监督设置之间插值，允许使用有限的辅助异常数据。

Result: AP-OOD在文本OOD检测中创造了新的最先进水平：在无监督设置下，将XSUM摘要任务的FPR95从27.84%降低到4.67%，将WMT15英法翻译任务的FPR95从77.08%降低到70.37%。

Conclusion: AP-OOD通过有效利用标记级信息和灵活的半监督框架，显著提升了文本OOD检测性能，为机器学习模型的可靠部署提供了更有效的解决方案。
...
### [251] [Can vision language models learn intuitive physics from interaction?](https://arxiv.org/abs/2602.06033)
*Luca M. Schulze Buschoff,Konstantinos Voudouris,Can Demircan,Eric Schulz*

Main category: cs.LG

TL;DR: 论文研究了预训练视觉语言模型在物理世界理解方面的局限性，发现即使通过监督微调或与环境交互的强化学习，模型仍难以学习到可泛化的物理直觉规则。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉语言模型缺乏对物理世界的良好直觉，虽然已有研究表明监督微调可以提升简单物理任务的表现，但这些模型似乎无法学习到能够泛化到新情境的稳健物理规则。基于认知科学研究，作者假设模型需要通过与环境交互来正确学习物理动态。

Method: 使用强化学习方法训练模型，让模型通过与环境交互进行学习。比较了监督微调和交互学习两种方法，并测试了模型在相关任务间的泛化能力。

Result: 虽然通过交互学习可以提升模型在特定任务内的表现，但这种方法同样无法产生具有可泛化物理直觉的模型。研究发现，在一个任务上训练的模型无法可靠地泛化到相关任务，即使这些任务共享视觉统计特征和物理原理，无论模型是否通过交互训练。

Conclusion: 当前的方法（包括监督微调和交互式强化学习）都无法让模型学习到稳健且可泛化的物理直觉规则。这表明仅通过任务特定训练或简单交互不足以获得真正的物理世界理解，需要更根本的方法来培养模型的物理推理能力。
