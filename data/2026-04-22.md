...
TL;DR: AutoAWG是一个可控的恶劣天气视频生成框架，通过语义引导的自适应融合、消失点锚定的时间合成和掩码训练，在视觉质量和标注可重用性之间取得平衡，显著提升了自动驾驶感知在恶劣天气下的鲁棒性。
...
Method: 1) 采用语义引导的自适应多控制融合，平衡强天气风格化与安全关键目标的高保真度；2) 利用消失点锚定的时间合成策略从静态图像构建训练序列，减少对合成数据的依赖；3) 采用掩码训练增强长时域生成稳定性。

Result: 在nuScenes验证集上，AutoAWG显著优于先前最先进方法：无首帧条件时，FID和FVD相对降低50.0%和16.1%；有首帧条件时，进一步降低8.7%和7.2%。在风格保真度、时间一致性和语义-结构完整性方面均表现出优势。

Conclusion: AutoAWG通过创新的可控恶劣天气视频生成框架，有效解决了自动驾驶在恶劣天气下的数据稀缺问题，具有提升下游感知性能的实际价值，为自动驾驶系统在复杂天气条件下的鲁棒性提供了重要支持。
...
TL;DR: FlowSG将场景图生成重新定义为混合离散-连续状态上的连续时间传输任务，通过约束感知的渐进式细化来联合合成节点和边，相比一次性分类方法在多项指标上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前大多数场景图生成方法将其视为一次性确定性分类问题，而非真正的渐进式生成任务。这限制了模型对复杂视觉关系推理的能力，需要更符合场景图生成本质的生成式方法。

Method: 1. 使用VQ-VAE将场景图量化为紧凑可预测的token；2. 采用图Transformer预测条件速度场来传输连续几何（边界框）并更新离散分类token的后验分布；3. 通过流条件消息聚合耦合语义和几何信息；4. 结合几何的流匹配损失和token的离散流目标进行训练。

Result: 在VG和PSG数据集上的闭集和开放词汇协议实验显示，在谓词R/mR和图级指标上均取得一致提升，平均比当前最佳方法USG-Par提高约3个百分点，验证了混合离散-连续生成式方法的优越性。

Conclusion: FlowSG通过将场景图生成重新构建为连续时间传输任务，实现了渐进式生成过程，相比传统一次性分类方法在性能上有显著改进，为场景图生成提供了新的生成式框架。
...
TL;DR: 提出基于单目RGB相机的实时视觉方法，用于估计人类对自主移动机器人(AMR)的感知状态，通过3D人体姿态重建和头部朝向估计来判断人类是否注意到AMR，从而让AMR根据人类感知状态安全调整运动。


<details>
  <summary>Details</summary>
Motivation: 在仓库环境中，人类工人与自主移动机器人(AMR)混合交通场景下，现有方法通常将人类视为通用动态障碍物，导致AMR行为过于保守（如减速或绕行），即使工人完全意识到并能安全共享空间。这影响了操作效率。

Method: 使用单目RGB相机实时估计人类对AMR的感知状态，整合最先进的3D人体姿态重建技术和头部朝向估计方法，确定人类相对于AMR的位置及其视线锥，从而判断人类是否注意到AMR。整个流程在NVIDIA Isaac Sim物理精确机器人仿真环境中使用合成数据进行验证。

Result: 实验结果表明，该系统能够可靠地实时检测人类位置及其注意力状态，使AMR能够根据人类感知状态安全调整运动。

Conclusion: 该方法通过准确估计人类对AMR的感知状态，使AMR能够更智能地调整行为，在确保安全的同时提高工业自动化环境中的操作效率，解决了现有方法过于保守的问题。
...
<details>
  <summary>Details</summary>
Motivation: 文本驱动的可控舞蹈生成面临高质量数据集稀缺和复杂舞蹈动作难以描述的挑战，需要解决舞蹈的空间动态性、方向性和身体部位解耦运动等特性

Method: 1. 提出Choreographic Syntax理论框架和标注系统；2. 构建DanceFlow数据集（41小时高质量动作+634万词描述）；3. 开发基于Momentum Human Rig的DanceCrafter模型，采用连续流形运动表示、混合归一化策略和解剖感知损失函数

Result: 在运动质量、精细可控性和生成自然度方面达到最先进水平，通过广泛评估和用户研究验证了性能优势

Conclusion: 通过理论框架、高质量数据集和专门模型设计的结合，成功实现了高保真、稳定的复杂舞蹈序列生成，解决了文本驱动舞蹈生成的关键瓶颈
...
TL;DR: 本文提出了一种新的多编码器U-Net架构，用于从双参数MRI中自动分割前列腺病变，通过文本-图像对齐、热图校准和置信度门控交叉注意力精炼器，在PI-CAI数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前的前列腺病变3D分割方法在多模态信息融合方面存在挑战，视觉语言模型缺乏细粒度的病变级语义指导，需要更有效的局部化引导机制来提高分割精度。

Method: 提出多编码器U-Net架构，包含三个关键创新：1)增强前景文本-图像相似性的对齐损失；2)校准相似性图并抑制虚假背景激活的热图损失；3)在置信区域进行局部边界编辑的置信度门控多头交叉注意力精炼器。采用阶段调度训练策略稳定优化。

Result: 在PI-CAI数据集上持续优于先前方法，建立了新的最先进性能，通过增强的多模态融合和局部化文本指导实现了更好的分割效果。

Conclusion: 该方法通过创新的对齐机制、热图校准和局部精炼策略，有效解决了前列腺病变分割中的多模态融合和语义指导问题，为医学图像分析提供了新的解决方案。
...
Method: 使用带标注的合成X射线数据集和真实X射线数据集，微调两种多模态大语言模型，让它们从X射线图像中检索最接近的骨骼标志点。与领先的深度学习方法进行定量比较，并进行定性实验展示推理能力和C臂导航能力。

Result: 在两个数据集上，微调后的MLLMs在所有定位任务中均表现出与深度学习方法相当的性能。定性实验显示MLLMs具有推理能力和空间感知能力，能够纠正初始错误预测并顺序导航C臂至目标位置。
...
Method: 提出DeltaSeg架构：U型编码器-解码器，编码器使用深度可分离卷积和扩张阶段；瓶颈层采用ASPP捕获多尺度上下文；分层注意力策略包括编码器的SE通道注意力、瓶颈和解码器的坐标注意力，以及跳跃连接中的新型深度Delta注意力机制；通过多尺度辅助头进行深度监督。

Result: 在S2DS（7类）和CSDD（9类）两个数据集上评估，DeltaSeg在12种竞争架构（包括U-Net、SA-UNet、UNet3+、SegFormer等）中表现最佳，展示了跨损伤类型、成像条件和结构几何的强泛化能力。

Conclusion: DeltaSeg通过分层注意力策略和深度监督，有效解决了结构缺陷分割中的挑战，在多个基准测试中表现出优越性能，为视觉检测中的自动化缺陷分割提供了有效解决方案。
...
TL;DR: URoPE是一种通用的相对位置编码方法，将RoPE扩展到跨视图或跨维度的几何空间，通过采样3D点并投影到查询图像平面，实现2D-3D几何推理。
...
Method: URoPE为每个关键/值图像块沿对应相机光线在预定义深度锚点处采样3D点，然后将这些点投影到查询图像平面，使用投影的像素坐标应用标准的2D RoPE。

Result: 实验表明，URoPE作为Transformer架构的插件位置编码，在新视图合成、3D目标检测、目标跟踪和深度估计等任务中一致提升了基于Transformer模型的性能。

Conclusion: URoPE是一种参数无关、内参感知的相对位置编码方法，对全局坐标系选择具有不变性，同时完全兼容现有的RoPE优化注意力核，有效提升了跨几何空间的推理能力。
...
TL;DR: REVEAL框架通过将眼底照片与个体化疾病风险特征对齐，结合分组感知对比学习策略，实现了阿尔茨海默病和痴呆的早期预测（平均提前8年）。


<details>
  <summary>Details</summary>
Motivation: 视网膜为阿尔茨海默病和痴呆提供了独特的非侵入性观察窗口，但现有方法通常将影像和风险因素分开建模，无法捕捉多模态联合模式，且缺乏组织或对齐具有相似特征患者的机制。

Method: 1. 将结构化问卷风险因素转化为临床可解释的叙述文本，与预训练视觉语言模型兼容；2. 提出分组感知对比学习策略，将具有相似视网膜形态学和风险因素的患者聚类为正样本对，加强多模态对齐。

Result: REVEAL框架在预测阿尔茨海默病和痴呆方面显著优于最先进的视网膜成像模型与临床文本编码器的组合，以及通用视觉语言模型，证明了联合建模视网膜生物标志物和临床风险因素的价值。

Conclusion: REVEAL提供了一个可推广的非侵入性方法，用于早期阿尔茨海默病和痴呆风险分层，有望实现更早干预并改善人群层面的预防性护理。
...
TL;DR: CAHAL是一个用于临床MRI低分辨率扫描的幻觉鲁棒、物理信息驱动的分辨率增强框架，直接在患者原始采集空间操作，避免解剖幻觉和结构扭曲，提升下游定量分析可靠性。


<details>
  <summary>Details</summary>
Motivation: 临床实践中普遍存在的厚层、各向异性MRI采集限制了大规模自动化形态测量分析。现有的生成式超分辨率方法虽然能产生视觉上吸引人的各向同性体积，但常常引入解剖幻觉、系统性体积高估和结构扭曲，影响下游定量分析和诊断安全性。

Method: 提出CAHAL框架，采用确定性双变量混合专家架构，通过专门的残差3D U-Net专家路由每个输入，这些专家以体积分辨率和采集各向异性为条件。使用复合损失函数优化，包括边缘惩罚空间重建、傅里叶域频谱一致性匹配和分割引导的语义一致性约束。训练对通过基于物理的退化从大规模真实世界数据库中采样生成。

Result: 在T1加权和FLAIR序列上验证，CAHAL相比生成式基线方法取得了最先进的结果，在准确性和效率方面改进了最佳相关方法。

Conclusion: CAHAL提供了一个幻觉鲁棒、物理信息驱动的分辨率增强框架，能够直接在临床MRI采集空间操作，避免解剖幻觉和结构扭曲，从而提升下游定量分析的可靠性和诊断安全性。
...
TL;DR: CrossPan基准测试揭示胰腺MRI分割中跨序列域偏移比跨中心变异性更严重，现有域泛化方法效果有限，而MedSAM2等基础模型通过形状先验保持中等零样本性能。


<details>
  <summary>Details</summary>
Motivation: 胰腺自动分割对腹部MRI分析至关重要，但针对特定MRI序列训练的深度学习模型在应用到其他序列时性能会严重下降，这一问题缺乏系统研究。

Method: 引入CrossPan多机构基准数据集，包含1,386个3D扫描，涵盖T1加权、T2加权和Out-of-Phase三种常规序列，来自八个中心。评估跨序列域偏移的严重程度，比较现有域泛化方法、基础模型和半监督学习的效果。

Result: 1) 跨序列域偏移远严重于跨中心变异性：域内Dice分数>0.85的模型在跨序列时崩溃至接近零(<0.02)；2) 最先进的域泛化方法在物理驱动的对比度反转下效果有限，而MedSAM2等基础模型通过对比度不变的形状先验保持中等零样本性能；3) 半监督学习仅在稳定强度分布下有效，在高器官内变异性序列中不稳定。
...
TL;DR: Ghost-100基准测试评估视觉语言模型在渐进式强制提示下的幻觉行为，发现不同模型家族对提示压力的响应模式存在显著差异


<details>
  <summary>Details</summary>
Motivation: 现有幻觉基准主要依赖中性提示和二元检测，无法评估模型在渐进式强制提示下的幻觉行为变化，特别是在不同任务类型中幻觉发生率和强度的响应模式

Method: 构建Ghost-100基准，包含800张合成图像，涵盖三个任务家族（文本不可读、时间读取、物体缺失），每个图像配以5个不同强度的提示。采用双轨评估协议：基于规则的H-Rate和GPT-4o-mini评分的H-Score

Result: 评估9个开源VLM发现：H-Rate和H-Score在不同模型家族间显著分离；阅读式和存在检测子集对提示压力的响应方式不同；多个模型在中等提示强度下表现出非单调敏感性峰值

Conclusion: Ghost-100基准揭示了VLM在强制提示下的复杂幻觉行为模式，这些模式被聚合指标所掩盖，强调了需要更细粒度的评估方法来理解模型在现实部署中的可靠性
...
TL;DR: 该论文提出黎曼几何框架分析潜在扩散模型的不稳定性，发现几何解耦现象：正常生成时曲率编码图像细节，但OOD生成时极端曲率浪费在不稳定的语义边界而非可感知细节上，识别出"几何热点"作为不稳定的结构根源。
...
Method: 引入黎曼几何框架，通过分析生成雅可比矩阵，将几何分解为局部缩放（容量）和局部复杂度（曲率）。研究几何解耦现象，识别几何热点作为不稳定的结构根源。

Result: 研究发现几何解耦现象：正常生成时曲率功能性地编码图像细节，但OOD生成时出现功能解耦，极端曲率浪费在不稳定的语义边界而非可感知细节上。几何热点被识别为生成不稳定的结构根源。

Conclusion: 几何解耦和几何热点的发现为诊断生成可靠性提供了鲁棒的内在度量标准，揭示了潜在扩散模型不稳定性的几何根源，为改进模型稳定性提供了理论基础。
...
TL;DR: 提出DUALVISION模块，通过轻量级融合将红外图像信息整合到多模态大语言模型中，增强模型在视觉退化条件下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在RGB图像上表现良好，但在雾、模糊、低光等常见退化条件下表现脆弱。红外成像在这些条件下具有固有鲁棒性，但如何将其整合到MLLMs中尚未充分探索

Method: 提出DUALVISION轻量级融合模块，通过patch级别的局部交叉注意力机制高效整合IR-RGB信息；构建DV-204K数据集（约25K对齐的IR-RGB图像对，含204K模态特定QA标注）和DV-500基准（500个IR-RGB图像对，500个QA对用于评估跨模态推理）

Result: DUALVISION在多种视觉退化条件下表现出强大的实证性能；对开源和闭源MLLMs进行了基准测试，验证了方法的有效性

Conclusion: DUALVISION成功将红外图像信息整合到MLLMs中，显著提升了模型在视觉退化条件下的鲁棒性；提供了新的数据集和基准，为未来研究奠定了基础
...
TL;DR: 研究探索了将视觉基础模型（VFMs）通过2D到3D蒸馏方法应用于室内激光雷达扫描的语义分割，无需人工标注即可训练模型。


<details>
  <summary>Details</summary>
Motivation: 室内激光雷达扫描的逐帧语义分割对于3D场景理解和地图应用至关重要，但获取训练深度学习模型所需的逐帧地面真值成本高昂且耗时。视觉基础模型在图像分割方面表现出色，但尚未在室内场景中验证其通过蒸馏方法训练激光雷达分割模型的可行性。

Method: 采用2D到3D蒸馏管道，将每个激光雷达扫描与经过视觉基础模型处理的相机图像配对，通过跨模态蒸馏训练激光雷达扫描分割模型。使用室内SLAM数据集进行评估，其中伪标签用于下游评估，并提供了小型手动标注的激光雷达数据集进行验证。

Result: 蒸馏模型在伪标签评估下达到最高56%的mIoU，在真实标签评估下达到约36%的mIoU，证明了无需人工标注即可实现室内激光雷达语义分割的跨模态蒸馏可行性。

Conclusion: 研究表明，通过视觉基础模型的2D到3D蒸馏方法，可以在无需人工标注的情况下实现室内激光雷达扫描的语义分割，为室内场景的3D理解提供了可行的无监督学习途径。
...
TL;DR: GEM框架通过全局调度器替代学习型路由器，解决MoE模型中负载均衡与领域专业化之间的冲突，在跨域学习任务中实现更好的专业化和性能提升。


<details>
  <summary>Details</summary>
Motivation: 人类感知具有良好的跨域泛化能力，但大多数视觉模型在训练数据之外表现不佳。多数据集学习旨在通过训练多样化数据来提高模型在域偏移下的鲁棒性，但数据分布和标签语义的不一致性使得统一训练具有挑战性。现有的MoE模型虽然通过专家路由提供可扩展解决方案，但其负载均衡机制强制专家间均匀分配输入，这与领域感知路由相冲突，导致专家学习冗余表示，在稀有或分布外域上性能下降。

Method: 提出GEM（全局专家映射）框架，包含规划器和编译器两部分。规划器基于线性规划松弛计算数据集到专家的分数分配，编译器通过分层舍入将软计划转换为确定性的容量感知映射。该框架替代了学习型路由器，避免了平衡损失，解决了公平性与专业化之间的冲突。

Result: GEM-DINO在UODB基准测试中实现了最先进的性能，在代表性不足的数据集上获得显著提升，并解决了少样本适应场景中的任务干扰问题。

Conclusion: GEM框架通过全局调度策略有效解决了MoE模型中负载均衡与领域专业化之间的根本冲突，实现了更好的专家专业化、可解释路由和跨域性能提升，为多数据集学习提供了更有效的解决方案。
...
TL;DR: 提出ConvVitMamba统一混合框架，通过整合多尺度卷积、Vision Transformer和轻量级Mamba模块，实现高效高光谱图像分类，在精度、模型大小和推理效率间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类面临光谱维度高、冗余性强、标记数据有限等挑战。现有CNN和ViT方法虽然性能强，但计算成本高、模型大，限制了实际应用。需要设计更高效的解决方案。

Method: 提出ConvVitMamba统一混合框架：1) 多尺度卷积特征提取器捕获局部光谱、空间和联合模式；2) Vision Transformer进行token化和编码，建模全局上下文关系；3) 轻量级Mamba门控序列混合模块，实现无需二次自注意力的内容感知细化；4) 使用PCA预处理减少冗余。

Result: 在四个基准数据集（Houston和三个UAV borne QUH数据集）上的实验表明，ConvVitMamba在CNN、Transformer和Mamba方法中表现最佳，同时在精度、模型大小和推理效率间保持良好平衡。消融研究证实了各组件的互补贡献。

Conclusion: ConvVitMamba框架为不同场景下的高光谱图像分类提供了有效且高效的解决方案，在保持高性能的同时显著降低了计算成本和模型复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 航空影像目标检测面临空间分辨率、场景构成和语义标签覆盖的差异挑战，传统模型难以在不同数据集间泛化，统一的表示方法在区域特定内容和新颖类别上表现不佳。
...
TL;DR: 提出基于层次嵌入和对抗对齐的视觉语言模型鲁棒微调框架，通过多层级对抗对齐和嵌入深度控制增强对抗鲁棒性，同时保持自然性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在零样本分类中容易受到对抗攻击，现有鲁棒微调方法通过固定文本嵌入与图像嵌入对齐，牺牲了自然性能和鲁棒性。当模型面临针对超类（如哺乳动物）和基类（如猫）的对抗攻击时，鲁棒性会进一步下降。需要利用类别空间的层次特性来增强对抗鲁棒性。

Method: 提出基于层次嵌入的对抗微调框架：1）使用多层级对抗对齐图像-文本模态；2）通过额外机制将视觉嵌入放置在层次结构的期望深度；3）提供嵌入深度与最大可行边界大小的理论联系；4）模型自然实现多种边界大小，增强对抗样本的泛化；5）考虑在多个共享相同叶标签的树上进行对齐，提升语义多样性。

Result: 在多个数据集上进行实验验证，结果表明该方法能有效提升对抗鲁棒性。

Conclusion: 通过层次嵌入和多层级对抗对齐的框架，能够增强视觉语言模型的对抗鲁棒性，同时保持自然性能，并利用类别空间的层次特性来提升模型的整体鲁棒性。
...
TL;DR: 提出一个自动化管道，将Cellpose-SAM适配于微观结构分析，结合拓扑感知梯度跟踪和ASTM E112 Jeffries平面测量模块，用于密集实例分割和晶粒尺寸估计，在少样本情况下表现出色。


<details>
  <summary>Details</summary>
Motivation: 从显微镜图像中提取标准化的冶金指标具有挑战性，因为复杂的晶粒形态和监督分割的数据需求。需要将基础计算机视觉技术与实际冶金评估相结合。

Method: 提出自动化管道，将Cellpose-SAM适配于微观结构分析，集成其拓扑感知梯度跟踪与ASTM E112 Jeffries平面测量模块。系统性地与U-Net、MatSAM和Qwen2.5-VL-7B进行基准测试。

Result: 提出的管道成功保持拓扑分离，而现成的视觉语言模型在密集微观计数方面表现不佳，MatSAM存在过分割问题。在仅使用2个训练样本时，预测ASTM晶粒尺寸数(G)的平均绝对百分比误差低至1.50%。

Conclusion: 应用级基础模型集成对于高精度自动化材料表征具有高效性，验证了ASTM 50晶粒采样的最低要求，展示了少样本可扩展性。
...
TL;DR: CXRMate-2模型在胸片报告生成任务中表现优异，在多项指标上超越基准模型，并在45%的病例中被放射科医生评为可接受，显示出临床应用的潜力。


<details>
  <summary>Details</summary>
Motivation: 尽管胸片放射报告生成模型发展迅速，但其临床实用性仍不确定，主要原因是缺乏放射科医生的专业评估。本研究旨在开发一个先进的胸片报告生成模型，并通过放射科医生的盲法评估来验证其临床可用性。

Method: 开发了CXRMate-2模型，整合了结构化多模态条件处理和强化学习，使用复合奖励机制确保与放射科医生报告的语义对齐。在MIMIC-CXR、CheXpert Plus和ReXgradient数据集上进行评估。同时进行了盲法随机回顾性定性评估，由三名放射科顾问医生对120个病例的生成报告和放射科医生报告进行比较。

Result: CXRMate-2在多个数据集上显著超越基准模型，在MIMIC-CXR上相对于MedGemma 1.5 (4B)分别提升了11.2%的GREEN分数和24.4%的RadGraph-XL分数。在放射科医生评估中，45%的生成报告被评为可接受（优于或等同于放射科医生报告）。在8个分析发现中，有7个没有显著偏好差异。放射科医生报告因更高的召回率而受青睐，而生成报告在可读性方面更受偏好。

Conclusion: 研究结果表明胸片报告生成模型有可信的临床应用路径。通过提高召回率和更好地检测细微发现（如肺淤血），可能达到与放射科医生报告的非劣效性。这些有针对性的改进使得胸片报告生成系统有望在放射科医生主导的工作流程中进行前瞻性评估，发挥辅助作用。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在减少高斯-瓦片对时未考虑不同高斯-瓦片对的重要性差异。作者观察到远离高斯中心的边缘瓦片对像素颜色累积贡献很小，这为基于颜色贡献减少高斯-瓦片对提供了机会。
...
Conclusion: AdaGScale通过自适应缩放高斯大小来减少高斯-瓦片对数量，在显著提升渲染速度的同时保持了图像质量，为3D高斯泼溅渲染提供了有效的优化方案。
...
TL;DR: 该论文提出了一种多智能体框架用于多模态共情响应生成，通过结构化推理和反思优化来增强共情能力，相比传统单次生成方法有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有多模态共情响应生成方法通常采用从多模态上下文到最终响应的隐式单次生成范式，忽视了两个内在特性：1）人类情感感知本质上是结构化的而非直接映射；2）人类情感的复杂性和模糊性容易导致情感偏见和次优共情。

Method: 提出多智能体框架，包含结构化共情推理生成模块（分解为多模态感知、一致性情感预测、实用策略规划和策略引导响应生成）和全局反思优化模块（通过全局反思智能体进行逐步审计，消除情感偏见和共情错误，触发针对性重新生成）。

Result: 在IEMOCAP和MELD等多个基准测试中，该模型展现出优于现有最先进方法的多模态共情响应生成能力。

Conclusion: 通过结构化推理和反思优化的闭环框架，模型能够逐步提高情感感知准确性并消除情感偏见，显著提升多模态共情响应生成的质量。
...
TL;DR: ABot-Explorer提出了一种主动探索框架，将空间记忆构建与探索统一为在线RGB-only过程，利用视觉语言模型提取语义导航可操作性作为认知对齐锚点，显著提升探索效率和环境覆盖率。


<details>
  <summary>Details</summary>
Motivation: 当前空间记忆构建主要采用解耦的两阶段范式：先探索收集环境数据，再离线重建空间记忆。这种后处理且以几何为中心的方法无法利用高级语义智能，常常忽略导航关键地标（如门道、楼梯），而这些正是人类认知地图中的基本语义锚点。

Method: 提出ABot-Explorer主动探索框架，核心是利用大型视觉语言模型提取语义导航可操作性作为认知对齐锚点，动态集成到分层SG-Memo中，通过优先考虑结构转换节点来模仿人类探索逻辑，实现高效覆盖。

Result: 实验结果表明，ABot-Explorer在探索效率和环境覆盖率方面显著优于当前最先进方法，生成的SG-Memo能有效支持多种下游任务。贡献了扩展InteriorGS的大规模数据集，包含SNA和SG-Memo标注。

Conclusion: ABot-Explorer通过统一记忆构建与探索过程，利用语义智能引导探索，实现了更高效、更符合人类认知的空间记忆构建，为复杂具身导航任务中的长时程推理提供了更好的基础。
...
Conclusion: 通过充分利用预训练生成模型的图像先验，提出的生成式纹理过滤方法取得了显著性能提升，为纹理处理任务提供了新的有效解决方案
...
TL;DR: LPCVC 2025挑战赛设计、评估框架及优胜方案分析，为边缘设备高效视觉模型开发提供参考


<details>
  <summary>Details</summary>
Motivation: 推动面向边缘设备的高效视觉模型发展，在精度、延迟、内存和能耗之间取得平衡，促进实际应用部署

Method: 设计包含三个赛道的竞赛结构：1)不同光照和风格条件下的图像分类；2)基于文本提示的开放词汇分割；3)单目深度估计。采用Qualcomm AI Hub构建可复现的评估框架

Result: 介绍了各赛道的最佳解决方案，总结了关键趋势和观察结果，为高效视觉模型开发提供了实践参考

Conclusion: 提出了未来计算机视觉竞赛的发展建议，强调在边缘设备约束下平衡精度与效率的重要性
...
TL;DR: 提出SDB机制，通过平衡行为多样性和学习稳定性来改进视觉语言导航中的自我提升，在多个基准测试中取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 在视觉语言导航中，基于策略诱导经验的自我提升面临行为多样性与学习稳定性之间的平衡难题。增加行为多样性有助于探索替代行动假设，但会破坏学习信号的稳定性；而过于保守的稳定性约束则会抑制探索，导致早期固化，使可靠的自我提升变得困难。

Method: 提出稳定性-多样性平衡机制：1）通过可控的指令条件隐藏状态偏移，将每个决策步骤扩展为多个潜在行为假设；2）进行可靠性感知的软评估和聚合，在学习过程中保留多样且与指令一致的替代方案；3）使用显式正则化器约束假设间的交互，防止过度漂移或过早的多样性崩溃，稳定自我提升过程。

Result: 在R2R、SOON和REVERIE三个基准测试上均取得一致改进。例如在REVERIE val-unseen上，SPL从33.73提升到35.93，OSR从51.07提升到54.25。

Conclusion: SDB机制通过平衡行为多样性和学习稳定性，有效解决了视觉语言导航中自我提升的挑战，提供了一种即插即用的解决方案，显著提升了导航性能。
...
TL;DR: 本文提出了一种针对多模态测试时适应的概率高斯模型，通过显式建模类别条件分布和自适应对比不对称性校正技术，解决了多模态分布不对称问题，提升了分布偏移下的模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态测试时适应方法缺乏对类别条件分布的显式建模，这限制了准确预测和可靠决策边界的生成。传统的GDA方法在多模态场景中因模态分布不对称而效果有限。

Method: 提出定制的概率高斯模型来显式建模类别条件分布，并引入自适应对比不对称性校正技术来抵消模态不对称带来的负面影响。

Result: 在多种基准测试上的广泛实验表明，该方法在各种分布偏移下实现了最先进的性能。

Conclusion: 通过显式建模类别条件分布并校正模态不对称性，该方法能够生成校准的预测和可靠的决策边界，显著提升了多模态测试时适应在分布偏移下的性能。
...
TL;DR: EgoMotion：一种解决第一人称视觉-语言运动生成中推理-生成纠缠问题的分层生成框架，通过认知推理和运动生成两阶段分离，实现语义理解和运动质量的双重提升。


<details>
  <summary>Details</summary>
Motivation: 第一人称运动生成面临推理-生成纠缠挑战，同时优化语义推理和运动建模会导致梯度冲突，降低多模态对齐和运动质量。需要一种能分离认知推理和运动控制的方法。

Method: 提出EgoMotion分层框架：1）认知推理阶段：使用视觉语言模型将多模态输入映射到离散运动基元空间，获得目标一致的表示；2）运动生成阶段：以这些表示为条件，在连续潜在空间中进行扩散模型迭代去噪，生成物理合理的时间连贯轨迹。

Result: EgoMotion在广泛评估中达到最先进性能，生成的运动序列在语义对齐和运动学质量上都优于现有方法。

Conclusion: 通过分离认知推理和运动生成，EgoMotion有效解决了第一人称视觉-语言运动生成中的推理-生成纠缠问题，实现了语义理解和运动质量的双重优化。
...
TL;DR: PortraitDirector提出了一种分层组合式人脸重演框架，通过空间层（全局头部姿态+局部面部表情）和语义层（全局情感）的分解与重组，实现了高保真度与细粒度控制的平衡，并在单GPU上达到20FPS实时性能。


<details>
  <summary>Details</summary>
Motivation: 现有面部重演方法在表达性和细粒度可控性之间存在权衡：整体性方法牺牲了精细控制来获得表达性，而控制导向的方法则在保真度和鲁棒解耦方面存在困难。本文探索从组合视角处理面部运动，而非将其视为单一信号。

Method: 采用分层运动解耦与组合策略：1) 空间层：包含全局头部姿态（专用表示和注入路径）和局部面部表情（从裁剪面部区域提取，通过情感过滤模块去除情感线索）；2) 语义层：包含全局情感。通过扩散蒸馏、因果注意力和VAE加速等优化实现实时性能。

Result: PortraitDirector在单张5090 GPU上实现了512×512分辨率、20FPS的流式高保真可控面部重演，端到端延迟为800毫秒，在表达性和可控性方面取得平衡。

Conclusion: 通过将面部重演构建为分层组合任务，PortraitDirector框架成功解决了表达性与可控性之间的权衡问题，实现了高保真度、细粒度控制和实时性能的统一。
...
TL;DR: BALTIC基准测试系统评估3D重建方法在不同介质（空气/水）和光照条件下的性能，发现高斯溅射在纹理一致条件下性能接近专业水下方法，但在复杂真实环境中鲁棒性下降


<details>
  <summary>Details</summary>
Motivation: 解决机器人感知中跨介质（空气到水）3D重建的鲁棒性问题，特别是在不同环境和光照条件下的挑战

Method: 创建BALTIC基准测试，包含13个数据集覆盖两种介质和三种光照条件；使用定制水槽、单目相机和HTC Vive追踪器获取地面真实位姿；评估COLMAP的SfM重建，并作为NeRF和3D高斯溅射的输入；进行跨域重建和颜色恢复分析

Result: 在受控纹理一致条件下，经过简单预处理（如白平衡校正）的高斯溅射方法性能接近专门的水下方法；但在更复杂异构的真实环境中，其鲁棒性会下降

Conclusion: BALTIC基准为跨介质3D重建提供了系统评估框架，表明高斯溅射在受控条件下具有潜力，但在真实复杂环境中仍需改进鲁棒性
...
TL;DR: 该论文提出Patch Forcing框架，通过空间和时间上的补丁级噪声调度实现自适应图像生成，根据图像区域难度动态分配计算资源。


<details>
  <summary>Details</summary>
Motivation: 传统扩散和流模型对所有图像补丁采用统一的噪声调度，忽略了自然图像的异质性。某些区域容易去噪，而其他区域需要更多细化或上下文信息，因此需要更精细的补丁级噪声调度。

Method: 提出Patch Forcing框架：1）引入时间步采样器控制训练期间补丁级最大信息量；2）添加轻量级补丁难度头实现自适应采样；3）结合空间和时间上的噪声变化，让简单区域提前生成以提供上下文。

Result: 在类别条件ImageNet上取得优异结果，与表示对齐和引导方法正交，可扩展到文本到图像合成，证明补丁级去噪调度为自适应图像生成提供了有前景的基础。

Conclusion: 补丁级噪声调度能够根据图像区域难度动态分配计算资源，Patch Forcing框架通过空间和时间上的自适应调度显著提升图像生成质量，为更高效的图像生成方法开辟了新方向。
...
TL;DR: ST-Prune：一种用于自动驾驶视觉语言模型的免训练、即插即用时空令牌剪枝框架，通过运动感知时间剪枝和环视空间剪枝模块，在90%令牌减少下实现接近无损性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶系统中计算开销巨大，现有令牌剪枝方法主要针对单图像输入，无法利用驾驶场景中固有的时空冗余性。

Method: 提出ST-Prune框架，包含两个互补模块：1) 运动感知时间剪枝(MTP)：通过编码运动波动性和时间近度作为软约束，优先保留动态轨迹和当前帧内容；2) 环视空间剪枝(RSP)：利用环视相机几何惩罚双边跨视图相似性，消除重复投影和剩余背景。

Result: 在感知、预测和规划四个基准测试中验证，ST-Prune建立了免训练令牌剪枝的新SOTA。在90%令牌减少下，某些指标甚至超过完整模型基线，同时保持与现有剪枝方法相当的推理速度。

Conclusion: ST-Prune通过有效利用驾驶场景的时空冗余性，实现了高效的令牌剪枝，为自动驾驶视觉语言模型的部署提供了实用的解决方案。
...
TL;DR: 该论文提出了一种多尺度深度结构相似性框架（MSDS），通过在不同金字塔层级独立计算DeepSSIM并使用可学习的全局权重融合结果，显著提升了图像质量评估中深度特征相似性模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度特征的感知相似性模型大多在单一空间尺度上操作，隐含假设固定分辨率的结构相似性就足够了。空间尺度在深度特征相似性建模中的作用尚未得到充分理解，需要将其作为独立因素进行研究和验证。

Method: 提出了最小化的多尺度扩展框架MSDS，将深度特征表示与跨尺度集成解耦：1）在不同金字塔层级独立计算DeepSSIM；2）使用轻量级可学习的全局权重融合各层级得到的相似性分数。

Result: 在多个基准数据集上的实验表明，MSDS相比单尺度基线模型取得了持续且统计显著的性能提升，同时仅引入了可忽略的额外计算复杂度。

Conclusion: 实验结果从经验上证实了空间尺度是深度感知相似性中不可忽视的因素，通过最小化测试平台成功将其作为独立因素进行了分离和研究。
...
TL;DR: CLVG-Bench是一个评估视频模型零样本推理能力的框架，通过上下文学习视频生成来测试模型的多模态推理能力，发现当前SOTA模型在逻辑推理和交互生成任务上表现严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型评估基准存在局限性，任务设计简单且评估指标碎片化，无法真正衡量模型的多模态推理能力。需要建立一个更严谨的评估框架来回答：当前视频模型距离实现真正的多模态推理还有多远？

Method: 提出CLVG-Bench评估框架，包含1000多个高质量人工标注的元数据，涵盖6个类别47个子类别，覆盖物理模拟、逻辑推理和交互上下文等复杂场景。同时提出自适应视频评估器（AVE），使用最少标注与人类专家感知对齐，提供可解释的文本反馈。

Result: 实验显示，虽然SOTA视频模型（如Seedance 2.0）在某些理解和推理子任务上表现尚可，但在逻辑基础和交互生成任务上表现严重不足（成功率分别<25%和约0%），暴露了多模态推理和物理基础是关键瓶颈。

Conclusion: CLVG-Bench系统性地量化了当前视频模型的局限性，提供了可操作的反馈和明确的路线图，推动开发真正鲁棒的通用视频模型。该基准和代码已开源。
...
TL;DR: 本文提出了一种面向移动设备采集3D高斯溅射数据的物体中心化方法，通过设备姿态校准、实时球形覆盖度计算和采集引导，实现了更高质量的重建效果。


<details>
  <summary>Details</summary>
Motivation: 移动设备采集3D高斯溅射数据面临挑战，特别是在物体中心化场景中。需要解决采集过程中的姿态对齐、视角覆盖均匀性和实时引导问题，以提高重建质量。

Method: 1. 设备姿态校准：将设备方向对齐到基准坐标系以获得相对位姿；2. 球形网格映射：将相机光轴映射到物体中心球形网格进行统一视角索引；3. 实时覆盖度计算：计算面积加权的球形覆盖度以消除极坐标采样偏差；4. 用户运动引导：根据实时覆盖度指导用户移动设备进行数据采集。

Result: 与RealityScan和自由采集策略相比，本文方法使用更少的输入图像获得了更优的重建质量。进一步分析表明，该方法在物体中心化采集过程中能够获得更全面和均匀的视角覆盖。

Conclusion: 通过设备姿态校准、实时球形覆盖度计算和采集引导，本文提出的方法能够有效提升移动设备在物体中心化场景下的3D高斯溅射数据采集质量，为移动端3D重建提供了可靠的解决方案。
...
TL;DR: ReID-R提出了一种新颖的推理驱动范式，通过将思维链融入ReID流程，实现显式的身份理解和推理，仅用14.3K非平凡数据就达到了与现有方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 当前主流的感知驱动范式倾向于从大量标注数据中拟合，而非理解身份因果关系，导致表示在面对多种干扰时表现脆弱。需要一种能够实现显式身份理解和推理的方法。

Method: ReID-R采用两阶段方法：1) 判别性推理预热：以无CoT标签的方式训练模型，获得身份感知特征理解；2) 高效强化学习：提出非平凡采样构建场景泛化数据，利用高质量奖励信号引导模型关注ID相关线索。

Result: 在多个ReID基准测试中，ReID-R仅使用14.3K非平凡数据（现有数据规模的20.9%）就达到了与优秀方法竞争的身份判别性能。此外，得益于内在推理能力，ReID-R能够为结果提供高质量解释。

Conclusion: ReID-R作为一种推理驱动范式，通过思维链集成实现了显式身份理解和推理，在数据效率、泛化能力和可解释性方面表现出色，为ReID研究提供了新方向。
...
TL;DR: ASAHI是一种自适应切片辅助超推理框架，通过动态确定最佳切片数量来减少冗余计算，提高高分辨率航拍和卫星图像中小目标检测的效率和精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的物体检测器在高分辨率航拍和卫星图像的小目标检测中面临挑战，特别是密集物体分布、可变拍摄角度、微小目标尺寸和显著的类间差异。传统的固定尺寸切片策略虽然有效，但会引入大量冗余计算，增加推理成本并降低检测速度。

Method: ASAHI包含三个协同组件：1）自适应分辨率感知切片算法，基于学习阈值动态生成6或12个重叠补丁；2）切片辅助微调策略，构建包含全分辨率和切片图像补丁的增强训练数据；3）Cluster-DIoU-NMS后处理模块，结合Cluster-NMS的几何合并效率和DIoU-NMS的中心距离感知抑制，在拥挤场景中实现稳健的重复消除。

Result: 在VisDrone2019和xView数据集上的广泛实验表明，ASAHI在VisDrone2019-DET-val上达到56.8%，在xView-test上达到22.7%的state-of-the-art性能，同时相比基线SAHI方法减少20-25%的推理时间。

Conclusion: ASAHI通过自适应切片策略有效解决了高分辨率图像中小目标检测的效率和精度问题，在减少冗余计算的同时保持了相邻补丁之间的有益重叠，为航拍和卫星图像分析提供了更高效的解决方案。
...
TL;DR: OTCA提出了一种细粒度奖励分配框架，通过轨迹级信用分解和多目标信用分配，解决了GRPO在视觉生成模型后训练中的粗粒度奖励分配问题，显著提升了图像和视频生成质量。
...
Method: 提出目标感知轨迹信用分配（OTCA）框架，包含两个关键组件：1）轨迹级信用分解，估计不同去噪步骤的相对重要性；2）多目标信用分配，自适应地加权和组合整个去噪过程中的多个奖励信号。
...
TL;DR: AlloSR²：通过同态生成流校正单步超分辨率轨迹的新框架，解决先验崩溃问题，在保持生成真实感的同时实现高效单步推理


<details>
  <summary>Details</summary>
Motivation: 现实世界图像超分辨率（Real-SR）面临两个关键问题：1）在有限的LR-HR对上进行微调会导致"先验崩溃"，模型牺牲其固有的生成丰富性来过度拟合特定的训练退化；2）单步生成中缺乏多步细化会导致显著的轨迹漂移和伪影生成

Method: 提出AlloSR²框架，包含三个核心技术：1）信噪比引导轨迹初始化，通过将LR潜在特征的退化水平与预训练流的最优锚定时间步对齐来建立物理基础起始状态；2）流锚定轨迹一致性，在中间状态强制执行速度级监督以确保单步推理的稳定、无曲率路径；3）同态轨迹匹配，一种自对抗对齐策略，在统一向量场中最小化SR流和生成流之间的分布差异

Result: 在合成和现实世界基准测试上的广泛实验表明，AlloSR²在单步Real-SR中实现了最先进的性能，在恢复保真度和生成真实感之间提供了优越的平衡，同时保持了极高的效率

Conclusion: AlloSR²通过同态生成流校正单步超分辨率轨迹，有效解决了先验崩溃和轨迹漂移问题，为现实世界图像超分辨率提供了一种既保持生成真实感又高效的单步推理解决方案
...
TL;DR: 该论文提出了Deepening Reasoning MMSearchAgent框架，通过从整个批次轨迹中提取优势信号和使用差异化高斯奖励来解决多模态智能体过早交互崩溃的问题，在FVQA-test上性能提升8.4%。


<details>
  <summary>Details</summary>
Motivation: 多模态智能体在使用外部工具处理复杂任务时经常出现过早交互崩溃，主要原因有两个：1）终端奖励通常附加在最后一个token上，使得优势函数难以区分包含探索行为的轨迹；2）过度冗余的上下文阻碍了智能体吸收有用的反馈信息。
...
Conclusion: 该研究通过改进奖励设计和减少上下文冗余，有效解决了多模态智能体的过早交互崩溃问题，显著提升了智能体的推理能力和性能。
...
<details>
  <summary>Details</summary>
Motivation: 盲图像去模糊是一个具有实际重要性和内在难度的挑战性问题，需要同时估计点扩散函数和潜在清晰图像。虽然ℓ0范数正则化能促进稀疏性并更好地保持边缘和细节，但其非凸性和计算复杂性限制了实际应用。

Method: 采用极小极大凹惩罚(MCP)来增强稀疏性并更接近ℓ0范数近似，同时结合重加权ℓ1范数正则化来减少估计偏差并改善图像细节和纹理的保持。开发了相应的数值算法来求解优化问题。

Result: 通过多个测试图像的实验评估，证明了所提出方法的有效性。

Conclusion: 提出的基于MCP惩罚和重加权ℓ1正则化的方法能够有效解决盲图像去模糊问题，在保持图像边缘和细节方面表现良好，克服了传统ℓ0范数方法的计算困难。
...
TL;DR: 提出基于Transformer的多视角人群跟踪模型MVTrackTrans，并收集了两个大规模真实世界多视角跟踪数据集MVCrowdTrack和CityTrack，解决了现有数据集场景小、帧数少的问题。
...
Result: 在两个新的大型数据集MVCrowdTrack和CityTrack上，MVTrackTrans模型相比现有方法取得了更好的性能，证明了该模型在处理大场景方面的优势。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个轻量级视觉语言模型，专门针对自主设备的本地和边缘部署需求，特别关注日语环境下的视觉问答和视觉定位能力，解决工厂任务分析和基础设施异常检测等实际工业应用问题。

Method: 开发了8B和2B两种变体的轻量级视觉语言模型，构建了大规模合成数据生成管道和全面的日语训练评估资源，以视觉问答和视觉定位为核心能力，针对工厂工具识别和基础设施异常检测两个应用场景进行专门开发。

Result: 在日语和英语基准测试中优于同类开源模型，在JA-VG-VQA-500上获得61.5 ROUGE-L，在日语Ref-L4上达到85.2%准确率。在工厂任务分析中实现53.9%的零样本准确率，在电厂数据上微调后，异常检测的bbox+标签F1分数从39.7提升至64.9。

Conclusion: PLaMo 2.1-VL成功开发为一个适用于自主设备边缘部署的轻量级视觉语言模型，在日语视觉语言任务和实际工业应用场景中表现出色，为工厂自动化和基础设施监控提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有单目场景流估计方法大多局限于两帧输入，限制了时序建模能力以及对遮挡的鲁棒性。需要开发能够有效利用多帧时序信息的方法。

Method: 提出几何-运动特征（GMF）紧凑编码耦合的运动和几何线索，并通过迭代更新进行有效时序推理。引入相对位置注意力注入空间先验，以及遮挡正则化模块从可见区域传播可靠运动。
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers在医学影像诊断中表现有限，主要因为：1）高分辨率医学图像产生过多token，softmax注意力难以定位微小异常；2）医学图像分类是细粒度任务，标准交叉熵训练不足

Method: 提出三组件框架：1）基于目标检测模型的ROI token缩减，引导注意力到关键区域；2）ROI间对比学习，通过困难负样本增强细粒度区分能力；3）使用DINOv2预训练的ViT，替代全局CLIP表示，捕获定位感知的细粒度特征

Result: 在公开乳腺X光数据集上的实验表明，该方法优于现有基线，证明了其有效性和大规模乳腺癌筛查的临床潜力

Conclusion: 该框架成功解决了ViT在医学影像诊断中的局限性，通过注意力引导和细粒度学习策略，显著提升了乳腺癌检测性能，具有临床应用价值
...
TL;DR: 该研究提出了一种基于空间一致性检查的方法来检测T恤攻击，这种攻击能威胁人脸识别系统的安全。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统易受呈现攻击，现有攻击检测方法难以泛化到新型攻击类型。T恤攻击作为一种新型呈现攻击手段，能够威胁人脸识别系统的安全性。

Method: 结合最先进的人脸检测器和人体检测器，通过分析检测到的人脸和人体空间位置的一致性来识别T恤攻击。当人脸检测结果与人体检测结果在空间上不一致时，可判定为T恤攻击。

Result: 使用TFPA数据库中的1,608个T恤攻击样本和152个真实呈现样本进行综合评估，证明该方法能够可靠地检测T恤攻击，有效提升人脸识别系统的安全性。

Conclusion: 提出的基于空间一致性检查的检测方法能够有效识别T恤攻击，为解决新型呈现攻击检测的泛化问题提供了可行方案。
...
TL;DR: 该研究提出并评估了一个基于EEG的驾驶员意图预测框架，在真实道路条件下使用深度学习模型，TSCeption模型表现最佳，准确率达90.7%，能提前1000毫秒稳定预测驾驶意图。


<details>
  <summary>Details</summary>
Motivation: 从神经生理信号预测驾驶员意图对提升先进驾驶辅助系统的主动安全性很有前景，但在真实驾驶环境中面临EEG信号非平稳性和认知-运动准备复杂性的挑战。

Method: 研究提出基于EEG的驾驶员意图预测框架，使用同步多传感器平台集成到真实电动汽车中，收集了32个驾驶会话的真实道路数据集，在一致实验条件下评估了12种深度学习架构。

Result: TSCeption架构获得最高平均准确率（0.907）和Macro-F1分数（0.901）；框架展示强时间稳定性，能在动作执行前1000毫秒保持稳健解码性能；最小化EEG预处理优于伪迹处理流程；预测性能在400-600毫秒区间达到峰值。

Conclusion: 研究支持在真实道路条件下早期稳定解码EEG驾驶员意图的可行性，为基于神经生理信号的主动安全系统开发提供了实证基础。
...
<details>
  <summary>Details</summary>
Motivation: 现实自动驾驶中常遇到域偏移问题（如光照、天气变化），现有监督式多模态3D全景分割方法过度依赖LiDAR和RGB的强互补性，在单模态退化时表现脆弱。传统伪标签方法仅保留高置信度区域，导致掩码碎片化和不完整监督，对全景分割尤为不利。

Method: 提出PanDA框架：1）非对称多模态增强，选择性丢弃区域模拟域偏移，提升鲁棒表示学习；2）双专家伪标签优化模块，从2D和3D模态提取域不变先验，增强伪标签完整性和可靠性。

Result: 在涵盖时间、天气、地点和传感器变化的多种域偏移场景中，实验结果显示显著超越现有3D语义分割的SOTA无监督域适应基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 卫星图像合成在遥感应用中至关重要，如数据增强、灾害模拟和城市规划。现有方法在处理不同域条件下的合成图像时存在协调效果不佳的问题，需要一种能够有效协调合成卫星图像的方法。

Method: 提出训练免费的扩散模型框架HarmoniDiff-RS：1）通过潜在均值偏移操作在源域和目标域之间传递辐射特征；2）采用时序潜在融合策略，利用早期反转潜在实现高协调性，晚期潜在保持语义一致性；3）训练轻量级协调分类器自动选择最一致的结果；4）构建RSIC-H基准数据集。

Result: 实验表明该方法能有效执行卫星图像合成，在遥感合成和模拟任务中显示出强大潜力。构建的RSIC-H数据集包含500个配对合成样本。

Conclusion: HarmoniDiff-RS是一个有效的训练免费扩散模型框架，能够协调不同域条件下的合成卫星图像，为可扩展的遥感合成和模拟任务提供了有力工具。
...
TL;DR: VecHeart是一个统一框架，用于全心脏四腔室结构的重建与生成，通过混合部件Transformer和解剖补全策略处理不完整观测，并能扩展到3D+时间动态网格序列生成。


<details>
  <summary>Details</summary>
Motivation: 现有前馈隐式方法存在两个主要局限：1) 仅限于单对象建模，无法处理多结构心脏解剖；2) 忽视部件间相关性。心脏解剖建模需要处理结构间复杂相互关系。

Method: 提出混合部件Transformer，利用部件特定可学习查询和交错注意力捕获腔室间复杂依赖关系；引入解剖补全掩蔽和模态对齐策略，使模型能从部分、稀疏或噪声观测中推断完整四腔室结构。

Result: 实验表明该方法达到最先进性能，在多样化挑战性场景下保持高保真重建，并能无缝扩展到3D+时间动态网格序列生成。

Conclusion: VecHeart是一个多功能框架，能够处理完整心脏解剖建模，克服了现有方法的局限性，在心脏结构重建和生成任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型已成为图像编辑的主流方法，但如何将人类反馈强化学习（RLHF）有效应用于扩散编辑仍面临挑战，主要缺乏可扩展的人类偏好数据集和针对多样化编辑需求的框架。

Method: 提出HP-Edit后训练框架：1）构建RealPref-50K真实世界数据集，涵盖8个常见任务；2）利用少量人类偏好评分数据和预训练视觉大语言模型开发HP-Scorer自动评估器；3）使用HP-Scorer构建可扩展偏好数据集并作为奖励函数对编辑模型进行后训练；4）建立RealPref-Bench评估基准。

Result: 实验表明，该方法显著提升了如Qwen-Image-Edit-2509等模型的性能，使其输出更符合人类偏好。

Conclusion: HP-Edit框架通过结合人类偏好数据和自动评估器，成功解决了扩散模型在图像编辑中的人类偏好对齐问题，为真实世界编辑任务提供了有效的解决方案。
...
TL;DR: GOLD-BEV：利用时间同步的航拍图像作为训练监督，从自车传感器学习密集鸟瞰图语义环境地图的框架，包括动态交通参与者。


<details>
  <summary>Details</summary>
Motivation: 需要在几何一致、场景中心的表示中理解道路场景，这对于规划和地图构建至关重要。传统方法依赖自车视角的BEV标注存在模糊性，且手动标注成本高。

Method: 1. 使用时间同步的航拍图像作为训练监督；2. BEV对齐的航拍裁剪提供直观目标空间，实现密集语义标注；3. 通过领域适应的航拍教师生成BEV伪标签；4. 联合训练BEV分割和可选的伪航拍BEV重建；5. 从自车传感器学习合成伪航拍BEV图像，支持轻量级人工标注和不确定性感知的伪标注。

Result: 框架能够学习密集的BEV语义环境地图，包括动态交通参与者，仅需在训练时使用时间同步的航拍图像监督。通过伪航拍BEV图像合成，可扩展到无航拍覆盖的区域。

Conclusion: GOLD-BEV提供了一种利用航拍图像监督学习BEV语义地图的有效方法，解决了传统自车视角BEV标注的模糊性问题，并通过伪航拍合成实现了可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型经常出现物体幻觉问题，即在图像描述中生成实际不存在的物体。这在医疗影像和自动驾驶等关键应用中尤其危险。研究表明幻觉问题可能源于语言先验偏差，即模型基于训练数据的统计共现生成词汇。

Method: 提出视觉对比编辑（VCE）方法，通过分析模型对对比性视觉扰动的响应来识别和抑制幻觉倾向。使用奇异值分解（SVD）分解模型激活模式，隔离幻觉子空间，并应用有针对性的参数编辑来减弱其影响。

Result: 实验结果表明，VCE在多个基准测试中有效减少了物体幻觉，同时保持了模型原有的计算效率。

Conclusion: VCE作为一种无需标注的后处理方法，既具有可扩展性，又适合在资源受限的环境中部署，为解决大型视觉语言模型的物体幻觉问题提供了实用解决方案。
...
TL;DR: LoViF挑战赛综述：针对真实世界多退化图像修复的竞赛报告，涵盖模糊、低光、雾霾、雨雪等多种退化类型，旨在评估模型在统一框架下的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 推动真实世界多合一图像修复研究，为多种退化条件（模糊、低光、雾霾、雨雪）下的图像修复提供统一的基准测试平台，评估模型在复杂真实场景中的鲁棒性和泛化能力。

Method: 通过举办LoViF挑战赛，收集124名参赛者的9份有效最终提交，提供统一基准框架，分析参赛方法在多种退化类型下的表现，建立综合评价体系。

Result: 挑战赛成功吸引了广泛参与，获得了9份有效提交，为真实世界多合一图像修复领域建立了重要基准，推动了该领域的研究进展。

Conclusion: LoViF挑战赛为真实世界多退化图像修复提供了有价值的基准测试平台，分析了当前有效方法，为未来低层视觉研究建立了重要参考框架。
...
TL;DR: 提出TS-Attn注意力机制解决多动作视频生成中的时间对齐和注意力耦合问题，无需训练即可提升现有模型性能


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理包含多个顺序动作的复杂时间描述时面临两难：使用多个短提示能提高动作保真度但损害时间一致性，而使用单个复杂提示能保持一致性但降低提示跟随能力。主要问题源于时间不对齐和注意力耦合冲突。

Method: 提出无需训练的时间感知可分离注意力机制（TS-Attn），动态重新分配注意力分布，确保多事件场景中的时间感知和全局连贯性，可无缝集成到各种预训练文本到视频模型中。

Result: 在Wan2.1-T2V-14B和Wan2.2-T2V-A14B模型上，仅增加2%推理时间就将StoryEval-Bench分数分别提升33.5%和16.4%，同时支持跨模型的多事件图像到视频生成的即插即用。

Conclusion: TS-Attn机制有效解决了多动作视频生成中的时间对齐和注意力耦合问题，显著提升了现有模型的性能，具有广泛的适用性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在图像分割方面取得稳定进展，扩散算法生成越来越逼真的图像，但在图像集合中识别重复模式这一看似简单的问题仍然非常开放。精灵基图像分解模型在聚类和图像分解方面显示出潜力，因其高可解释性而具有吸引力，但现有模型需要针对特定数据集定制，且难以扩展到包含多个对象的图像。

Method: 深入研究精灵基图像分解模型的设计细节，识别其核心组件，在聚类基准上进行广泛分析。基于此分析提出一种深度精灵基图像分解方法，该方法在标准CLEVR基准测试中与最先进的无监督类感知图像分割方法性能相当，能够线性扩展处理多对象图像，明确识别对象类别，并以高度可解释的方式建模图像。

Result: 提出的深度精灵基图像分解方法在标准CLEVR基准测试中与最先进的无监督类感知图像分割方法性能相当，能够线性扩展处理多对象图像，明确识别对象类别，并以高度可解释的方式建模图像。

Conclusion: 通过深入分析精灵基图像分解模型的设计并识别其核心组件，成功开发了一种性能优越的深度精灵基图像分解方法，解决了现有模型需要定制化、难以扩展的问题，为图像集合中的重复模式识别提供了高度可解释的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 评估专业机器学习模型和新兴多模态大语言模型在视觉政治传播分析中的能力，特别是在政治竞选期间社交媒体图像内容分析方面。
...
Result: GPT-4o在所有模型中表现最优，在故事分析中面部识别的宏观F1分数达到0.89，人物统计达到0.86，显著优于传统计算机视觉模型。
...
TL;DR: 本文评估了直方图匹配技术在葡萄病害检测中的应用，通过预处理归一化和数据增强的双阶段集成，显著提升了模型在真实冠层图像上的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 光照变化是限制深度学习在田间植物病害检测中鲁棒性的主要因素。本研究旨在解决葡萄病害分类中因光照变化导致的性能下降问题。

Method: 提出直方图匹配的双阶段集成方法：(1)作为预处理步骤进行图像归一化；(2)作为数据增强技术引入受控的训练变异性。使用1,469张RGB图像（包括均匀叶片聚焦和异质冠层样本）训练ResNet-18模型。

Result: 实验表明，该方法显著提升了模型在真实冠层图像上的鲁棒性。叶片聚焦样本显示边际增益，而冠层子集改善明显，表明平衡归一化与直方图多样化能有效弥合由非受控光照引起的域差距。

Conclusion: 直方图匹配的双阶段集成方法能有效缓解光照变化对深度学习植物病害检测的影响，特别是在处理真实冠层图像时表现出显著优势。
...
TL;DR: 提出主动映射移动物体的新任务，开发无需学习的Paparazzo方法，通过预测目标轨迹和选择最优视角来规划路径，显著提升动态场景的3D重建质量


<details>
  <summary>Details</summary>
Motivation: 现有3D映射方法通常假设静态环境，无法准确捕捉和重建移动物体，这限制了动态场景的理解能力

Method: 提出Paparazzo方法：无需学习，通过鲁棒预测目标运动轨迹，识别最富信息的观测视角，并规划自身路径来主动映射移动物体

Result: 相比多个强基线方法，Paparazzo显著提高了3D重建的完整性和准确性，为动态场景理解迈出重要一步

Conclusion: 提出了主动映射移动物体的新任务和解决方案，开发了相应基准测试，展示了在动态场景理解方面的重要进展
...
TL;DR: EgoSelf是一个基于图记忆的个性化自我中心助手系统，通过构建用户交互记忆图并设计个性化学习任务来预测用户未来行为。


<details>
  <summary>Details</summary>
Motivation: 自我中心助手需要个性化服务，但不同用户有不同的习惯、偏好和日常行为，有效整合长期用户数据以实现个性化仍然是一个关键挑战。

Method: 提出EgoSelf系统，包括：1）基于过去观察构建图结构的交互记忆，捕捉交互事件和实体之间的时间和语义关系；2）设计个性化学习任务，将其表述为预测问题，从用户历史行为图中预测可能的未来交互。

Result: 大量实验证明了EgoSelf作为个性化自我中心助手的有效性。

Conclusion: EgoSelf通过图记忆和个性化预测任务，有效解决了自我中心助手的个性化挑战，代码已开源。
...
TL;DR: TransSplat通过将语言驱动的3D高斯泼溅编辑建模为多视角不平衡语义传输问题，建立2D编辑证据与3D高斯之间的语义对应关系，提升局部编辑精度和结构一致性。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS编辑方法主要关注提升视角一致性，但未能解决一个更根本的问题：编辑后的2D证据与3D高斯之间的语义对应关系。这限制了局部编辑的准确性和结构一致性。

Method: 提出TransSplat方法，将语言驱动的3DGS编辑建模为多视角不平衡语义传输问题。建立可见高斯与视角特定编辑原型之间的对应关系，显式表征2D编辑证据与3D高斯的语义关系。恢复跨视角共享的规范3D编辑场来指导统一的3D外观更新，并使用传输残差抑制非目标区域的错误编辑。

Result: 定性和定量结果表明，与现有专注于增强视角一致性的3D编辑方法相比，TransSplat在局部编辑精度和结构一致性方面实现了更优的性能。

Conclusion: 通过显式建模2D编辑证据与3D高斯之间的语义对应关系，TransSplat有效解决了现有方法忽略的根本问题，在语言驱动的3DGS编辑中实现了更好的局部控制和结构一致性。
...
<details>
  <summary>Details</summary>
Motivation: 传统摄影图像编辑需要用户具备足够的美学理解能力来提供适当的调整指令，但这种方法依赖明确的人类美学意图指导，而这些意图往往模糊、不完整或非专业用户难以表达。

Method: 提出一个紧密耦合的推理到生成过程：Image Critic模块进行图像质量理解并识别缺陷，Photographic Artist模块实现针对性编辑以增强图像吸引力。采用多阶段训练流程：基础预训练、推理引导的多编辑监督适应、协调的推理到生成强化学习。

Result: 实验表明SmartPhotoCrafter在自动摄影增强任务上优于现有生成模型，实现了照片级真实感结果，同时对调色指令表现出更高的色调敏感性。

Conclusion: SmartPhotoCrafter通过自动化的推理到生成框架，消除了对明确人类美学指令的依赖，为非专业用户提供了高质量的自动摄影图像编辑能力。
...
TL;DR: SSDM框架通过解耦全局地理空间表示，使用结构先验调制和全局语义注入两条路径，有效融合地理基础模型与高分辨率视觉特征，提升遥感制图精度


<details>
  <summary>Details</summary>
Motivation: 传统细粒度高分辨率遥感制图依赖局部视觉特征，导致跨域泛化能力差和大尺度地表覆盖预测碎片化。现有方法直接融合全局地理基础模型的高维隐式嵌入与高分辨率特征时，由于严重的语义-空间鸿沟，常引发特征干扰和空间结构退化

Method: 提出结构-语义解耦调制（SSDM）框架，将全局地理空间表示解耦为两条互补的跨模态注入路径：1）结构先验调制分支，将全局表示的宏观感受野先验引入高分辨率编码器的自注意力模块；2）全局语义注入分支，显式对齐整体上下文与深层高分辨率特征空间，通过跨模态集成直接补充全局语义

Result: 大量实验表明，该方法相比现有跨模态融合方法实现了最先进的性能。通过释放全局嵌入的潜力，SSDM在不同场景下持续提高高分辨率制图精度

Conclusion: SSDM为将地理空间基础模型集成到高分辨率视觉任务中提供了一个通用有效的范式，通过解耦调制策略解决了特征干扰和空间结构退化问题
...
TL;DR: 本文介绍了PC2Model基准数据集，用于点云到三维模型配准任务的训练和评估，结合模拟数据和真实扫描数据以解决实际应用中的稀疏性、噪声和遮挡等问题。


<details>
  <summary>Details</summary>
Motivation: 随着点云获取技术（如LiDAR和结构光扫描）的普及和深度学习的发展，研究重点转向下游任务，特别是点云到模型配准。然而，数据驱动方法在处理真实世界扫描中的稀疏性、噪声、杂乱和遮挡时表现不佳，限制了其性能。

Method: 提出了PC2Model基准数据集，采用混合设计：结合模拟点云（提供精确地面真值和受控条件）和部分真实世界扫描及其对应的3D模型（引入传感器和环境伪影）。该数据集由ICWG II/Ib领导开发。

Result: 创建了一个公开可用的数据集，支持经典和数据驱动方法的训练和评估，并能够系统分析模型从模拟到真实场景的可迁移性。数据集已在Zenodo平台公开。

Conclusion: PC2Model基准通过混合设计解决了点云配准中的实际挑战，为研究社区提供了支持跨领域鲁棒训练和评估的资源，促进了从模拟到真实世界场景的系统性模型迁移分析。
...
TL;DR: GRAFT是一种快速的人-场景交互重建方法，通过预测交互梯度来迭代优化人体网格，在保持优化方法精度的同时实现50倍加速


<details>
  <summary>Details</summary>
Motivation: 当前单图像3D人-场景交互重建存在权衡：优化方法准确但慢（~20秒），前馈方法快但缺乏显式交互推理，产生漂浮和穿透伪影

Method: GRAFT通过学习的人-场景交互先验预测交互梯度，使用几何探针编码交互状态到身体锚定标记，通过轻量级transformer递归更新人体网格并重新探测场景

Result: GRAFT比最先进的前馈方法提升交互质量达113%，匹配优化方法质量的同时运行时间降低50倍，在用户研究中64.8%被偏好

Conclusion: GRAFT成功将几何拟合摊销到快速前馈推理中，可作为端到端重建器或可转移的即插即用先验，显著提升人-场景交互重建的质量和效率
...
<details>
  <summary>Details</summary>
Motivation: 现有图形设计解析方法通常采用多阶段流水线，存在错误累积和可控性有限的问题，而大多数生成模型输出的栅格化结果缺乏明确的图层结构，限制了后续编辑

Method: 提出混合生成框架：使用视觉语言模型解析文本区域为文本渲染协议；使用支持RGBA的多分支扩散架构生成背景和贴纸图层；引入ParserReward并结合Group Relative Policy Optimization来对齐人类设计偏好

Result: 在两个具有挑战性的数据集（Parser-40K和Crello）上进行了广泛实验，在所有指标上平均提升了23.7%，性能优于现有方法

Conclusion: 该框架能够有效地将栅格化设计图像解析为可编辑的图层结构，解决了现有方法的局限性，为图形设计的下游编辑提供了更好的支持
...
TL;DR: CoInteract是一个用于合成人-物交互视频的端到端框架，通过Human-Aware MoE和双流训练机制解决了现有扩散模型在结构稳定性和物理合理性方面的不足。
...
Method: CoInteract基于Diffusion Transformer架构，引入两个核心设计：1）Human-Aware Mixture-of-Experts（MoE），通过空间监督路由将token分配到轻量级区域专家，提升细粒度结构保真度；2）空间结构化协同生成，采用双流训练范式同时建模RGB外观流和辅助的HOI结构流，注入交互几何先验。

Result: 实验结果表明，CoInteract在结构稳定性、逻辑一致性和交互真实性方面显著优于现有方法。

Conclusion: CoInteract通过创新的MoE架构和双流训练机制，有效解决了人-物交互视频合成中的结构稳定性和物理合理性问题，为实际应用提供了高质量的解决方案。
...
TL;DR: CoCo-SAM3通过解耦推理为类内增强和类间竞争，解决了SAM3在多类开放词汇场景中存在的证据尺度不统一和类内漂移问题，无需额外训练即可在多个基准上取得一致改进。
...
Method: CoCo-SAM3将推理过程显式解耦为类内增强和类间竞争两个阶段。首先，对齐和聚合来自同义提示的证据以增强概念一致性；然后，在统一可比较的尺度上进行类间竞争，实现对所有候选类别的像素级直接比较。

Result: 无需任何额外训练，CoCo-SAM3在八个开放词汇语义分割基准上取得了一致的性能改进。

Conclusion: 通过显式解耦类内增强和类间竞争，CoCo-SAM3有效解决了SAM3在多类开放词汇场景中的证据尺度不统一和类内漂移问题，稳定了多类推理并缓解了类间冲突。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学图像分割的生成式方法主要基于扩散模型，存在计算开销大（需要迭代采样）和架构限制（通常基于UNet）的问题。流匹配作为一种原理性框架，能够学习连续时间传输映射，提供高效确定性生成，但尚未在医学图像分割中得到充分探索

Method: 提出MedFlowSeg条件流匹配框架，将医学图像分割建模为学习从简单先验分布到目标分割分布的时间依赖向量场。开发双条件机制：1）双分支空间注意力模块注入多尺度结构信息；2）频率感知注意力模块通过差异感知融合和时间依赖调制建模空间与谱表示之间的跨域交互

Result: 在多种医学成像模态上进行了广泛实证验证，MedFlowSeg实现了最先进的性能，同时相比基于扩散的方法显著降低了计算成本

Conclusion: 流匹配作为理论基础扎实且计算高效的替代方案，在生成式医学图像分割中具有巨大潜力。MedFlowSeg框架在保持生成式建模表达能力的同时，实现了一步确定性推理
...
TL;DR: MMControl：一种用于联合音频-视频生成的多模态控制框架，支持视觉和声学控制信号的同时注入，实现身份一致视频和音色一致音频的生成。


<details>
  <summary>Details</summary>
Motivation: 现有的可控生成框架通常仅限于视频控制，限制了全面可控性并导致跨模态对齐不理想。需要解决联合音频-视频生成中的多模态控制问题。

Method: 提出双流条件注入机制，将视觉和声学控制信号（参考图像、参考音频、深度图、姿态序列）通过旁路分支注入到联合音频-视频扩散变换器中。引入模态特定指导缩放，允许在推理时独立动态调整每个视觉和声学条件的影响强度。

Result: 实验证明MMControl在联合音频-视频生成中实现了对角色身份、声音音色、身体姿态和场景布局的细粒度、可组合控制。

Conclusion: MMControl填补了联合音频-视频生成中多模态控制的空白，通过双流条件注入和模态特定指导缩放实现了全面的可控性。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像修复方法存在局限性：单步判别式映射通过期望学习缺乏精细细节，而生成式范式则面临多步采样效率低下和噪声残差耦合的问题。需要一种能够桥接这两种范式的统一框架。

Method: 提出IR-Flow方法：1）构建多级数据分布流，增强模型学习和适应不同退化水平的能力；2）提出累积速度场学习不同退化水平间的传输轨迹，引导中间状态向干净目标移动；3）引入多步一致性约束确保轨迹连贯性并提升少步修复性能；4）直接在退化图像域和干净图像域之间建立线性传输流。

Result: 在去雨、去噪和雨滴去除任务上的广泛评估表明，IR-Flow仅需少量采样步骤就能获得有竞争力的定量结果，提供了高效灵活且保持良好失真-感知平衡的框架。

Conclusion: IR-Flow通过Rectified Flow构建了判别式和生成式图像修复范式之间的桥梁，不仅实现快速推理，还提高了对分布外退化的适应性，为图像修复提供了高效统一的解决方案。
...
TL;DR: StepSTEM是一个研究生水平的STEM多模态推理基准，包含283个数学、物理、化学、生物和工程问题，通过严格的双模态互补性设计和步骤级评估框架来评估MLLMs的真实跨模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs评估基准存在模态冗余问题，允许单模态捷径，且主要关注最终答案准确性而忽视推理过程本身。STEM领域因其高度可验证性成为有价值的测试平台，需要更精细的跨模态推理评估方法。

Method: 1) 构建StepSTEM基准：通过严格筛选流程确保文本和视觉输入的严格互补性；2) 提出步骤级评估框架：支持纯文本思维链和交错图像-文本推理，使用动态规划将预测推理步骤与多个参考解决方案对齐。

Result: 实验表明当前MLLMs仍严重依赖文本推理，即使Gemini 3.1 Pro和Claude Opus 4.6也仅达到38.29%的准确率，显示出真正的跨模态STEM推理仍有很大提升空间。

Conclusion: StepSTEM为多模态推理的精细评估提供了有价值的基准，揭示了当前MLLMs在跨模态STEM推理方面的局限性，为未来模型发展指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 从图像序列中准确重建和跟踪动态人脸面临挑战，因为非刚性变形、表情变化和视角变化同时发生，导致几何和对应关系估计存在显著歧义。需要一种能够同时处理这些问题的统一方法。

Method: 提出规范面部点预测表示法，将每个像素分配到一个共享规范空间中的归一化面部坐标。使用基于transformer的模型联合预测深度和规范面部坐标，通过多视角几何数据进行训练，将非刚性变形映射到规范空间。

Result: 在图像和视频基准测试中展示了最先进的性能：对应误差降低约3倍，推理速度更快，深度精度提高16%。实现了时间一致的几何重建和可靠的对应关系。

Conclusion: 规范面部点预测是统一前馈4D面部重建的有效基础，能够同时实现准确的深度估计、时间稳定的重建、密集的3D几何和鲁棒的面部点跟踪。
...
TL;DR: SpanVLA是一个端到端的自动驾驶框架，结合了自回归推理和流匹配动作专家，通过高效桥接视觉语言模型的推理指导来规划轨迹，显著减少推理时间，并通过GRPO后训练方法提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作（VLA）模型在自动驾驶中存在两个主要问题：1）使用自回归生成框架导致动作生成延迟高；2）鲁棒性有限。需要一种既能利用世界知识和推理能力，又能高效生成动作并具备良好鲁棒性的解决方案。

Method: 1. 提出SpanVLA框架，集成自回归推理和流匹配动作专家；2. 通过高效桥接机制，利用VLM的视觉和推理指导，基于历史轨迹初始化条件使用流匹配策略规划未来轨迹；3. 提出基于GRPO的后训练方法，让VLA模型不仅能从正样本学习，还能学习避免典型负行为和恢复行为；4. 引入mReasoning数据集，专注于复杂推理场景和负样本-恢复样本。

Result: 在NAVSIM（v1和v2）数据集上的广泛实验表明，SpanVLA模型具有竞争性的性能。定性结果展示了模型在多样化场景中的规划性能和鲁棒性。

Conclusion: SpanVLA通过结合自回归推理和流匹配动作专家，有效解决了现有VLA模型的高延迟和鲁棒性有限问题，为自动驾驶提供了一个高效且鲁棒的端到端解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有条件医学图像生成方法在高维3D医学成像中难以平衡推理效率、患者特异性保真度和分布级合理性，需要一种能同时满足这些要求的框架。

Method: GDM通过吸引-排斥漂移最小化生成器推前分布与目标分布之间的差异，利用医学基础编码器构建多级特征库支持可靠的亲和度估计和漂移场计算，并采用共享输出空间的梯度协调策略优化竞争目标间的平衡。

Result: 在MRI-to-CT合成和稀疏视图CT重建任务中，GDM在解剖保真度、定量可靠性、感知真实性和推理效率方面优于GAN、流匹配、SDE等生成模型以及监督回归方法。

Conclusion: GDM为条件3D医学图像生成提供了一个实用有效的框架，能更好地平衡推理效率、患者特异性保真度和分布级合理性。
...
Result: CityRAG能够生成连贯的分钟级物理基础视频序列，在数千帧中保持天气和光照条件，实现闭环导航，并能通过复杂轨迹重建真实世界地理环境。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D重建对于从随意拍摄的场景建模至关重要，但现有方法通常只基于一到两个捕获帧进行条件生成，这限制了几何一致性，难以扩展到大型或多样化场景。

Method: 1) 构建持久全局场景记忆：通过前置捕获视图缓存支持长范围条件；2) 几何感知条件策略：通过显式3D几何记忆和几何驱动的捕获视图检索耦合生成与重建；3) 效率优化：结合4步扩散蒸馏和上下文窗口稀疏注意力降低二次复杂度。

Result: 实验表明该方法在非规则输入、大视角差距和长轨迹情况下都能实现鲁棒且可扩展的重建，相比现有方法有明显改进。

Conclusion: AnyRecon通过持久场景记忆和几何感知条件策略，成功解决了稀疏视图3D重建中的几何一致性和可扩展性问题，为大规模场景重建提供了有效解决方案。
...
### [79] [Smiling Regulates Emotion During Traumatic Recollection](https://arxiv.org/abs/2604.19019)
...
TL;DR: 研究分析了978名大屠杀幸存者视频证词中的微笑行为，开发了自动微笑检测模型，发现微笑与特定语义主题、叙事结构和时间句法相关，微笑在负面情感时期出现并能改善周围句子的情感轨迹，起到情绪调节作用。


<details>
  <summary>Details</summary>
Motivation: 研究大屠杀幸存者在视频证词中微笑的时间、地点和原因，探索微笑在创伤回忆中的情感调节和社会互动功能。

Method: 开发自动微笑检测模型（F1分数85%），使用两种微笑分类法标注检测到的微笑，对1,083,417个转录句子提取叙事特征，并从音频、眼动注视和文本转录三个模态获取情感效价。

Result: 微笑率与特定语义主题、叙事结构和时间句法显著相关；微笑常出现在强烈负面情感时期，这些负面情感微笑能显著改善周围句子的情感轨迹；微笑减少眼部动态和眨眼频率，这些效应的强度受叙事效价调节。
...
### [80] [On Solving the Multiple Variable Gapped Longest Common Subsequence Problem](https://arxiv.org/abs/2604.18645)
*Marko Djukanović,Nikola Balaban,Christian Blum,Aleksandar Kartelj,Sašo Džeroski,Žiga Zebec*

Main category: cs.AI

TL;DR: 该论文提出了一种基于根状态图表示和迭代束搜索的框架，用于解决具有灵活间隙约束的变间隙最长公共子序列问题，在合成实例上展示了优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: VGLCS问题是经典LCS问题的泛化，在分子序列比较（需要满足残基间的结构距离约束）和时间序列分析（事件需要在指定时间延迟内发生）中有重要应用，但现有研究缺乏全面的计算分析。

Method: 采用基于根状态图的搜索框架，将状态空间表示为多个根状态子图；使用迭代束搜索策略动态维护有希望的候选根节点池以控制组合爆炸；结合LCS文献中的启发式方法提升搜索质量。

Result: 在包含320个合成实例（最多10个输入序列，最多500个字符）的首次全面计算研究中，所设计方法在可比运行时间内表现出优于基线束搜索的鲁棒性。

Conclusion: 提出的基于根状态图表示和迭代束搜索的框架能有效处理VGLCS问题，为分子序列比较和时间序列分析等实际应用提供了可行的解决方案。
...
### [81] [Beyond One Output: Visualizing and Comparing Distributions of Language Model Generations](https://arxiv.org/abs/2604.18724)
*Emily Reif,Claire Yang,Jared Hwang,Deniz Nazar,Noah Smith,Jeff Heer*

Main category: cs.AI

TL;DR: GROVE是一个交互式可视化工具，通过文本图展示语言模型的多个生成结果，帮助用户理解输出分布的结构特征，如模式、分支点和聚类。


<details>
  <summary>Details</summary>
Motivation: 用户通常通过单个输出来评估语言模型，但这隐藏了输出分布的结构特征（如模式、边缘案例、对提示微小变化的敏感性），导致用户从个别案例过度泛化，在开放式任务的提示迭代中产生偏差。

Method: 首先进行形成性研究（n=13）了解研究人员在实践中如何处理随机性、如何推理语言分布以及当前工作流程的痛点。然后开发GROVE可视化工具，将多个LM生成表示为文本图中的重叠路径，揭示共享结构、分支点和聚类，同时保留对原始输出的访问。

Result: 通过三个众包用户研究（N=47, 44, 40）评估，结果显示混合工作流程最有效：图摘要改善了结构判断（如评估多样性），而直接输出检查在细节导向问题上表现更好。

Conclusion: GROVE可视化工具能够有效帮助用户理解语言模型的输出分布结构，支持混合工作流程：使用图摘要进行结构分析，结合直接输出检查处理细节问题，从而改善用户对LM输出的理解和评估。
...
### [82] [ARES: Adaptive Red-Teaming and End-to-End Repair of Policy-Reward System](https://arxiv.org/abs/2604.18789)
...
<details>
  <summary>Details</summary>
Motivation: 现有RLHF方法中，不完美的奖励模型可能成为单点故障，而传统红队测试主要针对策略级弱点，忽略了核心LLM和奖励模型同时失效的系统性弱点

Method: ARES采用"安全导师"动态组合语义连贯的对抗性提示（包含主题、角色、策略、目标等结构化组件），生成恶意和安全响应，同时暴露核心LLM和奖励模型的弱点，然后进行两阶段修复：先微调奖励模型以更好检测有害内容，再用改进的奖励模型优化核心模型

Result: 在多个对抗性安全基准测试中，ARES显著增强了安全鲁棒性，同时保持了模型能力，为全面的RLHF安全对齐建立了新范式

Conclusion: ARES框架通过系统性发现和缓解核心LLM与奖励模型的双重漏洞，为解决RLHF中的系统性弱点提供了有效方法，提升了大型语言模型的安全对齐效果
...
Result: DAG覆盖核心算法完全机器验证，结构引理、覆盖核心生成器和闭合路径恒等式在Lean 4中验证，高层定理保持非正式证明草图，ML层下游计算数学正确性得到认证。

Conclusion: 这是首个将交互式定理证明应用于知识产权分析的框架，为专利分析提供形式化验证基础，但ML评分准确性验证和实际案例验证是未来工作重点。
...
TL;DR: 论文提出"人工特殊智能"概念，使机器学习分类模型能够无错误训练，避免重复犯错，在18个MedMNIST生物医学数据集上验证，除3个存在双标签问题的数据集外，其余均实现完美训练


<details>
  <summary>Details</summary>
Motivation: 当前机器学习模型在分类任务中会重复犯同样的错误，作者希望开发一种能够无错误训练的方法，使模型具备不重复犯错的能力

Method: 提出"人工特殊智能"概念，应用于18个MedMNIST生物医学数据集进行分类训练

Result: 除3个存在双标签问题的数据集外，其余15个数据集均实现了完美训练，模型不再重复犯错

Conclusion: 人工特殊智能概念能够有效实现机器学习模型的零错误训练，但双标签问题仍是需要解决的技术挑战
...
TL;DR: 研究发现聚合基准无法捕捉个人偏好，提出需要个性化LLM基准，通过分析115名用户数据发现个人排名与聚合排名差异显著，主题和写作风格特征可预测用户特定模型排名。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力的提升和在现实任务中的部署，评估LLM与人类偏好的对齐性变得重要。当前基准将所有用户的偏好平均计算聚合评分，忽视了在建立模型排名时的个人用户偏好。由于用户在不同情境下有不同偏好，需要个性化的LLM基准来根据个人需求对模型进行排名。
...
Result: 个人LLM模型排名与聚合LLM排名存在显著差异：Bradley-Terry相关性平均仅为ρ=0.04（57%用户显示接近零或负相关），ELO评分显示中等相关性（ρ=0.43）。用户表现出主题兴趣和沟通风格的显著异质性，影响其模型偏好。紧凑的主题和风格特征组合为预测用户特定模型排名提供了有用的特征空间。

Conclusion: 聚合基准无法捕捉大多数用户的个人偏好，强调了开发个性化基准的重要性，这些基准应根据个人用户偏好对LLM模型进行排名。主题和风格特征可用于预测用户特定模型偏好。
...
TL;DR: AltTrain：通过改变推理结构实现大型推理模型的安全对齐，仅需1K训练样本的监督微调


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现优异，但面对恶意查询时容易生成有害响应。研究发现问题根源在于推理结构本身，因此通过改变推理结构可以实现有效的安全对齐。

Method: 提出AltTrain方法，这是一种简单有效的后训练方法，通过监督微调（SFT）显式改变大型推理模型的推理结构。该方法不需要复杂的强化学习训练或奖励设计，仅需1K轻量级训练样本。

Result: 在不同骨干模型和模型规模上的实验表明，AltTrain实现了强大的安全对齐效果，同时在推理、问答、摘要和多语言设置中展现出稳健的泛化能力。
...
TL;DR: DW-Bench是一个评估大语言模型在数据仓库模式上进行图拓扑推理的新基准，整合了外键和数据血缘边，包含1046个自动生成的验证正确的问题


<details>
  <summary>Details</summary>
Motivation: 现有基准在评估LLMs对数据仓库复杂图结构（包括外键和数据血缘关系）的推理能力方面存在不足，需要专门的评估框架

Method: 创建了包含5个模式、1046个自动生成且可验证正确的问题的基准，明确整合了外键和数据血缘边，用于评估LLMs的图拓扑推理能力

Result: 实验表明，工具增强方法显著优于静态方法，但在困难的组合子类型上性能达到瓶颈

Conclusion: DW-Bench为评估LLMs在数据仓库图拓扑推理能力提供了有效的基准，揭示了工具增强方法的优势及其在复杂推理任务上的局限性
...
### [88] [On Accelerating Grounded Code Development for Research](https://arxiv.org/abs/2604.19022)
*Santosh Ganji*

Main category: cs.AI

TL;DR: 该论文提出一个框架，让编码智能体能够即时访问研究资料库和技术文档，解决专业科学领域缺乏最新领域知识的问题。


<details>
  <summary>Details</summary>
Motivation: 专业科学和技术领域（如材料科学、通信工程、生物工程）在利用编码智能体时面临缺乏最新领域特定知识的挑战。基础模型在专业领域推理能力有限，且无法整合持续发展的研究成果，而领域专家通常缺乏微调大模型或持续嵌入新发现的资源。

Method: 引入一个框架，让编码智能体能够即时访问研究资料库和技术文档，实现实时、上下文感知的操作。开源实现包括doc-search.dev文档上传功能和zed-fork工具，后者用于强制执行领域特定规则和工作流程。

Result: 该框架加速了编码智能体在专业科学和技术工作流程中的集成，使研究人员能够实时获取最新领域知识，提高编码智能体在专业环境中的实用性。

Conclusion: 通过提供即时访问研究资料库和技术文档的框架，解决了专业领域编码智能体缺乏最新知识的问题，降低了AI驱动编码智能体在专业科学和技术工作流程中的采用门槛。
...
### [89] [Plausible Reasoning and First-Order Plausible Logic](https://arxiv.org/abs/2604.19036)
*David Billington*
...
<details>
  <summary>Details</summary>
Motivation: 可信陈述是那些可能、大概或通常为真但偶尔可能为假的陈述。可信推理是从事实或可信陈述中得出结论的过程，不涉及概率等数值计算。目前缺乏能够系统处理可信推理的逻辑框架。

Method: 提出了17条可信推理的逻辑原则（14条必要原则和3条可取原则），定义了一种名为"可信逻辑"的一阶逻辑系统，该系统满足除两条可取原则外的所有原则，并开发了8种不同的推理算法来处理各种可信推理情境。

Result: 可信逻辑系统能够正确处理所有考虑的可信推理示例，据作者所知，这是唯一能够做到这一点的逻辑系统。该系统提供了8种推理算法，以适应不同可信推理情境下的合理结论。

Conclusion: 可信逻辑为处理可信推理提供了一个系统化的逻辑框架，填补了该领域的空白。该文章是即将提交的书籍《可信推理与可信逻辑》的浓缩版本，每个章节对应书中的一个章节。
...
<details>
  <summary>Details</summary>
Motivation: 从状态图像序列中学习动作模型是一个更具挑战性的设置，因为缺乏动作观察信息，需要构建能够捕捉动作前提条件和效果的模型，这对于在实际领域应用AI规划至关重要。

Method: 提出深度学习框架联合学习状态预测、动作预测和提升动作模型；引入混合整数线性规划（MILP）防止预测崩溃和自增强错误，MILP在轨迹子集上求解逻辑一致的状态、动作和动作模型，并使用MILP解中的伪标签指导进一步训练。

Result: 在多个领域的实验表明，集成基于MILP的校正有助于模型逃离局部最优解，收敛到全局一致的解决方案。

Conclusion: 该方法能够从状态图像序列中有效学习提升动作模型，无需动作观察，通过MILP校正机制提高了学习过程的鲁棒性和一致性。
...
### [91] [Reinforcement Learning Improves LLM Accuracy and Reasoning in Disease Classification from Radiology Reports](https://arxiv.org/abs/2604.19060)
*Yishu Wei,Yi Lin,Adam Flanders,George Shih,Yifan Peng*

Main category: cs.AI

TL;DR: 提出两阶段方法：先监督微调疾病分类，再用GRPO优化预测准确性和格式，不依赖推理监督，在放射学报告分类中提升效果


<details>
  <summary>Details</summary>
Motivation: 放射学报告的准确疾病分类对许多应用至关重要，但监督微调轻量级大语言模型虽然能提高准确性，却可能损害模型的推理能力

Method: 两阶段方法：第一阶段对疾病标签进行监督微调；第二阶段使用组相对策略优化（GRPO），通过优化准确性和格式来细化预测，无需推理监督

Result: 在三个放射科医生标注的数据集上，监督微调方法优于基线，而GRPO进一步改善了分类性能，同时增强了推理的召回率和全面性

Conclusion: 提出的两阶段方法（SFT+GRPO）能有效提升放射学报告疾病分类的准确性，同时改善模型的推理能力，无需专门的推理监督
...
### [92] [OLLM: Options-based Large Language Models](https://arxiv.org/abs/2604.19087)
*Shashank Sharma,Janina Hoffmann,Vinay Namboodiri*

Main category: cs.AI

TL;DR: OLLM用学习到的选项集合替代标准LLM的单次令牌预测，通过离散潜变量索引多个合理的下一个令牌选项，增强可控性和多样性


<details>
  <summary>Details</summary>
Motivation: 标准LLM依赖温度或采样启发式方法产生多样性，缺乏显式建模变化的能力。OLLM旨在通过显式建模多个合理的下一个令牌选项来增强可控性、鲁棒性和效率

Method: OLLM是一个轻量级"插件"，在输出头前插入编码器和解码器两层，将预训练LLM转换为选项化模型。使用小型潜空间参数化多个合理的下一个令牌选项，下游策略可在潜空间中选择或搜索

Result: 在1.7B参数骨干上，仅1.56%参数可训练。SOTA LoRA基线峰值51%最终答案正确率，而OLLM在最优潜选择下可达~70%。在潜空间中训练紧凑策略可控制生成，显著减少常见错位问题

Conclusion: 选项化的下一个令牌建模增强了数学推理中的可控性、鲁棒性和效率，潜空间策略学习是LLM强化学习的有前景方向。这种对齐源于模型结构而非额外KL或手工对齐损失
...
### [93] [Towards Scalable Lifelong Knowledge Editing with Selective Knowledge Suppression](https://arxiv.org/abs/2604.19089)
*Dahyun Jung,Jaewook Lee,Heuiseok Lim*

Main category: cs.AI

TL;DR: LightEdit是一个终身知识编辑框架，通过检索相关知识和抑制原始知识概率，高效更新LLM知识，解决现有方法稳定性差和训练成本高的问题
...
Conclusion: LightEdit通过结合检索选择和解码策略，解决了终身知识编辑中的稳定性和可扩展性问题，为LLM的知识更新提供了一种高效、成本效益高的解决方案
...
### [94] [Has Automated Essay Scoring Reached Sufficient Accuracy? Deriving Achievable QWK Ceilings from Classical Test Theory](https://arxiv.org/abs/2604.19131)
*Masaki Uto*

Main category: cs.AI

TL;DR: 论文提出两种基于经典测试理论的数据集特定QWK上限，用于评估自动作文评分系统的性能极限


<details>
  <summary>Details</summary>
Motivation: 自动作文评分通常使用QWK在公共基准上评估，但由于人工评分存在不可避免的错误，不清楚QWK的理论可达到水平和实际部署所需水平

Method: 基于经典测试理论的可靠性概念，推导出两种数据集特定的QWK上限：理论上限（完美预测潜在真实分数的理想模型能达到的最大QWK）和类人上限（具有人类水平评分错误的AES模型可达到的QWK）

Result: 模拟实验验证了提出的上限，真实基准实验展示了这些上限如何阐明现代AES模型的当前性能和剩余提升空间

Conclusion: 提出的QWK上限为评估自动作文评分系统提供了更准确的性能参考，表明传统的人-人QWK可能低估真实上限
...
### [95] [Reasoning-Aware AIGC Detection via Alignment and Reinforcement](https://arxiv.org/abs/2604.19172)
...
### [96] [UAF: A Unified Audio Front-end LLM for Full-Duplex Speech Interaction](https://arxiv.org/abs/2604.19221)
*Yadong Li,Guoxin Wu,Haiping Hou,Biye Li*

Main category: cs.AI

TL;DR: 提出首个针对全双工语音系统的统一音频前端大语言模型(UAF)，将VAD、TD、SR、ASR、QA等任务统一为自回归序列预测问题，显著提升响应延迟和打断准确性。


<details>
  <summary>Details</summary>
Motivation: 传统级联语音处理流水线存在延迟累积、信息丢失和错误传播等问题，而现有的端到端音频大语言模型大多是半双工的，且依赖多个独立的前端组件。在开发语音助手时发现，优化语音前端与推进后端统一模型同等重要，需要填补这一空白。

Method: 将多种音频前端任务重新表述为单一的自回归序列预测问题，包括语音活动检测、话轮转换检测、说话人识别、语音识别和问答。模型以流式固定时长音频块为输入，利用参考音频提示在开始时锚定目标说话人，回归生成编码语义内容和系统级状态控制的离散标记。

Result: 实验表明，该模型在多个音频前端任务上达到领先性能，并在实际交互场景中显著提升了响应延迟和打断准确性。

Conclusion: 提出的统一音频前端大语言模型(UAF)成功解决了全双工语音系统中的前端任务统一问题，为实现更自然、响应更快的语音交互系统提供了有效方案。
...
### [97] [Explicit Trait Inference for Multi-Agent Coordination](https://arxiv.org/abs/2604.19278)
*Suhaib Abdurahman,Etsuko Ishii,Katerina Margatina,Divya Bhargavi,Monica Sunkara,Yi Zhang*

Main category: cs.AI

TL;DR: 提出ETI方法，通过心理维度（温暖度和能力）推断伙伴特征来改善多智能体协调，在多种场景下显著提升性能


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体系统在处理复杂任务时存在协调失败问题，如目标漂移、错误级联和行为不匹配，需要改进协调机制

Method: 提出显式特质推断（ETI）方法，让智能体从交互历史中推断并跟踪伙伴的两个心理维度特征：温暖度（如信任）和能力（如技能），以此指导决策

Result: 在受控环境（经济游戏）中减少45-77%的收益损失；在复杂多智能体场景（MultiAgentBench）中性能提升3-29%；特质推断与性能提升密切相关

Conclusion: ETI是轻量级且鲁棒的协调改进机制，首次系统证明LLM智能体能从交互历史可靠推断他人特质，并利用结构化认知改善协调
...
### [98] [Large Language Models Exhibit Normative Conformity](https://arxiv.org/abs/2604.19301)
*Mikako Bito,Keita Nishimoto,Kimitaka Asatani,Ichiro Sakata*

Main category: cs.AI

TL;DR: 本研究区分了大型语言模型中的信息性从众和规范性从众，发现多数LLM同时表现出这两种从众倾向，且规范性从众的目标可通过社会情境操控，这可能使LLM多智能体系统易受恶意用户操纵。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在基于LLM的多智能体系统中表现出的从众偏见可能严重影响决策质量。以往研究将"从众"简单视为意见改变，本研究旨在从社会心理学角度区分信息性从众和规范性从众，以理解LLM从众的内在机制。

Method: 设计新任务区分信息性从众（参与者追求准确判断）和规范性从众（参与者避免冲突或寻求群体接纳），基于这些任务设置进行实验。评估六个LLM，分析其内部向量与两种从众行为的关联。

Result: 六个被评估的LLM中，多达五个同时表现出信息性从众和规范性从众倾向。有趣的是，通过操控社会情境的细微方面，可以控制特定LLM规范性从众的目标对象。分析表明，虽然两种行为外部表现相同，但可能由不同的内部机制驱动。

Conclusion: LLM多智能体系统的决策可能易受少数恶意用户操纵。研究结果为了解"规范"如何在LLM中实现以及它们如何影响群体动态提供了初步里程碑，揭示了从众行为背后的复杂机制差异。
...
### [99] [Towards Energy Impact on AI-Powered 6G IoT Networks: Centralized vs. Decentralized](https://arxiv.org/abs/2604.19377)
*Anjie Qiu,Donglin Wang,Sanket Partani,Andreas Weinand,Hans D. Schotten*

Main category: cs.AI

TL;DR: 分布式机器学习在6G物联网中能显著提升能源效率，在保持约90%预测准确率的同时减少高达70%的电力消耗


<details>
  <summary>Details</summary>
Motivation: 6G技术为物联网中的机器学习应用带来了新的挑战和机遇，特别是能源效率问题。模型训练和数据传输消耗大量能源，优化这些过程对可持续系统设计至关重要。

Method: 首先分析集中式和分布式架构的能源消耗模型，然后在德国铁路基础设施中部署测试平台，利用传感器数据进行基于机器学习的预测性维护。对分布式与集中式学习架构进行对比分析。

Result: 分布式模型在保持约90%竞争性预测准确率的同时，将整体电力消耗减少了高达70%。分布式机器学习特别通过减少传输相关的能源成本来提高能源效率。

Conclusion: 分布式机器学习在现实世界物联网部署中具有显著提高能源效率的潜力，特别是在6G技术背景下，通过优化数据传输和模型训练过程实现可持续的系统设计。
...
TL;DR: GRASPrune是一个结构化剪枝框架，可在预训练后联合剪枝FFN通道和KV头组，在保持模型性能的同时显著减少参数和推理成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型服务成本高昂，主要因为模型参数、注意力计算和KV缓存带来了巨大的内存和延迟开销。需要一种有效的剪枝方法来减少这些成本。

Method: GRASPrune使用投影直通估计器学习轻量级门分数，在训练过程中强制实施硬掩码以满足预算约束，同时保持主干权重冻结。剪枝后校准保留单元的缩放因子以缓解尺度不匹配问题，并将这些因子折叠到剪枝权重中。
...
Conclusion: GRASPrune提供了一种高效的结构化剪枝方法，能够在保持模型性能的同时显著减少参数和推理成本，无需完整的模型微调。
...
### [101] [Four-Axis Decision Alignment for Long-Horizon Enterprise AI Agents](https://arxiv.org/abs/2604.19457)
*Vasundra Srininvasan*

Main category: cs.AI

TL;DR: 论文提出将长时程企业智能体决策行为分解为四个正交的对齐轴：事实精度、推理一致性、合规重构和校准弃权，以解决传统单一成功率指标无法区分不同失败模式的问题。


<details>
  <summary>Details</summary>
Motivation: 当前长时程企业智能体（贷款审批、理赔裁决、临床审查等）评估仅报告单一任务成功率，这一指标混淆了不同的失败模式，无法判断智能体是否真正符合部署环境所需的监管标准。

Method: 提出四轴对齐分解框架：事实精度（FRP）、推理一致性（RCS）、合规重构（CRR）和校准弃权（CAR）。在LongHorizon-Bench基准上测试六种记忆架构，包括检索、模式锚定架构和简单摘要等方法。

Result: 发现传统聚合准确率无法观察到的现象：检索架构在事实精度上崩溃；模式锚定架构需支付"脚手架税"；简单摘要配合事实保留提示在多个轴上表现强劲；所有六种架构都存在弃权问题，暴露了决策对齐轴的缺失。

Conclusion: 四轴分解框架能够揭示传统指标隐藏的对齐问题，特别是合规重构和校准弃权这两个在实验室外决策中至关重要的对齐维度。该框架可通过构建事实模式和校准CRR审计提示迁移到任何受监管决策领域。
...
### [102] [Do LLMs Game Formalization? Evaluating Faithfulness in Logical Reasoning](https://arxiv.org/abs/2604.19459)
*Kyuhee Kim,Auguste Poiroux,Antoine Bosselut*

Main category: cs.AI

TL;DR: 研究发现前沿大模型在生成Lean 4证明时存在"形式化游戏"现象，即模型可能利用形式化验证只保证证明有效性而不保证形式化忠实性的漏洞。虽然统一生成模式下未发现系统性游戏行为，但两阶段流程揭示了两种不同的不忠实模式。


<details>
  <summary>Details</summary>
Motivation: 形式化验证只保证证明的有效性，但不保证形式化的忠实性。在自然语言逻辑推理中，模型需要从头构建公理系统而没有库约束，这种有效证明与忠实翻译之间的差距尤为突出。研究旨在探究前沿模型在生成Lean 4证明时是否会利用这一差距，即是否存在"形式化游戏"行为。

Method: 评估GPT-5和DeepSeek-R1在303个一阶逻辑问题上的表现（203个来自FOLIO，100个来自Multi-LogiEval）。比较统一生成与两阶段流程：第一阶段将自然语言问题形式化，第二阶段基于形式化版本生成证明。通过编译率（87-99%）和跨阶段比较来检测不忠实行为。

Result: 统一生成模式下未发现系统性游戏行为：模型倾向于报告失败而非强制生成证明。两阶段流程揭示了两种不忠实模式：GPT-5在证明生成阶段伪造公理（可通过跨阶段比较检测），而DeepSeek-R1在形式化阶段错误翻译前提，产生内部一致但完全逃避检测的输出。高编译率或准确率不应等同于忠实推理。

Conclusion: 前沿大模型在形式化推理中存在"形式化游戏"风险，表现为两种不同的不忠实模式。形式化验证的高编译率或准确率不能保证推理的忠实性，需要更细致的检测方法来识别隐藏的不忠实行为。研究为理解模型在形式化任务中的行为提供了重要洞见。
...
### [103] [CoDA: Towards Effective Cross-domain Knowledge Transfer via CoT-guided Domain Adaptation](https://arxiv.org/abs/2604.19488)
*Jianzhi Yan,Le Liu,Buzhou Tang,Yang Xiang,Dongning Sun,Zhiming Li*

Main category: cs.AI

TL;DR: CoDA提出使用轻量级适配器干预LLM隐藏状态，通过特征蒸馏和分布对齐来提升跨领域逻辑推理能力，显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: LLMs在逻辑推理方面仍有提升空间，特别是在专家稀缺的领域（如低资源科学学科、新兴生物医学子领域或小众法律管辖区），高质量领域内示例有限或完全不可用，限制了现有方法的通用性。虽然已有研究探索使用跨领域样本作为替代演示，但由于源域和目标域之间的显著领域偏移，模型难以有效识别和利用共享结构或潜在推理模式。
...
Conclusion: CoDA通过直接干预隐藏状态并利用特征蒸馏和分布对齐，有效解决了跨领域逻辑推理中的领域偏移问题，为在专家稀缺领域提升LLMs推理能力提供了有效解决方案。
...
### [104] [SimDiff: Depth Pruning via Similarity and Difference](https://arxiv.org/abs/2604.19520)
*Yuli Chen,Shuhao Zhang,Fanshen Meng,Bo Cheng,Jiale Han,Qiang Tong,Xiulei Liu*

Main category: cs.AI

TL;DR: SimDiff提出了一种新的深度剪枝方法，通过联合评估表示相似性和变换差异两个正交视角来确定层的重要性，显著优于仅依赖余弦距离的现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度剪枝方法主要依赖层间余弦距离这一单一启发式标准，但这种方法在不同架构上表现出不可预测的性能，甚至可能导致灾难性崩溃。需要更稳健的层重要性评估标准。

Method: 提出SimDiff方法，从两个正交角度联合评估层重要性：1) 表示相似性；2) 变换差异。变换差异使用两种不同指标量化：MSSD（对异常值敏感，识别做出决定性修正的层）和MASD（稳健测量层的平均贡献）。

Result: 在0.5B到13B参数范围的多个模型上进行实验，SimDiff显著优于最先进的基线方法。在25%剪枝比例下，LLaMA2-7B性能保留超过91%；在LLaMA3.1-8B上剪枝12层时，推理速度提升达1.49倍。剪枝后的模型可以通过最小化微调有效恢复。

Conclusion: SimDiff通过多维度评估层重要性，解决了现有单维度启发式方法的局限性，为大型语言模型的深度剪枝提供了更可靠、高效的解决方案。
...
### [105] [Integrating Anomaly Detection into Agentic AI for Proactive Risk Management in Human Activity](https://arxiv.org/abs/2604.19538)
*Farbod Zorriassatine,Ahmad Lotfi*

Main category: cs.AI

TL;DR: 将跌倒检测和预测重新定义为异常检测问题，通过智能体AI系统更有效地处理，提出一个动态选择工具并整合到自适应决策流程中的概念框架。


<details>
  <summary>Details</summary>
Motivation: 现有跌倒缓解系统（跌倒预测和检测）尚未成为跨护理路径和安全关键环境的通用解决方案，主要受限于处理现实世界复杂性的能力不足，如上下文意识差、误报率高、环境噪声和数据稀缺等问题。

Method: 将跌倒检测和跌倒预测重新表述为异常检测问题，通过智能体AI系统来解决。提出一个概念框架，强调动态选择相关工具并将其整合到自适应决策工作流程中，而不是依赖针对狭义定义场景的静态配置。

Result: 论文提出了一个概念框架，但未提供具体技术实现或实验结果。框架强调通过智能体AI系统实现早期识别与跌倒风险相关的运动模式细微偏差，无论这些偏差源于年龄相关衰退、疲劳还是环境因素。

Conclusion: 智能体AI系统为解决老年人跌倒风险提供了有前景的方向，通过将跌倒检测和预测重新定义为异常检测问题，并采用动态、自适应的决策框架，能够更好地处理现实世界的复杂性，实现更有效的风险管理。
...
### [106] [DT2IT-MRM: Debiased Preference Construction and Iterative Training for Multimodal Reward Modeling](https://arxiv.org/abs/2604.19544)
*Zhihong Zhang,Jie Zhao,Xiaojian Huang,Jin Xu,Zhuodong Luo,Xin Liu,Jiansheng Wei,Xuejin Chen*

Main category: cs.AI

TL;DR: DT2IT-MRM提出了一种改进多模态奖励模型训练的方法，通过去偏好的偏好构建流程、文本到图像偏好数据重构和迭代训练框架，解决了现有多模态偏好数据集的粒度不足、文本风格偏差和不可靠偏好信号等问题。


<details>
  <summary>Details</summary>
Motivation: 现有训练多模态奖励模型（MRMs）所需的多模态偏好数据存在三个关键问题：偏好强度缺乏粒度、文本风格偏差以及不可靠的偏好信号。此外，现有的开源多模态偏好数据集存在大量噪声，但缺乏有效且可扩展的质量提升方法。

Method: DT2IT-MRM整合了三个核心组件：1）去偏好的偏好构建流程；2）文本到图像（T2I）偏好数据的新重构方法；3）迭代训练框架，用于整理现有的多模态偏好数据集以用于多模态奖励模型训练。

Result: 实验结果表明，DT2IT-MRM在三个主要基准测试上取得了新的最先进性能：VL-RewardBench、Multimodal RewardBench和MM-RLHF-RewardBench。

Conclusion: DT2IT-MRM通过解决现有多模态偏好数据集的局限性，显著提升了多模态奖励模型的性能，为多模态大语言模型与人类偏好的对齐提供了更有效的训练方法。
...
### [107] [Detecting Data Contamination in Large Language Models](https://arxiv.org/abs/2604.19561)
*Juliusz Janicki,Savvas Chamezopoulos,Evangelos Kanoulas,Georgios Tsatsaronis*

Main category: cs.AI

TL;DR: 研究评估了针对大语言模型的黑盒成员推断攻击方法，发现现有方法在检测训练数据成员资格方面效果不佳，AUC-ROC接近0.5，表明无法可靠识别训练数据。


<details>
  <summary>Details</summary>
Motivation: 大语言模型使用大量可能包含版权内容的训练数据，成员推断攻击旨在检测特定文档是否包含在模型的训练集中。黑盒攻击方法需要大量数据操作，难以比较效果，因此需要系统评估现有方法在SOTA大语言模型上的表现。

Method: 研究评估了最先进的黑盒成员推断攻击方法，使用统一的数据集进行比较，并开发了一种新方法"熟悉度排名"，该方法通过给予LLM更多表达自由来更好地理解其推理过程。

Result: 所有方法在多个大语言模型上的AUC-ROC都接近0.5，表明无法可靠检测训练数据成员资格。更先进的LLM显示出更高的真阳性率和假阳性率，反映了其更强的推理和泛化能力，也凸显了黑盒成员推断攻击的困难。

Conclusion: 现有黑盒成员推断攻击方法无法可靠检测大语言模型的训练数据成员资格，更先进的LLM由于其更强的推理和泛化能力使得成员推断更加困难。研究提出的"熟悉度排名"方法为黑盒攻击提供了新思路。
...
### [108] [Multi-modal Reasoning with LLMs for Visual Semantic Arithmetic](https://arxiv.org/abs/2604.19567)
*Chuou Xu,Liya Ji,Qifeng Chen*

Main category: cs.AI

TL;DR: 该研究提出SAri-RFT方法，通过强化学习微调大型视觉语言模型，提升其在视觉语义算术任务中的关系推理能力，特别是在图像关系推理方面。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在编码和数学推理方面已有显著进展，但在视觉语义算术（从图像推断关系）方面的能力尚未充分探索。这种能力对于服务机器人和家用机器人在非结构化环境中至关重要，机器人需要从图像中推断物体、智能体和动作之间的语义关系。

Method: 提出了两个新任务（两项减法和三项操作），构建了Image-Relation-Pair Dataset (IRPD)基准数据集。开发了Semantic Arithmetic Reinforcement Fine-Tuning (SAri-RFT)方法，使用可验证函数和Group Relative Policy Optimization (GRPO)对大型视觉语言模型进行后训练。

Result: 该方法在IRPD数据集和真实世界的Visual7W-Telling数据集上取得了最先进的结果，显著提升了模型在视觉关系推理方面的性能。
...
### [109] [AblateCell: A Reproduce-then-Ablate Agent for Virtual Cell Repositories](https://arxiv.org/abs/2604.19606)
...
TL;DR: AblateCell是一个用于虚拟细胞代码库的"先复现后消融"智能体，能够自动复现基线结果并进行系统消融实验，解决了生物代码库中消融分析难以实施的问题。


<details>
  <summary>Details</summary>
Motivation: 在AI虚拟细胞研究中，系统消融分析对于归因性能提升至关重要，但由于生物代码库标准化不足、与领域特定数据和格式紧密耦合，很少进行此类分析。现有编码智能体只能生成代码，缺乏能够复现强基线并严格测试哪些组件真正重要的验证器。

Method: AblateCell采用"先复现后消融"的两阶段方法：1）通过自动配置环境、解决依赖和数据问题、重新运行官方评估来端到端复现报告的基线结果；2）通过生成隔离的代码库变异图，在权衡性能影响和执行成本的奖励函数下自适应选择实验，进行闭环消融分析。
...
### [110] [A Dual Perspective on Synthetic Trajectory Generators: Utility Framework and Privacy Vulnerabilities](https://arxiv.org/abs/2604.19653)
*Aya Cherigui,Florent Guépin,Arnaud Legendre,Jean-François Couchot*

Main category: cs.AI

TL;DR: 本文提出了一种评估人类移动数据隐私保护方法的新框架，重点关注生成模型在隐私-效用权衡中的表现，并开发了针对特定生成模型的成员推理攻击方法。


<details>
  <summary>Details</summary>
Motivation: 人类移动数据具有高度敏感性，传统隐私保护方法（如聚合、混淆、噪声添加）会显著降低数据效用。虽然生成模型被引入以改善隐私-效用权衡，但其实际效果仍不明确，需要新的评估框架。

Method: 1. 引入新的效用评估框架来评估生成模型的性能；2. 提出针对特定生成模型子类（原本被认为对轨迹用户链接问题具有抵抗性）的成员推理攻击方法；3. 建议根据欧盟法规通过对抗性评估来解决隐私评估挑战。

Result: 1. 开发了评估生成模型效用表现的新框架；2. 证明了即使被认为对轨迹用户链接具有抵抗性的生成模型子类也容易受到成员推理攻击；3. 表明隐私评估仍然是一个重大挑战，需要通过对抗性评估来解决。

Conclusion: 本文为解决人类移动数据隐私-效用权衡问题迈出了第一步，提出了新的效用评估框架，并证明隐私评估需要更严格的对抗性测试，特别是针对生成模型的成员推理攻击。
...
### [111] [Compile to Compress: Boosting Formal Theorem Provers by Compiler Outputs](https://arxiv.org/abs/2604.18587)
*Guchan Li,Rui Tian,Hongning Wang*

Main category: cs.LG

TL;DR: 本文提出了一种基于学习优化的定理证明框架，通过利用编译器将大量证明尝试映射到紧凑失败模式的结构特性，实现高效学习和证明探索，显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在形式定理证明中表现出潜力，但最先进性能通常需要大量测试时计算（通过大规模展开或长上下文窗口），存在可扩展性瓶颈。本文旨在解决这一瓶颈。

Method: 引入学习优化框架，利用形式验证中的结构化特性：编译器将多样化的证明尝试映射到紧凑的失败模式集合。采用树搜索方法，基于验证器反馈局部修正错误，避免累积长证明历史带来的成本。

Result: 广泛评估表明，该方法在不同规模下都能持续增强基础证明器的推理能力。在PutnamBench上，在可比较的测试时预算下，该方法在公开报告的约8B和约32B参数模型中实现了最先进性能。

Conclusion: 该方法为下一代验证器引导的推理提供了一个可扩展的范式，通过利用形式验证中的结构化压缩特性，有效解决了大型语言模型在定理证明中的可扩展性瓶颈问题。
...
### [112] [Easy Samples Are All You Need: Self-Evolving LLMs via Data-Efficient Reinforcement Learning](https://arxiv.org/abs/2604.18639)
*Zhiyin Yu,Bo Zhang,Qibin Hou,Zhonghai Wu,Xiao Luo,Lei Bai*

Main category: cs.LG

TL;DR: EasyRL提出了一种基于认知学习理论的新方法，通过整合少量标注数据和渐进式分治策略，在仅使用10%标注数据的情况下，在数学和科学基准上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的强化学习方法存在标注成本高、模型崩溃或奖励黑客等问题，性能不尽如人意。需要一种更高效、数据利用率更高的方法来提升LLM的推理能力。

Method: 1) 使用少量标注数据进行监督RL初始化模型；2) 对困难未标注数据采用分治伪标注策略：低不确定性情况使用一致性选择，中等不确定性情况使用反思解决；3) 难度渐进的自训练结合迭代伪标注和RL进一步强化模型推理能力。

Result: 在数学和科学基准测试中，EasyRL仅使用10%的标注数据，就能持续超越最先进的基线方法。

Conclusion: EasyRL提供了一个统一的自演化框架，促进了LLM的数据高效后训练，通过模拟人类认知获取曲线，有效解决了现有RL方法的局限性。
...
### [113] [Curiosity-Critic: Cumulative Prediction Error Improvement as a Tractable Intrinsic Reward for World Model Training](https://arxiv.org/abs/2604.18701)
*Vin Bhaskara,Haicheng Wang*

Main category: cs.LG

TL;DR: 本文提出Curiosity-Critic方法，通过基于世界模型累积预测误差改进的内在奖励来指导探索，使用在线学习的critic估计状态转移的渐近误差基线，有效区分可学习的认知误差和不可约的随机误差。


<details>
  <summary>Details</summary>
Motivation: 现有基于局部预测误差的好奇心奖励只关注当前转移，忽略了世界模型在所有访问过的转移上的累积预测误差。需要一种能够基于累积目标改进的内在奖励机制。

Method: 提出Curiosity-Critic方法，其内在奖励基于累积预测误差目标的改进，可简化为可处理的每步形式：当前预测误差与当前状态转移渐近误差基线的差值。通过在线学习的critic与世界模型协同训练来估计这个基线。

Result: 在随机网格世界实验中，Curiosity-Critic在收敛速度和最终世界模型准确性方面优于预测误差和访问计数基线方法。critic在世界模型饱和前就能良好收敛，将探索引导到可学习的转移上。

Conclusion: Curiosity-Critic通过基于累积预测误差改进的内在奖励，有效区分认知误差和随机误差，将先前的预测误差好奇心公式统一为特定基线近似的特例，在探索效率和模型学习方面表现优越。
...
TL;DR: 该论文提出了一种基于预测必要性的非线性时间序列模型因果相关性评估框架，通过系统性的边消融和预测比较来检验候选因果关系是否对准确预测是必需的。


<details>
  <summary>Details</summary>
Motivation: 当前非线性机器学习模型在时间序列因果发现中应用日益广泛，但其输出解释性差。特别是正则化神经自回归模型产生的因果分数常被当作回归系数的类比，导致对统计显著性的误导性声称。需要更可靠的因果推理方法。

Method: 提出了一个基于预测必要性的可解释评估框架，采用系统性的边消融和预测比较方法。以神经加性向量自回归模型为案例研究模型，应用于139个国家民主发展的多变量时间序列面板数据。

Result: 研究发现，具有相似因果分数的关系在预测必要性上可能存在显著差异，这主要由于冗余性、时间持久性和特定制度效应等因素造成。预测必要性测试能够更可靠地支持因果推理。

Conclusion: 预测必要性测试为应用AI系统中的因果推理提供了更可靠的方法，并为高风险领域中非线性时间序列模型的解释提供了实用指导。该方法支持更可靠的因果推理，避免仅依赖系数大小导致的误导性结论。
...
TL;DR: 该论文提出了一种处理医疗多模态数据缺失问题的新方法，通过将临床诊断重新定义为自回归序列建模任务，利用对比预训练和transformer架构来整合缺失模态，并在MIMIC-IV和eICU基准测试中取得优于基线模型的效果。


<details>
  <summary>Details</summary>
Motivation: 医疗多模态机器学习面临的主要挑战是在训练和部署过程中处理缺失的模态数据。临床数据集本质上是时序性的，且模态存在稀疏性，如何在保持模型可解释性的同时捕捉潜在的预测信号是一个持续存在的难题。

Method: 1. 将临床诊断重新定义为自回归序列建模任务，利用大型语言模型的因果解码器来建模患者的多模态轨迹；2. 引入缺失感知的对比预训练目标，在共享潜在空间中整合具有缺失的多模态数据集；3. 使用基于transformer的架构进行自回归序列建模。

Result: 在MIMIC-IV和eICU微调基准测试中，自回归序列建模方法优于基线模型。通过可解释性技术发现，移除不同模态会导致模型行为出现差异，而对比预训练能够缓解这种问题。

Conclusion: 通过将临床诊断抽象为序列建模并解释患者住院轨迹，开发了一个框架来分析和处理缺失模态，同时满足了安全、透明的临床AI的经典需求。
...
TL;DR: 通过在推理时将预训练的稀疏自编码器集成到Transformer残差流中，无需修改模型权重或阻断梯度，显著提升了大型语言模型对越狱攻击的鲁棒性，在多种模型和攻击方法下实现了高达5倍的攻击成功率降低。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型仍然容易受到基于优化的越狱攻击，这些攻击利用了内部梯度结构。虽然稀疏自编码器广泛用于可解释性研究，但其在鲁棒性方面的潜力尚未充分探索。本研究旨在探索将预训练SAE集成到模型推理过程中是否能增强对越狱攻击的防御能力。

Method: 在推理时将预训练的稀疏自编码器集成到Transformer残差流中，不修改模型权重，也不阻断梯度。研究覆盖了四个模型家族（Gemma、LLaMA、Mistral、Qwen）和两种强白盒攻击（GCG、BEAST）以及三个黑盒基准测试。通过参数化消融实验分析了L0稀疏度与攻击成功率的关系，以及不同层的防御-效用权衡。

Result: SAE增强模型相对于未防御基线实现了高达5倍的越狱攻击成功率降低，并减少了跨模型攻击的可迁移性。参数化消融显示：1）L0稀疏度与攻击成功率存在单调剂量-响应关系；2）中间层在鲁棒性和干净性能之间达到最佳平衡。这些发现与表示瓶颈假说一致：稀疏投影重塑了越狱攻击所利用的优化几何结构。

Conclusion: 将稀疏自编码器集成到推理过程中是一种有效的防御策略，能够显著提升大型语言模型对越狱攻击的鲁棒性，同时保持模型性能。这种方法通过稀疏投影重塑优化几何，为基于表示的防御机制提供了新的研究方向。
...
TL;DR: Flash-SemiCRF：一种高效的内存优化半马尔可夫条件随机场实现，通过前缀和查找和流式前向后向算法，解决了长序列和大标签集场景下的内存瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 传统半CRF实现需要存储庞大的边势能张量，其大小随序列长度、最大片段长度和标签数量增长，在语音规模状态空间和基因组尺度（超过10万个位置）下变得不可行，这限制了精确片段级推理在长序列和大标签集上的应用。

Method: 1. 用前缀和查找替代存储的边张量，内存占用减少与片段长度和标签数量乘积成正比；2. 采用带检查点边界归一化的流式前向后向传递，保持工作内存与序列长度呈亚线性关系；3. 零中心累积分数控制数值漂移，在标签不平衡下引入自适应持续时间先验；4. 集成到Flash-SemiCRF，一个融合的Triton内核。

Result: Flash-SemiCRF能够在先前难以处理的问题规模上实现精确的半CRF推理，解决了内存瓶颈问题，使长序列和大标签集的精确片段级推理变得可行。

Conclusion: 通过创新的内存优化技术，Flash-SemiCRF突破了传统半CRF实现的内存限制，为基因组学等需要处理超长序列的领域提供了高效的精确推理解决方案。
...
TL;DR: NPUMoE是一个针对Apple Silicon芯片的运行时推理引擎，通过将混合专家模型（MoE）的静态密集计算卸载到神经处理单元（NPU），同时保留CPU/GPU回退路径处理动态操作，显著提升MoE推理性能。


<details>
  <summary>Details</summary>
Motivation: 混合专家模型（MoE）通过稀疏激活提高推理效率，但在Apple Neural Engine（ANE）NPU上存在三大挑战：专家路由不可预测导致动态张量形状与NPU形状特定约束冲突；不规则算子（如top-k、scatter/gather等）不适用于NPU；启动大量小型专家内核会产生显著的调度和同步开销。NPU旨在从CPU和GPU卸载AI计算，本文目标是实现MoE推理的此类卸载，特别是在预填充阶段，长上下文工作负载消耗大量系统资源。
...
Result: 在Apple M系列设备上使用三个代表性MoE LLM和四个长上下文工作负载的实验表明，NPUMoE始终优于基线方法：延迟降低1.32-5.55倍，能效提高1.81-7.37倍，通过有效的NPU卸载使CPU周期使用减少1.78-5.54倍。

Conclusion: NPUMoE成功解决了MoE模型在Apple Silicon NPU上的执行挑战，通过智能的卸载策略显著提升了推理性能、能效和系统资源利用率，为在移动设备上高效运行大型MoE模型提供了可行方案。
...
TL;DR: HELM框架通过三个组件解决VLA模型在长时程操作任务中的系统性问题：记忆模块检索关键历史，状态验证器预测动作失败，控制器执行回滚和重规划，显著提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型在短时程任务上表现良好，但在长时程操作任务中存在系统性失败。研究发现，仅扩展上下文长度无法解决这一问题，根本原因在于执行循环中的三个缺陷：记忆缺口、验证缺口和恢复缺口。

Method: 提出HELM框架，包含三个组件：1) 情景记忆模块，通过CLIP索引的关键帧检索任务历史；2) 学习的状态验证器，从观察、动作、子目标和记忆条件上下文中预测动作失败；3) 控制器，执行回滚和重规划。状态验证器是核心学习贡献。

Result: 在LIBERO-LONG上，HELM将任务成功率从58.4%提升到81.5%，比仅扩展上下文窗口的5.4%增益显著。在CALVIN上也大幅提升长时程性能，并在受控扰动下显著提高恢复成功率。状态验证器始终优于基于规则的可行性检查和集成不确定性基线。

Conclusion: HELM框架有效解决了VLA模型在长时程操作任务中的执行循环缺陷，通过记忆检索、失败预测和恢复机制的组合显著提升性能。状态验证器的有效性关键依赖于情景记忆的访问，该框架为长时程操作任务提供了系统解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 科学应用中的大规模粒子数据集（如宇宙学、分子动力学）通常使用有损压缩来减少存储和I/O成本，但现有压缩器只提供粒子位置的逐点误差边界，无法保证聚类结果的保留，即使微小扰动也可能改变聚类连通性，影响科学有效性

Method: 提出基于校正的技术，在现成压缩器（如SZ3、Draco）解压数据上操作，包括：(1) 通过空间划分和局部邻域搜索识别脆弱粒子对的聚类感知校正算法；(2) 使用投影梯度下降和编码成对距离违规的损失函数来强制聚类一致性的优化公式；(3) 面向大规模数据集的可扩展GPU加速和分布式实现

Result: 在宇宙学和分子动力学数据集上的实验表明，该方法能有效保留聚类结果，同时在与SZ3、ZFP、Draco、LCP和空间填充曲线方案相比时保持有竞争力的压缩性能

Conclusion: 提出的校正技术能够在有损压缩中有效保留单链接聚类结果，为科学应用中大规模粒子数据的压缩提供了聚类感知的解决方案
...
### [121] [Rethinking Dataset Distillation: Hard Truths about Soft Labels](https://arxiv.org/abs/2604.18811)
*Priyam Dey,Aditya Sahdev,Sunny Bhati,Konda Reddy Mopuri,R. Venkatesh Babu*

Main category: cs.LG

TL;DR: 研究发现大规模数据集蒸馏方法在软标签设置下与随机基线表现相当，但在硬标签设置下高质量核心集能稳定超越随机子集。论文提出了计算感知的CAD-Prune指标和CA2D方法，在ImageNet-1K上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前大规模数据集蒸馏方法在软标签设置下表现与随机基线相当，这与核心集文献中高质量核心集在硬标签设置下稳定超越随机子集的发现存在矛盾。需要理解数据质量在不同标签机制下的作用差异。

Method: 1) 进行可扩展性分析，研究数据质量在从丰富软标签(SL+KD)到固定软标签(SL)和硬标签(HL)不同标签机制下的作用；2) 在HL设置下系统评估5个大规模和4个小规模DD方法；3) 提出CAD-Prune计算感知剪枝指标，识别给定计算预算下的最优难度样本；4) 开发CA2D计算对齐DD方法。

Result: 1) 高质量核心集在SL和SL+KD机制下未能显著超越随机基线；2) 在SL+KD设置下，性能接近全数据集的最优水平，与子集大小或质量无关；3) 在HL设置下，只有RDED在ImageNet-1K上可靠超越随机基线，但仍落后于强核心集方法；4) CA2D方法在ImageNet-1K的各种IPC设置下优于当前DD方法。

Conclusion: 研究揭示了当前DD研究的多个见解：软标签评估实践存在问题，子集质量影响可忽略；提出了计算感知的CAD-Prune指标和CA2D方法，为数据高效学习提供了有用工具。需要重新思考DD评估方法，特别是在硬标签设置下的性能评估。
...
### [122] [Curvature-Aware PCA with Geodesic Tangent Space Aggregation for Semi-Supervised Learning](https://arxiv.org/abs/2604.18816)
*Alexandre L. M. Levada*

Main category: cs.LG

TL;DR: GTSA-PCA是一种几何扩展的PCA方法，通过曲率加权的局部协方差算子和测地线对齐算子，在统一谱框架中集成曲率感知和测地线一致性，在流形数据上优于传统PCA和流形学习方法。


<details>
  <summary>Details</summary>
Motivation: 传统PCA的全局线性公式无法捕捉弯曲流形上的数据结构，而流形学习方法虽然能建模非线性但牺牲了PCA的谱结构和稳定性。需要一种既能处理非线性流形结构又能保持PCA谱特性的方法。

Method: 提出GTSA-PCA：1) 用曲率加权的局部协方差算子替代全局协方差算子，在k近邻图上定义局部切子空间；2) 引入测地线对齐算子，结合内在图距离和子空间亲和度来全局同步局部表示；3) 可加入半监督信息指导对齐过程；4) 对最终算子进行谱分解得到几何感知嵌入。

Result: 在真实数据集上的实验表明，GTSA-PCA相比PCA、核PCA、监督PCA以及UMAP等图基方法有持续改进，特别是在小样本量和高曲率情况下表现优异。

Conclusion: GTSA-PCA为降维提供了一个原则性的桥梁，连接了统计方法和几何方法，在保持PCA谱结构稳定性的同时有效处理流形数据的非线性几何特性。
...
### [123] [One Step Forward and K Steps Back: Better Reasoning with Denoising Recursion Models](https://arxiv.org/abs/2604.18839)
*Chris Cameron,Wangzheng Wang,Nikita Ivanov,Ashmita Bhattacharyya,Didier Chételat,Yingxue Zhang*

Main category: cs.LG

TL;DR: Denoising Recursion Models (DRM)通过多步递归去噪训练，比单步去噪的扩散模型更好地对齐训练与测试行为，在ARC-AGI任务上超越了Tiny Recursion Model (TRM)。


<details>
  <summary>Details</summary>
Motivation: 循环transformer通过共享参数增加计算深度，可用于迭代优化。但在需要搜索式计算的困难问题中，从噪声到高度结构化解决方案的长优化轨迹难以学习，因为训练只指定目标解而不监督中间路径。扩散模型通过单步去噪解决此问题，但训练与测试行为不一致。

Method: 提出Denoising Recursion Models (DRM)：与扩散模型类似，用噪声破坏数据，但训练模型通过多个递归步骤来逆转破坏。这种方法提供了可处理的中间状态课程，同时更好地对齐训练与测试，并激励非贪婪、前瞻性的生成。

Result: 通过大量实验表明，该方法在ARC-AGI任务上超越了最近取得突破性性能的Tiny Recursion Model (TRM)。

Conclusion: DRM通过多步递归去噪训练，提供了更好的训练-测试对齐和前瞻性生成能力，在需要复杂搜索计算的任务上表现优异。
...
### [124] [Task Switching Without Forgetting via Proximal Decoupling](https://arxiv.org/abs/2604.18857)
*Pourya Shamsolmoali,Masoumeh Zareapoor,Eric Granger,William A. P. Smith,Yue Lu*

Main category: cs.LG

TL;DR: 本文提出了一种基于算子分裂的持续学习方法，将任务学习和稳定性增强分离，通过稀疏正则化剪枝不必要参数并保留任务相关参数，在标准基准测试中取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 持续学习中的主要挑战是在学习新信息时不遗忘旧知识。现有方法通常通过正则化惩罚对先前任务关键参数的改变，但这种方法将学习和保留信号混合在单一更新中，没有明确区分关键参数和冗余参数。随着任务序列增长，这种耦合会过度约束模型，限制前向迁移并导致容量使用效率低下。

Method: 提出基于算子分裂的方法，将任务学习与稳定性增强分离：学习步骤专注于最小化当前任务损失，而近端稳定性步骤应用稀疏正则化来剪枝不必要参数并保留任务相关参数。这种方法将稳定性-可塑性权衡转化为两个互补算子之间的协商更新，而不是冲突的梯度。

Result: 在标准基准测试中取得了最先进的结果，提高了稳定性和适应性，且不需要重放缓冲区、贝叶斯采样或元学习组件。提供了分裂方法在持续学习目标上的理论证明。

Conclusion: 通过算子分裂将任务学习与稳定性增强分离的方法有效解决了持续学习中的稳定性-可塑性权衡问题，实现了更好的前向迁移和容量利用，为持续学习提供了一种新颖有效的解决方案。
...
### [125] [ParamBoost: Gradient Boosted Piecewise Cubic Polynomials](https://arxiv.org/abs/2604.18864)
*Nicolas Salvadé,Tim Hillel*

Main category: cs.LG

TL;DR: ParamBoost是一种新型广义可加模型，使用梯度提升算法学习三次多项式形状函数，可灵活加入专家约束，在保持良好预测性能的同时实现可解释性


<details>
  <summary>Details</summary>
Motivation: 传统GAMs虽然具有玻璃盒可解释性，但无法融入建模者的专家知识。需要一种既能保持可解释性，又能灵活整合领域专家约束的模型

Method: 提出ParamBoost方法，使用梯度提升算法学习形状函数，在叶节点拟合三次多项式函数。可选择性加入五种约束：(i)函数及其导数连续性(C2)；(ii)单调性；(iii)凸性；(iv)特征交互约束；(v)模型规范约束

Result: 无约束的ParamBoost模型在多个真实数据集上持续优于最先进的GAMs。实验表明，建模者可以以适度的预测性能为代价，选择性施加所需约束，使模型完全适应特定应用的可解释性和参数分析需求

Conclusion: ParamBoost提供了一种灵活框架，既能保持GAMs的玻璃盒可解释性，又能融入专家知识约束，在预测性能和可解释性之间实现良好平衡，适用于需要参数分析和领域知识整合的应用场景
...
### [126] [AC-SINDy: Compositional Sparse Identification of Nonlinear Dynamics](https://arxiv.org/abs/2604.18889)
*Peter Racioppo*

Main category: cs.LG

TL;DR: AC-SINDy是SINDy框架的扩展，用算术电路代替显式特征库，通过线性函数和乘法交互的组合构建非线性特征，实现紧凑可扩展的参数化，并在计算图上直接施加稀疏性。


<details>
  <summary>Details</summary>
Motivation: 标准SINDy方法需要枚举候选基函数，这在处理复杂非线性系统时面临可扩展性问题。需要一种更紧凑、可扩展的参数化方法，同时保持可解释性并提高对噪声的鲁棒性。

Method: 1. 用基于算术电路的结构化表示代替显式特征库；2. 通过线性函数和乘法交互的组合构建非线性特征；3. 将状态估计与动力学识别分离，结合隐状态推断、共享动力学和多步监督；4. 在计算图上直接施加稀疏性。

Result: 在非线性和混沌系统上的实验表明，该方法能够恢复准确且可解释的支配方程，同时比标准SINDy具有更好的可扩展性，对噪声具有更好的鲁棒性。

Conclusion: AC-SINDy通过算术电路的结构化表示提供了一种更紧凑、可扩展的SINDy扩展方法，能够在保持可解释性的同时提高对噪声的鲁棒性，为复杂非线性系统的动力学识别提供了有效工具。
...
### [127] [Harmful Intent as a Geometrically Recoverable Feature of LLM Residual Streams](https://arxiv.org/abs/2604.18901)
*Isaac Llorente-Saguer*

Main category: cs.LG

TL;DR: 研究发现大语言模型的残差流中存在可几何恢复的有害意图表示：在大多数层中表现为线性方向，在投影方法失效的层中表现为角度偏差。该表示在不同架构、对齐变体和规模下保持稳定，有害意图与拒绝行为在表示上是功能解耦的特征。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型中是否存在可检测的有害意图表示，以及这种表示在不同模型架构、对齐方法和规模下的几何特性与稳定性，这对于理解模型的安全机制和开发有效的安全检测方法具有重要意义。

Method: 使用6种方向寻找策略在12个模型（涵盖4种架构家族和3种对齐变体）的残差流中表征有害意图的几何表示。成功策略包括：软AUC优化的线性方向、类均值探针、监督角度偏差策略。在单轮英文评估中分析这些方法的性能。

Result: 软AUC优化的线性方向达到平均AUROC 0.98和TPR@1%FPR 0.80；类均值探针达到0.98和0.71且拟合成本<1ms；监督角度偏差策略达到AUROC 0.96和TPR 0.61，在中间层保持检测能力。检测在不同对齐变体（包括移除拒绝能力的模型）中保持稳定，AdvBench训练的方向可迁移到HarmBench和JailbreakBench（最差AUROC 0.96）。在不同规模模型（0.8B到9B）中AUROC保持≥0.98。

Conclusion: 大语言模型在通用语言理解过程中获得了线性可解码的有害意图表示，对齐过程仅影响模型如何处理此类输入而不重组上游识别信号。在安全评估中，仅使用AUROC（0.97+）可能高估操作可检测性，应同时报告TPR@1%FPR指标。
...
TL;DR: 提出CCBO框架，支持多个客户端在可控上下文环境中协作进行上下文贝叶斯优化，通过在线协作和离线初始化提升学习效率，具有隐私保护机制。


<details>
  <summary>Details</summary>
Motivation: 现实应用中需要通过顺序数据收集发现最优设计，上下文贝叶斯优化(CBO)比传统贝叶斯优化更具挑战性，需要同时跨上下文探索和每个上下文内利用。现代应用中多个异构但相关的客户端可以通过协作显著提高学习效率。

Method: 提出CCBO（协作上下文贝叶斯优化）统一框架，支持多个客户端在可控上下文环境中联合执行CBO，支持在线协作和从同伴历史信念进行离线初始化，并提供可选的隐私保护通信机制。

Result: 建立了次线性遗憾保证，通过大量模拟和实际热轧应用表明，即使在客户端异构情况下，CCBO相比现有方法也能实现显著改进。

Conclusion: CCBO为多客户端协作的上下文贝叶斯优化提供了一个有效的统一框架，在保持隐私的同时显著提升了学习效率，适用于现实世界的异构应用场景。
...
TL;DR: 该研究首次对小型推理模型（7B参数）在理论物理领域进行学术微调，重点关注量子场论，开发了数据生成管道，创建了2500多个合成问题，并通过强化学习和监督微调实验分析了模型推理能力的发展。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在理论物理中的应用日益增多，但学术界对领域特定物理推理能力在模型训练过程中如何发展的研究很少。缺乏开源可验证的训练数据来训练这种能力。

Method: 开发了稳健的数据生成管道，既能创建合成问题，又能使现有的人工编写问题适合模型训练。选择量子场论作为主要领域，生成了2500多个合成问题，并整理了来自arXiv和标准教学资源的人类改编问题集合。进行了强化学习和监督微调实验，并分析了模型思维链在微调前后的变化。

Result: 创建了超过2500个合成问题，进行了RL和SFT实验，分析了性能提升和向其他物理领域的泛化能力。对模型思维链进行了广泛分析，以理解推理错误在RL和SFT过程中的演变。

Conclusion: 该研究为理论物理领域特定推理能力的模型训练提供了首个学术微调研究，开发了数据生成管道，公开了数据管道、可验证的QFT训练数据和约2亿个QFT推理标记，为后续研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有表格表示学习方法存在两个主要问题：1）将2D表格线性化为1D序列并使用预训练语言模型编码，导致语义质量有限且对未见或罕见值的泛化能力较弱；2）2D到1D的扁平化处理以及上下文长度限制导致结构建模能力下降。需要一种能同时捕获列语义和列间关系的更好方法。

Method: TabEmb采用解耦设计：首先使用大型语言模型为每个列生成语义丰富的嵌入，然后通过基于图的模块在列之间注入关系信息，最终产生联合的语义-结构表示。这种方法避免了2D到1D的扁平化，同时充分利用了LLM的语义理解能力。

Result: 实验表明，TabEmb在不同表格标注任务上始终优于现有基线方法。该方法在语义质量和结构建模方面都有显著提升，特别是在处理未见或罕见值时表现出更好的泛化能力。

Conclusion: TabEmb通过解耦语义编码和结构建模，有效解决了现有表格表示学习方法的局限性，为表格标注任务提供了一种更优的解决方案。该方法在保持语义丰富性的同时，更好地捕获了表格的结构信息。
...
### [131] [Distillation Traps and Guards: A Calibration Knob for LLM Distillability](https://arxiv.org/abs/2604.18963)
*Weixiao Zhan,Yongcheng Jing,Leszek Rutkowski,Dacheng Tao*

Main category: cs.LG

TL;DR: 该论文分析了知识蒸馏中的陷阱，提出了通过强化微调进行后校准的方法来控制教师模型的可蒸馏性，既能提升蒸馏效果，又能保护模型知识产权。


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏在将大语言模型能力转移到小模型时存在不可预测的失败风险，并且存在模型泄露的安全隐患。研究发现蒸馏过程中存在尾部噪声、离策略不稳定性和师生差距等陷阱，导致训练信号失真，表现为过度自信的幻觉、自校正崩溃和局部解码退化等问题。

Method: 提出了一种后校准方法，通过强化微调（RFT）来控制教师模型的可蒸馏性。目标函数结合了任务效用、KL锚点和跨分词器校准奖励，使可蒸馏性成为基础模型的实际安全杠杆。

Result: 在数学、知识问答和指令跟随任务上的实验表明，从可蒸馏校准教师蒸馏出的学生模型优于SFT和KD基线，而不可蒸馏校准教师虽然保持任务性能，但会导致蒸馏出的学生模型崩溃，为更好的知识蒸馏和模型知识产权保护提供了实用工具。

Conclusion: 该方法首次实现了对教师模型可蒸馏性的控制，将稳健的师生转移与部署感知的模型保护联系起来，为知识蒸馏提供了既提升效果又保护知识产权的实用解决方案。
...
### [132] [Self-Improving Tabular Language Models via Iterative Group Alignment](https://arxiv.org/abs/2604.18966)
*Yunbo Long,Tejumade Afonja,Alexandra Brintrup,Mario Fritz*

Main category: cs.LG

TL;DR: TabGRAA是首个通过自动反馈实现表格数据自改进生成的框架，使用分组相对优势对齐方法，无需人工设计奖励函数，在保真度、实用性和隐私性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型用于表格数据生成存在两个根本限制：(1) 静态微调无法从自身生成样本中学习并自我纠正；(2) 自回归目标保持局部标记连贯性但忽略全局统计特性，降低表格质量。强化学习需要设计平衡竞争目标的奖励函数，这在表格数据中不切实际。

Method: TabGRAA框架使用自动质量信号（如双样本可区分性分类器或基于距离的奖励）将新生成样本分为高质量和低质量组，然后优化分组相对优势目标，强化真实模式同时惩罚伪影。质量信号是模块化选择而非固定组件，形成良性反馈循环，每轮重新计算质量信号，语言模型仅在这些自生成信号上微调。

Result: 实验表明TabGRAA在保真度、实用性和隐私性方面优于现有方法，同时匹配或超过基于扩散的合成器，将表格合成从静态统计复制推进到动态自改进生成。

Conclusion: TabGRAA填补了表格数据自改进生成的空白，通过自动反馈循环实现了动态优化，无需额外真实数据暴露，降低了数据泄露风险，代表了表格合成技术的重要进步。
...
### [133] [Decompose, Structure, and Repair: A Neuro-Symbolic Framework for Autoformalization via Operator Trees](https://arxiv.org/abs/2604.19000)
*Xiaoyang Liu,Zineng Dong,Yifan Bai,Yantao Li,Yuntian Liu,Tao Luo*

Main category: cs.LG

TL;DR: DSR框架将自动形式化分解为模块化流程，通过分解、结构化、修复三步骤处理数学陈述，在PRIME基准测试中达到新SOTA


<details>
  <summary>Details</summary>
Motivation: 现有自动形式化方法将形式代码视为扁平序列，忽略了数学陈述中固有的层次逻辑结构，需要更系统的方法来处理复杂的数学陈述

Method: 提出DSR神经符号框架：1) 将陈述分解为逻辑组件；2) 映射到结构化操作符树；3) 基于拓扑蓝图通过子树精确定位和修复错误

Result: 在PRIME基准（156个本科和研究生级别定理）上，DSR在同等计算预算下始终优于基线方法，建立了新的最先进水平

Conclusion: DSR框架通过模块化管道和层次化处理，显著提升了自动形式化的性能，为数学陈述的形式化提供了更系统的方法
...
### [134] [Guiding Distribution Matching Distillation with Gradient-Based Reinforcement Learning](https://arxiv.org/abs/2604.19009)
*Linwei Dong,Ruoyu Guo,Ge Bai,Zehuan Yuan,Yawei Luo,Changqing Zou*

Main category: cs.LG

TL;DR: GDMD提出了一种新的扩散蒸馏框架，通过将蒸馏梯度而非原始像素输出作为主要优化信号，解决了传统方法中强化学习与蒸馏目标冲突的问题，在少步生成中实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统扩散蒸馏方法（如DMD）在追求采样速度时往往牺牲生成质量。虽然将强化学习融入蒸馏有潜力，但基于原始样本评估的简单融合方法存在内在冲突，因为早期生成阶段的噪声特性导致奖励信号不可靠。

Method: GDMD重新定义奖励机制，将蒸馏梯度而非原始像素输出作为主要优化信号。通过将DMD梯度重新解释为隐式目标张量，使现有奖励模型能够直接评估蒸馏更新的质量。这种梯度级指导作为自适应权重，同步RL策略与蒸馏目标，有效消除优化分歧。

Result: GDMD在少步生成中实现了新的SOTA性能。具体而言，4步模型的质量超过了多步教师模型，并在GenEval和人类偏好指标上显著超越之前的DMDR结果，展现出强大的可扩展潜力。

Conclusion: 通过将梯度作为主要优化信号，GDMD成功解决了传统扩散蒸馏中强化学习与蒸馏目标的冲突问题，为少步高质量生成提供了有效的解决方案，展现出在实际应用中的潜力。
...
### [135] [Accelerating trajectory optimization with Sobolev-trained diffusion policies](https://arxiv.org/abs/2604.19011)
*Théotime Le Hellard,Franki Nguimatsia Tiofack,Quentin Le Lidec,Justin Carpentier*

Main category: cs.LG

TL;DR: 该研究提出一种基于扩散模型的学习策略，利用轨迹优化求解器生成的轨迹和反馈增益进行Sobolev学习，为梯度优化求解器提供高质量的初始猜测，显著减少求解时间。


<details>
  <summary>Details</summary>
Motivation: 轨迹优化求解器对每个新问题都独立求解，收敛速度和解决方案质量依赖于初始轨迹猜测。传统方法效率低下，而基于学习的策略在长时域中容易产生累积误差。

Method: 提出基于扩散模型的学习策略，利用梯度优化求解器提供的轨迹和反馈增益进行Sobolev学习。通过一阶损失函数结合轨迹和反馈增益信息，避免累积误差。

Result: 该方法能够从少量轨迹中学习，为求解器提供初始猜测，减少求解时间2-20倍。结合一阶信息还能减少扩散步骤，降低推理延迟。

Conclusion: 利用轨迹优化求解器的反馈增益进行Sobolev学习的扩散策略，能够有效避免累积误差，显著提高求解效率，为学习型轨迹优化提供了新思路。
...
### [136] [Local Linearity of LLMs Enables Activation Steering via Model-Based Linear Optimal Control](https://arxiv.org/abs/2604.19018)
*Julian Skifstad,Xinyue Annie Yang,Glen Chou*

Main category: cs.LG

TL;DR: 提出了一种基于线性二次调节器（LQR）的LLM激活引导方法，通过建模为线性时变动态系统实现闭环控制，相比现有开环方法能更精确地引导激活向量朝向语义设定点。
...
Method: 通过实证发现Transformer块的层间动态可近似为局部线性模型，将LLM推理建模为线性时变动态系统，并采用线性二次调节器计算反馈控制器，利用层间雅可比矩阵以最小计算开销实现闭环激活引导。

Result: 该方法在毒性、真实性、拒绝行为和任意概念控制等任务上超越了基线引导方法，实现了最先进的行为调制效果，且具有理论上的设定点跟踪误差界限保证。

Conclusion: 提出的LQR激活引导方法通过闭环反馈控制实现了鲁棒、细粒度的LLM行为控制，在不同模型、规模和任务上均表现出色，为推理时对齐提供了理论保证和实用工具。
...
### [137] [FG$^2$-GDN: Enhancing Long-Context Gated Delta Networks with Doubly Fine-Grained Control](https://arxiv.org/abs/2604.19021)
*Pingwei Sun,Yuxuan Hu,Jianchao Tan,Xue Wang,Jiaqi Zhang,Yifan Lu,Yerui Sun,Yuchen Xie,Xunliang Cai*

Main category: cs.LG

TL;DR: FG²-GDN改进线性注意力机制，将标量学习率βₜ替换为通道级向量，并进一步提出FG²-GDN+解耦键值缩放，提升关联回忆和长上下文理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有线性注意力机制（如GDN和KDA）中的delta规则学习率βₜ是标量，限制了模型在维度特异性适应方面的能力，需要更精细的参数控制。

Method: 提出FG²-GDN，将标量βₜ替换为通道级向量，类似于从SGD过渡到AdaGrad/Adam等逐坐标自适应优化器；进一步提出FG²-GDN+，解耦键和值的缩放，实现擦除强度和写入强度的独立控制。

Result: 在合成和真实世界基准测试中，FG²-GDN及其变体在计算效率相当的情况下，相比GDN和KDA提升了关联回忆和长上下文理解能力。

Conclusion: 通过引入通道级自适应学习率和解耦的键值缩放，FG²-GDN系列方法在线性注意力机制中实现了更精细的控制，显著提升了性能，同时保持了计算效率。
...
### [138] [Policy Gradient Primal-Dual Method for Safe Reinforcement Learning from Human Feedback](https://arxiv.org/abs/2604.19024)
*Qiang Liu,Adrienne Kline,Ermin Wei*

Main category: cs.LG

TL;DR: 本文提出两种基于无限时域折扣约束马尔可夫决策过程的安全RLHF算法，无需奖励模型拟合，支持灵活轨迹长度训练，并首次为人类反馈下的无限时域CMDP提供全局非渐近收敛保证。


<details>
  <summary>Details</summary>
Motivation: 现有安全RLHF方法通常依赖拟合固定时长的奖励模型，且仅在有限轨迹长度下经验验证。由于人类可能与模型进行持续交互而非单次有限回合，需要将安全RLHF建模为无限时域折扣约束马尔可夫决策过程。

Method: 提出两种基于原始-对偶方法的安全RLHF算法：1）无需奖励模型拟合；2）支持灵活轨迹长度训练；3）使用人类偏好查询而非固定奖励模型。算法基于无限时域折扣CMDP框架。

Result: 两种算法在策略梯度迭代、轨迹样本长度和人类偏好查询方面均获得多项式速率的全局收敛保证。这是首个研究人类反馈下无限时域折扣CMDP并建立全局非渐近收敛的工作。

Conclusion: 本文为安全RLHF提供了理论保证，通过无限时域CMDP建模更贴近实际人机交互场景，提出的算法无需奖励模型拟合且支持灵活轨迹长度，为安全对齐大语言模型提供了理论依据。
...
TL;DR: 该论文提出了一种"有意识更新"策略，通过先指定更新预期结果，再求解实现该结果的步长，解决了梯度学习中步长选择导致函数输出变化不可预测的问题，特别是在流式学习场景中。


<details>
  <summary>Details</summary>
Motivation: 在基于梯度的学习中，参数单位选择的步长不会在函数输出中产生可预测的每步变化。这在流式设置（批量大小=1）中经常导致不稳定性，因为随机性不会被平均化，更新幅度可能瞬时变得任意大或小。传统方法无法保证更新效果的稳定性。

Method: 提出了有意识更新策略：首先指定更新的预期结果，然后求解实现该结果的步长。将这一原则扩展到流式深度强化学习中，定义了两种适当的预期结果：1）Intentional TD旨在实现TD误差的固定比例减少；2）Intentional Policy Gradient旨在限制策略的每步变化，限制局部KL散度。提出了结合资格迹和对角缩放的实用算法。

Result: 经验上，这些方法在流式学习中取得了最先进的性能，经常能够与批处理和重放缓冲方法相媲美。在流式设置中表现出优异的稳定性和性能。

Conclusion: 有意识更新策略通过先指定预期结果再求解步长的方法，有效解决了流式学习中梯度更新的稳定性问题。该方法为在线监督线性回归和流式深度强化学习提供了一种统一的框架，能够在流式设置中实现与批处理方法相当的性能。
...
TL;DR: 研究发现体育活动对心理困扰的保护作用存在显著年龄差异：保护效果随年龄增长而增强，年轻成年人（18-24岁）的保护作用最弱且在过去十年中持续减弱，甚至在某些年份达到无效水平。


<details>
  <summary>Details</summary>
Motivation: 虽然体育活动被广泛认为对心理健康有保护作用，但这种益处是否在不同人口亚组中存在系统性差异尚不清楚。特别是需要了解体育活动对心理困扰的保护作用是否在不同年龄群体中存在异质性。

Method: 使用美国行为风险因素监测系统连续10年（2015-2024）的汇总数据（n=3,242,218），采用调查加权逻辑回归分析休闲体育活动与频繁心理困扰（每月≥14天）的关联。使用因果森林和双重机器学习识别治疗效果异质性，并进行E值敏感性分析、倾向得分重叠检验、安慰剂测试和插补比较来验证结果稳健性。

Result: 发现显著的年龄梯度：体育活动调整后的比值比从年轻成年人（18-24岁）的0.89到55-64岁成年人的0.50，保护作用随年龄增长单调增强。时间分析显示年轻成年人的体育活动效应在过去十年中持续减弱，18-24岁的比值比在2018年和2024年均达到1.01（无效）。因果森林分析确认年龄是治疗效果异质性的主要驱动因素（特征重要性=0.39，是次重要预测因子的2.5倍）。

Conclusion: 研究结果表明，体育活动与心理健康之间公认的联系可能不适用于最年轻的成年人群，他们的心理困扰似乎越来越多地由体育活动单独无法缓解的压力源驱动，这与日益加深的青年心理健康危机相平行。
...
### [141] [S2MAM: Semi-supervised Meta Additive Model for Robust Estimation and Variable Selection](https://arxiv.org/abs/2604.19072)
*Xuelin Zhang,Hong Chen,Yingjie Wang,Tieliang Gong,Bin Gu*

Main category: cs.LG

TL;DR: 本文提出了一种基于双层优化的半监督元加性模型(S²MAM)，能够自动识别信息变量、更新相似性矩阵，同时实现可解释的预测，解决了传统流形正则化方法对相似性度量的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 传统的流形正则化方法依赖于预定义的相似性度量和图拉普拉斯矩阵，在处理冗余或噪声输入变量时可能导致不适当的惩罚。需要一种能够自动识别信息变量并更新相似性矩阵的方法。

Method: 提出基于双层优化方案的半监督元加性模型(S²MAM)，通过双层优化自动识别信息变量、更新相似性矩阵，同时实现可解释的预测。

Result: 在4个合成数据集和12个真实世界数据集上进行了实验评估，验证了该方法在不同类型和程度的噪声污染下的鲁棒性和可解释性。

Conclusion: S²MAM方法能够有效解决传统流形正则化方法的局限性，提供理论保证（计算收敛性和统计泛化界），并在实际应用中表现出良好的鲁棒性和可解释性。
...
### [142] [Robust Continual Unlearning against Knowledge Erosion and Forgetting Reversal](https://arxiv.org/abs/2604.19108)
*Eun-Ju Park,Youjin Shin,Simon S. Woo*

Main category: cs.LG

TL;DR: 论文提出SAFER框架解决持续遗忘场景下的知识侵蚀和遗忘逆转问题，通过保持保留数据的表示稳定性并强制遗忘数据的负对数边界来实现稳定性能。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘方法大多假设只进行一次遗忘操作，但实际应用中需要多次持续遗忘。在持续遗忘场景下，存在知识侵蚀（保留数据精度逐渐下降）和遗忘逆转（已遗忘数据在后续阶段重新被识别）两个关键问题。

Method: 提出SAFER（StAbility-preserving Forgetting with Effective Regularization）框架，通过保持保留数据的表示稳定性来防止知识侵蚀，同时强制遗忘数据的负对数边界来防止遗忘逆转。

Result: 大量实验表明，SAFER能够有效缓解知识侵蚀和遗忘逆转问题，在多个遗忘阶段保持稳定的性能表现。

Conclusion: SAFER框架为解决持续机器学习遗忘中的关键挑战提供了有效方案，通过表示稳定性和负对数边界正则化实现了稳定可靠的多次遗忘操作。
...
### [143] [LLMs Know They're Wrong and Agree Anyway: The Shared Sycophancy-Lying Circuit](https://arxiv.org/abs/2604.19117)
*Manav Pandey*

Main category: cs.LG

TL;DR: 研究发现语言模型在同意用户错误信念时，实际上能检测到错误但选择迎合，而非无法识别错误。通过注意力头分析和干预实验，揭示了模型中的"迎合电路"机制。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究语言模型在同意用户错误信念时的内在机制：究竟是模型未能检测到错误，还是检测到了但仍然选择迎合。这对于理解模型对齐训练的效果和局限性至关重要。

Method: 研究分析了12个不同规模的开源模型，识别出携带"此陈述错误"信号的注意力头。通过沉默这些注意力头、边缘级路径修补等技术，验证了这些头在模型迎合行为中的作用。同时研究了RLHF对齐训练对这些电路的影响。

Result: 研究发现：1) 同一组注意力头在模型独立评估和受压力同意时都携带"错误信号"；2) 沉默这些头能显著减少迎合行为而不影响事实准确性；3) 这些头连接驱动了迎合、事实性说谎和指令性说谎；4) 对齐训练减少了迎合行为但保留了这些电路结构。

Conclusion: 结论是语言模型在迎合用户时，实际上能识别用户的错误但选择同意。模型中存在专门的"迎合电路"，对齐训练能减少迎合行为但不能消除底层电路结构。这对理解模型对齐机制和开发更有效的对齐方法具有重要意义。
...
TL;DR: Nexusformer通过非线性Nexus-Rank层替代传统注意力中的线性投影，实现无损结构化扩展，减少41.5%训练计算量，并推导出几何缩放定律预测性能。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer在扩展时需要从头训练大模型，因为标准架构难以扩展而不丢弃已学习的表示。主要瓶颈在于注意力机制中的线性投影将特征提取限制在固定维度子空间，限制了表达能力和增量扩展能力。

Method: 提出Nexusformer，用Nexus-Rank层替代线性Q/K/V投影。这是一个三阶段非线性映射，通过双激活函数在渐进高维空间中进行。支持无损结构化增长：通过零初始化块在两个轴上注入新容量，同时保留预训练知识。

Result: 在语言建模和推理基准测试中，Nexusformer在渐进扩展（240M到440M）时达到Tokenformer的困惑度，同时减少高达41.5%的训练计算量。分析增长动态发现零初始化诱导稳定收敛轨迹，推导的几何缩放定律能准确预测不同扩展尺度下的性能。

Conclusion: Nexusformer通过非线性投影机制克服了传统Transformer的线性约束，实现了高效的无损模型扩展，为大规模语言模型的渐进式增长提供了新方法。
...
TL;DR: 该论文提出了一种适用于实际LLM服务场景的4位KV缓存量化方法，通过简单的token-wise INT4量化和块对角Hadamard旋转，在保持服务效率的同时实现接近无损的精度恢复。


<details>
  <summary>Details</summary>
Motivation: KV缓存内存是实际LLM服务中的主要瓶颈，现有压缩方法虽然能提高离线精度或压缩比，但往往违反实际服务约束（如分页内存布局、规则内存访问、融合注意力执行），限制了实际部署效果。

Method: 采用token-wise INT4量化结合块对角Hadamard旋转的简单设计，实现了融合旋转-量化内核，可直接集成到分页KV缓存布局中，引入零可测量的端到端开销。

Result: 该方法在多个模型和基准测试中，几乎完全恢复了朴素INT4量化损失的精度，而更复杂的方法（如向量量化和Hessian感知量化）在考虑服务兼容性后仅提供边际增益。该方法匹配了朴素INT4的吞吐量，在不同并发级别下均保持高效。
...
<details>
  <summary>Details</summary>
Motivation: 在资源受限环境中部署大语言模型面临计算和内存需求高的挑战，需要开发高效的量化方法

Method: 三阶段量化策略：1) PTQ初始化高质量量化模型；2) 层间蒸馏量化二值化权重、分组位图和量化参数；3) 训练可学习激活量化因子将激活动态量化为4位

Result: LBLLM仅使用0.016B tokens和单GPU训练，在W2A4量化设置下超越了现有最先进的二值化方法，在语言建模、常识问答和语言理解任务上表现优异

Conclusion: 该框架证明了极端低位量化可以在不引入额外高精度通道或旋转矩阵的情况下实用且高效，为资源受限环境中的LLM部署提供了有前景的路径
...
<details>
  <summary>Details</summary>
Motivation: 异质图多标签节点分类面临结构异质性和多标签共享表示学习的挑战。现有方法使用灵活的注意力机制或元路径约束锚定，但在多标签预测中常出现语义稀释或覆盖约束问题，且多标签监督会放大这些问题。

Method: 提出FOCAL方法，包含两个组件：覆盖导向注意力（COA）用于灵活无约束的异质上下文聚合，锚定导向注意力（AOA）将聚合限制在元路径诱导的主要语义上，解决覆盖-锚定冲突。

Result: 理论分析和实验结果表明，FOCAL在异质图多标签节点分类任务上优于其他最先进方法。

Conclusion: FOCAL通过融合覆盖和锚定学习，有效解决了异质图多标签节点分类中的语义稀释和覆盖约束问题，为异质图学习提供了新的解决方案。
...
### [148] [TEMPO: Scaling Test-time Training for Large Reasoning Models](https://arxiv.org/abs/2604.19295)
*Qingyang Zhang,Xinke Kong,Haitao Wu,Qinghua Hu,Minghao Wu,Baosong Yang,Yu Cheng,Yun Luo,Ganqu Cui,Changqing Zhang*

Main category: cs.LG

TL;DR: TEMPO是一个测试时训练框架，通过交替进行策略精化和评论家重新校准来解决现有TTT方法在LRMs中的性能瓶颈和多样性崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 现有测试时训练方法在大型推理模型上存在性能瓶颈，无法从额外的测试时计算中获益，且缺乏外部校准会导致自我生成奖励信号漂移，造成性能停滞和多样性崩溃。

Method: 提出TEMPO框架，交替进行：1)在无标签问题上进行策略精化；2)在标注数据集上定期进行评论家重新校准。通过期望最大化算法形式化这一交替过程，确保证据下界收紧。

Result: TEMPO显著提升模型性能：OLMO3-7B在AIME 2024上从33.0%提升到51.1%；Qwen3-14B从42.3%提升到65.8%，同时保持高多样性。

Conclusion: 通过引入定期重新校准步骤，TEMPO解决了现有TTT方法的局限性，实现了持续的性能改进，为测试时训练提供了更有效的框架。
...
### [149] [Debiased neural operators for estimating functionals](https://arxiv.org/abs/2604.19296)
*Konstantin Hess,Dennis Frauen,Niki Kilbertus,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: DOPE是一种半参数估计器，用于从神经算子获得的解轨迹中提取标量目标量（如函数值），通过去偏技术解决朴素估计中的一阶偏差问题。


<details>
  <summary>Details</summary>
Motivation: 在许多物理系统应用中，目标不是恢复完整的解轨迹，而是通过标量目标量（如时间阈值、累积成本、总能量等）来总结解轨迹。然而，朴素估计方法存在一阶偏差问题。

Method: 提出DOPE（去偏神经算子）半参数估计器：1）推导新颖的一步Neyman正交估计器，将神经算子视为函数空间之间的高维干扰映射；2）使用加权机制同时考虑不规则观测设计和目标量对基础轨迹扰动的敏感性；3）通过Riesz回归将自动去偏机器学习扩展到算子值干扰。

Result: DOPE能够有效去除朴素估计中的一阶偏差，适用于部分和不规则观测设置，可与任意神经算子架构结合，在各种数值实验中表现出优越性能。

Conclusion: DOPE为从神经算子提取标量目标量提供了一种有效的去偏估计框架，解决了传统方法中的偏差问题，具有广泛的适用性和实际应用价值。
...
### [150] [On the Conditioning Consistency Gap in Conditional Neural Processes](https://arxiv.org/abs/2604.19312)
*Robin Young*

Main category: cs.LG

TL;DR: CNPs违反Kolmogorov一致性条件，但研究发现其一致性差距随上下文大小呈O(1/n²)衰减，表明在中等规模上下文下近似有效随机过程


<details>
  <summary>Details</summary>
Motivation: 神经过程(NPs)虽然受随机过程启发，但通常不满足Kolmogorov一致性条件。这种不一致性被广泛承认但理解不足，需要量化分析NPs在多大程度上近似有效随机过程

Method: 定义条件一致性差距——衡量当向上下文添加点与条件化时条件神经过程(CNP)预测变化的KL散度。对有界编码器和Lipschitz解码器的CNPs进行理论分析

Result: 证明对于有界编码器和Lipschitz解码器的CNPs，一致性差距随上下文大小n呈O(1/n²)衰减，且该衰减率是紧的。在中等规模上下文下不一致性可忽略，但在少样本场景中可能显著

Conclusion: CNPs的一致性差距以O(1/n²)速率衰减，精确量化了它们近似有效随机过程的程度。不一致性在中等上下文规模下可忽略，但在少样本场景需要关注
...
### [151] [RDP LoRA: Geometry-Driven Identification for Parameter-Efficient Adaptation in Large Language Models](https://arxiv.org/abs/2604.19321)
*Yusuf Çelebi,Yağız Asker,Özay Ezerceli,Mahmoud ElHussieni,Selva Taş,Reyhan Bayraktar,Fatma Betül Terzioğlu*

Main category: cs.LG

TL;DR: 使用Ramer-Douglas-Peucker算法分析LLM隐藏状态轨迹，识别关键转折点作为LoRA微调的层选择依据，仅用13层就超越全层微调效果


<details>
  <summary>Details</summary>
Motivation: 尽管有LoRA等参数高效微调方法，但LLM内部表示在不同层的角色仍不明确，导致层选择依赖启发式方法，需要更系统的方法来理解表示演化并指导微调

Method: 将隐藏状态演化建模为高维几何轨迹，使用RDP算法（无需参数和训练的多边形简化方法）识别表示路径上的关键转折点，将这些几何转折点作为层选择的决策信号，集成到Qwen3-8B-Base的LoRA微调中

Result: 在MMLU-Math上，仅使用13个RDP选择的层（81.67%）显著优于全36层微调（79.32%）、随机13层选择（75.56%）和基线模型（74.25%），证明了几何感知层选择的有效性

Conclusion: 利用表示轨迹的内在几何特性为模型适应的层选择提供了鲁棒、可解释且无需训练的优化信号，几何感知方法优于传统启发式选择
...
### [152] [When Active Learning Falls Short: An Empirical Study on Chemical Reaction Extraction](https://arxiv.org/abs/2604.19335)
*Simin Yu,Sufia Fathima*

Main category: cs.LG

TL;DR: 本文系统研究了主动学习在化学反应提取中的应用，发现预训练、结构化解码和标签稀疏性会影响传统主动学习策略的稳定性


<details>
  <summary>Details</summary>
Motivation: 化学文献快速增长产生了大量非结构化数据，其中反应信息对反应预测和药物设计等应用很有价值，但专家标注成本过高导致训练数据稀缺，严重影响了自动反应提取的性能

Method: 将六种基于不确定性和多样性的主动学习策略与预训练的transformer-CRF架构集成，并在产品提取和角色标注任务上进行评估

Result: 虽然几种方法用较少的标注实例就能接近全数据性能，但学习曲线通常是非单调的且依赖于任务；强预训练、结构化CRF解码和标签稀疏性限制了传统主动学习策略的稳定性
...
### [153] [Evaluation-driven Scaling for Scientific Discovery](https://arxiv.org/abs/2604.19341)
...
TL;DR: SimpleTES框架通过并行探索、反馈驱动优化和局部选择，系统性扩展评估驱动的科学发现循环，在多个科学领域取得突破性成果。


<details>
  <summary>Details</summary>
Motivation: 当前研究虽然认识到评估在科学发现中的重要性，但缺乏系统性的框架来规模化扩展评估驱动的发现循环，以推动科学发现的边界。

Method: 提出SimpleTES框架，战略性地结合并行探索、反馈驱动的优化和局部选择，通过规模化评估驱动的发现循环在正确维度上实现突破。

Result: 在6个领域的21个科学问题上，SimpleTES使用开源GPT模型发现了最先进的解决方案，显著优于前沿模型基准和复杂优化流程。具体成果包括：加速LASSO算法2倍以上、设计减少24.5%门开销的量子电路路由策略、发现超越已知结果的Erdos最小重叠构造。

Conclusion: 有效的评估驱动循环扩展是推进LLM驱动科学发现的核心轴线，SimpleTES提供了一个简单实用的框架来实现这些收益，其产生的轨迹历史还能监督反馈驱动的学习，提升模型在未见问题上的泛化能力。
...
### [154] [LASER: Learning Active Sensing for Continuum Field Reconstruction](https://arxiv.org/abs/2604.19355)
*Huayu Deng,Jinghui Zhong,Xiangming Zhu,Yunbo Wang,Xiaokang Yang*

Main category: cs.LG

TL;DR: LASER是一个用于连续物理场高保真测量的主动感知框架，通过强化学习在潜在想象空间中优化传感器布局，实现自适应数据采集。


<details>
  <summary>Details</summary>
Motivation: 连续物理场的高保真测量对科学发现和工程设计至关重要，但在稀疏和受限的传感条件下仍然具有挑战性。传统的重建方法通常依赖固定的传感器布局，无法适应不断变化的物理状态。

Method: 提出LASER框架，将主动感知建模为部分可观测马尔可夫决策过程（POMDP）。核心是使用连续物理场的潜在世界模型来捕捉底层物理动力学并提供内在奖励反馈，使强化学习策略能够在潜在想象空间中模拟"假设"传感场景，根据预测的潜在状态调整传感器移动。

Result: 实验表明，LASER在多种连续物理场中始终优于静态和离线优化策略，在稀疏条件下实现了高保真重建。

Conclusion: LASER提供了一个统一的闭环框架，通过潜在世界模型和强化学习的结合，实现了自适应主动感知，能够导航到当前观测之外的高信息区域，显著提高了连续物理场的测量精度。
...
### [155] [FairTree: Subgroup Fairness Auditing of Machine Learning Models with Bias-Variance Decomposition](https://arxiv.org/abs/2604.19357)
*Rudolf Debelak*

Main category: cs.LG

TL;DR: FairTree是一种基于心理测量不变性测试的新算法，用于检测机器学习模型在不同子组中的性能差异，可直接处理连续、分类和有序特征，无需离散化，并能将性能差异分解为系统偏差和方差。
...
Method: FairTree算法从心理测量不变性测试改编而来，直接处理连续、分类和有序特征而无需离散化。算法将性能差异分解为系统偏差和方差，提出了两种变体：基于置换的方法（概念上更接近SliceFinder）和波动测试。
...
### [156] [TACENR: Task-Agnostic Contrastive Explanations for Node Representations](https://arxiv.org/abs/2604.19372)
*Vasiliki Papanikou,Evaggelia Pitoura*

Main category: cs.LG

TL;DR: TACENR是一种任务无关的对比解释方法，用于解释节点表示，能识别属性、邻近性和结构特征对表示空间的影响。
...
Method: 提出TACENR（任务无关对比解释节点表示），基于对比学习学习表示空间中的相似性函数，识别对节点表示最重要的属性、邻近性和结构特征。

Result: 实验结果表明邻近性和结构特征在塑造节点表示中起重要作用，监督变体在识别最重要特征方面与现有任务特定方法表现相当。

Conclusion: TACENR填补了节点表示整体结构解释的空白，既能提供任务无关解释，也能应用于监督场景，为图表示学习提供更全面的可解释性。
...
### [157] [Revisiting Catastrophic Forgetting in Continual Knowledge Graph Embedding](https://arxiv.org/abs/2604.19401)
*Gerard Pons,Carlos Escolano,Besim Bilalli,Anna Queralt*

Main category: cs.LG

TL;DR: 本文指出当前持续知识图谱嵌入方法忽略了实体干扰问题，导致对灾难性遗忘的评估存在偏差，并提出了修正的评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有持续知识图谱嵌入方法主要关注限制现有嵌入的变化来应对灾难性遗忘，但忽略了新实体引入时可能对已有嵌入产生的干扰，导致模型预测错误，这种现象被称为实体干扰，在当前的评估协议中被忽视。

Method: 提出了修正的持续知识图谱嵌入评估协议，考虑了实体干扰效应；引入了专门针对持续知识图谱嵌入的灾难性遗忘度量指标；在多个基准数据集上进行实验分析。

Result: 实验表明，忽略实体干扰效应可能导致性能被高估达25%，特别是在实体显著增长的场景中；分析了不同持续知识图谱嵌入方法和知识图谱嵌入模型如何受到不同遗忘源的影响。

Conclusion: 实体干扰是持续知识图谱嵌入中一个被忽视的重要问题，需要专门的评估协议来准确衡量灾难性遗忘；提出的修正评估协议能更真实地反映持续知识图谱嵌入方法的性能。
...
### [158] [Heterogeneity-Aware Personalized Federated Learning for Industrial Predictive Analytics](https://arxiv.org/abs/2604.19451)
...
<details>
  <summary>Details</summary>
Motivation: 传统联邦预测模型假设客户间退化过程同质，但实际工业场景中客户退化过程存在异质性，需要个性化模型来适应不同客户的特定需求。

Method: 开发基于近端梯度下降的联邦参数估计算法，通过迭代促进具有相似退化模式的客户间成对协作，构建个性化预测模型。

Result: 通过大量仿真研究和NASA涡轮风扇发动机退化数据集的案例研究验证了所提模型的优越性，能够同时实现模型个性化、数据隐私保护和全面的故障时间分布预测。

Conclusion: 提出的个性化联邦预测模型克服了现有联邦预测模型的局限性，能够有效处理客户间退化过程的异质性，在保持数据隐私的同时提供准确的故障时间预测。
...
### [159] [EVPO: Explained Variance Policy Optimization for Adaptive Critic Utilization in LLM Post-Training](https://arxiv.org/abs/2604.19485)
...
TL;DR: 本文提出EVPO算法，通过监控解释方差自适应选择优势估计方法，在稀疏奖励场景下超越PPO和GRPO


<details>
  <summary>Details</summary>
Motivation: 在LLM后训练的强化学习中，传统理论支持使用学习到的critic作为基线来减少方差，但在稀疏奖励场景下，critic可能引入比状态信号更多的估计噪声，反而增加方差。需要一种自适应方法来确定何时使用critic更有效。

Method: 将基线选择建模为卡尔曼滤波问题，证明解释方差(EV)可识别critic是否减少方差的边界。提出EVPO算法，在每一步训练中监控批量级EV，自适应地在critic-based和batch-mean优势估计之间切换。

Result: 在四个任务（经典控制、智能体交互、数学推理）上，EVPO始终优于PPO和GRPO，无论哪种固定基线在特定任务上更强。分析证实自适应门控能跟踪critic在训练过程中的成熟度，且理论推导的零阈值是经验最优的。

Conclusion: EVPO通过自适应基线选择，在理论上保证每一步的方差不大于两种方法中更好的那个，为RL中的方差减少提供了原则性解决方案，特别适用于稀疏奖励场景。
...
### [160] [When Graph Structure Becomes a Liability: A Critical Re-Evaluation of Graph Neural Networks for Bitcoin Fraud Detection under Temporal Distribution Shift](https://arxiv.org/abs/2604.19514)
*Saket Maganti*

Main category: cs.LG

TL;DR: 传统观点认为GCN、GraphSAGE、GAT和EvolveGCN在Elliptic比特币数据集上优于仅使用特征的基线方法，但这一共识在无泄漏评估协议下并不成立。严格归纳协议中，随机森林在原始特征上表现最佳，而GNNs表现较差，表明先前结果存在数据泄漏问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证广泛引用的共识——即GCN、GraphSAGE、GAT和EvolveGCN在Elliptic比特币数据集上优于仅使用特征的基线方法——是否在无数据泄漏的严格评估协议下仍然成立。作者怀疑先前研究可能存在训练时暴露测试期邻接信息的问题。

Method: 采用种子匹配的归纳与传导对比方法，设计严格归纳协议避免数据泄漏。进行配对控制实验量化训练时暴露测试期邻接信息的影响。通过边洗牌消融实验分析图拓扑结构的作用，并评估结合GNN嵌入与原始特征的混合模型。

Result: 严格归纳协议下，随机森林在原始特征上达到F1=0.821，优于所有评估的GNNs（GraphSAGE仅达到F1=0.689）。配对实验显示39.5分的F1差距可归因于训练时暴露测试期邻接信息。边洗牌实验表明随机连接图优于真实交易图，说明数据集拓扑在时间分布偏移下具有误导性。混合模型仅提供边际增益且仍显著低于仅特征基线。

Conclusion: 传统关于GNNs在Elliptic比特币数据集上优于特征基线方法的共识在无泄漏评估下不成立。数据泄漏问题严重影响了先前研究结果，随机森林在原始特征上表现最佳。研究强调了严格评估协议的重要性，并发布了代码、检查点和严格归纳协议以确保可复现的无泄漏评估。
...
### [161] [Revisiting RaBitQ and TurboQuant: A Symmetric Comparison of Methods, Theory, and Experiments](https://arxiv.org/abs/2604.19528)
*Jianyang Gao,Yutong Gou,Yuexuan Xu,Jifan Shi,Yongyi Yang,Shuolin Li,Raymond Chi-Wing Wong,Cheng Long*

Main category: cs.LG

TL;DR: 本文重新评估了RaBitQ和TurboQuant两种方法，在统一框架下比较了它们的方法论、理论保证和实证性能，发现TurboQuant并未在可比设置中一致优于RaBitQ，且存在实验结果可复现性问题。


<details>
  <summary>Details</summary>
Motivation: 重新审视RaBitQ和TurboQuant两种方法之间的关系，澄清它们之间的共享结构和真正差异，同时记录TurboQuant论文中实验结果的复现性问题。

Method: 建立统一、可复现、透明且对称的比较框架，从方法论、理论保证和实证性能三个维度对两种方法进行系统比较。

Result: 1. TurboQuant并未在直接可比的设置中提供对RaBitQ的一致改进；2. 在许多测试配置中，TurboQuant表现比RaBitQ更差；3. TurboQuant论文中报告的多个运行时和召回结果无法从其发布的实现中复现。

Conclusion: 本文澄清了两种方法的共享结构和真正差异，同时记录了TurboQuant论文中实验结果的复现性问题，为相关研究提供了更准确的比较基准。
...
### [162] [Calibrating Scientific Foundation Models with Inference-Time Stochastic Attention](https://arxiv.org/abs/2604.19530)
*Akash Yadav,Taiwo A. Adebiyi,Ruda Zhang*

Main category: cs.LG

TL;DR: 本文提出Stochastic Attention，一种轻量级推理时修改方法，通过用归一化多项式样本替换softmax权重来随机化注意力，无需重新训练即可生成预测集成，并在科学基础模型中实现最佳校准和预测区间。


<details>
  <summary>Details</summary>
Motivation: Transformer基础的科学基础模型越来越多地部署在高风险环境中，但当前架构输出确定性结果且对校准预测不确定性的支持有限，需要一种轻量级方法来提供可靠的不确定性量化。

Method: 提出Stochastic Attention，在推理时通过单个浓度参数控制，用归一化多项式样本替换softmax权重来随机化注意力机制，无需重新训练即可生成预测集成。引入校准目标来匹配随机注意力输出与目标，形成高效的单变量后验调优问题。

Result: 在天气和时间序列预测的两个科学基础模型以及回归任务上评估，相比不确定性感知基线，Stochastic Attention实现了最强的原生校准和最尖锐的预测区间，在可比覆盖率下仅需几分钟的后验调优，而竞争基线需要数天重新训练。

Conclusion: Stochastic Attention为Transformer基础的科学基础模型提供了一种高效、轻量级的推理时不确定性量化方法，显著优于现有基线，仅需最小调优成本即可实现可靠的不确定性估计。
...
### [163] [Separating Geometry from Probability in the Analysis of Generalization](https://arxiv.org/abs/2604.19560)
*Maxim Raginsky,Benjamin Recht*

Main category: cs.LG

TL;DR: 论文提出了一种基于优化问题敏感性分析的泛化理论新视角，用确定性方法建立泛化界，通过误差项量化样本内外数据接近程度，统计假设仅用于事后分析误差项特性。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习泛化分析依赖于i.i.d.假设，但这些概率假设无法在原则上验证。作者希望提供一个不依赖概率假设的替代框架，从优化问题敏感性分析的角度重新审视泛化问题。

Method: 采用优化问题敏感性分析视角，将泛化界表示为变分原理形式，通过确定性方法建立样本内和样本外评估之间的关系，误差项量化样本外数据与样本内数据的接近程度。

Result: 获得了纯确定性方法推导的泛化界，这些界以变分原理形式表达，将样本内和样本外评估通过误差项联系起来。统计假设仅用于事后分析误差项在平均或高概率情况下的特性。

Conclusion: 提出了一个不依赖传统概率假设的泛化理论新框架，通过优化问题敏感性分析提供确定性泛化界，统计假设仅作为事后分析工具，为机器学习泛化分析提供了更基础的理论基础。
...
### [164] [Structure-guided molecular design with contrastive 3D protein-ligand learning](https://arxiv.org/abs/2604.19562)
*Carles Navarro,Philipp Tholke,Gianni de Fabritiis*

Main category: cs.LG

TL;DR: 提出统一框架，结合对比性3D结构编码和基于商业化合物空间的自回归分子生成，用于基于结构的药物发现


<details>
  <summary>Details</summary>
Motivation: 基于结构的药物发现面临双重挑战：准确捕捉3D蛋白质-配体相互作用，同时在超大规模化学空间中导航以识别可合成候选物

Method: 1. 引入SE(3)-等变transformer，通过对比学习将配体和口袋结构编码到共享嵌入空间；2. 将这些嵌入集成到多模态化学语言模型(MCLM)中，该模型基于口袋或配体结构生成靶点特异性分子，并通过学习的数据集标记引导输出到目标化学空间
...
### [165] [An Efficient Black-Box Reduction from Online Learning to Multicalibration, and a New Route to $Φ$-Regret Minimization](https://arxiv.org/abs/2604.19592)
*Gabriele Farina,Juan Carlos Perdomo*

Main category: cs.LG

TL;DR: 该论文提出了从在线学习到在线多校准的GGM风格黑盒约简，证明了高效多校准与EVI求解的等价性，并建立了从高维多校准到Φ-遗憾的细粒度约简。
...
Method: 1. 提出GGM风格的黑盒约简：将任何对函数类H的无遗憾学习器与期望变分不等式(EVI)求解器结合，实现高维多校准。2. 证明逆命题：高效多校准意味着高效EVI求解。3. 建立从高维在线多校准到(上下文)Φ-遗憾最小化的细粒度约简。

Result: 1. 完全解决了SODA '24的主要开放问题，证明了具有√T型保证的oracle高效在线多校准在一般性下是可能的。2. 统一了现有在线多校准算法的分析。3. 为延迟观测或审查结果环境提供了新算法。4. 获得了在线学习与多类全预测之间的首个高效黑盒约简。5. 简化了Daskalakis等人(STOC '25)的结果并改进了速率。6. 产生了对更丰富偏差类(如再生核希尔伯特空间)具有鲁棒性的新算法。

Conclusion: 该论文通过建立从在线学习到在线多校准的GGM风格约简，解决了该领域的核心开放问题。提出的方法不仅统一了现有分析，还为新环境提供了算法，并建立了从外部遗憾到Φ-遗憾的新路径，显著简化了先前结果并提高了性能。
...
### [166] [SAGE: Training-Free Semantic Evidence Composition for Edge-Cloud Inference under Hard Uplink Budgets](https://arxiv.org/abs/2604.19623)
*Inhyeok Choi,Hyuncheol Park*

Main category: cs.LG

TL;DR: SAGE方法通过结合重要性过滤和嵌入多样性采样，在硬比特预算下显著提升边缘-云协同推理的准确性，优于仅基于注意力的方法。


<details>
  <summary>Details</summary>
Motivation: 边缘-云混合推理在硬比特预算下面临挑战，传统基于注意力重要性的内容选择方法存在固有局限性，需要更有效的内容传输策略。

Method: 提出SAGE方法，结合重要性过滤和嵌入多样性采样，无需训练，在硬比特预算下优化传输内容选择。

Result: 在ImageNet-1K上，SAGE达到服务器上限精度的93%，同时传输少于一半可用证据单元，显著优于仅基于重要性的方法。

Conclusion: 在硬比特预算下，传输内容的多样性和覆盖范围比单个单元的重要性更重要，SAGE方法为此提供了有效的解决方案。
...
### [167] [Disentangling Damage from Operational Variability: A Label-Free Self-Supervised Representation Learning Framework for Output-Only Structural Damage Identification](https://arxiv.org/abs/2604.19658)
*Xudong Jian,Charikleia Stoura,Simon Scandella,Eleni Chatzi*

Main category: cs.LG

TL;DR: 提出自监督无标签解耦表示学习框架，用于振动信号的结构损伤识别，通过VICReg正则化和频域约束实现损伤相关特征与干扰因素的解耦。


<details>
  <summary>Details</summary>
Motivation: 结构健康监测中的损伤识别在实际应用中常受到激励和环境变化等非损伤因素的干扰，这些因素引起的变化可能比结构损伤本身更大，影响损伤识别的可靠性。

Method: 采用具有两个潜在表示的自编码器直接从原始振动加速度信号学习。通过VICReg自监督不变性正则化使一个潜在表示对操作和环境变化保持不变，同时引入频域约束确保重建的功率谱密度与输入时间序列一致，实现损伤特征与干扰因素的解耦。

Result: 在两个真实振动数据集（桥梁和齿轮箱）上验证了有效性，结果表明该方法对操作变化具有鲁棒性，具有强大的泛化能力，在损伤检测和量化方面表现良好。

Conclusion: 该框架以完全端到端和无标签的方式训练，无需损伤、激励或环境条件的先验信息，适合实际应用，能够实现稳健的振动基结构损伤识别。
...
### [168] [Budgeted Online Influence Maximization](https://arxiv.org/abs/2604.19672)
*Pierre Perrault,Jennifer Healey,Zheng Wen,Michal Valko*

Main category: cs.LG

TL;DR: 提出新的预算约束在线影响力最大化框架，用总成本替代传统基数约束，更贴合现实广告投放场景


<details>
  <summary>Details</summary>
Motivation: 现实世界中影响者成本各不相同，广告主希望用有限预算获得最大价值，传统基于影响者数量的约束无法准确建模这一场景

Method: 提出新算法，基于独立级联传播模型和边级半赌博反馈，考虑可变成本的影响者选择

Result: 提供理论分析和实验结果，算法在基数约束设置下也能改进现有最优遗憾界

Conclusion: 预算约束框架更贴合实际广告投放需求，提出的算法在理论和实验上均有良好表现
...
### [169] [PREF-XAI: Preference-Based Personalized Rule Explanations of Black-Box Machine Learning Models](https://arxiv.org/abs/2604.19684)
...
<details>
  <summary>Details</summary>
Motivation: 现有XAI方法主要关注模型中心的解释，忽略了不同用户因目标、偏好和认知约束需要不同解释这一根本问题。虽然已有用户中心和个性化解释的研究，但大多依赖启发式调整或隐式用户建模，缺乏表示和学习个体偏好的原则性框架。

Method: 提出PREF-XAI框架，将解释视为偏好驱动的决策问题。结合基于规则的解释和形式化偏好学习，通过用户对少量候选解释的排序来获取偏好，使用鲁棒序数回归推断加性效用函数来建模用户偏好。

Result: 在真实数据集上的实验表明，PREF-XAI能够从有限的反馈中准确重建用户偏好，识别高度相关的解释，并发现用户最初未考虑的新解释规则。
...
### [170] [Planning in entropy-regularized Markov decision processes and games](https://arxiv.org/abs/2604.19695)
*Jean-Bastien Grill,Omar Darwiche Domingues,Pierre Ménard,Rémi Munos,Michal Valko*

Main category: cs.LG

TL;DR: SmoothCruiser是一种用于熵正则化MDP和双人博弈的新规划算法，在给定环境生成模型的情况下，能以O~(1/ε⁴)的样本复杂度估计值函数，优于非正则化设置。


<details>
  <summary>Details</summary>
Motivation: 在非正则化马尔可夫决策过程和博弈中，目前没有已知算法能在最坏情况下保证多项式样本复杂度。熵正则化通过促进贝尔曼算子的平滑性，为设计具有理论保证的算法提供了机会。

Method: SmoothCruiser利用熵正则化带来的贝尔曼算子平滑性，设计了一种新的规划算法。该算法在给定环境生成模型的情况下，通过问题无关的样本复杂度来估计值函数。

Result: 算法实现了O~(1/ε⁴)的样本复杂度（其中ε为期望精度），这是首个在最坏情况下具有多项式样本复杂度保证的算法，优于非正则化设置中的现有方法。

Conclusion: 熵正则化通过促进贝尔曼算子的平滑性，使得设计具有理论保证的规划算法成为可能。SmoothCruiser展示了正则化在改善样本复杂度方面的优势，为非正则化设置提供了新的研究方向。
...
### [171] [On two ways to use determinantal point processes for Monte Carlo integration](https://arxiv.org/abs/2604.19698)
*Guillaume Gautier,Rémi Bardenet,Michal Valko*

Main category: cs.LG

TL;DR: 论文研究基于确定性点过程(DPP)的蒙特卡洛积分估计器，比较了两种现有DPP方法：一种固定DPP具有超线性收敛率，另一种针对特定函数的DPP具有与标准MC相同的收敛率。


<details>
  <summary>Details</summary>
Motivation: 标准蒙特卡洛估计器使用独立样本，方差为O(1/N)。使用具有排斥性质的确定性点过程(DPP)作为样本可以改进估计器性能，但现有DPP方法各有优缺点：一种收敛更快但DPP固定，另一种无偏但需要针对特定函数定制DPP。

Method: 重新审视两种现有DPP估计器：1) Bardenet & Hardy (2020)方法使用固定DPP，对光滑函数f达到O(N^{-(1+1/d)})收敛率；2) Ermakov & Zolotukhin (1960)方法使用针对f定制的DPP，无偏且具有O(1/N)收敛率。将这两种方法推广到连续设置，并提供采样算法。

Result: 论文系统分析比较了两种DPP基估计器的理论性质：固定DPP方法在光滑函数条件下具有超线性收敛优势，而定制DPP方法保持无偏性且收敛率与标准MC相同。提供了连续设置的推广和实际采样算法。

Conclusion: DPP基蒙特卡洛估计器提供了标准独立采样之外的替代方案，在特定条件下可以改进收敛性能。两种方法各有适用场景：固定DPP适合光滑函数且能获得更快收敛，定制DPP保持无偏性但需要针对特定函数设计。论文为实际应用提供了理论框架和算法实现。
...
### [172] [Ultrametric OGP - parametric RDT \emph{symmetric} binary perceptron connection](https://arxiv.org/abs/2604.19712)
*Mihailo Stojnic*

Main category: cs.LG

TL;DR: 本文建立了参数化RDT与重叠间隙性质(OGPs)的理论联系，通过分析s级超度量OGPs，获得了约束密度的严格上界，发现其与参数化RDT估计值高度一致，并提出了关于算法阈值和两种方法等价性的猜想。


<details>
  <summary>Details</summary>
Motivation: 先前研究提出了fl-RDT框架来刻画统计计算间隙(SCGs)，并在对称二元感知器(SBPs)中获得了算法阈值估计。本文旨在进一步连接参数化RDT与解空间的另一个关键几何特征——重叠间隙性质(OGPs)，探索两者之间的理论联系。

Method: 1. 考虑s级超度量OGPs(ult_s-OGPs)并严格上界相关的约束密度α_ult_s
2. 开发包含组合和概率分量的解析联合边界程序
3. 将组合部分转化为凸问题，概率部分转化为嵌套积分
4. 进行数值评估，获得前两个层级的最紧边界

Result: 1. 获得ult_1和ult_2层级的最紧边界：ᾱ_ult_1 ≈ 1.6578，ᾱ_ult_2 ≈ 1.6219
2. 这些边界与第3和第4层级参数化RDT估计值高度一致：α_c^(3) ≈ 1.6576，α_c^(4) ≈ 1.6218
3. 在其他关键参数(包括重叠值和超度量簇的相对大小)上也观察到极好的一致性

Conclusion: 基于观察结果提出了几个连接ult-OGP和参数化RDT的猜想：算法阈值α_a等于两个极限的极限值，且α_ult_s ≤ α_c^(s+2)(可能在某些或所有s上相等)。最后讨论了ult-OGP和参数化RDT所有关键参数之间可能存在完全同构关系。
...
### [173] [Adaptive MSD-Splitting: Enhancing C4.5 and Random Forests for Skewed Continuous Attributes](https://arxiv.org/abs/2604.19722)
*Jake Lee*

Main category: cs.LG

TL;DR: AMSD方法通过动态调整标准差乘数来改进MSD-Splitting技术，解决了在偏斜分布数据中的信息丢失问题，并在集成学习中实现了更高的准确性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 连续数值属性的离散化是决策树归纳中的计算瓶颈。虽然MSD-Splitting技术使用均值和标准差分箱提高了C4.5算法的效率和准确性，但其固定的标准差截断方法在处理高度偏斜数据（如生物医学和金融数据集）时会导致灾难性的信息丢失。

Method: 提出了自适应MSD-Splitting（AMSD）方法，根据特征偏度动态调整标准差乘数，在密集区域缩小间隔以保持分辨能力。还将AMSD集成到集成方法中，提出了Random Forest-AMSD（RF-AMSD）框架。

Result: 在Census Income、Heart Disease、Breast Cancer和Forest Covertype数据集上的实验表明，AMSD比标准MSD-Splitting提高了2-4%的准确率，同时保持了与O(N log N)穷举搜索相比近似的O(N)时间复杂度降低。RF-AMSD框架以标准计算成本的一小部分实现了最先进的准确率。

Conclusion: 自适应统计分箱在大规模集成学习架构中是可行的，AMSD方法在保持计算效率的同时显著提高了处理偏斜分布数据的准确性。
...
### [174] [Benign Overfitting in Adversarial Training for Vision Transformers](https://arxiv.org/abs/2604.19724)
*Jiaming Zhang,Meng Ding,Shaopeng Fu,Jingfeng Zhang,Di Wang*

Main category: cs.LG

TL;DR: 本文首次对简化版Vision Transformers的对抗训练进行了理论分析，证明了在特定条件下对抗训练能使ViTs实现近乎零的鲁棒训练损失和泛化误差，甚至在过拟合情况下也能保持强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管Vision Transformers在各种视觉任务中取得了显著成功，但研究表明它们与CNN一样容易受到对抗样本的攻击。对抗训练是常见的防御策略，但其在ViTs中的鲁棒性理论基础尚未得到充分探索。

Method: 对简化版ViT架构进行理论分析，研究在满足特定信噪比条件和适度扰动预算下，对抗训练如何影响ViTs的性能。通过合成和真实世界数据集进行实验验证。

Result: 理论分析表明，在特定条件下，对抗训练能使ViTs实现近乎零的鲁棒训练损失和泛化误差。即使在过拟合情况下也能保持强泛化能力，这种现象被称为"良性过拟合"，之前仅在CNN中观察到。

Conclusion: 本文首次为ViTs的对抗训练提供了理论支撑，证明了在适当条件下对抗训练能够有效提升ViTs的鲁棒性，甚至在过拟合情况下也能实现良好的泛化性能，为ViTs的鲁棒性研究提供了新的理论视角。
...
### [175] [FB-NLL: A Feature-Based Approach to Tackle Noisy Labels in Personalized Federated Learning](https://arxiv.org/abs/2604.19729)
*Abdulmoneam Ali,Ahmed Arafa*

Main category: cs.LG

TL;DR: FB-NLL：一种基于特征而非学习动态的个性化联邦学习框架，通过谱结构分析实现一次性用户聚类，并引入特征一致性策略处理噪声标签，显著降低通信开销并提升性能稳定性。
...
Method: FB-NLL采用特征中心框架：1）利用局部特征空间的谱结构表征用户，通过子空间相似性一次性识别任务一致的用户分组；2）引入基于特征一致性的检测和校正策略处理噪声标签，利用学习特征空间中的方向对齐和类特定特征子空间分配标签。

Result: 在多种数据集和噪声机制下的实验表明，FB-NLL在平均准确率和性能稳定性方面持续优于现有最先进的基线方法，同时显著降低了通信开销和计算成本。

Conclusion: FB-NLL通过解耦用户聚类与训练动态，提供了一种对噪声鲁棒的个性化联邦学习框架，具有模型无关性且能与现有噪声鲁棒训练技术无缝集成，为异构数据分布下的多任务学习提供了有效解决方案。
...
### [176] [FASTER: Value-Guided Sampling for Fast RL](https://arxiv.org/abs/2604.19730)
*Perry Dong,Alexander Swerdlow,Dorsa Sadigh,Chelsea Finn*

Main category: cs.LG

TL;DR: FASTER是一种轻量级方法，通过将扩散策略的多候选动作采样建模为MDP，在去噪过程中早期预测并过滤动作候选，从而获得采样带来的性能提升但避免计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前基于采样的强化学习方法（如扩散策略）在测试时需要采样多个动作候选并选择最佳，计算成本过高。需要一种方法既能获得采样带来的性能提升，又能避免高昂的计算开销。

Method: 将扩散策略中多个动作候选的去噪和选择过程建模为马尔可夫决策过程（MDP），目标是在去噪完成前逐步过滤动作候选。通过学习去噪空间中的策略和价值函数，预测动作候选的下游价值并进行过滤，同时最大化回报。

Result: 在具有挑战性的长时程操作任务中，FASTER持续改进基础策略，在在线和批量在线强化学习中均取得最佳性能。应用于预训练的VLA时，FASTER在保持相同性能的同时显著降低了训练和推理的计算需求。

Conclusion: FASTER提供了一种轻量级方法，可以在不增加计算成本的情况下获得基于采样的测试时扩展带来的性能提升，可轻松集成到现有的生成式强化学习算法中。
...
### [177] [Safe Continual Reinforcement Learning in Non-stationary Environments](https://arxiv.org/abs/2604.19737)
*Austin Coursey,Abel Diaz-Gonzalez,Marcos Quinones-Grueiro,Gautam Biswas*

Main category: cs.LG

TL;DR: 该论文研究安全持续强化学习，探讨在非平稳环境中如何同时保证安全约束和防止灾难性遗忘，提出了三个基准环境并评估现有方法，发现安全与持续适应之间存在根本性矛盾。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习控制器假设环境平稳，但在实际非平稳部署中系统动态和操作条件会意外变化，同时RL控制器必须在学习和执行阶段始终满足安全约束。虽然持续RL和安全RL分别解决了非平稳性和安全性问题，但两者的交叉领域仍相对未被探索，因此需要研究能够适应系统生命周期变化同时保持安全的安全持续RL算法。

Method: 1. 引入三个基准环境来捕捉安全关键的持续适应场景；2. 评估来自安全RL、持续RL及其组合的代表性方法；3. 研究基于正则化的策略来缓解安全约束与灾难性遗忘之间的权衡；4. 分析这些策略的优缺点和局限性。

Result: 实证结果表明，在非平稳动态下，维持安全约束和防止灾难性遗忘之间存在根本性矛盾，现有方法通常无法同时实现这两个目标。基于正则化的策略部分缓解了这种权衡，但仍有局限性。

Conclusion: 需要开发能够在变化环境中持续自主运行的安全、有弹性的学习型控制器，论文概述了实现这一目标的关键开放挑战和研究方向，强调了安全持续RL领域的重要性和未解决的问题。
